...
if (! empty($action) && $action == 'fetch' && ! empty($id))
{
	...
	if ($ret > 0) {
		...
		// Multiprice
		if (! $found && isset($price_level) && $price_level >= 1 && (! empty($conf->global->PRODUIT_MULTIPRICES)))
		{
	...
	if (empty($mode) || $mode == 1) {
		$arrayresult = $formv->select_produits_list_v("", $htmlname, $type, "", $price_level, $searchkey, $status, 2, $outjson, $socid);
	} elseif ($mode == 2) {
		$arrayresult = $formv->select_produits_fournisseurs_list($socid, "", $htmlname, $type, "", $searchkey, $status, $outjson, $socid);
	}
