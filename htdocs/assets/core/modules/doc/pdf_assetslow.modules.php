...
class pdf_assetslow extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->assets->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->assets->dir_output."/".$period_year.'/assetslow';
				if (empty($object->depreciation))
					$file = $dir . '/assetslow.pdf';
				else
					$file = $dir . '/assetslow_'.$object->depreciation.'.pdf';
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					if($resCG > 0){
						$pdf->MultiCell($this->posxfin-$this->posxini-1, 1, $objCgroup->label, 0, 'L',1);
					}else{
						$pdf->MultiCell($this->posxfin-$this->posxini-1, 1, $j, 0, 'L',1);
					}
					...
					foreach ($line as $cg => $value)
					{
						...
						$pdf->SetXY($this->posxini, $curY);
						$pdf->MultiCell($this->posxrefe-$this->posxini-1, 1, $value['ref'], 0, 'L',0);
						$aPosY[] = $pdf->GetY();

						$pdf->SetXY($this->posxrefe, $curY);
						$pdf->MultiCell($this->posxrefx-$this->posxrefe-1, 1, $value['ref_ext'], 0, 'L',0);
						$aPosY[] = $pdf->GetY();

						$pdf->SetXY($this->posxrefx, $curY);
						$pdf->MultiCell($this->posxdesc-$this->posxrefx-1, 1, $value['descrip'], 0, 'L',0);
						$aPosY[] =$pdf->GetY();
						$pdf->SetXY($this->posxdesc, $curY);
						$pdf->MultiCell($this->posxfech-$this->posxdesc-1, 1, dol_print_date($value['date_adq'],'day'), 0, 'C',0);

						$pdf->SetXY($this->posxfech, $curY);
						$pdf->MultiCell($this->posxvalo-$this->posxfech-1, 1, dol_print_date($value['date_reval'],'day'), 0, 'C',0);

						$pdf->SetXY($this->posxvalo, $curY);
						$pdf->MultiCell($this->posxcost-$this->posxvalo-1, 1, price(price2num($value['coste'],'MT')), 0, 'R',0);

						$pdf->SetXY($this->posxcost, $curY);
						$pdf->MultiCell($this->posxutil-$this->posxcost-1, 1, $value['useful_life'], 0, 'C',0);


						$pdf->SetXY($this->posxutil, $curY);
						$pdf->MultiCell($this->posxfin-$this->posxutil-1, 1, dol_print_date($value['dateoff'],'day'), 0, 'L',0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxrefx-$this->posxrefe-1,2, $outputlangs->transnoentities("Refext"),'','C');
			$pdf->line($this->posxrefx-1, $tab_top, $this->posxrefx-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxrefx-1, $tab_top+2);
			$pdf->MultiCell($this->posxdesc-$this->posxrefx-1,2, $outputlangs->transnoentities("Description"),'','C');
			$pdf->line($this->posxdesc-1, $tab_top, $this->posxdesc-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxdesc-1, $tab_top+2);
			$pdf->MultiCell($this->posxfech-$this->posxdesc-1,2, $outputlangs->transnoentities("Acquisitiondate"),'','C');
			$pdf->line($this->posxfech-1, $tab_top, $this->posxfech-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxfech-1, $tab_top+2);
			$pdf->MultiCell($this->posxvalo-$this->posxfech-1,2, $outputlangs->transnoentities("Revaluationdate"),'','C');
			$pdf->line($this->posxvalo-1, $tab_top, $this->posxvalo-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxvalo-1, $tab_top+2);
			$pdf->MultiCell($this->posxcost-$this->posxvalo-1,2, $outputlangs->transnoentities("Coste"),'','C');
			$pdf->line($this->posxcost-1, $tab_top, $this->posxcost-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxcost-1, $tab_top+2);
			$pdf->MultiCell($this->posxutil-$this->posxcost-1,2, $outputlangs->transnoentities("Usefullife"),'','C');
			$pdf->line($this->posxutil-1, $tab_top, $this->posxutil-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxutil-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxutil-1,2, $outputlangs->transnoentities("Dateoff"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$titulo = "Lista de activos, prontos a ser dado de baja";
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("ACtivos de baja en ")." : " . $aReporte[2], '', 'R');
