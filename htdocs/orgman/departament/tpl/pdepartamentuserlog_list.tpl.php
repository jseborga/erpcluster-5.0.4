...
$search_fk_departament_user=GETPOST('search_fk_departament_user','int');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmandepartamentuserloglist';
...
't.fk_departament_user'=>array('label'=>$langs->trans("Fieldfk_departament_user"), 'checked'=>1),
...
$object=new Pdepartamentuserlog($db);
...
$sql.= " FROM ".MAIN_DB_PREFIX."p_departament_user_log as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_departament_user_log_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
$sql.= " AND t.fk_member = ". $id;
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";
//echo $sql;
if ($search_fk_departament_user) $sql.= natural_search("fk_departament_user",$search_fk_departament_user);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/pdepartamentuserlog/card.php?id='.$id);
	exit;
}
...
//
if (! empty($arrayfields['t.fk_departament_user']['checked'])) print_liste_field_titre($arrayfields['t.fk_departament_user']['label'],$_SERVER['PHP_SELF'],'t.fk_departament_user','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
//
if (! empty($arrayfields['t.fk_departament_user']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_departament_user" value="'.$search_fk_departament_user.'" size="10"></td>';
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				$aKey = explode('.',$key);
				$key2 = $aKey[1];
				if ($key2 == 'fk_departament_user')
				{
					$objDepar->fetch($obj->$key2);
					$obj->$key2 = $objDepar->getNomUrl();
				}
				if ($key2 == 'ref')
				{
					$object->id = $obj->rowid;
