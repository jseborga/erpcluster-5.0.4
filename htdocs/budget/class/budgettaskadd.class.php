...
/**
 * \file    budget/budgettaskadd.class.php
 * \ingroup budget
 ...
 * @see CommonObject
 */
class Budgettaskadd extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'budgettaskadd';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'budget_task_add';

	/**
	 * @var BudgettaskaddLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->complementary)) {
			 $this->complementary = trim($this->complementary);
		}
		if (isset($this->unit_budget)) {
			 $this->unit_budget = trim($this->unit_budget);
		}
		...
		if (isset($this->order_ref)) {
			 $this->order_ref = trim($this->order_ref);
		}
		...
		$sql .= ' '.(! isset($this->complementary)?'NULL':"'".$this->db->escape($this->complementary)."'").',';
		$sql .= ' '.(! isset($this->unit_budget)?'NULL':"'".$this->unit_budget."'").',';
		...
		$sql .= ' '.(! isset($this->order_ref)?'NULL':$this->order_ref).',';
	 ...
	 */
	public function fetch($id, $fk_budget_task=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("budgettaskadd", 1) . ")";
		}
		if (count($sqlwhere) > 0) {
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' complementary = '.(isset($this->complementary)?"'".$this->db->escape($this->complementary)."'":"null").',';
		$sql .= ' unit_budget = '.(isset($this->unit_budget)?$this->unit_budget:"null").',';
		$sql .= ' unit_amount = '.(isset($this->unit_amount)?$this->unit_amount:"null").',';
		$sql .= ' total_amount = '.(isset($this->total_amount)?$this->total_amount:"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Budgettaskadd($this->db);
