...
if ($action == 'create' && $user->rights->mant->jobs->crear)
{
  ...
  print "<form action=\"fichesend.php\" method=\"post\">\n";
...
else
{
  if ($_GET["id"])
      ...
      if ($action <> 'edit' && $action <> 're-edit')
	{
	  ...
	  if ($action == '')
	    {
	      if ($user->rights->mant->jobs->mod && $object->statut == 0)
...
function htmlemaillink($id)
{
  global $object,$langs,$objCharge,$objDepartament;
  $html = '<!DOCTYPE HTML>';
  $html.= '<html>';
  $html.= '<head>';
  $html.= '<link rel="stylesheet" media="screen" href="http://192.168.1.105/bcb/mant/css/style-email.css">';
  $html.= '</head>';
  $html.= '<body>';  
  $html.= '<p>Su numero de ticket es '.$object->ref.'</p>';
  $html.= '<p>Favor hacer click a la siguiente direccion para completar la informacion: http://192.168.1.105/bcb/mant/jobs/fichereg.php?id='.$id.'</p>';
   $html.= '</body>';
   $html.= '</html>';
  return $html;
}
