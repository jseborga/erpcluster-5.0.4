...
/**
 *   	\file       clientes/coccupation_list.php
 *		\ingroup    clientes
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-06-22 11:40
 */
...
dol_include_once('/orgman/class/coccupation.class.php');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'occupationlist';

// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array
$hookmanager->initHooks(array('occupationlist'));
...
if (empty($reshook))
{
	...
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/occupation/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/occupation/list.php',1));
			exit;
		}
		else
		{
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_occupation as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_occupation_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/coccupation/card.php?id='.$id);
	exit;
}
...
if ($action == 'create')
{
	...
	print '<input class="butAction" type="submit" name="submit" value="'."Cancelar".'">';
	print '</td>';
	print '</tr>';
}
/* endCodLaiwett */
...
if ($action == 'delete') {
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteOccupation'), $langs->trans('ConfirmDeleteOccupation').' : '.$object->ref.' '.$object->label, 'confirm_delete', '', 0, 1);
	print $formconfirm;
}
...
while ($i < min($num, $limit))
{
	...
	{
		$var = !$var;
		/* Liet -> aqui ya pongo el codigo de editar */
		$object->id = $obj->rowid;
		$object->ref = $obj->ref;
		$object->label = $obj->label;
		/* Liet -> va  mostrando los cambios */
		...
		else
		{
			...
			if (! empty($arrayfields['t.ref']['checked']))
			{
				print '<td>'.$object->getNomUrl(0,'',0,24,'',($user->rights->orgman->occ->write?'&action=edit':'')).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked']))
			{
...
if (empty($reshook))
{
	if ($user->rights->orgman->occ->write)
	{
