...
dol_include_once('/orgman/class/csources.class.php');
...
if (!$user->rights->orgman->source->lire) accessforbidden();
...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->orgman->source->write)
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/sources/list.php',1);
	...
	if ($action == 'update' && ! GETPOST('cancel')  && $user->rights->orgman->source->write)
	{
		$error=0;
	...
	// Action to delete
	if ($action == 'confirm_delete'  && $user->rights->orgman->source->del)
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/sources/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Foundingsource');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_sources as t";
...
if ($resql)
{
	...
	print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $params, $sortfield, $sortorder, '', $num, $nbtotalofrecords, DOL_URL_ROOT.'/orgman/img/foundingsource', 1, '', '', $limit);
	...
	if (empty($reshook))
	{
		if ($user->rights->orgman->source->write)
		{
