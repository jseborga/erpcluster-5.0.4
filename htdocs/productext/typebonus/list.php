...
/**
 *   	\file       productext/cbonustype_list.php
 *		\ingroup    productext
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'productextlist';
...
$search_status='';
	...
	//$permtoread = $user->rights->cbonustype->read;
	//$permtodelete = $user->rights->cbonustype->delete;
	//$uploaddir = $conf->cbonustype->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_bonus_type as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_bonus_type_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/cbonustype/card.php?id='.$id);
	exit;
}
