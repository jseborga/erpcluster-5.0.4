...
/**
 *   	\file       contab/contabaccounting_list.php
 *		\ingroup    contab
...
$hookmanager->initHooks(array('contabaccountinglist'));
...
't.cta_top'=>array('label'=>$langs->trans("Fieldcta_top"), 'checked'=>1),
't.cta_name'=>array('label'=>$langs->trans("Fieldcta_name"), 'checked'=>1),
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."contab_accounting_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_cta_top) $sql.= natural_search("cta_top",$search_cta_top);
if ($search_cta_name) $sql.= natural_search("cta_name",$search_cta_name);
...
if ($search_cta_top != '') $params.= '&amp;search_cta_top='.urlencode($search_cta_top);
if ($search_cta_name != '') $params.= '&amp;search_cta_name='.urlencode($search_cta_name);
...
if (! empty($arrayfields['t.cta_top']['checked'])) print_liste_field_titre($arrayfields['t.cta_top']['label'],$_SERVER['PHP_SELF'],'t.cta_top','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.cta_name']['checked'])) print_liste_field_titre($arrayfields['t.cta_name']['label'],$_SERVER['PHP_SELF'],'t.cta_name','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.cta_top']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_cta_top" value="'.$search_cta_top.'" size="10"></td>';
if (! empty($arrayfields['t.cta_name']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_cta_name" value="'.$search_cta_name.'" size="10"></td>';
