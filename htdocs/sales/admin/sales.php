...
/**
 *	\file       htdocs/sales/admin/sales.php
 *	\ingroup    Almacen
...
if ($action == 'setdoc')
{
	...
	if (dolibarr_set_const($db, "SALES_ADDON_PDF",$value,'chaine',0,'',$conf->entity))
	{
		$conf->global->SALES_ADDON_PDF = $value;
	}
...
if ($action == 'setmod')
{
  ...
  // par appel methode canBeActivated
	...
	dolibarr_set_const($db, "SALES_ADDON",$value,'chaine',0,'',$conf->entity);
}

if ($action == 'set_SALES_DRAFT_WATERMARK')
{
	$draft = GETPOST("SALES_DRAFT_WATERMARK");
	$res = dolibarr_set_const($db, "SALES_DRAFT_WATERMARK",trim($draft),'chaine',0,'',$conf->entity);
...
if ($action == 'set_SALES_FREE_TEXT')
{
    $freetext = GETPOST("SALES_FREE_TEXT");	// No alpha here, we want exact string

    $res = dolibarr_set_const($db, "SALES_FREE_TEXT",$freetext,'chaine',0,'',$conf->entity);
...
llxHeader("",$langs->trans("SalesSetup"));

$form=new Form($db);

$linkback='<a href="'.DOL_URL_ROOT.'/admin/modules.php">'.$langs->trans("BackToModuleList").'</a>';
print_fiche_titre($langs->trans("SalesSetup"),$linkback,'setup');
print '<br>';
...
print_titre($langs->trans("SalesNumberingModules"));
...
foreach ($dirmodels as $reldir)
{
	...
	if (is_dir($dir))
	{
		...
		if (is_resource($handle))
		{
			...
			{
				if (substr($file, 0, 10) == 'mod_sales_' && substr($file, dol_strlen($file)-3, 3) == 'php')
				{
					$file = substr($file, 0, dol_strlen($file)-4);

					require_once DOL_DOCUMENT_ROOT ."/sales/core/modules/".$file.'.php';
 ...
 */
print_titre($langs->trans("SalesModelModule"));
...
foreach ($dirmodels as $reldir)
{
	...
	{
		$dir = dol_buildpath($reldir."sales/core/modules".$valdir);
		...
		if (is_dir($dir))
		{
			...
			if (is_resource($handle))
			{
				...
				foreach($filelist as $file)
				{
			      ...
			      // Defaut
								print '<td align="center">';
								if ($conf->global->SALES_ADDON_PDF == $name)
								{
...
print '<input type="hidden" name="action" value="set_SALES_FREE_TEXT">';
...
print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
print "<input type=\"hidden\" name=\"action\" value=\"set_SALES_DRAFT_WATERMARK\">";
print '<tr '.$bc[$var].'><td colspan="2">';
print $langs->trans("WatermarkOnDraftFacture").'<br>';
