 ...
 *					Initialy built by build_class_from_table on 2016-10-28 08:31
 */
...
require_once DOL_DOCUMENT_ROOT.'/budget/lib/verifcontact.lib.php';
...
$arrayfields=array(
	...
	't.fk_user_creat'=>array('label'=>$langs->trans("Fieldfk_user_creat"), 'checked'=>0),
	't.public'=>array('label'=>$langs->trans("Fieldpublic"), 'checked'=>0),
	't.fk_statut'=>array('label'=>$langs->trans("Fieldfk_statut"), 'checked'=>1),
	't.fk_opp_status'=>array('label'=>$langs->trans("Fieldfk_opp_status"), 'checked'=>0),
	't.opp_percent'=>array('label'=>$langs->trans("Fieldopp_percent"), 'checked'=>0),
	't.fk_user_close'=>array('label'=>$langs->trans("Fieldfk_user_close"), 'checked'=>0),
	...
	't.opp_amount'=>array('label'=>$langs->trans("Fieldopp_amount"), 'checked'=>0),
...
if (! $user->rights->budget->all->lire) $budgetListId = $objectstatic->getBudgetAuthorizedForUser($user,0,1,$socid);
...
$sql.= " WHERE 1 = 1";
if (! $user->rights->budget->bud->readall)
	$sql.= " AND t.rowid IN (".$budgetListId.")";
...
if ($resql)
{
	...
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			if (! empty($arrayfields['t.type_structure']['checked']))
			{
				$typestr->fetch(0,$obj->type_structure);
				if ($typestr->code == $obj->type_structure)
					print '<td>'.$typestr->getNomUrl(1).'</td>';
				else
					print '<td>'.$obj->type_structure.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			...
			if (! empty($arrayfields['t.version']['checked']))
			{
			...
			if (! empty($arrayfields['t.public']['checked']))
			{
				print '<td>'.$obj->public.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			...
			if (! empty($arrayfields['t.opp_status']['checked']))
			{
				print '<td>'.$obj->opp_status.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.opp_percent']['checked']))
			{
				print '<td>'.$obj->opp_percent.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.fk_user_close']['checked']))
			{
				print '<td>'.$obj->fk_user_close.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			...
			if (! empty($arrayfields['t.opp_amount']['checked']))
			{
				print '<td>'.$obj->opp_amount.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.budget_amount']['checked']))
			{
				print '<td>'.$obj->budget_amount.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.model_pdf']['checked']))
			{
				print '<td>'.$obj->model_pdf.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.datec']['checked']))
			{
