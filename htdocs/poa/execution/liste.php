...
//eliminando la lista de preventivos segun el inicio de proceso
if (!isset($_SESSION['aListip']))
  $_SESSION['aListip'] = array();
if (!isset($_SESSION['aListco']))
  $_SESSION['aListco'] = array();
...
$sql = "SELECT p.rowid AS id, p.gestion, p.fk_pac, p.label, p.pseudonym, s.fk_father, p.nro_preventive, p.date_preventive, p.statut, p.active, p.fk_user_create, ";
$sql.= " p.amount, p.priority, p.date_create, p.fk_user_create, s.fk_prev_ant,";
$sql.= " u.login, u.firstname, u.lastname ";
if ($idp)
  $sql.= ", r.partida, r.amount AS amountpartida ";
$sql.= " FROM ".MAIN_DB_PREFIX."poa_prev as p ";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."poa_prev_seg AS s ON p.rowid = s.fk_prev";
if ($idp)
...
if ($result)
  {
    ...
    print_barre_liste($langs->trans("Preventives"), $page, "liste.php", "", $sortfield, $sortorder,'',$num);
...
<?php
    ...
    print '<td nowrap class="liste_titre"><input type="text" class="flat" name="search_label" value="'.$search_label.'" size="40">';
    print '</td>';
	...
	while ($i < min($num,$limit))
	  {	      
	    ...
	    $_SESSION['aLisprev'] = prev_ant($obj->id,$_SESSION['aLisprev']);
		...
		$lFather = true;
		$lContinue = false;
		list($cClass,$lContinue) = filterres($aDays,$aColors,$filterw,$daydelay,$cClass,$nDiamax,$workstatut);
		list($cClassall,$lCont) = filterres($aDaysall,$aColorsall,$filterx,$daydelayall,$cClass,$nDiamaxall,$workstatut);
	      }
	    elseif ($filterx)
	      {
		$lContinue = false;
		list($cClass,$lCont) = filterres($aDays,$aColors,$filterw,$daydelay,$cClass,$nDiamax);
		list($cClassall,$lContinue) = filterres($aDaysall,$aColorsall,$filterx,$daydelayall,$cClass,$nDiamaxall);
	      }
	    elseif ($filters)
	      {
		$lContinue = false;
		$cClass = '';
		$cClassall = '';
		if ($obj->statut == substr($filters,1,4))
		  $lContinue = true;
		list($cClass,$lCont) = filterres($aDays,$aColors,$filterw,$daydelay,$cClass,$nDiamax);
		list($cClassall,$lCont) = filterres($aDaysall,$aColorsall,$filterx,$daydelayall,$cClass,$nDiamaxall);
	      }
	    elseif ($filtery)
	      {
		$lContinue = false;
		...
		list($cClass,$lContinue) = filterres($aDays,$aColors,'',$daydelay,$cClass,$nDiamax);
		//para totales
		list($cClassall,$lContinue) = filterres($aDaysall,$aColorsall,'',$daydelayall,$cClass,$nDiamaxall);
		$lContinue = $lContinueAnt;
	      }
	    ...
	    // if ( $filterx && $daydelayall > $nDaymaxall) $cClass = 'ffaaaa';
		...
		print '<td nowrap>'.'<a href="fiche.php?id='.$obj->id.'&dol_hide_leftmenu=1" title="'.$langs->trans('Create for').' '.$nombre.'">'.img_picto($langs->trans("Preventive"),DOL_URL_ROOT.'/poa/img/prev.png','',1).'&nbsp;'.$obj->nro_preventive.'</a>'.'</td>';
		$aHtml[$i]['nro'] = $obj->nro_preventive;	
		...
		if (empty($obj->fk_father))
		  {
		    if ($user->rights->poa->prev->mod || $user->admin)
		      {
		...
		print '</td>';
		print '<td>'.dol_print_date($obj->date_preventive,'day').'</td>';
		$aHtml[$i]['date_preventive'] = $obj->date_preventive;
		if ($idp)
		...
		print $object->LibStatut($obj->statut,0,1).'</td>';
		...
		if (empty($obj->fk_father))
		  {
		    ...
		    $objact->fetch('',$obj->id);
		    if ($objact->fk_prev == $obj->id)
		      {
			//ficheprev
			print '<a href="'.DOL_URL_ROOT.'/poa/execution/ficheprev.php?ida='.$objact->id.'&dol_hide_leftmenu=1" title="'.$langs->trans('Activity').'">'.img_picto($langs->trans("Activity"),DOL_URL_ROOT.'/poa/img/activ.png','',1).'</a>';
		      }
		    ...
		    // 	if ($objproc->id>0)
		    // 	  {
		    // 	    $objproccont->getlist_contrat($objproc->id);
		    // 	    $_SESSION['aListip'][$objproc->id] = $obj->id;
		    // 	    //$_SESSION['aLisprev'][$obj->id]['idProcesant'] = $objproc->id;
		    // 	    if (count($objproccont->aContrat)>0)
		    // 	      foreach ((array) $objproccont->aContrat AS $fk_contrat => $fkcontrat)
		    // 		{
		    // 		  $_SESSION['aListco'][$objproc->id][$fk_contrat] = $fk_contrat;
		    // 		  //$_SESSION['aLisprev'][$obj->id]['contract'][$fk_contrat] = $fk_contrat;
		    // 		}
		    // 	  }
		    ...
		    //   {
		    // 	print '<a href="'.DOL_URL_ROOT.'/poa/process/fiche_pas1.php?id='.$objproc->id.(isset($_GET['nopac'])?'&nopac=1&idp='.$idp:'&idp='.$obj->id).'">'.img_picto($langs->trans("Contract"),DOL_URL_ROOT.'/poa/img/comp.png','',1).'</a>';
		    // 	//payment
		    // 	if ($obj->statut >=2)
		    // 	  {
		    // 	    print '&nbsp;';
		    // 	    print '&nbsp;';
		    // 	    print '<a href="'.DOL_URL_ROOT.'/poa/process/fiche_pas2.php?id='.$objproc->id.'&clid=1'.(isset($_GET['nopac'])?'&nopac=1&idp='.$idp:'&idp='.$obj->id).'">'.img_picto($langs->trans("Payments"),DOL_URL_ROOT.'/poa/img/deve.png','',1).'</a>';
		    // 	  }
		    //   }
		    // else
		    //   print '&nbsp;';
