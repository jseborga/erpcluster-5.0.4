...
$arrayfields=array(
	...
	't.fk_budget'=>array('label'=>$langs->trans("Budget"), 'checked'=>1),
	...
	't.ejecution'=>array('label'=>$langs->trans("Ejecution"), 'checked'=>1),
...
if ($search_ejecution) $sql.= natural_search("ejecution",STRTOUPPER($search_ejecution));
...
if ($resql)
{
	...
	if ($search_ejecution != '') $params.= '&amp;search_ejecution='.urlencode($search_ejecution);
    ...
    if (! empty($arrayfields['t.ejecution']['checked'])) print_liste_field_titre($arrayfields['t.ejecution']['label'],$_SERVER['PHP_SELF'],'t.ejecution','',$params,'',$sortfield,$sortorder);
    ...
    if (! empty($arrayfields['t.ejecution']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ejecution" value="'.$search_ejecution.'" size="10"></td>';
    ...
    /*if (! empty($arrayfields['u.status']['checked']))
    {
        // Status
        print '<td class="liste_titre" align="center">';
        print $form->selectarray('search_status', array('-1'=>'','0'=>$langs->trans('Disabled'),'1'=>$langs->trans('Enabled')),$search_status);
        print '</td>';
    }*/
    // Action column
    ...
    while ($i < min($num, $limit))
    {
    	...
    	if ($obj)
    	{
            ...
            {
            	$budget->fetch($obj->fk_budget);
            	if ($budget->id == $obj->fk_budget)
            ...
            {
            	$objcat->fetch($obj->fk_categorie);
            	if ($objcat->id == $obj->fk_categorie)
	                print '<td>'.$objcat->getNomUrl(1).'</td>';
	            else
	            	print '<td>&nbsp;</td>';
    		    if (! $i) $totalarray['nbfield']++;
            }
            ...
            if (! empty($arrayfields['t.ejecution']['checked']))
            {
                print '<td align="center">'.$obj->ejecution.'</td>';
    		    if (! $i) $totalarray['nbfield']++;
            }
            ...
            if (! empty($arrayfields['u.status']['checked']))
            {
              ...
              print '<td align="center">'.$userstatic->getLibstatus(3).'</td>';
          }*/
