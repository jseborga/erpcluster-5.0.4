...
/**
 *   	\file       contab/contabtransaction_list.php
 *		\ingroup    contab
...
$search_type_seat=GETPOST('search_type_seat','alpha');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'contabtransactionlist';
...
$arrayfields=array(
	...
	't.bits'=>array('label'=>$langs->trans("Fieldbits"), 'checked'=>1),
...
$aActive = array(0=>$langs->trans('Inactive'),1=>$langs->trans('Active'));
...
$help_url='';
$title = $langs->trans('Typetransaction');
...
$sql.= " FROM ".MAIN_DB_PREFIX."contab_transaction as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."contab_transaction_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_bits) $sql.= natural_search("bits",$search_bits);
if ($search_type_seat) $sql.= natural_search("type_seat",$search_type_seat);
if ($search_active != -1) $sql.= natural_search("active",$search_active);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/contabtransaction/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.bits']['checked'])) print_liste_field_titre($arrayfields['t.bits']['label'],$_SERVER['PHP_SELF'],'t.bits','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.bits']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_bits" value="'.$search_bits.'" size="10"></td>';
if (! empty($arrayfields['t.type_seat']['checked']))
{
	print '<td class="liste_titre">';
	print $form->selectarray('search_type_seat',$aTypeseat,$search_type_seat,1);
	print '</td>';
}
...
print '</form>'."\n";
	...
	if (empty($reshook))
	{
		if ($user->rights->contab->trans->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/contab/transaction/card.php?action=create">'.$langs->trans("New").'</a></div>'."\n";
		}
	}
	print '</div>'."\n";
