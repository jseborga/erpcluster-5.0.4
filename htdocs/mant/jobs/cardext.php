...
if ($action == 'addcontact' )
{
	...
	if ($result > 0)
	{
		...
		setEventMessages($objJobscontact->error,$objJobscontact->errors,'errors');
	$action = 'asignjobs';
}
...
if ($id||$ref)
{
	...
	if ($action <> 'edit' && $action <> 're-edit')
	{
		...
		dol_fiche_head($head, 'ext', $langs->trans("Jobs"), 0, 'mant');
		...
		print "</table>";
		dol_fiche_end();
		include_once DOL_DOCUMENT_ROOT.'/mant/jobs/tpl/contact_list.tpl.php';
		...
		//
	if (($action == 'edit' || $action == 're-edit') && $abc)
	{
