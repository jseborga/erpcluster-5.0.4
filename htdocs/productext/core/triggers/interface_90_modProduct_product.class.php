 ...
 */
class InterfaceProduct extends DolibarrTriggers
{
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		switch ($action) {
			...
			{
				if ($object->element == 'product' && $conf->global->PRODUCT_CODEPRODUCT_ADDON=='mod_codeproduct_fractal')
				{
					$partida = (GETPOST('partida')?GETPOST('partida'):GETPOST('options_partida'));
					$partida = $object->array_options['options_partida'];
					// Load object modCodeProduct
					$module=(! empty($conf->global->PRODUCT_CODEPRODUCT_ADDON)?$conf->global->PRODUCT_CODEPRODUCT_ADDON:'mod_codeproduct_leopard').'.php';
					if (substr($module, 0, 16) == 'mod_codeproduct_' && substr($module, -3) == 'php')
					{
						...
						if ($result > 0)
						{
							...
							$newcode = $modCodeProduct->getNextValue($object,$object->type);
							$object->ref = $newcode;
