...
/**
 *   	\file       multicurren/csindexescountry_list.php
 *		\ingroup    multicurren
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-01-24 13:49
 */
...
$search_date_ind=dol_mktime(12, 0, 0, GETPOST("date_indmonth"), GETPOST("date_indday"), GETPOST("date_indyear"));
...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield="t.date_ind, t.ref";
...
$hookmanager->initHooks(array('csindexescountrylist'));
$extrafields = new ExtraFields($db);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('multicurren');
...
$arrayfields=array(
	...
	'cs.label'=>array('label'=>$langs->trans("Fieldlabel"), 'checked'=>1),
	't.date_ind'=>array('label'=>$langs->trans("Fielddate"), 'checked'=>1),
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/multicurren/exchangerate/list.php',1));
			exit;
		}
		else
		{
		...
		// Action to update
	if ($action == 'update' && $user->rights->multicurren->exch->mod)
	{
		...
		{
			// Delete OK
			setEventMessages("Savesuccessfull", null, 'mesgs');
			//header("Location: ".dol_buildpath('/multicurren/exchanterate/list.php',1));
			//exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Exchangerate');
...
$sql.= " , cs.label, cs.registry, cs.order_currency ";
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."cs_indexes_country_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."cs_currency_type AS cs ON t.entity = cs.entity AND t.ref = cs.ref";
//$sql.= " WHERE 1 = 1";
$sql.= " WHERE t.entity IN (".getEntity('cs_indexes_country',1).")";

if ($search_entity) $sql.= natural_search("t.entity",$search_entity);
if ($search_ref) $sql.= natural_search("t.ref",$search_ref);
if ($search_label) $sql.= natural_search("cs.label",$search_label);
if ($search_date_ind)
{
	$aDate = dol_getdate($search_date_ind);
...
if ($search_fk_user_create) $sql.= natural_search("t.fk_user_create",$search_fk_user_create);
...
if ($resql)
{
	...
	$objcurrency = new Cscurrencytypeext($db);
	...
	if ($search_date_ind != '')
	{
		$params.= '&amp;search_date_ind='.urlencode($search_date_ind);
		$params.= '&amp;date_indmonth='.urlencode(GETPOST("date_indmonth"));
		$params.= '&amp;date_indday='.urlencode(GETPOST("date_indday"));
		$params.= '&amp;date_indyear='.urlencode(GETPOST("date_indyear"));
	}
	...
	if (! empty($arrayfields['cs.label']['checked'])) print_liste_field_titre($arrayfields['cs.label']['label'],$_SERVER['PHP_SELF'],'cs.label','',$params,'',$sortfield,$sortorder);
	if (! empty($arrayfields['t.date_ind']['checked'])) print_liste_field_titre($arrayfields['t.date_ind']['label'],$_SERVER['PHP_SELF'],'t.date_ind','',$params,'',$sortfield,$sortorder);
	...
	if (! empty($arrayfields['cs.label']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_label" value="'.$search_label.'" size="10"></td>';
	if (! empty($arrayfields['t.date_ind']['checked']))
	{
		print '<td class="liste_titre">';
		print $form->select_date($search_date_ind,'date_ind','','',1);
		print'</td>';
	}
	...
	print '</tr>'."\n";
	 	...
	 	{
	 		$form = new Form($db);
	 		$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Deleteexchangerate"),$langs->trans("Confirmdeleteexchangerate").' '.$object->ref.' '.$langs->trans('The').' '.dol_print_date($object->date_ind,'day'),"confirm_delete",'',0,2);
	...
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			if (! empty($arrayfields['cs.label']['checked']))
			{
			...
			if (! empty($arrayfields['t.date_ind']['checked']))
			{
				print '<td>'.dol_print_date($db->jdate($obj->date_ind),'day').'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.amount']['checked']))
			{
				if ($action == 'edit' && $id == $obj->rowid)
				{
					...
					print '<input type="number" min="0" step="any" name="amount" value="'.$obj->amount.'">';
					print '</td>';
				}
				else
				{
			...
			// Action column
			print '<td>';
			if ($user->rights->multicurren->exch->mod)
			{
				if ($action == 'edit' && $id == $obj->rowid)
					print '<input type="submit" name="submit" value="'.$langs->trans('Update').'">';
				elseif($action != 'edit')
					print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$obj->rowid.'&action=edit'.$params.'">'.img_picto('','edit').'</a>';
			}
			if ($user->rights->multicurren->exch->del)
			{
				if($action != 'edit')
					print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$obj->rowid.'&action=delete'.$params.'">'.img_picto('','delete').'</a>';
			}
			print '</td>';
			if (! $i) $totalarray['nbfield']++;
	...
	if ($user->rights->multicurren->exch->creer)
	...
	if ($user->rights->multicurren->exch->creer)
		print "<a class=\"butAction\" href=\"import.php?action=create\">".$langs->trans("Import")."</a>";
	else
		print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Import")."</a>";
	print "</div>";
	$db->free($result);
}
else
