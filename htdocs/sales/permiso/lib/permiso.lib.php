 ...
 */
function permiso_prepare_head($object)
{
	global $langs, $conf;
	...
	$head[$h][0] = DOL_URL_ROOT.'/ventas/permiso/fiche.php?id='.$object->id;
	$head[$h][1] = $langs->trans("PermissionIP");
	$head[$h][2] = 'card';
	$h++;
	...
	*/
    ...
    complete_head_from_modules($conf,$langs,$object,$head,$h,'cajaChica');

    // $head[$h][0] = DOL_URL_ROOT.'/cajachica/info.php?id='.$object->id;
    // 	$head[$h][1] = $langs->trans("Info");
    // 	$head[$h][2] = 'info';
    // 	$h++;

    complete_head_from_modules($conf,$langs,$object,$head,$h,'permiso','remove');
