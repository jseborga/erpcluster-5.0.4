...
/**
 * \file    orgman/ccountry.class.php
 * \ingroup orgman
...
class Ccountry extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->favorite)) {
			 $this->favorite = trim($this->favorite);
		}
		...
		$sql .= ' '.(! isset($this->favorite)?'NULL':$this->favorite);
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("ccountry", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new CcountryLine();

				$line->id = $obj->rowid;
				
				$line->code = $obj->code;
				$line->code_iso = $obj->code_iso;
				$line->label = $obj->label;
				$line->active = $obj->active;
				$line->favorite = $obj->favorite;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' favorite = '.(isset($this->favorite)?$this->favorite:"null");
