...
/**
 *   	\file       budget/putypestructure_list.php
 *		\ingroup    budget
...
$hookmanager->initHooks(array('putypestructurelist'));
...
$arrayfields=array(
	...
	't.active'=>array('label'=>$langs->trans("Fieldactive"), 'checked'=>0),
...
$help_url='';
$title = $langs->trans('Typestructure');
...
$sql.= " FROM ".MAIN_DB_PREFIX."pu_type_structure as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pu_type_structure_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
//	$moreforfilter.= '</div>';
	...
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			{
				print '<td>'.($obj->active?$langs->trans('Yes'):$langs->trans('Not')).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			// Extra fields
			if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label))
