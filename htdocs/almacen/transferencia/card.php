...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/almacen/transferencia/card.php?id='.$id,1);
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/almacen/transferencia/list.php',1);
			header("Location: ".$urltogo);
			exit;
		}
		...
		$object->fk_entrepot_from=GETPOST('fk_entrepot_from','int');
		$object->fk_entrepot_to=GETPOST('fk_entrepot_to','int');
		...
		$object->fk_source=GETPOST('fk_source','int');
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/almacen/transferencia/card.php?id='.$result,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	{
		$error=0;
		//vamos a verificar el almacen de y para
		$fk_entrepot_from = $object->fk_entrepot_from;
		$fk_entrepot_to = $object->fk_entrepot_to;
		//$object->ref=GETPOST('ref','alpha');
		$object->ref_ext=GETPOST('ref_ext','alpha');
		$object->fk_entrepot_from=GETPOST('fk_entrepot_from','int');
		$object->fk_entrepot_to=GETPOST('fk_entrepot_to','int');
		...
		//verificacion
		if ($fk_entrepot_from != $object->fk_entrepot_from) $lModiffrom = true;
		if ($fk_entrepot_to != $object->fk_entrepot_to) $lModifto = true;
		$db->begin();
		if ($lModiffrom)
		{
			$filter = " AND t.ref = '".$object->ref."'";
			$filter.= " AND t.fk_entrepot = ".$fk_entrepot_from;
			$res = $objStocktemp->fetchAll('','',0,0,array(),'AND',$filter);
			if ($res >0)
			{
				$lines = $objStocktemp->lines;
				foreach ($lines AS $j => $line)
				{
					$objStocktemp->fetch($line->id);
					$objStocktemp->fk_entrepot = $object->fk_entrepot_from;
					$objStocktemp->fk_user_mod = $user->id;
					$objStocktemp->datem = $now;
					$res = $objStocktemp->update($user);
					if ($res <=0)
					{
						$error++;
		...
		{
			$filter = " AND t.ref = '".$object->ref."'";
			$filter.= " AND t.fk_entrepot = ".$fk_entrepot_to;
			$res = $objStocktemp->fetchAll('','',0,0,array(),'AND',$filter);
			if ($res >0)
			{
				$lines = $objStocktemp->lines;
				...
				{
					$objStocktemp->fetch($line->id);
					$objStocktemp->fk_entrepot = $object->fk_entrepot_to;
					$objStocktemp->fk_user_mod = $user->id;
					$objStocktemp->datem = $now;
					$res = $objStocktemp->update($user);
					if ($res <=0)
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/almacen/transferencia/liste.php',1));
			exit;
		}
		else
		{
...
if (($id || $ref) && $action == 'edit')
{
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldref").'</td><td><input class="flat" type="text" name="ref" value="'.$object->ref.'" readonly></td></tr>';
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldfk_entrepot_from").'</td><td>';
	print $formproduct->selectWarehouses((GETPOST('fk_entrepot_from')?GETPOST('fk_entrepot_from'):$object->fk_entrepot_from), 'fk_entrepot_from', 'warehouseopen,warehouseinternal', 1, 0, 0, '', 0, 0, array(), 'minwidth200imp');
	print '</td></tr>';
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldfk_entrepot_to").'</td><td>';
	print $formproduct->selectWarehouses((GETPOST('fk_entrepot_to')?GETPOST('fk_entrepot_to'):$object->fk_entrepot_to), 'fk_entrepot_to', 'warehouseopen,warehouseinternal', 1, 0, 0, '', 0, 0, array(), 'minwidth200imp');

	print '</td></tr>';
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldfk_departament").'</td><td>';
	print $form->select_departament($object->fk_departament,'fk_departament','',0,1,'',0);
	print '</td></tr>';
	...
	$res = $objTypemov->fetchAll('ASC', 't.label', 0, 0, array(1=>1),'AND',$filterstatic);
	$options = '';
	if ($res>0)
	{
		foreach ($objTypemov->lines AS $j => $line)
		{
			$options.= '<option value="'.$line->id.'" '.((GETPOST('fk_type_mov')?GETPOST('fk_type_mov'):$object->fk_type_mov) == $line->id?'selected':'').'>'.$line->label.'</option>';
		}
	}
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldfk_type_mov").'</td><td>';
	print '<select name="fk_type_mov">'.$options.'</select>';
	print '</td></tr>';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $title, 0, 'transfer');
	...
	if (empty($reshook))
	{
		if ($user->rights->almacen->transfin->mod)
		{
	...
	//$linktoelem = $form->showLinkToObjectBlock($object, null, array('stockmouvementdoc'));
