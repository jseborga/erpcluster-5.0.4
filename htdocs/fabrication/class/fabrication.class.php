...
/**
 * \file    fabrication/fabrication.class.php
 * \ingroup fabrication
...
class Fabrication extends CommonObject
{
	 ...
	 */
	public $element = 'fabrication';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'fabrication';

	/**
	 * @var FabricationLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("fabrication", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->date_init = $this->db->jdate($obj->date_init);
				$line->date_finish = $this->db->jdate($obj->date_finish);
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' date_init = '.(! isset($this->date_init) || dol_strlen($this->date_init) != 0 ? "'".$this->db->idate($this->date_init)."'" : 'null').',';
		$sql .= ' date_finish = '.(! isset($this->date_finish) || dol_strlen($this->date_finish) != 0 ? "'".$this->db->idate($this->date_finish)."'" : 'null').',';
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Fabrication") . '</u>';
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;

        $url = DOL_URL_ROOT.'/fabrication/'.'fiche.php?id='.$this->id;
