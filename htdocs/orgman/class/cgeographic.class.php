...
/**
 * \file    orgman/cgeographic.class.php
 * \ingroup orgman
 ...
 * @see CommonObject
 */
class Cgeographic extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'cgeographic';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'c_geographic';

	/**
	 * @var CgeographicLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
			$sql .= " AND entity IN (" . getEntity("cgeographic", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Cgeographic($this->db);
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='',$action='')
	{
		...
		$label = '<u>' . $langs->trans("Clasificador Geográfico") . '</u>';
		$label.= '<br>';
		$label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;

		$url = DOL_URL_ROOT.'/orgman/geographic/'.'list.php?id='.$this->id.($action?'&action='.$action:'');
