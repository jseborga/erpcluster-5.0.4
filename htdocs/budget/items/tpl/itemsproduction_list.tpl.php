...
/**
 *   	\file       budget/itemsproduction_list.php
 *		\ingroup    budget
...
't.fk_items_product'=>array('label'=>$langs->trans("Fieldfk_items_product"), 'align'=>'align="left"', 'checked'=>1),
...
$search_status='';
	...
	$permtoread = $user->rights->itemsproduction->read;
	$permtodelete = $user->rights->itemsproduction->delete;
	$uploaddir = $conf->itemsproduction->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."items_production as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."items_production_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_items_product) $sql.= natural_search("fk_items_product",$search_fk_items_product);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/itemsproduction/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.fk_items_product']['checked'])) print_liste_field_titre($arrayfields['t.fk_items_product']['label'],$_SERVER['PHP_SELF'],'t.fk_items_product','',$params,$arrayfields['t.fk_items_product']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_items_product']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_items_product']['align'].'><input type="text" class="flat" name="search_fk_items_product" value="'.$search_fk_items_product.'" size="10"></td>';
