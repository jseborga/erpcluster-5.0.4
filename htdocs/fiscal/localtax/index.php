...
require_once DOL_DOCUMENT_ROOT.'/fiscal/lib/tax.lib.php';
...
//$result = restrictedArea($user, 'tax', '', '', 'fiscal');
...
$textprevyear="<a href=\"index.php?localTaxType=".$localTaxType."&type_tva=".GETPOST('type_tva')."&year=" . ($year_current-1) . "\">".img_previous()."</a>";
$textnextyear=" <a href=\"index.php?localTaxType=".$localTaxType."&type_tva=".GETPOST('type_tva')."&year=" . ($year_current+1) . "\">".img_next()."</a>";
...
print '<input type="hidden" name="localTaxType" value="'.($localTaxType?$localTaxType:1).'">';

print $form->load_type_tva('type_tva', GETPOST('type_tva'), 0,'code', false,0);
print '</form>';
print '</div>';
$type_tva = GETPOST('type_tva');
if (!empty($type_tva)) $objTypetva->fetch(0,$type_tva,$mysoc->country_id);

//$langs->transcountry($LT,$mysoc->country_code);
print load_fiche_titre($objTypetva->code,"$textprevyear ".$langs->trans("Year")." $year_start $textnextyear", 'title_accountancy.png');
...
if ($type_tva)
{
	...
	//$modetax = 2;
		$coll_listsell = vat_by_date($db, $y, 0, 0, 0, 2, 'sell', $m,$type_tva);
		$coll_listbuy = vat_by_date($db, $y, 0, 0, 0, 2, 'buy', $m,$type_tva);
