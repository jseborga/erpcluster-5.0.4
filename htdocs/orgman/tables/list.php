...
/**
 *   	\file       orgman/tablesdef_list.php
 *		\ingroup    orgman
...
$search_status='';
	...
	//$permtoread = $user->rights->tablesdef->read;
	//$permtodelete = $user->rights->tablesdef->delete;
	//$uploaddir = $conf->tablesdef->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."tables_def as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."tables_def_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/tablesdef/card.php?id='.$id);
	exit;
}
