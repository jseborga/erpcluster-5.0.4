...
$arrayfields=array(
	...
	't.fk_asset_group'=>array('label'=>$langs->trans("Fieldfk_asset_group"), 'checked'=>0),
...
if (empty($reshook))
{
	...
	$objectclass='Mgroups';
	$objectlabel='groups';
	$permtoread = $user->rights->mgroups->read;
	$permtodelete = $user->rights->mgroups->delete;
	$uploaddir = $conf->mant->dir_output;
...
$help_url='';
$title = $langs->trans('Groupsequipment');
...
$sql.= " FROM ".MAIN_DB_PREFIX."m_groups as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."m_groups_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_ref) $sql.= natural_search("ref",$search_ref);
if ($search_fk_asset_group) $sql.= natural_search("fk_asset_group",$search_fk_asset_group);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/mgroups/card.php?id='.$id);
	exit;
}
...
if ($conf->assets->enabled)
	if (! empty($arrayfields['t.fk_asset_group']['checked'])) print_liste_field_titre($arrayfields['t.fk_asset_group']['label'],$_SERVER['PHP_SELF'],'t.fk_asset_group','',$params,'',$sortfield,$sortorder);
...
//
	...
	if ($conf->assets->enabled)
	{
		if (! empty($arrayfields['t.fk_asset_group']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_asset_group" value="'.$search_fk_asset_group.'" size="10"></td>';
	}
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<input type="text" size="7" maxlength="30" name="ref" value="'.GETPOST('ref').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.fk_asset_group']['checked']))
	{
		if ($conf->assets->enabled)
		{
			print '<td>'.'<input type="text" size="8" name="fk_asset_group" value="">'.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
	}
	...
	{
		print '<td>'.'<input type="number" min="0" name="useful_life" value="'.GETPOST('useful_life').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.percent']['checked']))
	{
		print '<td>'.'<input type="number" min="0" name="percent" value="'.GETPOST('percent').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.account_accounting']['checked']))
	{
		print '<td>'.'<input type="text" size="10" name="account_accounting" value="'.GETPOST('account_accounting').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.account_spending']['checked']))
	{
		print '<td>'.'<input type="text" size="10" name="account_spending" value="'.GETPOST('account_spending').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if (! empty($arrayfields['t.fk_asset_group']['checked']))
		{
			if ($conf->assets->enabled)
			{
				print '<td>'.$obj->fk_asset_group.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
		}
