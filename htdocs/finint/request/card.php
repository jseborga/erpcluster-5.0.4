...
dol_include_once('/finint/class/bankcateg.class.php');
...
require_once(DOL_DOCUMENT_ROOT."/finint/lib/utils.lib.php");
...
$aStatut = array(9=>$langs->trans('All'),
	...
	4=>$langs->trans('Approveclosure').'/'.$langs->trans('Approverecharge'),
...
if ($conf->societe->enabled) $soc = new Societeext($db);
...
$objUser = new User($db);
$objcfact = new Ctypefacture($db);
$objFactureadd 	= new Facturefournadd($db);
...
if (($id > 0 || ! empty($ref)) && $action != 'add')
{
	...
	//verificamos si existe registro de request_cash_log
	if ($object->status_trans == 1)
	{
		$statuslog = 0;
		$filter = " AND t.fk_request_cash = ".$id;
		$res = $objectLog->fetchAll('DESC','t.datec',1,0,array(),'AND',$filter,true);
		if ($res>0)
		{
			$cText = '';
			foreach($objectLog->lines AS $j => $line)
			{
				if (empty($cText))
				{
					$statuslog = $line->status;
					$objUser->fetch($line->fk_user_create);
					$cText = $langs->trans('Theuser').' '.$objUser->lastname.' '.$objUser->firstname;
					$cText.= $langs->trans('Says').' : '. $line->description;
					if ($line->amount >0)
						$cText.= ' '.$langs->trans('Amount').' '.price(price2num($line->amount,'MT'));
					setEventMessages($cText,null,'mesgs');
				}
			}
		}
	}
...
if (empty($reshook))
{
	if ($action == 'updateparent' && $user->admin && $conf->global->FININT_UPDATE_DEPLACEMENT)
	{
		//ACTUALIZAMOS
		$aParent = GETPOST('aParent');
		$db->begin();
		foreach ((array) $aParent AS $fk_id => $value)
		{
			if (!$error)
			{
				if ($value>0)
				{
					$res = $deplacement->fetch($fk_id);
					if ($res == 1)
					{
						$deplacement->fk_parent = $value;
		...
		if (!$error)
		{
			...
			header('Location: '.$_SERVER['PHP_SELF'].'?id='.$id.'&action=listdep');
			exit;
		}
		else
		{
	...
	if ($action == "validate_dep" && $user->rights->finint->desc->val)
	{
		...
		if ($res>0)
		{
			...
			{
				//vamos a actualizar el fk_parent
				if ($deplacement->fk_parent != GETPOST('fk_transfert_ult'))
				{
					$deplacement->fk_parent = GETPOST('fk_transfert_ult');
					$res = $deplacement->update_fk_parent($user);
					if ($res<=0)
					{
						$error++;
						setEventMessages($deplacement->error,$deplacement->errors,'errors');
	...
	if ($action == "confirm_soltransf" && $user->rights->finint->desc->crear)
	{
		$res = $object->fetch(GETPOST('id'));
		if ($res>0)
		{
			$db->begin();
			//cambiamos de estado a status_transf
			$object->status_trans = 1;
			$res = $object->update($user);
			if ($res>0)
			{
				//agregamos en el log
				$objectLog->fk_request_cash = $object->id;
				$objectLog->description = GETPOST('description');
				$objectLog->amount = GETPOST('amount','int');
				if (empty($objectLog->amount)) $objectLog->amount = 0;
				$objectLog->fk_user_create = $user->id;
				$objectLog->fk_user_mod = $user->id;
				$objectLog->datec = $now;
				$objectLog->datem = $now;
				$objectLog->tms = $now;
				$objectLog->status_cash = $object->status;
				$objectLog->status = 1;
				//1 es traspaso de fondos
				//2 es cierre de caja
				$res = $objectLog->create($user);
				if ($res<=0)
				{
					$error++;
					setEventMessages($objectLog->error,$objectLog->errors,'errors');
				}
			}
			else
			{
		...
		{
			$db->commit();
			setEventMessages($langs->trans('Sendrequestcashtransfer'),null,'mesgs');
		}
		else
		{
			$db->rollback();
	...
	if ($action == "modifyfourn" && ($user->rights->finint->desc->crear || $user->rights->finint->desc->val))
	{
		...
		$dateo = dol_mktime(12,0,0,GETPOST('do_month','int'),GETPOST('do_day','int'),GETPOST('do_year','int'));

		$lViewqr = false;
		$deplacement = new Requestcashdeplacementext($db);
		$deplacement->fetch(GETPOST('idrcd'));
		$code_type_purchase = GETPOST('code_type_purchase');
		$type_operation = GETPOST('type_operation');
		$fk_transfert_ult = GETPOST('fk_transfert_ult');
		if ($deplacement->id ==GETPOST('idrcd'))
		{
			...
			$fourn_nit = GETPOST('fourn_nit');
			$lSearchsoc = false;
			if ($fk_soc>0)
			{
				//buscamos y comparamos
				$res = $soc->fetch($fk_soc);
				if ($res>0 && $soc->tva_intra != $fourn_nit) $lSearchsoc = true;
				if ($res == 0) $lSearchsoc = true;
				if ($res < 0) setEventMessages($soc->error,$soc->errors,'errors');
			}
			if ($conf->fiscal->enabled && !empty(GETPOST('code_f')) && $objcfact->nit_required && empty($type_operation))
			{
				$error++;
				setEventMessages($langs->trans('Review document type and invoice type'),null,'errors');
			}

			if (empty($fk_soc) && !empty($fourn_nit)) $lSearchsoc = true;
			if ($lSearchsoc)
			{
				//buscar al proveedor mediante el nit
				$filtersoc = " AND (t.code_client = '".$fourn_nit."'";
				$filtersoc.= " OR t.code_fournisseur = '".$fourn_nit."'";
				$filtersoc.= " OR t.tva_intra = '".$fourn_nit."')";
				...
				{
					//creamos al proveedor o cliente
					$soc = new Societeext($db);
					$soc->name 			= GETPOST('fourn_soc');
					$soc->code_client 	= GETPOST('fourn_nit');
					$soc->code_fournisseur = GETPOST('fourn_nit');
					$soc->tva_intra 	= GETPOST('fourn_nit');
					$soc->country_id 	= $mysoc->country_id;
					$soc->particulier 	= 1;
					$soc->client 		= 1;
					$soc->fournisseur 	= 1;
            		//$soc->civility_id       = GETPOST('civility_id');	// Note: civility id is a code, not an int
            		// Add non official properties
					$soc->name_bis          = $soc->name;
					$soc->firstname         = $soc->name;
					...
					if ($result >= 0)
					{
						...
						if ($soc->particulier)
						{
							...
							$result=$soc->create_individual($user);
							if (! $result >= 0)
							{
						...
						$soc->setCategories($custcats, 'customer');

						// Supplier categories association
						$suppcats = GETPOST('suppcats', 'array');
						$soc->setCategories($suppcats, 'supplier');
					}
					else
					{
						if($result == -3) {
							$duplicate_code_error = true;
							$soc->code_fournisseur = null;
							$soc->code_client = null;
						}
						$error+=$soc->error; $errors+=$soc->errors;
						setEventMessages($langs->trans('No se pudo crear'),null,'errors');
					}
				}
			}
			...
			if (!$error)
			{
				$deplacement->fk_projet_dest = $_POST['fk_projet__0'];
				$deplacement->fk_projet_task_dest = $_POST['fk_task'];
				$deplacement->amount  = $_POST['amount'];
				$deplacement->dateo = $dateo;
				if (!$lViewqr)
					$deplacement->nro_chq = $_POST['num_chq'];
				else
					$deplacement->nro_chq = GETPOST('fourn_facture');
				$deplacement->detail = $_POST['dp_desc'];
				$deplacement->code_facture = $_POST['code_f'];
				$deplacement->type_operation = $_POST['type_operation'];
				$deplacement->code_type_purchase = $_POST['code_type_purchase'];
				if ($fk_transfert_ult>0)
					$deplacement->fk_parent = $fk_transfert_ult+0;
				$deplacement->fk_categorie = ($_POST['cat1']?$_POST['cat1']:$object->fk_categorie);
				$deplacement->fk_soc = $fk_soc+0;

				$deplacement->nit_company = GETPOST('nit_company');
				$deplacement->fourn_nit = GETPOST('fourn_nit');
				$deplacement->fourn_soc = GETPOST('fourn_soc');
				if ($lViewqr)
				{
					$fourn_date = dol_mktime(12,0,0,GETPOST('fdo_month','int'),GETPOST('fdo_day','int'),GETPOST('fdo_year','int'));

					$deplacement->codeqr = GETPOST('codeqr');
					$deplacement->fourn_date = $fourn_date;
					$deplacement->fourn_facture = GETPOST('fourn_facture');
					$deplacement->fourn_numaut = GETPOST('fourn_numaut');
					$deplacement->fourn_codecont = GETPOST('fourn_codecont');
				}
				//fijo por la operacion de transferencia
				$deplacement->date_dest = $dateo;
				$deplacement->tms = dol_now();
				$deplacement->status = 1;
				$resc = $deplacement->update($user);
	...
	if ($action == "confirm_deplacement" && $_REQUEST['confirm'] == 'yes')
	{
		...
		foreach ($_POST as $key => $value)
		{
			...
			//{
			if (count($aKey)>2)
				$aData[$aKey[1]][$aKey[2]] = $value;
			else
				$aData[$aKey[1]][$aKey[0]] = $value;
			//}
		}
		...
		foreach ($aData AS $j => $row)
		{
			if ($row['amount']>0)
			{
				$k = $j + 1;
				$data = array();
				$dateo = dol_mktime(12,0,0,$row['month'],$row['day'],$row['year']);
				if (!$error)
				{
					if ($row['fourn_nit'])
					{
						//buscamos al proveedor
						$filterstatic = " AND (t.code_client = '".$row['fourn_nit']."'";
						$filterstatic.= " OR t.code_fournisseur = '".$row['fourn_nit']."'";
						$filterstatic.= " OR t.tva_intra = '".$row['fourn_nit']."')";
						$res = $soc->fetchAll('','',0,0,array(1=>1),'AND',$filterstatic,true);
						if ($res ==1)
						{
							$row['fk_soc'] = $soc->id;
							$row['fourn_soc'] = $soc->name;
						}
						elseif($res>1)
						{
							foreach ($soc->lines AS $s => $datasoc)
							{
								$row['fk_soc'] = $datasoc->id;
								$row['fourn_soc'] = $datasoc->name;
							}
						}
					}
					$deplacement = new Requestcashdeplacementext($db);
					$numref = $deplacement->getNextNumRef($soc);
					//verificamos los obligatorios
					if (empty($row['dp_desc']))
					{
						$error++;
					...
					if (!$error)
					{
						$deplacement->fk_request_cash = $id;
						$deplacement->entity = $object->entity;
						$deplacement->ref = $numref;
						$deplacement->fk_request_cash_dest = 0;
						$deplacement->url_id = 0;
						$deplacement->fk_bank = 0;
						$deplacement->fk_account_from = $object->fk_account;
						$deplacement->fk_account_dest = 0;
						$deplacement->fk_type = $object->fk_type;
						$deplacement->fk_categorie = $object->fk_categorie;
						$deplacement->fk_soc = $row['fk_soc'];
						$deplacement->fk_projet_dest = $row['fk_projet']+0;
						$deplacement->fk_projet_task_dest = $row['fk_task']+0;
						$deplacement->fk_parent = $fk_transfert_ult+0;
						$deplacement->fk_parent_app = $fk_transfert_ult+0;
						$deplacement->amount  = $row['amount']+0;
						$deplacement->concept = 'deplacement';
						$deplacement->dateo = $dateo;
						$deplacement->quant = $row['quant']+0;
						$deplacement->fk_unit = 0;
						$deplacement->nro_chq = $row['num_chq'];
						$deplacement->detail = $row['dp_desc'];
						$deplacement->code_facture = $_POST['code_f'];
						$deplacement->type_operation = $_POST['type_operation'];
						if ($deplacement->type_operation == -1 && is_null($deplacement->code_facture))
							$deplacement->code_facture = $conf->global->FININT_CODE_FACTURE_OTHER;
						$deplacement->fourn_facture = (isset($row['fourn_facture']) && !empty($row['fourn_facture']))?$row['fourn_facture']:$row['num_chq'];
						$deplacement->fourn_nit = $row['fourn_nit'];
						$deplacement->fourn_date = $dateo;
						$deplacement->fourn_soc = $row['fourn_soc'];

						$deplacement->codeqr = $row['codeqr'];
						//fijo por la operacion de transferencia
						$deplacement->fk_user_from = $user->id;
						$deplacement->fk_user_to = 0;
						$deplacement->fk_user_create = $user->id;
						$deplacement->fk_user_approved = $user->id;
						$deplacement->date_dest = $dateo;
						$deplacement->date_create = dol_now();
						$deplacement->tms = dol_now();
						$deplacement->status = 1;
						$resc = $deplacement->create($user);
						if ($resc<=0)
						{
							setEventMessages($deplacement->error,$deplacement->errors,'errors');
							$error++;
		...
		if (!$error)
		{
			unset($_SESSION['aUpload']);
			unset($_SESSION['aPost']);
			unset($_SESSION['aPostr'][$object->id]);
			unset($_SESSION['aDel'][$object->id]);
			$_SESSION['aK'][$object->id] = 0;
			$db->commit();
		...
		foreach ($_POST['quant'] AS $j => $value)
		{
			...
			if ($value>0)
			{
				...
				if (empty($data['fk_soc']))
					$data['fk_soc'] = $conf->global->FININT_ID_SOCIETE_DEFAULT;
				//$data['fk_soc'] = 1;
			...
			if (!$error)
			{
				...
				$cashdet->fk_parent_app = GETPOST('fk_transfert_ult');
	...
	// En get ou en post
	{
		...
		if (GETPOST('model'))
		{
			...
			if (GETPOST('model') == 'requestcashrecharge')
			{
				//vamos a buscar al ultimo
				$filter = " AND t.fk_request_cash_dest = ".$object->id;

				$resdep = $deplacement->fetchAll('DESC','ref',2,0,array(),'AND',$filter);
				if ($resdep>0)
				{
					//vamos a buscar al primero encontrado
					$fk_request_cash_deplacement=0;
					$lines = $deplacement->lines;
					foreach ($lines AS $j => $line)
					{
						if (empty($fk_request_cash_deplacement)) $fk_request_cash_deplacement = $line->id;
					}
				}
			}
		}
		else
			$object->modelpdf = 'cashapproved';
		...
		if ($object->modelpdf== 'requestcashrecharge')
		{
			$ret = $deplacement->fetch($fk_request_cash_deplacement);
			echo 'model '.$model='requestcashrecharge';

			echo 'result '.$result=$deplacement->generateDocument($model, $outputlangs, $hidedetails, $hidedesc, $hideref);
		}
		else
	...
	if ($action == 'builddocdischarg')	
	// En get ou en post
	{
		if ($idd>0)
		{
			$deplacement->fetch($idd);
			$document= $deplacement->nro_chq;
		}
		$object->fetch($id);
		if (empty($object->id))
			$object->id = $id;
		$object->fetch_thirdparty();
		$object->fetch_lines();
		$object->modelpdf = 'requestcashdischarg';

		//vamos a indicar el id dedescargo especifico
		$object->idd = $idd+0;
		$object->document = $document;
		// Define output language
		$outputlangs = $langs;
		...
		if (! $error)
		{
			...
			if ($insertid > 0)
			{
				...
				if (!$error)
				{
					...
					$deplacement = new Requestcashdeplacementext($db);
					$deplacement->fk_finint_cash = $object->id;
					$deplacement->url_id = $idDep;
					$deplacement->fk_bank = $insertid;
					$deplacement->quant = GETPOST('quant');
					$deplacement->fk_unit = GETPOST('fk_unit');
					$deplacement->amount  = $amount * -1;
					$deplacement->concept = 'deplacement';
					$deplacement->fk_type = $operation;
					$deplacement->nro_chq = $nro_chq;
					$deplacement->detail  = $label;
					$deplacement->fk_categorie = $fk_categorie;
					$deplacement->dateo = $dateop;
	...
	//verificamos primero el finint_cash_deplacement
		$deplacement = new Requestcashdeplacementext($db);
		$res = $deplacement->fetch($idrcd);
		if ($res > 0 && $deplacement->id == $idrcd)
		{
			$idDep = $deplacement->url_id;
			$idBank = $deplacement->fk_bank;
			$db->begin();
	...
	if ($action == 'add')
	{
		...
		if (empty($object->fk_account))$object->fk_account = 0;
		...
		$object->tms = dol_now();
		$object->amount_authorized=GETPOST('amount_authorized','int');
		if (empty($object->amount_authorized)) $object->amount_authorized=0;
		$object->status=0;
		$object->status_trans=0;

		if (empty($object->fk_type_cash) || $object->fk_type_cash<=0 )
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Typeofrequest")),null,'errors');
		}
	...
	if ($id && $action == 'confirm_outlay' && $confirm == 'yes' && $object->status == 2)
	{
		$langs->load('errors');
		$error= 0;
		$mesg='';
		...
		if ($object->id == $id)
		{
			...
			{
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("TransferFrom")), null, 'errors');
			}
			if (! GETPOST('account_to','int'))
			{
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("TransferTo")), null, 'errors');
			}
			...
			{
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Categorie")), null, 'errors');
			}
			if (! GETPOST('user_to','int'))
			{
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("UserTo")), null, 'errors');
			}
			if (! $error)
			{
				...
				if ($accountto->id != $accountfrom->id)
				{
					...
					{
						$error++;
						setEventMessages($accountfrom->error,$accountfrom->errors,'errors');
					}
					...
					{
						$error++;
						setEventMessages($accountto->error,$accountto->errors,'errors');
					}
					...
					if (!$error)
					{
						$deplacement = new Requestcashdeplacementext($db);
						$numref = $deplacement->getNextNumRef($soc);
						$deplacement->fk_request_cash = 0;
						$deplacement->fk_parent = 0;
						$deplacement->entity = $object->entity;
						$deplacement->ref = $numref;
						$deplacement->fk_request_cash_dest = $object->id;
						$deplacement->url_id = $bank_line_id_to;
						$deplacement->fk_bank = $bank_line_id_from;
						$deplacement->fk_type = $fk_type;
						$deplacement->fk_categorie = $cat1;
						$deplacement->fk_account_from = $accountfrom->id;
						$deplacement->fk_account_dest = $accountto->id;
						$deplacement->fk_projet = $object->fk_projet;
						$deplacement->amount  = $amount;
						$deplacement->concept = 'banktransfert';
						$deplacement->type_operation =0;
						$deplacement->dateo = $dateo;
						$deplacement->nro_chq = ($numdoc?$numdoc:$object->ref);
						$deplacement->detail = $label;
						//fijo por la operacion de transferencia
						$deplacement->fk_user_from = $user->id;
						$deplacement->fk_user_to = GETPOST('user_to','int')+0;
						$deplacement->fk_user_create = $user->id;
						$deplacement->fk_user_approved = $user->id;
						$deplacement->date_dest = $dateo;
						$deplacement->date_create = dol_now();
			...
			{
				$db->commit();
				//vamos a generar un documento de desembolso
				if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
				{
					...
					$ret = $deplacement->fetch($resc);
					$model='requestcashrecharge';
					
					$result=$deplacement->generateDocument($model, $outputlangs, $hidedetails, $hidedesc, $hideref);
	...
	if ($id && $action == 'confirm_apprecharge' && $_REQUEST["confirm"] == 'yes' && $object->status == 4 && $user->rights->finint->efe->appr)
	{
		...
		if ($object->id == $id)
		{
			...
			$idLasttransfer= GETPOST('idLasttransfer','int');
			...
			if (! $error)
			{
				...
				if ($accountto->id != $accountfrom->id)
				{
					//descargamos cada factura o recibo
					if (!$error)
					{
						...
						foreach ((array) $lines AS $j => $line)
						{
							...
							if ($concept == 'deplacement')
							{
								...
								if ($line->fourn_nit)
								{
									...
									$data['ref_soc'] = $line->fourn_facture.'|'.$line->fourn_nit.'|'.$line->fourn_numaut.'|'.$refcode;
									$data['fourn_nit'] = $line->fourn_nit;
									$data['codeqr'] = $line->codeqr;
									$data['fourn_soc'] = $line->fourn_soc;
									$data['fourn_facture'] = $line->fourn_facture;
									$data['fourn_numaut'] = $line->fourn_numaut;
									$data['fourn_date'] = $line->fourn_date;
									if (!$line->fk_soc)
									{
										...
										elseif($res > 1)
										{
											foreach($soc->lines AS $s => $linesoc)
												$fk_soc = $linesoc->id;
										}
										else
								...
								else
								{
									...
									$fk_soc = $conf->global->FININT_ID_SOCIETE_DEFAULT;
									$data['ref_soc'] = $line->ref.'|'.$refcode;
									$data['datefacture'] = $line->dateo;
								}
								$data['fk_soc'] 	= $fk_soc;
								$data['type'] 		= 0;
								$data['fk_projet'] 	= $line->fk_projet_dest;
								$data['fk_projet_task'] 	= $line->fk_projet_task_dest;
								...
								$data['code_facture'] = $line->code_facture;
								$data['code_type_purchase'] = $line->code_type_purchase;
								$data['nit_company'] = $line->nit_company;
								$data['type_operation'] = $line->type_operation;
								$data['closepaidinvoices'] = 'on';
								...
								{
									list($fk_facture_fourn,$paiement_id,$idBank,$error) = payfacturefourn($line->fk_facture_fourn,$data);
								}
								else
								{
									$data['origin'] = 'requestcashdeplacement';
									$data['originid'] = $line->id;
									list($fk_facture_fourn,$paiement_id,$idBank,$error) = facturefourn($data);
								}
							}
							else
							{
							...
							if ($concept == 'deplacement')
							{
								$deplacement->fk_facture_fourn = $fk_facture_fourn;
								$deplacement->url_id = $paiement_id;
								$deplacement->fk_bank = $idBank;
							}
							$deplacement->date_app = dol_now();
							//$deplacement->fk_parent_app = $idLasttransfer;
					...
					if (!$error)
					{
						...
						$object->status_trans=0;
						$object->fk_user_authorized = $user->id;
						$object->amount_authorized+=$amount_recharge;
						$object->tms = dol_now();
						//update
						$res = $object->update($user);
						if ($res <=0)
					...
					if (!$error)
					{
						...
						if (empty($bank_line_id_from)) $bank_line_id_from=0;
						if (empty($bank_line_id_to)) $bank_line_id_to=0;
						$deplacement = new Requestcashdeplacementext($db);
						$numref = $deplacement->getNextNumRef($soc);
						$deplacement->fk_request_cash = 0;
						$deplacement->entity = $object->entity;
						...
						$deplacement->fk_account_from = $accountfrom->id;
						$deplacement->fk_account_dest = $accountto->id;
						$deplacement->fk_projet_dest = $object->fk_projet;
						$deplacement->type_operation = 0;
						$deplacement->amount  = $amount;
						$deplacement->concept = 'banktransfert';
						$deplacement->dateo = $dateo;
						...
						//fijo por la operacion de transferencia
						$deplacement->fk_user_from = $user->id;
						$deplacement->fk_user_to = $object->fk_user_create;
						$deplacement->fk_user_create = $user->id;
						$deplacement->fk_user_approved = $user->id;
						$deplacement->date_dest = $dateo;
						$deplacement->date_create = dol_now();
					...
					if (! $error)
					{
						$mesg = $langs->trans("TransferFromToDone").' '.$amount_recharge.' '.$langs->transnoentities("Currency".$conf->currency);
						setEventMessages($mesg,null,'mesgs');
					}
					else
					{
			...
			{
				$db->commit();
				//vamos a generar el documento de transferencia
				if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
				{
					$outputlangs = $langs;
					$newlang = '';
	...
	if ($id && $action == 'confirm_noapprecharge' && $_REQUEST["confirm"] == 'yes' && $object->status == 4 && $user->rights->finint->efe->appr)
	{
		...
		if ($object->id == $id)
		{
			...
			{
				setEventMessages($langs->trans('Existe errores'),null,'errors');
				$db->rollback();
			}
		}
		if (!$error)
	...
	if ($id && $action == 'confirm_closetmp' && $_REQUEST["confirm"] == 'yes' && $object->status == 3)
	{
		...
		if ($object->id == $id)
		{
			...
			if (! $error)
			{
				...
				$deplacement->fk_parent = 0;
				$deplacement->fk_request_cash = 0;
				$deplacement->fk_request_cash_dest = $fk_finint_cash_to;
				$deplacement->fk_projet_dest = 0;
				$deplacement->fk_account_from = $object->fk_account;
				$deplacement->fk_account_dest = GETPOST('account_to')+0;
				$deplacement->fk_categorie = $cat1+0;
				$deplacement->dateo = $dateo;
				$deplacement->url_id = $bank_line_id_from+0;
					//de
				$deplacement->fk_bank = $bank_line_id_to+0;
				$deplacement->type_operation = 0;
					//a
				$deplacement->fk_type = $fk_type;
				$deplacement->nro_chq = $numdoc;
				$deplacement->detail = $label;
				$deplacement->amount  = $amount + 0;
				$deplacement->concept = 'banktransfert';
					//fijo por la operacion de gastos
				$deplacement->fk_user_from = $object->fk_user_create+0;
				$deplacement->fk_user_to = GETPOST('user_to','int')+0;

				$deplacement->fk_user_create = $object->fk_user_create;
				$deplacement->date_dest = $dateo;
				$deplacement->date_create = dol_now();
				$deplacement->tms = dol_now();
				$deplacement->status = 4;
				$resc = $deplacement->create($user);
				if ($resc<=0)
				{
					$error++;
	...
	if ($id && $action == 'confirm_closenoapp' && $_REQUEST["confirm"] == 'yes' && $object->status == 4)
	{
		...
		if ($object->id == $id)
		{
			...
			$deplacement = new Requestcashdeplacementext($db);
			$db->begin();
			$resc = $deplacement->fetch($idr);
			//echo '<hr>cond '.$deplacement->id.' == '.$idr.' && '.$deplacement->fk_request_cash_dest.' == '.$id;
			if ($deplacement->id == $idr && $deplacement->fk_request_cash_dest == $id)
			{
				$deplacement->status = -1;
				$deplacement->amount = 0;
				$deplacement->tms = dol_now();
				$res = $deplacement->update($user);
				if ($res <= 0)
				{
	...
	if ($id && $action == 'confirm_closeapp' && $_REQUEST["confirm"] == 'yes' && $object->status == 4)
	{
		...
		if ($object->id == $id && !empty($_SESSION['aPost']))
		{
			...
			$filterstatic.= " AND t.entity = ".$object->entity;
			$filterstatic.= " AND t.status = 2 ";
			$res = $deplacement->fetchAll('ASC','ref',0,0,array(),'AND',$filterstatic,false);
			if ($res < 0)
			{
				$error++;
				setEventMessages($deplacement->error,$deplacement->errors,'errors');
			...
			foreach ((array) $lines AS $j => $line)
			{
				...
				$resd = $deplacement->fetch($line->id);
				if ($resd>0)
				{
					$deplacement->status = 3;
					$resd = $deplacement->update($user);
					if ($resd <=0)
					{
						$error++;
						setEventMessages($deplacement->error,$deplacement->errors,'errors');
			...
			//recuperamos
			$deplacement->fetch($_POST['idr']);

			if ($deplacement->id == $_POST['idr'] && $deplacement->fk_request_cash_dest == $id)
			{
				$dateo = $deplacement->dateo;
				$amount = $deplacement->amount;
				$label = $deplacement->detail;
				$numdoc = $deplacement->nro_chq;
				$fk_type = $deplacement->fk_type;
				$cat1 = $deplacement->fk_categorie;
				$fk_account_from = $deplacement->fk_account_from;
				$fk_account_dest = $deplacement->fk_account_dest;
				...
				if ($amount > 0 && $abc)
				{
					...
					$accountfrom=new Account($db);
					$accountfrom->fetch($deplacement->fk_account_from);

					$accountto=new Account($db);
					$accountto->fetch($deplacement->fk_account_dest);
					if ($accountto->id != $accountfrom->id)
					{
						$error=0;
						$bank_line_id_from=0;
						...
						{
							// This is transfert of change
							$typefrom=$deplacement->fk_type;
							$typeto=$deplacement->fk_type;
						}
						if ($accountto->courant == 2)
						{
							$typeto='LIQ';
	...
	if ($id && $action == 'confirm_close' && $_REQUEST["confirm"] == 'yes' && $object->status == 3)
	{
		...
		if ($object->id == $id)
		{
			...
			// if (empty($error))
			//   {
			// 	$deplacement = new Requestcashdeplacement($db);
			// 	$deplacement->fk_finint_cash = 0;
			// 	$deplacement->fk_finint_cash_dest = $object->id;
			// 	$deplacement->url_id = $bank_line_id_to;
			// 	$deplacement->fk_bank = $bank_line_id_from;
			// 	$deplacement->amount  = $amount;
			// 	$deplacement->concept = 'banktransfert'; //fijo por la operacion de transferencia
			// 	$deplacement->fk_user_to = $user->id;
			// 	$deplacement->fk_user_create = $user->id;
			// 	$deplacement->date_create = dol_now();
			// 	$deplacement->tms = dol_now();
			// 	$deplacement->status = 1;
			...
			if (! $error)
			{
				...
				if ($amount > 0)
				{
					...
					if ($amount_close > 0)
					{
						...
						$deplacement->fk_parent = 0;
						$deplacement->fk_finint_cash = $object->id;
						$deplacement->fk_finint_cash_dest = 0;
						$deplacement->url_id = 0;
						//$deplacement->fk_bank = $bank_line_id_from;
						$deplacement->fk_bank = 0;
						$deplacement->amount  = $amount;
						...
						$deplacement->type_operation = 0;
	 					//fijo por la operacion de transferencia
						$deplacement->fk_account_from = $object->fk_account;
						$deplacement->fk_account_dest = $object->fk_account;
						$deplacement->fk_user_from = $object->fk_user_create;
						$deplacement->fk_user_to = $object->fk_user_create;
						$deplacement->fk_user_create = $object->fk_user_create;
						$deplacement->date_create = dol_now();
						$deplacement->tms = dol_now();
	...
	if ($id && $action == 'confirm_transfer' && $_REQUEST["confirm"] == 'yes' && $user->rights->finint->trans->crear)
	{
		...
		if ($object->id == $id)
		{
			...
			if (! $error)
			{
				...
				$deplacement->fk_parent = 0;
				$deplacement->fk_finint_cash = $object->id;
				$deplacement->fk_finint_cash_dest = $fk_finint_cash_to;
				$deplacement->fk_projet_dest = $fk_projet;
				$deplacement->fk_account_from = $object->fk_account;
				$deplacement->fk_account_dest = GETPOST('account_to');
				$deplacement->fk_categorie = $cat1+0;
				$deplacement->dateo = $dateo;
				$deplacement->url_id = $bank_line_id_from;
					//de
				$deplacement->fk_bank = $bank_line_id_to;
					//a
				$deplacement->fk_type = $fk_type;
				$deplacement->type_operation = 0;
				$deplacement->nro_chq = $numdoc;
				...
				if (empty($error))
				{
					...
					if ($fk_finint_cash_to>0)
					{
						...
						if ($objectnew->id == $fk_finint_cash_to)
						{
							...
							$deplacement->fetch($resc);
							if ($deplacement->id == $resc)
							{
								$deplacement->fk_finint_cash_dest = $fk_finint_cash_to;
								$deplacement->dateo = $dateo;
								$res = $deplacement->update($user);
					...
					else
					{
						...
						if (!$error)
						{
							...
							//actualizamos el finint_cash_deplacement con el destino
						$deplacement->fetch($resc);
						if ($deplacement->id == $resc && $idres>0)
						{
							$deplacement->fk_finint_cash_dest = $idres;
							$deplacement->dateo = $dateo;
							$res = $deplacement->update($user);
							if ($res<=0) $error++;
						}
	...
	if ($id && $action == 'confirm_reject' && $_REQUEST["confirm"] == 'yes' && $user->rights->finint->trans->aarr)
	{
		...
		if ($object->id == $id)
		{
			...
			$deplacement = new Requestcashdeplacementext($db);
			$res = $deplacement->fetch(GETPOST('idr','int'));
			if ($res > 0 && $deplacement->id == GETPOST('idr'))
			{
				$deplacement->status = -1;
				$res = $deplacement->update($user);
				if ($res<=0)
				{
					setEventMessages($deplacement->error, null, 'errors');
					header("Location: ".dol_buildpath('/finint/request/card.php?id='.$id,1));
					exit;
				}
				else
	...
	if ($id && $action == 'confirm_accept' && $_REQUEST["confirm"] == 'yes' && $user->rights->finint->trans->aarr)
	{
		...
		if ($object->id == $id)
		{
			...
			if ($res > 0 && $deplacement->id == GETPOST('idr'))
			{
				$dateo = $deplacement->date_dest;
				if (empty($dateo)) $dateo = dol_now();

				$label = $deplacement->detail;
				$amount= $deplacement->amount;
				$account_from = $deplacement->fk_account_from;
				$user_to = $deplacement->fk_user_to;
				$user_from = $deplacement->fk_user_from;
				$account_to = $deplacement->fk_account_dest;
				$fk_projet = $deplacement->fk_projet_dest;
				$fk_type = $deplacement->fk_type;
				$cat1 = $deplacement->fk_categorie;
				$nro_chq = $deplacement->nro_chq;
				if (! $label)
				{
					$error++;
				...
				{
					$error++;
					setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentities("UserTo")),null,'errors');
				}
				if (! $error)
				{
			...
			{
				setEventMessages($deplacement->errors, null, 'errors');
				header("Location: ".dol_buildpath('/finint/request/card.php?id='.$id,1));
				exit;
			}
		}
...
if ($action == 'list')
{
	...
	if ($resql)
	{
		...
		while ($i < $num)
		{
			...
			{
				$var = !$var;
				//buscamos si tiene solicitud de traspaso
				$cText='';
				$img = '';
				$statuslog = 0;
				if ($obj->status_trans > 0)
				{
					$filtertmp = " AND t.fk_request_cash = ".$obj->id;
				...
				$res = $deplacement->fetchAll('ASC','rowid',0,0,$filter,'AND',$filterstatic,true);
				foreach ((array) $deplacement->lines AS $j => $lined)
				{
					$totalg+= $lined->amount;
				}
				...
				if ($object->id == $obj->id && $object->status >= 3)
				{
					if (empty($object->nro_chq))
					{
						...
						$res = $deplacement->fetchAll('ASC','rowid',0,0,$filter,'AND',$filterstatic,true);
						if (!empty($deplacement->fk_bank))
						{
							$objbank = new Accountline($db);
							$objbank->fetch($deplacement->fk_bank);
							if ($objbank->id == $deplacement->fk_bank)
							{
				...
				if ($cText)
				{
					$img = img_picto($cText,'warning');
				}
				print '<td nowrap>'.$object->getNomUrl(1).($cText?' '.$img:'').'</td>';
				if ($conf->monprojet->enabled)
				...
				$balance = $obj->amount_authorized - $totalg;
				if ($obj->status==5) $balance -= $obj->amount_close;
				$balance = price2num($balance,'MT');
				$style = '';
				if ($balance >0) $style = 'style="color:#ff0000;"';

				print '<td align="right" '.$style.'>'.price(price2num($balance,'MT')).'</td>';
				print '<td align="right">'.$object->getLibStatut($objtypecash->recharge?($obj->status_trans==2?2:3):2).'</td>';
...
if ($action == 'create' && $user->rights->finint->efe->crear)
{
	...
	if ($nbtypecash)
		print '<select name="fk_type_cash" required>'.$options.'</select>';
	else
	{
		$error++;
		print img_picto($langs->trans('Error'),'error').' '.$langs->trans('Definethetypesofrequirements');
	}
	print '</td></tr>';
	...
	print '<tr><td>'.$langs->trans("Attachment").'</td><td>';
...
if (($id || $ref ) && (empty($action) || $action == 'view' ||
	...
	$action == 'addformline' || $action == 'delformline' ||
	$action == 'listdep' || $action == 'soltransf' ||
...
if ($lView)
{
	//cambiamos de acion si es addformline
	$lsubmiton = false;
	if ($action == 'addformline')
	{
		$lsubmiton = true;
		$action = 'createrefr';
	}
	if ($action == 'delformline')
	{
		$aDel = $_SESSION['aDel'][$object->id];
		foreach ($_POST AS $key => $value)
		{
			$aKey = explode('_',$key);
			if ($aKey[0] == 'butdel') $aDel[$aKey[1]]=$aKey[1];
		}
		$_SESSION['aDel'][$object->id] = $aDel;
		$action = 'createrefr';
	}
		// Confirm accept
	unset($_SESSION['aGet']);
	...
	if ($action == 'soltransf')
	{
		...
		$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Requestcashtransfer"),$langs->trans("Confirmrequestcashtransfer",$object->ref),"confirm_soltransf",$form_question,1,2);
		if ($ret == 'html') print '<br>';
	}
	...
	if ($action == 'noapprecharge')
	{
		...
		$form_question['lVal'] = array (
			'name' => 'lVal',
			'type' => 'select',
			'label' => $langs->trans('Revert validation of downloads'),
			'values' => $array,
			'default' => 0
		);
	...
	if ($action == 'closeapp')
	{
		...
		$deplacement = new Requestcashdeplacementext($db);
		$deplacement->fetch($idr);
		$amount = 0;
		if ($deplacement->id == $idr)
		{
			$amount = $deplacement->amount;
			//$form = new Form($db);
	...
	if ($action == 'closenoapp')
	{
		...
		if ($deplacement->id == $idr)
		{
			$amount = $deplacement->amount;
			//$form = new Form($db);
			$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id.'&idr='.$idr,$langs->trans("Rejectclose"),$langs->trans("Confirmrejectclose",$object->ref).' '.$langs->trans('The').' '.price($amount),"confirm_closenoapp",'',0,2);
			if ($ret == 'html') print '<br>';
		}
		else
			$action = '';
	...
	if ($action == 'accept')
	{
		...
		{
			$deplacement = new Requestcashdeplacementext($db);
			$res = $deplacement->fetch($idr);
			if ($user->admin || $deplacement->fk_user_to == $user->id)
			{
				$_SESSION['aGet'] = $_GET;
				//$form = new Form($db);
				$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Accept"),$langs->trans("Confirmacceptcashtransfer",$object->ref),"confirm_accept",'',0,2);
				if ($ret == 'html') print '<br>';
			}
		}
	}
	if ($action == 'noaccept')
	{
		//buscamos el registro
		$idr = GETPOST('idr','int');
		echo $object->fk_user_create.' == '.$user->id;
		if ($user->admin || $object->fk_user_create == $user->id)
		{
			$deplacement = new Requestcashdeplacementext($db);
			$res = $deplacement->fetch($idr);
			if ($user->admin || $deplacement->fk_user_to == $user->id)
			{
				$_SESSION['aGet'] = $_GET;
				//$form = new Form($db);
				$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Reject"),$langs->trans("Confirmrejectcashtransfer",$object->ref),"confirm_reject",'',0,2);
				if ($ret == 'html') print '<br>';
			}
		}
	}
	...
	if ($action == 'validate')
	{
		...
		$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,
			$langs->trans("Validaterequest"),
			$langs->trans("Confirm validation for an amount of").' '.' <span style="font-size:2em;">'.price(price2num($nTotal,'MT'),$object->ref).'</span>', "confirm_validate", $formquestion,
			0,2);
		if ($ret == 'html') print '<br>';
	}
	// Confirm validate request/card
	...
	if ($action == 'rechargeconf')
	{
		...
		//$form = new Form($db);
		$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Requestrecharge"),$langs->trans("Confirmrequestrecharge",$object->ref).' '.$langs->trans('The').' '.price($_POST['amount']),"confirm_rechargetmp",'',0,2);
		if ($ret == 'html') print '<br>';
	}
		// Confirm closeconf
	if ($action == 'closeconf')
	...
	if ($action != 'mod_depval' && $action != 'recharge' && $action != 'discharg' && $action != 'createrefr' )
	{
		...
		if ($action == 'editassigned')
		{
			...
			print '<input type="hidden" name="backtopage" value="'.$backtopage.'">';
			print '<input type="hidden" name="id" value="'.$object->id.'">';
			print $form->select_users($object->fk_user_assigned,'fk_user_assigned',0,array(1,$object->fk_user_create),0);
			print '<input class="butAction" type="submit" name="save" value="'.$langs->trans('Save').'">';
			print '</form>';
		}
		else
		{
	...
	if ($object->status >=3 )
	{
		...
		//if ($objtypecash->recharge && ( $action == 'discharg' )) $lViewdischarg = false;
		//if ($objtypecash->recharge && ( $action == 'createrefr' )) $lViewdischarg = false;
		if ($action == 'mod_depval') $lViewres = false;
		if ($lViewres)
		{
			print '<table class="noborder centpercent">'."\n";
			// Fields title
		...
		foreach ((array) $objdeplac->lines AS $j => $line)
		{
			...
			if ($action != 'discharg' && $action != 'createrefr')
			{
				if ($lViewres)
				{
					...
					if ($objaccount->id == $fkacc) print $objaccount->getNomUrl(1);
					else print '';
					print '</td>';
					if($conf->browser->layout=='classic')
					{
					...
					if ($line->status == 1)
					{
						print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&action=listdep'.'&idd='.$line->id.'">'.img_picto($langs->trans('Discarg'),'list').'</a>';
						print '&nbsp;';
						print img_picto($langs->trans('Accepted'),DOL_URL_ROOT.'/finint/img/ok','',1);
						print '&nbsp;';
						print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&action=builddocdischarg'.'&idd='.$line->id.'">'.img_picto($langs->trans('ReportPdf'),'pdf3').'</a>';
			...
			//se incluyo el estado 0 para no considerar como efectivo
			if ($line->status != -1 && $line->status != 4)
			{
				if ($line->status != 0) $sumatransf+=$line->amount;
				$sumadep += $line->amount;
			}
		}
		if ($lViewres)
	...
	if ($object->status >=3 )
	{
		...
		foreach ((array) $objdeplac->lines AS $j => $line)
		{
			if ($line->status == 1) $sumapar += $line->amount*-1;
			if ($line->status == 0) $sumapar0 += $line->amount*-1;
		...
		foreach ((array) $objdeplac->lines AS $j => $line)
		{
			if ($line->status == 1) $sumaparc += $line->amount*-1;
			if ($line->status == 0) $sumaparc0 += $line->amount*-1;
			if ($line->status == 4)
			{
				$lCierre = true;
				$sumaparcierre += $line->amount*-1;
			}
			$fk_transfert_ult = $line->id;
		}

		//echo '<hr> sumtratransf '.$sumatransf.'+ sumadep '.$sumadep.'+sumapar '.$sumapar.'+sumapar0 '.$sumapar0.'+sumaparcierre '.$sumaparcierre;
		$balance = price2num($sumatransf+$sumadep+$sumapar+$sumapar0+$sumaparcierre,'MT');
		$balance = price2num($sumatransf+$sumadep+$sumapar+$sumapar0,'MT');
		...
		{
			print '<td>'.($conf->browser->layout=='classic'?$langs->trans('Transferido'):$langs->trans('T.')).'= <b>';
		...
		if (!$objtypecash->recharge || ($objtypecash->recharge && $sumapar0<>0) || $lCierre)
		{
			...
			if ($object->status != 5)
			{
				...
				if ($lCierre)
					print '<span style="color:#ff0000;">'.price($sumaparcierre).'</span></b></td>';
				else
		...
		print '</tr>';
		print '</table>';
		$object->amount.' - '.$balance.'+'.$sumanoappdep;
		$sumrecharge = $object->amount - $balance+$sumanoappdep;
		$sumbalance = $object->amount + $sumanoappdep;
		$sumbalance = $balance;
	...
	if (empty($reshook))
	{
		...
		if ($lContinue)
		{
			...
			if ($user->rights->finint->desc->leer && $object->status == 3 && ($action!='discharg' && $action!='createrefr' && $action!='close' && $action!='transfd') || $user->rights->finint->desc->all && ($action!='discharg' && $action!='createrefr' && $action!='close' && $action!='transfd' && $action!='mod_depval'))
			{
				if ($user->rights->finint->desc->all || ($user->id == $object->fk_user_assigned || $user->id == $object->fk_user_create))
				{
					if ($object->status>2 && $object->status < 5)
			...
			//echo $objtypecash->recharge.' && '.$object->status.' == 3 && '.$action.' != recharge';
			if ($objtypecash->recharge && $object->status == 3 && $action != 'recharge')
			{
				//echo '<br>'.$user->id.' == '.$object->fk_user_assigned.' || '.$user->id.' == '.$object->fk_user_create;
			...
			{
				if (empty($sumapar0) && $action != 'close' && $action !='transfd')
				{
			...
			if ($action != 'mod_depval' && $user->rights->finint->desc->leer && $object->status > 2)
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=listdep">'.$langs->trans("Viewdischarg").'</a></div>'."\n";
			}

			if ($balance < 0 && (empty($action) || $action == 'discharg') && $object->status ==3)
			{
				if ($user->rights->finint->desc->crear)
					print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=soltransf">'.$langs->trans("Requestcashtransfer").'</a></div>'."\n";
				if ($user->rights->finint->trans->crear)
					print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/finint/transfer/card.php'.'?fk_request_cash='.$object->id.'&action=create">'.$langs->trans("Createcashtransfer").'</a></div>'."\n";
			}
		}
	...
	if ($action == 'listdep')
	{
		include DOL_DOCUMENT_ROOT.'/finint/tpl/viewdeplacement.tpl.php';
		$lViewdischarg = false;
	}
	...
	if ($object->status == 5 || $action == 'discharg' || $action == 'createrefr' || $action == 'modifyrefr' || $action == 'modifyvalrefr' || $action=='close' || $action=='mod_dep' || $action=='mod_depval' || $action=='recharge' || $action == 'transfd' || $action == 'formapprecharge')
	{
		...
		if ($action == 'createrefr')
		{
			...
			if (empty($k)) $k = 0;
			if ($lsubmiton) $k++;
			//guardamos en session el numero de repeticiones
			$_SESSION['aK'][$object->id] = $k;
			$dateo = dol_mktime(12,0,0,GETPOST('domonth','int'),GETPOST('doday','int'),GETPOST('doyear','int'));
			...
			foreach ($_POST AS $key => $value)
			{
				...
				$aKey = explode('__',$key);
				if ($aKey[0] == 'fk_projet')
					$_POST['fk_projet__'].$aKey[1] = $_POST[$key];
				elseif ($aKey[0] == 'fk_task')
					$_POST['fk_task__'].$aKey[1] = $_POST[$key];
				elseif ($aKey[0]== 'num_chq')
					$_POST['num_chq__'.$aKey[1]] = $_POST[$key];
				elseif ($aKey[0]== 'do')
				{
					$date_o = dol_mktime(12,0,0,GETPOST('do__'.$aKey[1].'__month','int'),GETPOST('do__'.$aKey[1].'__day','int'),GETPOST('do__'.$aKey[1].'__year','int'));
					$_POST['do__'.$aKey[1]] = $date_o;
					//echo '<hr>dateo '.dol_print_date($date_o,'day');exit;
				}
			...
			$_SESSION['aPostr'][$object->id] = $_POST;
			//echo '<hr>res <pre>';
			//print_r($aKey);
			//echo '</pre>';
		}
		...
		if ($subprocess == 'modifyvalrefr')
		{
			$myclass = new Requestcashdeplacementext($db);
			$myclass->fetch(GETPOST('idrcd'));
			$myclass->dateo = dol_mktime(12,0,0,GETPOST('do_month','int'),GETPOST('do_day','int'),GETPOST('do_year','int'));
			if (isset($_POST['codeqr']))
			{
				//VAMOS a convertir el codigo qr en texto
				$aCodeqr = explode('|',GETPOST('codeqr'));
				$supplier_nit = $aCodeqr[0];
				$supplier_facture = $aCodeqr[1];
				$supplier_numaut = $aCodeqr[2];
				$aSupplier_date = explode('/',$aCodeqr[3]);
				$supplier_date = dol_mktime(12,0,0,$aSupplier_date[1],$aSupplier_date[0],$aSupplier_date[2]);
				$supplier_amount = $aCodeqr[4];
				$supplier_amount_base = $aCodeqr[5];
				$supplier_code_control = $aCodeqr[6];
				$_POST['fourn_nit'] = $supplier_nit;
			}
			if (GETPOST('fourn_nit'))
			{
				$myclass->fourn_nit = GETPOST('fourn_nit');
				//buscamos al cliente o proveedor
				$filterstatic = " AND (t.code_client = '".GETPOST('fourn_nit')."'";
				$filterstatic.= " OR t.code_fournisseur = '".GETPOST('fourn_nit')."'";
				$filterstatic.= " OR t.tva_intra = '".GETPOST('fourn_nit')."')";
				$res = $soc->fetchAll('','',0,0,array(1=>1),'AND',$filterstatic,true);
				if ($res ==1)
				{
					$myclass->fk_soc = $soc->id;
					$myclass->fourn_soc = $soc->name;
				}
				elseif($res>1)
				{
					foreach ($soc->lines AS $s => $datasoc)
					{
						$myclass->fk_soc = $datasoc->id;
						$myclass->fourn_soc = $datasoc->name;
					}
				}

			}
			if (GETPOST('amount')<> $supplier_amount)
			{
				$error++;
				setEventMessages($langs->trans('Theamountinitiallyregisteredisdifferentfromtheamountoftheinvoice'),null,'warnings');
			}
			$myclass->user_to = GETPOST('user_to');
			$myclass->detail = GETPOST('dp_desc');
			$myclass->amount = ($supplier_amount?$supplier_amount:GETPOST('amount'));
			$myclass->amount_ttc = ($supplier_amount?$supplier_amount:GETPOST('amount'));
			$myclass->fk_projet_dest = GETPOST('fk_projet__0');
			$fk_projetsel = GETPOST('fk_projet__0');
			if ($fk_projetsel>0) $filtertask = " t.fk_projet = ".$fk_projetsel;
			$myclass->fk_projet_task_dest = GETPOST('fk_task');
			$fk_tasksel = GETPOST('fk_task');

			$myclass->code_facture = GETPOST('code_f');
			$myclass->code_control = $supplier_code_control;
			$myclass->fourn_date = ($supplier_date?$supplier_date:$myclass->fourn_date);
			$myclass->fourn_facture = ($supplier_facture?$supplier_facture:$myclass->fourn_facture);
			$myclass->fourn_numaut = ($supplier_numaut?$supplier_numaut:$myclass->fourn_numaut);
			$myclass->fourn_codecont = ($supplier_code_control?$supplier_code_control:$myclass->fourn_codecont);
	...
	if ($action == 'listdep')
	{
		//vamos a mostrar un resumen
		dol_fiche_head(array(),0,$langs->trans('Summary'));
		print '<div style="min-width:450px;overflow-x: auto; white-space: nowrap;">';
		...
		if ($conf->monprojet->enabled)
			print_liste_field_titre($langs->trans('Projet'),$_SERVER['PHP_SELF'],'','',$param,'',$sortfield,$sortorder);

		print_liste_field_titre($langs->trans('Recibo'),$_SERVER['PHP_SELF'],'','',$param,'align="right"',$sortfield,$sortorder);
		print_liste_field_titre($langs->trans('Factura'),$_SERVER['PHP_SELF'],'','',$param,'align="right"',$sortfield,$sortorder);
		print_liste_field_titre($langs->trans('Total'),$_SERVER['PHP_SELF'],'','',$param,'align="right"',$sortfield,$sortorder);
		print '</tr>';
		print '</thead>';
		print '<tbody>';
		$aTotal = array();
		foreach ($aSummary AS $fk_projet => $data)
		{
			$var = !$var;
			$aTotal[1]+= $data[1];
			$aTotal[-1]+= $data[-1];
			$aTotal[0]+=$data[1]+$data[-1];
			$objProjet->fetch($fk_projet);
			print '<tr '.$bc[$var].'>';
			print '<td>'.$objProjet->getNomUrl(1).'</td>';
			print '<td align="right">'.price(price2num($data[-1],'MT')).'</td>';
			print '<td align="right">'.price(price2num($data[1],'MT')).'</td>';
			print '<td align="right">'.price(price2num(($data[-1]+$data[1]),'MT')).'</td>';
			print '</tr>';
		}
		//vamos a mostrar totales
			print '<tr class="liste_total">';
			print '<td>'.$langs->trans('Total').'</td>';
			print '<td align="right">'.price(price2num($aTotal[-1],'MT')).'</td>';
			print '<td align="right">'.price(price2num($aTotal[1],'MT')).'</td>';
			print '<td align="right">'.price(price2num($aTotal[0],'MT')).'</td>';
			print '</tr>';
