...
/**
 *   	\file       purchase/facturefourndetadd_list.php
 *		\ingroup    purchase
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-04-04 17:02
 */
...
dol_include_once('/purchase/class/facturefourndetadd.class.php');
...
't.fk_facture_fourn_det'=>array('label'=>$langs->trans("Fieldfk_facture_fourn_det"), 'checked'=>1),
...
$object=new Facturefourndetadd($db);
...
$search_status='';
    ...
    $permtoread = $user->rights->facturefourndetadd->read;
    $permtodelete = $user->rights->facturefourndetadd->delete;
    $uploaddir = $conf->facturefourndetadd->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."facture_fourn_det_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."facture_fourn_det_add_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_facture_fourn_det) $sql.= natural_search("fk_facture_fourn_det",$search_fk_facture_fourn_det);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/facturefourndetadd/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_facture_fourn_det']['checked'])) print_liste_field_titre($arrayfields['t.fk_facture_fourn_det']['label'],$_SERVER['PHP_SELF'],'t.fk_facture_fourn_det','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_facture_fourn_det']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_facture_fourn_det" value="'.$search_fk_facture_fourn_det.'" size="10"></td>';
