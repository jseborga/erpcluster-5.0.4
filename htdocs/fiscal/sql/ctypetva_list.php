...
$search_active='';
    ...
    $permtoread = $user->rights->ctypetva->read;
    $permtodelete = $user->rights->ctypetva->delete;
    $uploaddir = $conf->ctypetva->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_tva_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/ctypetva/card.php?id='.$id);
    exit;
}
