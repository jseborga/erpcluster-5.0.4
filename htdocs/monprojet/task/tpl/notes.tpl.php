...
$permission=(isset($permission)?$permission:(isset($user->rights->monprojet->notepub->leer)?$user->rights->monprojet->notepub->leer:0));    // If already defined by caller page
...
elseif ($module == 'project_task')      { $permission=$user->rights->monprojet->notepub->crear;}
...
<?php if ($user->rights->monprojet->note->leer) { ?>
...
<?php if (empty($user->societe_id)) { ?>
  <div class="table-border-row">
      <?php if ($user->rights->monprojet->note->leer) {$permission = $user->rights->monprojet->notepri->crear;}else $permission= false; ?>
