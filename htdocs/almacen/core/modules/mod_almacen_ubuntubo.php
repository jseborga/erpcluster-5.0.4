 ...
 */
class mod_almacen_ubuntubo extends ModeleNumRefAlmacen
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefixinvoice='PALM';
	var $prefixcreditnote='AV';
	var $error='';
	var $nom='ubuntubo_palm';
	...
	function getExample()
	{
		return $this->prefixinvoice."0501-00001";
	}
	...
	function getNextValue($objsoc,$facture,$mode='next')
	{
		...
		$sql.= " AND entity = ".$conf->entity;
		$resql=$db->query($sql);
		dol_syslog("mod_almacen_ubuntubo::getNextValue sql=".$sql);
		if ($resql)
		{
		  ...
		  {
		    dol_syslog("mod_almacen_ubuntubo::getNextValue sql=".$sql, LOG_ERR);
		    return -1;
		  }
		...
		if ($mode == 'last')
		...
		else if ($mode == 'next')
		  {
		    ...
		    dol_syslog("mod_almacen_ubuntubo::getNextValue return ".$prefix.$fayymm."-".$num);
