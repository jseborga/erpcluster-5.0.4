...
/**
 *   	\file       budget/itemsformula_list.php
 *		\ingroup    budget
...
$hookmanager->initHooks(array('itemsformulalist'));
...
't.fk_pu_structure'=>array('label'=>$langs->trans("Fieldfk_pu_structure"), 'checked'=>1),
...
't.date_create'=>array('label'=>$langs->trans("Fielddate_create"), 'checked'=>1),
't.date_mod'=>array('label'=>$langs->trans("Fielddate_mod"), 'checked'=>0),
...
$sql.= " FROM ".MAIN_DB_PREFIX."items_formula as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."items_formula_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('item_formula',1).")";

if ($search_fk_budget) $sql.= natural_search("fk_budget",$search_fk_budget);
if ($search_fk_pu_structure) $sql.= natural_search("fk_pu_structure",$search_fk_pu_structure);
...
if ($search_date_create) $sql.= natural_search("date_create",$search_date_create);
if ($search_date_mod) $sql.= natural_search("date_mod",$search_date_mod);
...
if ($search_fk_pu_structure != '') $params.= '&amp;search_fk_pu_structure='.urlencode($search_fk_pu_structure);
...
if ($search_date_create != '') $params.= '&amp;search_date_create='.urlencode($search_date_create);
...
if (! empty($arrayfields['t.fk_pu_structure']['checked'])) print_liste_field_titre($arrayfields['t.fk_pu_structure']['label'],$_SERVER['PHP_SELF'],'t.fk_pu_structure','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.date_create']['checked'])) print_liste_field_titre($arrayfields['t.date_create']['label'],$_SERVER['PHP_SELF'],'t.date_create','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_pu_structure']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_pu_structure" value="'.$search_fk_pu_structure.'" size="10"></td>';
...
if (! empty($arrayfields['t.date_create']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_date_create" value="'.$search_date_create.'" size="10"></td>';
...
if (! empty($arrayfields['t.statut']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_statut" value="'.$search_statut.'" size="10"></td>';
    ...
    while ($i < min($num, $limit))
    {
        ...
        if ($obj)
        {
            ...
            {
                print '<td>'.$obj->fk_budget.'</td>';
    		    if (! $i) $totalarray['nbfield']++;
            }
            if (! empty($arrayfields['t.fk_pu_structure']['checked'])) 
            {
                print '<td>'.$obj->fk_pu_structure.'</td>';
    		    if (! $i) $totalarray['nbfield']++;
            }
            if (! empty($arrayfields['t.fomula']['checked'])) 
            {
