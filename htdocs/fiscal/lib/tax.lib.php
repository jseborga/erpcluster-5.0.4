 ...
 */
function vat_by_date($db, $y, $q, $date_start, $date_end, $modetax, $direction, $m=0,$code_tva='')
{
	global $conf;
	...
	if ($direction == 'sell')
	{
		...
		$invoicedetfiscaltable='facturedet_fiscal';
	...
	if ($direction == 'buy')
	{
		...
		$invoicedetfiscaltable='facture_fourn_det_fiscal';
	...
	elseif($modetax == 2)
	{
		...
		//if ($direction == 'buy')
			$sql .=" fi.total_tva as total_localtax1, d.".$total_localtax2." as total_localtax2, ";
		//if ($direction == 'sell')
		//	$sql .=" fi.valimp1 as total_localtax1, d.".$total_localtax2." as total_localtax2, ";
		$sql.= " d.date_start as date_start, d.date_end as date_end,";
		$sql.= " f.".$invoicefieldref." as facnum, f.type, f.total_ttc as ftotal_ttc, f.datef, s.nom as company_name, s.rowid as company_id,";
		$sql.= " p.rowid as pid, p.ref as pref, p.fk_product_type as ptype,";
		$sql.= " 0 as payment_id, 0 as payment_amount";
		...
		$sql.= " INNER JOIN ".MAIN_DB_PREFIX.$invoicedettable." as d ON f.rowid = d.".$fk_facture ;
		$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."product as p on d.fk_product = p.rowid";
		if ($direction == 'buy')
			$sql.= " INNER JOIN ".MAIN_DB_PREFIX.$invoicedetfiscaltable." as fi on fi.fk_facture_fourn_det = d.rowid";
		if ($direction == 'sell')
			$sql.= " INNER JOIN ".MAIN_DB_PREFIX.$invoicedetfiscaltable." as fi on fi.fk_facturedet = d.rowid";

		$sql.= " WHERE f.entity = " . $conf->entity;
		if ($direction == 'buy')
			$sql.= " AND fi.code_tva = '".$code_tva."'";
		$sql.= " AND f.fk_statut in (1,2)"; // Validated or paid (partially or completely)
		if (! empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS)) $sql.= " AND f.type IN (0,1,2,5)";
		else $sql.= " AND f.type IN (0,1,2,3,5)";
		//$sql.= " AND f.rowid = d.".$fk_facture;
		...
		if ($code_tva) $sql.= " AND fi.code_tva = '".$code_tva."'";
		if ($y && $m)
		{
			$sql.= " AND f.datef >= '".$db->idate(dol_get_first_day($y,$m,false))."'";
			$sql.= " AND f.datef <= '".$db->idate(dol_get_last_day($y,$m,false))."'";
		...
		if ($date_start && $date_end) $sql.= " AND f.datef >= '".$db->idate($date_start)."' AND f.datef <= '".$db->idate($date_end)."'";
		$valorcero = 0;
		//$sql.= " AND ( d.product_type = ".$valorcero;
		                              // Limit to products
