...
/**
 * \file    monprojet/projetpaiement.class.php
 * \ingroup monprojet
 ...
 * @see CommonObject
 */
class Projetpaiement extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'projetpaiement';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'projet_paiement';

	/**
	 * @var ProjetpaiementLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_fourn_facture)) {
			 $this->fk_fourn_facture = trim($this->fk_fourn_facture);
		}
		...
		$sql .= ' '.(! isset($this->fk_fourn_facture)?'NULL':$this->fk_fourn_facture).',';
	 ...
	 */
	public function fetch($id, $ref = null,$fk_projet=0,$fk_soc=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		if (null !== $ref && $fk_projet >0 && $fk_soc >0) {
			$sql .= ' WHERE t.ref = ' . '\'' . $ref . '\'';
			$sql.= " AND t.fk_projet = ".$fk_projet;
			$sql.= " AND t.fk_soc = ".$fk_soc;
		} else {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fk_fourn_facture = $obj->fk_fourn_facture;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->fk_fourn_facture = $obj->fk_fourn_facture;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_fourn_facture = '.(isset($this->fk_fourn_facture)?$this->fk_fourn_facture:"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Projetpaiement($this->db);
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Paiement") . '</u>';
        $label.= '<div width="100%">';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;

        $link = '<a href="'.DOL_URL_ROOT.'/monprojet/paiement.php?id='.$this->fk_projet.'&idp='.$this->id.'"';
        ...
        if ($withpicto)
        {
            $result.=($link.img_picto(($notooltip?'':$label), DOL_URL_ROOT.'/monprojet/img/payment', ($notooltip?'':'class="classfortooltip"'),1).$linkend);
	...
	function LibStatut($status,$mode=0)
	{
		...
		if ($mode == 1)
		{
			if ($status == 1) return $langs->trans('Approved');
