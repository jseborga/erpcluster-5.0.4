 ...
 */
class FormProjetsext extends FormProjets
{
	/**
	 ...
	 */
	function select_projects_v($socid=-1, $selected='', $htmlname='projectid', $maxlength=16, $option_only=0, $show_empty=1, $discard_closed=0, $forcefocus=0, $disabled=0, $mode = 0, $filterkey = '')
	{
		global $langs,$conf;
		if (! empty($conf->use_javascript_ajax) && ! empty($conf->global->PROJECT_USE_SEARCH_TO_SELECT))
		{
			...
			print ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.'/monprojet/ajax/projects.php', $urloption, $conf->global->PROJECT_USE_SEARCH_TO_SELECT, 0, array());

			print '<input type="text" size="20" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="'.$selected_input_value.'"'.$placeholder.' />';
		}
		else
		{
			print $this->select_projects_listadd($socid, $selected, $htmlname, $maxlength, $option_only, $show_empty, $discard_closed, $forcefocus, $disabled, 0, $filterkey);
		}
	}
	 ...
	 *	@return int         			Nber of project if OK, <0 if KO
	 */
	function select_projects_listadd($socid=-1, $selected='', $htmlname='projectid', $maxlength=24, $option_only=0, $show_empty=1, $discard_closed=0, $forcefocus=0, $disabled=0, $mode = 0, $filterkey = '')
	{
		...
		{
			$projectstatic=new Projectext($this->db);
			$projectsListId = $projectstatic->getMonProjectsAuthorizedForUser($user,2,1);
		}
		// Search all projects
		...
		if ($resql)
		{
			...
			if (empty($option_only)) {
				$out.= '<select class="flat'.($minmax?' '.$minmax:'').' form-control"'.($disabled?' disabled="disabled"':'').' id="'.$htmlname.'" name="'.$htmlname.'"'.$nodatarole.'>';
			}
			if (!empty($show_empty)) {
			...
			if ($num)
			{
				...
				//echo '<hr>perm '.$socid.' '.$user->rights->societe->lire.'|'.$obj->fk_soc.'='.$socid;
					if ($socid > 0
						&& ! $user->rights->societe->lire
						&& (empty($obj->fk_soc) || $obj->fk_soc == $socid))
					{
				// Do nothing
					}
					else
	 ...
	 *	@return int         			Nber of project if OK, <0 if KO
	 */
	function selectTasks_v($socid=-1, $selected='', $htmlname='taskid', $maxlength=24, $option_only=0, $show_empty=1, $discard_closed=0, $forcefocus=0, $disabled=0,$filterprojet=0,$filtergroup=0,$viewcode=1)
	{
		global $user,$conf,$langs;
		...
		if ($filtergroup)
			$sql.= " INNER JOIN ".MAIN_DB_PREFIX."projet_task_add AS ta ON ta.fk_task = t.rowid AND ta.c_grupo = 0";
		$sql.= " WHERE p.entity = ".$conf->entity;
		$sql.= " AND t.fk_projet = p.rowid";
		if ($projectsListId !== false) $sql.= " AND p.rowid IN (".$projectsListId.")";
		if ($socid == 0) $sql.= " AND (p.fk_soc=0 OR p.fk_soc IS NULL)";
		if ($socid > 0)  $sql.= " AND (p.fk_soc=".$socid." OR p.fk_soc IS NULL)";
		if ($filterprojet) $sql.= " AND p.rowid = ".$filterprojet;
			...
			if ($num)
			{
				while ($i < $num)
				{
					...
					else
					{
						...
						if (!$filterprojet)
						{
							if ($viewcode) $labeltoshow=dol_trunc($obj->ref,18);
							$labeltoshow.=' '.dol_trunc($obj->title,$maxlength);
						...
						// Label for task
						if (!empty($labeltoshow)) $labeltoshow.=' - ';
						$labeltoshow.=($viewcode?$obj->tref:'').' '.dol_trunc($obj->tlabel,$maxlength);
	 ...
	 */
	function select_element_v($table_element, $socid=0, $morecss='')
	{
		global $conf, $langs;
	 ...
	 *    @return  int|string                      The HTML select list of element or '' if nothing or -1 if KO
	 */
	function selectOpportunityStatus_v($htmlname, $preselected=0, $showempty=1, $useshortlabel=0, $showallnone=0)
	{
		global $conf, $langs;
