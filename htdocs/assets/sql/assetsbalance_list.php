...
/**
 *   	\file       assets/assetsbalance_list.php
 *		\ingroup    assets
...
$search_status='';
    ...
    $permtoread = $user->rights->assetsbalance->read;
    $permtodelete = $user->rights->assetsbalance->delete;
    $uploaddir = $conf->assetsbalance->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."assets_balance as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."assets_balance_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/assetsbalance/card.php?id='.$id);
    exit;
}
