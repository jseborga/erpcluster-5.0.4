...
$search_saleprice=GETPOST('search_saleprice','alpha');
...
$arrayfields=array(
	...
	't.saleprice'=>array('label'=>$langs->trans("Fieldsaleprice"), 'align'=>'align="right"','checked'=>1),
	't.qty'=>array('label'=>$langs->trans("Fieldqty"), 'align'=>'align="center"', 'checked'=>1),
	't.price'=>array('label'=>$langs->trans("Fieldprice"), 'align'=>'align="right"', 'checked'=>1),
	't.subprice'=>array('label'=>$langs->trans("Fieldsubprice"), 'align'=>'align="right"', 'checked'=>0),
...
if ($search_saleprice) $sql.= natural_search("saleprice",$search_saleprice);
...
$massactionbutton=$form->selectMassAction('', $arrayofmassactions);
	...
	print '$(document).ready(function () {
		...
		$("#search_fk_product").change(function() {
...
//
if (! empty($arrayfields['t.fk_promotions']['checked'])) print_liste_field_titre($arrayfields['t.fk_promotions']['label'],$_SERVER['PHP_SELF'],'t.fk_promotions','',$params,$arrayfields['t.fk_promotions']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.saleprice']['checked'])) print_liste_field_titre($arrayfields['t.saleprice']['label'],$_SERVER['PHP_SELF'],'t.saleprice','',$params,$arrayfields['t.saleprice']['align'],$sortfield,$sortorder);

if (! empty($arrayfields['t.qty']['checked'])) print_liste_field_titre($arrayfields['t.qty']['label'],$_SERVER['PHP_SELF'],'t.qty','',$params,$arrayfields['t.qty']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.subprice']['checked'])) print_liste_field_titre($arrayfields['t.subprice']['label'],$_SERVER['PHP_SELF'],'t.subprice','',$params,$arrayfields['t.subprice']['align'],$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
//
if (! empty($arrayfields['t.fk_promotions']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_promotions']['align'].'><input type="text" class="flat" name="search_fk_promotions" value="'.$search_fk_promotions.'" size="10"></td>';
...
if (! empty($arrayfields['t.saleprice']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.saleprice']['align'].'><input type="text" class="flat" name="search_saleprice" value="'.$search_saleprice.'" size="10"></td>';
if (! empty($arrayfields['t.qty']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.qty']['align'].'><input type="text" class="flat" name="search_qty" value="'.$search_qty.'" size="10"></td>';
...
if (! empty($arrayfields['t.subprice']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.subprice']['align'].'><input type="text" class="flat" name="search_subprice" value="'.$search_subprice.'" size="10"></td>';
	...
	foreach ($arrayfields as $key => $value)
	{
		...
		if (!empty($arrayfields[$key]['checked']))
		{
			...
			if ($key2 == 'fk_product')
			{
				...
				print '<td>';
				$form->select_produits((GETPOST('fk_product')?GETPOST('fk_product'):$obj->$key2), 'fk_product', $filtertype='', $limit=0, $price_level=0, $status=1, $finished=2, $selected_input_value='', $hidelabel=1, $ajaxoptions=array(), $socid=0, $showempty='1', $forcecombo=0, $morecss='', $hidepriceinlabel=1, $warehouseStatus='');

				if ($fk_product > 0)
				{
					$res = $objProduct->fetch($fk_product);
					if ($res == 1) print $objProduct->label;
				}
				print '</td>';

			}
			elseif ($key2 == 'saleprice')
			{
				$fk_product = (GETPOST('fk_product')?GETPOST('fk_product'):$object->fk_product);
				if ($fk_product>0)
					$objProduct->fetch($fk_product);
				$obj->$key2 = price2num($objProduct->price_ttc,'MU');
				$price_ttc = price2num($objProduct->price_ttc,'MU');
				$obj->$key2 = '<input type="number" step="any" name="saleprice" value="'.$objProduct->price_ttc.'" readonly>';
			}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
			...
			foreach ($arrayfields as $key => $value)
			{
				if (!empty($arrayfields[$key]['checked']))
				{
					...
					if ($key2 == 'fk_product')
					{
						...
						print '<td>'.$objProduct->getNomUrl(1).' '.$objProduct->label;
					//	$form->select_produits((GETPOST('fk_product')?GETPOST('fk_product'):$obj->$key2), 'fk_product', $filtertype='', $limit=0, $price_level=0, $status=1, $finished=2, $selected_input_value='', $hidelabel=1, $ajaxoptions=array(), $socid=0, $showempty='1', $forcecombo=0, $morecss='', $hidepriceinlabel=1, $warehouseStatus='');
						print '</td>';
					...
					{
						$objProduct->fetch($obj->fk_product);
						$obj->$key2 = '<input type="number" name="saleprice" value="'.$objProduct->price_ttc.'" readonly>';
					}
					elseif ($key2 == 'status')
					{
						$objPromotiondet->status = $obj->$key2;
						$obj->$key2 = $objPromotiondet->getLibStatut(3);
					}
					elseif ($key2 == 'fk_user_create' || $key2 == 'fk_user_mod')
					{
						$res = $objUser->fetch($obj->$key2);
			...
			foreach ($arrayfields as $key => $value)
			{
				...
				//$key2 = str_replace('t.', '', $key);
					...
					if ($key2 == 'fk_product')
					{
						...
						$obj->$key2 = $objProduct->getNomUrl(1).' '.$objProduct->label;
					}
					...
					if ($key2 == 'ref')
					{
						...
						$object->idr = $idr;
						$objProduct->fetch($obj->$key2);
						$object->ref = $objProduct->ref;
						$object->label = $objProduct->label;
						$obj->$key2 = $object->getNomUrlext(1);
					}
					if ($key2 == 'active')
					{
						$img = 'switch_off';
					...
					if ($key2 == 'price' || $key2 == 'subprice' || $key2 == 'saleprice')
					{
						//$format = $value['align'];
						$formatNumber = true;
					}
					...
					print '<td '.$format.'>' . ($formatNumber?price(price2num($obj->$key2,'MT')):$obj->$key2) . '</td>';
					if (!$i)
						$totalarray['nbfield'] ++;
				}
			}
		...
		if ($action !='edit'){
			...
			{
				print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$idr.'&idrd='.$obj->rowid.'&action=edit">'.img_picto('','edit').'</a>';
				print '&nbsp;<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$idr.'&idrd='.$obj->rowid.'&action=delete">'.img_picto('','delete').'</a>';
			}
		}
		if ($action =='edit' && $idrd == $obj->rowid)
		{
...
if ($user->rights->productext->prom->write && (empty($action) || $action == 'view'))
{
	if (empty($objPromotion->status))
