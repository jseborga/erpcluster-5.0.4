...
class Pustructuredet extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->status_print_det)) {
			 $this->status_print_det = trim($this->status_print_det);
		}
		...
		$sql .= ' '.(! isset($this->status_print_det)?'NULL':$this->status_print_det).',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("pustructuredet", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->status_print_det = $obj->status_print_det;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->status_print_det = $obj->status_print_det;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' status_print_det = '.(isset($this->status_print_det)?$this->status_print_det:"null").',';
