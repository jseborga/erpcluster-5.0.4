...
require_once DOL_DOCUMENT_ROOT.'/purchase/class/fournisseur.factureadd.class.php';
...
require_once DOL_DOCUMENT_ROOT.'/purchase/class/html.formadd.class.php';
require_once DOL_DOCUMENT_ROOT.'/purchase/class/facturefourndetfiscaladd.class.php';
require_once DOL_DOCUMENT_ROOT.'/fiscal/class/html.formfadd.class.php';
...
$object=new FactureFournisseuradd($db);
$objtmp=new FactureFournisseuradd($db);
...
$objectdetfiscal = new Facturefourndetfiscaladd($db);
...
elseif ($action == 'confirm_delete_line' && $confirm == 'yes' && $user->rights->fournisseur->facture->creer)
{
	...
	$objectdetfiscal->fetch(0,GETPOST('lineid'));
	if ($objectdetfiscal->fk_facture_fourn_det == GETPOST('lineid'))
		$objectdetfiscal->delete($user);
	if ($ret > 0)
	{
...
// Create
elseif ($action == 'confirm_add' && $_REQUEST['confirm'] == 'yes' && $user->rights->purchase->inv->crear)
{
	$error=0;
	...
	if (empty(GETPOST('code_facture')))
	{
		setEventMessage($langs->trans('ErrorFieldRequired',$langs->transnoentities('Codefacture')), 'errors');
		$action='create';
		$_GET['socid']=$_POST['socid'];
		$error++;
	}
	...
	if (! $error)
	{
		...
		if ($id > 0)
		{
			...
			if (GETPOST('code_facture') =='STDC')
			{
				...
				$objectadd->nit = GETPOST('num_autoriz');
				$objectadd->nit = GETPOST('nfiscal');
				$objectadd->nit = GETPOST('cod_control');
			}
			else
			{
				$objectadd->nit = GETPOST('nit_');
				$objectadd->nit = GETPOST('num_autoriz_');
				$objectadd->nit = GETPOST('nfiscal_');
				$objectadd->nit = GETPOST('cod_control_');				
			}
			...
			if ($result > 0)
			{
				...
				for ($i = 0; $i < $num; $i++)
				{
					...
					{
							//echo '<hr>inidianco con '.$data->code_tva. ' '.$data->taux;
						$tvatx[$data->code_tva] = $data->taux;
						$nbase = 1;
						if ($productadd->fk_product == $lines[$i]->fk_product)
						{
						...
						{
								//incluye iva
							$pricetot = $lines[$i]->qty *$lines[$i]->subprice;
							$pricebase = $lines[$i]->qty *$lines[$i]->subprice * $nbase;
							...
							$lines[$i]->price = $lines[$i]->subprice;
						...
						else
						{
							...
							$priceht = $lines[$i]->qty * $lines[$i]->subprice;
							$pricebaseht = $lines[$i]->qty * $lines[$i]->subprice*$nbase;
							...
							$lines[$i]->price = $price / $lines[$i]->qty;
							$lines[$i]->pricebase = $pricebase;
							if (empty($data->taux))
								$lines[$i]->pricebase = 0;
						...
						else
						{
							...
							$lines[$i]->$campotx = $tvalc[$data->code_tva];
					...
					$aData = array($tvatx,$tvacalc,$tvaht,$tvattc);
					...
					foreach ((array) $tvacalc AS $code => $value)
					{
						...
						$objectdetfiscal->amount_base = $tvabase[$code];
		...
		$objtmp->fetch($id);
		$objtmp->tva = $aTotaltav[$id]['tva_tx'];
		$objtmp->total_tva = $aTotaltav[$id]['total_tva'];
		$objtmp->localtax1 = $aTotaltav[$id]['localtax1'];
		$objtmp->localtax2 = $aTotaltav[$id]['localtax2'];
		$objtmp->total_ht = $aTotaltav[$id]['total_ht'];
		$objtmp->total_ttc = $aTotaltav[$id]['total_ttc'];
		$resup = $objtmp->updatetot($user);
	}
	...
	if (GETPOST('prod_entry_mode') != 'free')	// With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
	{
		...
		if ($idprod > 0)
		{
			...
			foreach ((array) $tvaline AS $j => $data)
			{
				...
				if ($productadd->fk_product == $lines->fk_product)
				{
					$nbase = $productadd->percent_base/100;
					if (!$productadd->sel_iva && $data->code_tva == 'IVA')
					{
				...
				if (trim(strtoupper($type_price)) == 'TTC')
				{
					//incluye iva
					if ($remise_percent>0)
					{
						$pricetot = $qty * $pu;
						$pu = $pu-($pu * $remise_percent / 100);
						$pricetmp = $qty * $pu;
						$pricebase = $pricetmp * $nbase;
						$discount = $pricetot - $pricetmp;
						$pricetot = $qty * $pu;
			...
			foreach ((array) $tvacalc AS $code => $value)
			{
				...
				$objectdetfiscal->amount_ice = $amount_ice;
...
if ($action == 'create' || $action == 'addconf')
{
	...
	print '<form id="add" name="add" action="'.$_SERVER["PHP_SELF"].'" method="post">';
	...
	$typefilter = 0;
	if (empty($code_facture)) $code_facture = 'STDC';
	print $formf->load_type_facture('code_facture',(GETPOST('code_facture')?GETPOST('code_facture'):$code_facture),0, 'code', false,$typefilter);
	...
	print $form->load_type_purchase('code_type_purchase',(GETPOST('code_type_purchase')?GETPOST('code_type_purchase'):'STOCK'),0, 'code', false);
	print '</td></tr>';
	...
	// Ref supplier modificado para registro de factura bolivia
	//print '<tr><td class="fieldrequired">'.$langs->trans('RefSupplier').'</td><td><input name="ref_supplier" value="'.(isset($_POST['ref_supplier'])?$_POST['ref_supplier']:$ref_supplier).'" type="text"></td>';
	//print '</tr>';
	$fieldrequired = '';
	if ($code_facture == 'STDC') $fieldrequired = 'fieldrequired';
	// NIT
	print '<tr class="datfiscal"><td class="'.$fieldrequired.'">'.$langs->trans('Datos de la factura').'</td><td><input name="nit" value="'.(isset($_POST['nit'])?$_POST['nit']:$nit).'" type="text" placeholder="'.$langs->trans('NIT').'">';
	print '<input name="nfiscal" id="nfiscal" value="'.(isset($_POST['nfiscal'])?$_POST['nfiscal']:$nfiscal).'" type="text" placeholder="'.$langs->trans('Facturenumber').'" >';
	print '<input name="num_autoriz" id="num_autoriz" value="'.(isset($_POST['num_autoriz'])?$_POST['num_autoriz']:$num_autoriz).'" type="text" placeholder="'.$langs->trans('Numberautoriz').'">';
	print '<input name="cod_control" id="cod_control" value="'.(isset($_POST['cod_control'])?$_POST['cod_control']:$cod_control).'" type="text" placeholder="'.$langs->trans('Codcontrol').'">';
	print '</td>';
	print '</tr>';
	...
	*/
	...
	// Date invoice
	print '<tr><td class="fieldrequired">'.$langs->trans('DateToInvoice').'</td><td>';
	$form->select_date($dateinvoice,'','','','',"add",1,1);
	print ' <b>'.$langs->trans('Datefingering').'</b> '.dol_print_date(dol_now(),'dayhour');
	print '</td></tr>';
...
else
{
	if ($id > 0 || ! empty($ref))
	{
		...
		if ($objectadd->code_facture == 'STDC')
		{
			...
			print '<tr><td>'.$langs->trans('AmountVAT').'</td><td align="right">'.price(price2num($object->total_tva,'MU'),1,$langs,0,-1,-1,$conf->currency).'</td><td colspan="2" align="left">';
		...
		else
		{
			...
			foreach ($objectdetfiscal->aData AS $code_facture => $data)
			{
				if($code_facture != 'STDC')
				{
					...
					print '<tr><td>'.$objfacture->label.'</td><td align="right">'.price(price2num($data['total_tva'],'MU'),1,$langs,0,-1,-1,$conf->currency).'</td><td colspan="2" align="left">&nbsp;</td></tr>';
				}
			}
		...
		if ($objectadd->fk_facture_fourn == $object->id && $objectadd->code_facture == 'STDC')
		{
				//mostramos los datos adicionales
			print '<tr><td>'.$langs->trans('Base amount for tax credit').'</td><td align="right">'.price($objectadd->amountfiscal,1,$langs,0,-1,-1,$conf->currency).'</td><td colspan="2" align="left">&nbsp;</td></tr>';			
		...
		for ($i = 0; $i < $num; $i++)
		{
			if ($i == 0)
			{
				...
				print '<td align="right">'.$langs->trans('ICE').'</td>';
				if (!$conf->global->PURCHASE_PRICE_TAXES_INCLUDED)
			...
			//buscamos datos adicionales de det fiscal
			$objectdetfiscal->get_sum_taxesdet($object->lines[$i]->rowid);
			...
			print '</td>';
				...
				// VAT
			print '<td align="right">';
			//print $form->load_tvaadd('tauxtva',$object->lines[$i]->tva_tx,$societe,$mysoc);
			print '</td>';
			...
			if (!$conf->global->PURCHASE_PRICE_TAXES_INCLUDED)
				...
				print '<td align="right" class="nowrap"><input size="4" name="puttc" type="text" value="'.price($object->lines[$i]->pu_ttc).'"></td>';

			print '<td align="right"><input size="1" name="qty" type="text" value="'.$object->lines[$i]->qty.'"></td>';

			print '<td align="right" class="nowrap"><input size="1" name="remise_percent" type="text" value="'.(isset($object->lines[$i]->remise_percent)?($object->lines[$i]->remise_percent>0?$object->lines[$i]->remise_percent:''):'').'"><span class="hideonsmartphone">%</span> ';
			print '<input size="5" name="discount" type="text" value="'.$object->lines[$i]->discount.'"><span class="hideonsmartphone"></span></td>';
			print '<td align="right" class="nowrap">';
			print '<input size="5" name="amount_ice" type="text" value="'.$objectdetfiscal->aDatadet['IVA']['total_amountice'].'"></td>';
			...
			// Affichage simple de la ligne
		{
			...
			if ($object->lines[$i]->fk_product)
			{
				...
				print '<td align="right">'.(($objectdetfiscal->aDatadet['total_discount'] > 0)?price($objectdetfiscal->aDatadet['total_discount']):'').'</td>';

				print '<td align="right">'.(($objectdetfiscal->aDatadet['IVA']['total_amountice'] > 0)?price($objectdetfiscal->aDatadet['IVA']['total_amountice']):'').'</td>';
