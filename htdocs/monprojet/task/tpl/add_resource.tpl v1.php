...
if ($user->rights->monprojet->con->crear)
{
	...
	if($type_resource == 'MAText' || $type_resource == 'MODext' || $type_resource == 'MAQext')
	{
		...
		print $form->select_company_v($fk_soc,'fk_soc',$filtertype,0, 0, -1,-1,'',1, array(),0,'','');
		print '</td>';
	}
	elseif ($group_resource == 'MA' && $type_resource == 'MAT')
	{
		...
		$ajaxOptions = '';
		print $form->select_produits_projet($projectstatic->id,$fk_product_task,'product','',$conf->product->limit_size,0,-1,2,'',1,$ajaxOptions,'','','resource');
	...
	//.'<input id="ref_ext" type="text" name="ref_ext" value="" size="7">';
	print '</td>';

	if(($type_resource == 'MAText' || $type_resource == 'MODext' || $type_resource == 'MAQext') || $group_resource == 'MO' && $type_resource == 'MOD')
	{
		print '<td>';
