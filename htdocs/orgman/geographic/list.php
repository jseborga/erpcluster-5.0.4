...
dol_include_once('/orgman/class/cgeographic.class.php');
...
$object=new Cgeographic($db);
if (($id > 0 || ! empty($ref)) && $action != 'add')
{
	$result=$object->fetch($id,$ref);
	if ($result < 0) dol_print_error($db);
}
$objecttmp = new Cgeographic($db);
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/geographic/list.php',1);
	...
	if ($action == 'add')
	{
		...
		if (!empty($code_father)) $object->ref=$code_father.'.'.GETPOST('ref','alpha');
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/geographic/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Clasificador Geográfico');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_geographic as t";
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if ($action == 'edit' && $id == $obj->rowid)
		{
			...
			if (! empty($arrayfields['t.ref']['checked']))
			{
				print '<td nowrap>'.($obj->code_father?$obj->code_father.'.':'').'<input type="text" name="ref" value="'.substr($obj->ref,strlen($obj->code_father),30).'" size="3">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.sigla']['checked']))
			{
