...
/**
 *   	\file       assets/cassetsgroup_list.php
 *		\ingroup    assets
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-01-25 14:59
 */
...
dol_include_once('/assets/class/cassetsmethod.class.php');
...
// Get parameters
$id			= GETPOST('id','int');
$fk_method_dep	= GETPOST('fk_method_dep','int');
...
$hookmanager->initHooks(array('cassetsgrouplist'));
...
$objMethod = new Cassetsmethod($db);
$res = $objMethod->fetchAll('ASC','t.ref',0,0,array('active'=>1),'AND');
$options = '<option value="0">'.$langs->trans('Select').'</option>';
$aMethod = array();
if ($res>0)
{
	$lines = $objMethod->lines;
	foreach ((array) $lines AS $j => $line)
	{
		$selected = '';
		if ($line->id == $fk_method_dep) $selected = ' selected';
		$options.= '<option value="'.$line->id.'" '.$selected.'>'.$line->ref.' '.$line->label.'</option>';
		$aMethod[$line->id] = $line->code.' '.$line->label;
	}
}
$aActive = array(0=>$langs->trans('Deactivated'),1=>$langs->trans('Activated'));
...
$arrayfields=array(
	...
	't.depreciate'=>array('label'=>$langs->trans("Fielddepreciate"), 'checked'=>1),
	't.toupdate'=>array('label'=>$langs->trans("Fieldtoupdate"), 'checked'=>1),
	't.account_accounting'=>array('label'=>$langs->trans("Fieldaccount_accounting"), 'checked'=>1),
	't.account_spending'=>array('label'=>$langs->trans("Fieldaccount_spending"), 'checked'=>1),
...
// All test are required to be compatible with all browsers
{
	...
	$search_label='';
	$search_useful_life='';
	$search_fk_method_dep=-1;
	$search_account_accounting='';
	$search_account_spending='';
	$search_active=-1;
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		...
		$object->depreciate=GETPOST('depreciate','int');
		$object->toupdate=GETPOST('toupdate','int');
		...
		if (empty($object->useful_life))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Useful_life")), null, 'errors');
		}
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/assets/group/list.php',1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/assets/group/list.php',1));
			exit;
		}
		else
		{
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_assets_group as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_assets_group_extrafields as ef on (u.rowid = ef.fk_object)";
//$sql.= " WHERE 1 = 1";
$sql.= " WHERE t.entity IN (".getEntity('c_assets_group',1).")";
...
if ($search_fk_method_dep>=0) $sql.= natural_search("fk_method_dep",$search_fk_method_dep);
if ($search_account_accounting) $sql.= natural_search("account_accounting",$search_account_accounting);
if ($search_account_spending) $sql.= natural_search("account_spending",$search_account_spending);
if ($search_active>=0) $sql.= natural_search("active",$search_active);
...
if ($resql)
{
	...
	if ($search_useful_life != '') $params.= '&amp;search_useful_life='.urlencode($search_useful_life);
	if ($search_fk_method_dep != '') $params.= '&amp;search_fk_method_dep='.urlencode($search_fk_method_dep);
	if ($search_account_accounting != '') $params.= '&amp;search_account_accounting='.urlencode($search_account_accounting);
	if ($search_account_spending != '') $params.= '&amp;search_account_spending='.urlencode($search_account_spending);
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Deletegroup'), $langs->trans('ConfirmDeletegroup'), 'confirm_delete', '', 0, 1);
		print $formconfirm;
	}
	...
	if (! empty($arrayfields['t.depreciate']['checked'])) print_liste_field_titre($arrayfields['t.depreciate']['label'],$_SERVER['PHP_SELF'],'t.depreciate','',$params,'',$sortfield,$sortorder);
	if (! empty($arrayfields['t.toupdate']['checked'])) print_liste_field_titre($arrayfields['t.toupdate']['label'],$_SERVER['PHP_SELF'],'t.toupdate','',$params,'',$sortfield,$sortorder);
	...
	if (! empty($arrayfields['t.account_spending']['checked'])) print_liste_field_titre($arrayfields['t.account_spending']['label'],$_SERVER['PHP_SELF'],'t.account_spending','',$params,'',$sortfield,$sortorder);
	...
	//
		...
		if (! empty($arrayfields['t.fk_method_dep']['checked']))
		{
			...
			print $form->selectarray('search_fk_method_dep',$aMethod,$search_fk_method_dep,1);
			print '</td>';
		}
		if (! empty($arrayfields['t.depreciate']['checked'])) print '<td class="liste_titre"></td>';
		if (! empty($arrayfields['t.toupdate']['checked'])) print '<td class="liste_titre"></td>';
		...
		if (! empty($arrayfields['t.account_spending']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_account_spending" value="'.$search_account_spending.'" size="10"></td>';
		if (! empty($arrayfields['t.active']['checked']))
		{
			print '<td class="liste_titre">';
			print $form->selectarray('search_active',$aActive,$search_active,1);
			print '</td>';
		}
	...
	if ($action=='create' || $action == 'edit')
	{
		...
		include DOL_DOCUMENT_ROOT.'/assets/tpl/add_line.tpl.php';
	}
	...
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			// LIST_OF_TD_FIELDS_LIST
				...
				if (! empty($arrayfields['t.useful_life']['checked']))
				{
					print '<td>'.$obj->useful_life.'</td>';
					if (! $i) $totalarray['nbfield']++;
				}
				if (! empty($arrayfields['t.fk_method_dep']['checked']))
				{
					$obj->fk_method_dep;
					$res = $objMethod->fetch($obj->fk_method_dep);
					print '<td>'.$objMethod->label.'</td>';
					if (! $i) $totalarray['nbfield']++;
				}
				if (! empty($arrayfields['t.depreciate']['checked']))
				{
					$img = 'switch_off';
					if ($obj->depreciate) $img = 'switch_on';
				...
				{
					$img = 'switch_off';
					if ($obj->toupdate) $img = 'switch_on';
					print '<td>'.img_picto('',$img).'</td>';
					if (! $i) $totalarray['nbfield']++;
				}

				if (! empty($arrayfields['t.account_accounting']['checked']))
				{
					print '<td>'.$obj->account_accounting.'</td>';
					if (! $i) $totalarray['nbfield']++;
				}
				if (! empty($arrayfields['t.account_spending']['checked']))
				{
					print '<td>'.$obj->account_spending.'</td>';
					if (! $i) $totalarray['nbfield']++;
				}
				if (! empty($arrayfields['t.active']['checked']))
				{
...
$parameters=array('sql' => $sql);
	...
	if ($user->rights->assets->teach->creer)
		print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=create">'.$langs->trans("Create").'</a>';
	else
