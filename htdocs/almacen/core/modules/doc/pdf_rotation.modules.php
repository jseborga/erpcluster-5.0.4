 ...
 */
class pdf_rotation extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "rotation";
		$this->description = $langs->trans('PDFRotationDescription');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->almacen->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->almacen->dir_output . "/" . $objectref.'/'.$period_year.'/rotation';
				$file = $dir . "/" . 'rotation'.'_'.date('Ym') . ".pdf";
			...
			if (file_exists($dir))
			{
				...
				foreach ((array) $aRotation AS $i => $lines)
				{
					...
					$pdf->MultiCell($this->posxtro-$this->posxuni-1, 3, $unit, 0, 'C',0);

					// Residence time
					$timePerm=$lines['timePerm'];
					$pdf->SetXY($this->posxtro, $curY);
					$pdf->MultiCell($this->posxdat-$this->posxtro-1, 3, $timePerm, 0, 'R',0);

					// fecha final
					$datem=$lines['datem'];
					$pdf->SetXY($this->posxdat, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxdat-1, 3, dol_print_date($datem,'day'), 0, 'C',0);
	...
	*/
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxuni-$this->posxdesc-1,2, $outputlangs->transnoentities("Detalle"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxtro-$this->posxuni-1,2, $outputlangs->transnoentities("Unidad"),'','C');
		}

		if (empty($hidetop))
		{
			$pdf->line($this->posxtro-1, $tab_top, $this->posxtro-1, $tab_top + $tab_height);
			$pdf->SetXY($this->posxtro-1, $tab_top+1);
			$pdf->MultiCell($this->posxdat-$this->posxtro-1,2, $outputlangs->transnoentities("Tiempo de permanencia (días)"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxdat,2, $outputlangs->transnoentities("Fecha última salida"),'','C');
		}
	...
	*/
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Reporte de Rotación de Materiales");
		...
		$pdf->SetXY($posxx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Reportdate")." : " . dol_print_date(dol_now(),"day",false,$outputlangs), '', 'R');
		$posy+=1;
