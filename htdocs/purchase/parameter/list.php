...
/**
 *   	\file       purchase/ctypepurchase_list.php
 *		\ingroup    purchase
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-07 11:31
 */
...
dol_include_once('/purchase/class/ctypepurchase.class.php');
...
$hookmanager->initHooks(array('ctypepurchaselist'));
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if ( $user->rights->purchase->teach->write)
		{
			if (GETPOST('cancel'))
			{
			...
			if (! $error)
			{
				...
				{
				// Creation OK
					$urltogo=$backtopage?$backtopage:dol_buildpath('/purchase/parameter/list.php',1);
					header("Location: ".$urltogo);
					exit;
				}
				{
	...
	if ($action == 'active')
	{
		...
		{
			$error=0;
			$object->active = ($object->active>0?0:1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/purchase/parameter/list.php',1));
			exit;
		}
		else
		{
			setEventMessages($langs->trans('It is not possible to delete the record since it is related to one or more tables'),null,'errors');
			//if (! empty($object->errors)) setEventMessages(null,$object->errors,'errors');
			//else setEventMessages($object->error,null,'errors');
		}
	}
...
$help_url='';
$title = $langs->trans('Purchasedestination');
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_purchase_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($resql)
{
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Deletepurchasedestination'), $langs->trans('ConfirmDeletepurchasedestination').' '.$object->code.' '.$object->label, 'confirm_delete', '', 0, 1);
		print $formconfirm;
	}
	...
	if ($action == 'create')
	{
		$newdata = new Ctypepurchase($db);
		include DOL_DOCUMENT_ROOT.'/purchase/parameter/tpl/object_line_create.tpl.php';
	}
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			{
				$newdata = $obj;
				include DOL_DOCUMENT_ROOT.'/purchase/parameter/tpl/object_line_edit.tpl.php';
			}
			else
			{
			...
			// LIST_OF_TD_FIELDS_LIST
				...
				if (! empty($arrayfields['t.active']['checked']))
				{
					print '<td>';
					if ($user->rights->purchase->teach->write)
						print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$obj->rowid.'&action=active">';
					if ($obj->active>0)
					{
					...
					if ($user->rights->purchase->teach->write) print '</a>';
					print '</td>';
					if (! $i) $totalarray['nbfield']++;
				}
			// Extra fields
			...
			if ($action != 'edit' && $action != 'create')
			{
				...
				if ($user->rights->purchase->teach->del)
	...
	if (empty($reshook))
	{
		if ($user->rights->purchase->teach->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=create">'.$langs->trans("Create").'</a></div>'."\n";
		}
	}
	print '</div>'."\n";
