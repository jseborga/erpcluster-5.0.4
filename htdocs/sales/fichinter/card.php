...
if ($action == 'create')
{
	...
	if ($socid > 0)
	{
        ...
        if (! empty($origin) && ! empty($originid) && is_object($objectsrc))
        {
        	...
        	print '<tr><td>'.$langs->trans($newclassname).'</td><td colspan="2">'.$objectsrc->getNomUrladd(1).'</td></tr>';
        }
		...
		if ($user->societe_id == 0)
		{
            if ($action != 'editdescription' && ($action != 'presend')) {
				...
				if (! empty($conf->propal->enabled) && $object->statut > 0)
				{
					...
					if ($object->statut < 2)
					{
						if ($user->rights->propal->creer) print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/sales/propal/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans("AddProp").'</a></div>';
				...
				if (! empty($conf->facture->enabled) && $object->statut > 0)
				{
					...
					if ($object->statut < 2)
					{
						if ($user->rights->facture->creer) print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/sales/compta/facture.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans("AddBill").'</a></div>';
