...
$fk = GETPOST('fk');
$WS_DOL_URL = DOL_MAIN_URL_ROOT.'/wsassist/server_assistance.php';
//$WS_DOL_URL = 'http://localhost:8080/';	// To test with Soapui mock. If not a page, should end with /
if (empty($nameservice))
	$WS_METHOD0  = 'createassistance';
else
	$WS_METHOD1  = $nameservice;
...
if (count($aArray)>0)
{
	foreach ($aArray AS $j => $data)
	{
		if (!empty($data))
		{
			...
			//vamos a armar la fecha
			$aDatenew = explode('/',$aTmp[1]);
			$aHournew = explode(':',$aTmp[2]);
			$date_ass = dol_mktime($aHournew[0],$aHournew[1],$aHournew[2],$aDatenew[1],$aDatenew[0],$aDatenew[2]);
			$line = array(
				"id" => $a,
				"fk_contact" => 0,
				"fk_member" => $aTmp[0],
				...
				"fk_user_mod" => $fk_user,
				"manual_reg" => 3,
				"marking_number" => $aTmp[3],
				"datec" => date("Y-m-d h:i:sa"),
				"datem" => date("Y-m-d h:i:sa"),
...
if ($WS_METHOD0)
{
	...
	if ($lGet)
	{
		...
		$result = array('nreg'=>$nreg,'error'=>$result0['result']['result_code']);
