...
class TaskAdd extends Task
{
   ...
   */
     ...
     */
  function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lRow=false)
    {
  ...
  /**
     ...
     */
  function getTasksArray($usert=0, $userp=0, $projectid=0, $socid=0, $mode=0, $filteronprojref='', $filteronprojstatus=-1, $morewherefilter='',$filteronprojuser=0,$filterontaskuse=0,$limit=0,$ofset=1,$filterdate=0,$modepay=0)
    {
      global $conf,$langs;
	...
	if ($mode != 4)
	  {
	    ...
	    $sql.= " g.fk_contrat, g.c_grupo, g.fk_unit, g.fk_type, g.fk_item, g.unit_program, g.unit_declared, g.unit_ejecuted, g.unit_amount, g.detail_close";
	  }
	else
	  {
	    $sql = "SELECT p.rowid as projectid, p.ref, p.title as plabel, p.public, p.fk_statut,";
	...
	if ($mode == 4)
        ...
        elseif ($mode == 2 || $mode == 3)//lista relacionada con los requerimientos aprobados
	  {
	    ...
	    $sql.= " INNER JOIN ".MAIN_DB_PREFIX."request AS r ON ri.fk_request = r.rowid";
        ...
        elseif ($mode == 4)//lista relacionada con los requerimientos aprobados
	  {
            $sql.= " FROM ".MAIN_DB_PREFIX."projet as p";
            $sql.= " INNER JOIN ".MAIN_DB_PREFIX."projet_task as t on t.fk_projet = p.rowid";
            $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."projet_task_add as g ON g.fk_task = t.rowid" ;
        ...
        if ($resql)
        {
                ...
                if (! $error)
		  {
			...
			if (empty($this->date_ini))
			  ...
			  if ($this->db->jdate($obj->date_start) <= $this->date_ini)
			    $this->date_ini = $this->db->jdate($obj->date_start);
			//verificamos la fecha maxima
			if (empty($this->date_end))
			  $this->date_end = $this->db->jdate($obj->date_end);
			else
			  if ($this->db->jdate($obj->date_end) >= $this->date_end)
			    $this->date_end = $this->db->jdate($obj->date_end);
			...
			if ($mode == 2 || $mode == 3)
			  {
			    ...
			    $objtimedoc->getsum($obj->taskid,0,1,$obj->requestitemid);//(idTask,fk_payment,$statut)
