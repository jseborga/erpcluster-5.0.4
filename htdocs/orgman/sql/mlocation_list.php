...
/**
 *   	\file       orgman/mlocation_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-04-19 10:51
 */
...
dol_include_once('/orgman/class/mlocation.class.php');
...
$search_safety=GETPOST('search_safety','int');
...
't.safety'=>array('label'=>$langs->trans("Fieldsafety"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->mlocation->read;
    $permtodelete = $user->rights->mlocation->delete;
    $uploaddir = $conf->mlocation->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."m_location_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_safety) $sql.= natural_search("safety",$search_safety);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/mlocation/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.safety']['checked'])) print_liste_field_titre($arrayfields['t.safety']['label'],$_SERVER['PHP_SELF'],'t.safety','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.safety']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_safety" value="'.$search_safety.'" size="10"></td>';
