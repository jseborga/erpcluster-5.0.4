...
$selected		= GETPOST('shipping_to_invoice');
...
if ($action == 'create')
{
	...
	{
		$error++;
		setEventMessages($langs->trans('Error_ExpeditionNotChecked'), null, 'errors');
	}
	else
	{
...
if (($action == 'create' || $action == 'add') && !$error)
{
	...
	$langs->load('products');
	$langs->load('main');
	if (isset($_GET['shipping_to_invoice']))
	{
		$orders_id = $_GET['shipping_to_invoice'];
		$n        = count($orders_id);
		$i        = 0;
	...
	if (isset($_POST['shipping_to_invoice']))
	{
		$orders_id = $_POST['shipping_to_invoice'];
		$nn        = count($orders_id);
		$ii        = 0;
...
if ($action == 'create' && !$error)
{
	...
	include_once DOL_DOCUMENT_ROOT.'/sales/class/expeditionext.class.php';
	
	$objectsrc = new ExpeditionLigneext($db);
	$listoforders = array();
	foreach ($selected as $sel)
	{
		...
		if ($result > 0)
		{
			$objectsr = new Expedition($db);
			$objectsr->fetch($sel->fk_expedition);
			$listoforders[] = array('ref'=>$objectsr->ref,'rowid'=>$sel->fk_expedition);
		}
	}
	...
	while ($i < $n)
	{
		print '<input type="hidden" name="shipping_to_invoice[]" value="'.$orders_id[$i].'">';
		$i++;
	}
...
if (($action != 'create' && $action != 'add') || ($action == 'create' && $error))
{
	...
	$sql = 'SELECT s.nom, s.rowid as socid, s.client, c.rowid, ed.rowid AS edrowid, c.ref, c.ref_ext,';
	$sql.= ' c.date_valid, c.date_creation, c.date_delivery, c.fk_statut, c.rowid AS erowid';
	$sql.= " , cd.qty ";
	$sql.= " , p.label ";
	$sql.= ' FROM '.MAIN_DB_PREFIX.'societe as s';
	$sql.= ', '.MAIN_DB_PREFIX.'expedition as c';
	$sql.= ', '.MAIN_DB_PREFIX.'expeditiondet as ed ';
	$sql.= ', '.MAIN_DB_PREFIX.'expeditiondet_add as eda ';
	$sql.= ', '.MAIN_DB_PREFIX.'commandedet as cd ';
	$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'product as p ON cd.fk_product = p.rowid ';
	if (!$user->rights->societe->client->voir && !$socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
	$sql.= ' WHERE c.fk_soc = s.rowid';
	$sql.= " AND ed.fk_expedition = c.rowid ";
	$sql.= " AND ed.fk_origin_line = cd.rowid ";
	$sql.= " AND eda.fk_expeditiondet = ed.rowid ";
	// Show orders with status validated, shipping started and delivered (well any order we can bill)
	$sql.= " AND c.fk_statut IN (1,2) ";
	$sql.= " AND eda.status IN (0) ";
	...
	//Date filter
	if ($date_start && $date_end) $sql.= " AND c.date_creation >= '".$db->idate($date_start)."' AND c.date_creation <= '".$db->idate($date_end)."'";
	if ($date_starty && $date_endy) $sql.= " AND c.date_delivery >= '".$db->idate($date_starty)."' AND c.date_delivery <= '".$db->idate($date_endy)."'";
	...
	if ($resql)
	{
		...
		print_liste_field_titre($langs->trans('Product'),$_SERVER["PHP_SELF"],'p.label','','&amp;socid='.$socid,'',$sortfield,$sortorder);
		print_liste_field_titre($langs->trans('Qty'),$_SERVER["PHP_SELF"],'ed.qty','','&amp;socid='.$socid,'',$sortfield,$sortorder);
		print_liste_field_titre($langs->trans('Datecreation'),$_SERVER["PHP_SELF"],'c.date_commande','','&amp;socid='.$socid, 'align="center"',$sortfield,$sortorder);
		...
		print '<form method="get" action="shippingtoinvoice.php">';
		...
		print '<form name="orders2invoice" action="shippingtoinvoice.php" method="GET">';
		$var=true;
		...
		while ($i < $num)
		{
			...
			$generic_commande->id=$objp->erowid;
			$generic_commande->ref=$objp->ref;
			$generic_commande->statut = $objp->fk_statut;
			$generic_commande->date_creation = $db->jdate($objp->date_creation);
			$generic_commande->date_delivery = $db->jdate($objp->date_delivery);
			...
			print '<input class="flat checkformerge" type="checkbox" name="shipping_to_invoice[]" value="'.$objp->edrowid.'">';
			print '</td>' ;
		...
		*/
		...
		print '<input type="hidden" name="origin" value="expedition"><br>';
