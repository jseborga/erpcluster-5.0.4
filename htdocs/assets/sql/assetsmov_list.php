...
$search_status='';
    ...
    $permtoread = $user->rights->assetsmov->read;
    $permtodelete = $user->rights->assetsmov->delete;
    $uploaddir = $conf->assetsmov->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."assets_mov_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/assetsmov/card.php?id='.$id);
    exit;
}
