...
print '<form name="fiche_dev" action="'.$_SERVER['PHP_SELF'].'?ida='.$ida.'" method="post">';
...
print '<input type="hidden" name="fk_prev" value="'.$id.'">';
...
{
	print '<tr>';
	print '<td colspan="7">'.$langs->trans('Details partidas').'</td>';
	print '</tr>';
	print '<tr>';
	...
	foreach ($aObjcomp[$gestion]->array AS $icomp => $objComppart)
	{
		...
		{
			print  '<td class="col-xs-2" align="right">'.'<input id="amoun" class="form-control" type="number" min="0" step="any" name="amount['.$icomp.']" value="'.$saldoPartida.'" ></td>';
			print  '<input type="hidden" name="partida['.$icomp.']" value="'.$objComppart->partida.'">';
...
if ($user->admin)
{
	...
	print '<input type="number" id="nro_dev" class="form-control input-normal" name="nro_dev" value="'.$objdev->nro_dev.'" maxlength="5">'.'/'.'<input type="number" id="gestion" class="form-control input-normal" name="gestion" value="'.(empty($objdev->gestion)?date('Y'):$objdev->gestion).'"  maxlength="4">';
	print '</span>';
}
else
{
