...
/**
 * \file    budget/budgettaskproduction.class.php
 * \ingroup budget
...
/**
 * Class Budgettaskproduction
 *
 * Put here description of your class
 *
 * @see CommonObject
 */
class Budgettaskproduction extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'budgettaskproduction';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'budget_task_production';

	/**
	 * @var BudgettaskproductionLine[] Lines
	 */
	public $lines = array();
	...
	/**
	 * [fetch busqueda de registro]
	 * @param  [type]  $id                [id principal del registro]
	 * @param  integer $fk_budget_task    [id de la tarea o item]
	 * @param  integer $fk_variable       [id de la variable]
	 * @param  integer $fk_product_budget [id del producto de la tabla product_budget]
	 * @return [type]                     [1=si existe 0=vacio, -1= error]
	 */
	public function fetch($id, $fk_budget_task=0,$fk_variable=0,$fk_product_budget=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		$sql.= ' WHERE 1 = 1';
		if ($fk_budget_task>0 && $fk_variable>0 && $fk_product_budget > 0) {
			$sql .= ' AND t.fk_budget_task = ' . $fk_budget_task;
			$sql .= ' AND t.fk_variable = ' . $fk_variable;
			$sql .= ' AND t.fk_product_budget = ' . $fk_product_budget;
		} else {
			$sql .= ' AND t.rowid = ' . $id;
		}
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("budgettaskproduction", 1) . ")";
		}
		if (count($sqlwhere) > 0) {
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new BudgettaskproductionLine();
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Budgettaskproduction($this->db);
...
/**
 * Class BudgettaskproductionLine
 */
class BudgettaskproductionLine
{
	/**
