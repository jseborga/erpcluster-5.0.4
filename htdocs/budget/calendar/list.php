...
$hookmanager->initHooks(array('calendarlist'));
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/budget/calendar/list.php',1));
			exit;
		}
		else
		{
...
$sql.= " FROM ".MAIN_DB_PREFIX."calendar as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."calendar_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($resql)
{
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteCalendar'), $langs->trans('ConfirmDeleteCaLendar'), 'confirm_delete', '', 0, 1);
		print $formconfirm;
	}
	...
	if ($action == 'create')
	{
		...
		{
			print '<td>'.'<input type="text" name="ref" value="'.$obj->ref.'" required></td>';
		...
		print '<td nowrap>';
		print '<button class="butAction" type="submit" name="save">'.img_picto('',DOL_URL_ROOT.'/budget/img/save','',1).'</button>';
		print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'">'.$langs->trans('Return').'</a>';
		print '</td>';
		print '</tr>';
	}
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			if ($action == 'edit')
			{
				print '<button type="submit" name="save">'.img_picto('',DOL_URL_ROOT.'/budget/img/save','',1).'</button>';
			}
			else
			{
	...
	if ($user->rights->budget->tea->crear && empty($action))
