...
class pdf_contador extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function __construct($db)
	{
		...
		$langs->load("main");
		$langs->load("bills");
		$titulo = "CONTADOR DEPRESIACION DE ACTIVOS";
		$this->db = $db;
		$this->name = "contador";
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->assets->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->assets->dir_output."/assets/".$period_year.'/contador/'.$aValores[1];
				$file = $dir . "/contador".$aValores[1].".pdf";
			...
			if (file_exists($dir))
			{
				...
				$tab_top = 50;
				$tab_top_newpage = (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)?50:10);//38
				$tab_height = 50;
				//130
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					$pdf->MultiCell($this->posxmese-$this->posxini-1, 1, $line['mes'], 0, 'C',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxmese, $curY);
					$pdf->MultiCell($this->posxanio-$this->posxmese-1, 1, $line['anio'], 0, 'C',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxanio, $curY);
					$pdf->MultiCell($this->posxdepe-$this->posxanio-1, 1, price(price2num($line['dep_per'],'MT')), 0, 'R',0);	
					$aPosY[] =$pdf->GetY();
					$pdf->SetXY($this->posxdepe, $curY);
					$pdf->MultiCell($this->posxdeva-$this->posxdepe-1, 1, price(price2num($line['dep_acu'],'MT')), 0, 'R',0);	
					
					$pdf->SetXY($this->posxdeva, $curY);
					$pdf->MultiCell($this->posxfin-$this->posxdeva-1, 1, price(price2num($line['saldo'],'MT')), 0, 'R',0);	
			...
			if ($pagenb == 1)
			{
				$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforfooter-5, 0, $outputlangs, 0, 0);
				$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxmese-$this->posxini-1,2, $outputlangs->transnoentities("Month"),'','C');
			$pdf->line($this->posxmese-1, $tab_top, $this->posxmese-1, $tab_top + $tab_height);
			
			$pdf->SetXY($this->posxmese-1, $tab_top+2);
			$pdf->MultiCell($this->posxanio-$this->posxmese-1,2, $outputlangs->transnoentities("Year"),'','C');
			$pdf->line($this->posxanio-1, $tab_top, $this->posxanio-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxanio-1, $tab_top+2);
			$pdf->MultiCell($this->posxdepe-$this->posxanio-1,2, $outputlangs->transnoentities("Depreciationperiod"),'','C');
			$pdf->line($this->posxdepe-1, $tab_top, $this->posxdepe-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxdepe-1, $tab_top+2);
			$pdf->MultiCell($this->posxdeva-$this->posxdepe-1,2, $outputlangs->transnoentities("Accumulateddepreciation"),'','C');
			$pdf->line($this->posxdeva-1, $tab_top, $this->posxdeva-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxdeva-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxdeva-1,2, $outputlangs->transnoentities("Balance"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$titulo = "DEPRECIACION DE ACTIVOS";
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Code')." : " . $aValores[1], '', 'R');
		
		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Group')." : " . $aValores[2], '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Detail')." : " . $aValores[3], '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(207, 3, $langs->trans('Coste')." : " . $aValores[4], '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Costeresidual')." : " . $aValores[5], '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Warranty')." : " . $aValores[6], '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Depreciationfactor")." : " . $aValores[7], '', 'R');
		
		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Status')." : " . $aValores[8], '', 'R');
	}
