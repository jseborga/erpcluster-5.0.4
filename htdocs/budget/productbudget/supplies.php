...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		...
		if (empty($object->ref) && $object->fk_product>0)
		{
			//buscamos el producto y reemplazamos ref por objProduct->ref;
			$resprod = $objProduct->fetch($object->fk_product);
			if ($resprod==1)
...
if ($id && (empty($action) || $action == 'view' || $action == 'delete' || $action == 'verupdate'))
{
	...
	if ($id && $idr)
	{
		if ($idrd && ($action == 'create' || $action == 'edit' || $action == 'delete' || $action == 'validate'))
			include DOL_DOCUMENT_ROOT.'/budget/productbudget/tpl/budgettaskproduct_card.tpl.php';
		else
			include DOL_DOCUMENT_ROOT.'/budget/productbudget/tpl/budgettaskproduct_list.tpl.php';
	}
