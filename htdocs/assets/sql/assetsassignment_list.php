...
/**
 *   	\file       assets/assetsassignment_list.php
 *		\ingroup    assets
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-04-07 12:16
 */
...
dol_include_once('/assets/class/assetsassignment.class.php');
...
$search_fk_projet_from=GETPOST('search_fk_projet_from','int');
$search_fk_user_from=GETPOST('search_fk_user_from','int');
$search_fk_property_from=GETPOST('search_fk_property_from','int');
...
$search_type_assignment=GETPOST('search_type_assignment','alpha');
...
't.fk_projet_from'=>array('label'=>$langs->trans("Fieldfk_projet_from"), 'checked'=>1),
't.fk_user_from'=>array('label'=>$langs->trans("Fieldfk_user_from"), 'checked'=>1),
't.fk_property_from'=>array('label'=>$langs->trans("Fieldfk_property_from"), 'checked'=>1),
...
't.type_assignment'=>array('label'=>$langs->trans("Fieldtype_assignment"), 'checked'=>1),
...
$object=new Assetsassignment($db);
...
$search_fk_user='';
$search_fk_projet='';
$search_fk_projet_from='';
$search_fk_user_from='';
$search_fk_property_from='';
$search_fk_user_to='';
$search_fk_property='';
$search_fk_location='';
$search_detail='';
$search_type_assignment='';
$search_mark='';
...
$search_status='';
    ...
    $permtoread = $user->rights->assetsassignment->read;
    $permtodelete = $user->rights->assetsassignment->delete;
    $uploaddir = $conf->assetsassignment->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."assets_assignment_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_projet_from) $sql.= natural_search("fk_projet_from",$search_fk_projet_from);
if ($search_fk_user_from) $sql.= natural_search("fk_user_from",$search_fk_user_from);
if ($search_fk_property_from) $sql.= natural_search("fk_property_from",$search_fk_property_from);
if ($search_fk_user_to) $sql.= natural_search("fk_user_to",$search_fk_user_to);
if ($search_fk_property) $sql.= natural_search("fk_property",$search_fk_property);
...
if ($search_type_assignment) $sql.= natural_search("type_assignment",$search_type_assignment);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/assetsassignment/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.fk_projet_from']['checked'])) print_liste_field_titre($arrayfields['t.fk_projet_from']['label'],$_SERVER['PHP_SELF'],'t.fk_projet_from','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_property_from']['checked'])) print_liste_field_titre($arrayfields['t.fk_property_from']['label'],$_SERVER['PHP_SELF'],'t.fk_property_from','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.type_assignment']['checked'])) print_liste_field_titre($arrayfields['t.type_assignment']['label'],$_SERVER['PHP_SELF'],'t.type_assignment','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_projet_from']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_projet_from" value="'.$search_fk_projet_from.'" size="10"></td>';
...
if (! empty($arrayfields['t.fk_property_from']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_property_from" value="'.$search_fk_property_from.'" size="10"></td>';
...
if (! empty($arrayfields['t.type_assignment']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_type_assignment" value="'.$search_type_assignment.'" size="10"></td>';
