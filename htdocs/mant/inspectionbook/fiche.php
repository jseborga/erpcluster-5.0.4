...
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->mant->teacher->del)
{
  ...
  if ($result > 0)
    {
      header("Location: ".DOL_URL_ROOT.'/mant/departament/liste.php');
      exit;
    }
  else
    {
...
if ($action == 'create' && $user->rights->mant->teacher->crear)
  {
    ...
    print $object->select_departament($object->fk_father,'fk_father','','',1);
    print '</td></tr>';
 ...
 else
   {
      ...
      if ($action <> 'edit' && $action <> 're-edit')
	{
	  ...
	  dol_fiche_head($head, 'card', $langs->trans("Departament"), 0, 'mant');
	  ...
	  $linkback = '<a href="'.DOL_URL_ROOT.'/mant/departament/liste.php">'.$langs->trans("BackToList").'</a>';
