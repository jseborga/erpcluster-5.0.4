...
class Requestcashdeplacement extends CommonObject
{
	...
	function LibStatut($status,$mode=0)
	{
		...
		if ($mode == 0)
		{
			...
			if ($status == 1) return $langs->trans('Discharged');
			if ($status == 2) return $langs->trans('Reviewed');
			if ($status == 3) return $langs->trans('Validated');
		}
		if ($mode == 1)
		{
			if ($status == 0) return $langs->trans('Disabled');
		...
		if ($mode == 2)
		{
			...
			if ($status == 1) return img_picto($langs->trans('Discharged'),'statut4').' '.$langs->trans('Discharged');
			if ($status == 2) return img_picto($langs->trans('Reviewed'),'statut3').' '.$langs->trans('Reviewed');
			if ($status == 3) return img_picto($langs->trans('Validated'),'statut2').' '.$langs->trans('Validated');
		}
		if ($mode == 3)
		{
