...
/**
 * \file    budget/itemsgroup.class.php
 * \ingroup budget
 ...
 * @see CommonObject
 */
class Itemsgroup extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'itemsgroup';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'items_group';

	/**
	 * @var ItemsgroupLine[] Lines
	 */
	public $lines = array();
	 ...
	 */
	public function fetch($id, $ref = null, $ref_ext=null,$label='')
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("itemsgroup", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		}elseif (null !== $ref_ext) {
			$sql .= ' AND t.ref_ext = ' . '\'' . $ref . '\'';
		}elseif(!empty($label))
		{
			$sql .= ' AND t.detail = ' . '\'' . $label . '\'';
			$sql.= " AND t.status >=0";
		} else {
			$sql .= ' AND t.rowid = ' . $id;
		}
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Itemsgroup") . '</u>';
        $label.= '<br>';
