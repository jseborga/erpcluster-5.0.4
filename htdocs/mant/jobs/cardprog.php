...
if ($action == 'confirm_asignloc' && $_REQUEST['confirm'] == 'yes')
{
	...
	$action = '';
	   		...
	   		// Confirmation de la validation
if ($action == 'confirm_validateprogram' && $_REQUEST['confirm'] == 'yes')
{
	// on verifie si l'objet est en numerotation provisoire
	$object->fetch(GETPOST('id'));
	//cambiando a programado
	...
	if ($object->status == 3)
	{
		//cambiamos para habilitar la ejecución
		$object->status = 4;
  		//update
		$res = $object->update($user);
		if ($res<=0)
	...
	{
		$db->rollback();
		setEventMessages($langs->trans('No es posible validar la programación'),null,'errors');
	}
	$action = '';
}
...
if ($action == 'upjobs' && $user->rights->mant->jobs->assignjobs || 	$action == 'upjobs' && $user->rights->mant->jobs->upjobs)
{
	$actiondes = 'asignjobs';
	...
	$res = $objectprogram->fetchAll('DESC','ref', 0,0,array(1=>1),'AND',$filterstatic,true);
	if ($res == 1)
		$ref = $objectprogram->ref + 1;
	elseif($res>1)
	{
		foreach ($objectprogram->lines AS $j => $line)
		{
			echo '<hr>'.$line->id;
			if (empty($ref))
				$ref = $line->ref + 1;
	...
	$objectprogram->fk_jobs = $object->id;
	$objectprogram->fk_speciality = GETPOST("fk_speciality",'int');
	$objectprogram->ref = $ref;
	$objectprogram->date_ini    = $date_ini_prog;
	$objectprogram->date_fin    = $date_fin_prog;
	$objectprogram->description = GETPOST('description','alpha');
	$objectprogram->image_req = GETPOST('image_req','int')+0;
	$objectprogram->members = $members;

	$objectprogram->fk_user_create = $user->id;
	$objectprogram->fk_user_mod = $user->id;
	$objectprogram->datec = dol_now();
	$objectprogram->datem = dol_now();
	$objectprogram->tms = dol_now();
	$objectprogram->status = 0;
	//validamos
	if ($objectprogram->fk_speciality <=0)
	{
		$error++;
		setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Speciality")), null, 'errors');
	}
	if ($objectprogram->date_ini > $objectprogram->date_fin)
	{
		$error++;
	...
	if (!$error)
	{
		$result = $objectprogram->create($user);
		if ($result<=0)
		{
			$error++;
			setEventMessages($objectprogram->error,$objectprogram->errors,'errors');
		}
	}
	//actualizamos la fecha de reporte en object
	if (empty($object->date_ini_prog)) $object->date_ini_prog = $date_ini_prog;
	else
	{
		if ($date_ini_prog <= $object->date_ini_prog) $object->date_ini_prog = $date_ini_prog;
	}
	if (empty($object->date_fin_prog)) $object->date_fin_prog = $date_fin_prog;
	else
	{
		if ($date_fin_prog >= $object->date_fin_prog) $object->date_fin_prog = $date_fin_prog;
	}
	$object->fk_user_mod = $user->id;
	$object->datem = dol_now();
	$object->tms = dol_now();
...
if ($id||$ref)
{
	...
	dol_fiche_head($head, 'prog', $langs->trans("Jobs"), 0, 'mant');
	...
	if ($action == '')
	{
		...
		{
			print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=program&amp;id='.$object->id.'">'.$langs->trans('Register programming').'</a>';
		}
	...
	if ($object->status == 3 || $object->status == 4 || $object->status == 5 || $object->status == 6)
	{
		...
		include_once DOL_DOCUMENT_ROOT.'/mant/jobs/tpl/programation_list.tpl.php';
		//if ($object->status == 5)
		//	include_once DOL_DOCUMENT_ROOT.'/mant/jobs/tpl/material.tpl.php';
	}

	if ($object->status == 5 && $action == 'editregjobs')
	{
		  		//orders
		$objJobsOrder = new Mjobsorder($db);
		  		//$objcomm = new Commande($db);
