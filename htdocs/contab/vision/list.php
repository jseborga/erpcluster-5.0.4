 ...
 *					Initialy built by build_class_from_table on 2017-09-28 08:18
 */
...
if (isset($_POST['search_ref']))
	$_SESSION['vision_search_ref'] = strtoupper(trim(GETPOST('search_ref')));

$search_ref = $_SESSION['vision_search_ref'];
...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield="t.ref, t.sequence"; // Set here default search field
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'contabvisionlist';
...
$arrayfields=array(
	...
	't.name_vision'=>array('label'=>$langs->trans("Fieldname_vision"), 'checked'=>0),
	't.detail_managment'=>array('label'=>$langs->trans("Fielddetail_managment"), 'checked'=>1),
	't.cta_normal'=>array('label'=>$langs->trans("Fieldcta_normal"), 'checked'=>0),
	't.cta_column'=>array('label'=>$langs->trans("Fieldcta_column"), 'checked'=>0),
	't.cta_class'=>array('label'=>$langs->trans("Fieldcta_class"), 'checked'=>0),
	//'t.cta_identifier'=>array('label'=>$langs->trans("Fieldcta_identifier"), 'checked'=>0),
	//'t.cta_operation'=>array('label'=>$langs->trans("Fieldcta_operation"), 'checked'=>0),
	//'t.cta_balances'=>array('label'=>$langs->trans("Fieldcta_balances"), 'checked'=>0),
	//'t.cta_totalvis'=>array('label'=>$langs->trans("Fieldcta_totalvis"), 'checked'=>0),
	't.line'=>array('label'=>$langs->trans("Fieldline"), 'checked'=>0),
	't.fk_accountini'=>array('label'=>$langs->trans("Fieldfk_accountini"), 'checked'=>0),
	't.fk_accountfin'=>array('label'=>$langs->trans("Fieldfk_accountfin"), 'checked'=>0),
...
if (empty($reshook))
{
	...
	if (GETPOST("button_removefilter_x") || GETPOST("button_removefilter.x") ||GETPOST("button_removefilter")) // All tests are required to be compatible with all browsers
	{
		...
		$search_fk_user_mod='';
		$search_status='';
		unset($_SESSION['vision_search_ref']);
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."contab_vision_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/contabvision/card.php?id='.$id);
	exit;
}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if (! empty($arrayfields['t.fk_parent']['checked']))
		{
			print '<td>'.$obj->fk_parent.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.sequence']['checked']))
		{
			print '<td>'.str_pad($obj->sequence, 10, "0", STR_PAD_LEFT).'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.account']['checked']))
		{
			print '<td>'.$obj->account.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.account_sup']['checked']))
		{
			print '<td>'.$obj->account_sup.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.name_vision']['checked']))
		{
			print '<td>'.$obj->name_vision.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.detail_managment']['checked']))
		{
			print '<td>'.$obj->detail_managment.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.cta_normal']['checked']))
		{
			print '<td>'.select_cta_normal($obj->cta_normal,'cta_normal','',0,0,1).'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.cta_column']['checked']))
		{
			print '<td>'.$obj->cta_column.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.cta_class']['checked']))
		{
			print '<td>'.select_cta_clase($obj->cta_class,'cta_class','',0,0,1).'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.line']['checked']))
		{
			print '<td>'.$obj->line.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.fk_accountini']['checked']))
		{
			print '<td>'.$obj->fk_accountini.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.fk_accountfin']['checked']))
		{
			print '<td>'.$obj->fk_accountfin.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
