...
/**
 * \file    orgman/cregiongeographic.class.php
 * \ingroup orgman
...
/**
 * Class Cregiongeographic
 *
 * Put here description of your class
 *
 * @see CommonObject
 */
class Cregiongeographic extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'cregiongeographic';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'c_region_geographic';

	/**
	 * @var CregiongeographicLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("cregiongeographic", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new CregiongeographicLine();
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Cregiongeographic($this->db);
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Cregiongeographic") . '</u>';
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Label') . ':</b> ' . $this->label;

        $url = DOL_URL_ROOT.'/orgman/regiongeographic/'.'card.php?id='.$this->id;
