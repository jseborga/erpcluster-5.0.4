...
$search_active='';
    ...
    $permtoread = $user->rights->poaactivity->read;
    $permtodelete = $user->rights->poaactivity->delete;
    $uploaddir = $conf->poaactivity->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."poa_activity_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/poaactivity/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.fk_prev_ant']['checked'])) print_liste_field_titre($arrayfields['t.fk_prev_ant']['label'],$_SERVER['PHP_SELF'],'t.fk_prev_ant','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_prev']['checked'])) print_liste_field_titre($arrayfields['t.fk_prev']['label'],$_SERVER['PHP_SELF'],'t.fk_prev','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.nro_activity']['checked'])) print_liste_field_titre($arrayfields['t.nro_activity']['label'],$_SERVER['PHP_SELF'],'t.nro_activity','',$params,'',$sortfield,$sortorder);
