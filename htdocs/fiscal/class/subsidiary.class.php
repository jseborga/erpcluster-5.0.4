...
class Subsidiary extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->matriz_def)) {
			 $this->matriz_def = trim($this->matriz_def);
		}
		...
		if (isset($this->matriz_address_two)) {
			 $this->matriz_address_two = trim($this->matriz_address_two);
		}
		if (isset($this->matriz_zone)) {
			 $this->matriz_zone = trim($this->matriz_zone);
		}
		...
		$sql .= ' '.(! isset($this->matriz_def)?'NULL':$this->matriz_def).',';
		...
		$sql .= ' '.(! isset($this->matriz_address_two)?'NULL':"'".$this->db->escape($this->matriz_address_two)."'").',';
		$sql .= ' '.(! isset($this->matriz_zone)?'NULL':"'".$this->db->escape($this->matriz_zone)."'").',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("subsidiary", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->matriz_def = $obj->matriz_def;
				$this->matriz_name = $obj->matriz_name;
				$this->matriz_address = $obj->matriz_address;
				$this->matriz_address_two = $obj->matriz_address_two;
				$this->matriz_zone = $obj->matriz_zone;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND', $filterstatic='', $lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->matriz_def = $obj->matriz_def;
				$line->matriz_name = $obj->matriz_name;
				$line->matriz_address = $obj->matriz_address;
				$line->matriz_address_two = $obj->matriz_address_two;
				$line->matriz_zone = $obj->matriz_zone;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' matriz_def = '.(isset($this->matriz_def)?$this->matriz_def:"null").',';
		...
		$sql .= ' matriz_address_two = '.(isset($this->matriz_address_two)?"'".$this->db->escape($this->matriz_address_two)."'":"null").',';
		$sql .= ' matriz_zone = '.(isset($this->matriz_zone)?"'".$this->db->escape($this->matriz_zone)."'":"null").',';
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $url = DOL_URL_ROOT.'/fiscal/subsidiary/'.'card.php?id='.$this->id;
