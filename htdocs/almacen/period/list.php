...
if (empty($reshook))
{
	...
	if ($action=='actal' && $user->rights->almacen->per->actal)
	{
	...
	if ($action=='actper' && $user->rights->almacen->per->act)
	{
	...
	if ($action == 'add')
	{
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/almacen/period/list.php',1);
				header("Location: ".$urltogo);
				exit;
			}
			{
...
if ($resql)
{
	...
	while ($i < min($num, $limit))
	{
		...
		if ($obj)
		{
			...
			if (! empty($arrayfields['t.status_af']['checked']))
			{
				print '<td>'.$object->getLibStatut_af(1).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.status_al']['checked']))
			{
