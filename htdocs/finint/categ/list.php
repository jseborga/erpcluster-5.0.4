...
/**
 *   	\file       finint/bankcateg_list.php
 *		\ingroup    finint
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-02-09 17:55
 */
...
$hookmanager->initHooks(array('bankcateglist'));
...
if (GETPOST('cancel')) { $action=''; $massaction=''; }
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/finint/categ/list.php',1);
	...
	if ($action == 'update' && ! GETPOST('cancel') && $user->rights->finint->cat->mod)
	{
		$error=0;
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/finint/categ/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Transactioncategories');
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."bank_categ_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($resql)
{
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteTransactioncategory'), $langs->trans('ConfirmDeleteTransactioncategory'), 'confirm_delete', '', 0, 1);
		print $formconfirm;
	}
	...
	if ($action == 'create' && $user->rights->finint->cat->creer)
	{
		print '<tr '.$bc[$var].'>';
			// LIST_OF_TD_FIELDS_LIST
		if (! empty($arrayfields['t.entity']['checked'])) 
		{
			print '<td>'.'<input type="number" name="entity" value="'.$obj->entity.'"></td>';
	...
	if (empty($reshook))
	{
		...
		if ($lContinue)
		{
			if ($user->rights->finint->cat->creer && empty($action))
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=create">'.$langs->trans("Createnew").'</a></div>'."\n";
			}
		}
	}
	print '</div>';
