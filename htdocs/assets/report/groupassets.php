...
//variable para definir la integreacion o no de departamentos y miembros
$confint = $conf->global->ASSETS_INTEGRATION_WITH_DEPARTAMENT_MEMBER;
...
if ($action == 'reporteExcel')
{
	...
	$sheet->setCellValueByColumnAndRow(0,2, $langs->trans("Reportassetsgroups"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B5',$langs->trans("Group"));
	if($fk_group==0)
	{
	...
	{
		$objPHPExcel->getActiveSheet()->setCellValue('C5',$cGroup);
	}
	...
	if($confint)
	{
		...
		// localizcion
		$objPHPExcel->getActiveSheet()->setCellValue('H8',$langs->trans("location"));
		//	fecha asignacion
		$objPHPExcel->getActiveSheet()->setCellValue('I8',$langs->trans("Dateassignment"));
		// Responsable
		$objPHPExcel->getActiveSheet()->setCellValue('J8',$langs->trans("Responsible"));
		//condicion
		$objPHPExcel->getActiveSheet()->setCellValue('K8',html_entity_decode($langs->trans("Been")));
		//valor actual
		$objPHPExcel->getActiveSheet()->setCellValue('L8',html_entity_decode($langs->trans("ValorAct")));
		//depreciacion acumulada
		$objPHPExcel->getActiveSheet()->setCellValue('M8',html_entity_decode($langs->trans("DepreciationAcum")));
		//saldo
		$objPHPExcel->getActiveSheet()->setCellValue('N8',html_entity_decode($langs->trans("Balance")));
		//estado
		$objPHPExcel->getActiveSheet()->setCellValue('O8',$langs->trans("Status"));
	}
	else
	{
		// Departament
		$objPHPExcel->getActiveSheet()->setCellValue('G8',$langs->trans("Departament"));
		// Responsable
		$objPHPExcel->getActiveSheet()->setCellValue('H8',$langs->trans("Responsible"));
			//condicion
		$objPHPExcel->getActiveSheet()->setCellValue('I8',html_entity_decode($langs->trans("Been")));

		//valor actual
		$objPHPExcel->getActiveSheet()->setCellValue('J8',html_entity_decode($langs->trans("ValorAct")));
		//depreciacion acumulada
		$objPHPExcel->getActiveSheet()->setCellValue('K8',html_entity_decode($langs->trans("DepreciationAcum")));
		//saldo
		$objPHPExcel->getActiveSheet()->setCellValue('L8',html_entity_decode($langs->trans("Balance")));
		//estado
		$objPHPExcel->getActiveSheet()->setCellValue('M8',$langs->trans("Status"));
	}
	// TABLA COLOR
	if($conint)
	{
	...
	{
		$objPHPExcel->getActiveSheet()->getStyle('A8:M8')->applyFromArray(
			array(
				'font'    => array(
					'bold'      => true
					),
	...
	foreach ((array) $aReportdetasset AS $i => $lines)
	{
		...
		if($type = $lines['type']=='G' )
		{
			$objPHPExcel->getActiveSheet()->setCellValue('B' .$j,$Etiqueta);
				// COLOR DEL TITULO
		...
		if($type = $lines['type']=='D' && $confint==1)
		{
			// VISTA
			$contt++;
			...
			->setCellValue('J' .$j,$Responsable)
			->setCellValue('K' .$j,$Condicion)
			->setCellValue('L'.$j,$lines['Valoract'])
			->setCellValue('M'.$j,$lines['Depreacum'])
			->setCellValue('N'.$j,$lines['Balance'])
			->setCellValue('O' .$j,$lines['Estado']);

		}
		if($type = $lines['type']=='D' && $confint==0)
		{
			$contt++;
			$objPHPExcel->getActiveSheet()->setCellValue('A' .$j,$contt)
			->setCellValue('B' .$j,$Codigo)
			...
			->setCellValue('E' .$j,dol_print_date($FechaAdquisicion,'day'))
			->setCellValue('F' .$j,$costo)
			->setCellValue('G' .$j,$departament)
			->setCellValue('H' .$j,$resp)
			->setCellValue('I' .$j,$Condicion)
			->setCellValue('J'.$j,$lines['Valoract'])
			->setCellValue('K'.$j,$lines['Depreacum'])
			->setCellValue('L'.$j,$lines['Balance'])
			->setCellValue('M' .$j,$Estado);
		...
		{
			$objPHPExcel->getActiveSheet()->getStyle('A8:M'.$j)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		}
...
llxHeader("",$langs->trans("Groupassets"),$help_url,'','','',$aArrjs,$aArrcss);
...
dol_fiche_head($head, 'card', $langs->trans("Byassetgroup"), 0, DOL_URL_ROOT.'/assets/img/assets',1);

print '<table class="border" width="100%">';
		// Entrepot Almacen
print '<tr>';
print '<td width="25%" class="fieldrequired">'.$langs->trans('Group').'</td><td colspan="3">';
$res = $objGroup->fetchAll('ASC','label',0,0,array(1=>1),'AND',$filterstatic);
...
if(($action == 'edit'||  $action=='edits'))
{
	...
	if ($res > 0)
	{
		foreach ($objGroup->lines AS $j => $line)
			$aGroup[$line->code] = $line->label;
	}
	if ($fk_group > 0)
	{
		$res = $objGroup->fetch($fk_group);
		if ($res > 0) $filter = " AND t.type_group = '".$objGroup->code."'";
	}
	...
	if($level==0)
	{
		$res = $objAssets->fetchAll('ASC','t.ref_ext',0,0,array(1=>1),'AND',$filter);
	}
	else
	{
		$res = $objAssets->fetchAll('ASC','t.ref_ext',0,0,array(1=>1),'AND',$filter);
	...
	if ($res>0)
	{
		...
		foreach ($aGroupasset AS $type_group => $obj)
		{
			...
			$cGroup=$aGroup[$type_group];
			print '</tr>';
			/*
			$aReportasset[]=array('Codigo'=>"",'Codigoext'=>"",'Etiqueta'=>$aGroup[$type_group],'FechaAdquisicion'=>"",'costo'=>"",'Inmueble'=>"",'location'=>"",'FechaAsignacion'=>"",'Responsable'=>"",'Condicion'=>"",'Estado'=>"",'departament'=>"",'resp'=>"",'type'=>"G");
			*/
			$aReportasset[]=array('Codigo'=>"",'Codigoext'=>"",'Etiqueta'=>$aGroup[$type_group],'FechaAdquisicion'=>"",'costo'=>"",'Inmueble'=>"",'location'=>"",'FechaAsignacion'=>"",'Responsable'=>"",'Condicion'=>"",'Estado'=>"",'departament'=>"",'resp'=>"",'Valoract'=>"",'Depreacum'=>"",'Balance'=>"",'type'=>"G");
			...
			foreach ($obj AS $ref => $line)
			{
				...
				{
					print '<td align="center">'.$line->departament_name.'</td>';
				...
				print '<td align="right" nowrap>'.$objAssets->getLibStatut(3).'</td>';

				print '</tr>';


				$aReportasset[]=array('Codigo'=>$line->ref,'Codigoext'=>$line->ref_ext,'Etiqueta'=>$line->descrip,'FechaAdquisicion'=>$line->date_adq,'costo'=>$line->coste,'Inmueble'=>$objMproperty->label,'location'=>$objMlocation->detail,'FechaAsignacion'=>$FechaAsignacion,'Responsable'=>$objAdherent->lastname.' '.$objAdherent->firstname,'Condicion'=>($line->been?$objbeen->label:''),'Estado'=>$objAssets->getLibStatut(0),'departament'=>$line->departament_name,'resp'=>$line->resp_name,'Valoract'=>$cValoract,'Depreacum'=>$cDepreacum,'Balance'=>$cBalance,'type'=>"D");
		...
		// enviar group y conint and level
		...
		if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
		{
			...
			if ($fk_group>0)
			{
				$objAssets->group = $objGroup->code;
			}
			$model='fractalgroupassets';
...
$conf->assets->dir_output;
$urlsource=$_SERVER['PHP_SELF'].'?fk_group='.$fk_group.'&confint='.$confint.'&level='.$level;
$genallowed=0;
$delallowed=0;


		//$objGroup->modelpdf = 'fractalinventario';
$objGroup->modelpdf = 'fractalgroupassets';
