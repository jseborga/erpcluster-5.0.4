...
$WS_DOL_URL = DOL_MAIN_URL_ROOT.'/wsfractal/server_thirdparty.php';
...
{
	$aData = explode(':',$data);
	$_POST['fk_user'] = $aData[0];
	$_POST['ref'] = $aData[1];
	$ref = $aData[1];
	$_POST['address'] = $aData[2];
	$_POST['email'] = $aData[3];
	$_POST['phone'] = $aData[4];
	$_POST['nit'] = $aData[5];
}
...
$authentication=array(
	...
	'login'=>'admindb',
	'password'=>'admindbvisual',
	'entity'=>'');


// Test URL
if ($action=='get' || $action == 'sincSoc')
{

	$parameters = array('authentication'=>$authentication,'id'=>$fk_soc,'name'=>$ref,'ref_ext'=>$ref_ext);
...
if ($action=='create')
{
	...
	//France
		'customer_code'=>'-1',
	//Generate code regarding module configuration
		'supplier_code'=>'0',
		'phone'=>'0141414141',
		'fax'=>'0121212121',
	...
	$newthirdparty=array(
		'ref'=>GETPOST('ref'),
		'ref_ext'=>GETPOST('ref_ext'),
		'fk_user_author'=>GETPOST('fk_user'),
		'status'=>'1',
		'client'=>'1',
		'supplier'=>'0',
...
if ($result)
{
	if ($action=='getList')
	{
		$res['thirdparties'] = $result['thirdparties'];
		echo json_encode($res);
	}
	if ($action=='get')
	{
		$res['thirdparty'] = $result['thirdparty'];
		echo json_encode($res);
	}
	if ($action=='create')
	{
		$resu = array();
		$resu[0]['result_code'] = $result['result']['result_code'];
		$resu[0]['id'] = $result['id'];
		$resu[0]['ref'] = $result['ref'];
		$res['thirdparty'] = $resu;
		echo json_encode($res);
	}
}
