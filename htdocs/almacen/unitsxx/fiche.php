...
// Ajout entrepot
if ($action == 'add' && $user->rights->almacen->crearunidad)
  {
    $object = new Units($db);
...
// Delete warehouse
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->almacen->delunidad)
  {
    $object = new Units($db);
    ...
    if ($result > 0)
      {
	header("Location: ".DOL_URL_ROOT.'/almacen/units/liste.php');
	exit;
      }
    else
      {
