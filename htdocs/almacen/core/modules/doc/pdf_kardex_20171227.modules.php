 ...
 */
class pdf_kardex extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
  ...
  // Objet societe qui emet
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "kardex";
		$this->description = $langs->trans('PDFKardekDescription');
	...
	// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		...
		if ($conf->almacen->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->almacen->dir_output . "/" . $objectref.'/'.$period_year.'/kar';
			...
			if (file_exists($dir))
			{
				...
				//for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$desc = dol_trunc($lines['detail'],99);
					...
					$pdf->MultiCell($this->posxsal-$this->posxent-1, 3, price2num($ent), 0, 'R',0);
					// salida
					$sal = $lines['salida'];
					$pdf->SetXY($this->posxsal, $curY);
					$pdf->MultiCell($this->posxval-$this->posxsal-1, 3, price2num($sal), 0, 'R',0);

					// balance
					$val = $lines['balance'];
					$pdf->SetXY($this->posxval, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxval, 3, price2num($val), 0, 'R',0);
	...
	*/
	...
	// line prend une position y en 2eme param et 4eme param
		...
		$pdf->line($this->posxval, $tab_top, $this->posxval, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxval-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxval,2, $outputlangs->transnoentities("Balance"),'','C');
		}
	}
	...
	*/
	...
	// Show Draft Watermark
		...
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Physical kardex");
						...
						if (preg_match('/\.jpg$/i',$file)||preg_match('/\.jpeg$/i',$file) || preg_match('/\.png$/i',$file))
						{
							if (file_exists($dir.'/'.$file))
							{
								...
								if (is_readable($image))
								{
									...
									$pdf->Image($image, $this->marge_gauche+65, $posy+6, 0, $height);
									// width=0 (auto)
									$posy+=4;
								}
							}
		...
		//$pdf->SetXY($posx,$posy);
		//$pdf->SetTextColor(0,0,60);
		//$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Code")." : " . $aKardex['productref'].'; '.$langs->trans('Unidad.').': '.$aKardex['unit'], '', 'R');
		//Product label
		//$posy+=8;
		//$pdf->SetXY($posx,$posy);
		//$pdf->SetTextColor(0,0,60);
		//$pdf->MultiCell(150, 3, $outputlangs->transnoentities("Name")." : " . $aKardex['productlabel'], '', 'R');
		//fecha ini fecha fin
		//$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
