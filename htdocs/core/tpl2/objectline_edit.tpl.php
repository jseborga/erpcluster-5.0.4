...
<tr <?php echo $bc[$var]; ?>>
	...
	if ($user->societe_id)
	{
		...
		print '<input type="hidden" name="product_desc" value="'.$line->description.'">';
	}
	else
	{
	...
	<?php if ($line->element != 'propaldet') { ?>
	<td align="right"><?php $coldisplay++; ?><?php //echo $form->load_tva('tva_tx',$line->tva_tx,$seller,$buyer,0,$line->info_bits,$line->product_type); ?>
		<input type="hidden" name="tva_tx" value="<?php echo $line->tva_tx; ?>">
	</td>
	<?php } ?>
	<?php if (!$conf->global->PRICE_TAXES_INCLUDED) { ?>
	<td align="right"><?php $coldisplay++; ?><input type="text" class="flat" size="8" id="price_ht" name="price_ht" value="<?php echo price2num($line->subprice,'MS'); ?>"></td>
	<?php } ?>
	<?php if ($inputalsopricewithtax || $conf->global->PRICE_TAXES_INCLUDED) { ?>
	<td align="right"><?php $coldisplay++; ?><input type="text" class="flat" size="8" id="price_ttc" name="price_ttc" value="<?php echo price2num($line->pu_ttc,'MS'); ?>"></td>
	<?php } ?>
	...
	<td align="center" nowrap><?php $coldisplay++; ?>
	<?php if ($user->societe_id) {
		$label = $line->getLabelOfUnit();
		if ($label !== '') {
			print $langs->trans($label);
		}
		print '<input type="hidden" name="fk_unit" value="'.$line->fk_unit.'">';
	} else {
		print $form->selectUnits($line->fk_unit,'fk_unit');
	} ?>
	</td>
	<?php } ?>
	<td align="right" nowrap><?php $coldisplay++; ?>
	<?php if (($line->info_bits & 2) != 2) { ?>
		% <input size="1" type="text" class="flat" name="remise_percent" id="remise_percent" value="<?php echo $line->remise_percent; ?>"><?php echo $langs->trans('Value').' ';?> <input size="5" type="text" class="flat" name="discount" id="discount" value="<?php echo $line->discount; ?>">
	<?php } else { ?>
		&nbsp;
	<?php } ?>
	</td>
	<?php if ($object->element == 'order_supplier' || $object->element == 'invoice_supplier' || $line->element == 'facturedet') { ?>
	<td align="right" nowrap><?php $coldisplay++; ?><input type="text" size="5" name="amount_ice" value="<?php echo $line->amount_ice; ?>"></td>
	<?php } ?>
	<?php
	if (! empty($usemargins))
	{
...
if ($conf->poa->enabled)
{
	...
	if ($fk_poa>0)
	{
		//vamos a buscar el poa para obtener la gestión
		$respoa = $objPoa->fetch($fk_poa);
		if ($respoa>0)
		{
			$period_year = $objPoa->gestion;
			$label_poa = $objPoa->label;
		}
	}
	...
	if ($res > 0)
	{
		...
		$filterpartida = " AND t.gestion = ".$period_year;
		...
		if ($respoa>0)
		{
			foreach ($objPoa->lines AS $j => $linepoa)
			{
				...
				$optionspoa.= '<option value="'.$linepoa->id.'" '.$selected.'>'.$linepoa->label.'</option>';
			}
		}
	}
	elseif ($res < 0)
	...
	else
	{
		...
		if ($respoa>0)
		{
			foreach ($objPoa->lines AS $j => $linepoa)
			{
				...
				$aPoa[$linepoa->id] = ($respoa?$objObjetive->label:'').' : '.$linepoa->label;
				//buscamos el nombre de la partida
				$respartida = $objPartida->fetch(0,$linepoa->partida,$_SESSION['period_year']);
				//con poa
				if ((GETPOST('fk_poa')?GETPOST('fk_poa'):$fk_poa)== $linepoa->id)
			...
			if(count($aPoa)>0)
			{
				foreach ($aPoa AS $fk => $label)
				{
					...
					$optionspoa.= '<option value="'.$fk.'" '.$selected.' title="'.$label.'">'.dol_trunc($label,40).'</option>';
				}
	...
	print '<tr class="liste_titre nodrag nodrop">';
	print '<td>x'.$langs->trans('Catprog').'</td>';
