...
$search_active = GETPOST("search_active");

// Security check
if ($user->societe_id > 0) accessforbidden();
if (! $user->rights->contab->account->read) accessforbidden();
...
$accounting = new AccountingAccountext($db);
...
else $sql .= " LEFT JOIN ".MAIN_DB_PREFIX."accounting_account as a2 ON a2.account_number = CAST(aa.account_parent AS UNSIGNED) AND a2.fk_pcg_version = aa.fk_pcg_version";
...
if ($search_active != -1) {
	$sql .= natural_search("aa.active", $search_active);
}
...
if ($resql) {
	...
	$accountstatic = new AccountingAccountext($db);
	$accountparent = new AccountingAccountext($db);
	...
	while ( $i < min($num, $limit) )
	{
		...
		print '<tr ' . $bc[$var] . '>';
		print '<td>' . $accountstatic->getNomUrladd(1) . '</td>';
		...
		if (! empty($obj->account_parent))
		{
			...
			print '<td>' . $accountparent->getNomUrladd(1) . '</td>';
		}
		else
		{
			print '<td>&nbsp;</td>';
		...
		if ($user->rights->contab->account->write) {
