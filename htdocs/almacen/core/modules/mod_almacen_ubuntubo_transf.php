 ...
 */
class mod_almacen_ubuntubo_transf extends ModeleNumRefAlmacen
{
	...
	function info()
	{
		...
		return $langs->trans('UbuntuboNumRefModelDesc3',$this->prefixinvoice,$this->prefixcreditnote);
	}
	...
	function getExample()
	{
		return $this->prefixinvoice."1509-0001";
	}
	...
	function canBeActivated()
	{
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."stock_mouvement_temp";
	...
	function getNextValue($objsoc,$object,$mode='next')
	{
		...
		$sql.= " AND entity = ".$conf->entity;
		$resql=$db->query($sql);
		dol_syslog("mod_almacen_ubuntubo_transf::getNextValue sql=".$sql);
		if ($resql)
		{
		...
		{
			dol_syslog("mod_almacen_ubuntubo_transf::getNextValue sql=".$sql, LOG_ERR);
			return -1;
		}
		...
		else if ($mode == 'next')
		{
			...
			dol_syslog("mod_almacen_ubuntubo_transf::getNextValue return ".$prefix.$yymm."-".$num);
