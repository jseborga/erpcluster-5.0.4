...
/**
 * \file    mant/mjobsadvance.class.php
 * \ingroup mant
 ...
 * @see CommonObject
 */
class Mjobsadvance extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'mjobsadvance';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'm_jobs_advance';

	/**
	 * @var MjobsadvanceLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_speciality)) {
			 $this->fk_speciality = trim($this->fk_speciality);
		}
		if (isset($this->ref)) {
		...
		if (isset($this->used_time)) {
			 $this->used_time = trim($this->used_time);
		}
		if (isset($this->image_adv)) {
			 $this->image_adv = trim($this->image_adv);
		}
		if (isset($this->members)) {
			 $this->members = trim($this->members);
		}
		...
		$sql .= ' '.(! isset($this->fk_speciality)?'NULL':$this->fk_speciality).',';
		...
		$sql .= ' '.(! isset($this->used_time)?'NULL':"'".$this->used_time."'").',';
		$sql .= ' '.(! isset($this->image_adv)?'NULL':"'".$this->db->escape($this->image_adv)."'").',';
		$sql .= ' '.(! isset($this->members)?'NULL':"'".$this->db->escape($this->members)."'").',';
	 ...
	 */
	public function fetch($id, $ref = null,$fk=0,$fkp=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("mjobsadvance", 1) . ")";
		}
		if (null !== $ref && $fkp>0) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
			$sql .= ' AND t.fk_jobs_program = ' . $fkp;
		} elseif (null !== $ref && $fk>0) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
			$sql .= ' AND t.fk_jobs = ' . $fk;
		} else {
			$sql .= ' AND t.rowid = ' . $id;
		}
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fk_speciality = $obj->fk_speciality;
				$this->ref = $obj->ref;
				$this->description = $obj->description;
				$this->date_ini = $this->db->jdate($obj->date_ini);
				$this->date_fin = $this->db->jdate($obj->date_fin);
				$this->used_time = $obj->used_time;
				$this->image_adv = $obj->image_adv;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_speciality = '.(isset($this->fk_speciality)?$this->fk_speciality:"null").',';
		...
		$sql .= ' used_time = '.(isset($this->used_time)?$this->used_time:"null").',';
		$sql .= ' image_adv = '.(isset($this->image_adv)?"'".$this->db->escape($this->image_adv)."'":"null").',';
		$sql .= ' members = '.(isset($this->members)?"'".$this->db->escape($this->members)."'":"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Mjobsadvance($this->db);
