...
$hookmanager->initHooks(array('ctypetvalist'));
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/fiscal/typevat/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/fiscal/typevat/list.php',1));
			exit;
		}
		else
		{
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_tva_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
