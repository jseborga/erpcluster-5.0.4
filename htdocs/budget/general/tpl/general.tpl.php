...
$lAccess = $user->rights->budget->par->read+$user->rights->budget->par->write+$user->rights->budget->par->mod;
if ($user->rights->budget->bud->readall) $lAccess=1;
if ($lAccess <=0)
{
...
if ($general->id && (empty($subaction) || $subaction == 'view' || $subaction == 'delete'))
{
	...
	print '<table class="border centpercent">'."\n";
	print '<tr class="liste_titre"><td>'.$langs->trans("Label").'</td><td>'.$langs->trans('Value').'</td></tr>';
	...
	print '</td></tr>';
	$var = !$var;
	print '<tr '.$bc[$var].'><td>'.$langs->trans("Fieldexchange_rate").'</td><td>'.$general->exchange_rate.'</td></tr>';
	$var = !$var;
	print '<tr '.$bc[$var].'><td>'.$langs->trans("Fieldsecond_currency").'</td><td>';
	...
	print '</td></tr>';
	$var = !$var;
	print '<tr '.$bc[$var].'><td>'.$langs->trans("Fielddecimal_quant").'</td><td>'.$general->decimal_quant.'</td></tr>';
	$var = !$var;
	print '<tr '.$bc[$var].'><td>'.$langs->trans("Fielddecimal_pu").'</td><td>'.$general->decimal_pu.'</td></tr>';
	$var = !$var;
	print '<tr '.$bc[$var].'><td>'.$langs->trans("Fielddecimal_total").'</td><td>'.$general->decimal_total.'</td></tr>';
...
// Part to show record
if (($user->rights->budget->bud->readall || $user->rights->budget->par->read) && $subaction != 'edit')
{
	...
	print_liste_field_titre($langs->trans('Valuecalc'),'','','',$params,'align="right"',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans('Valueassumed'),'','','',$params,'align="right"',$sortfield,$sortorder);
	if ($subaction != 'editc')
	...
	if ($nb > 0)
	{
		...
		$lVerif = true;
		if (is_array($aDelete) && count($aDelete)>0)
		{
		...
		foreach ($lines AS $j => $line)
		{
			...
			if ($subaction == 'editc')
			{
				...
				print '<td>'.price(price2num($line->amount_def,'MT')).'</td>';
			...
			{
				print '<td>'.$langs->trans($line->label).'</td>';
				print '<td align="right">'.price(price2num($line->amount_def,'MT')).'</td>';
				print '<td align="right">'.price(price2num($line->amount,'MT')).'</td>';
