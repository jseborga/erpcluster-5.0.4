...
if ($object->id > 0)
{
	...
	$res = $objIncidents->fetchAll('','',0,0,array(),'AND',$filter,true);
	$search_fk = 0;
	if ($res==1) $search_fk = $objIncidents->id;
	$filter = " AND t.type='politic'";
	$filter.= " AND t.fk_incident = ".$search_fk;
	...
	print '<td align="center">'.$objIncidents->njobs.'</td>';
	$nAnnualamount= ($suma * ($incident/100) * 12 * $objIncidents->njobs);
