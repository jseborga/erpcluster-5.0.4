 ...
 * Copyright (C) 2014      Ramiro Queso          <ramiroques@gmail.com>
 *
...
/**
 * \file 	htdocs/ventas/fichefiscal.php
 * \ingroup facture
 * \brief 	Page to view facture fiscal
 */
...
require_once DOL_DOCUMENT_ROOT . '/fiscal/class/factureext.class.php';
...
//action confirm delete paiemente
if ($action == 'confirm_delete_paiement' && $confirm == 'yes' && $user->rights->facture->paiement)
{
	$db->begin();
	$objpaiement = new Paiement($db);

	$objpaiement->fetch(GETPOST('idp'));
	$result = $objpaiement->delete();
	if ($result > 0)
	{
		$db->commit();
		header("Location: fichefiscal.php?facid=".$id);
		exit;
	}
	else
	{
...
if ($action =='updatevfiscal' && ($user->rights->ventas->fact->edit || $user->rights->sales->fac->edit))
{
	//recibimos valores
	$facid = GETPOST('facid');
	$idr = GETPOST('idr');
	//buscamo
	$objvfiscal = new Vfiscal($db);
	$objvfiscal->fetch($idr);
	if ($objvfiscal->id == $idr && $objvfiscal->fk_facture == $facid)
	{
		//cambiamos valores recibidos
		$objvfiscal->nfiscal = GETPOST('nfiscal');
		$objvfiscal->nit = GETPOST('nit');
		$objvfiscal->razsoc = GETPOST('razsoc');
		$objvfiscal->date_exp = dol_mktime(12, 0, 0, GETPOST('remonth'),  GETPOST('reday'),  GETPOST('reyear'));
		$objvfiscal->num_autoriz = GETPOST('num_autoriz');
		$objvfiscal->baseimp1 = GETPOST('baseimp1','int');
		$objvfiscal->valimp1 = GETPOST('valimp1','int');
		//verificamos el valor introducido
		//$valimp1 = price2num($objvfiscal->baseimp1 - (($objvfiscal->baseimp1 / (1 + ($objvfiscal->aliqimp1/100)))),'MT');
		$valimp1 = price2num($objvfiscal->baseimp1 * $objvfiscal->aliqimp1/100,'MT');
		if ($valimp1 <> $objvfiscal->valimp1)
			$objvfiscal->valimp1 = $valimp1;
		...
		{
			$langs->load("errors");
			$mesg='<div class="error">'.$langs->trans($objvfiscal->error).'</div>';
			$db->rollback();
		}
	}
}
...
// Delete invoice
  ...
  //anulamos la factura
	$close_code = 'abandon';
	$close_note = $langs->trans('Anulacion manual factura');
	$result = $object->set_canceled($user, $close_code, $close_note);
	if ($result <= 0) $error++;
  //cambiamos el estado de vfiscal
	$objvfiscal = new Vfiscal($db);
	$objvfiscal->fetch('',$id);
	if ($objvfiscal->fk_facture == $id)
	{
	  $objvfiscal->statut = 2; //anulado
	  $result = $objvfiscal->update($user);
	  if ($result <= 0) $error++;
	}
	$sql .= " DELETE FROM ".MAIN_DB_PREFIX."paiement_facture WHERE fk_facture = ".$id;
	$ressql = $db->query($sql);
	if ($ressql <=0) $error++;
  //eliminamos los pagos
	$objpaiement = new Paiement($db);
	foreach ($_SESSION['paimentid'] AS $e => $idpai)
	{
	  //eliminamos en la tabla paiement_facture;

		$objpaiement->fetch($idpai);
		$result = $objpaiement->delete();
		if ($result <=0)
			$error++;
	}
  ...
  //  echo '<hr>'.$error;
	if (empty($error)) {
		$db->commit();
		header('Location: ' . DOL_URL_ROOT . '/fiscal/list.php');
		exit();
	} else {
		$db->rollback();
		setEventMessage($object->error, 'errors');
...
// confirm_re_annulled
else if ($action == 'confirm_re_annulled' && $confirm == 'yes' && $user->rights->facture->supprimer)
{
	$urlant = GETPOST('urlant');
	$error=0;
	$result = $object->fetch($id);
	//clonamos el object
	$objectcop = $object;
	...
	$result = $object->deleteadd($user, 1, $idwarehouse,true);
	if ($result <=0)
	{
		$error++;
		setEventMessages($object->error,$object->errors,'errors');
	}
	//agregamos como nuevo registro
	$facnumber = $objectcop->ref;
	$facantid  = $objectcop->id;

	$objnew = new Facture($db);
	$objnew = $objectcop;
	$newid = $objnew->create($user);
	//reemplazamos el id y el facnumber
	if ($newid > 0)
	{
		$sql = " UPDATE ".MAIN_DB_PREFIX."facture SET facnumber ='".$facnumber."'";
		$sql.= " , fk_statut = 3";
		$sql.= " WHERE rowid = ".$newid;
		$result = $db->query($sql);
		if ($result <=0)
			$error++;
	}
	...
	//echo '<hr>errorf '.$error;

	//cambiamos el estado de vfiscal
	$objvfiscal = new Vfiscalext($db);
	$objvfiscal->fetch('',$id);
	$objvfiscal->fk_facture.' == '.$id;
	if ($objvfiscal->fk_facture == $id)
	{
		$objvfiscal->fk_facture = $newid;
		$objvfiscal->status = 2;
		//anulado
		$result = $objvfiscal->update($user);
		if ($result <= 0)
		{
			$error++;
			setEventMessages($objvfiscal->error,$objvfiscal->errors,'errors');
		}
	}

	if (empty($error)) {
		$db->commit();
		setEventMessages($langs->trans('Satisfactorycancellation'),null,'mesgs');
		header('Location: ' . DOL_URL_ROOT . '/fiscal/list.php');
		exit();
	} else {
		$db->rollback();
...
if ($action == 'delete_paiement')
{
	$objpaiement = new Paiement($db);
	$objpaiement->fetch(GETPOST('idp'));
	print $form->formconfirm($_SERVER['PHP_SELF'].'?id='.$id.'&idp='.$objpaiement->id, $langs->trans("DeletePayment"), $langs->trans("ConfirmDeletePayment"), 'confirm_delete_paiement','',0,2);
...
if ($action == 'create')
{
	...
	dol_fiche_head($head, 'Fiscal', $langs->trans('InvoiceCustomer'), 0, 'bill');
	...
	if ($action == 'delete')
	{
		...
		if (
			...
			! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change && $object->statut >= 1)
		{
			$langs->load("stocks");
	...
	// Confirmation to delete invoice
	if ($action == 'confirm_annulled')
	{

	  //confirmamos el abandono
		$object->fetch($id);
		$close_code = 'abandon';
		$close_note = $langs->trans('Anulacion manual factura');
		...
		$text = $langs->trans('ConfirmAnnulled', $object->ref);
		$formquestion = array();
			...
			! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change && $object->statut >= 1)
		{
			...
			$formquestion = array(
								...
								// => 1),
				array('type' => 'other','name' => 'idwarehouse','label' => $label,'value' => $formproduct->selectWarehouses(GETPOST('idwarehouse')?GETPOST('idwarehouse'):'ifone', 'idwarehouse', '', 1, 0, 0, $langs->trans("NoStockAction"))));
			$formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $langs->trans('DeleteBill'), $text, 'confirm_re_annulled', $formquestion, "yes", 1);
		} else {
			$formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $langs->trans('DeleteBill'), $text, 'confirm_re_annulled', '', 'yes', 1);
		}
	}
	...
	if ($action == 'valid')
	{
		...
		if ($object->type != Facture::TYPE_DEPOSIT &&
			! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change)
		{
			$langs->load("stocks");
			require_once DOL_DOCUMENT_ROOT . '/product/class/html.formproduct.class.php';
		...
		// on ne permet pas de classer abandonner la facture.
			...
			// Cree un tableau formulaire
				$formquestion = array('text' => $langs->trans("ConfirmCancelBillQuestion"),
					...
					array('type' => 'text',
						'name' => 'close_note',
						'label' => $langs->trans("Comment"),
						'value' => $langs->trans('Manualcancellation'),
						'size' => '100'));

				$formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?facid=' . $object->id, $langs->trans('CancelBill'), $langs->trans('ConfirmCancelBill', $object->ref), 'confirm_annulled', $formquestion, "yes");
			}
		}
	...
	{
		print $langs->trans("CompanyHasNoAbsoluteDiscount");
		if ($object->statut == 0 && $object->type != Facture::TYPE_CREDIT_NOTE &&
			$object->type != Facture::TYPE_DEPOSIT)
		{
			print ' (' . $addabsolutediscount . ')<br>';
		}
	...
	print '</td>';
	if (
		$object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && ! empty($object->brouillon) && $user->rights->facture->creer)
	{
	...
	if ($result)
	{
		...
		if ($num > 0)
		{
			while ($i < $num)
			{
				...
				$_SESSION['paimentid'][$objp->rowid] = $objp->rowid;
				print dol_print_date($db->jdate($objp->dp), 'day') . '</a>';
				print '&nbsp;&nbsp;';
				print '<a href="' . DOL_URL_ROOT . '/fiscal/fichefiscal.php?facid='.$id.'&idp=' . $objp->rowid.'&action=delete_paiement' . '">' . img_picto($langs->trans('Deletepaiement'), 'delete') . '</a></td>';
	...
	if ($objfiscal->status != 2)
		...
		print '<td align="left" colspan="3">' . $langs->trans('Annulled') . '</td></tr>';
	...
	$objdosing->fetch($objfiscal->fk_dosing);
	$lDosing = false;
	if ($objdosing->lote == 1) $lDosing = true;
	//vista del registro
	if ($action == 'editv' && $lDosing)
	{
		print '<form action="'.$_SERVER['PHP_SELF'].'" method="post">';
		print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
		print '<input type="hidden" name="action" value="updatevfiscal">';
	...
	else
	{
		...
		if ($lDosing && ($user->rights->ventas->fact->edit|| $user->rights->sales->fac->edit))
		{
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline')
	{
		...
		if (empty($reshook)) {
			...
			// Classify 'closed not completely paid' (possible si validee et pas encore classee payee)
			if ($object->statut == 1 && $object->paye == 0 && $resteapayer > 0 && ($user->rights->ventas->fact->nul || $user->rights->sales->fac->nul))
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?facid=' . $object->id . '&amp;action=annulled">' . $langs->trans('Sortingannulled') . '</a></div>';
			}
