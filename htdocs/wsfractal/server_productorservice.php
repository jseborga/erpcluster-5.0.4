...
function getListOfProductsOrServices($authentication,$filterproduct)
{
	...
	{
		$product = new Product($db);
		$sql ="SELECT rowid, ref, ref_ext, label, price_ttc ";
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				$arrayproducts[]=array('id'=>$obj->rowid,'ref'=>$obj->ref,'ref_ext'=>$obj->ref_ext, 'label'=> $obj->label,'price'=>$price,'balance'=>$saldoStock);
				$i++;
			}
		}
		else
