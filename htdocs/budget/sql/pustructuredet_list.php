...
/**
 *   	\file       budget/pustructuredet_list.php
 *		\ingroup    budget
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-11-14 10:20
 */
...
dol_include_once('/budget/class/pustructuredet.class.php');
...
$search_ref_structure=GETPOST('search_ref_structure','alpha');
...
$search_status_print=GETPOST('search_status_print','alpha');
...
$hookmanager->initHooks(array('pustructuredetlist'));
...
't.ref_structure'=>array('label'=>$langs->trans("Fieldref_structure"), 'checked'=>1),
...
't.status_print'=>array('label'=>$langs->trans("Fieldstatus_print"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."pu_structure_det as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pu_structure_det_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_ref_structure) $sql.= natural_search("ref_structure",$search_ref_structure);
...
if ($search_status_print) $sql.= natural_search("status_print",$search_status_print);
...
if ($search_ref_structure != '') $params.= '&amp;search_ref_structure='.urlencode($search_ref_structure);
...
if ($search_status_print != '') $params.= '&amp;search_status_print='.urlencode($search_status_print);
...
if (! empty($arrayfields['t.ref_structure']['checked'])) print_liste_field_titre($arrayfields['t.ref_structure']['label'],$_SERVER['PHP_SELF'],'t.ref_structure','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.status_print']['checked'])) print_liste_field_titre($arrayfields['t.status_print']['label'],$_SERVER['PHP_SELF'],'t.status_print','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.ref_structure']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ref_structure" value="'.$search_ref_structure.'" size="10"></td>';
...
if (! empty($arrayfields['t.status_print']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_status_print" value="'.$search_status_print.'" size="10"></td>';
