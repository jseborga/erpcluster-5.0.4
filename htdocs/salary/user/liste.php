...
$sql = "SELECT d.rowid, d.login, d.lastname, d.firstname, d.societe as company, d.fk_soc, p.lastnametwo, ";
$sql.= " d.datefin,";
...
if ($resql)
{
	...
	while ($i < $num && $i < $conf->liste_limit)
	{
		...
		$objectpu->id=$objp->rowid;
		$objectpu->ref=$objp->rowid;
		$objectpu->lastname=$objp->lastname;
		$objectpu->firstname=$objp->firstname;
		$objectpu->lastnametwo=$objp->lastnametwo;
		...
		// Ref
		print "<td>";
		print $objectpu->getNomUrl(1);
		print "</td>\n";

		// Lastname
		print "<td><a href=\"fiche.php?rowid=$objp->rowid\">";
		print ((! empty($objp->lastname) || ! empty($objp->firstname)) ? dol_trunc($objectpu->getFullName($langs)) : '');
