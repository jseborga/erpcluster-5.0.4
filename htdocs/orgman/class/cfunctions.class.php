...
/**
 * \file    orgman/cfunctions.class.php
 * \ingroup orgman
 ...
 * @see CommonObject
 */
class Cfunctions extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'cfunctions';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'c_functions';

	/**
	 * @var CfunctionsLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null,$period_year=0)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
			$sql .= " AND entity IN (" . getEntity("cfunctions", 1) . ")";
		}
	 ...
	 */
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='',$action='')
	{
		...
		$label = '<u>' . $langs->trans("Functions") . '</u>';
		$label.= '<br>';
		$label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;
		$label.= '<br>';
		$label.= '<b>' . $langs->trans('Label') . ':</b> ' . $this->label;

		$url = DOL_URL_ROOT.'/orgman/functions/'.'list.php?id='.$this->id.($action?'&action='.$action:'');
