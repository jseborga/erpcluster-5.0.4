...
if (! empty($action) && $action == 'fetch' && ! empty($id))
{
	...
	$ret = $object->fetch($id);
	if ($ret > 0) {
		$outref = $object->tva_intra;
		$outlabel = $object->name;
		...
		$found = false;
		$outjson = array('fk_soc'=>$id, 'ref' => $outref,'label' => $outlabel,'desc' => $outdesc);
	}
...
else 
{
	...
	$form = new Formv($db);
	$arrayresult = $form->select_client_list_v("", $htmlname, $type, "", $price_level, $searchkey, $status, 2, $outjson, $socid);
