...
/**
 *   	\file       clientes/coccupation_card.php
 *		\ingroup    clientes
...
$hookmanager->initHooks(array('coccupation'));
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/clientes/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/clientes/list.php',1));
			exit;
		}
		else
		{
...
print '<tr><td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>$object->active</td></tr>';
	...
	//$linktoelem = $form->showLinkToObjectBlock($object, null, array('coccupation'));
