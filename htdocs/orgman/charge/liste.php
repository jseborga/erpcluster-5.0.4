...
/**
 *   	\file       orgman/csources_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-30 12:18
 */
...
if (!$user->rights->orgman->char->lire) accessforbidden();

// Initialize technical object to manage hooks of modules. Note that conf->hooks_modules contains array array
$hookmanager->initHooks(array('csources'));
...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->orgman->char->write)
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/charge/list.php',1);
		...
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Labe")), null, 'errors');
		}
	...
	if ($action == 'update' && ! GETPOST('cancel') && $user->rights->orgman->char->write)
	{
		$error=0;
	...
	// Action to delete
	if ($action == 'confirm_delete'  && $user->rights->orgman->char->del)
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/charge/list.php',1));
			exit;
		}
		else
		{
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			// Delete OK
			setEventMessages("RecordDeleted", null, 'mesgs');
			header("Location: ".dol_buildpath('/orgman/list.php',1));
			exit;
		}
		else
		{
...
$sql.= " FROM ".MAIN_DB_PREFIX."p_charge as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_sources_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_detail) $sql.= natural_search("label",$search_detail);
...
if ($resql)
{
	...
	if ($search_entity != '') $params.= '&amp;search_entity='.urlenref($search_entity);
	if ($search_ref != '') $params.= '&amp;search_ref='.urlenref($search_ref);
	if ($search_label != '') $params.= '&amp;search_label='.urlenref($search_label);
	if ($search_detail != '') $params.= '&amp;search_detail='.urlenref($search_detail);
	if ($search_fk_user_create != '') $params.= '&amp;search_fk_user_create='.urlenref($search_fk_user_create);
	if ($search_fk_user_mod != '') $params.= '&amp;search_fk_user_mod='.urlenref($search_fk_user_mod);
	if ($search_active != '') $params.= '&amp;search_active='.urlenref($search_active);
	...
	foreach ($search_array_options as $key => $val)
	{
		$crit=$val;
		$tmpkey=preg_replace('/search_options_/','',$key);
		if ($val != '') $param.='&search_options_'.$tmpkey.'='.urlenref($val);
	} 
	...
	print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $params, $sortfield, $sortorder, '', $num, $nbtotalofrecords, DOL_URL_ROOT.'/orgman/img/charge', 1, '', '', $limit);
    ...
    while ($i < min($num, $limit))
    {
    	...
    	if ($obj)
    	{
            ...
            // Action column
    			print '<td>';
    			if ($user->rights->orgman->char->write)
