...
/**
 *   	\file       poa/poaplanstrategic_list.php
 *		\ingroup    poa
...
$diroutputmassaction=$conf->poa->dir_output . '/temp/massgeneration/'.$user->id;
...
$arrayfields=array(
	...
	't.year_ini'=>array('label'=>$langs->trans("Fieldyear_ini"), 'checked'=>1),
	't.year_fin'=>array('label'=>$langs->trans("Fieldyear_fin"), 'checked'=>1),
	...
	//'t.entit'y=>array('label'=>$langs->trans("Entity"), 'checked'=>1, 'enabled'=>(! empty($conf->multicompany->enabled) && empty($conf->multicompany->transverse_mode))),
...
if (empty($reshook))
{
	...
	// Mass actions
	$objectclass='Poaplanstrategic';
	$objectlabel='Planstrategic';
	$permtoread = $user->rights->poa->read;
	$permtodelete = $user->rights->poa->delete;
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/plan/list.php',1);
			header("Location: ".$urltogo);
			exit;
		}
		...
		$object->year_ini=GETPOST('year_ini','int');
		$object->year_fin=GETPOST('year_fin','int');
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/poa/plan/list.php',1));
			exit;
		}
		else
		{
...
$sql.= " FROM ".MAIN_DB_PREFIX."poa_plan_strategic as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."poa_plan_strategic_extrafields as ef on (t.rowid = ef.fk_object)";
//$sql.= " WHERE 1 = 1";
$sql.= " WHERE t.entity IN (".getEntity('poaplanstrategic',1).")";
//fitramos por period_year
$sql.= " AND ".$period_year.">= t.year_ini ";
$sql.= " AND ".$period_year."<= t.year_fin ";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_ref) $sql.= natural_search("ref",$search_ref);
if ($search_year_ini) $sql.= natural_search("year_ini",$search_year_ini);
if ($search_year_fin) $sql.= natural_search("year_fin",$search_year_fin);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/poaplanstrategic/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.year_ini']['checked'])) print_liste_field_titre($arrayfields['t.year_ini']['label'],$_SERVER['PHP_SELF'],'t.year_ini','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.year_fin']['checked'])) print_liste_field_titre($arrayfields['t.year_fin']['label'],$_SERVER['PHP_SELF'],'t.year_fin','',$params,'',$sortfield,$sortorder);
...
// 
	...
	if (! empty($arrayfields['t.year_ini']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_year_ini" value="'.$search_year_ini.'" size="10"></td>';
	if (! empty($arrayfields['t.year_fin']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_year_fin" value="'.$search_year_fin.'" size="10"></td>';
...
if ($action == 'delete') {
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeletePlanStrategic'), $langs->trans('ConfirmDeletePlanStrategic'), 'confirm_delete', '', 0, 1);
	print $formconfirm;
}
...
if ($action =='create')
{
	...
	{
		print '<td>'.'<input type="text" id="ref" name="ref" value="'.$object->ref.'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.year_ini']['checked'])) 
	{
		print '<td>'.$form->selectarray('year_ini',$aYear,(GETPOST('year_ini')?GETPOST('year_ini'):$actualyear-4)).'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.year_fin']['checked'])) 
	{
		print '<td>'.$form->selectarray('year_fin',$aYear,(GETPOST('year_fin')?GETPOST('year_fin'):$actualyear)).'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.label']['checked'])) 
	{
		print '<td>'.'<input type="text" id="label" name="label" value="'.$object->label.'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.pseudonym']['checked'])) 
	{
		print '<td>'.'<input type="text" id="pseudonym" name="pseudonym" value="'.$object->pseudonym.'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	...
	print '<td>'.'<input type="submit" name="submit" value="'.$langs->trans('Save').'">'.'</td>';
	print '</tr>';
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		$object->id = $obj->rowid;
		$object->ref = str_pad($obj->ref, $conf->global->POA_CODE_SIZE_PLAN_STRATEGIC, "0", STR_PAD_LEFT);
		...
		if ($action ='edit' && $obj->rowid == $id)
		{
			...
			if (! empty($arrayfields['t.year_ini']['checked'])) 
			{
				print '<td>'.$form->selectarray('year_ini',$aYear,$obj->year_ini).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.year_fin']['checked'])) 
			{
				print '<td>'.$form->selectarray('year_fin',$aYear,$obj->year_fin).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked'])) 
			{
		...
		else
		{
			...
			if (! empty($arrayfields['t.year_ini']['checked'])) 
			{
				print '<td>'.$obj->year_ini.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.year_fin']['checked'])) 
			{
				print '<td>'.$obj->year_fin.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked'])) 
			{
		...
		// Status
			...
			if (! empty($arrayfields['t.status']['checked']))
			{
				print '<td align="center">'.$object->getLibStatut(3).'</td>';
			}
...
// Show total line
if (isset($totalarray['totalhtfield']) && $abc)
{
