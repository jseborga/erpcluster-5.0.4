...
$langs->load("products");
if ($conf->almacen->enabled) $langs->load('almacen');
$langs->load("orders");
$langs->load("bills");
$langs->load("stocks");
$langs->load("sendings");
...
if ($conf->almacen->enabled)
{
	...
	$movementadd = new Stockmouvementadd($db);
}
...
if ($action == "correct_stock" && ! $cancel)
{
	...
	if (! $error)
	{
		...
		if (is_numeric(GETPOST("nbpiece")) && $id)
		{
			...
			if (!$error)
			{
				//actualizamos la tabla stock_mouvement_add
				if (GETPOST('mouvement') == 1 && GETPOST('fk_stock_mouvement_entry'))
				{
					//restamos la cantidad que sale
					$res = $movementadd->fetch(0,GETPOST('fk_stock_mouvement_entry'));
					if ($res == 1)
					{
						$nbpiece = GETPOST('nbpiece');
						//restamos la cantidad que sale
						$movementadd->balance_peps = $movementadd->balance_peps - $nbpiece;
						//$movementadd->value_peps = GETPOST('unitprice');
						//$movementadd->value_ueps = 0;
						$movementadd->fk_user_mod = $user->id;
						$movementadd->date_mod = dol_now();
						$res = $movementadd->update($user);
						if ($res <=0)
						{
							$error++;
							setEventMessages($movementadd->error,$movementadd->errors,'errors');
						}
...
if ($id > 0 || $ref)
{
	...
	if ($result > 0)
	{
		...
		dol_fiche_head($head, 'Peps', $titre, 0, $picto);
	...
	{
		include DOL_DOCUMENT_ROOT.'/productext/stock/tpl/stockcorrection.tpl.php';
		print '<br><br>';
	}
...
if ($resql)
{
	...
	while ($i < $num)
	{
		...
		$lastPricepeps = $movement->lastPricepeps[$id];
		print '<td align="right">'.$aSaldo['qty'].($aSaldo['qty'] < 0 ?' '.img_warning():'').'</td>';
		...
		print '<td align="right">'.price(price2num($lastPricepeps,'MT')).'</td>';
		...
		$totalvalue = $totalvalue + $valueproduct;
		// PMP
