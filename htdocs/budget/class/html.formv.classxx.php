...
class Formv extends Form
{
	 ...
	 */
  	function select_all_categories($type, $selected='', $htmlname="parent", $maxlength=64, $excludeafterid=0,$includearray='',$required=0)
  	{
  		global $langs;
  		...
  		$output = '<select class="flat" name="'.$htmlname.'" '.$required.'>';
  		if (is_array($cate_arbo))
  		{
  			...
  			else
  			{
  				...
  				foreach($cate_arbo as $key => $value)
  				{
  					...
  					{
  						if ($includearray[$cate_arbo[$key]['id']])
   ...
   *  @return   void
   */
  	function select_produits_v($selected='', $htmlname='productid', $filtertype='', $limit=20, $price_level=0, $status=1, $finished=2, $selected_input_value='', $hidelabel=0, $ajaxoptions=array(),$socid=0,$action='',$filterstatic='',$filtercat='')
  	{
  		global $langs,$conf;
	  ...
	  //Price by customer
  			...
  			print ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.'/priceunits/ajax/products.php', $urloption, $conf->global->PRODUIT_USE_SEARCH_TO_SELECT, 0, $ajaxoptions);
  		...
  		{
  			//print $this->select_produits_list_v($selected,$htmlname,$filtertype,$limit,$price_level,$filterkey,$status,$finished,0,$socid,$filterstatic,$filtercat);
  		}
  	}
	 ...
	 */
	function select_produits_list_v($selected='',$htmlname='productid',$filtertype='',$limit=20,$price_level=0,$filterkey='',$status=1,$finished=2,$outputmode=0,$socid=0,$filterstatic='',$filtercat='')
	{
	...
	// Price by quantity
		...
		if ($filtercat)
		{
		//	$sql.= " INNER JOIN ".MAIN_DB_PREFIX."categorie_product AS cpr ON cpr.fk_product = p.rowid ";
		}
		//Price by customer
		if (! empty($conf->global->PRODUIT_CUSTOMER_PRICES) && !empty($socid)) {
		...
		//if ($filtercat) $sql.= " AND cpr.fk_categorie = ".$filtercat;
   ...
   */
	  ...
	  // Sort
  			...
  			foreach($array as $key => $value)
  			{
  				...
  				if ($id != '' && $id == $key) $out.=' selected="selected"';  
