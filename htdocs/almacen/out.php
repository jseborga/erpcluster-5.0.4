...
if (!$user->rights->almacen->creartransfout) accessforbidden();
...
if ($action == "create" || $action=='endt' || $action == 'edit')
{
	...
	if (count($aItemRow)>0 && $action=="endt")
	{
		...
		print $formproduct->selectWarehouses(($_GET["dwid"]?$_GET["dwid"]:GETPOST('id_entrepot_destination')),'id_entrepot_destination','',1);
		print '</td>';
		print '</tr>';
		print '<tr>';
