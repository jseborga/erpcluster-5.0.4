...
/**
 *   	\file       assets/assetsassignmentdet_list.php
 *		\ingroup    assets
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-04-07 12:17
 */
...
dol_include_once('/assets/class/assetsassignmentdet.class.php');
...
't.fk_asset_assignment'=>array('label'=>$langs->trans("Fieldfk_asset_assignment"), 'checked'=>1),
...
$object=new Assetsassignmentdet($db);
...
$search_status='';
    ...
    $permtoread = $user->rights->assetsassignmentdet->read;
    $permtodelete = $user->rights->assetsassignmentdet->delete;
    $uploaddir = $conf->assetsassignmentdet->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."assets_assignment_det_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_asset_assignment) $sql.= natural_search("fk_asset_assignment",$search_fk_asset_assignment);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/assetsassignmentdet/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_asset_assignment']['checked'])) print_liste_field_titre($arrayfields['t.fk_asset_assignment']['label'],$_SERVER['PHP_SELF'],'t.fk_asset_assignment','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_asset_assignment']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_asset_assignment" value="'.$search_fk_asset_assignment.'" size="10"></td>';
