...
class pdf_licenrep extends ModelePDFAssistance
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
			...
			if (file_exists($dir))
			{
				...
				foreach ((array)$aLicencias as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-1);
						...
						$pdf->MultiCell($this->posxnom-$this->posxci-1, 1, $line['paterno'].' '.$line['materno'].' '.$line['nombre'], 0, 'L',0);
						
						$pdf->SetXY($this->posxnom, $curY);
						$pdf->MultiCell($this->posxfini-$this->posxnom-1, 1, $line['date_ini'], 0, 'C',0);
						$pdf->SetXY($this->posxfini, $curY);
						$pdf->MultiCell($this->posxffin-$this->posxfini-1, 1, $line['date_fin'], 0, 'C',0);
						$pdf->SetXY($this->posxffin, $curY);
						$pdf->MultiCell($this->posxfrini-$this->posxffin-1, 1, $line['date_ini_ejec'], 0, 'C',0);
						
						$pdf->SetXY($this->posxfrini, $curY);
						$pdf->MultiCell($this->posxfrfin-$this->posxfrini-1, 1, $line['date_fin_ejec'], 0, 'C',0);
						$pdf->SetXY($this->posxfrfin, $curY);
						$pdf->MultiCell($this->posxfin-$this->posxfrfin-1, 1, $line['estado'], 0, 'C',0);
				...
				//Tabla del total
				//$this->_tableau_total($pdf, $object, $outputlangs , $bottomlasttab, $totA);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxnom-$this->posxci-1,2, $outputlangs->transnoentities("Apellidos y Nombre"),'','C');
			$pdf->line($this->posxnom-1, $tab_top, $this->posxnom-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxnom-1, $tab_top+2);
			$pdf->MultiCell($this->posxfini-$this->posxnom-1,2, $outputlangs->transnoentities("Desde Fecha"),'','C');
			$pdf->line($this->posxfini-1, $tab_top, $this->posxfini-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxfini-1, $tab_top+2);
			$pdf->MultiCell($this->posxffin-$this->posxfini-1,2, $outputlangs->transnoentities("Hasta Fecha"),'','C');
			$pdf->line($this->posxffin-1, $tab_top, $this->posxffin-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxffin-1, $tab_top+2);
			$pdf->MultiCell($this->posxfrini-$this->posxffin-1,2, $outputlangs->transnoentities("Fecha y Hora Salida"),'','C');
			$pdf->line($this->posxfrini-1, $tab_top, $this->posxfrini-1, $tab_top + $tab_height);
			
			$pdf->SetXY($this->posxfrini-1, $tab_top+2);
			$pdf->MultiCell($this->posxfrfin-$this->posxfrini-1,2, $outputlangs->transnoentities("Fecha y Hora Retorno"),'','C');
			$pdf->line($this->posxfrfin-1, $tab_top, $this->posxfrfin-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxfrfin-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxfrfin-1,2, $outputlangs->transnoentities("Estado"),'','C');
			//$pdf->line($this->posxci-1, $tab_top, $this->posxci-1, $tab_top + $tab_height);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$titulo = "LISTADO DE SOLICITUDES DE LICENCIA";
	...
	function pdf_pie_page_Ltrescol(&$pdf, $object, $posy, $outputlangs,$marge_gauche, $page_largeur,$marge_droite,$page_hauteur,$type_page=0,$bottomlasttab)
	{
  	...
  	function _tableau_total(&$pdf, $object, $outputlangs , $bottomlasttab, $sumaTotal)
	{
		global $conf,$mysoc;
		...
		$pdf->SetXY($col1x, $tab2_top + 2);
		$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("Total"), 0, 'L', 1);

		$total_ttc =$sumaTotal;
		$pdf->SetXY(250, $tab2_top + 2);
		$pdf->MultiCell(20, $tab2_hl, price($total_ttc), 0, 'R', 0);
