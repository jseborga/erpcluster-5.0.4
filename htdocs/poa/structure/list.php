...
$sql  = "SELECT p.rowid AS id, p.ref, p.label, p.type, p.pseudonym, p.fk_area, p.fk_area_ej, p.statut, p.fk_father, p.sigla, p.pos, ";
$sql.= " p.version ";
		...
		{	      
			...
			else
			{
				...
				if ($obj->pos == $nStructure)
					print '<a href="'.DOL_URL_ROOT.'/poa/structure/card.php?id='.$obj->id.'">'.img_picto($langs->trans("Planification"),DOL_URL_ROOT.'/poa/img/planif.png','',1).'</a>';
				if ($obj->pos < $nStructure && $user->rights->poa->plan->write)
					print '&nbsp;&nbsp;<a href="'.$_SERVER['PHP_SELF'].'?fk_father='.$obj->id.'&action=create" title="'.$langs->trans('Createnewlevel').'">'.img_picto($langs->trans("Createnewlevel"),'rightarrow').'</a>';
				print '</td>';
			}
			print "</tr>\n";
