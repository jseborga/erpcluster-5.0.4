...
/**
 *   	\file       orgman/caltitude_card.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-04-12 09:35
 */
...
dol_include_once('/orgman/class/caltitude.class.php');
...
$hookmanager->initHooks(array('caltitude'));
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/altitude/list.php',1);
	...
	if ($action == 'add')
	{
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/altitude/card.php?id='.$result,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/altitude/list.php',1));
			exit;
		}
		else
		{
	...
	if ($action == 'confirm_validate')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/altitude/card.php?id='.$id,1));
			exit;
		}
		else
		{
...
llxHeader('',$langs->trans('Caltitude'),'');
...
//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldentity").'</td><td><input class="flat" type="text" name="entity" value="'.GETPOST('entity').'"></td></tr>';
	...
	print '<td>'.$langs->trans("Fieldfk_parent").'</td><td>';
...
{
	print load_fiche_titre($langs->trans("Caltitude"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteAltitude'), $langs->trans('ConfirmDeleteAltitude'), 'confirm_delete', '', 0, 1);
		print $formconfirm;
	}
	if ($action == 'validate') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ValidateAltitude'), $langs->trans('ConfirmValidateAltitude'), 'confirm_validate', '', 0, 1);
		print $formconfirm;
	}
	...
	if (empty($reshook))
	{
		if ($user->rights->orgman->alt->write)
		{
		...
		if ($object->status == 0 && $user->rights->orgman->alt->del)
		{
	...
	//$linktoelem = $form->showLinkToObjectBlock($object, null, array('caltitude'));
