...
class Assetsassignment extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fi_property_from)) {
			 $this->fi_property_from = trim($this->fi_property_from);
		}
		...
		$sql .= ' '.(! isset($this->fi_property_from)?'NULL':$this->fi_property_from).',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fi_property_from = $obj->fi_property_from;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->fi_property_from = $obj->fi_property_from;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fi_property_from = '.(isset($this->fi_property_from)?$this->fi_property_from:"null").',';
	...
	function LibStatut($status,$mode=0)
	{
		...
		if ($mode == 0)
		{
			...
			if ($status == 1) return $langs->trans('Requested');
		...
		if ($mode == 2)
		{
			...
			if ($status == 1) return img_picto($langs->trans('Requested'),'statut1').' '.$langs->trans('Requested');
			if ($status == 2) return img_picto($langs->trans('Validated'),'statut3').' '.$langs->trans('Validated');
			if ($status == 3) return img_picto($langs->trans('Accepted'),'statut4').' '.$langs->trans('Accepted');
		}
		if ($mode == 3)
		{
