...
/**
 * \file    orgman/cdepartements.class.php
 * \ingroup orgman
 ...
 * @see CommonObject
 */
class Cdepartements extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'cdepartements';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'c_departements';

	/**
	 * @var CdepartementsLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->tncc)) {
			 $this->tncc = trim($this->tncc);
		}
		if (isset($this->ncc)) {
			 $this->ncc = trim($this->ncc);
		}
		if (isset($this->nom)) {
		...
		$sql .= ' '.(! isset($this->tncc)?'NULL':$this->tncc).',';
		$sql .= ' '.(! isset($this->ncc)?'NULL':"'".$this->db->escape($this->ncc)."'").',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("cdepartements", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new CdepartementsLine();

				$line->id = $obj->rowid;
				
				$line->code_departement = $obj->code_departement;
				$line->fk_region = $obj->fk_region;
				$line->cheflieu = $obj->cheflieu;
				$line->tncc = $obj->tncc;
				$line->ncc = $obj->ncc;
				$line->nom = $obj->nom;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' tncc = '.(isset($this->tncc)?$this->tncc:"null").',';
		$sql .= ' ncc = '.(isset($this->ncc)?"'".$this->db->escape($this->ncc)."'":"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Cdepartements($this->db);
