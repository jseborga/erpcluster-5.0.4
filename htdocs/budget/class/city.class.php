...
class City extends CommonObject
{
	 ...
	 */
	public function fetch($id, $ref = null,$fk_country=0,$fk_departement=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		$sql.= ' WHERE 1 = 1';
		if (null !== $ref && $fk_country>0 && $fk_departement>0) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
			$sql .= ' AND t.fk_country = ' . $fk_country;
			$sql .= ' AND t.fk_departement = ' . $fk_departement;
		} else {
			$sql .= ' AND t.rowid = ' . $id;
		}
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("city", 1) . ")";
		}
		if (count($sqlwhere) > 0) {
