...
if ($action == 'addgroupot' && $user->rights->mant->jobs->crear)
  {
    $error=0;
    $date_create  = dol_now();
    $db->begin();
	...
	//debemos crear la orden de trabajo
	...
	$object->address_ip = $objwork->address_ip;
	...
	if ($res>0)
	  {
	    $aJobs[$object->fk_soc][$object->ref] = $object->ref;
	...
	foreach ((array) $_SESSION['seletick'] AS $idw => $value)
	  {
	    ...
	    if ($resu > 0) $objTypent = fetch_typent($objSoc->typent_id);
	    if ($objTypent->id == $objSoc->typent_id && $objTypent->code == 'TE_BCB')
	      $lSelectSocid = false;
	    //envio de correo a tecnicos del ticket
	    $emailto = $objSoc->email;
	    if ($emailto && $lSelectSocid)
	      {
		//sendmail
		...
		$tmpsujet = $langs->trans('Sende-workorder');
		$sendto   = $emailto;
		...
		$tmpbody = htmlsendemailassignte($id,$url,$aTick);
		$msgishtml = 1;
...
if ($action == 'create' && $user->rights->mant->tick->ass) //asignar
  {
    ...
    if (!empty($aseletick))
      {
	print_fiche_titre($langs->trans("Createworkorder"));
	...
	print '<input type="hidden" name="action" value="addgroupot">';
    ...
    print '<center><br>'.'<span style="color:#ff0000;">'.$langs->trans('Recomendacion:').'</span>'.' '.$langs->trans('Esta accion creara las ordenes de trabajo. Una vez presionado el boton').' '.'<span style="color:#002aff">'.$langs->trans('CREAR').'</span>'.' '.$langs->trans('no se puede volver atras.').'</center>';
