...
if ($action == 'update' && ! $_POST["cancel"] && $user->rights->salary->member->creer)
{
	...
	if ($result > 0 && ! $error)
	{
		...
		//$objectpu->oldcopy=dol_clone($objectpu);
		...
		if ($objectpu->fk_user == $rowid)
			$objectpu->update($user);
		else
		{
			//creando el registro
			$objectpu->fk_user = $rowid;
			$objectpu->create($user);
		}
...
else
{
	...
	if ($action == 'edit')
	{
	 ...
	 {
	 	$sql = "SELECT rowid, code, label as label from ".MAIN_DB_PREFIX."c_country where rowid = ".(!empty($country)?$country:$object->country_id);
	 	$resql=$db->query($sql);
	 	if ($resql)
	 	{
	 		$obj = $db->fetch_object($resql);
	 ...
	 if ($backtopage) print '<input type="hidden" name="backtopage" value="'.($backtopage != '1' ? $backtopage : $_SERVER["HTTP_REFERER"]).'">';
		...
		// Lastnametwo
	 	print '<tr><td id="tdlastnametwo">'.$langs->trans("Lastnametwo").'</td><td><input type="text" name="lastnametwo" size="40" value="'.(isset($_POST["lastnametwo"])?$_POST["lastnametwo"]:$objectpu->lastnametwo).'"></td>';
	 	print '</tr>';
	...
	if ($rowid && $action != 'edit')
	{
		...
		*/
		...
		print select_depto($objectpu->issued_in,'issued_in','',0,0,1,'code','');
		print '</td>';
		print '</tr>';
		...
		// Tel emergency
		print '<tr><td>'.$langs->trans("Phoneemergency").'</td><td>';
		print $objectpu->phone_emergency;
		print '</td></tr>';
		...
		if (! empty($conf->societe->enabled))
		{
			...
			print '</td>';
			if ($action != 'editthirdparty' && $user->rights->salary->member->creer) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editthirdparty&amp;rowid='.$object->id.'">'.img_edit($langs->trans('SetLinkToThirdParty'),1).'</a></td>';
			print '</tr></table>';
		...
		print '</td>';
		if ($action != 'editlogin' && $user->rights->salary->member->creer)
		{
