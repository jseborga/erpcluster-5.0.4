 ...
 *				Initialy built by build_class_from_table on 2014-03-10 15:10
 */
...
class Mjobs extends CommonObject
{
	...
	var $element='mjobs';			//!< Id that identify managed objects
	...
	$modulepart = 'mant';
        ...
        if ($imageview == 'ini')
	  {
            ...
            $smallfile=$object->image_ini;
            $smallfile=preg_replace('/(\.png|\.gif|\.jpg|\.jpeg|\.bmp)/i','_small\\1',$smallfile);
            if ($object->image_ini) $file=$id.'/images/thumbs/'.$smallfile;
	    $namephoto = 'photoini';
	  }
        if ($imageview == 'fin')
	  {
            $dir=$conf->mant->multidir_output[$entity];
            $smallfile=$object->image_fin;
            $smallfile=preg_replace('/(\.png|\.gif|\.jpg|\.jpeg|\.bmp)/i','_small\\1',$smallfile);
            if ($object->image_fin) $file=$id.'/images/thumbs/'.$smallfile;
	    $namephoto = 'photofin';
	  }
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
    function getlist($fk_work_request)
    {
      global $langs;
      $sql = "SELECT";
      $sql.= " t.rowid,";
