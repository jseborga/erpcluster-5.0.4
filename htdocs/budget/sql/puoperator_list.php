...
/**
 *   	\file       priceunits/puoperator_list.php
 *		\ingroup    priceunits
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-10-24 15:40
 */
...
dol_include_once('/priceunits/class/puoperator.class.php');
...
$search_operator=GETPOST('search_operator','alpha');
...
$hookmanager->initHooks(array('puoperatorlist'));
...
't.operator'=>array('label'=>$langs->trans("Fieldoperator"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."pu_operator as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pu_operator_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_detail) $sql.= natural_search("detail",$search_detail);
if ($search_operator) $sql.= natural_search("operator",$search_operator);
...
if ($search_operator != '') $params.= '&amp;search_operator='.urlencode($search_operator);
...
if (! empty($arrayfields['t.operator']['checked'])) print_liste_field_titre($arrayfields['t.operator']['label'],$_SERVER['PHP_SELF'],'t.operator','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.operator']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_operator" value="'.$search_operator.'" size="10"></td>';
