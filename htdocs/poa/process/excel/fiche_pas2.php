...
if ($id || $_GET['id'])
  {
    ...
    if ($user->rights->poa->prev->leer)
      print '<a class="butAction" href="'.DOL_URL_ROOT.'/poa/execution/ficheprev.php'.(isset($_GET['nopac'])?'?nopac=1&id='.$idp:'?id='.$idp).'">'.$langs->trans("Return").'</a>';
    else
	...
	foreach((array) $objpcon->array AS $j => $objpcontrat)
	  {
		...
		$var=!$var;
		print "<tr $bc[$var]>";
		print '<td>'.'<a href="fiche_pas1.php?action=selcon&id='.$object->id.'&idrc='.$objpcontrat->id.(isset($_GET['nopac'])?'&nopac=1&idp='.$_GET['idp']:'').'">'.$objcont->array_options['options_ref_contrato'].($contratAdd?', '.$contratAdd:'').'</a></td>';
		...
		if (empty($action) && $user->rights->poa->deve->crear)
		  if ($user->admin || $user->id == $objprev->fk_user_create)
		    print '<td align="center">'.'<a href="fiche_pas2.php?id='.$id.'&idrc='.$objpcontrat->id.'&idp='.$idp.'&action=create'.(isset($_GET['nopac'])?'&nopac=1&idp='.$_GET['idp']:'').'">'.img_picto($langs->trans('Pay'),DOL_URL_ROOT.'/poa/img/payment.png','',1).'</a>'.'</td>';
		  else
		...
		if ($action == 'create' && $idrc == $objpcontrat->id && $user->rights->poa->deve->crear)
		  {
			...
			if ($objcompr->get_sum_pcp2($fk_poaprev,$idc))
			  {
			    //total comprometido
			...
			if ($objdeveng->get_sum_pcp2($fk_poaprev,$idc,true))
			  {
			    //total comprometido
		    ...
		    // echo '</pre>';
 		    ...
 		    $saldo = price2num($aTotalcomp[$gestion] - $totaldev,'MT');
		    //cambiar
		    //$saldo = 1000;
		    // //determinamos saldo
			...
			foreach((array) $objdev->array AS $j => $objdeve)
			  {
				...
				print number_format(price2num($objdeve->amount,'MT'),2);
				print '</td>';
				...
				print '<td align="center">';
				if ($user->rights->poa->deve->mod || ($nLoop == $nLen && $user->id == $objprevnew->fk_user_create))
				  {
				    if ($gestion == $gest)
				      print '<a href="fiche_pas2.php?id='.$id.'&idr='.$objdeve->id.'&idp='.$objprevnew->id.'&action=editdev'.'">'.img_picto($langs->trans("Edit"),'edit').'</a>';
				    print '&nbsp;';
				  }
				if ($user->rights->poa->deve->del)
				  {
				    if ($gestion == $gest)
				      print '<a href="fiche_pas2.php?id='.$id.'&idr='.$objdeve->id.'&action=deletedev'.'">'.img_picto($langs->trans("Delete"),'delete').'</a>';
				  }
				else
				  print '&nbsp;';
				//exporta excel
				...
				print '<a href="fiche_autpay.php?id='.$id.'&idr='.$objdeve->id.'&amp;idc='.$objcont->id.'">'.img_picto($langs->trans("Authorization by payment"),DOL_URL_ROOT.'/poa/img/excel-icon','',1).'</a>';
				print '&nbsp;';
				...
				if ($user->admin || $user->rights->poa->deve->mod)
				  if ($action == 'upload' && $idr == $objdeve->id)
				    ...
				    {
				      print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$objdeve->id.'&action=upload'.'">'.img_picto($langs->trans('Uploaddoc'),DOL_URL_ROOT.'/poa/img/subir.png','',1).'</a>';
