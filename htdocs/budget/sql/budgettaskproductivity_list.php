...
/**
 *   	\file       budget/budgettaskproductivity_list.php
 *		\ingroup    budget
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-01-13 19:12
 */
...
dol_include_once('/budget/class/budgettaskproductivity.class.php');
...
$search_fk_budget_task_resource=GETPOST('search_fk_budget_task_resource','int');
...
$hookmanager->initHooks(array('budgettaskproductivitylist'));
...
$object=new Budgettaskproductivity($db);
...
't.fk_budget_task_resource'=>array('label'=>$langs->trans("Fieldfk_budget_task_resource"), 'checked'=>1),
't.code_parameter'=>array('label'=>$langs->trans("Fieldcode_parameter"), 'checked'=>1),
...
$search_fk_budget_task_resource='';
...
$sql.= " FROM ".MAIN_DB_PREFIX."budget_task_productivity as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."budget_task_productivity_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_budget_task_resource) $sql.= natural_search("fk_budget_task_resource",$search_fk_budget_task_resource);
...
if ($search_fk_budget_task_resource != '') $params.= '&amp;search_fk_budget_task_resource='.urlencode($search_fk_budget_task_resource);
if ($search_code_parameter != '') $params.= '&amp;search_code_parameter='.urlencode($search_code_parameter);
...
if ($search_status != '') $params.= '&amp;search_status='.urlencode($search_status);
    ...
    // 
if (! empty($arrayfields['t.fk_budget_task_resource']['checked'])) print_liste_field_titre($arrayfields['t.fk_budget_task_resource']['label'],$_SERVER['PHP_SELF'],'t.fk_budget_task_resource','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.code_parameter']['checked'])) print_liste_field_titre($arrayfields['t.code_parameter']['label'],$_SERVER['PHP_SELF'],'t.code_parameter','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.status']['checked'])) print_liste_field_titre($arrayfields['t.status']['label'],$_SERVER['PHP_SELF'],'t.status','',$params,'',$sortfield,$sortorder);
	...
	print '<tr class="liste_titre">';
	// 
if (! empty($arrayfields['t.fk_budget_task_resource']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_budget_task_resource" value="'.$search_fk_budget_task_resource.'" size="10"></td>';
if (! empty($arrayfields['t.code_parameter']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_code_parameter" value="'.$search_code_parameter.'" size="10"></td>';
