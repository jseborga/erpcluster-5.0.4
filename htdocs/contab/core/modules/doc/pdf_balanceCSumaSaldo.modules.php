...
class pdf_balanceCSumaSaldo extends ModelePDFContab
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->contab->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->contab->dir_output . "/contab/".$period_year.'/sumBalance';
				$file = $dir . "/balanceSumaSaldo.pdf";
			}
			//echo $file;
			if (! file_exists($dir))
			{
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-1);
						...
						$pdf->MultiCell($this->posxnom-$this->posxref-1, 1, $line['nombre'], 0, 'L',0);
						$nexY = $pdf->GetY();

						if($line['debito'] > 0){
							$pdf->SetXY($this->posxnom, $curY);
							$pdf->MultiCell($this->posxdeb-$this->posxnom-1, 1, price(price2num($line['debito'],'MT')), 0, 'R',0);
						}else{
							$pdf->SetXY($this->posxnom, $curY);
							$pdf->MultiCell($this->posxdeb-$this->posxnom-1, 1, price(price2num($line['debito'],'MT')), 0, 'R',0);
						}
						...
						if($line['credito'] > 0){
							...
							$pdf->MultiCell($this->posxcre-$this->posxdeb-1, 1, price(price2num($line['credito'],'MT')), 0, 'R',0);
						}else{
							$pdf->SetXY($this->posxdeb, $curY);
							$pdf->MultiCell($this->posxcre-$this->posxdeb-1, 1, price(price2num($line['credito'],'MT')), 0, 'R',0);
						}
						...
						if($line['deudor'] > 0){
							...
							$pdf->MultiCell($this->posxdeu-$this->posxcre-1, 1,price(price2num($line['deudor'],'MT')), 0, 'R',0);
						}else{
							$pdf->SetXY($this->posxcre, $curY);
							$pdf->MultiCell($this->posxdeu-$this->posxcre-1, 1,price(price2num($line['deudor'],'MT')), 0, 'R',0);
						}
						...
						if($line['deudor1'] > 0){
							...
							$pdf->MultiCell($this->posxfin-$this->posxdeu-1, 1, price(price2num($line['deudor1'],'MT')), 0, 'R',0);
						}else{
							$pdf->SetXY($this->posxdeu, $curY);
							$pdf->MultiCell($this->posxfin-$this->posxdeu-1, 1, price(price2num($line['deudor1'],'MT')), 0, 'R',0);
						}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		*/
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxnom-$this->posxref-1,2, $outputlangs->transnoentities("Name"),'','C');
			$pdf->line($this->posxnom-1, $tab_top, $this->posxnom-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxnom-1, $tab_top+3);
			$pdf->MultiCell($this->posxdeb-$this->posxnom-1,2, $outputlangs->transnoentities("Debito"),'','C');
			$pdf->line($this->posxdeb-1, $tab_top, $this->posxdeb-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxdeb-1, $tab_top+2);
			$pdf->MultiCell($this->posxcre-$this->posxdeb-1,2, html_entity_decode($outputlangs->transnoentities("Crédito")),'','C');
			$pdf->line($this->posxcre-1, $tab_top, $this->posxcre-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxcre-1, $tab_top+2);
			$pdf->MultiCell($this->posxdeu-$this->posxcre-1,2, $outputlangs->transnoentities("Deudor"),'','C');
			$pdf->line($this->posxdeu-1, $tab_top, $this->posxdeu-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxdeu-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxdeu-1,2, $outputlangs->transnoentities("Acreedor"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxini, 3, $aReporte[7], '', 'C');
		$posy+=6;
		$titulo = "BALANCE DE COMPROBACION DE SUMAS Y SALDOS";
		$pdf->SetFont('','B', $default_font_size+3);
		$pdf->SetXY($this->posxini +2,$posy);
		$pdf->SetTextColor(0,0,60);
		//$title=$outputlangs->transnoentities("Determinacion de Servicios");
		$title=$outputlangs->transnoentities($titulo);
		$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxini, 3, $title, '', 'C');

		$pdf->SetFont('','B',$default_font_size+1);
		//fecha
		$title = $outputlangs->transnoentities("Tothe").' '. dol_print_date($aReporte[6],'daytext');
		$posy+=6;
		$pdf->SetXY($this->posxini+2,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxini, 3, $title, '', 'C');
		//expresado en
		$title = '('.$outputlangs->transnoentities("Expressedinbolivianos").')';
		$posy+=6;
		$pdf->SetXY($this->posxini +2,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell( $this->page_largeur-$this->marge_droite-$this->posxini, 3, $title, '', 'C');
	...
	function pdf_pie_total(&$pdf, $object, $posy, $outputlangs,$marge_gauche, $page_largeur,$marge_droite,$page_hauteur,$type_page=0,$bottomlasttab)
	{
		...
		else
		{
			...
			$pdf->SetXY($this->posxref, $curY);
			$pdf->MultiCell($this->posxnom-$this->posxref-1, 1, $langs->trans("Total")." : ", 1, 'R',0);
			$pdf->SetXY($this->posxnom, $curY);
			$pdf->MultiCell($this->posxdeb-$this->posxnom-1, 1, price(price2num($aReporte[2],'MT')), 0, 'R',1);
			$pdf->SetXY($this->posxdeb, $curY);
			$pdf->MultiCell($this->posxcre-$this->posxdeb-1, 1, price(price2num($aReporte[3],'MT')), 0, 'R',1);
			$pdf->SetXY($this->posxcre, $curY);
			$pdf->MultiCell($this->posxdeu-$this->posxcre-1, 1, price(price2num($aReporte[4],'MT')), 0, 'R',1);
			$pdf->SetXY($this->posxdeu, $curY);
			$pdf->MultiCell($this->posxfin-$this->posxdeu-1, 1, price(price2num($aReporte[5],'MT')), 0, 'R',1);
		}
