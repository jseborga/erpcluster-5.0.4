...
/**
 * \file    salary/pregional.class.php
 * \ingroup salary
 ...
 * @see CommonObject
 */
class Pregional extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'pregional';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'p_regional';

	/**
	 * @var PregionalLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("pregional", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Regional") . '</u>';
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Label') . ':</b> ' . $this->label;

        $url = DOL_URL_ROOT.'/salary/regional/'.'fiche.php?id='.$this->id;
        ...
        if ($withpicto)
        {
            $result.=($linkstart.img_object('', 'globe' ).$linkend);
