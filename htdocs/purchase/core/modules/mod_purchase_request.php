...
/**
 *	\file       htdocs/almacen/core/modules/mod_purchase_request.php
 *	\ingroup    almacenes
 *	\brief      File containing class for numbering module Ubuntubo
 */
require_once DOL_DOCUMENT_ROOT .'/purchase/core/modules/purchase/modules_purchase.php';

/**	    \class      mod_purchase_request
 *		\brief      Classe du modele de numerotation de reference de pedidos almacen ubuntubo
 */
class mod_purchase_request extends ModeleNumRefPurchase
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefixinvoice='SOLC';
	var $prefixcreditnote='AV';
	var $error='';
	var $nom='Purchaserequest';
	...
	function info()
	{
		global $langs;
		$langs->load("purchase");
		return $langs->trans('FractalNumRefModelDesc1',$this->prefixinvoice,$this->prefixcreditnote);
	}
	...
	function getExample()
	{
		return $this->prefixinvoice."1707-0001";
	}
	...
	function canBeActivated()
	{
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."purchase_request";
	...
	function getNextValue($objsoc,$facture,$mode='next')
	{
		...
		$sql.= " AND entity = ".$conf->entity;
		$resql=$db->query($sql);
		dol_syslog("mod_purchase_request::getNextValue sql=".$sql);
		if ($resql)
		{
		...
		{
			dol_syslog("mod_purchase_request::getNextValue sql=".$sql, LOG_ERR);
			return -1;
		}
		...
		{
			$date=$facture->datec;	// This is invoice date (not creation date)
			...
			if (empty($date))
			{
				...
				$fayymm = strftime("%y%m",$date);
			dol_syslog("mod_purchase_request::getNextValue return ".$prefix.$fayymm."-".$num);
			return $prefix.$fayymm."-".$num;
		}
