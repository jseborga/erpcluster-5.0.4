...
if (empty($reshook))
{
	...
	elseif ($action == 'addline' && $user->rights->contrat->creer)
	{
		...
		if (! $error)
		{
			...
			if ($conf->fiscal->enabled)
			{
				...
				$lines->date_start = $date_start;
				$lines->date_end = $date_end;
			...
			if ($result > 0)
			{
				...
				if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
				{
					...
					if (!$object->modelpdf) $object->modelpdf = 'fractal';
	...
	{
		$objectline = new ContratLigneext($db);
		if ($objectline->fetch(GETPOST('elrowid')))
		{
			...
			$tvattc = array();
			$tvatx = array();
			$lines = new Contratligneext($db);
			$lines->fetch(GETPOST(elrowid));
			$k = 1;
			$lines->qty = GETPOST('elqty');
			$qty = GETPOST('elqty');
			$pu = GETPOST('elprice');
			$price_base_type = 'HT';
			$lines->subprice = $pu;
			...
			$objectline->price_ht=$lines->subprice;
			$objectline->subprice=$lines->subprice;
			$objectline->qty=GETPOST('elqty');
			$objectline->remise_percent=$lines->remise_percent;
			$objectline->remise=$lines->remise;
			$objectline->tva_tx=$lines->tva_tx;
			$objectline->localtax1_tx=is_numeric($lines->localtax1_tx)?$lines->localtax1_tx:0;
			$objectline->localtax2_tx=is_numeric($lines->localtax2_tx)?$lines->localtax2_tx:0;

			$objectline->total_ht=$lines->total_ht;
			$objectline->total_ttc=$lines->total_ttc;
			$objectline->total_tva=$lines->total_tva;

			$objectline->total_localtax1=$lines->total_localtax1;
			$objectline->total_localtax2=$lines->total_localtax2;
			$objectline->localtax1_type=$lines->localtax1_type;
			$objectline->localtax2_type=$lines->localtax2_type;
			...
			$objectline->fk_unit = $lines->fk_unit+0;
			...
			$result=$objectline->updateadd($user);
			if ($result > 0)
			{
				$db->commit();
			}
...
/* *************************************************************************** */
{
	...
	if ($object->id > 0)
	{
		...
		$head = sales_contract_prepare_head($object);
		...
		$linkback = '<a href="'.DOL_URL_ROOT.'/sales/contrat/list.php'.(! empty($socid)?'?socid='.$socid:'').'">'.$langs->trans("BackToList").'</a>';
	   ...
	   $arrayothercontracts=$object->getListOfContracts('others');
		...
		while ($cursorline <= $nbofservices)
		{
			...
			$sql.= " cd.total_ht, cd.total_tva, cd.total_ttc, cd.remise,";
			...
			if ($result)
			{
				...
				print '<td width="50" align="center">'.$langs->trans("Total").'</td>';
				...
				if ($action != 'editline' || GETPOST('rowid') != $objp->rowid)
				{
					...
					if ($objp->fk_product > 0)
					{
						...
						print $form->textwithtooltip($text,$description,3,'','',$cursorline,0,(!empty($line->fk_parent_line)?img_picto('', 'rightarrow'):''));
					...
					if ($conf->global->PRICE_TAXES_INCLUDED)
					{
						$price = price2num($objp->total_ttc / $objp->qty,'MU');
						print '<td align="right">'.($price != '' ? price($price) : '')."</td>\n";
					}
					else
				...
				else
				{
					...
					$doleditor->Create();
					print '<input type="hidden" name="eltva_tx" value="'.$objp->tva_tx.'">';
					print '</td>';
					...
					{
						$price = price2num($objp->total_ttc / $objp->qty,'MU');
						print '<td align="right"><input size="5" type="text" name="elprice" value="'.price($price).'"></td>';
					}
					else
					...
					print '<td align="right" class="nowrap"><input size="1" type="text" name="elremise_percent" value="'.$objp->remise_percent.'">% <input size="5" type="text" name="discount" value="'.$objp->remise.'"></td>';
					if (! empty($usemargins))
					{
						print '<td align="right">';
		...
		if ($user->societe_id == 0)
		{
			...
			//agregamos boton para crear proyecto
			if (! empty($conf->monprojet->enabled) && $object->statut > 0 && $object->nbofservicesclosed < $nbofservices && $solovisual)
			{
				$langs->load("sales@sales");
				if ($user->rights->projet->creer) print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/monprojet/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->thirdparty->id.'">'.$langs->trans("CreateServiceOrder").'</a></div>';
			...
			{
				$langs->load("bills");
				if ($user->rights->facture->creer) print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/sales/compta/facture.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->thirdparty->id.'">'.$langs->trans("CreateBill").'</a></div>';
				else print '<div class="inline-block divButAction"><a class="butActionRefused" href="#" title="'.$langs->trans("NotEnoughPermissions").'">'.$langs->trans("CreateBill").'</a></div>';
			}
