 ...
 */
abstract class ModelePDFAlmacen extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleAlmacen extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleNumRefAlmacen
{
	var $error='';
 ...
 */
function almacen_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false)
{
	...
	foreach($dirmodels as $reldir)
	{
		foreach(array('doc','pdf') as $prefix)
		{
			...
			$file=dol_buildpath($reldir."almacen/core/modules/doc/".$file,0);
			if (file_exists($file))
			{
				$filefound=1;
	...
	if ($filefound)
	{
		...
		{
			$outputlangs->charset_output=$sav_charset_output;
			dol_print_error($db,"almacen_pdf_create Error: ".$obj->error);
			return -1;
		}
