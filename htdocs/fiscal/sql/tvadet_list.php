...
/**
 *   	\file       fiscal/tvadet_list.php
 *		\ingroup    fiscal
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-05-11 16:38
 */
...
dol_include_once('/fiscal/class/tvadet.class.php');
...
't.fk_tva'=>array('label'=>$langs->trans("Fieldfk_tva"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->tvadet->read;
    $permtodelete = $user->rights->tvadet->delete;
    $uploaddir = $conf->tvadet->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."tvadet as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."tvadet_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_tva) $sql.= natural_search("fk_tva",$search_fk_tva);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/tvadet/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_tva']['checked'])) print_liste_field_titre($arrayfields['t.fk_tva']['label'],$_SERVER['PHP_SELF'],'t.fk_tva','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_tva']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_tva" value="'.$search_fk_tva.'" size="10"></td>';
