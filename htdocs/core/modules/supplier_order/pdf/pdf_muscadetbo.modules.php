 ...
 */
class pdf_muscadetbo extends ModelePDFSuppliersOrders
{
    var $db;
    var $name;
    var $description;
		...
		if ($conf->fournisseur->dir_output.'/commande')
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					// Unit price before discount  se cambio de subprice a price
					...
					// Total HT line
                    $total_with_tax = price($object->lines[$i]->total_ttc);
					$pdf->SetXY($this->postotalht, $curY);
