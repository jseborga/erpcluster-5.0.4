...
class Items extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_type_item)) {
			 $this->fk_type_item = trim($this->fk_type_item);
		}
		...
		if (isset($this->especification)) {
			 $this->especification = trim($this->especification);
		}
		if (isset($this->plane)) {
			 $this->plane = trim($this->plane);
		}
		if (isset($this->quant)) {
			 $this->quant = trim($this->quant);
		}
		if (isset($this->manual_performance)) {
			 $this->manual_performance = trim($this->manual_performance);
		}
		...
		$sql .= ' '.(! isset($this->manual_performance)?'NULL':$this->manual_performance).',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("items", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->manual_performance = $obj->manual_performance;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->fk_type_item = $obj->fk_type_item;
				$line->fk_parent = $obj->fk_parent;
				$line->type = $obj->type;
				$line->detail = $obj->detail;
				$line->fk_unit = $obj->fk_unit;
				$line->especification = $obj->especification;
				$line->plane = $obj->plane;
				$line->quant = $obj->quant;
				$line->manual_performance = $obj->manual_performance;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' manual_performance = '.(isset($this->manual_performance)?$this->manual_performance:"null").',';
 ...
 * Class ItemsLine
 */
class ItemsLine extends CommonObjectLine
{
	/**
	 * @var int ID
	 */
