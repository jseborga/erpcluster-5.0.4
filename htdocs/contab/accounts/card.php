...
$object = new AccountingAccountext($db);
$objectadd = new AccountingAccountadd($db);
...
if (GETPOST('cancel'))
{
	$urltogo=$backtopage?$backtopage:dol_buildpath('/contab/accounts/account.php',1);
	header("Location: ".$urltogo);
	exit;
}

if ($action == 'add' && $user->rights->contab->account->write)
{
	if (! $cancel) {
		$sql = 'SELECT pcg_version FROM ' . MAIN_DB_PREFIX . 'accounting_system WHERE rowid=' . $conf->global->CHARTOFACCOUNTS;
		
		dol_syslog('contab/accounts/card.php:: $sql=' . $sql);
		$result = $db->query($sql);
		...
		if (!$error)
		{
			$objectadd->fk_accounting_account = $res;
			$objectadd->cta_class = GETPOST('cta_class');
			$objectadd->cta_normal = GETPOST('cta_normal');
...
} else if ($action == 'edit' && $user->rights->contab->account->write) {
	if (! $cancel) {
		...
		if ($result>0)
		{
			...
			{
				$objectadd->fk_accounting_account = $id;
				$objectadd->cta_class = GETPOST('cta_class');
				$objectadd->cta_normal = GETPOST('cta_normal');
				$objectadd->fk_user_create = $user->id;
				$objectadd->fk_user_mod = $user->id;
...
} else if ($action == 'delete' && $user->rights->contab->account->del) {
	$result = $object->fetch($id);
...
$aCtaclass = array('A'=>$langs->trans('Analitica'),'D'=>$langs->trans('Descriptiva'));
$aCtanormal = array('D'=>$langs->trans('Deudor'),'A'=>$langs->trans('Acreedor'));
...
if ($action == 'create') {
	...
	print '</td></tr>';
	// cta_class
	print '<tr><td>' . $langs->trans("Ctaclass") . '</td>';
	print '<td>';
	print $form->selectarray('cta_class',$aCtaclass,GETPOST('cta_class'),1);
	//print $htmlacc->select_pcgsubtype($object->pcg_subtype, 'pcg_subtype', 1);
	print '</td></tr>';
	// cta_class
	print '<tr><td>' . $langs->trans("Ctanormal") . '</td>';
	print '<td>';
	print $form->selectarray('cta_normal',$aCtanormal,GETPOST('cta_normal'),1);
	//print $htmlacc->select_pcgsubtype($object->pcg_subtype, 'pcg_subtype', 1);
	print '</td></tr>';
	...
	$result = $object->fetch($id);
	$resultadd = $objectadd->fetch(0,$id);
	...
	// cta_class
			print '<tr><td>' . $langs->trans("Ctaclass") . '</td>';
			print '<td>';
			print $form->selectarray('cta_class',$aCtaclass,(GETPOST('cta_class')?GETPOST('cta_class'):$objectadd->cta_class),1);
	//print $htmlacc->select_pcgsubtype($object->pcg_subtype, 'pcg_subtype', 1);
			print '</td></tr>';
	// cta_class
			print '<tr><td>' . $langs->trans("Ctanormal") . '</td>';
			print '<td>';
			print $form->selectarray('cta_normal',$aCtanormal,(GETPOST('cta_normal')?GETPOST('cta_normal'):$objectadd->cta_normal),1);
	//print $htmlacc->select_pcgsubtype($object->pcg_subtype, 'pcg_subtype', 1);
			print '</td></tr>';
			...
			// ctaclass
			print '<tr><td>' . $langs->trans("Ctaclass") . '</td>';
			print '<td colspan="2">' . $aCtaclass[$objectadd->cta_class] . '</td></tr>';
			// ctanormal
			print '<tr><td>' . $langs->trans("Ctanormal") . '</td>';
			print '<td colspan="2">' . $aCtanormal[$objectadd->cta_normal] . '</td></tr>';
