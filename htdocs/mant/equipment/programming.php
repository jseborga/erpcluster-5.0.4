...
require_once(DOL_DOCUMENT_ROOT."/mant/equipment/class/mmaintenanceprogramming.class.php");
...
if (! $sortfield) $sortfield="p.code_mant";
...
$offset1 = $limit1 * $page;
$sql  = "SELECT p.rowid AS id, p.code_mant, p.date_ini, p.frequency, p.statut ";
$sql.= " FROM ".MAIN_DB_PREFIX."m_maintenance_programming as p ";

if ($sref)
{
    $sql.= " AND p.code_mant like '%".$sref."%'";
}
if ($sall)
{
    $sql.= " AND (p.code_mant like '%".$sall."%' OR p.frequency like '%".$sall."%')";
}
$sql.= " WHERE fk_equipment =".$id;
...
if ($result)
  {
    ...
    llxHeader("",$langs->trans("Liste programming"),$help_url);
    ...
    print_barre_liste($langs->trans("Liste programming"), $page, "liste.php", "", $sortfield, $sortorder,'',$num);
      ...
      while ($i < min($num,$limit))
	{
	  ...
	  $var=!$var;
	  print "<tr $bc[$var]>";
	  print '<td><a href="liste.php?id='.$obj->id.'&action=sub">'.img_picto($langs->trans("Ref"),'rightarrow').' '.$obj->code_mant.'</a></td>';
	  print '<td>'.$obj->date_ini.'</td>';
	  print '<td>'.$obj->frequency.'</td>';
	  print '<td align="right">'.$object->LibStatut($obj->statut,3).'</td>';
	  print "</tr>\n";
