...
class pdf_tracing extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->assets->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->assets->dir_output."/assets/".$period_year.'/tracing/'.$headerss['ref'];
				$file = $dir . "/tracing".$headerss['ref'].".pdf";
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					$pdf->MultiCell($this->posxusua-$this->posxini-1, 1, $line['nombre'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxusua, $curY);
					$pdf->MultiCell($this->posxbeen-$this->posxusua-1, 1, $line['label'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxbeen, $curY);
					$pdf->MultiCell($this->posxdesc-$this->posxbeen-1, 1,$line['descripcion'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();

					$pdf->SetXY($this->posxdesc, $curY);
					$pdf->MultiCell($this->posxfech-$this->posxdesc-1, 1, $line['fecha'], 0, 'C',0);	
										
					$pdf->SetXY($this->posxfech, $curY);
					$pdf->MultiCell($this->posxdepa-$this->posxfech-1, 1,$line['estado'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(145, 3, html_entity_decode($langs->trans('Code'))." : " . $headerss['ref'], '', 'R');
		
		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(145, 3, html_entity_decode($langs->trans('Group'))." : " . $headerss['grupo'], '', 'R');

		//Departamentos
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(145, 3,html_entity_decode( $langs->trans('Item'))." : " . $headerss['intem'] , '', 'R');

		//Departamentos
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(145, 3, html_entity_decode($langs->trans('Qualifiter'))." : " . $headerss['calificacion'] , '', 'R');

		//Departamentos
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(145, 3, html_entity_decode($langs->trans('Description'))." : " . $headerss['descripcion'], '', 'R');
		$aPosY = $pdf->GetY();
		//Departamentos
		$posy = $aPosY+1;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(145, 3, html_entity_decode($langs->trans('Status'))." : " . $headerss['estado'] , '', 'R');
	...
	function pdf_pie_total(&$pdf, $object, $posy, $outputlangs,$marge_gauche, $page_largeur,$marge_droite,$page_hauteur,$type_page=0,$bottomlasttab)
	{
		...
		$pdf->SetFont('times', '',10);
			...
			$pdf->MultiCell($this->posxresp-$this->posxdepa-1, 1, $langs->trans("Total")." : ", 1, 'R',0);
