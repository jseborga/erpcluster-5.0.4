...
/**
 * \file    budget/budgetconcept.class.php
 * \ingroup budget
 ...
 * @see CommonObject
 */
class Budgetconcept extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'budgetconcept';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'budget_concept';

	/**
	 * @var BudgetconceptLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->amount_def)) {
			 $this->amount_def = trim($this->amount_def);
		}
		...
		$sql .= ' '.(! isset($this->amount_def)?'NULL':"'".$this->amount_def."'").',';
	 ...
	 */
	public function fetch($id, $ref = null, $fk_budget=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("budgetconcept", 1) . ")";
		}
		if (null !== $ref && $fk_budget>0) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->type = $obj->type;
				$this->amount_def = $obj->amount_def;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->type = $obj->type;
				$line->amount_def = $obj->amount_def;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' amount_def = '.(isset($this->amount_def)?$this->amount_def:"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Budgetconcept($this->db);
