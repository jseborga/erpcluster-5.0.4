...
if ($action == 'add' && $user->rights->mant->jobs->crear)
  {
    ...
    //    $object->fk_charge      = GETPOST("fk_charge")+0;
    //    $object->fk_departament = GETPOST("fk_departament")+0;
    $object->fk_work_request= GETPOST("idw");
...
if ($action == 'updaterech' && $_POST["cancel"] <> $langs->trans("Cancel") && $user->rights->mant->jobs->rechasig)
  {
	...
	if (!$error)
      ...
      {
	$mesg='<div class="error">'.$object->error.'</div>';
	$action="rechasignjobs";   // Force retour sur page rejectworkorder
      }
  }
...
if ($action == 'upwork' && $user->rights->mant->jobs->regjobs)
  {
	...
	$adate = dol_getdate(dol_now());
	  $present_date  = dol_mktime(0, 0, 0, $adate['mon'], $adate['mday'],  $adate['year']);
    ...
    $datefin = dol_mktime(0, 0, 0, GETPOST('df_month'),GETPOST('df_day'),GETPOST('df_year'));    
...
if ($action == 'closework' && $user->rights->mant->jobs->regjobs)
  {
    ...
    $object->fetch(GETPOST('id'));
    $statut = $object->statut;
    $object->statut      = 4;  //trabajo concluido
...
if ($action == 'addorder' && $user->rights->mant->jobs->regjobs)
  {
    ...
    if (!$error)
      {
	$db->commit();
	header("Location: fiche.php?id=".$id.'&action=editregjobs&order_number='.GETPOST('order_number').'&date_order='.$date_order);
	exit;
      }
    else 
      {
...
if ($action == 'confirm_delete_user')
  {
	...
	if ($result > 0)
	  {
	    header("Location: ".$_SERVER['PHP_SELF'].'/mant/jobs/fiche.php?id='.$id.'&amp;action=assignjobsdet');
	    exit;
	  }
	else
	  {
  ...
  if ($result)
    {
      ...
      if ($result > 0)
	{
	  header("Location: ".DOL_URL_ROOT.'/mant/jobs/fiche.php?id='.$id.'&action=editregjobs');
	  exit;
	}
      else
	{
  ...
  if (!empty($objwork->detail_problem))
    {
      ...
      print '<input type="text" name="detail_problem" value="'.$object->detail_problem.'">';
  print '</td></tr>';
...
else
{
  if ($id||$ref)
    {
      dol_htmloutput_mesg($mesg);      
      $result = $object->fetch($id,$ref,$conf->entity);
      $lImage = true;
      ...
      if ($result < 0)
	{
      ...
      if (!$user->admin && $object->statut > 0 && $action == 'edit')
	$action = '';
      ...
      if ($action <> 'edit' && $action <> 're-edit')
	{
	  ...
	  if ($action == '')
	    {
	      if ($user->admin || ($user->rights->mant->jobs->crear && $object->statut == 0))
		print "<a class=\"butAction\" href=\"fiche.php?action=edit&id=".$object->id."\">".$langs->trans("Modify")."</a>";
	      else
		print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Modify")."</a>";
	      
	      if ($user->admin || ($object->statut==0 && $user->rights->mant->jobs->del))
		...
		if ($user->societe_id > 0)
	      ...
	      if ($object->statut == 1 && $user->rights->mant->jobs->rechasig ||
		  ($object->statut == 2 && $user->rights->mant->jobs->rechasig && 
		   $object->fk_soc != -1)
 // ||
 // 		  ($object->statut == 3 && $user->rights->mant->jobs->assignjobs && 
 // 		   $user->societe_id > 0 && empty($object->date_ini))
		  )
		{
	      ...
	      // rechazar
	      // if ($object->statut == 1 && $user->rights->mant->jobs->rech)
	      // 	{
	      // 	  print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=refuse">'.$langs->trans('Refuse').'</a>';
	      // 	}
	      //echo '<hr>ver |'.$object->statut.'| |'.$user->rights->mant->jobs->assignjobs.'| '.$object->fk_soc.'|';
	      // assign jobs
	      if ($object->statut == 1 && $user->rights->mant->jobs->assignjobs ||
		  ($object->statut == 2 && $user->rights->mant->jobs->assignjobs && 
		   $object->fk_soc != -1) ||
		  ($object->statut == 3 && $user->rights->mant->jobs->assignjobs && 
		   $user->societe_id > 0 && empty($object->date_ini)))
		{
	      ...
	      // init work
	      //echo '<hr>antes'.$object->statut.' '.$user->rights->mant->jobs->upjobs.' '.$object->fk_soc.' ';
	      //echo '<hr>'.$object->statut.' == 2 && '.$user->rights->mant->jobs->upjobs.' && '.$object->fk_soc.' != -1 && '.$user->societe_id.' <= 0)';
	      if ($object->statut == 2 && $user->rights->mant->jobs->upjobs && $object->fk_soc != -1 && (empty($user->societe_id) || $user->societe_id <= 0))
		{
		  print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=editjobs">'.$langs->trans('Programmedworkorder').'</a>';
		}
	      //impres ot
	      if ($object->statut == 4)
		{
	      ...
	      // open jobs
	      if (($object->statut == 4 || $object->statut == 8) && $user->rights->mant->jobs->openwork)
		{
	  ...
	  // //refuse
	  // if ($object->statut == 1 && $action == 'refuse')
	  //   {	      
	  ...
	  if ($action == 'rechasignjobs' && 
	      ($object->statut == 1 && $user->rights->mant->jobs->rechasig ||
	       ($object->statut == 2 && $user->rights->mant->jobs->rechasig && 
		$object->fk_soc != -1))
	      )
	    {
	  ...
	  if (($object->statut == 2 && $action == 'asignjobs' && $user->societe_id <= 0) ||
	      ($object->statut == 1 && $action == 'asignjobs') ||
	      ($object->statut == 2 && $action == 'asignjobsdet'))
	    {
		...
		$action = 'asignjobsdet';
	      //echo $action.' '.$objTypent->id.' '.$objSoc->typent_id.' '.$objTypent->code.' '.$object->statut;
	      ...
	      if (($objTypent->id == $objSoc->typent_id && $objTypent->code == 'TE_BCB' && $action == 'asignjobsdet')) //asignjobsdet
		{
		  print '<table class="border" width="100%">';
		  print '<tr class="liste_titre">';
		...
		print '<center><br><input type="submit" class="button" value="'.$langs->trans("Saveuser").'"></center>';
		  ...
		  ($objTypent->id == $objSoc->typent_id && 
		   $objTypent->code == 'TE_BCB' && $object->statut > 1))
		{
	  ...
	  if ($object->statut == 2 && $action == 'editjobs' ||
	      ($object->statut == 2 && $action == 'asignjobs' && $user->societe_id > 0) ||
	      ($object->statut == 3 && $action == 'asignjobs' && $user->societe_id > 0) && empty($object->date_ini) )
	    {
	      if ($user->societe_id > 0)
		{
		  $objSoc = new Societe($db);
	  ...
	  if ($object->statut == 3 || $object->statut == 4 || $object->statut == 5)
	    {
	      ...
	      if ($object->statut == 4 || $object->statut == 5)
		include_once DOL_DOCUMENT_ROOT.'/mant/jobs/tpl/material.tpl.php';
	    }

	  if ($object->statut == 3 && $action == 'editregjobs')
	    {
	      //orders
	      $objJobsOrder = new Mjobsorder($db);
	      //$objcomm = new Commande($db);
	  ...
	  if ($action == 'editjobs' && $object->statut == 3)
	    {
	      if ($action == 'editjobs')
		{
		  if ($numberContact > 0 && !empty($object->date_ini_prog))
		    {
		      if ($user->rights->mant->jobs->upjobs && $object->statut == 2)
			{
			  print '<form action="fiche.php" method="POST">';
			  print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
			  print '<input type="hidden" name="action" value="startjobs">';
	  ...
	  if ($action == 'editregjobs')
		...
		{
		  if ($user->rights->mant->jobs->regjobs && $object->statut == 3)
		    {
		      print '<form action="fiche.php" method="POST">';
		      print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
	  ...
	  if ($action == 'asignjobsdet')
		...
		{
		  if ($user->rights->mant->jobs->assignjobs && $object->statut == 2)
		    {
		      print '<form action="fiche.php" method="POST">';
		      print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
