 ...
 */
class pdf_fractalgroupassets extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
  ...
  // Minimum version of PHP required by module
   ...
   //*
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "groupassets";
		$this->description = $langs->trans('PDFGroupassets');
...
function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
{
	...
	if ($conf->assets->dir_output)
	{
		...
		{
			$objectref = 'groupasset';
			if (!empty($object->ref))
				$objectref .= dol_sanitizeFileName($object->ref);
			$dir = $conf->assets->dir_output.'/group_';
			//if ($object->group)
			//	$dir = $conf->assets->dir_output.'/'.$object->group;
			$file = $dir . "/" . $cGroup . ".pdf";
		}
			...
			if($this->type_page==1)
				...
				foreach ((array) $aReportdetasset AS $i => $lines)
				{
					...
					if($type=='D')
					{
						...
						$Etiqueta = $lines['Etiqueta'];
						$pdf->SetXY($this->posxlabel, $curY);
						$pdf->MultiCell($this->posxda-$this->posxlabel-1, 5, dol_trunc($Etiqueta,44), 0, 'L',0);
						...
						//$Etiqueta = $lines['Etiqueta'];
						//$pdf->SetXY($this->posxlabel, $curY);
						//$pdf->MultiCell($this->posxda-$this->posxlabel-1, 5, dol_trunc($Etiqueta,50), 0, 'L',0);
						//$nPosy = round($pdf->GetY());
						...
						$pdf->MultiCell($this->posxcond-$this->posxresp-1, 5, dol_trunc($Responsable,20), 0, 'L',0,0,'','',false);
						...
						//$Etiqueta = $lines['Etiqueta'];
						//$pdf->SetXY($this->posxlabel, $curY);
						//$pdf->MultiCell($this->posxda-$this->posxlabel-1, 5, dol_trunc($Etiqueta,46), 0, 'L',0);
						//$nPosy = round($pdf->GetY());
						...
						$costo = $lines['costo'];
						$pdf->SetXY($this->posxcos, $curY);
						$pdf->MultiCell($this->posxinm-$this->posxcos-1, 5, price(price2num($costo,'MT')), 0, 'R',0);
						//departament
						$departament = $lines['departament'];
						$pdf->SetXY($this->posxinm, $curY);
						$pdf->MultiCell($this->posxfasig-$this->posxinm-1, 5, dol_trunc($departament,20), 0, 'L',0);
						//$nPosy = round($pdf->GetY());
						//Responsable
						$resp = $lines['resp'];
						$pdf->SetXY($this->posxfasig, $curY);
						$pdf->MultiCell($this->posxvalact-$this->posxfasig-1, 5, dol_trunc($resp,16), 0, 'L',0);

						// valor actualizada
						$Valoract = $lines['Valoract'];
						$pdf->SetXY($this->posxvalact, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxcond-$this->posxvalact-1, 5,price(price2num($Valoract,'MT')), 0, 'R',0);

						// depreciacion acumulada
						$Depreacum = $lines['Depreacum'];
						$pdf->SetXY($this->posxcond, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxest-$this->posxcond-1, 5, price(price2num($Depreacum,'MT')), 0, 'R',0);

						// balance
						$Balance = $lines['Balance'];
						$pdf->SetXY($this->posxest, $curY);
						$pdf->MultiCell($this->page_largeur-$this->marge_haute-$this->posxest-1, 5, price(price2num($Balance,'MT')), 0, 'R',0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxcodext-$this->posxcod-1,2, $outputlangs->transnoentities("Code"),'','L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxlabel-$this->posxcodext-1,2, $outputlangs->transnoentities("Code.Ext"),'','L');
		}


		////////////////////////////////////////////////////////////////////////////////////////////
		// CAMBIO DE POSICIONES		 METDO 1											///
		if($confint)
		{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->page_largeur-$this->marge_haute-$this->posxest-1,2, $outputlangs->transnoentities("Balance"),'','C');
		}
		...
		{
			$pdf->SetXY($this->posxinm-1, $tab_top+1);
			$pdf->MultiCell($this->posxfasig-$this->posxinm-1,2, $outputlangs->transnoentities("Departament"),'','C');
		}
		...
		*/
		...
		{
			$pdf->SetXY($this->posxfasig-1, $tab_top+1);
			$pdf->MultiCell($this->posxvalact-$this->posxfasig-1,2, $outputlangs->transnoentities("Responsible"),'','C');
		}
		$pdf->line($this->posxfasig-1, $tab_top, $this->posxfasig-1, $tab_top + $tab_height);
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxcond-$this->posxvalact-1,2, $outputlangs->transnoentities("ValorAct"),'','C');
		}
		...
		{
			$pdf->SetXY($this->posxcond-1, $tab_top+1);
			$pdf->MultiCell($this->posxest-$this->posxcond-1,2, $outputlangs->transnoentities("DepreciationAcum"),'','C');
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Reportassetsgroups");
		...
		*/
		...
		if($fk_group==0)
		{
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Group")." : " . "Todos", '', 'R');
		}
		else
		{
			$posy+=4;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Group")." : " .$cGroup, '', 'R');
