...
/**
 *	\file       htdocs/contab/class/contab.class.php
 *	\ingroup    contab
 *	\brief      File of class to manage exports contab
 */


/**
 *	Class to manage contab exports
 */
class Contab extends CommonObject
{
	var $db;
	 ...
	 *      @return		int								<0 if KO, >0 if OK
	 */
	function build_file($user, $model, $datatoexport, $array_selected, $array_Value, $array_export_type,$array_export_fields,$sqlquery = '')
 	{
	  global $conf,$langs;
	  ...
	  if (empty($array_export_fields) || ! is_array($array_export_fields))
	    {
	  ...
	  if (count($array_Value) > 0)
	    {
	      ...
	      $dirname=$conf->contab->dir_temp.'/'.$user->id;
	      ...
	      if ($result >= 0)
		{
		  ...
		  $objmodel->write_title($array_export_fields,$array_selected,$outputlangs,$array_export_type);

		  $var=true;
		  foreach ((array) $array_Value AS $i => $data)
		    {
		      $var=!$var;
		      // end of special operation processing
		      $objmodel->write_record($array_selected,$data,$outputlangs,$array_selected_type);
		    }
