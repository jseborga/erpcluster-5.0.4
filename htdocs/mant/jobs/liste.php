...
$sql = "SELECT p.rowid, p.fk_work_request, p.ref, p.fk_member, p.detail_problem, p.email, p.date_create, p.fk_soc, p.speciality, p.statut, ";
$sql.= " s.fk_typent, ";
...
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."m_work_request AS w ON p.fk_work_request = w.rowid ";
        ...
        while ($i < min($num,$limit))
        {
            ...
            if ($objp->fk_soc != -2)
            {
                ...
                if ($objSoc->fetch($objp->fk_soc)>0 && $objp->fk_soc > 0)
                {
                    ...
                    if ($object->fk_soc == -2 || ($objTypent->id == $objSoc->typent_id && $objTypent->code == 'TE_BCB' )) //asignacion interna
                        $lUser = true;
                    $htmlsoc = $objSoc->name;
                }
                if (!$lUser)
	  ...
	  if ($lPrint)
		{
		  ...
		  print '<td nowrap><a href="fiche.php?id='.$objp->rowid.'">'.dol_print_date($objp->date_create,'day').'</a></td>';
