...
// Security check
$fieldvalue = (! empty($idr) ? $idr : (! empty($ref) ? $ref : ''));
...
$title = $langs->trans('Assetsdocument');
$helpurl = '';
$shortlabel = dol_trunc($object->ref,16);
$title = $langs->trans('Assets')." ". $shortlabel ." - ".$langs->trans('Documents');
$helpurl='EN:Module_Assets|FR:Module_Assets|ES:M&oacute;dulo_Activos';
...
if ($object->id)
{
	...
	$titre=$langs->trans("CardAssets".$object->type);
	$picto='assets';
	...
	{
		if ($file['name']) $file['name'] = $idr.'/'.$file['name'];
		$totalsize+=$file['size'];
	}

	$linkback = '<a href="'.DOL_URL_ROOT.'/assets/assets/list.php">'.$langs->trans("BackToList").'</a>';
	...
	$permission = $user->rights->assets->doc->up;
	$param = '&id=' . $object->id.'&idr='.$idr;
	$relativepathwithnofile=$object->ref.'/'.$idr.'/';
	$linklist = 0;
	include_once DOL_DOCUMENT_ROOT . '/core/tpl2/document_actions_post_headers.tpl.php';
