...
// Add
if ($action == 'add' && $user->rights->salary->proces->creer)
  {
    $ref   = GETPOST('ref');
    $label = GETPOST('label');
	...
	If ($result > 0)
	  {
		...
		if ($id > 0)
	      ...
	      {
		$mesg='<div class="error">'.$langs->trans("Errorrefexist").'</div>';
...
// Delete proces
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->salary->proces->del)
  {
    $object->fetch($_REQUEST["id"]);
    $result=$object->delete($user);
    if ($result > 0)
      {
	header("Location: ".DOL_URL_ROOT.'/salary/proces/liste.php');
	exit;
      }
    else
      {
...
if ($action == 'create' && $user->rights->salary->proces->creer)
  {
    print_fiche_titre($langs->trans("Newproces"));
	 ...
	 if ($action <> 'edit' && $action <> 're-edit')
	   {
	     ...
	     dol_fiche_head($head, 'proces', $langs->trans("Proces"), 0, 'proces');
	     ...
	     if ($action == 'delete')
	       {
		 $form = new Form($db);
		 $ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Deleteproces"),$langs->trans("Confirmdeleteproces",$object->period_month.' '.$object->period_year),"confirm_delete",'',0,2);
		 ...
		 if ($user->rights->salary->proces->del  && $object->state==0)
