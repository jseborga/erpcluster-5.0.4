...
/**
 *	\file       htdocs/commande/liste_pedido.php
 *	\ingroup    commande
...
if (!$user->rights->fabrication->leerpedidov)
	accessforbidden();
...
$viewstatut = GETPOST('search_statut','int');
if (isset($_POST['date_pyear']) && $_POST['date_pyear'] >0)
	$search_datep = dol_mktime(0, 0, 0, GETPOST('date_pmonth'),GETPOST('date_pday'),GETPOST('date_pyear'));
if (isset($_POST['date_eyear']) && $_POST['date_eyear'] >0)
	$search_datee = dol_mktime(0, 0, 0, GETPOST('date_emonth'),GETPOST('date_eday'),GETPOST('date_eyear'));
// Security check
$id = (GETPOST('orderid')?GETPOST('orderid'):GETPOST('id','int'));
if ($user->societe_id) $socid=$user->societe_id;
//$result = restrictedArea($user, 'fabrication', $id,'');
...
$aStatut = array(9=>'',-1=>$langs->trans('Annulled'),0=>$langs->trans('Draft'),-2=>$langs->trans('Pending'),5=>$langs->trans('En Proceso'),6=>$langs->trans('Entregado'),3=>$langs->trans('Entregado a Cliente'));
...
if (isset($_POST['nosearch']) || isset($_POST['nosearch_x']) || isset($_GET['nosearch_x']))
{
	$search_categ='';
	$search_user='';
	$search_sale='';
...
$sql.= ' c.date_valid, c.date_commande, c.date_livraison, c.fk_statut, c.facture as facturee, f.ref AS refop, f.rowid AS frowid, f.statut AS statut';
$sql.= ' FROM '.MAIN_DB_PREFIX.'societe as s';
$sql.= ' INNER JOIN '.MAIN_DB_PREFIX.'commande as c ON c.fk_soc = s.rowid';
$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'fabrication as f ON c.rowid = f.fk_commande';
...
if ($viewstatut <> '')
{
	if ($viewstatut == -1)
	{
		$sql.= ' AND c.fk_statut = -1'; // brouillon, validee, en cours, annulee
	}
	if ($viewstatut == 0)
	{
		$sql.= ' AND c.fk_statut = 0'; // brouillon, validee, en cours, annulee
	}
	if ($viewstatut == 5)
	{
		$sql.= ' AND c.fk_statut = 1'; // brouillon, validee, en cours, annulee
		$sql.= ' AND f.statut = 1'; // need to create invoice
	}
	if ($viewstatut == 6)
	{
		$sql.= ' AND c.fk_statut =1'; // brouillon, validee, en cours, annulee
		$sql.= ' AND f.statut = 2'; // need to create invoice
	}
	if ($viewstatut == -2)
	{
		$sql.= ' AND c.fk_statut = 1'; // brouillon, validee, en cours, annulee
		$sql.= " AND f.statut is null";
	}
	if ($viewstatut == 3)
	{
		$sql.= ' AND c.fk_statut = 3'; // brouillon, validee, en cours, annulee
	}
...
if ($search_sale > 0) $sql.= " AND s.rowid = sc.fk_soc ";
...
if ($search_datep)
{
	$sql.= " AND c.date_commande = '".$db->idate($search_datep)."'";
}
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans('OrderProduction'),$_SERVER["PHP_SELF"],'c.date_livraison','',$param, 'align="right"',$sortfield,$sortorder);
	...
	$form->select_date(empty($search_datep)?'':$search_datep,'date_p','','',1,"crea_commande",1,0);
	print '</td><td class="liste_titre" align="right">';
	$form->select_date(empty($search_datee)?'':$search_datee,'date_e','','',1,"crea_commande",1,0);
	print '</td><td class="liste_titre" align="center">';
	print '<input class="flat" type="text" size="8" name="search_op" value="'.$search_op.'">';
	print '</td><td align="right" class="liste_titre">';
	print $form->selectarray('search_statut',$aStatut,$viewstatut);
	...
	$generic_commande = new Commandeext($db);
	$generic_fabrication = new Fabricationext($db);
	...
	while ($i < min($num,$limit))
	{
		...
		print $generic_commande->getNomUrladd(1,($viewstatut != 2?0:$objp->fk_statut));
		print '</td>';
		...
		if (!empty($objp->frowid))
			print ' <a href="'.DOL_URL_ROOT.'/fabrication/fiche.php?id='.$objp->frowid.'">'.$objp->refop.'</a>';
		else
		{
			if ($objp->fk_statut > 0 && $objp->fk_statut < 3)
				print ' <a style="color:#ff0000;" href="'.DOL_URL_ROOT.'/fabrication/fiche.php?action=create&commandeid='.$objp->rowid.'">'.$langs->trans('Create').'</a>';
		}
		print '</td>';
		// Statut
		if ($viewstatut == -2)
		...
		print '<td align="right" nowrap="nowrap">'.$generic_fabrication->LibStatut($objp->fk_statut,5).'</td>';
