...
if (!$user->rights->salary->member->lire)
	accessforbidden();
...
$canaddmember=$user->rights->salary->member->creer;
// Define variables to determine what the current user can do on the properties of a member
if ($rowid)
{
	$caneditfieldmember=$user->rights->salary->member->creer;
}
...
if ($action == 'update' && ! $_POST["cancel"] && $user->rights->salary->member->creer)
{
	require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
	$error=0;
	$birthdate='';
	if (isset($_POST["birthday"]) && $_POST["birthday"] && isset($_POST["birthmonth"]) && $_POST["birthmonth"] && isset($_POST["birthyear"]) && $_POST["birthyear"])
	{
	...
	if (! $error)
	{
		...
		//$res = $object->updateadd($user,1);
		//if ($res<0)
		//{
		//	setEventMessages($langs->trans('No se puede actualizar adherent'),null,'errors');
		//	setEventMessages($object->error,$object->errors,'errors');
		//	$error=101;
		//}
		$objectpu->firstname       = trim($_POST["firstname"]);
		$objectpu->lastname        = trim($_POST["lastname"]);
		$objectpu->lastnametwo     = trim($_POST["lastnametwo"]);
		$objectpu->state_marital   = $_POST["state_marital"];

		$objectpu->docum           = trim($_POST["docum"]);
		$objectpu->registration    = trim(GETPOST("registration"));
		$objectpu->issued_in       = trim($_POST["issued_in"]);
		$objectpu->sex             = $_POST["sex"]+0;
		$objectpu->phone_emergency = trim($_POST["phone_emergency"]);
		$objectpu->blood_type      = $_POST["blood_type"];
		$objectpu->dependents      = $_POST["dependents"]+0;
		$objectpu->fk_user_mod  = $user->id;
		$objectpu->datem = $now;
		if (empty($objectpu->fk_user_create))
		{
			$objectpu->fk_user_create = $user->id;
			$objectpu->datec = $now;
			$objectpu->status = 1;
		}

		if ($objectpu->fk_user == $rowid)
		{
			$res = $objectpu->update($user);
			if ($res <=0)
			{
				$error=201;
				setEventMessages($objectpu->error,$objectpu->errors,'errors');
			}
		}
		else
		{
			$objectpu->days_assigned = 0;
			$objectpu->fk_user_create  = $user->id;
			$objectpu->fk_user_mod  = $user->id;
			$objectpu->datec = $now;
			$objectpu->datem = $now;
			$objectpu->status  = 1;
			//creando el registro
			$objectpu->fk_user = $rowid;
			$res = $objectpu->create($user);
			if ($res <=0)
			{
				$error=202;
				setEventMessages($objectpu->error,$objectpu->errors,'errors');
...
if ($action == 'add' && $user->rights->salary->member->creer)
{
	...
	//puser
	$lastnametwo=$_POST["lastnametwo"];
	$state_marital=$_POST['state_marital'];
	$docum = $_POST['docum'];
	$issued_in = $_POST['issued_in'];
	$sex = $_POST['sex'];
	...
	$objectpu->firstname       = $object->firstname;
	$objectpu->lastname        = $object->lastname;
	$objectpu->lastnametwo     = $lastnametwo;
	$objectpu->state_marital   = $state_marital;

	$objectpu->docum           = $docum;
	$objectpu->registration    = trim(GETPOST("registration"));
	$objectpu->issued_in       = $issued_in;
	$objectpu->sex             = $sex;
	$objectpu->phone_emergency = $phone_emergency;
	$objectpu->blood_type      = $blood_type;
	$objectpu->dependents      = $dependents;
	...
	// Email a peu pres correct et le login n'existe pas
		...
		//registrando en la tabla p_user informacion adicional
	  		$db->begin();
	  		$objectpu->fk_user = $rowid;

	  		$result = $objectpu->create($user);
	  		if ($result > 0)
	  		{
	  			$db->commit();
	  		}
	...
	if ($user->rights->salary->member->del && $action == 'confirm_delete' && $confirm == 'yes')
	{
		$res = $objectpu->fetch(0,$rowid);
		if ($res >0)
		{
			$result=$objectpu->delete($user);
			if ($result > 0)
			{
				if (! empty($backtopage))
				{
	...
	if ($user->rights->salary->member->creer && $action == 'confirm_valid' && $confirm == 'yes')
	{
		$error=0;
	...
	if ($user->rights->salary->member->del && $action == 'confirm_resign')
	{
		if ($confirm == 'yes')
		{
			$adht = new AdherentType($db);
...
// SPIP Management
	if ($user->rights->salary->member->del && $action == 'confirm_del_spip' && $confirm == 'yes')
	{
	...
	if ($user->rights->salary->member->creer && $action == 'confirm_add_spip' && $confirm == 'yes')
	{
		if (! count($object->errors))
		{
...
else
{
	...
	if ($action == 'create')
	{
	 ...
	 // lastanetwo
		print '<tr><td id="tdlastnametwo">'.$langs->trans("Lastnametwo").'</td><td><input type="text" name="lastnametwo" size="40" value="'.(GETPOST('lastnametwo','alpha')?GETPOST('lastnametwo','alpha'):$object->lastnametwo).'"></td>';
		print '</tr>';

	 // Firstname
		print '<tr><td class="fieldrequired" id="tdfirstname">'.$langs->trans("Firstname").'</td><td><input type="text" name="firstname" size="40" value="'.(GETPOST('firstname','alpha')?GETPOST('firstname','alpha'):$object->firstname).'"></td>';
		print '</tr>';
	 ...
	 // Docum
		print '<tr><td id="tddocum">'.$langs->trans("Docum").'</td><td><input type="text" name="docum" maxlenght="15" size="13" value="'.(GETPOST('docum','alpha')?GETPOST('docum','alpha'):$objectpu->docum).'"></td>';
		print '</tr>';
		// matricula
		print '<tr><td id="tddocum">'.$langs->trans("Matricula").'</td><td><input type="text" name="registration" maxlenght="30" size="13" value="'.(GETPOST('registration','alpha')?GETPOST('registration','alpha'):$objectpu->registration).'"></td>';
		print '</tr>';

	 // issued_in
		print '<tr><td id="tddocum">'.$langs->trans("Issuedin").'</td><td>';
		//print $formcompany->select_departement($object->issued_in,isset($_POST["country_id"])?$_POST["country_id"]:$object->country,'issued_in');
		print $formcompany->select_region($selected='',$htmlname='issued_in');
	 ...
	 // Tel emergency
		print '<tr><td>'.$langs->trans("Phoneemergency").'</td><td><input type="text" name="phone_emergency" size="20" value="'.(GETPOST('phone_emergency','alpha')?GETPOST('phone_emergency','alpha'):$objectpu->phone_emergency).'"></td></tr>';
	 ...
	 //state civility
		print '<tr><td>'.$langs->trans('Statemarital').'</td><td colspan="2">';
		form_p_civility($object->state_marital, 'state_marital', 1);
		print '</td></tr>';

	 //blood
		print '<tr><td>'.$langs->trans('Bloodtype').'</td><td colspan="2">';
		form_p_blood($objectpu->blood_type, 'blood_type', 1);
		print '</td></tr>';

	 //dependents
		print '<tr><td>'.$langs->trans('Dependents').'</td><td colspan="2">';
		print '<input type="text" id="dependents" name="dependents" value="'.$objectpu->dependents.'"  maxlength="1" size="1">';
		print '</td></tr>';
	...
	if ($action == 'edit')
	{
		...
		********************************************/
		...
		dol_fiche_head($head, 'tabname1', $langs->trans("Member"), 0, 'user');
	 ...
	 // Lastname
		print '<tr><td id="tdlastname">'.$langs->trans("Lastname").'</td><td><input type="text" name="lastname" value="'.(GETPOST('lastname','alpha')?GETPOST('lastname','alpha'):$objectpu->lastname).'" size="40"></td>';
		print '</tr>';

	 // lastanetwo
		print '<tr><td id="tdlastnametwo">'.$langs->trans("Lastnametwo").'</td><td><input type="text" name="lastnametwo" size="40" value="'.(GETPOST('lastnametwo','alpha')?GETPOST('lastnametwo','alpha'):$objectpu->lastnametwo).'"></td>';
		print '</tr>';
	 ...
	 // Firstname
		...
		// issued_in
		print '<tr><td id="tddocum">'.$langs->trans("Issuedin").'</td><td>';
			//print $formcompany->select_state($object->issued_in,isset($_POST["issued_in"])?$_POST["issued_in"]:$object->country_id,'issued_in');
		print $formcompany->select_region($objectpu->issued_in,$htmlname='issued_in');
		print '</td>';
		print '</tr>';
		...
		//state civility
		print '<tr><td>'.$langs->trans('Statemarital').'</td><td colspan="2">';
		form_p_civility($objectpu->state_marital, 'state_marital', 1);
		print '</td></tr>';

		//sex
		print '<tr><td>'.$langs->trans('Sex').'</td><td colspan="2">';
		print select_sex($objectpu->sex,'sex','',0,1,0);
		print '</td></tr>';
	...
	if ($rowid && $action != 'edit')
	{
		...
		*/
		...
		// Confirm removed from spip
			...
			$linkback = '<a href="'.DOL_URL_ROOT.'/salary/user/liste.php">'.$langs->trans("BackToList").'</a>';
		...
		// Morphy
			...
			// Lastname
			print '<tr><td>'.$langs->trans("Lastname").'</td><td class="valeur">'.$objectpu->lastname.'&nbsp;</td>';
			print '</tr>';
			// lastNametwo
			print '<tr><td>'.$langs->trans("Lastnametwo").'</td><td class="valeur">'.$objectpu->lastnametwo.'&nbsp;</td>';
			print '</tr>';
			...
			{
			// Docum
				print '<tr><td id="tddocum">'.$langs->trans("Docum").'</td><td>';
				print $objectpu->docum;
				print '</td>';
				print '</tr>';
			// Docum
				print '<tr><td id="tddocum">'.$langs->trans("Matricula").'</td><td>';
				print $objectpu->registration;
				print '</td>';
				print '</tr>';

			// issued_in
				print '<tr><td id="tddissuedin">'.$langs->trans("Issuedin").'</td><td>';
				$objRegion = fetch_regions(0,$objectpu->issued_in);

				if ($objRegion->code_region == $object->issued_in)
					print $objRegion->label;
				print '</td>';
				print '</tr>';

			//state civility
				print '<tr><td>'.$langs->trans('Statemarital').'</td><td colspan="2">';
				print $langs->trans($objectpu->state_marital);
				print '</td></tr>';

		//sex
				print '<tr><td>'.$langs->trans('Sex').'</td><td colspan="2">';
				print select_sex($objectpu->sex,'sex','',0,0,1);
				print '</td></tr>';

		//blood
				print '<tr><td>'.$langs->trans('Bloodtype').'</td><td colspan="2">';
				print $objectpu->blood_type;
				print '</td></tr>';

		//dependents
				print '<tr><td>'.$langs->trans('Dependents').'</td><td colspan="2">';
				print $objectpu->dependents;
				print '</td></tr>';
			}
		...
		*/
		...
		{
			// Modify
			if ($user->rights->salary->member->creer)
			{
			...
			if ($object->statut >= 1)
			{
				if ($user->rights->salary->member->del)
				{
