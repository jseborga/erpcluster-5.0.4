	...
	foreach((array) $aAmount AS $fk_poa_partida_com => $value)
	  {
		...
		$objdev->fk_poa_prev = $object->fk_poa_prev;
	...
	if ($objprev->fetch($object->fk_poa_prev))
	  {
		...
		//exito;
		$db->commit();
		header("Location: fiche_pas2.php?id=".$id.'&idc='.$idc.'&action=create');
		exit;
	      }
	    else
	      {
	...
	foreach((array) $objpcon->array AS $j => $objpcontrat)
	  {
		...
		$var=!$var;
		print "<tr $bc[$var]>";
		print '<td>'.'<a href="fiche_pas1.php?action=selcon&id='.$object->id.'&idc='.$objpcontrat->id.(isset($_GET['nopac'])?'&nopac=1&idp='.$_GET['idp']:'').'">'.$objcont->array_options['options_ref_contrato'].($contratAdd?', '.$contratAdd:'').'</a></td>';
		...
		if (empty($action) && $user->rights->poa->deve->crear)
		  if ($user->admin || $user->id == $objprev->fk_user_create)
		    print '<td align="center">'.'<a href="fiche_pas2.php?id='.$id.'&idc='.$objpcontrat->id.'&action=create'.(isset($_GET['nopac'])?'&nopac=1&idp='.$_GET['idp']:'').'">'.img_picto($langs->trans('Pay'),DOL_URL_ROOT.'/poa/img/payment.png','',1).'</a>'.'</td>';
		  else
		 ...
		 */
		if ($action == 'create' && $idc == $objpcontrat->id && $user->rights->poa->deve->crear)
		  {		    
		    $saldo = 0;
		    $objcompr = new Poapartidacom($db);
		    if ($objcompr->get_sum_pcp($object->fk_poa_prev,$idc))
		      {
			//total comprometido
			$totalcomp = $objcompr->total;
		      }
		    $objdeveng = new Poapartidadev($db);
		    if ($objdeveng->get_sum_pcp($object->fk_poa_prev,$idc))
		      {
			//total comprometido
		...
		if (count($objdev->array) > 0)
		  {
			...
			if ($idr == $objdeve->id && $action == 'editdev')
			...
			else
			  {
			    ...
			    print '<td align="center">';
			    if ($user->rights->poa->deve->mod || ($nLoop == $nLen && $user->id == $objprev->fk_user_create))
			      {
				print '<a href="fiche_pas2.php?id='.$id.'&idr='.$objdeve->id.'&action=editdev'.'">'.img_picto($langs->trans("Edit"),'edit').'</a>';
				print '&nbsp;';
			      }
