<?php
 ...
 class pdf_resourcejob extends ModelePDFMant
 {
 	var $db;
 	var $name;
 	var $um;
	...
	//Pocion que se mostrara el reporte 0 = Horizontal y 1 = Vertical
 	...
 	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
 	{
 		...
 		if ($conf->mant->dir_output)
 		{
			...
			// Definition of $dir and $file
 			...
 			else
 			{
 				...
 				$dir = $conf->mant->dir_output."/mant/".$period_year.'/resourcejob';
 				$file = $dir . "/resourcejob.pdf";
			...
			//Rescatamos la sesiones enviadas
 			...
 			if (file_exists($dir))
 			{
				...
				//linea para el cuerpo
 				$tab_top = 40;
				$tab_top_newpage = (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)?40:10);//38
				$tab_height = 44;
				//130
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					$pdf->MultiCell($this->posxequi-$this->posxini-1, 1, $line['ref'], 0, 'L',0);

					$pdf->SetXY($this->posxequi, $curY);
					$pdf->MultiCell($this->posxorde-$this->posxequi-1, 1, $line['refequi'], 0, 'L',0);

					$pdf->SetXY($this->posxorde, $curY);
					$pdf->MultiCell($this->posxfech-$this->posxorde-1, 1, dol_print_date($line['date_ini'],'day'), 0, 'L',0);

					$pdf->SetXY($this->posxfech, $curY);
					$pdf->MultiCell($this->posxtipo-$this->posxfech-1, 1, $line['tipo'], 0, 'L',0);

					$pdf->SetXY($this->posxtipo, $curY);
					$pdf->MultiCell($this->posxdesc-$this->posxtipo-1, 1, $line['descripcion'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();

					$pdf->SetXY($this->posxdesc, $curY);
					$pdf->MultiCell($this->posxcant-$this->posxdesc-1, 1, $line['cantidad'], 0, 'C',0);


					$pdf->SetXY($this->posxcant, $curY);
					$pdf->MultiCell($this->posxunid-$this->posxcant-1, 1, $line['unidad'], 0, 'C',0);


					$pdf->SetXY($this->posxunid, $curY);
					$pdf->MultiCell($this->posxprec-$this->posxunid-1, 1, price(price2num($line['precio'],'MT')), 0, 'R',0);


					$pdf->SetXY($this->posxprec, $curY);
					$pdf->MultiCell($this->posxfin-$this->posxprec-1, 1, html_entity_decode($line['estado']), 0, 'C',0);
				...
				// Show square
					if ($pagenb == 1)
					{
						$this->_tableau($pdf, $tab_top, 155, 0, $outputlangs, 0, 0);
						$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
			...
			if($type_page==0)
				{   /* La linea rectangular de pagina horizontal */
					...
					if (empty($hidetop)){
						...
						$pdf->MultiCell($this->posxequi-$this->posxini-1,2, $outputlangs->transnoentities("Equipment"),'','C');
						$pdf->line($this->posxequi-1, $tab_top, $this->posxequi-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxequi-1, $tab_top+2);
						$pdf->MultiCell($this->posxorde-$this->posxequi-1,2, $outputlangs->transnoentities("Workorder"),'','C');
						$pdf->line($this->posxorde-1, $tab_top, $this->posxorde-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxorde-1, $tab_top+2);
						$pdf->MultiCell($this->posxfech-$this->posxorde-1,2, $outputlangs->transnoentities("Date"),'','C');
						$pdf->line($this->posxfech-1, $tab_top, $this->posxfech-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxfech-1, $tab_top+2);
						$pdf->MultiCell($this->posxtipo-$this->posxfech-1,2, $outputlangs->transnoentities("Type"),'','C');
						$pdf->line($this->posxtipo-1, $tab_top, $this->posxtipo-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxtipo-1, $tab_top+2);
						$pdf->MultiCell($this->posxdesc-$this->posxtipo-1,2, $outputlangs->transnoentities("Description"),'','C');
						$pdf->line($this->posxdesc-1, $tab_top, $this->posxdesc-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxdesc-1, $tab_top+2);
						$pdf->MultiCell($this->posxcant-$this->posxdesc-1,2, $outputlangs->transnoentities("Quantity"),'','C');
						$pdf->line($this->posxcant-1, $tab_top, $this->posxcant-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxcant-1, $tab_top+2);
						$pdf->MultiCell($this->posxunid-$this->posxcant-1,2, $outputlangs->transnoentities("Unit"),'','C');
						$pdf->line($this->posxunid-1, $tab_top, $this->posxunid-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxunid-1, $tab_top+2);
						$pdf->MultiCell($this->posxprec-$this->posxunid-1,2, $outputlangs->transnoentities("Price"),'','C');
						$pdf->line($this->posxprec-1, $tab_top, $this->posxprec-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxprec-1, $tab_top+2);
						$pdf->MultiCell($this->posxfin-$this->posxprec-1,2, $outputlangs->transnoentities("Status"),'','C');
		...
		// Show Draft Watermark
					if($object->statut==0 && (! empty($conf->global->MANT_DRAFT_WATERMARK)) )
					{
						pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',$conf->global->MANT_DRAFT_WATERMARK);
					}
		...
		// Logo
			...
			// width=0 (auto)
					...
					$titulo = "RECURSOS UTILIZADOS PARA EL MANTENIMIENTO";
		...
		//Fecha
					...
					$pdf->SetXY($posx,$posy);
					$pdf->SetTextColor(0,0,60);
					$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Dateini")." : " . dol_print_date($aReporte[2],'day'), '', 'R');

		//Dato
					$posy+=4;
					$pdf->SetXY($posx,$posy);
					$pdf->SetTextColor(0,0,60);
					$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Datefin")." : " . dol_print_date($aReporte[5],'day'), '', 'R');
		...
		//Estado
					...
					$pdf->SetTextColor(0,0,60);
					if($aReporte[3] == ""){
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Status")." : TODOS LOS ESTADOS" , '','R');
					}else{
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Status")." : " . html_entity_decode($aReporte[3]), '','R');
					}
		...
		//Tipo de Recurso
					...
					$pdf->SetTextColor(0,0,60);
					if($aReporte[4] == ""){
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Equipment")." : TODOS LOS RECURSOS" , '','R');
					}else{
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Typeofresource")." : " . html_entity_decode($aReporte[4]), '','R');
					}
				}

				function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
				{
					return pdf_pagefoot_fractal($pdf,$outputlangs,'MANT_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,0,$hidefreetext);
				}
		...
		//$pdf->SetTextColor(0,0,0);
			...
			//$this->printRect($pdf,$this->marge_gauche,$posy, $this->page_largeur-$this->marge_gauche-$this->marge_droite, 37, $hidetop, $hidebottom);
					$pdf->SetXY($this->posxdesc, $curY);
					$pdf->MultiCell($this->posxparc-$this->posxdesc-1, 1, $langs->trans("Total")." : ", 1, 'R',0);
			...
			//$cText = num2texto($aReporte[4]);
					...
					$pdf->MultiCell($this->posxfin-$this->posxini-1, 1, $langs->trans("Son ")." : ".$cText, 0, 'R',0);

					$pdf->SetXY($this->posxini,220);
					$pdf->MultiCell($this->posxdesc-$this->posxini-1, 1, $langs->trans("Beneficiary ")." : ".$aDetalle['beneficiario'] , 0, 'L',0);

					$pdf->SetXY($this->posxdesc,220);
					$pdf->MultiCell($this->posxfin-$this->posxdesc-1, 1, $langs->trans("History ")." : ".$aDetalle['glosa'] , 0, 'L',0);

					$curY +=4;
					if (empty($hidetop)){

						$pdf->SetXY($this->posxini-1, $curY+47);
						$pdf->MultiCell($this->posxcodi-$this->posxini-1,2, $outputlangs->transnoentities("Elaborado por"),'','C');
						$pdf->line($this->posxcodi-1, $curY+34, $this->posxcodi-1, $curY+52);

						$pdf->SetXY($this->posxcodi-1, $curY+47);
						$pdf->MultiCell($this->posxdesc-$this->posxcodi-1,2, $outputlangs->transnoentities("REVISADO"),'','C');
						$pdf->line($this->posxdesc-1, $curY+10, $this->posxdesc-1, $curY+52);

						$pdf->SetXY($this->posxdesc-1, $curY+47);
						$pdf->MultiCell($this->posxparc-$this->posxdesc-1,2, $outputlangs->transnoentities("APROBADO"),'','C');
						$pdf->line($this->posxparc-1, $curY+34, $this->posxparc-1, $curY+52);

						$pdf->SetXY($this->posxparc-1,$curY+47);
						$pdf->MultiCell($this->posxdebe-$this->posxparc-1,2, $outputlangs->transnoentities("APROBADO"),'','C');
						$pdf->line($this->posxdebe-1, $curY+34, $this->posxdebe-1, $curY+52);

						$pdf->SetXY($this->posxdebe-1, $curY+47);
