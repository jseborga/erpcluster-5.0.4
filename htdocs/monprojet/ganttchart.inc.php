...
for ($tcursor=0; $tcursor < $tnums; $tcursor++) {
	$t = $tasks[$tcursor];
	if ($t["task_parent"] == 0) {
	  constructGanttLine($tasks,$t,$project_dependencies,$level,$project_id,$aRes);
 ...
 * @return	void
 */
function constructGanttLine($tarr,$task,$project_dependencies,$level=0,$project_id=null,$aRes=null)
{
    ...
    if ($task['task_id'] == $aRes[$task['task_id']])
      $task['task_color'] = 'ffaaff';
    //$depend .= "\"";
    // Define parent
    ...
    $s = "g.AddTaskItem(new JSGantt.TaskItem(".$task['task_id'].",'".dol_escape_js($name)."','".dol_escape_js($namelg)."','".$start_date."', '".$end_date."', '".$task['task_color']."', '".$link."', ".$task['task_milestone'].", '".$resources."', ".$percent.", ".($task["task_is_group"]>0?1:0).", '".$parent."', 1 ".($depend?", '".$depend."'":'')." ));";
    echo $s."\n";
}
