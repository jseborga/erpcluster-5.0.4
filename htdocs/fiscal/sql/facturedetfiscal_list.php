...
/**
 *   	\file       fiscal/facturedetfiscal_list.php
 *		\ingroup    fiscal
...
't.fk_facturedet'=>array('label'=>$langs->trans("Fieldfk_facturedet"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->facturedetfiscal->read;
    $permtodelete = $user->rights->facturedetfiscal->delete;
    $uploaddir = $conf->facturedetfiscal->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."facturedet_fiscal as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."facturedet_fiscal_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_facturedet) $sql.= natural_search("fk_facturedet",$search_fk_facturedet);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/facturedetfiscal/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_facturedet']['checked'])) print_liste_field_titre($arrayfields['t.fk_facturedet']['label'],$_SERVER['PHP_SELF'],'t.fk_facturedet','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_facturedet']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_facturedet" value="'.$search_fk_facturedet.'" size="10"></td>';
