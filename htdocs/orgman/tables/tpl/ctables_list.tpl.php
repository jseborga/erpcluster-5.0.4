...
/**
 *   	\file       orgman/ctables_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-03-23 18:37
 */
...
dol_include_once('/orgman/class/ctables.class.php');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmanlistdet';

// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array
$hookmanager->initHooks(array('orgmanlistdet'));
...
$arrayfields=array(
	...
	't.tabledb'=>array('label'=>$langs->trans("Fieldtabledb"), 'align'=>'align="left"', 'checked'=>1),
	't.codetable'=>array('label'=>$langs->trans("Fieldcodetable"), 'align'=>'align="left"', 'checked'=>1),
	...
	't.type'=>array('label'=>$langs->trans("Fieldtype"), 'align'=>'align="left"', 'checked'=>0),
	't.range_ini'=>array('label'=>$langs->trans("Fieldrange_ini"), 'align'=>'align="left"', 'checked'=>1),
	't.range_fin'=>array('label'=>$langs->trans("Fieldrange_fin"), 'align'=>'align="left"', 'checked'=>1),
	't.fk_user_create'=>array('label'=>$langs->trans("Fieldfk_user_create"), 'align'=>'align="left"', 'checked'=>1),
	't.fk_user_mod'=>array('label'=>$langs->trans("Fieldfk_user_mod"), 'align'=>'align="left"', 'checked'=>1),
...
if (empty($reshook))
{
	...
	//$permtoread = $user->rights->ctables->read;
	//$permtodelete = $user->rights->ctables->delete;
	//$uploaddir = $conf->ctables->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_tables as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_tables_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
$sql.= " AND t.tabledb = '".trim($object->ref)."'";
...
if ($search_tabledb) $sql.= natural_search("tabledb",$search_tabledb);
if ($search_codetable) $sql.= natural_search("codetable",$search_codetable);
...
if ($search_range_ini) $sql.= natural_search("range_ini",$search_range_ini);
if ($search_range_fin) $sql.= natural_search("range_fin",$search_range_fin);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/ctables/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.tabledb']['checked'])) print_liste_field_titre($arrayfields['t.tabledb']['label'],$_SERVER['PHP_SELF'],'t.tabledb','',$params,$arrayfields['t.tabledb']['align'],$sortfield,$sortorder);
if (! empty($arrayfields['t.codetable']['checked'])) print_liste_field_titre($arrayfields['t.codetable']['label'],$_SERVER['PHP_SELF'],'t.codetable','',$params,$arrayfields['t.codetable']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.description']['checked'])) print_liste_field_titre($arrayfields['t.description']['label'],$_SERVER['PHP_SELF'],'t.description','',$params,$arrayfields['t.description']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.range_ini']['checked'])) print_liste_field_titre($arrayfields['t.range_ini']['label'],$_SERVER['PHP_SELF'],'t.range_ini','',$params,$arrayfields['t.range_ini']['align'],$sortfield,$sortorder);
if (! empty($arrayfields['t.range_fin']['checked'])) print_liste_field_titre($arrayfields['t.range_fin']['label'],$_SERVER['PHP_SELF'],'t.range_fin','',$params,$arrayfields['t.range_fin']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.tabledb']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.tabledb']['align'].'><input type="text" class="flat" name="search_tabledb" value="'.$search_tabledb.'" size="10"></td>';
if (! empty($arrayfields['t.codetable']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.codetable']['align'].'><input type="text" class="flat" name="search_codetable" value="'.$search_codetable.'" size="10"></td>';
...
if (! empty($arrayfields['t.description']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.description']['align'].'><input type="text" class="flat" name="search_description" value="'.$search_description.'" size="10"></td>';
...
if (! empty($arrayfields['t.range_ini']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.range_ini']['align'].'><input type="text" class="flat" name="search_range_ini" value="'.$search_range_ini.'" size="10"></td>';
if (! empty($arrayfields['t.range_fin']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.range_fin']['align'].'><input type="text" class="flat" name="search_range_fin" value="'.$search_range_fin.'" size="10"></td>';
...
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
	...
	print $formfile->showdocuments('massfilesarea_orgman','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
...
//{
	...
	print '<a class="butAction" href="' . dol_buildpath('/orgman/tables/carddet.php',1).'?id='.$id.'&action=create">' . $langs->trans('Create') . '</a>';
	print '</div>';
//}
