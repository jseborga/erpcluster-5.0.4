...
/**
 *	\file       htdocs/fabrication/admin/fabrication.php
 *	\ingroup    fabrication
...
if ($action == 'specimen')
{
	...
	{
	    $file=dol_buildpath($reldir."fabrication/core/modules/doc/pdf_".$modele.".modules.php",0);
		if (file_exists($file))
		{
			$filefound=1;
	...
	if ($filefound)
	{
		...
		{
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=fabrication&file=SPECIMEN.pdf");
			return;
		}
		else
		{
...
if ($action == 'del')
{
	...
	if ($db->query($sql))
	{
        if ($conf->global->FABRICATION_ADDON_PDF == "$value") dolibarr_del_const($db, 'FABRICATION_ADDON_PDF',$conf->entity);
	}
}
...
if ($action == 'setdoc')
{
	...
	if (dolibarr_set_const($db, "FABRICATION_ADDON_PDF",$value,'chaine',0,'',$conf->entity))
	{
		$conf->global->FABRICATION_ADDON_PDF = $value;
	}
...
if ($action == 'setmod')
{
	...
	dolibarr_set_const($db, "FABRICATION_ADDON",$value,'chaine',0,'',$conf->entity);
}
...
llxHeader("",$langs->trans("FabricationsSetup"));

$form=new Form($db);

$linkback='<a href="'.DOL_URL_ROOT.'/admin/modules.php">'.$langs->trans("BackToModuleList").'</a>';
print_fiche_titre($langs->trans("FabricationsSetup"),$linkback,'setup');
print '<br>';
...
print_titre($langs->trans("FabricationsNumberingModules"));
...
foreach ($dirmodels as $reldir)
{
  $dir = dol_buildpath($reldir."fabrication/core/modules/");
  var_dump(is_dir($dir));
	if (is_dir($dir))
	{
	  $handle = opendir($dir);
	  if (is_resource($handle))
		...
		{
		  if (substr($file, 0, 16) == 'mod_fabrication_' && substr($file, dol_strlen($file)-3, 3) == 'php')
		    {
		      $file = substr($file, 0, dol_strlen($file)-4);
		      
		      require_once DOL_DOCUMENT_ROOT ."/fabrication/core/modules/".$file.'.php';
		      ...
		      if ($module->isEnabled())
			{
			  ...
			  if ($conf->global->FABRICATION_ADDON == $file)
			    {
 ...
 */
print_titre($langs->trans("FabricationsModelModule"));
...
foreach ($dirmodels as $reldir)
{
    ...
    {
    	$dir = dol_buildpath($reldir."/fabrication/core/modules".$valdir);
                    ...
                    if (preg_match('/\.modules\.php$/i',$file) && preg_match('/^(pdf_|doc_)/',$file))
                    {
                    	...
                    	if (file_exists($dir.'/'.$file))
                    	{
	                        ...
	                        if ($modulequalified)
	                        {
	                            ...
	                            // Defaut
	                            print '<td align="center">';
	                            if ($conf->global->FABRICATION_ADDON_PDF == $name)
	                            {
