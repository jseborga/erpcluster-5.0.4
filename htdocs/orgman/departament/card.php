...
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->orgman->dpto->del)
{
	...
	if ($result > 0)
	{
		header("Location: ".DOL_URL_ROOT.'/orgman/departament/list.php');
		exit;
	}
	else
	{
...
llxHeader("",$langs->trans("Departaments"),$help_url);
...
if ($action == 'create' && $user->rights->orgman->dpto->write)
{
	...
	print $form->select_member(GETPOST('fk_user_resp'),'fk_user_resp', " d.statut = 1 ",1,0,0,array(),0);
...
else
{
	if ($id || $ref)
	{
		...
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			{
				$form = new Form($db);
				$ret=$form->form_confirm($_SERVER["PHP_SELF"]."?id=".$object->id,$langs->trans("Deletedepartament"),$langs->trans("ConfirmDeletedepartament",$object->ref).' '.$object->label,"confirm_delete",'',1,2);
			...
			$linkback = '<a href="'.DOL_URL_ROOT.'/orgman/departament/list.php">'.$langs->trans("BackToList").'</a>';
	  ...
	  //responsable
			...
			print $form->select_member($object->fk_user_resp,'fk_user_resp', " d.statut = 1 ",1,0,0,array(),0);
			print '</td></tr>';
