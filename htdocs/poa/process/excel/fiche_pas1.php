...
$aArrjs = array('poa/js/config.js','poa/js/ajax.js','poa/js/enviaoc.js');
	...
	foreach((array) $objpcon->array AS $j => $objpc)
	  {
		...
		$var=!$var;
		print "<tr $bc[$var]>";
		print '<td>'.'<a href="fiche_pas1.php?action=selcon&id='.$object->id.'&idrc='.$objpcontrat->id.(isset($_GET['nopac'])?'&nopac=1&idp='.$_GET['idp']:'').'">'.$objcont->array_options['options_ref_contrato'].'</a></td>';
		...
		foreach ((array) $aPrev AS $fk_poaprev => $gest)
		  {
		    ...
		    $amountc = 0;
		    //echo $ncolspan.' '.$fk_poaprev.' '.$objpcontrat->id;
			...
			if ($user->rights->poa->comp->mod && $gest == $gestion)
			  {
			    ...
			    print '<a href="fiche_pas1.php?id='.$id.'&amp;idrc='.$objpcontrat->id.'&action=editc'.(isset($_GET['nopac'])?'&nopac=1&idp='.$fk_poaprev:'&idp='.$fk_poaprev).'">'.img_picto($langs->trans('Edit'),DOL_URL_ROOT.'/poa/img/edit.png','',1).'</a>';
			    print '&nbsp';
			    print '&nbsp';
			  }
			else
			...
			if ($lOrderproceed)
			  {
			    ...
			    if (!empty($objpcontrat->date_order_proceed) && !is_null($objpcontrat->date_order_proceed))			  
			      {
				if ($user->admin || $user->id == $objprev->fk_user_create)
				  print '<a href="fiche_pas2.php?id='.$id.'&idc='.$objpcontrat->fk_contrat.'&action=create'.(isset($_GET['nopac'])?'&nopac=1&idp='.$_GET['idp']:'').'">'.img_picto($langs->trans('Payment'),DOL_URL_ROOT.'/poa/img/payment.png','',1).'</a>';
				else
				  print '&nbsp;';
			      }
			  }
