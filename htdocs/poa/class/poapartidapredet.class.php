...
/**
 * \file    poa/poapartidapredet.class.php
 * \ingroup poa
...
class Poapartidapredet extends CommonObject
{
	 ...
	 */
	public $element = 'poapartidapredet';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'poa_partida_pre_det';

	/**
	 * @var PoapartidapredetLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_poa_partida_pre)) {
			 $this->fk_poa_partida_pre = trim($this->fk_poa_partida_pre);
		}
		...
		if (isset($this->quant_adj)) {
			 $this->quant_adj = trim($this->quant_adj);
		}
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("poapartidapredet", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND')
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new PoapartidapredetLine();

				$line->id = $obj->rowid;
				
				$line->fk_poa_partida_pre = $obj->fk_poa_partida_pre;
