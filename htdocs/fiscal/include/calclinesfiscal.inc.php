...
{
    $tvadef = new Tvadefext($this->db);
    $objtype = new Ctypefacture($this->db);
}
...
foreach ((array) $tvaline AS $j => $data)
{
    ...
    if (trim(strtoupper($price_base_type)) == 'TTC')
    {
        if ($remise_percent>0)
        {
            ...
            $lines[$i]->remise = $discounttmp+0;
            $lines[$i]->remise_percent = $remise_percent+0;
        }
        elseif ($discount>0)
        {
            $pricetot = $qty * $pu;
        ...
        $lines[$i]->subprice = price2num($lines[$i]->total_ht/$lines[$i]->qty);
        $lines[$i]->price = price2num($lines[$i]->total_ttc/$lines[$i]->qty);
    ...
    {
        $lines[$i]->vat_src_code = $data->code_tva;
        $lines[$i]->tva_tx = $data->taux;
        $lines[$i]->total_tva = $tvacalc[$data->code_tva];
        $lines[$i]->total_ht = $lines[$i]->total_ttc - $lines[$i]->total_tva;
    }
    else
    {
        //$lines[$i]->tva_tx = 0;
...
if (count($tvaline)>0)
{
    ...
    {
        $lines[$i]->total_ht = $lines[$i]->total_ttc - $lines[$i]->total_tva -$lines[$i]->total_localtax1-$lines[$i]->total_localtax2;
        $lines[$i]->subprice = $lines[$i]->total_ht / $qty;
        $lines[$i]->price = $lines[$i]->total_ttc / $qty;
    }
}
else
{
    $lines[$i]->total_ttc = $qty * $lines[$i]->price;
    $lines[$i]->total_ht = $qty * $lines[$i]->subprice;
    $lines[$i]->total_tva = 0;
}
?>