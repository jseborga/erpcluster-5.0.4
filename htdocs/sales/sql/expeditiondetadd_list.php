...
/**
 *   	\file       sales/expeditiondetadd_list.php
 *		\ingroup    sales
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-12-23 14:58
 */
...
dol_include_once('/sales/class/expeditiondetadd.class.php');
...
$search_fk_expeditiondet=GETPOST('search_fk_expeditiondet','int');
...
$hookmanager->initHooks(array('expeditiondetaddlist'));
...
$object=new Expeditiondetadd($db);
...
't.fk_expeditiondet'=>array('label'=>$langs->trans("Fieldfk_expeditiondet"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."expeditiondet_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."expeditiondet_add_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_expeditiondet) $sql.= natural_search("fk_expeditiondet",$search_fk_expeditiondet);
...
if ($search_fk_expeditiondet != '') $params.= '&amp;search_fk_expeditiondet='.urlencode($search_fk_expeditiondet);
...
if ($search_status != '') $params.= '&amp;search_status='.urlencode($search_status);
    ...
    // 
if (! empty($arrayfields['t.fk_expeditiondet']['checked'])) print_liste_field_titre($arrayfields['t.fk_expeditiondet']['label'],$_SERVER['PHP_SELF'],'t.fk_expeditiondet','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.status']['checked'])) print_liste_field_titre($arrayfields['t.status']['label'],$_SERVER['PHP_SELF'],'t.status','',$params,'',$sortfield,$sortorder);
	...
	print '<tr class="liste_titre">';
	// 
if (! empty($arrayfields['t.fk_expeditiondet']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_expeditiondet" value="'.$search_fk_expeditiondet.'" size="10"></td>';
