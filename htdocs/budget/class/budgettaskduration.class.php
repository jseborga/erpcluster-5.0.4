...
/**
 * \file    budget/budgettaskduration.class.php
 * \ingroup budget
...
/**
 * Class Budgettaskduration
 *
 * Put here description of your class
 * @see CommonObject
 */
class Budgettaskduration extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'budgettaskduration';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'budget_task_duration';

	/**
	 * @var BudgettaskdurationLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->successor)) {
			 $this->successor = trim($this->successor);
		}
		if (isset($this->predecessor)) {
			 $this->predecessor = trim($this->predecessor);
		}
		...
		$sql .= ' '.(! isset($this->successor)?'NULL':"'".$this->db->escape($this->successor)."'").',';
		$sql .= ' '.(! isset($this->predecessor)?'NULL':"'".$this->db->escape($this->predecessor)."'").',';
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND', $filterstatic='', $lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new BudgettaskdurationLine();

				$line->id = $obj->rowid;
				
				$line->fk_budget_task = $obj->fk_budget_task;
				$line->successor = $obj->successor;
				$line->predecessor = $obj->predecessor;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' successor = '.(isset($this->successor)?"'".$this->db->escape($this->successor)."'":"null").',';
		$sql .= ' predecessor = '.(isset($this->predecessor)?"'".$this->db->escape($this->predecessor)."'":"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Budgettaskduration($this->db);
