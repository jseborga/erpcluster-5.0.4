...
/**
 *	\file       htdocs/salary/salarycharge/fiche.php
 *	\ingroup    salary Charges
...
if ($action == 'add' && $user->rights->salary->creardpto)
  {
    ...
    $object->salary_practiced  = GETPOST('salary_practiced');
    $object->salary_market     = GETPOST('salary_market');
    $object->salary_calc       = GETPOST('salary_calc');

    if ($object->fk_charge && $object->salary_practiced) 
      {
...
if ($action == 'create' && $user->rights->salary->creardpto)
  {
    ...
    //detail
    print '<tr><td class="fieldrequired">'.$langs->trans('Details').'</td><td colspan="2">';
    ...
    //nivel
    print '<tr><td class="fieldrequired">'.$langs->trans('Nivel').'</td><td colspan="2">';
    print '<input id="nivel" type="text" value="'.$object->nivel.'" name="nivel" size="1" maxlength="1">';
    print '</td></tr>';

    //practiced
    print '<tr><td class="fieldrequired">'.$langs->trans('Salarypracticed').'</td><td colspan="2">';
    print '<input id="salary_practiced" type="text" value="'.$object->salary_practiced.'" name="salary_practiced" size="12" maxlength="18">';
    print '</td></tr>';

    //market
    print '<tr><td>'.$langs->trans('Salarymarket').'</td><td colspan="2">';
    print '<input id="salary_market" type="text" value="'.$object->salary_market.'" name="salary_market" size="12" maxlength="18">';
    print '</td></tr>';

    //calc
    print '<tr><td>'.$langs->trans('Salarycalc').'</td><td colspan="2">';
    print '<input id="salary_calc" type="text" value="'.$object->salary_calc.'" name="salary_calc" size="12" maxlength="18">';
    print '</td></tr>';
 ...
 else
   {
      ...
      if ($action <> 'edit' && $action <> 're-edit')
	{
	  ...
	  if ($objectC->fetch($object->fk_charge))
	    {
	      $objectD->fetch($objectC->fk_dpto);
	      // charge
	      print $objectD->ref." : ".$objectC->ref;
	    }
	  else
	    print "";
	  print '</td></tr>';
	  ...
	  //practiced
	  print '<tr><td class="fieldrequired">'.$langs->trans('Salarypracticed').'</td><td colspan="2">';
	  print price($object->salary_practiced);
	  print '</td></tr>';
	  
	  //market
	  print '<tr><td>'.$langs->trans('Salarymarket').'</td><td colspan="2">';
	  print price($object->salary_market);
	  print '</td></tr>';
	  
	  //calc
	  print '<tr><td>'.$langs->trans('Salarycalc').'</td><td colspan="2">';
	  print price($object->salary_calc);
	  print '</td></tr>';
