...
class Paiementfournext extends Paiementfourn
{
	...
	function createadd($user,$closepaidinvoices=0)
	{
		...
		if ($totalamount <> 0) // On accepte les montants negatifs
		{
			...
			$sql.= " '".$this->db->idate($this->datepaye)."', '".$total."', '".$mtotal."', ".$this->paiementid.", '".$this->num_paiement."', '".$this->db->escape($this->note)."', ".$user->id.", $this->fk_bank)";

			dol_syslog("PaiementFournext::createadd", LOG_DEBUG);
			...
			if ($resql)
			{
				...
				foreach ($this->amounts as $key => $amount)
				{
					...
					{
						dol_syslog('PaiementFournext::Createadd Montant non numerique',LOG_ERR);
					}
				}
