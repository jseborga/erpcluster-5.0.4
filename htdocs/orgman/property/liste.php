...
/**
 *   	\file       orgman/mproperty_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-04-02 15:13
 */
...
dol_include_once('/orgman/class/mpropertyext.class.php');
...
$search_fk_state=GETPOST('search_fk_state','int');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmanlistproperty';
...
$arrayfields=array(
	...
	't.address'=>array('label'=>$langs->trans("Fieldaddress"), 'align'=>'align="left"', 'checked'=>1),
	't.fk_country'=>array('label'=>$langs->trans("Fieldfk_country"), 'align'=>'align="left"', 'checked'=>1),
	't.fk_state'=>array('label'=>$langs->trans("Fieldfk_state"), 'align'=>'align="left"', 'checked'=>1),
...
if (empty($reshook))
{
	...
	//$permtoread = $user->rights->mproperty->read;
	//$permtodelete = $user->rights->mproperty->delete;
	//$uploaddir = $conf->mproperty->dir_output;
...
$help_url='';
$title = $langs->trans('ListProperty');
...
$sql.= " FROM ".MAIN_DB_PREFIX."m_property as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."m_property_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_state) $sql.= natural_search("fk_state",$search_fk_state);
...
if ($search_status!=9) $sql.= natural_search("status",$search_status);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/mproperty/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.address']['checked'])) print_liste_field_titre($arrayfields['t.address']['label'],$_SERVER['PHP_SELF'],'t.address','',$params,$arrayfields['t.address']['align'],$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_country']['checked'])) print_liste_field_titre($arrayfields['t.fk_country']['label'],$_SERVER['PHP_SELF'],'t.fk_country','',$params,$arrayfields['t.fk_country']['align'],$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_state']['checked'])) print_liste_field_titre($arrayfields['t.fk_state']['label'],$_SERVER['PHP_SELF'],'t.fk_state','',$params,$arrayfields['t.fk_state']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.address']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.address']['align'].'><input type="text" class="flat" name="search_address" value="'.$search_address.'" size="10"></td>';
if (! empty($arrayfields['t.fk_country']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_country']['align'].'><input type="text" class="flat" name="search_fk_country" value="'.$search_fk_country.'" size="10"></td>';
if (! empty($arrayfields['t.fk_state']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_state']['align'].'><input type="text" class="flat" name="search_fk_state" value="'.$search_fk_state.'" size="10"></td>';
...
if ($user->rights->orgman->prop->write)
	print "<a class=\"butAction\" href=\"fiche.php?action=create\">".$langs->trans("New")."</a>";
