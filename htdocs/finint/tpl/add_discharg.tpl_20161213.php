	...
	if ($user->rights->finint->desc->crear)
	{
		...
		if ($lAdd)
		{
			...
			if ($conf->monprojet->enabled)
			{
				...
				print '<div class="form-group">';
				print '<label for="code_facture" class="sr_only">'.$langs->trans('Project').'</label>';
				...
				if ($fk_projetsel > 0)
				{
					if (!empty($fk_tasksel) ) $lViewtask = false;
					print '<div class="form-group">';
					print '<label for="code_facture" class="sr_only">'.$langs->trans('Item').'</label>';

					$filtertask = " t.fk_projet = ".$fk_projetsel;
					print $formtask->select_task($fk_tasksel, 'fk_tas', $filtertask, 1,0,0,array(),0,'',0);
					print '</div>';
			...
			if($conf->browser->layout=='classic')
			{
				...
				print '<tbody>';
				//qr
				print '<tr class="fila-base">';
				if ($lViewqr)
				{
					print '<td>';
					print '<input id="codeqr" class="flat" size="4" type="text" name="codeqr[]" value="'.$myclass->codeqr.'">';
					print '</td>';
					//nit
					print '<td>';
					print '<input id="nit" class="flat" size="6" type="text" name="nit[]" value="'.$myclass->nit.'">';
					print '</td>';
				}
				//date
				print '<td nowrap>';
				//$form->select_date_v(($myclass->fourn_date?$myclass->fourn_date:$myclass->dateo),'_do','','','','transaction',1,1,0,0,'','','',0);
				$aHoy = dol_getdate(dol_now());
				print '<input type="date" name="_do[]" value="'.date('Y-m-d').'">';
				print '</td>';
				//num_chq
				print '<td>';
				print '<input id="num_chq" class="flat" size="4" type="text" name="num_chq[]" value="'.$myclass->num_chq.'">';
				print '</td>';
				...
				if ($lViewproj)
				{
					print '<td>';
					$filterkey = '';
					$numprojet = $formproject->select_projects(($user->societe_id>0?$user->societe_id:-1), $fk_projet, 'fk_projet__-1', 0,0,1,0,0,0,0,$filterkey);
					print '</td>';
				}
				if ($lViewtask)
				{
					print '<td>';
					print $formtask->select_task(0, 'fk_task__-1', $filtertask, 0,0,0,array(),0,'',0);
					print '</td>';
				}
				//qty
				print '<td>';
				print '<input name="dp_desc[]" class="flat" size="7" type="text" size="24"  value="'.$myclass->label.'">';
				print '</td>';
				print '<td>';
				print '<input name="amount[]" class="flat len80" type="number" step="any" min="0" value="">';
				print '</td>';			
