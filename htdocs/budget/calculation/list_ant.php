...
$hookmanager->initHooks(array('parametercalculationlist'));
...
if (empty($reshook))
{
	...
	if ($action == 'active')
	{
		...
		{
			// Delete OK
			setEventMessages("Updatesuccessfull", null, 'mesgs');
...
$help_url='';
$title = $langs->trans('Calculationparameters');
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."parameter_calculation_extrafields as ef on (u.rowid = ef.fk_object)";
//$sql.= " WHERE 1 = 1";
$sql.= " WHERE t.entity IN (".getEntity('parameter_calculation',1).")";
    ...
    while ($i < min($num, $limit))
    {
    	...
    	if ($obj)
    	{
    		...
    		if (! empty($arrayfields['t.active']['checked']))
    		{
    			...
    			{
	    			print '<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?id='.$obj->rowid.'&action=active">'.($obj->active?img_picto('','switch_on'):img_picto('','switch_off')).'</a>'.'</td>';
    			}
    			else
    		...
    		{
    			print '<td align="center">'.$object->getLibStatut(2).'</td>';
    		}
