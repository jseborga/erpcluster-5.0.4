...
/**
 * \file    almacen/stockmouvementdoc.class.php
 * \ingroup almacen
...
/**
 * Class Stockmouvementdoc
 *
 * Put here description of your class
 * @see CommonObject
 */
class Stockmouvementdoc extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'stockmouvementdoc';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'stock_mouvement_doc';

	/**
	 * @var StockmouvementdocLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_entrepot_to)) {
			 $this->fk_entrepot_to = trim($this->fk_entrepot_to);
		}
		...
		$sql .= ' '.(! isset($this->fk_entrepot_to)?'NULL':$this->fk_entrepot_to).',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fk_entrepot_to = $obj->fk_entrepot_to;
	 ...
	 */
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$fiterstatic='',$lView=false)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_entrepot_to = '.(isset($this->fk_entrepot_to)?$this->fk_entrepot_to:"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Stockmouvementdoc($this->db);
	...
	function LibStatut($status,$mode=0)
	{
		...
		if ($mode == 6)
		{
			if ($status == 0) return $langs->trans('RequestClosed');
			if ($status == 1) return $langs->trans('RequestOpen');
			if ($status == 2) return $langs->trans('RequestClosed');
		}
		if ($mode == 7)
		{
			if ($status == 0) return img_picto($langs->trans('RequestClosed'),'statut4').' '.$langs->trans('RequestClosed');
			if ($status == 1) return img_picto($langs->trans('RequestOpen'),'statut5').' '.$langs->trans('RequestOpen');
			if ($status == 2) return img_picto($langs->trans('RequestClosed'),'statut4').' '.$langs->trans('RequestClosed');
		}
	}
