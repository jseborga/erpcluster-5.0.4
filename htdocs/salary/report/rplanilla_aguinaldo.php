...
if ($action == 'reporteExcel')
{
	...
	$objPHPExcel->getActiveSheet()->getStyle('K')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_DATE_DDMMYYYY);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('L7',$langs->trans("Totalrend"));
	$objPHPExcel->getActiveSheet()->setCellValue('M7',$langs->trans("Liquido"));
	$objPHPExcel->getActiveSheet()->setCellValue('N7',$langs->trans("Firma"));
	...
	$objPHPExcel->getActiveSheet()->getStyle('A7:N7')->applyFromArray(
		array(
			'font'    => array(
				'bold'      => true
			),
	...
	foreach ((array) $aReportplanilla AS $idUser => $data)
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('L' .$j,$data['totalrend']);
		$objPHPExcel->getActiveSheet()->setCellValue('M' .$j,$data['liquido']);
...
else
{
	...
	if ($action=='edit' || $action == 'generate')
	{
		...
		foreach ((array) $aPlanilla AS $idUser => $dataUser)
		{
			...
			//dias trabajados
			$res105 = $objectsp->fetch_salary_concept($idUser,$fk_period,$fk_proces,$fk_type_fol,'110');
			...
			//total rendimiento
			$objectsp->initAsSpecimen();
			$objectsp->fetch_salary_concept($idUser,$fk_period,$fk_proces,$fk_type_fol,'112');
			print '<td align="right">'.price(price2num($objectsp->amount,'MT')).'</td>';
			$aReportplanilla[$idUser]['totalrend'] = $objectsp->amount;

			//liquido
			$objectsp->initAsSpecimen();
			$objectsp->fetch_salary_concept($idUser,$fk_period,$fk_proces,$fk_type_fol,'598');
			print '<td align="right">'.price(price2num($objectsp->amount,'MT')).'</td>';
		...
		if ($action == 'edit' && $lPeriodClose == false && $user->rights->salary->validsal)
		{
			...
			if ($state == $nMaxAprob) $nSeqAprob = $nMaxAprob;
