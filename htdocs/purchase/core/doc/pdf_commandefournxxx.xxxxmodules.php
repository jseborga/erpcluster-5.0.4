 ...
 */
class pdf_commandefourn extends ModelePDFSuppliersOrders
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->aArraytype = array(1=>$langs->trans('Contract'),2=>$langs->trans('Purchase order'),3=>$langs->trans('Service order'),4=>$langs->trans('Other'));
		$this->aTypeprocess = array(1=>array('WELL' => $langs->trans('Goods')),0=>array('OTHERSERVICE'=>$langs->trans('Otherservice'),'SERVICE'=>$langs->trans('Service')));
		$this->aTerm = array(1=>$langs->trans('D.C.'),2=>$langs->trans('D.H.'),3=>$langs->trans('Fixed term'));
	...
	function write_file($object,$outputlangs='',$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		{
			$objectadd = new Commandefournisseuradd($this->db);

			$object->fetch_thirdparty();
			$resadd = $objectadd->fetch(0,$object->id);
			if ($resadd>0)
			{
				$object->ref_contrat = $objectadd->ref_contrat;
				$object->term = $objectadd->term;
				$object->ref_term = $objectadd->ref_term;
				$object->typeadd = $objectadd->type;
				$object->advance = $objectadd->advance;
				$object->order_proceed = $objectadd->order_proceed;
				$object->designation_fiscal = $objectadd->designation_fiscal;
				$object->designation_supervisor = $objectadd->designation_supervisor;
				$object->date_ini = $objectadd->date_ini;
				$object->date_fin = $objectadd->date_fin;
			}
			$deja_regle = "";
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Quotation")." ".$outputlangs->convToOutputCharset($object->ref);
		if ($object->typeadd)
		{
			$title = $outputlangs->transnoentities($this->aTypeprocess[$object->typeadd])." ".$outputlangs->convToOutputCharset($object->ref_contrat);
		}
