...
class pdf_departamentasset extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function __construct($db)
	{
		...
		$titulo = "";
		$this->db = $db;
		$this->name = "departamentasset";
		$this->description = $langs->trans('PDFDepartamentasset');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->assets->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->assets->dir_output.'/departament';
			//if ($object->group)
			//	$dir = $conf->assets->dir_output.'/'.$object->group;
				$file = $dir . "/" . $aExtras['nombre'] . ".pdf";
			}
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReportdetasset as $j => $line)
				{
					...
					if(empty($line['Codigo'])){
						...
						$pdf->SetXY($this->posxcodext, $curY);
						$pdf->MultiCell($this->posxeti-$this->posxcodext-1, 1, dol_trunc($line['Etiqueta'],51), 0, 'L',0);
							//$nexYaux = $pdf->GetY();
						...
						$pdf->MultiCell($this->posxcos-$this->posxfead-1, 1, price(price2num($line['costo'],'MT')), 0, 'R',0);
							...
							$pdf->MultiCell($this->posxloca-$this->posxinmu-1, 1, $line['location'], 0, 'C',0);
							...
							*/
							...
							$pdf->MultiCell($this->posxvalact-$this->posxres-1, 1, price(price2num($line['Valoract'],'MT')), 0, 'R',0);

							// depreciacion acumulada
							$pdf->SetXY($this->posxvalact, $curY);
							$pdf->MultiCell($this->posxdepacum-$this->posxvalact-1, 1, price(price2num($line['Depreacum'],'MT')), 0, 'R',0);

							// balance
							$pdf->SetXY($this->posxdepacum, $curY);
							$pdf->MultiCell($this->posxfin-$this->posxdepacum-1, 1, price(price2num($line['Balance'],'MT')), 0, 'R',0);
		...
		if($type_page==0)
			{   /* La linea rectangular de pagina horizontal */
				...
				if (empty($hidetop)){
					...
					$pdf->MultiCell($this->posxcodext-$this->posxcod-1,2, $outputlangs->transnoentities("Refext"),'','C');
					...
					$pdf->MultiCell($this->posxfead-$this->posxeti-1,2, $outputlangs->transnoentities("Dateacquired"),'','C');
					$pdf->line($this->posxfead-1, $tab_top, $this->posxfead-1, $tab_top + $tab_height);

					$pdf->SetXY($this->posxfead-1, $tab_top+2);
					$pdf->MultiCell($this->posxcos-$this->posxfead-1,2, $outputlangs->transnoentities("Coste"),'','C');
			...
			*/
			...
			$pdf->SetXY($this->posxcos-1, $tab_top+2);
			$pdf->MultiCell($this->posxres-$this->posxcos-1,2, $outputlangs->transnoentities("Responsible"),'','C');
			$pdf->line($this->posxres-1, $tab_top, $this->posxres-1, $tab_top + $tab_height);


			// valor actualizado
			$pdf->SetXY($this->posxres-1, $tab_top+2);
			$pdf->MultiCell($this->posxvalact-$this->posxres-1,2, $outputlangs->transnoentities("Valoract"),'','C');
			$pdf->line($this->posxvalact-1, $tab_top, $this->posxvalact-1, $tab_top + $tab_height);
			// depreciacion acumulada
			$pdf->SetXY($this->posxvalact-1, $tab_top+2);
			$pdf->MultiCell($this->posxdepacum-$this->posxvalact-1,2, $outputlangs->transnoentities("Depracum"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		if(empty($aExtras ['nombre'])){
			$titulo = "TODOS LOS DEPARTAMENTOS";
		}else{
			$titulo = "DEPARTAMENTO ".$aExtras ['nombre'];
		}
		...
		*/
		...
		//$title=$outputlangs->transnoentities("Determinacion de Servicios");
			$title=$outputlangs->transnoentities("Reportassetsdepartment");
		//$title=$outputlangs->transnoentities($titulo);
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DESDE FECHA")." : ". dol_print_date($aExtras['date_ini'],'daytext'), '', 'R');

			//Fecha de elaborarion
			$posy+=7;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("HASTA FECHA")." : " . dol_print_date($aExtras['date_fin'],'daytext'), '', 'R');
