...
class pdf_journal extends ModelePDFContab
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		$this->aType = array(1=>$outputlangs->transnoentities('Ingresos'),2=>$outputlangs->transnoentities('Egresos'),3=>$outputlangs->transnoentities('Traspasos'));
		...
		if ($conf->contab->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->contab->dir_output . "/contab/".$period_year.'/journal';
				$file = $dir . "/journal.pdf";
			}
			//echo $file;
			if (! file_exists($dir))
			{
			...
			if (file_exists($dir))
			{
				...
				$tab_top = 45;
				$tab_top_newpage = (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)?45:10);//60:10
				$tab_height = 45;
				//130
				...
				foreach ( (array) $aReporte[1] as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-1);
						...
						//Color
						$pdf->SetFillColor(219, 246, 249);
						$pdf->SetXY($this->posxini, $curY);
						$pdf->MultiCell(257, 1,"" , 0, 'C',1);
						//Fin Color
						...
						$pdf->MultiCell($this->posxfech-$this->posxrefe-1, 1, dol_print_date($line['fecha'],'day'), 0, 'L',0);

						$pdf->SetXY($this->posxfech, $curY);
						//$pdf->MultiCell($this->posxdivi-$this->posxfech-1, 1, $line['divisa'], 0, 'L',0);


						$pdf->SetXY($this->posxdivi, $curY);
						$pdf->MultiCell($this->posxcodi-$this->posxdivi-1, 1, $line['codigo'], 0, 'C',0);

						$pdf->SetXY($this->posxcodi, $curY);
						$pdf->MultiCell($this->posxcomp-$this->posxcodi-1, 1, dol_print_date($line['comprobante'],'day'), 0, 'L',0);


						$pdf->SetXY($this->posxcomp, $curY);
						$pdf->MultiCell($this->posxresp-$this->posxcomp-1, 1, $line['respaldo'], 0, 'L',0);
						$nexY = $pdf->GetY();

						if($line['debito'] > 0){
							$pdf->SetXY($this->posxresp, $curY);
							$pdf->MultiCell($this->posxdebi-$this->posxresp-1, 1, price(price2num($line['debito'],'MT')), 0, 'R',0);
							$sumaDebito = $sumaDebito + $line['debito'];
						}else{
							$pdf->SetXY($this->posxresp, $curY);
							$pdf->MultiCell($this->posxdebi-$this->posxresp-1, 1, $line['debito'], 0, 'R',0);
						}

						if($line['credito'] > 0){
							$pdf->SetXY($this->posxdebi, $curY);
							$pdf->MultiCell($this->posxcred-$this->posxdebi-1, 1,price(price2num($line['credito'],'MT')), 0, 'R',0);
							$sumaCredito = $sumaCredito + $line['credito'];
						}else{
							$pdf->SetXY($this->posxdebi, $curY);
							$pdf->MultiCell($this->posxcred-$this->posxdebi-1, 1,$line['credito'], 0, 'R',0);
						}

						$pdf->SetXY($this->posxcred, $curY);
						$pdf->MultiCell($this->posxfin-$this->posxcred-1, 1, $line['estado'], 0, 'R',0);
						...
						$pdf->MultiCell(257, 1,"HISTORIA :". $line['historia'] , 0, 'L',0);
						$nexY = $pdf->GetY();
						//$curY = $curY +3;
						$nexY = $nexY + 2;
						...
						$sw = 0;
						/*Aqui Comienza a mostrar los hijos*/
						foreach ( (array) $aSub[$j] as $key => $value)
						{
							...
							$pdf->SetFont('','', $default_font_size-1);
								...
								if($sw == 0){
									/***************************************************************************************************/
										//Color
										$pdf->SetFillColor(179, 214, 219);
										$pdf->SetXY($this->posxini, $curY);
										$pdf->MultiCell(257, 1,"" , 0, 'C',1);
										//Fin Color

										$pdf->SetXY($this->posxini, $curY);
										$pdf->MultiCell($this->posxrefe-$this->posxini-1, 1, "Ref", 0, 'L',0);

										$pdf->SetXY($this->posxrefe, $curY);
										$pdf->MultiCell($this->posxdivi-$this->posxrefe-1, 1, "Cuenta Debito" , 0, 'C',0);

										$pdf->SetXY($this->posxdivi, $curY);
										$pdf->MultiCell($this->posxcomp-$this->posxdivi-1, 1, "Cuenta Credito", 0, 'C',0);


										$pdf->SetXY($this->posxcomp, $curY);
										$pdf->MultiCell($this->posxresp-$this->posxcomp-1, 1, "Parcial", 0, 'R',0);

										$pdf->SetXY($this->posxresp, $curY);
										$pdf->MultiCell($this->posxdebi-$this->posxresp-1, 1, "Monto Debito", 0, 'R',0);

										$pdf->SetXY($this->posxdebi, $curY);
										$pdf->MultiCell($this->posxcred-$this->posxdebi-1, 1, "Monto Credito", 0, 'R',0);
										$nexY = $pdf->GetY();
										$curY = $nexY + 2;
										$sw = 1;
									/***************************************************************************************************/
								...
								//Color fondo
								$pdf->SetFillColor(212, 236, 252);
								$pdf->SetXY($this->posxini, $curY);
								$pdf->MultiCell(257, 1,"" , 0, 'C',1);
								//Fin Color de Fondo

								$pdf->SetXY($this->posxini, $curY);
								$pdf->MultiCell($this->posxrefe-$this->posxini-1, 1, $value['ref'], 0, 'C',0);

								$pdf->SetXY($this->posxrefe, $curY);
								$pdf->MultiCell($this->posxdivi-$this->posxrefe-1, 1, $value['cuentadebito']." ". $value['detalledebito'] , 0, 'L',0);

								$pdf->SetXY($this->posxdivi, $curY);
								$pdf->MultiCell($this->posxcomp-$this->posxdivi-1, 1, $value['cuentacredito']." ". $value['detallecredito'], 0, 'L',0);


								$pdf->SetXY($this->posxcomp, $curY);
								$pdf->MultiCell($this->posxresp-$this->posxcomp-1, 1, price($value['parcial']), 0, 'R',0);

								$pdf->SetXY($this->posxresp, $curY);
								$pdf->MultiCell($this->posxdebi-$this->posxresp-1, 1, price($value['montodebito']), 0, 'R',0);

								$pdf->SetXY($this->posxdebi, $curY);
								$pdf->MultiCell($this->posxcred-$this->posxdebi-1, 1, price($value['montocredito']), 0, 'R',0);
				...
				if ($pagenb == 1)
				{
					...
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
					$this->pdf_pie_total($pdf, $object, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, $outputlangs,$marge_gauche, $page_largeur,$marge_droite,$page_hauteur,$type_page,$bottomlasttab,$sumaDebito,$sumaCredito);
				}
				else
				{
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)){
			...
			$pdf->line($this->posxrefe-1,  $tab_top, $this->posxrefe-1, $tab_top+10);

			$pdf->SetXY($this->posxrefe-1, $tab_top+3);
			$pdf->MultiCell($this->posxdivi-$this->posxrefe-1,2, $outputlangs->transnoentities("Date"),'','C');
			$pdf->line($this->posxdivi-1,  $tab_top, $this->posxdivi-1, $tab_top+10);

			$pdf->SetXY($this->posxdivi-1, $tab_top+2);
			$pdf->MultiCell($this->posxcodi-$this->posxdivi-1,2, $outputlangs->transnoentities("Codigo de Transaccion"),'','C');
			$pdf->line($this->posxcodi-1,  $tab_top, $this->posxcodi-1, $tab_top+10);


			$pdf->SetXY($this->posxcodi-1, $tab_top+2);
			$pdf->MultiCell($this->posxcomp-$this->posxcodi-1,2, $outputlangs->transnoentities("Fecha Comprobante"),'','C');
			$pdf->line($this->posxcomp-1,  $tab_top, $this->posxcomp-1, $tab_top+10);

			$pdf->SetXY($this->posxcomp-1, $tab_top+2);
			$pdf->MultiCell($this->posxresp-$this->posxcomp-1,2, $outputlangs->transnoentities("Respaldo"),'','C');
			$pdf->line($this->posxresp-1,  $tab_top, $this->posxresp-1, $tab_top+10);

			$pdf->SetXY($this->posxresp-1, $tab_top+2);
			$pdf->MultiCell($this->posxdebi-$this->posxresp-1,2, $outputlangs->transnoentities("Debito"),'','C');
			$pdf->line($this->posxdebi-1,  $tab_top, $this->posxdebi-1, $tab_top+10);

			$pdf->SetXY($this->posxdebi-1, $tab_top+2);
			$pdf->MultiCell($this->posxcred-$this->posxdebi-1,2, $outputlangs->transnoentities("Credito"),'','C');
			$pdf->line($this->posxcred-1,  $tab_top, $this->posxcred-1, $tab_top+10);

			$pdf->SetXY($this->posxcred-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxcred-1,2, $outputlangs->transnoentities("Estado"),'','C');
			//$pdf->line($this->posxcred-1,  $tab_top, $this->posxcred-1, $tab_top+10);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(200, 3, $outputlangs->transnoentities("Unidad Ejecutora")." : ".$aReporte[6] , '', 'R');
		//Fecha de elaborari$aCabecera['nombre']on
		$posy+=5;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(200, 3, $outputlangs->transnoentities("Tipo de Asiento")." : " . $this->aType[$aReporte[5]] , '', 'R');
		//Divisa
		$posy+=5;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(200, 3, $outputlangs->transnoentities("Divisa")." : " . $aReporte[7] , '', 'R');

		$posy+=3;
		$pdf->SetXY(10,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(270, 3, $langs->trans('De ')." ".dol_print_date($aReporte[3],'day')." ".$langs->trans('Al ')." ".dol_print_date($aReporte[4],'day'), '', 'C');
	...
	function pdf_pie_total(&$pdf, $object, $posy, $outputlangs,$marge_gauche, $page_largeur,$marge_droite,$page_hauteur,$type_page=0,$bottomlasttab,$sumaDebito,$sumaCredito)
	{
		global $conf,$langs;
		$default_font_size = pdf_getPDFFontSize($outputlangs);
		...
		else
		{
			...
			$pdf->SetXY($this->posxcomp, $curY);
			$pdf->MultiCell($this->posxresp-$this->posxcomp-1, 1, $langs->trans("Total")." : ", 1, 'R',0);
			$pdf->SetXY($this->posxresp, $curY);
			$pdf->MultiCell($this->posxdebi-$this->posxresp-1, 1, price(price2num($sumaDebito,'MT')), 0, 'R',1);
			$pdf->SetXY($this->posxdebi, $curY);
			$pdf->MultiCell($this->posxcred-$this->posxdebi-1, 1, price(price2num($sumaCredito,'MT')), 0, 'R',1);
