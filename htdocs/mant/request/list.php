...
if (!isset($_SESSION['tsearch_statut'])) $_SESSION['tsearch_statut']=-1;
...
$objWorkuser = new Mworkrequestuserext($db);

$form=new Form($db);

$sql  = "SELECT p.rowid, p.ref, p.fk_member, p.email, p.internal, p.date_create, p.detail_problem, p.status, p.fk_soc, p.speciality, ";
$sql.= " s.ref AS refjobs, s.fk_work_request, s.date_create AS date_createjobs, s.rowid AS rowidjobs, ";
$sql.= " s.fk_soc AS fk_socjobs, s.speciality AS specialityjobs, s.status AS statutjobs ";
...
if ($search_statut>=0)
{
	$sql.= " AND p.status = ".$search_statut;
}
...
if ($result)
{
	...
	if ($num)
	{
					...
					if (count($aArray) > 0)
					{
						foreach((array) $aArray As $j => $obju)
						{
							...
							if ($objUser->fetch($obju->fk_user)>0)
							{
								...
								$htmlc.=$objUser->login.'</td>';
				  //$objUser->lastname.' '.$objUser->firstname.'</td>';
							}
						}
					}
				}
		...
		if ($lPrint)
		{
			...
			$var=!$var;
			print "<tr $bc[$var]>";
			print '<td><a href="card.php?id='.$objp->rowid.'">'.img_picto($langs->trans('Ticket'),DOL_URL_ROOT.'/mant/img/ticket','',1). $objp->ref.'</a></td>';

			print '<td><a href="card.php?id='.$objp->rowid.'">'.img_object($langs->trans("Showjobsorder"),'calendar').' '. dol_print_date($objp->date_create,'day').'</a></td>';
		//buscamos al usuario
			$objAdherent->fetch($objp->fk_member);
			...
			print '<td>';
			if ($objp->rowidjobs>0)
				print $objMjobs->getNomUrl(1);
			print '</td>';
			//.'<a href="'.DOL_URL_ROOT.'/mant/jobs/fiche.php?id='.$objp->rowidjobs.'">'.$objp->refjobs.'</a>'.'</td>';
			$object->status = $objp->status;
			print '<td nowrap align="right">'.$object->getLibStatut(6).'</td>';
			if ($objp->statut == 4 && $search_statut == 4)
		  //tecnico asignado
			...
			//se debe crear una asignacion para un ticket
				...
				if ($objp->statut == 1 && $search_statut == 1)
		  //validado
					if ($user->rights->mant->tick->ass)
					{
						print '<td align="center">';
