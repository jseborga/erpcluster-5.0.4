 ...
 * @see CommonObject
 */
class Vdosing extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'vdosing';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'v_dosing';

	/**
	 * @var VdosingLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("vdosing", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Dosing") . '</u>';
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;

        $url = DOL_URL_ROOT.'/fiscal/dosing/card.php?id='.$this->id;
