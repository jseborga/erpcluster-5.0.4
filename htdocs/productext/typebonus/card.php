...
/**
 *   	\file       productext/cbonustype_card.php
 *		\ingroup    productext
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-03-20 18:38
 */
...
dol_include_once('/productext/class/cbonustypeext.class.php');
...
$hookmanager->initHooks(array('cbonustype'));
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/productext/typebonus/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/productext/typebonus/list.php',1));
			exit;
		}
		else
		{
...
$aType=array();
$aType['L']=$langs->trans('Limit');
$aType['C']=$langs->trans('Conditional');
...
{
	$selected = '';
	if((GETPOST('type')?GETPOST('type'):$object->type)==$j)$selected = 'selected';
	$optionstype.= '<option value="'.$j.'" '.$selected.' >'.$line.'</option>';
}
...
llxHeader('',$langs->trans('Typebonus'),'');
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewTypebonus"));
...
//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldentity").'</td><td><input class="flat" type="text" name="entity" value="'.GETPOST('entity').'"></td></tr>';
	...
	print '<tr>';
	print '<td width="20%" class="fieldrequired">'.$langs->trans("Fieldtype").'</td><td>';
	print '<select name="type" value="'.$type.'">'.$optionstype.'</select>';;
	print '</td>';
	print '</tr>';
...
{
	print load_fiche_titre($langs->trans("Typebonus"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if($object->type=='C')
	{
		print '<tr><td class="fieldrequired">'.$langs->trans("Fieldtype").'</td><td>'.$langs->trans("Conditional").'</td></tr>';
	}
	if($object->type=='L')
	{
		print '<tr><td class="fieldrequired">'.$langs->trans("Fieldtype").'</td><td>'.$langs->trans("Limit").'</td></tr>';
	...
	if (empty($reshook))
	{
		if ($user->rights->productext->teacher->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>'."\n";
		}

		if ($user->rights->productext->teacher->del)
		{
	...
	//$linktoelem = $form->showLinkToObjectBlock($object, null, array('cbonustype'));
