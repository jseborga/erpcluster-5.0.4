...
/**
 * \file    fiscal/entityadd.class.php
 * \ingroup fiscal
 ...
 * @see CommonObject
 */
class Entityadd extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'entityadd';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'entity_add';

	/**
	 * @var EntityaddLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_entity)) {
			 $this->fk_entity = trim($this->fk_entity);
		}
		...
		$sql .= ' '.(! isset($this->fk_entity)?'NULL':$this->fk_entity).',';
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND', $filterstatic='', $lView=false)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("entityadd", 1) . ")";
		}
		if (count($sqlwhere) > 0) {
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new EntityaddLine();

				$line->id = $obj->rowid;

				$line->fk_entity = $obj->fk_entity;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_entity = '.(isset($this->fk_entity)?$this->fk_entity:"null").',';
