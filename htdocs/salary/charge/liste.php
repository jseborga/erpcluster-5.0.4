...
if (!$user->rights->salary->charge->lire)
  accessforbidden();
...
$sql  = "SELECT p.rowid AS id, p.ref, p.detail, p.skills ";
...
if ($sall)
{
    $sql.= " AND (p.ref like '%".$sall."%' OR p.detail like '%".$sall."%' OR p.skills like '%".$sall."%')";
}
...
if ($result)
{
	...
	print_barre_liste($langs->trans("Liste charges"), $page, "liste.php", "", $sortfield, $sortorder,'',$num);
	...
	print_liste_field_titre($langs->trans("Skills"),"liste.php", "p.skills","","","",$sortfield,$sortorder);
	print "</tr>\n";
	if ($num) {
	  $var=True;
	  while ($i < min($num,$limit))
	    {
	      ...
	      $var=!$var;
	      print "<tr $bc[$var]>";
	      print '<td><a href="fiche.php?id='.$obj->id.'">'.img_object($langs->trans("Ref"),'charge').' '.$obj->ref.'</a></td>';	      
	      print '<td>'.$obj->detail.'</td>';
	      print '<td>'.$obj->skills.'</td>';
	      print "</tr>\n";
	      $i++;
	    }
	}
