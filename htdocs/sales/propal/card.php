...
if (empty($reshook))
{
	...
	// Add line
		...
		if (!$conf->global->FISCAL_CODE_FACTURE_SALES)
		{
			$error++;
			setEventMessages($langs->trans('Error, no esta definido el codigo de factura para ventas'),null,'errors');
		}
		...
		if (! $error && ($qty >= 0) && (! empty($product_desc) || ! empty($idprod)))
		{
			...
			// Local Taxes
				...
				if ($conf->fiscal->enabled)
				{
					...
					$result = $object->addlineadd($desc, $lines->subprice, $qty, $txtva, $txlocaltax1, $txlocaltax2, $idprod, $remise_percent, $price_base_type, $lines->price, $type, $info_bits, $notrigger, $date_start, $date_end, $array_options, $lines->fk_unit,$lines);
				}
				else
				{
				...
				if ($result > 0)
				{
					$objecttmp = new PropaleLigneext($db);
					$objecttmp->get_sum_taxes($id);
					$aData = $objecttmp->aData;
					$objtmp = new Propalext($db);
	...
	// Update a line within proposal
		...
		//procesamos el calculo de los impuestos
			...
			$lines->subprice = GETPOST('price_ht');
			$lines->price = $pu;
			$lines->fk_unit = GETPOST('fk_unit');
			$lines->special_code = GETPOST('special_code');
			$remise_percent = GETPOST('remise_percent');
			$discount = GETPOST('discount');
		//if (empty($lines->fk_unit)) $lines->fk_unit = $productsupplier->fk_unit;
		//if (!empty(GETPOST('fk_unit'))) $lines->fk_unit = GETPOST('fk_unit');
...
$parameters = array('colspan' => ' colspan="3"');
	...
	$head = sales_propal_prepare_head($object);
			...
			if ($action != 'statut' && $action != 'editline')
			{
				...
				if (! empty($conf->commande->enabled) && $object->statut == Propal::STATUS_SIGNED) {
					if ($user->rights->commande->creer) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/sales/commande/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddOrder") . '</a></div>';
					}
				}
