...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield="t.date_ass"; // Set here default search field
...
if ($action == 'create')
{
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Selectname").'</td><td>';
	if ($mc=='m')
	{
...
{
	print '<table class="border centpercent">'."\n";
	print '<tr><td align="right">'.'<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=create">'.$langs->trans("Newmark").'</a></td><tr>';
	print '</table>'."\n";
...
if ($action == 'list' || $action == 'delete' || (empty($id) && $action != 'register') )
{
	...
	if ($resql)
	{
		...
		print_liste_field_titre($langs->trans('Name'),$_SERVER['PHP_SELF'],'a.lastname','',$param,'',$sortfield,$sortorder);
		print_liste_field_titre($langs->trans('Type'),$_SERVER['PHP_SELF'],'t.marking_number','',$param,'',$sortfield,$sortorder);
		...
		print '</td>';
		print '<td class="liste_titre">';
		print $form->select_date($search_date,'se_',0,0,1);
		print '</td>';
