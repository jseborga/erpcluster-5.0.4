...
if (empty($reshook))
{
	...
	// Mass actions
	$objectclass='Cpartida';
	$objectlabel='Partida';
	$permtoread = $user->rights->orgman->part->lire;
	$permtodelete = $user->rights->orgman->part->del;
	$uploaddir = $conf->orgman->dir_output;
	...
	if ($action == 'add')
	{
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/partida/list.php?action=create',1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	if ($action == 'update')
	{
		...
		if (GETPOST('code_father')>0)
		{
			$objecttmp->fetch(GETPOST('code_father'));
			$code_father = $objecttmp->code;
		}
		else
			$code_father = '00000';
...
$help_url='';
$title = $langs->trans('Expenditure items');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_partida as t";
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<input type="text" name="code" value="'.GETPOST('code').'" autofocus>'.'</td>';
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if ($action == 'edit' && $id == $obj->rowid)
		{
			...
			if (! empty($arrayfields['t.code_father']['checked']))
			{
				print '<td>'.'<input type="text" name="code_father" value="'.$obj->code_father.'">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.type']['checked']))
			{
...
if (empty($reshook))
{
	...
	{
		print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/orgman/partida/import.php'.'?action=create">'.$langs->trans("Import").'</a></div>'."\n";
