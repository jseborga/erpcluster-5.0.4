...
//registro nuevo
$nLoop = 1;
$nLen = count($objcomm->lines);
$var = true;

foreach ($objcomm->lines AS $j => $objc)
{
  if (!$aProduct[$order_number][$objc->fk_product])
    {
      $var = !$var;
      print "<tr $bc[$var]>";
      print '<td>';
	...
	print $order_number;
      print '</td>';
      print '<td>';
      print dol_print_date($objcomm->date_commande,'day');
      print '</td>';
      ...
      print '<td>';
      print $objc->libelle;
      print '<input type="hidden" name="description['.$objc->fk_product.']" value="'.$objc->libelle.'"';
      ...
      print '<td>';
      print '<input type="text" name="unit['.$objc->fk_product.']" value="'.(empty($unit[$objc->fk_product])?'Pieza':$unit[$objc->fk_product]).'" size="10" required>';
      print '</td>';
      
      print '<td>';
      print '<input type="number" class="len50" min="0.1" step="any" name="quant['.$objc->fk_product.']" value="">';
      print '</td>';
    }
  if ($nLoop != $nLen)
    print '</tr>';
