 ...
 */
class InterfaceWsFractal extends DolibarrTriggers
{
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		switch ($action) {
			...
			{
				//registramos las modificaciones de precios
				...
				$objProductlog = new Productlog($this->db);
				$objProductlog->fk_product = $object->id;
				$objProductlog->description = $action;
				$objProductlog->status_product = 1;
				//3 = es critico se debe actualizar
				$objProductlog->fk_user_create = $user->id;
				$objProductlog->fk_user_mod = $user->id;
				$objProductlog->datec = dol_now();
				$objProductlog->datem = dol_now();
				$objProductlog->tms = dol_now();
				$objProductlog->status = 1;
				$reslog = $objProductlog->create($user);
				if ($reslog<=0)
				{
					$errorlog++;
					setEventMessages($objProductlog->error,$objProductlog->errors,'errors');
				}
				if ($errorlog)
					return -1;
				else
			...
			//registramos las modificaciones de precios
				...
				$objProductlog->fk_product = $object->id;
				$objProductlog->description = $action;
				$objProductlog->status_product = 3;
				//3 = es critico se debe actualizar
				$objProductlog->fk_user_create = $user->id;
				$objProductlog->fk_user_mod = $user->id;
				$objProductlog->datec = dol_now();
