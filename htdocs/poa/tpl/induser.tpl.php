...
$ret =dol_htmlentities($objuser->getFullName($langs));
$linkback = '<a href="'.$_SERVER['PHP_SELF'].'?search_login='.$objuser->id.'">'.$ret.'</a>';
dol_banner_tabpoa($objuser,'id',$linkback,1);

//print '<img class="img-circle" src="./img/user.png" alt="'.$obj->ref.'">';
print '</div><!-- /.widget-user-image -->';
//print '<h3 class="widget-user-username">'.'<a href="'.DOL_URL_ROOT.'/poa/poa/liste.php?search_login='.$objuser->id.'" title="'.$objuser->lastname.' '.$objuser->firstname.'">'.$objuser->login.'</a>'.'</h3>';
...
print '</a></li>';
//print '<li><a href="#">'.$langs->trans('Tasks').' <span class="pull-right badge bg-aqua">'.$aCounttask[$obj->id].'</span>&nbsp;<span class="pull-right badge bg-aqua">'.price($aSumtask[$obj->id]).'</span></a></li>';
//print '<li><a href="#">'.$langs->trans('Completedtasks').' <span class="pull-right badge bg-green">'.$aCounttaskfin[$obj->id].'</span>&nbsp;<span class="pull-right badge bg-green">'.price($aSumtaskfin[$obj->id]).'</span></a></li>';
