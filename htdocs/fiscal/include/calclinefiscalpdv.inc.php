...
//archivo oficial desde 01/01/2017 TODO DENTRO DE FISCAL
...
{
	$code_facture = $conf->global->FISCAL_CODE_FACTURE_SALES;
	$objtype = new Ctypefacture($db);
	$objtype->fetch(0,$code_facture);

	$filterstatic = " AND t.code_facture = '".trim($code_facture)."'";
	$restva = $tvadef->fetchAll('','',0,0,array(),'AND',$filterstatic,false);
