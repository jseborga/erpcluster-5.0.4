...
function select_wage_inf($selected='',$htmlname='wage_inf',$htmloption='',$maxlength=0,$showempty=0,$showLabel=0)
{
	global $conf,$langs;
	...
	$label=array();
	$i=1;
	$countryArray['A']['rowid'] = 'A';
	$countryArray['A']['label'] = $langs->trans('Salaries');
	$label['A'] = $countryArray[$i]['label'];
	$i++;
	$countryArray['B']['rowid'] = 'B';
	$countryArray['B']['label'] = $langs->trans('Antiquity');
	$label['B'] = $countryArray[$i]['label'];
	$i++;
	$countryArray['C']['rowid'] = 'C';
	$countryArray['C']['label'] = $langs->trans('Overtime');
	$label['C'] = $countryArray[$i]['label'];
	$i++;
	$countryArray['D']['rowid'] = 'D';
	$countryArray['D']['label'] = $langs->trans('Buy bonds');
	$label['D'] = $countryArray[$i]['label'];
	$i++;
	$countryArray['E']['rowid'] = 'E';
	$countryArray['E']['label'] = $langs->trans('Dominical');
	$label['E'] = $countryArray[$i]['label'];
	$i++;
	$countryArray['F']['rowid'] = 'F';
	$countryArray['F']['label'] = $langs->trans('Other bonds');
	$label['F'] = $countryArray[$i]['label'];
	$i++;
	$countryArray['H']['rowid'] = 'H';
	$countryArray['H']['label'] = $langs->trans('Liquido');
	$label['H'] = $countryArray[$i]['label'];
...
function print_select($selected='',$htmlname='status',$htmloption='',$maxlength=0,$showempty=0,$showLabel=0,$countryArray,$label)
{
	...
	foreach ((array) $countryArray as $row)
	{
...
function salary_prepare_head($object)
{
	...
	$head[$h][0] = dol_buildpath("/adherents/card.php?rowid=".$object->id,1);
	$head[$h][1] = $langs->trans("Fiche");
	$head[$h][2] = 'Ficha miembro';
	$h++;

	$head[$h][0] = dol_buildpath("/adherents/subscriptions.php?rowid=".$object->id,1);
...
function fetch_regions($fk_region='',$code_region='')
{
	global $conf,$langs,$db;

	$langs->load("salary");
	if (empty($fk_region) && empty($code_region)) return -1;
  ...
  //dol_syslog(get_class($this).'::form_prospect_level',LOG_DEBUG);
	$sql = "SELECT rowid, nom AS label, code_region AS code";
	$sql.= " FROM ".MAIN_DB_PREFIX."c_regions";
	$sql.= " WHERE active > 0";
	if (!empty($fk_region))
		$sql.= " AND fk_region = ".$fk_region;
...
function LibState($statut,$mode)
{
  ...
  elseif ($mode == 6) //payment_state
  {
  	if ($statut==0) return $langs->trans('StatusPendent').' '.img_picto($langs->trans('StatusPendent'),'statut0');
  	if ($statut==1) return $langs->trans('StatusPayment').' '.img_picto($langs->trans('StatusPayment'),'statut1');
  }
}
