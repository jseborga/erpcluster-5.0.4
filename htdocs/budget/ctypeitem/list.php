...
/**
 *   	\file       budget/ctypeitem_list.php
 *		\ingroup    budget
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-04-17 17:58
 */
...
//dol_include_once('/budget/class/ctypeitem.class.php');
...
$aStatus[1]=$langs->trans('Activated');
$aStatus[0]=$langs->trans('Disabled');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'budgetctypeitemlist';
...
$arrayfields=array(
	...
	't.code'=>array('label'=>$langs->trans("Fieldcode"), 'align'=>'align="left"', 'checked'=>1),
...
if (empty($reshook))
{
	...
	//$permtoread = $user->rights->ctypeitem->read;
	//$permtodelete = $user->rights->ctypeitem->delete;
	//$uploaddir = $conf->ctypeitem->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_type_item as t";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX.'user AS u ON t.fk_user_create = u.rowid';
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_item_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_user_create) $sql.= natural_search(array("t.fk_user_create","u.firstname","u.lastname"),$search_fk_user_create);
if ($search_fk_user_mod) $sql.= natural_search(array("t.fk_user_mod","u.lastname","u.firstname"),$search_fk_user_mod);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/ctypeitem/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.code']['checked'])) print_liste_field_titre($arrayfields['t.code']['label'],$_SERVER['PHP_SELF'],'t.code','',$params,$arrayfields['t.code']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.code']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.code']['align'].'><input type="text" class="flat" name="search_code" value="'.$search_code.'" size="10"></td>';
...
print '<div class="tabsAction">';
	//print '<a class="butAction" href="' . dol_buildpath('/budget/variable/card.php',1).'?id='.$id.'&action=create">' . $langs->trans('Create') . '</a>';
print '<a class="butAction" href="' . dol_buildpath('/budget/ctypeitem/card.php',1).'?action=create">' . $langs->trans('Create') . '</a>';
print '</div>';
