...
/**
 *  \file			./request/core/modules/modules_request.php
 *  \ingroup		Request
 *  \brief			Fichier contenant la classe mere de generation des commandes en PDF
 *  				et la classe mere de numerotation des request
 */
 ...
 */
abstract class ModelePDFAssistance extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleAssistance extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleNumRefAssistance
{
	var $error='';
 ...
 */
function assistance_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false)
{
	...
	foreach($dirmodels as $reldir)
	{
    	foreach(array('doc','pdf') as $prefix)
    	{
	        ...
	        $file=dol_buildpath($reldir."assistance/core/modules/doc/".$file,0);
    		if (file_exists($file))
    		{
    			$filefound=1;
	...
	if ($filefound)
	{
		...
		if ($obj->write_file($object, $outputlangs, $srctemplatepath, $hidedetails, $hidedesc, $hideref, $hookmanager) > 0)
		{
			...
			$result=$interface->run_triggers('ASSISTANCE_BUILDDOC',$object,$user,$langs,$conf);
		...
		{
			$outputlangs->charset_output=$sav_charset_output;
			dol_print_error($db,"assistance_pdf_create Error: ".$obj->error);
			return -1;
		}
...
/*
function assistance_pdf_create(DoliDB $db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0)
{
