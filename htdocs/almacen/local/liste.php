...
$sql  = "SELECT e.rowid, e.label as ref, e.statut, e.lieu, e.address, e.zip AS cp, e.town AS ville, e.fk_pays, ee.fk_entrepot_father";
$sql.= " FROM ".MAIN_DB_PREFIX."entrepot as e";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."entrepot_relation as ee ";
$sql.= " ON e.rowid = ee.rowid ";
...
if ($result)
  {
    ...
    print_liste_field_titre($langs->trans("localSuperior"),"liste.php", "ee.fk_entrepot_father","","","",$sortfield,$sortorder);
    print_liste_field_titre($langs->trans("Status"),"liste.php", "e.statut",'','','align="right"',$sortfield,$sortorder);
    print "</tr>\n";
    if ($num)
      {
	$entrepot=new Entrepot($db);
	$urqentrepot = new Entrepotrelationext($db);
	$var=True;
	while ($i < min($num,$limit))
	  {
	    $entrepotPadre=new Entrepot($db);
	    $objp = $db->fetch_object($result);
	    $entrepotPadre->fetch($objp->fk_entrepot_father);
	    $var=!$var;
	    print "<tr $bc[$var]>";
	    print '<td><a href="fiche.php?id='.$objp->rowid.'">'.img_object($langs->trans("ShowWarehouse"),'stock').' '.$objp->ref.'</a></td>';
	    print '<td>'.$objp->lieu.'</td>';
	    if ($entrepotPadre->id == $objp->fk_entrepot_father)
	      print '<td>'.$entrepotPadre->libelle.'</td>';
	    else
	      print '<td>&nbsp;</td>';
