...
if (empty($reshook))
{
	if ($cancel) 
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/partida/listgas.php',1);
	...
	$objectlabel='Partida';
	$permtoread = $user->rights->poa->teacher->read;
	$permtodelete = $user->rights->poa->teacher->del;
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/poa/partida/card.php?id='.$id);
	exit;
}
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<select name="fk_father">'.$options.'</select>'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		{		
			...
			if (! empty($arrayfields['t.fk_father']['checked'])) 
			{
				print '<td>'.$aPartida[$obj->fk_father].'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.fk_user_create']['checked'])) 
			{
