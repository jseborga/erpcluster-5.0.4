...
/**
 *  \file			./fabrication/core/modules/modules_fabrication.php
 *  \ingroup		fabrication
 *  \brief			Fichier contenant la classe mere de generation des commandes en PDF
 *  				et la classe mere de numerotation des fabrication
 */
 ...
 */
abstract class ModelePDFFabrications extends CommonDocGenerator
{
	var $error='';
 ...
 *	Classe mere des modeles de commandes
 */
abstract class ModeleFabrication extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleNumRefFabrications
{
	var $error='';
 ...
 */
function fabrication_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false)
{
	...
	{
	    if (! empty($conf->global->FABRICATION_ADDON_PDF))
	    {
	        $modele = $conf->global->FABRICATION_ADDON_PDF;
	    }
	    else
	    {
	        $modele = 'fabrication';
	...
	foreach($dirmodels as $reldir)
	{
	  ...
	  $file = $prefix."_".$modele.".modules.php";
	        ...
	        $file=dol_buildpath($reldir."fabrication/core/modules/doc/".$file,0);
    		if (file_exists($file))
    		{
    			$filefound=1;
