 ...
 *					Initialy built by build_class_from_table on 2017-05-05 15:32
 */
...
$search_active='';
    ...
    $permtoread = $user->rights->ctypefacture->read;
    $permtodelete = $user->rights->ctypefacture->delete;
    $uploaddir = $conf->ctypefacture->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_facture_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/ctypefacture/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.type_fact']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_type_fact" value="'.$search_type_fact.'" size="10"></td>';
if (! empty($arrayfields['t.type_value']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_type_value" value="'.$search_type_value.'" size="10"></td>';
...
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
    ...
    print $formfile->showdocuments('massfilesarea_fiscal','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
