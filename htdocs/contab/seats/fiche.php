...
$type_seat_det = GETPOST('type_seat_det');
...
if ($action == 'adddet' && $user->rights->contab->seatma->write)
{
	...
	$objdet->type_seat = GETPOST("type_seat_det");
...
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->contab->seatma->del)
{
	$error = 0;
	//borrando los flags
	$objseatflag = new Contabseatflagext($db);
	...
	if (empty($error))
	{
		...
		if ($result > 0)
		{
			header("Location: ".DOL_URL_ROOT.'/contab/seats/list.php');
			exit;
		}
		else
		{
...
if ($res >0)
{
	...
	foreach ($lines AS $j => $line)
	{
		$selected = '';
		if (
			(GETPOST('codtr')?GETPOST('codtr'):$object->codtr) == $line->ref)
		{
			$selected = ' selected';
			$type_seat = $line->type_seat;
		}
		$options.= '<option value="'.$line->ref.'" '.$selected.'>'.$line->ref.' '.$line->label.'</option>';
	}
}
...
if ($action == 'create' && $user->rights->contab->seatma->write)
{
  ...
  // yemer
	...
	print '<tr>';
	print '<td width="25%" class="fieldrequired">'.$langs->trans('Typetransaction').'</td><td colspan="3">';
	print '<select id="codtr" name="codtr">'.$options.'</select>';
	print '</td>';
	print '</tr>';
	...
	print '<input id="history" type="text" value="'.(GETPOST('history')?GETPOST('history'):$object->history).'" name="history" size="20" maxlength="40">';
	print '</td></tr>';
...
else
{
	if ($id)
	{
		...
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			//recuperando los registros hijos del asiento
			...
			if ($object->status == 0)
			{
				...
				if ($conf->use_javascript_ajax)
				{
					...
					print '$(document).ready(function () {
						...
						$("#selecttype_seat_det").change(function() {
							document.formc.action.value="";
							document.formc.submit();
						});
				...
				print '<label for="" class="col-lg-2 control-label">'.$langs->trans('Typeofregister').'</label>';
				print '<div class="col-lg-10">';
				print select_seat((GETPOST('type_seat_det')?GETPOST('type_seat_det'):$objdet->type_seat),'type_seat_det','','',1);
				print '</div>';
				print "</div>\n";
				...
				if ($type_seat_det == 1 ||$type_seat_det == 3 )
				{
					print '<div class="form-group">';
				...
				if ($type_seat_det == 2 ||$type_seat_det == 3 )
				{
					print '<div class="form-group">';
			...
			if (! empty($conf->contab->enabled) && $user->rights->contab->seatma->write)
			{
				...
				$aAccountgroup = array();
				$aAccountgroupid = array();
				$aAccountgroupdet = array();
				$aAccountdet = array();
				...
				if ($num >0)
				{
					...
					{
						if (!empty($objc->debit_account)) $objc->type_seat = 1;
						if (!empty($objc->credit_account)) $objc->type_seat = 2;
						if (!empty($objc->debit_account) && !empty($objc->credit_account)) $objc->type_seat = 3;
						if ($objc->debit_account) $aAccountgroupid[$objc->type_seat][$objc->debit_account] = $objc->id;
						if ($objc->credit_account) $aAccountgroupid[$objc->type_seat][$objc->credit_account] = $objc->id;
						if ($objc->debit_account) $aAccountgroupdet[$objc->type_seat][$objc->debit_account] = $objc->debit_detail;
						if ($objc->credit_account) $aAccountgroupdet[$objc->type_seat][$objc->credit_account] = $objc->credit_detail;
						if ($objc->type_seat == 1)
						{
				...
				foreach ($aAccountgroup AS $type_seat => $aData)
				{
					...
					foreach ($aData AS $account => $aRow)
					{
						...
						{
							$idreg = $aAccountgroupid[$type_seat][$account];
							print '<a href="fiche.php?id='.$object->id.'&aid='.$idreg.'&action=editline">'.img_object($langs->trans("Showaccountseat"),"account")." ".$idreg.'</a>';
						}
						print '</td>';
						...
						$aLineas [$i]['detalleDebito'] = $aAccountgroupdet[$type_seat][$account];
						//print "<td>".$objc->credit_detail."</td>\n";
						$aLineas [$i]['detalleCredito'] = $aAccountgroupdet[$type_seat][$account];
						print '<td></td>';
						if ($type_seat == 1)
						{
						...
						if (count($aAux)<=0)
						{
							...
							{
								$idreg = $aAccountgroupid[$type_seat][$account];
								print '<a href="fiche.php?id='.$object->id.'&aid='.$idreg.'&action=deleteline">'.img_picto($langs->trans("Delete"),"delete")."</a></td>\n";
							}
							print '</td>';
						}
						print '</tr>';
	 	...
	 	if (($action == 'edit' || $action == 're-edit') && 1)
	 	{
	 		...
	 		if ($conf->use_javascript_ajax)
	 		{
	 			...
	 			print '$(document).ready(function () {
	 				$("#codtr").change(function() {
	 					document.formsocnn.action.value="edit";
	 					document.formsocnn.submit();
	 				});
	 			})';
	 			print '</script>'."\n";
	 		}
	 		...
	 		print '<form name="formsocnn" action="'.$_SERVER['PHP_SELF'].'" method="POST">';
	 		print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
	 		...
	 		print '<table class="border" width="100%">';
	  		...
	  		// date seat
	 		print '<tr><td class="fieldrequired">'.$langs->trans('Dateseat').'</td><td colspan="2">';
	 		$form->select_date(($date_seat?$date_seat:$object->date_seat),'date_seat','','','',"crea_seat",1,1);
	 		print '</td></tr>';
	  		//currency
	  		$currency="";
	 		if ($object->currency=='BOB')
	 		$currency=2;
	 		if ($object->currency=='USD')
	 		$currency=1;
	 		print '<tr><td class="fieldrequired">'.$langs->trans('Currency').'</td><td colspan="2">';
	 		//print select_currency((GETPOST('currency')?GETPOST('currency'):$object->currency),'currency','','',1);
	 		print select_currency((GETPOST('currency')?GETPOST('currency'):$currency),'currency','','',1);
	 		print '</td></tr>';
	 		// Tipo de transaccion
	 		print '<tr>';
	 		print '<td width="25%" class="fieldrequired">'.$langs->trans('Typetransaction').'</td><td colspan="3">';
			...
			//type seat
	 		print '<tr><td class="fieldrequired">'.$langs->trans('Typeseat').'</td><td colspan="2">';
	 		print select_type_seat((GETPOST('type_seat')?GETPOST('type_seat'):$object->type_seat),'type_seat','','',1);
	 		print '</td></tr>';
