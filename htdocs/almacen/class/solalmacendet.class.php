...
/**
 * \file    almacen/solalmacendet.class.php
 * \ingroup almacen
...
class Solalmacendet extends CommonObject
{
	 ...
	 */
	public $element = 'solalmacendet';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'sol_almacendet';

	/**
	 * @var SolalmacendetLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_almacen)) {
			 $this->fk_almacen = trim($this->fk_almacen);
		}
		...
		if (isset($this->qty_livree)) {
			 $this->qty_livree = trim($this->qty_livree);
		}
		if (isset($this->price)) {
	 ...
	 */
	public function fetch($id, $fk_product=0, $fk_almacen=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		$sql.= ' WHERE 1 = 1';
		if ($fk_product>0 && $fk_almacen >0) {
			$sql .= ' AND t.fk_product = ' . $fk_product;
			$sql .= ' AND t.fk_almacen = ' . $fk_almacen;
		} else {
			$sql .= ' AND t.rowid = ' . $id;
		}
		$resql = $this->db->query($sql);
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND', $filterstatic='', $lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new SolalmacendetLine($this->db);

				$line->id = $obj->rowid;

				$line->fk_almacen = $obj->fk_almacen;
				...
				$line->qty = $obj->qty;
				$line->qty_livree = $obj->qty_livree;
				$line->price = $obj->price;
				$line->price_peps = $obj->price_peps;
				$line->price_ueps = $obj->price_ueps;
				$line->date_shipping = $this->db->jdate($obj->date_shipping);
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' date_shipping = '.(! isset($this->date_shipping) || dol_strlen($this->date_shipping) != 0 ? "'".$this->db->idate($this->date_shipping)."'" : 'null').',';
...
/**
 * Class SolalmacendetLine
 */
class SolalmacendetLine extends CommonObjectLine
{
	/**
	 * @var int ID
	 */
