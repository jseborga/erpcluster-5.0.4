...
/**
 *   	\file       monprojet/projetpaiement_list.php
 *		\ingroup    monprojet
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-02-16 14:19
 */
...
dol_include_once('/monprojet/class/projetpaiement.class.php');
...
$search_fk_fourn_facture=GETPOST('search_fk_fourn_facture','int');
...
$hookmanager->initHooks(array('projetpaiementlist'));
...
't.fk_fourn_facture'=>array('label'=>$langs->trans("Fieldfk_fourn_facture"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."projet_paiement as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."projet_paiement_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_fourn_facture) $sql.= natural_search("fk_fourn_facture",$search_fk_fourn_facture);
...
if ($search_fk_fourn_facture != '') $params.= '&amp;search_fk_fourn_facture='.urlencode($search_fk_fourn_facture);
...
if (! empty($arrayfields['t.fk_fourn_facture']['checked'])) print_liste_field_titre($arrayfields['t.fk_fourn_facture']['label'],$_SERVER['PHP_SELF'],'t.fk_fourn_facture','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_fourn_facture']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_fourn_facture" value="'.$search_fk_fourn_facture.'" size="10"></td>';
