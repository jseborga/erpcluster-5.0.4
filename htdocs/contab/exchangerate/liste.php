...
include_once(DOL_DOCUMENT_ROOT.'/contab/lib/verificatc.lib.php');
...
if ($action == 'list')
  {
    ...
    foreach((array) $currency_array AS $i => $data)
      {
	print_liste_field_titre($langs->trans($data['ref']),$_SERVER['PHP_SELF'],'t.currency'.$pos,'',$param,'align="right"',$sortfield,$sortorder);
	$pos++;
      }
    print '</tr>';
    ...
    if ($resql)
        ...
        if ($num)
	  {
		...
		$var=!$var;
		  ...
		  {
		    print "<tr $bc[$var]>";
		    print '<td><a href="fiche.php?id='.$obj->id.'">'.img_object($langs->trans("Exchangerate"),'tc').' '.$obj->date_ind.'</a></td>';
		    $j = 1;
		    foreach((array) $currency_array AS $k)
		      {
			$currency = 'currency'.$j;
