...
class Purchaserequestdet extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_poa_partida_pre_det)) {
			 $this->fk_poa_partida_pre_det = trim($this->fk_poa_partida_pre_det);
		}
		...
		$sql .= ' '.(! isset($this->fk_poa_partida_pre_det)?'NULL':$this->fk_poa_partida_pre_det).',';
	...
	public function fetch($id, $ref = null, $fk=0)
	{
		...
		if (null !== $ref && $fk > 0) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
			$sql .= ' AND t.fk_purchase_request = ' . $fk;
		} else {
			$sql .= ' AND t.rowid = ' . $id;
		}
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fk_poa_partida_pre_det = $obj->fk_poa_partida_pre_det;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND', $filterstatic='', $lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->fk_poa_partida_pre_det = $obj->fk_poa_partida_pre_det;
				...
				if ($line->fk_product > 0)
				{
					...
					$resp = $objProduct->fetch($line->fk_product);
					if ($resp>0)
					{
						$line->product_label = $objProduct->label;
						$line->product_ref = $objProduct->ref;
					}
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_poa_partida_pre_det = '.(isset($this->fk_poa_partida_pre_det)?$this->fk_poa_partida_pre_det:"null").',';
