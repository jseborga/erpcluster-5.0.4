...
/**
 *	\file       htdocs/salary/departament/fiche.php
 *	\ingroup    Departaments
 *	\brief      Page fiche salary departament
 */
...
require_once DOL_DOCUMENT_ROOT.'/almacen/class/pdepartamentext.class.php';
...
if (!$user->rights->almacen->dep->lire) accessforbidden();
...
// Add
if ($action == 'add' && $user->rights->almacen->dep->write)
{
...
// Delete period
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->almacen->dep->del)
{
...
if ($action == 'create' && $user->rights->almacen->dep->write)
{
...
else
{
	...
	// Affichage fiche
	  ...
	  //$head = fabrication_prepare_head($object);
			...
			dol_fiche_head($head, 'card', $langs->trans("Departament"), 0, 'salary');
	...
	// Confirmation de la validation
	  ...
	  //respon
			...
			if ($action == '')
			{
				if ($user->rights->almacen->dep->write)
				...
				if ($user->rights->almacen->dep->del)
