...
class pdf_groupasset extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function __construct($db)
	{
		...
		$titulo = "";
		$this->db = $db;
		$this->name = "groupasset";
		$this->description = $langs->trans('PDFGroupasset');
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aGral as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-3);
						...
						$pdf->MultiCell($this->posxgrc-$this->posxnro-1, 1, $line['grupo'],0, 'L',0);
						$nexYaux = $pdf->GetY();

						$pdf->SetXY($this->posxgrc, $curY);
						$pdf->MultiCell($this->posxcan-$this->posxgrc-1, 1, $line['cantidad'],0, 'C',0);

						$pdf->SetXY($this->posxcan, $curY);
						$pdf->MultiCell($this->posxvut-$this->posxcan-1, 1, $line['util'],0, 'C',0);

						$pdf->SetXY($this->posxvut, $curY);
						$pdf->MultiCell($this->posxchi-$this->posxvut-1, 1, price(price2num($line['cHistorico'],'MT')), 0, 'R',0);

						//$pdf->SetXY($this->posxchi, $curY);
						//$pdf->MultiCell($this->posxcai-$this->posxchi-1, 1,price(price2num($line['cActInicial'],'MT')), 0, 'R',0);

						//$pdf->SetXY($this->posxcai, $curY);
						//$pdf->MultiCell($this->posxdat-$this->posxcai-1, 1, price(price2num($line['dAcuTotal'],'MT')), 0, 'R',0);


						$pdf->SetXY($this->posxchi, $curY);
						$pdf->MultiCell($this->posxvalact-$this->posxchi-1, 1, price(price2num($line['cValorac'],'MT')), 0, 'R',0);

						$pdf->SetXY($this->posxvalact, $curY);
						$pdf->MultiCell($this->posxdepacum-$this->posxvalact-1, 1, price(price2num($line['cDepreacuml'],'MT')), 0, 'R',0);

						$pdf->SetXY($this->posxdepacum, $curY);
						$pdf->MultiCell($this->posxfin-$this->posxdepacum-1, 1, price(price2num($line['saldo'],'MT')), 0, 'R',0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		*/
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxgrc-$this->posxnro-1,2, $outputlangs->transnoentities("Accountinggroup"),'','C');
			$pdf->line($this->posxgrc-1, $tab_top, $this->posxgrc-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxgrc-1, $tab_top+3);
			$pdf->MultiCell($this->posxcan-$this->posxgrc-1,2, $outputlangs->transnoentities("Quantity"),'','C');
			$pdf->line($this->posxcan-1, $tab_top, $this->posxcan-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxcan-1, $tab_top+3);
			$pdf->MultiCell($this->posxvut-$this->posxcan-1,2, $outputlangs->transnoentities("Usefullife"),'','C');
			$pdf->line($this->posxvut-1, $tab_top, $this->posxvut-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxvut-1, $tab_top+2);
			$pdf->MultiCell($this->posxchi-$this->posxvut-1,2, html_entity_decode($outputlangs->transnoentities("Historicalcost")),'','C');
			$pdf->line($this->posxchi-1, $tab_top, $this->posxchi-1, $tab_top + $tab_height);

			//$pdf->SetXY($this->posxchi-1, $tab_top+2);
			//$pdf->MultiCell($this->posxcai-$this->posxchi-1,2, $outputlangs->transnoentities("Initialcurrentcost"),'','C');
			//$pdf->line($this->posxcai-1, $tab_top, $this->posxcai-1, $tab_top + $tab_height);

			//$pdf->SetXY($this->posxcai-1, $tab_top+2);
			//$pdf->MultiCell($this->posxdat-$this->posxcai-1,2, $outputlangs->transnoentities("Accumulatedtotal"),'','C');
			//$pdf->line($this->posxvalact-1, $tab_top, $this->posxvalact-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxchi-1, $tab_top+2);
			$pdf->MultiCell($this->posxvalact-$this->posxchi-1,2, $outputlangs->transnoentities("Costo Act."),'','C');
			$pdf->line($this->posxvalact-1, $tab_top, $this->posxvalact-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxvalact-1, $tab_top+2);
			$pdf->MultiCell($this->posxdepacum-$this->posxvalact-1,2, $outputlangs->transnoentities("DepreciationAcum"),'','C');
			$pdf->line($this->posxdepacum-1, $tab_top, $this->posxdepacum-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxdepacum-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxdepacum-1,2, $outputlangs->transnoentities("Balance"),'','C');
			//$pdf->line($this->posxfin-1, $tab_top, $this->posxfin-1, $tab_top + $tab_height);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		if($object->statut==0 && (! empty($conf->global->ASSETS_DRAFT_WATERMARK)) )
		{
			pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',$conf->global->ASSETS_DRAFT_WATERMARK);
		}
		...
		$title=$outputlangs->transnoentities("Summarybygroup");
		...
		$pdf->SetFont('','B',$default_font_size+1);
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Datefin")." : ". dol_print_date($aExtras['date_ini'],'day'), '', 'R');
