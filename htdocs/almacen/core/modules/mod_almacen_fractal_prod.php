 ...
 */
class mod_almacen_fractal_prod extends ModeleNumRefAlmacen
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefixinvoice='MALM';
	var $prefixcreditnote='SV';
	var $error='';
	var $nom='fractalproduct';
	...
	function info()
	{
		...
		return $langs->trans('No funcional',$this->prefixinvoice,$this->prefixcreditnote);
	}
	...
	function getExample()
	{
		return $this->prefixinvoice."397-00001";
	}
	...
	function canBeActivated()
	{
		...
		$sql.= " INNER JOIN ".MAIN_DB_PREFIX."product_extrafields AS e ON p.rowid = e.fk_object ";
		$sql.= " WHERE ref LIKE '".$this->prefixinvoice.$this->partida."-%'";
		$sql.= " AND e.partida = '".$this->partida."'";
		...
		$posindice=10;
		$sql = "SELECT MAX(SUBSTRING(ref FROM ".$posindice.")) as max";	// This is standard SQL
		$sql.= " FROM ".MAIN_DB_PREFIX."stock_mouvement_add";
	...
	* @param   string		$mode       'next' for next value or 'last' for last value
	...
	function getNextValue($objsoc,$facture,$mode='next')
	{
		...
		$sql.= " AND entity = ".$conf->entity;
		$resql=$db->query($sql);
		dol_syslog("mod_almacen_ubuntubo_sol::getNextValue sql=".$sql);
		if ($resql)
		{
		  ...
		  {
		    dol_syslog("mod_almacen_ubuntubo_sol::getNextValue sql=".$sql, LOG_ERR);
		    return -1;
		  }
		...
		if ($mode == 'last')
		...
		else if ($mode == 'next')
		  {
		    ...
		    dol_syslog("mod_almacen_ubuntubo_sol::getNextValue return ".$prefix.$yymm."-".$num);
