...
/**
 *   	\file       finint/requestcashdet_list.php
 *		\ingroup    finint
...
$search_fk_request=GETPOST('search_fk_request','int');
...
$hookmanager->initHooks(array('requestcashdetlist'));
...
't.fk_request'=>array('label'=>$langs->trans("Fieldfk_request"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."request_cash_det as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."request_cash_det_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_request) $sql.= natural_search("fk_request",$search_fk_request);
...
if ($search_fk_request != '') $params.= '&amp;search_fk_request='.urlencode($search_fk_request);
...
if ($search_status != '') $params.= '&amp;search_status='.urlencode($search_status);
    ...
    // 
if (! empty($arrayfields['t.fk_request']['checked'])) print_liste_field_titre($arrayfields['t.fk_request']['label'],$_SERVER['PHP_SELF'],'t.fk_request','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.status']['checked'])) print_liste_field_titre($arrayfields['t.status']['label'],$_SERVER['PHP_SELF'],'t.status','',$params,'',$sortfield,$sortorder);
	...
	print '<tr class="liste_titre">';
	// 
if (! empty($arrayfields['t.fk_request']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_request" value="'.$search_fk_request.'" size="10"></td>';
