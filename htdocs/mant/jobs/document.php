...
//$result=restrictedArea($user,'mant',$fieldvalue,'mant&mant','','',$fieldtype);
...
if ($id > 0 || ! empty($ref))
{
	...
	if (! empty($conf->mant->enabled)) $upload_dir = $conf->mant->multidir_output[$object->entity].'/'.get_exdir(0, 0, 0, 0, $object, 'mant').dol_sanitizeFileName($object->ref);

	if (! empty($conf->global->MANT_USE_OLD_PATH_FOR_PHOTO))    // For backward compatiblity, we scan also old dirs
	{
		if (! empty($conf->mant->enabled)) $upload_dirold = $conf->mant->multidir_output[$object->entity].'/'.substr(substr("000".$object->id, -2),1,1).'/'.substr(substr("000".$object->id, -2),0,1).'/'.$object->id."/photos";
	}
}
$modulepart='mant';
...
if (empty($reshook))
{
	...
	if (!empty($conf->global->MANT_PDF_MERGE_PROPAL))
	{
	...
	if (!isset($_SESSION['dol_events']['errors']) && isset($_SESSION['dol_events']['mesgs']))
	{
		$emailto = $object->email;
		$sendUser = $user->lastname.' '.$user->firstname;
		if (empty($object->email))
		{
			//buscamos al del solicitante
			$res = $objAdherent->fetch($object->fk_member);
			if ($res>0) $emailto = $objAdherent->email;
		}
		...
		if ($emailto)
		{
		  ...
		  //$arr_mime[] = 'aplication/rtf';
			...
			$tmpsujet = $langs->trans('Uploadoffileinorderofwork');
			$sendto   = $emailto;
			$email_from = $conf->global->MAIN_MAIL_EMAIL_FROM;
			$object->sendUser = $sendUser;
			$url = DOL_URL_ROOT;
			$tmpbody = htmlsendemailfile($id,$url);
			$msgishtml = 1;
...
$title = $langs->trans('Mant')." ". $shortlabel ." - ".$langs->trans('Documents');
$helpurl='EN:Module_Mant|FR:Module_Mant|ES:M&oacute;dulo_Mantenimiento';
...
if ($object->id)
{
	...
	$permission = $user->rights->mant->up->write;
	$param = '&id=' . $object->id;
	...
	if (!empty($conf->global->PRODUIT_PDF_MERGE_PROPAL))
	{
		...
		if (! empty($conf->global->MANT_USE_OLD_PATH_FOR_PHOTO))
