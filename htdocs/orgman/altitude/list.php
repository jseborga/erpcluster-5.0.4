...
/**
 *   	\file       orgman/caltitude_list.php
 *		\ingroup    orgman
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmancaltitudelist';
...
if (empty($reshook))
{
	...
	// Mass actions
	$objectclass='Caltitude';
	$objectlabel='Caltitude';
	$permtoread = $user->rights->orgman->alt->read;
	$permtodelete = $user->rights->orgman->alt->del;
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_altitude as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_altitude_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/caltitude/card.php?id='.$id);
	exit;
}
...
if (empty($reshook))
{
	...
	{
		print '&nbsp;<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/orgman/altitude/card.php?action=create">'.$langs->trans("New").'</a></div>'."\n";
	}
}
print '</div>'."\n";
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
	...
	$diroutputmassaction = $conf->orgman->dir_output . '/' . dol_sanitizeFileName('caltitude');
	$filedir=$diroutputmassaction;
	$genallowed=$user->rights->orgman->alt->read;
	$delallowed=$user->rights->orgman->alt->del;

	print $formfile->showdocuments('orgman','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
