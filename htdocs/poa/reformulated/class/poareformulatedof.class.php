...
/**
 *  \file       dev/skeletons/poareformulatedof.class.php
 ...
 */
class Poareformulatedof extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	var $element='poa_reformulated_of';			//!< Id that identify managed objects
	var $table_element='poa_reformulated_of';		//!< Name of table without prefix where object is stored
    ...
    function create($user, $notrigger=0)
    {
        ...
        // Insert request
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."poa_reformulated_of(";
		...
		if (! $error)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."poa_reformulated_of");
    ...
    function fetch($id)
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."poa_reformulated_of as t";
    ...
    function update($user=0, $notrigger=0)
    {
        ...
        // Update request
        $sql = "UPDATE ".MAIN_DB_PREFIX."poa_reformulated_of SET";
	...
	function delete($user, $notrigger=0)
	{
		...
		if (! $error)
		{
    		$sql = "DELETE FROM ".MAIN_DB_PREFIX."poa_reformulated_of";
    		$sql.= " WHERE rowid=".$this->id;
	...
	function createFromClone($fromid)
	{
		...
		$object=new Poareformulatedof($this->db);
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
	function getlist($fk_poa_reformulated,$idof=0)
	{
	  global $langs;
	  $sql = "SELECT";
	  $sql.= " t.rowid,";
	...
	$this->array = array();
        if ($resql)
	  {
		...
		while ($i < $num)
		  {
		    $obj = $this->db->fetch_object($resql);
		    $objnew = new Poareformulatedof($this->db);
		    $objnew->id    = $obj->rowid;
	...
	function get_sumaref($aReform)
	{
	  ...
	  $objreformto = new Poareformulatedto($this->db);
	  foreach((array) $aReform AS $id)
	      ...
	      foreach((array) $this->array AS $i => $objnew)
		{
		  ...
		  //buscamos los dependientes
		  $objreformto->getlist($i);
		  foreach((array) $objreformto->array AS $l => $objto)
		    {
		      $aTo[$objto->fk_structure][$objto->fk_poa_poa][$objto->partida]+= $objto->amount;
		      $aToone[$objto->fk_structure][$objto->fk_poa_poa][$objto->partida]= $objto->id;
		    }
		}
	    }
	  return array($aOf,$aTo,$aOfone,$aToone);
	}
}
?>
