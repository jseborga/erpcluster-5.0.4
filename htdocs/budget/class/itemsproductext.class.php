...

{
	...
	var $aSpreadparameter;
	var $aCel;
	var $aSpreadf;
	var $aFormuladet;
	var $aResources;
	var $aSpreadresources;
	var $linestmp;
	var $aParameter;

	//procedimiento de calculo unitario
	function procedure_calc($idr=0,$type_structure, $fk_region,$fk_sector,$rep=false,array $aResources=array(),array $aItemproduct=array())
	{
		global $conf,$langs;
		//global $objectdetadd;
		global $object;
		...
		foreach ((array) $linesstr AS $h => $lineh)
		{
			...
			if ($lPrint)
			{
					...
					foreach ((array) $linesi AS $i => $linei)
					{
						...
						if ($resfd>0)
						{
							...
							foreach((array) $objformdet->lines AS $j => $linej)
							{
								...
								{
									$formuladet.= '+('.$lineh->group_structure.')';
									//$resop = 0;
									//utilizamos $aChange
									...
									if ($type == 'pu_structure')
									{
										...
										if ($resbtr>0)
										{
											...
											foreach((array) $linesbtr AS $k => $linek)
											{
												...
												if ($resipr==1)
												{
													$objTmp = new ItemsproductLineext($this->db);
													$objTmp->fk_unit = $linek->fk_unit;
													...
													$linek->unit = $objTmp->getLabelOfUnit('short');

													$aResources[$linek->group_structure][$linek->ref]['ref']= $linek->ref;
													$aResources[$linek->group_structure][$linek->ref]['amount']= $objItemsproductregion->amount;
													$aResources[$linek->group_structure][$linek->ref]['label']= $linek->label;
													$aResources[$linek->group_structure][$linek->ref]['amount_noprod']= $objItemsproductregion->amount_noprod;
													$aResources[$linek->group_structure][$linek->ref]['unit']= $linek->unit;
													$aResources[$linek->group_structure][$linek->ref]['group_structure']= $linek->group_structure;
													$aResources[$linek->group_structure][$linek->ref]['formula']= $linek->formula;
													$aResources[$linek->group_structure][$linek->ref]['id']= $linek->id;
												}
												$aItemproduct[$linek->id] =$linek;
												...
												if (($lineh->fk_categorie == $code && $linek->group_structure == $lineh->group_structure) || $aCategorieGroup[$code] == $linek->group_structure)
												{
													...
													//$formuladetnew[$linej->ref_formula][$linej->id]= '+('.$linek->group_structure.')';
													//echo '<br>esoperator '.
													$operator = 'sum';
													//echo  ' para '.$lineh->group_structure.' conlinea '.$n;

													$aContenidover[$linsum]=(($linei->status_print||$linei->status_print_det)?1:0);
													...
													$aContenido[$linsum]['group']=$linek->group_structure;
													$aContenido[$linsum]['ref']=$linek->ref;
													//if ($linek->id == 4) print_r($aContenido);
													$linsum++;
												}
											}
												...
												foreach ($aContenido AS $linsum => $aValue)
												{
													...
													if ($aContenidover[$linsum])
													{
														...
														if ($linei->status_print_det)
														{
															...
															$aSubtotal[$linej->ref_formula][$n] = $n;
															//echo ' <hr>N '.$n.' formlinea '.$this->aCel[$idr][$n]['G'];
															if ($lAddtmp) $aTmpsum[$lineh->group_structure][$n]=$n;

															$html.= '<tr>';
															$html.= '<td>'.$aValue['A'].'</td>';

															$this->aSpreadresources[$idr][$n]['group'] = $aValue['group'];
															$this->aSpreadresources[$idr][$n]['ref'] = $aValue['ref'];
															$this->aSpreadresources[$idr][$n]['label'] = $aValue['A'];
															...
															$this->aSpreadformula[$idr][$nx]['formula'] = 'sum()';
															//$aSt[$lineh->ref][$linei->formula]+=$ntotal;
															//$linek->code_structure.' '.$code.' '.$linek->quant;
															$_SESSION['cItem'][$idr][$linei->formula]+= $aValue['G'];
															$_SESSION['cFila'][$idr][$linei->formula]='G'.$nx;
															$aTmp[$idr]='G'.$colini[$j].':G';
															$resop += $aValue['G'];
															$this->aLine[]=$myclass;
															$nx++;
									...
									$filafin=0;
									//print_r($aSubtotal);
									//echo ' <hr>reemplazay prepara mGroup '.$lineh->group_structure;

									//if($aSubtotal[$lineh->group_structure])
									//{
									//	foreach ((array) $aSubtotal[$lineh->group_structure] AS $fila)
									//	{
									//		if (empty($filaini)) $filaini = $fila;
									//		$filafin=$fila;
									//	}
									//	//vamos a cambiar la formula de la sumatoria
									//	//vamos a asignar los valores segun la estrucutra
									//	//echo '<hr>mgroup '.
									...
									//	for ($a = $nTmp; $a <= $nTmp; $a++)
									//	$newSum.= '+'.'G_'.$a.'_';
									//	//$aMgroup['('.$lineh->group_structure.')']=$newSum;
									...
									if ($objop->operator == '+')
									{
										...
										if ($type == 'parameter_calculation')
										{
											...
											if ($objParametercalculation->ref == $code)
											{
												...
												$this->aParameter[$linei->ref_structure][$linei->formula]=$objParametercalculation->type;
											}
											else
										...
										if ($type == 'valor')
										{
											$resop += (is_numeric($value)?$value:0);
											$value_post = $value;
											$formuladet.='+G_'.$n.'_';
										}
									}
						...
						//echo ' <hr>status_print |'.$linei->status_print.'|'.' conlinea  '.$n;
						if ($linei->status_print)
						{
							...
							if (!empty($this->aParameter[$linei->ref_structure][$linei->formula]))
							{
								$this->aSpreadparameter[$idr][$n]=$this->aParameter[$linei->ref_structure][$linei->formula];
							}
							//.' %';
							...
							{
								$newSum = '';
								for ($a = $colini[$j]; $a <= $nAnt; $a++)
								$newSum.= '+'.'G_'.$a.'_';
								//echo '<hr>acel  '.
								$this->aCel[$idr][$n]=$newSum;
								//echo ' en '.$n;
				...
				//print_r($aFormula);
				//echo ' <hr>ini '.$this->aCel[$idr][$n].' paralinea '.$n;
				foreach ($aFormula[$lineh->ref] AS $ja => $val1)
				{
					if ($aSubtotal[$ja])
					{
						//echo '<hr>ja '.$ja.' n '.$n;
						//echo '<hr>';
						//print_r($aSubtotal[$ja]);
						$filaini=0;
						$filafin=0;
						foreach ($aSubtotal[$ja] AS $jb)
						{
							if(empty($filaini)) $filaini = $jb;
							$filafin=$jb;
						}
						//echo '<hr> '.$filaini.' '.$filafin;
						$this->aCel[$idr][$n].='+SUM(G_'.$filaini.'_:G_'.$filafin.'_)';
						$aTotal[$lineh->ref][$ja]='+SUM(G_'.$filaini.'_:G_'.$filafin.'_)';
					}
					else
					{
						if (!empty($val1))
		...
		//print_r($aTotal);
		$fTotal = '';
		foreach ($aTotal AS $j => $aVal)
		{
			foreach ($aVal AS $f => $val)
			{
				if (substr($val,0,1)!='+')
					$fTotal.='+'.$val;
				else {
		...
		$this->aResources = $aResources;
		$this->aItemproduct =$aItemproduct;
		if($rep)
		{
