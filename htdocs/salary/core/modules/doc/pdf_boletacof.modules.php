 ...
 */
class pdf_boletacof extends ModelePDFSalary
{
	var $db;
	var $name;
	var $description;
  ...
  // Minimum version of PHP required by module
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "boleta de pago Cofadena";
	...
	*/
	function write_file(&$object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		global $objectAd,$objectU,$objectCh;
		if (! is_object($outputlangs)) $outputlangs=$langs;
		// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		if (! empty($conf->global->MAIN_USE_FPDF)) $outputlangs->charset_output='ISO-8859-1';

		$objectU = new Puserext($this->db);
		$objTypefol = new Ptypefolext($this->db);
		...
		$res = $objTypefol->fetch($fk_type_fol);
		...
		$_SESSION['aParamBoleta'] = array('fk_period'   => $fk_period,
			...
			'mes'         => $mes,
			'anio'        => $anio );
		if (!empty($object->fk_adherent) && $object->fk_adherent > 0)
		{
			$objectU->fetch_user(0,$object->fk_adherent);

			$idUser = $object->fk_adherent;
			$_SESSION['aPlanilla'][$object->fk_adherent] = array('id'         => $object->fk_adherent,
				'basic'      => $objectU->basic,
				'date_ini'   => $objectU->date_ini,
				'date_fin'   => $objectU->date_fin,
				'date_fin_p' => $object->date_fin,
				'date_ini_p' => $object->date_ini);
		}
		else
		{
			s_cargamie($object->fk_adherent,$fk_proces);
		}
		$aPlanilla = $_SESSION['aPlanilla'];
		$res =  $this->_boleta($object, $aPlanilla);
		$aReportpdfIng = $this->aReportpdfIng;
		$aReportpdfDes = $this->aReportpdfDes;
		$aHistoryref = $this->aHistoryref;
		...
		// $objectref = dol_sanitizeFileName($aParamBoleta['anio'].$aParamBoleta['mes']);
				...
				$dir = $conf->salary->dir_output.'/boleta/'.$object->anio;
				$file = $dir . "/" . $objectref.($objTypefol->ref?'_'.$objTypefol->ref:'').(!empty($proces)?'_'.$proces:'').($objectU->id>0?'_'.$objectU->docum:'') . ".pdf";
			}
			if (! file_exists($dir))
			{
				if (dol_mkdir($dir) < 0)
					...
					for ($j = 0; $j < 2; $j++)
					{
						...
						while ( $i< $may)
						{
							...
							if($i <count($aReportpdfDes[$idUser]))
							{
								...
								if ($type_coddes == 2) $nSumagas += $amountdes;
							}
	...
	**/
	...
	function _boleta(&$object, $aPlanilla)
	{

		$htmlRes = '';
		$this->aHistoryref = array();
		$this->aReportpdfDes = array();
		$this->aReportpdfIng = array();
		...
		foreach ((array) $aPlanilla AS $idUser => $dataUser)
		{
			...
			$objres = search_planilla($idUser,4,$object->id,$object->fk_proces,$object->fk_type_fol,'',$lPeriodClose);
			...
			$sql.= " p.fk_period = ".$object->id ." AND ";
			$sql.= " p.fk_proces = ".$object->fk_proces ." AND ";
			$sql.= " p.fk_type_fol = ".$object->fk_type_fol ." AND ";
			...
			if ($resql)
			{
				...
				if ($num)
				{
					...
					while ($i < $num)
					{
						...
						$this->aHistoryref[$idUser]['ref'] = $obj->ref;
						$this->aHistoryref[$idUser]['period_year'] = $obj->period_year;
						...
						if ( $obj->print == 1 && $obj->amount > 0)
						{
							...
							{
								$nSumaIng += $obj->amount;
								$this->aReportpdfIng[$idUser][]=array('detail'=>$obj->detail,'hours'=>$obj->hours,'amount'=>$obj->amount,'type_cod'=>1,'type_mov'=>$obj->type_mov,'cont'=>$i);

							}
							if ($obj->type_cod == 3)
							{
								$this->aReportpdfIng[$idUser][]=array('detail'=>$obj->detail,'hours'=>$obj->hours,'amount'=>$obj->amount,'type_cod'=>3,'type_mov'=>$obj->type_mov,'cont'=>$i);
							}
							if ($obj->type_cod == 2)
							{
								$nSumaEgr += $obj->amount;
								$this->aReportpdfDes[$idUser][]=array('detail'=>$obj->detail,'hours'=>$obj->hours,'amount'=>$obj->amount,'type_cod'=>2,'type_mov'=>$obj->type_mov,'cont'=>$i);
							}
						}
						$i++;
					}
