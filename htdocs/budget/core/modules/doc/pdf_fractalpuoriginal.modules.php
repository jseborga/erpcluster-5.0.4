...
class pdf_fractalpu extends ModelePDFPropales
{
		...
		if ($conf->budget->dir_output)
		{
			...
			if (file_exists($dir))
			{
	            ...
	            foreach ($lines AS $k => $aData)
	            {
	            	foreach ($aData AS $nom => $row)
	            	{
	            		...
	            		if (isset($imglinesize['width']) && isset($imglinesize['height']) && ($curY + $imglinesize['height']) > ($this->page_hauteur-($heightforfooter+$heightforfreetext+$heightforsignature+$heightforinfotot)))	
	            		{
	            			$pdf->AddPage('','',true);
	            			if (! empty($tplidx)) $pdf->useTemplate($tplidx);
	            			if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $this->_pagehead($pdf, $object, 0, $outputlangs,$societe);
	            		...
	            		if ($lines[$i]->format == 'group' ||$lines[$i]->format == 'partial'||$lines[$i]->format == 'total')
	            		{
	            		...
	            		$posx = $this->posxuni-$this->posxdes-0.8;
	            		if ($lines[$i]->format == 'partial'||$lines[$i]->format == 'total')
	            			$posx = $this->posxuni-$this->posxdes+2.8;
	            		$pdf->MultiCell($posx, 3, $label, 0, 'L');
					...
					// We suppose that a too long description or photo were moved completely on next page
	            		...
	            		elseif($nom == 'data')
	            		{
	            			foreach ($row AS $nReg => $value)
	            			{
	            				...
	            				if($nReg==2)
	            				{
	            					...
	            					$pdf->MultiCell($this->posxqty-$this->posxuni, 3, $value, 0, 'C');
	            				}

	            				if($nReg==3)
	            				{
	            					$pdf->SetXY($this->posxqty, $curY);
	            					$pdf->MultiCell($this->posxpor-$this->posxqty, 3, $value, 0, 'R');
	            				}
	            				...
	            				if($nReg==4)
	            				{
	            					...
	            					$pdf->MultiCell($this->posxnpri-$this->posxpor, 3, $value, 0, 'R');
	            				}
	            				...
	            				if($nReg==5)
	            				{
	            					...
	            					$pdf->MultiCell($this->posxpri-$this->posxnpri, 3, $value, 0, 'R');
	            				}

	            				if($nReg==6)
	            				{
	            					$pdf->SetXY($this->posxpri, $curY);
	            					$pdf->MultiCell($this->posxpar-$this->posxpri, 3, $value, 0, 'R');
	            				}
	            				...
	            				if($nReg==7)
	            				{
	            					...
	            					$pdf->MultiCell($this->page_largeur-$this->posxpar-$this->marge_droite, 3, $value, 0, 'R');
	            				}
						...
						//control de si se necesita mas pagina
	            		...
	            		if (isset($object->lines[$i+1]->pagebreak) && $object->lines[$i+1]->pagebreak)
	            		{
							...
							/*
									...
									//$qty = $object->lines[$i]->unit_budget;
										if ($lines[$i]->format=='det')
										{
											$qty = $lines[$i]->unit_budget;
											$pdf->SetXY($this->posxqty, $curY);
											$pdf->MultiCell(($seltype=='PU'?$this->posxpor:$this->posxpri)-$this->posxqty-0.8, 3, number_format($qty,$general->decimal_quant), 0, 'R');
										}
										if ($lines[$i]->format=='det')
										{
											if ($seltype == 'PU')
											{
										//nporc
												$porc = $lines[$i]->percent_prod;
												$pdf->SetXY($this->posxpor, $curY);
												$pdf->MultiCell($this->posxnpri-$this->posxpor-0.8, 3, number_format($porc,$general->decimal_pu), 0, 'R');
										//noprod
												$nprod = $lines[$i]->amount_noprod;
												$pdf->SetXY($this->posxnpri, $curY);
												$pdf->MultiCell($this->posxpri-$this->posxnpri-0.8, 3, number_format($nprod,$general->decimal_pu), 0, 'R');
											}
									...
									//$qty = $object->lines[$i]->unit_budget;
										$ma = $lines[$i]->ma;
										$pdf->SetXY($this->posxqty, $curY);
										$pdf->MultiCell($this->posxpri-$this->posxqty-0.8, 3, number_format($ma,$general->decimal_total), 0, 'R');

									//price
										$mo = $lines[$i]->mo;
										$pdf->SetXY($this->posxpri, $curY);
										$pdf->MultiCell($this->posxpar-$this->posxpri-0.8, 3, number_format($mo,$general->decimal_total), 0, 'R');

									//parcial
										$mq = $lines[$i]->mq;
										$pdf->SetXY($this->posxpar, $curY);
										$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxpar, 3, number_format($mq, $general->decimal_total), 0, 'R');

									}
									$object->total_ttc+= $partial;
									$object->total_ma+= $ma+0;
									$object->total_mo+= $mo+0;
									$object->total_mq+= $mq+0;
				...
				*/
				...
				//$posy=$this->_tableau_tot($pdf, $object, 0, $bottomlasttab, $outputlangs,$general);
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs,$general)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				{
					$total_ma = number_format($object->total_ma,$general->decimal_total);
					
					$pdf->SetXY($this->posxqty, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($this->posxpri-$this->posxqty-0.8, $tab2_hl, $total_ma, $useborder, 'R', 1);					
					$total_mo = number_format($object->total_mo,$general->decimal_total);
					$pdf->SetXY($this->posxpri, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($this->posxpar-$this->posxpri-0.8, $tab2_hl, $total_mo, $useborder, 'R', 1);					
					$total_mq = number_format($object->total_mq,$general->decimal_total);
					$pdf->SetXY($this->posxpar, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxpar, $tab2_hl, $total_mq, $useborder, 'R', 1);					
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='',$seltype='')
	{
		...
		if ($seltype != 'RUB')
		{
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell(($seltype=='PU'?$this->posxpor:$this->pri)-$this->posxqty-1,2, $outputlangs->transnoentities("Cantidad"),'','C');
			}
			if ($seltype == 'PU')
			{
				...
				if (empty($hidetop))
				{
					...
					$pdf->MultiCell($this->posxnpri-$this->posxpor-1,2, $outputlangs->transnoentities("Percentproductivity"),'','C');
				}
				...
				if (empty($hidetop))
				{
					...
					$pdf->MultiCell($this->posxpri-$this->posxnpri-1,2, $outputlangs->transnoentities("Amountnoprod"),'','C');
				}
			}
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell($this->posxpar-$this->posxpri-1,2, $outputlangs->transnoentities("P.U"),'','C');
			}
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell(30,2, $outputlangs->transnoentities("Costtotal"),'','C');
			}
		}
	 ...
	 *  @return	void
	 */
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs,$societe)
	{
		global $conf,$langs;
		...
		if ($object->refitem)
		{
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Item")." : " . $outputlangs->convToOutputCharset($object->refitem), '', 'R');
		}
		if ($object->labelitem)
		{
			$posy+=4;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->convToOutputCharset($object->labelitem), '', 'R');
		}
