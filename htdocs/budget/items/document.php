...
$result=restrictedArea($user,'budget',$fieldvalue,'budget','','',$fieldtype);
...
if ($id > 0 || ! empty($ref))
{
	...
	$upload_dir = $conf->budget->multidir_output[$object->entity].'/'.get_exdir(0, 0, 0, 0, $object, 'product').dol_sanitizeFileName($object->ref);

	if (! empty($conf->global->BUDGET_ITEM_USE_OLD_PATH_FOR_PHOTO))    // For backward compatiblity, we scan also old dirs
	{
		$upload_dirold = $conf->budget->multidir_output[$object->entity].'/'.substr(substr("000".$object->id, -2),1,1).'/'.substr(substr("000".$object->id, -2),0,1).'/'.$object->id."/photos";
	}
}
$modulepart='budget';
...
//{
$title = $langs->trans('Items')." ". $shortlabel ." - ".$langs->trans('Documents');
$helpurl='EN:Module_Budget|FR:Module_Budget|ES:M&oacute;dulo_Presupuestos';
//}
...
if ($object->id)
{
	...
	$permission = $user->rights->budget->ite->crear;
	$param = '&id=' . $object->id;
	include_once DOL_DOCUMENT_ROOT . '/core/tpl/document_actions_post_headers.tpl.php';


	// Merge propal PDF document PDF files
	if (!empty($conf->global->BUDGET_PDF_MERGE_PROPAL))
	{
		$filetomerge = new Propalmergepdfproduct($db);
		...
		if (! empty($conf->global->BUDGET_ITEM_USE_OLD_PATH_FOR_PHOTO))
		...
		if (count($filearray) > 0)
		{
			...
			if (count($filetomerge->lines) > 0) {
				print $langs->trans('PropalMergePdfBudgetActualFile');
			}

			print '<form name="filemerge" action="' . DOL_URL_ROOT . '/budget/items/document.php?id=' . $object->id . '" method="post">';
