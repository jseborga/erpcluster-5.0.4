 ...
 */
function calcul_price_totaladd($qty, $pu, $remise_percent_ligne, $txtva, $uselocaltax1_rate, $uselocaltax2_rate, $remise_percent_global, $price_base_type, $info_bits, $type, $seller = '',$localtaxes_array='')
{
	...
	{
		dol_syslog("calcul_price_totaladd Warning: function is called with parameter seller that is missing", LOG_WARNING);
	...
	{
		dol_syslog("calcul_price_totaladd Warning: function is called with parameter localtaxes_array that is missing", LOG_WARNING);
	}
	...
	if (is_array($localtaxes_array))
	{
		...
		$localtax3_type = $localtaxes_array[4];
		$localtax3_rate = $localtaxes_array[5];
		$localtax4_type = $localtaxes_array[6];
		$localtax4_rate = $localtaxes_array[7];
		$localtax5_type = $localtaxes_array[8];
		$localtax5_rate = $localtaxes_array[9];
	}
	else
	{
    ...
    {
    	if ($conf->global->PURCHASE_PRICE_TAX_INCLUDED)
    	{
			$tot_sans_remise_wt = price2num($tot_sans_remise * ($txtva / 100),'MU');
    		$tot_avec_remise_wt = price2num($tot_avec_remise * ($txtva / 100),'MU');
    		$pu_wt = price2num($pu * ($txtva / 100),'MU');
    	}
    	else
    	{
    		$tax = 1/(1-($txtva/100));
    		$tot_sans_remise_wt = price2num($tot_sans_remise * ($tax / 100),'MU');
    		$tot_avec_remise_wt = price2num($tot_avec_remise * ($tax / 100),'MU');
    		$pu_wt = price2num($pu * ($tax / 100),'MU');
    	}
    }
    else
    {
