...
require_once DOL_DOCUMENT_ROOT . '/purchase/class/supplierproposalext.class.php';
require_once DOL_DOCUMENT_ROOT . '/purchase/class/supplierproposaladd.class.php';
require_once DOL_DOCUMENT_ROOT . '/purchase/class/supplierproposaldetadd.class.php';
...
{
	require_once DOL_DOCUMENT_ROOT.'/orgman/class/ccountry.class.php';
	require_once DOL_DOCUMENT_ROOT.'/orgman/class/cregions.class.php';
	require_once DOL_DOCUMENT_ROOT.'/orgman/class/cdepartements.class.php';
}
...
//$result = restrictedArea($user, 'supplier_proposal', $id,'','purchase');
if (!$user->rights->purchase->sup->lire)
	accessforbidden();

$object = new SupplierProposalext($db);
...
if ($conf->orgman->enabled)
{
	$objCcountry = new Ccountry($db);
	$objCregions = new Cregions($db);
	$objCdepartements = new Cdepartements($db);
}
...
if ($id > 0 || ! empty($ref)) {
	...
	$retadd = $objSupplieradd->fetch(0,$id);
}
...
if (empty($reshook))
{
	...
	else if ($action == 'confirm_enablesupplier' && $confirm == 'yes' && $user->rights->purchase->sup->ena)
	{
		$res = $objSupplieradd->fetch(0,$object->id);
		if ($res > 0)
		{
			$objSupplieradd->status = 1;
			$res = $objSupplieradd->update($user);
			if ($res <=0)
			{
				$error++;
		...
		if (!$error) {
			setEventMessages($langs->trans('Enabledsupplier'),null,'mesgs');
	...
	else if ($action == 'add' && $user->rights->supplier_proposal->creer)
	{
		...
		if (! $error)
		{
			...
			if (! $error)
			{
							...
							for ($i=0;$i<$num;$i++)
							{
								...
								if ($conf->fiscal->enabled)
								{
									...
									if (!$error)
									{
										...
										$objSupplierdetadd->object = 'revisar';
				...
				if ($id > 0)
				{
					//creamos registro en la tabla adicional
					$objSupplieradd->fk_supplier_proposal = $id;
					$objSupplieradd->fk_purchase_request = $idRequest+0;
					$objSupplieradd->code_facture = (GETPOST('code_facture','alpha')?GETPOST('code_facture','alpha'):$conf->global->FISCAL_CODE_FACTURE_PURCHASE);
					$objSupplieradd->code_type_purchase = (GETPOST('code_type_purchase','alpha')?GETPOST('code_type_purchase','alpha'):$conf->global->PURCHASE_CODE_TYPE_PURCHASE);
					$objSupplieradd->fk_pays = GETPOST('fk_pays', 'int');
					if (empty($objSupplieradd->fk_pays)) $objSupplieradd->fk_pays=0;
					$objSupplieradd->fk_province = GETPOST('fk_province', 'int');
					if (empty($objSupplieradd->fk_province)) $objSupplieradd->fk_province=0;
	...
	if ($action == 'addline' && $user->rights->supplier_proposal->creer)
	{
		...
		if (!$conf->fiscal->enabled)
		{
			if (GETPOST('prod_entry_mode') == 'free' && empty($idprod) && $price_ht == '')
		// Unit price can be 0 but not ''. Also price can be negative for proposal.
			{
				setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv("UnitPriceHTx")), null, 'errors');
				$error++;
			}
		}
		...
		if (! $error && ($qty >= 0))
		{
			...
			if ((GETPOST('prod_entry_mode') != 'free') && empty($error))
			{
				...
				if (preg_match('/^idprod_([0-9]+)$/',GETPOST('idprodfournprice'), $reg))
				{
					...
					$productsupplier->get_buyprice(0, -1, $idprod, 'none');
			        		// We force qty to -1 to be sure to find if a supplier price exist
				...
				elseif (GETPOST('idprodfournprice') > 0)
				{
					...
					$resfourn=$productsupplier->get_buyprice(GETPOST('idprodfournprice'), -1);
					if ($resfourn>0)
					{
						$idprod = GETPOST('idprodfournprice');
						//REVISAR SI CORRESPONDE
						//$idprod = $resfourn;
					}
					else
					{
						//agregamos el producto fourn
				...
				if ($idprod > 0)
				{
					...
					if ($conf->fiscal->enabled)
					{
						...
						$lines->fk_unit = GETPOST('fk_unit')+0;
						$fk_unit = GETPOST('fk_unit')+0;
						$lines->fk_unit = GETPOST('fk_unit')+0;
						$discount = GETPOST('discount');
						if (empty($discount)) $discount = 0;
						$remise_percent = GETPOST('remise_percent');
						$objectadd = new Stdclass();
						$objectadd->code_facture = $conf->global->FISCAL_CODE_FACTURE_PURCHASE;
						$origin = '';
						$origin_id = 0;
						...
						if (!$error)
						{
							$objSupplierdetadd->initAsSpecimen();
							$objSupplierdetadd->fk_supplier_proposaldet = $result;
							$objSupplierdetadd->fk_object = 0;
							$objSupplierdetadd->object = '';
							$objSupplierdetadd->fk_fabrication = GETPOST('fk_fabrication');
							if (empty($objSupplierdetadd->fk_fabrication))$objSupplierdetadd->fk_fabrication=0;
							$objSupplierdetadd->fk_fabricationdet = GETPOST('fk_fabricationdet');
							if (empty($objSupplierdetadd->fk_fabricationdet))$objSupplierdetadd->fk_fabricationdet=0;
							$objSupplierdetadd->fk_projet = GETPOST('fk_projet');
							if (empty($objSupplierdetadd->fk_projet))$objSupplierdetadd->fk_projet=0;
							$objSupplierdetadd->fk_projet_task = GETPOST('fk_projet_task');
							if (empty($objSupplierdetadd->fk_projet_task))$objSupplierdetadd->fk_projet_task=0;
							$objSupplierdetadd->fk_jobs = GETPOST('fk_jobs');
							if (empty($objSupplierdetadd->fk_jobs))$objSupplierdetadd->fk_jobs=0;
							$objSupplierdetadd->fk_jobsdet = GETPOST('fk_jobsdet');
							if (empty($objSupplierdetadd->fk_jobsdet))$objSupplierdetadd->fk_jobsdet=0;
							$objSupplierdetadd->fk_structure = GETPOST('fk_structure');
							if (empty($objSupplierdetadd->fk_structure))$objSupplierdetadd->fk_structure=0;
							$objSupplierdetadd->fk_poa = GETPOST('fk_poa');
							if (empty($objSupplierdetadd->fk_poa))$objSupplierdetadd->fk_poa=0;
							$objSupplierdetadd->partida = GETPOST('partida');
							$objSupplierdetadd->amount_ice = GETPOST('amount_ice');
							if (empty($objSupplierdetadd->amount_ice))$objSupplierdetadd->amount_ice=0;
							$objSupplierdetadd->discount = $discount+0;
							$objSupplierdetadd->fk_user_create = $user->id;
							$objSupplierdetadd->fk_user_mod = $user->id;
							$objSupplierdetadd->datec = dol_now();
							$objSupplierdetadd->datem = dol_now();
			...
			// Free product
			{
				...
				if ($conf->fiscal->enabled)
				{
					...
					if (!$error)
					{
						...
						$objSupplierdetadd->fk_object = 0;
						$objSupplierdetadd->object = '';
						$objSupplierdetadd->fk_fabrication = GETPOST('fk_fabrication')+0;
						$objSupplierdetadd->fk_fabricationdet = GETPOST('fk_fabricationdet')+0;
						$objSupplierdetadd->fk_projet = GETPOST('fk_projet')+0;
						$objSupplierdetadd->fk_projet_task = GETPOST('fk_projet_task')+0;
						$objSupplierdetadd->fk_jobs = GETPOST('fk_jobs')+0;
						$objSupplierdetadd->fk_jobsdet = GETPOST('fk_jobsdet')+0;
						$objSupplierdetadd->fk_structure = GETPOST('fk_structure')+0;
						$objSupplierdetadd->fk_poa = GETPOST('fk_poa')+0;
						$objSupplierdetadd->partida = GETPOST('partida');
						$objSupplierdetadd->amount_ice = GETPOST('amount_ice')+0;
						$objSupplierdetadd->discount = $discount+0;
						$objSupplierdetadd->fk_user_create = $user->id;
						$objSupplierdetadd->fk_user_mod = $user->id;
						$objSupplierdetadd->datec = dol_now();
			...
			if ($conf->fiscal->enabled && empty($error) && $result > 0)
			{
				$error = update_total_supplier($id);
			}
	...
	// Mise a jour d'une ligne dans la demande de prix
		...
		if (! $error)
		{
			...
			if ($conf->fiscal->enabled)
			{
				$lines = new SupplierProposalLineext($db);
				$lines->fetch(GETPOST('lineid'));
				if ($idprod>0)
				{
				...
				include DOL_DOCUMENT_ROOT.'/fiscal/include/calclinefiscal.inc.php';
				$result = $object->updatelineadd(GETPOST('lineid'), $lines->subprice, GETPOST('qty'), $lines->remise_percent, $lines->tva_tx, $lines->localtax1, $lines->localtax2, $description, $price_base_type, $info_bits, $special_code, GETPOST('fk_parent_line'), 0, $fournprice, $buyingprice, $label, $type, $array_options, $ref_fourn,$lines);

				if (!$error)
				{
					//buscamos
					$res = $objSupplierdetadd->fetch(0,GETPOST('lineid'));
					if ($res == 1)
					{
						//$objSupplierdetadd->fk_object = 0;
						//$objSupplierdetadd->object = '';
						...
						$objSupplierdetadd->tms = dol_now();
						$objSupplierdetadd->status = 1;
						$res = $objSupplierdetadd->update($user);
						if ($res <=0)
						{
							$error++;
							setEventMessages($objSupplierdetadd->error,$objSupplierdetadd->errors,'errors');
...
//vamos a filtrar las regiones (cregions)
$filter = " AND t.fk_pays = ".(GETPOST('fk_pays')?GETPOST('fk_pays'):$mysoc->country_id);
$rescr = $objCregions->fetchAll('ASC','t.nom',0,0,array(),'AND',$filter);
$optionsprov = '<option value="">'.'</option>';
if ($rescr>0)
{
	$lines = $objCregions->lines;
	foreach ($lines AS $j => $line)
	{
		$optionsprov.= '<optgroup label="'.$line->nom.'">';
		//vamos a armar las provincias (cdepartements) segun la selección de fk_region
		$filter = " AND t.fk_region = '".$line->code_region."'";
		$rescd = $objCdepartements->fetchAll('ASC','t.nom',0,0,array(),'AND',$filter);
		if ($rescd>0)
		{
			$linesdep = $objCdepartements->lines;
			foreach ($linesdep AS $k => $linedep)
			{
				$selected = '';
				if ((GETPOST('fk_province')?GETPOST('fk_province'):$object->fk_province)==$linedep->id) $selected = ' selected';
					$optionsprov.='<option value="'.$linedep->id.'" '.$selected.'>'.$linedep->nom.'</option>';
				}
			}
		}
	}
 ...
 * View
 */
$title = $langs->trans('CommRequests');
...
if (! empty($conf->use_javascript_ajax))
{
	...
	print '$(document).ready(function () {
		$("#selectfk_pays").change(function() {
			document.addprop.action.value="'.$action.'";
			document.addprop.submit();
		});
	});';
	print '</script>'."\n";
}
...
if ($action == 'create')
{
	...
	if ($socid > 0 && !empty($origin)) {
		...
		print '<td colspan="2">';
		print $form->select_company(GETPOST('socid'), 'socid', 's.fournisseur = 1', 'SelectThirdParty');
		print '</td>';
	}
	print '</tr>' . "\n";

	//registramos el pais
	print '<tr><td class="nowrap">' . $langs->trans('Country') . '</td><td colspan="2">';
	print $form->select_country((GETPOST('fk_pays')?GETPOST('fk_pays'):$mysoc->country_id),'fk_pays','',40);
	...
	print '<tr><td class="nowrap">' . $langs->trans('Province') . '</td><td colspan="2">';
	print '<select id="fk_province" name="fk_province">'.$optionsprov.'</select>';
	//$form->select_conditions_paiements(GETPOST('cond_reglement_id') > 0 ? GETPOST('cond_reglement_id') : $soc->cond_reglement_id, 'cond_reglement_id', -1, 1);
	print '</td></tr>';
	...
	// Confirm enablesupplier
	else if ($action == 'enablesup') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Enablesupplier'), $langs->trans('ConfirmEnablesupplier', $object->ref), 'confirm_enablesupplier', '', 0, 1);
	}
	...
	// country
	if ($objSupplieradd->fk_pays>0)
	{
		$objCcountry->fetch($objSupplieradd->fk_pays);
		if ($objCcountry->code)
		{
			print '<tr><td>' . $langs->trans('Country') . '</td>';
			print '<td>';
			$img=picto_from_langcode($objCcountry->code);
			print $img?$img.' ':'';
			print getCountry($objCcountry->code,1);
			print '</td></tr>';
		}
	}
	// province
	if ($objSupplieradd->fk_province>0)
	{
		$objCdepartements->fetch($objSupplieradd->fk_province);
		print '<tr><td>' . $langs->trans('Province') . '</td><td colspan="5">' . $objCdepartements->nom . '</td>';
		print '</tr>';
	}
	...
	if (! empty($object->lines))
		$ret = $object->printObjectLinesadd($action, $soc, $mysoc, $lineid, 1);
	...
	if ($object->statut == 0 && $user->rights->supplier_proposal->creer)
	{
		if ($action != 'editline')
		{
			$var = true;
			$lAdddet =true;
			if ($objSupplieradd->fk_purchase_request>0 && $user->societe_id)
				$lAdddet = false;
			// Add products/services form
			if ($lAdddet)
				$object->formAddObjectLineadd(1, $soc, $mysoc);
		...
		if (empty($reshook))
		{
			if ($action != 'statut' && $action != 'editline')
			{
				...
				if ($object->statut == 0 && $object->total_ttc >= 0 && count($object->lines) > 0 &&
					((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $user->rights->supplier_proposal->creer) || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->supplier_proposal->validate_advance)))
				) {
				...
				if (! empty($conf->commande->enabled) && $object->statut == 2) {
					if ($user->rights->fournisseur->commande->creer) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/purchase/commande/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddOrder") . '</a></div>';
					}
				}
				...
				// Delete
				if ($object->statut == 0 && $user->rights->supplier_proposal->supprimer) {
					print '<div class="inline-block divButAction"><a class="butActionDelete" href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;action=delete"';
					print '>' . $langs->trans('Delete') . '</a></div>';
				}
				//validar recepcion propuesta a tiempo
				if ($object->statut == 1 && empty($objSupplieradd->status) && $user->rights->purchase->sup->ena) {
					print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;action=enablesup"';
					print '>' . $langs->trans('Enablesupplier') . '</a></div>';
				}
			}
		}
