 ...
 */
class pdf_fractalvisual extends ModelePDFContract
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->description = $langs->trans("Modelo de contrato para visual");
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		$outputlangs->load("monprojet@monprojet");
		...
		if ($conf->contrat->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (! empty($object->array_options['options_place_event']))
				{
					//$tab_top = 88;

					$pdf->SetFont('','', $default_font_size - 1);
					$pdf->writeHTMLCell(190, 3, $this->posxdesc-1, $tab_top, $langs->trans('Place of the event').': '.dol_htmlentitiesbr($object->array_options['options_place_event']).', '.$langs->trans('Dateevent').' '.dol_print_date($object->array_options['options_date_event'],'dayhour'), 0, 1);
					$nexY = $pdf->GetY();
					$height_note=$nexY-$tab_top;
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					if ($valide > 0 || $object->specimen)
					{
						...
						$pdf->writeHTMLCell(0, 0, $curX, $curY, dol_concatdesc($txtpredefinedservice,$desc), 0, 1, 0);
						$nexY = $pdf->GetY()+2;
						$curY = $nexY;
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		*/
		...
		// Output Rect
		$this->printRect($pdf, $this->marge_gauche, $tab_top, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $tab_height+3, $hidetop, $hidebottom);	
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Work order");
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		//obtenemos el total del contrato
		$total_ttc = 0;
