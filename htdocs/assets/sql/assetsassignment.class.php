...
class Assetsassignment extends CommonObject
{
	...
	function LibStatut($status,$mode=0)
	{
		...
		if ($mode == 2)
		{
			if ($status == -1) return img_picto($langs->trans('Annulled'),'statut5').' '.$langs->trans('Annulled');
			if ($status == 0) return img_picto($langs->trans('Draft'),'statut4').' '.$langs->trans('Draft');
			if ($status == 1) return img_picto($langs->trans('Validated'),'statut6').' '.$langs->trans('Validated');
			if ($status == 2) return img_picto($langs->trans('Accepted'),'statut7').' '.$langs->trans('Accepted');
		}
		if ($mode == 3)
		{
