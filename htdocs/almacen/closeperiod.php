...
if (!$user->rights->almacen->close->write) accessforbidden();
...
if ($action == 'closeperiod')
{
	...
	$date_closed  = dol_mktime(0, 0, 0, GETPOST('dc_month'),  GETPOST('dc_day'),  GETPOST('dc_year'));
...
if ($action == 'confirm_closeperiod' && $user->rights->almacen->per->actal)
{
	...
	$date_closed = $aCloseperiod['date_closed'];
	$aBalanceact = $aCloseperiod['aBalanceact'];
	...
	$db->begin();
	$datem = $datefinselect;
	$aDatem = dol_getdate($datem);
	$aDateinvini = dol_get_next_day($aDatem['mday'], $aDatem['mon'],$aDatem['year']);

	foreach ($aBalanceact AS $fk_entrepot => $aData)
	{
		foreach ($aData AS $fk_product => $saldocalc)
		{
											//vamos a actualizar los saldos por producto y entrepot
								//vamos a verificar y actualizar los saldos de la tabla product_stock
								//buscamos y actualizamos el product_stock
			$sqls = "SELECT rowid, reel FROM ".MAIN_DB_PREFIX."product_stock";
			$sqls.= " WHERE fk_entrepot = ".$fk_entrepot." AND fk_product = ".$fk_product;
			$resqls=$db->query($sqls);
			$alreadyarecord = 0;
			if ($resqls)
			{
				$objx = $db->fetch_object($resqls);
				if ($objx)
				{
					$alreadyarecord = 1;
			...
			if (! $error)
			{
				if ($alreadyarecord > 0)
				{
					$sqls = "UPDATE ".MAIN_DB_PREFIX."product_stock SET reel = ".$saldocalc;
					$sqls.= " WHERE fk_entrepot = ".$fk_entrepot." AND fk_product = ".$fk_product;
				}
				else
				{
					$sqls = "INSERT INTO ".MAIN_DB_PREFIX."product_stock";
					$sqls.= " (reel, fk_entrepot, fk_product) VALUES ";
					$sqls.= " (".$saldocalc.", ".$fk_entrepot.", ".$fk_product.")";
				}
				//if ($obj->rowid == 46) echo $sqls;
				$resqls=$db->query($sqls);
				if (! $resqls)
				{
					$error = 302;
	...
	foreach ($lines AS $j => $row)
	{
		...
		if ($row['type'] == 'l')
		{
			if (!$error)
			{
				$rowid = $row['rowidstock'];
				if ($rowid>0 && $row['saldoo']>0)
				{
					$newprice = $row['valorAct'] / $row['saldoo'];
					//nuevo precio
					$newpriceppp  = $row['valorActppp']  / $row['saldoo'];
					$newpricepeps = $row['valorActpeps'] / $row['saldoo'];
					$newpriceueps = $row['valorActueps'] / $row['saldoo'];
					$res = $objStock->fetch($rowid);
					$fk_entrepot = $objStock->warehouse_id;
					$fk_product  = $objStock->product_id;
					if ($res > 0)
					{
						$qty = $objStock->qty;
					...
					$res = $objStockadd->fetch(0,$rowid);
					if ($res>0)
					{
						//if(empty($typemethod)) $qty = $objStockadd->qty;
						//elseif($typemethod==1) $qty = $objStockadd->balance_peps;
						//elseif($typemethod==2) $qty = $objStockadd->balance_ueps;
						$qty = $row['saldoo']+0;
						//$dif = ($qty - $row['saldoo'])*1;
						//$sumadif+= $dif;
						//if ($dif <>0)
						//{
						//echo '<hr>rowid '.$rowid.' entr '.$fk_entrepot.' prod '.$fk_product.' qty '.$qty.' qtyact '.$aBalanceact[$fk_entrepot][$fk_product];
						//echo ' cantidaactualiza '.$qty.' saldoo '.$row['saldoo'].' DIF '.price($dif);
						//}
						...
						$objStockpricemod->date_closed = $date_closed;
						...
						$objStockpricemod->price = $price+0;
						...
						$objStockpricemod->value_peps_new = $newpricepeps;
						$objStockpricemod->value_ueps_new = $newpriceueps;
						$objStockpricemod->price_new = $newpriceppp;
						$objStockpricemod->datec = $now;
						$objStockpricemod->datem = $now;
						$objStockpricemod->tms = $now;
						$objStockpricemod->status = 1;
						$resn = $objStockpricemod->create($user);
						if ($resn <=0)
						{
							$error=101;
							setEventMessages($objStockpricemod->error,$objStockpricemod->errors,'errors');
						}
						//vamos a generar una salida y un ingreso del mismo producto
						if (!$error)
						{
							$transf = new Transf($db);
							//echo '<br>restransf '.
							$resultn=$transf->fetch($fk_product);
							$transf->load_stock();
							$pricesrc=0;
							if (isset($transf->stock_warehouse[$fk_entrepot]->pmp))
								$pricesrc=$transf->stock_warehouse[$fk_entrepot]->pmp;
							if (empty($pricesrc))
								$pricesrc = $transf->pmp;
							...
							$idreg=0;
							$fk_type_mov=0;
							$aSales[$rowid]=array('id'=>$rowid,'qty'=>$qty,'value'=>$value_peps);
							foreach ((array) $aSales AS $fk_stock => $rowstock)
							{
								$transf->origin = '';
								...
								for($a=1; $a<=2;$a++)
								{
									if ($a == 1)
									{
										...
										$operation = 1;
										$dateop = $datem;
										if ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==1) $rowstock['value'] = $value_peps;
										elseif ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==2) $rowstock['value'] = $value_ueps;

										$pricedest = $price+0;
										$label = $langs->trans('Warehouseexitaccordingmanagementclosing').' '.$_SESSION['period_year'].($conf->global->ALMACEN_CLOSE_PERIOD!=9?'-'.$_SESSION['period_month']:'');
									}
									else
									{
										//ingreso
										$aDatenow = dol_getdate(dol_now());
										if ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==1) $rowstock['value'] = $newpricepeps;
										elseif ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==2) $rowstock['value'] = $newpriceueps;
										$operation = 0;
										$pricedest = $newpriceppp+0;
										$dateop = dol_mktime($aDatenow['hours'],$aDatenow['minutes'],$aDatenow['seconds'],$aDateinvini['month'],$aDateinvini['day'],$aDateinvini['year']);
										$label = $langs->trans('Inventoryentryupdatedbymanagementstart').' '.$aDateinvini['year'].($conf->global->ALMACEN_CLOSE_PERIOD!=9?'-'.$aDateinvini['month']:'');
									}
									$rowstock['operation'] = $operation;

									$result2=$transf->add_transfer_ok($user,$fk_entrepot,$rowstock['qty'],$operation,$label,$pricedest,$idreg,$fk_type_mov,$dateop);
									if ($result2 <= 0)
									{
										$error=2011;
										//echo '<hr>'.$a.' res '.$result2.' rowid '.$rowid;
									...
									if (!$error)
									{
										...
										$lAdd = true;
										if ($resadd==1) $lAdd = false;

										$resadd = closeperiod_add($rowstock, $result2, $iddoc,$lAdd);
										if ($resadd <=0)
										{
					...
					{
						$error++;
						setEventMessages($langs->trans('No se encuentra en stock_mouvment').' '.$rowid,null,'errors');
					}
					if (!$error)
					{
						if ($res>0)
						{
							$objStockadd->value_peps_adq = $newprice;
							$objStockadd->value_ueps_adq = $newprice;
							$objStockadd->fk_user_mod = $user->id;
							$objStockadd->date_mod = $now;
							$resm = $objStockadd->update_close_period($user);
							if ($resm <=0)
							{
								$error=102;
	...
	//echo '<hr>errr '.$error.' regcreate '.$resn.' sumadif '.$sumadif;
	//exit;
	//si no existe error
		...
		else
		{
			...
			if ($res>0)
			{
				...
				foreach ($lines AS $j => $line)
				{
					...
					{
						$object->status_al = 0;
						//echo '<hr>'.$object->period_month.'-'.$object->period_year;
						$res = $object->update($user);
						if ($res <=0)
						{
							$error++;
			...
			if (!$error)
			{
				...
				if ($res==0)
				{
					...
					$object->status_af = 1;
					$object->status_al = 1;
					//$object->period_month.'-'.$object->period_year;
					$res = $object->create($user);
					if ($res <=0)
					{
						$error++;
...
$sql.= " value = 0 ";
$sql.= " WHERE rowid IN(10877,10879)";
$resql = $db->query($sql);
//view
//
...
if ($conf->global->ALMACEN_CLOSE_PERIOD == 1)
	$date_closed = dol_get_last_day($period_year,$_SESSION['period_month']);
else
	$date_closed = dol_get_last_day($period_year,12);
print '<td>';
print $form->select_date($date_closed,'dc_',0,0,0,'',1,0,0,1);
print '</td>';
print '</tr>';
	...
	if ($resent)
	{
		...
		if ($nument > 0)
		{
			...
			while ($e < $nument)
			{
				...
				//fk_entrepot
				$id = $objent->rowid;
				//vamos a obtener los productos que tuvieron movimiento en este almacen
				$sql  = "SELECT t.fk_product ";
				$sql.= " FROM ".MAIN_DB_PREFIX."stock_mouvement AS t";
				$sql.= " WHERE t.fk_entrepot = ".$id;
				$sql.= " GROUP BY t.fk_product";

				$resstock = $db->query($sql);
				$idsProduct = '0';
				if ($resstock)
				{
					$numstock = $db->num_rows($resstock);
					if ($numstock>0)
					{
						$s = 0;
						while ($s < $numstock)
						{
							...
							$idsProduct.=$objstock->fk_product;
							$s++;
						}
					}
				}
				...
				$resmov = $movement->mouvement_period($id,$dateinisel,$datefinselect,0,$idsProduct);
				if ($resmov > 0)
				{
						...
						if ($num > 0)
						{
							...
							while ($o < $num)
							{
								...
								foreach ((array) $aIng AS $fk => $data)
								{
									...
									$aMov['pricepeps'] = $data['price_peps'];
									$aMov['priceueps'] = $data['price_ueps'];
									$aMov['priceppp'] = $data['price_ppp'];
									...
									$aMov['amountppp'] = $aMov['saldo']*$aMov['priceppp'];
									$aMov['amountpeps'] = $aMov['saldo']*$aMov['pricepeps'];
									$aMov['amountueps'] = $aMov['saldo']*$aMov['priceueps'];
									...
									// valor actualizado ufv
									$valorActppp=($factor)*$aMov['amountppp'];
									// Valor diferencia ufv
									$valorDifppp=$valorActppp-$aMov['amountppp'];
									// valor actualizado ufv
									$valorActpeps=($factor)*$aMov['amountpeps'];
									// Valor diferencia ufv
									$valorDifpeps=$valorActpeps-$aMov['amountpeps'];
									// valor actualizado ufv
									$valorActueps=($factor)*$aMov['amountueps'];
									// Valor diferencia ufv
									$valorDifueps=$valorActueps-$aMov['amountueps'];

									$unit = $langs->trans($product->getLabelOfUnit());
									$saldocalc+= $aMov['saldo'];
									if (price2num($aMov['saldo'],'MU')>0)
									{
										...
										{
											print '<td align="right" widht="7%">'.price(price2num($aMov['amountpeps'],'MT')).'</td>';
											print '<td align="right" widht="7%">'.price(price2num($valorActpeps,'MT')).'</td>';
											print '<td align="right" widht="7%">'.price(price2num($valorDifpeps,'MT')).'</td>';
										}
										elseif ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==2)
										{
											print '<td align="right" widht="7%">'.price(price2num($aMov['amountueps'],'MT')).'</td>';
											print '<td align="right" widht="7%">'.price(price2num($valorActueps,'MT')).'</td>';
											print '<td align="right" widht="7%">'.price(price2num($valorDifueps,'MT')).'</td>';
										}
										else
										{
											print '<td align="right" widht="7%">'.price(price2num($aMov['amountppp'],'MT')).'</td>';
											print '<td align="right" widht="7%">'.price(price2num($valorActppp,'MT')).'</td>';
											print '<td align="right" widht="7%">'.price(price2num($valorDifppp,'MT')).'</td>';
										}
										if ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==1)
										{
											$sumParcial+=$aMov['amountpeps'];
											$sumEntrepot+=$aMov['amountpeps'];
											$sumTotal += $aMov['amountpeps'];

											$sumParcialufv+=$valorActpeps;
											$sumEntrepotufv+=$valorActpeps;
											//$sumTotalufv += $valorDif;
											$sumTotalufv += $valorActpeps;
											$sumValorDif+=$valorDifpeps;
											$sumEntrepotDif+=$valorDifpeps;
											$sumTotalDif+=$valorDifpeps;
										}
										elseif ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY==2)
										{
											$sumParcial+=$aMov['amountueps'];
											$sumEntrepot+=$aMov['amountueps'];
											$sumTotal += $aMov['amountueps'];

											$sumParcialufv+=$valorActueps;
											$sumEntrepotufv+=$valorActueps;
											//$sumTotalufv += $valorDif;
											$sumTotalufv += $valorActueps;
											$sumValorDif+=$valorDifueps;
											$sumEntrepotDif+=$valorDifueps;
											$sumTotalDif+=$valorDifueps;
										}
										else
										{
											$sumParcial+=$aMov['amountppp'];
											$sumEntrepot+=$aMov['amountppp'];
											$sumTotal += $aMov['amountppp'];

											$sumParcialufv+=$valorActppp;
											$sumEntrepotufv+=$valorActppp;
											//$sumTotalufv += $valorDif;
											$sumTotalufv += $valorActppp;
											$sumValorDif+=$valorDifppp;
											$sumEntrepotDif+=$valorDifppp;
											$sumTotalDif+=$valorDifppp;
										}

										$inventory[] = array('fk'=>$fk,'rowid'=>$obj->rowid,'ref'=>$obj->ref,'label'=>$obj->label,'unit'=>$unit,'pricee'=>$aMov['price'],'saldoo'=>$aMov['saldo'],'amountt'=>$aMov['amount'],'valorAct'=>$valorAct,
											'amountppp'=>$aMov['amountppp'],'amountpeps'=>$aMov['amountpeps'],'amountueps'=>$aMov['amountueps'],
											'valorActppp'=>$valorActppp,'valorActpeps'=>$valorActpeps,'valorActueps'=>$valorActueps,
											'valorDifppp'=>$valorDifppp,'valorDifpeps'=>$valorDifpeps,'valorDifueps'=>$valorDifueps,
											'valueufvini'=>$valueufvini,'valueufvfin'=>$valueufvfin,'factor'=>$factor,'rowidstock'=>$fk,'valorDif'=>$valorDif,'Grupo'=>$objent->label,'comp'=>$comp,'sumParcial'=>$sumParcial,'type'=>'l');
										print '</tr>';
									}
								}
								$aBalanceact[$id][$obj->rowid]=$saldocalc;
							...
							if ($sumParcial > 0 && $abc)
							{
								...
								print '</tr>';
								$inventory[] = array('rowid'=>$obj->rowid,'ref'=>'','label'=>$langs->trans('Total').' '.$objcat->label,'unit'=>'','pricee'=>'','saldoo'=>'','amountt'=>$sumParcial,'valorAct'=>$sumParcialufv,'valorDif'=>$sumValorDif,'Grupo'=>$objent->label,'type'=>'g');
								if ($sumParcial >0)
									$inventoryGroup[$objent->label] = $objent->label;
								else
									unset($inventoryGroup[$objent->label]);
							}
							$sumParcial =0;
							$sumParcialufv=0;
							$sumValorDif=0;
				...
				if ($sumEntrepot> 0)
				{
					print '<tr class="liste_total">';
					print '<td></td>';
					print '<td colspan="10">'.$langs->trans('Total').' '.$objent->label.'</td>';
					...
					print '</tr>';
					$inventory[] = array('rowid'=>$obj->rowid,'ref'=>'','label'=>$langs->trans('Total').' '.$objent->label,'unit'=>'','pricee'=>'','saldoo'=>'','amountt'=>$sumEntrepot,'valorAct'=>$sumEntrepotufv,'valorDif'=>$sumEntrepotDif,'Grupo'=>$objent->label,'type'=>'g','typef'=>1);
					if ($sumEntrepot >0)
						$inventoryGroup[$objent->label] = $objent->label;
					else
						unset($inventoryGroup[$objent->label]);
		...
		$aCloseperiod['date_closed'] = $date_closed;
		$aCloseperiod['inventoryGroup'] = $inventoryGroup;
		$aCloseperiod['lines'] = $inventory;
		$aCloseperiod['aBalanceact'] = $aBalanceact;
...
function closeperiod_add(array $row, $result2, $iddoc=0,$lAdd=true)
{
	global $conf,$langs;
	global $objectadd, $user;
