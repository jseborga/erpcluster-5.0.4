...
if (!$user->rights->budget->par->leer)
  accessforbidden();
...
if (empty($id) && !$user->admin)
  {
    $result=$object->fetch('',$user);
    if ($result < 0) dol_print_error($db);
    else
      {
	if ($object->fk_user == $user->id)
	...
	else
	  {
	    ...
	    if ($result < 0) dol_print_error($db);
	      ...
	      {
		$fromid = $object->id;
		$id = $object->createFromClone($fromid);
...
if ($action == 'add')
  {
    if (GETPOST('cancel'))
      {
	$urltogo=$backtopage?$backtopage:dol_buildpath('/budget/admin/budget.php',1);
