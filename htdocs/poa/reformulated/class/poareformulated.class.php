...
/**
 *  \file       dev/skeletons/poareformulated.class.php
 *  \ingroup    mymodule othermodule1 othermodule2
 *  \brief      This file is an example for a CRUD class file (Create/Read/Update/Delete)
 *				Initialy built by build_class_from_table on 2014-06-06 10:33
 */
 ...
 */
class Poareformulated extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	var $element='poareformulated';			//!< Id that identify managed objects
	var $table_element='poareformulated';		//!< Name of table without prefix where object is stored
    ...
    function create($user, $notrigger=0)
    {
        ...
        // Insert request
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."poa_reformulated(";
		...
		$sql.= " ".(! isset($this->date_reform) || dol_strlen($this->date_reform)==0?'NULL':$this->db->idate($this->date_reform)).",";
		...
		if (! $error)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."poa_reformulated");
    ...
    function fetch($id)
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."poa_reformulated as t";
        ...
        if ($resql)
        {
            if ($this->db->num_rows($resql))
            {
				...
				$this->ref = $obj->ref;
				$this->date_reform = $this->db->jdate($obj->date_reform);
    ...
    function update($user=0, $notrigger=0)
    {
        ...
        // Update request
        $sql = "UPDATE ".MAIN_DB_PREFIX."poa_reformulated SET";
		...
		$sql.= " date_reform=".(dol_strlen($this->date_reform)!=0 ? "'".$this->db->idate($this->date_reform)."'" : 'null').",";
	...
	function delete($user, $notrigger=0)
	{
		...
		if (! $error)
		{
    		$sql = "DELETE FROM ".MAIN_DB_PREFIX."poa_reformulated";
    		$sql.= " WHERE rowid=".$this->id;
	...
	function createFromClone($fromid)
	{
		...
		$object=new Poareformulated($this->db);
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
	function fetch_version($gestion,$statut=0)
	{
	  global $langs,$conf;
	  $sql = "SELECT";
	  $sql.= " t.rowid,";
	  ...
	  dol_syslog(get_class($this)."::fetch_version sql=".$sql, LOG_DEBUG);
	  $resql=$this->db->query($sql);
	  if ($resql)
	    {
	      if ($this->db->num_rows($resql))
	    ...
	    {
	      $this->error="Error ".$this->db->lasterror();
	      dol_syslog(get_class($this)."::fetch_version ".$this->error, LOG_ERR);
	      return -1;
	    }
	}
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
	function fetch_version_gestion($gestion)
	{
	  global $langs,$conf;
	  $sql = "SELECT";
	  $sql.= " t.rowid,";
	  ...
	  dol_syslog(get_class($this)."::fetch_version_gestion sql=".$sql, LOG_DEBUG);
	  $resql=$this->db->query($sql);
	  $this->array = array();
	  if ($resql)
	    {
	      ...
	      if ($this->db->num_rows($resql))
		{
		  ...
		  while ($i < $num)
		    {
		      $obj = $this->db->fetch_object($resql);
		      $objnew = new Poareformulated($this->db);
		      $objnew->id    = $obj->rowid;                
		      $objnew->entity = $obj->entity;
		      $objnew->fk_area = $obj->fk_area;
		      $objnew->gestion = $obj->gestion;
		      $objnew->ref = $obj->ref;
		      $objnew->date_reform = $this->db->jdate($obj->date_reform);
	    ...
	    {
	      $this->error="Error ".$this->db->lasterror();
	      dol_syslog(get_class($this)."::fetch_version_gestion ".$this->error, LOG_ERR);
	      return -1;
	    }
	}
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
	function fetch_version_backup($gestion)
	{
	  global $langs,$conf;
	  $sql = "SELECT";
	  $sql.= " t.rowid,";
	  ...
	  if ($resql)
		  ...
		  while ($i < $num)
		    {
		      ...
		      $objnew->statut = $obj->statut;
		      $objnew->active = $obj->active;
		      $this->aVersion[$obj->version][$obj->rowid] = $objnew;
		      $i++;
		    }
		}
	      $this->db->free($resql);
	    ...
	    {
	      $this->error="Error ".$this->db->lasterror();
	      dol_syslog(get_class($this)."::fetch_version_backup ".$this->error, LOG_ERR);
	      return -1;
	    }
	}
