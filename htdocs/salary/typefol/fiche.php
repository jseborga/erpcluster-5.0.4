...
$now = dol_now();
// Add
if ($action == 'add' && $user->rights->salary->typefol->creer)
{
	$object->ref     = $_POST["ref"];
	$object->entity  = $conf->entity;
	$object->detail  = GETPOST('detail');
	$object->details = GETPOST('details');
	$object->name_report = GETPOST('name_report');
	$object->state   = 1;
	...
	if ($object->ref)
	{
		$rid = $object->create($user);
		if ($rid > 0)
		{
			header("Location: fiche.php?id=".$rid);
			exit;
		}
		$action = 'create';
...
// Adddet
if ($action == 'adddet' && $user->rights->salary->typefol->creer)
{
	$error = 0;
	...
	{
		$error++;
		setEventMessages($objconcept->error,$objconcept->errors,'errors');
	}
	$objects->state       = GETPOST('state');
	$objects->details     = GETPOST('details');
	$objects->fk_user_create = $user->id;
	$objects->fk_user_mod = $user->id;
	$objects->datec = $now;
	$objects->datem = $now;
	$objects->tms = $now;
	$objects->status = 1;
	...
	if (empty($error))
	{
		...
		$action = 'createdet';
		setEventMessages($objects->error,$objects->errors,'errors');
	}
	else
	{
		$action="createdet";
...
// Updatedet
if ($action == 'updatedet' && $user->rights->salary->typefol->creer)
{
	$error = 0;
	if ($objects->fetch($_POST["rid"])>0)
	{
		...
		{
			$objects->sequen      = GETPOST('sequen');
			if ($objconcept->fetch($_POST['fk_concept']))
			{
				if ($objconcept->id == $_POST['fk_concept'])
					$objects->ref_concept = $objconcept->ref;
				else
...
// Delete period
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->salary->typefol->del)
{
...
$options = '';
foreach (array('/report') as $valdir)
{
	$dir = dol_buildpath("salary".$valdir);
	...
	if (is_dir($dir))
	{
		...
		if (is_resource($handle))
		{
			...
			{
				if (preg_match('/rplanilla/',substr($file,0,9),$aList))
				{
					if (file_exists($dir.'/'.$file))
					{
						$name = substr($file, 0, -4);
						$selected = '';
						if ($object->name_report == $name) $selected = ' selected';
						$options.= '<option value="'.$name.'" '.$selected.'>'.(empty($module->name)?$name:$module->name).'</option>';
...
if ($action == 'create' && $user->rights->salary->typefol->creer)
{
	...
	print '</td></tr>';
	//name_report
	print '<tr><td class="fieldrequired">'.$langs->trans('Namereport').'</td><td colspan="2">';
	print '<select id="name_report" name="name_report">'.$options.'</select>';
	print '</td></tr>';
...
else
{
	if ($_GET["id"])
	{
	  ...
	  //detail
			...
			print '</td></tr>';
	  //name_report
			print '<tr><td>'.$langs->trans('Namereport').'</td><td colspan="2">';
			print $object->name_report;
			print '</td></tr>';
