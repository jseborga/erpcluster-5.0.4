...
// Ajout entrepot
if ($action == 'addunit'  && $user->rights->fabrication->crearlistproduct)
  {
...
if ($action == 'addlines' && $user->rights->fabrication->crearlistproduct)
  {
    ...
    $object->statut            = 1;
    if ($object->fk_product_father > 0 && $object->fk_product_son > 0)
      {
...
if ($action == 'addlinesson' && $user->rights->fabrication->crearlistproduct)
  {
    ...
    $objpunit->fetch('',GETPOST('id'));
    //agregando
    $object->fk_product_father = GETPOST("id");
    $object->fk_unit_father    = $objpunit->fk_unit;
    $object->qty_father        = 1;
    $object->entity            = $conf->entity;
    $object->fk_product_son    = GETPOST("idprod1");
    $object->fk_unit_son       = GETPOST("fk_unit_son");
      ...
      if ($res > 0)
	{
      ...
      $mesg='<div class="error">'.$langs->trans("ErrorRefRequired").'</div>';
      $action="createnew";   // Force retour sur page creation
    }
}
  ...
  if ($action != 'editline')
    {
      ...
      else
  	{
  	  ...
  	  if (! empty($conf->product->enabled) || ! empty($conf->service->enabled))
  	    {
  	      $var=!$var;
  	      $object->formAddPredefinedProduct_lm(0,$mysoc,$soc,$hookmanager);
  	    }
  	}
  ...
  if ($_GET["id"])
    {
      ...
      $result = $objproductf->fetch($_GET["id"]);
      //buscamos la unidad
	  ...
	  if ($conf->almacen->enabled)
	    {
	      ...
	      if ($objpunit->fk_product == $objproductf->id)
		{
		  $objunit->fetch($objpunit->fk_unit);
		  if ($objunit->id == $objpunit->fk_unit)
		    print $objunit->ref." - ".$objunit->description;
		  else
		    print 'Err';
		}
	      else
		{
		  if ($user->rights->fabrication->crearlistproduct)
		    include DOL_DOCUMENT_ROOT.'/almacen/tpl/addunit.tpl.php';
		  else
		    print 'None';
		}
	      print '</td></tr>';
	  ...
	  if ($action == 'createnew')
	      ...
	      else
		{
		  ...
		  if (! empty($conf->product->enabled) || ! empty($conf->service->enabled))
		    {
		      $var=!$var;
		      $objfabcom->formAddPredefinedProductSon(0,$mysoc,$soc,$hookmanager,$objproductf->id);
		    }
		}
	      
	      $parameters=array();
	      $reshook=$hookmanager->executeHooks('formAddObjectLine',$parameters,$objproductf,$action);    // Note that $action and $object may have been modified by hook
	    }
	  print '</table>';
	  print '</td>';
	  print '</tr>';
