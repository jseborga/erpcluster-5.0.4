 ...
 */
class pdf_outputalm extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Output Alm";
		$this->description = $langs->trans('PDFSalidaalmDescription');
	...
	*/
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		$objmouvementadd = new Stockmouvementaddext($db);
		...
		if ($conf->almacen->dir_output)
		{
			...
			$aData = $object->fetchlinesadd(" AND a.fk_stock_mouvement_doc = ".$object->id);
			...
			if (file_exists($dir))
			{
				...
				if ($object->label)
				{
					...
					$pdf->writeHTMLCell(190, 3, $this->posxnro-1, $tab_top-2, $langs->trans('Note'), 0, 1);
					$pdf->writeHTMLCell(0, 3, $this->posxnro+10, $tab_top-2, dol_htmlentitiesbr($object->label), 0, 1);
					$nexY = $pdf->GetY();
				...
				//for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$desc = dol_trunc($product->label,50);
					$pdf->SetXY($this->posxdesc, $curY);
					$pdf->MultiCell($this->posxsaa-$this->posxdesc-1, 3, $desc, 0, 'L',0);


					// unit
					//$productunit->fetch('',$lines->fk_product);
					$unit = $langs->trans($product->getLabelOfUnit('short'));
					$pdf->SetXY($this->posxuni, $curY);
					$pdf->MultiCell($this->posxsen-$this->posxuni-1, 3, $unit, 0, 'L',0);

					// quant
					$sent = abs($lines->value);
					$pdf->SetXY($this->posxsen, $curY);
					$pdf->MultiCell($this->posxbal-$this->posxsen-1, 3, $sent, 0, 'L',0);

					// pu
					//buscamos el movimiento real
					$pu = $lines->price;
					$bal = price(price2num($sent*$lines->price,'MT'));
					//if ($lines->origintype == 'stock_mouvement')
					//{
						//$res = $objmouvement->fetch($lines->fk_origin);
						if (empty($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY))
						{
							$pu = $lines->price;
							echo 'bal '.$bal = price(price2num($sent*$pu,'MT'));
						}
						elseif ($conf->global->ALMACEN_METHOD_VALUATION_INVENTORY == 1)
						{
							//$objmouvementadd->fetch(0,$lines->fk_origin);
							//$pu = $objmouvementadd->value_peps;
							$pu = $lines->value_peps;
							$bal = price(price2num($sent*$pu,'MT'));
						}
					//}

					$pdf->SetXY($this->posxpu, $curY);
					$pdf->MultiCell($this->posxbal-$this->posxpu-1, 3, price(price2num($pu,'MT')), 0, 'R',0);

					// balance
					$pdf->SetXY($this->posxbal, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxbal, 3, $bal, 0, 'R',0);

					$sumValor += price2num($sent*$pu,'MT');
				...
				if ($pagenb == 1)
				{
					$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforfooter-20, 0, $outputlangs, 0, 0);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
					pdf_pie_page_mod11($pdf, $object, $posy, $outputlangs,$this->marge_gauche, $this->page_largeur,$this->marge_droite,$this->page_hauteur,$this->type_page,$bottomlasttab-10);
				}
				else
				{
					$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter-20, 0, $outputlangs, 0, 0);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
					pdf_pie_page_mod11($pdf, $object, $posy, $outputlangs,$this->marge_gauche, $this->page_largeur,$this->marge_droite,$this->page_hauteur,$this->type_page,$bottomlasttab-10);
				}
	...
	*/
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("SALIDA DE ALMACEN");
		...
		$pdf->SetXY($posxx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Date")." : " . dol_print_date($object->datem,"day",false,$outputlangs), '', 'R');
		//de
		if ($object->fk_entrepot_from)
		{
			$entrepot->fetch($object->fk_entrepot_from);
			$posy+=4;
			$pdf->SetXY($posxx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("De")." : " . $entrepot->libelle, '', 'R');
		}
		//a
		if ($object->fk_entrepot_to)
		{
			$entrepot->fetch($object->fk_entrepot_to);
			$posy+=4;
			$pdf->SetXY($posxx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("De")." : " . $entrepot->libelle, '', 'R');
		...
		$pdf->SetXY($posxx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Label")." : " . $object->label, '', 'R');
