...
if (empty($reshook))
{
					...
					if ($object_id > 0)
					{
						...
						if ($result > 0)
						{
							...
							for($i = 0; $i < $num; $i ++)
							{
								...
								if ($conf->fiscal->enabled)
								{
									...
									if ($conf->global->PRICE_TAXES_INCLUDED)
									{
										$price_base_type = 'TTC';
										$pu = $lines[$i]->price;
										if (empty($pu)) $pu = price2num($lines[$i]->total_ttc / $lines[$i]->qty);
										$lines[$i]->price = $pu;
									}
									$fk_unit = $lines[$i]->fk_unit;
									$discount = $lines[$i]->remise+0;
									...
									$result = $object->addlineadd($desc, $lines[$i]->subprice, $lines[$i]->qty, $lines[$i]->tva_tx, $lines[$i]->localtax1_tx, $lines[$i]->localtax2_tx, $lines[$i]->fk_product, $lines[$i]->remise_percent, $lines[$i]->info_bits, $lines[$i]->fk_remise_except, $price_base_type, $lines[$i]->price, $date_start, $date_end, $product_type, $lines[$i]->rang, $lines[$i]->special_code, $fk_parent_line, $lines[$i]->fk_fournprice, $lines[$i]->pa_ht, $label, $array_options, $lines[$i]->fk_unit, $object->origin,$lines[$i]->rowid,$lines[$i]);
								}
								else
								{
							...
							if (!$error && $conf->fiscal->enabled)
							{
								...
								$res = updatetotal_commande($object_id);
							}
	...
	else if ($action == 'addline' && $user->rights->commande->creer)
	{
		...
		if (empty($price_ht)) $price_ht = GETPOST('price_ttc');
		...
		if (! $error && ($qty >= 0) && (! empty($product_desc) || ! empty($idprod))) {
			...
			if ($conf->fiscal->enabled)
			{
				...
				$result = $object->addlineadd($desc, $lines->subprice, $qty, $lines->tva_tx, $lines->localtax1_tx, $lines->localtax2_tx, $idprod, $lines->remise_percent, $info_bits, 0, $price_base_type, $lines->price, $date_start, $date_end, $type, - 1, 0, GETPOST('fk_parent_line'), $fournprice, $buyingprice, $label, $array_options, $fk_unit, $origin,$originid,$lines);
			}
			else
			{
	...
	else if ($action == 'updateline' && $user->rights->commande->creer && GETPOST('save'))
	{
		...
		if (! $error)
		{
			if ($conf->fiscal->enabled)
			{
				...
				$result = $object->updatelineadd(GETPOST('lineid'), $description, $lines->subprice, $qty, $lines->remise_percent, $lines->txtva, $lines->localtax1_tx, $lines->localtax2_tx, $price_base_type, $info_bits, $date_start, $date_end, $type, GETPOST('fk_parent_line'), 0, $fournprice, $buyingprice, $label, $special_code, $array_options, $fk_unit,$lines);
			}
			else
			{
		...
		)
	{
		...
		if (! $error) {
			$result = $object->valid($user, $idwarehouse,1);
...
print '</tr>' . "\n";
	...
	if ($object->id > 0) {
		...
		$head = sales_commande_prepare_head($object);
		...
		*/
		...
		$linkback = '<a href="' . DOL_URL_ROOT . '/sales/commande/list.php' . (! empty($socid) ? '?socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
		...
		if (!empty($conf->incoterm->enabled))
		{
			...
			if ($user->rights->commande->creer) print '<a href="'.DOL_URL_ROOT.'/sales/commande/card.php?id='.$object->id.'&action=editincoterm">'.img_edit().'</a>';
			else print '&nbsp;';
			print '</td></tr></table>';
			print '</td>';
			print '<td colspan="3">';
			...
			if (empty($reshook)) {
				...
				if ($conf->ficheinter->enabled) {
					...
					if ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED && $object->getNbOfServicesLines() > 0) {
						if ($user->rights->ficheinter->creer) {
							print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/sales/fichinter/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans('AddIntervention') . '</a></div>';
						} else {
				...
				if ($conf->contrat->enabled && ($object->statut == Commande::STATUS_VALIDATED || $object->statut == Commande::STATUS_ACCEPTED || $object->statut == Commande::STATUS_CLOSED)) {
					...
					if ($user->rights->contrat->creer) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/sales/contrat/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans('AddContract') . '</a></div>';
					}
				}
				...
				if ($object->statut > Commande::STATUS_DRAFT && ! $object->billed) {
					if (! empty($conf->facture->enabled) && $user->rights->facture->creer && empty($conf->global->WORKFLOW_DISABLE_CREATE_INVOICE_FROM_ORDER)) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/sales/compta/facture.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("CreateBill") . '</a></div>';
					}
					if (! empty($conf->facture->enabled) && $user->rights->facture->creer && empty($conf->global->WORKFLOW_DISABLE_CREATE_INVOICE_FROM_ORDER)) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/sales/compta/shippingtoinvoice.php?socid=' . $object->socid . '&shipping=1">' . $langs->trans("Shippinginvoice") . '</a></div>';
					}
