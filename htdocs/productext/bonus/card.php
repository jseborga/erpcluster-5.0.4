...
$parameters=array();
$reshook=$hookmanager->executeHooks('doActions',$parameters,$objProductbonus,$action);    // Note that $action and $object may have been modified by some hooks
...
if (empty($reshook))
{
	if ($cancel)
	{
		...
		if ($id > 0 || ! empty($ref)) $ret = $objProductbonus->fetch($id,$ref);
		$action='';
	}
	...
	if ($action == 'add')
	{
		...
		$objProductbonus->fk_product=$id;
		$objProductbonus->ref=GETPOST('ref','alpha');
		$objProductbonus->label=GETPOST('label','alpha');
		$objProductbonus->fk_bonus_type=GETPOST('fk_bonus_type','int');
		$objProductbonus->type_value=GETPOST('type_value','alpha');
		$objProductbonus->active=GETPOST('active','int');
		$objProductbonus->fk_user_create=$user->id;
		$objProductbonus->fk_user_mod=$user->id;
		$objProductbonus->datec=$now;
		$objProductbonus->datem=$now;
		$objProductbonus->tms=$now;
		$objProductbonus->status=0;

		if (empty($objProductbonus->ref))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Ref")), null, 'errors');
		}
		if (empty($objProductbonus->label))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Label")), null, 'errors');
		}
		if ($objProductbonus->fk_bonus_type<=0)
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Fieldfk_bonus_type")), null, 'errors');
		}

		if (! $error)
		{
			$result=$objProductbonus->create($user);
			if ($result > 0)
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/productext/bonus/card.php?id='.$id.'&idr='.$result,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
				// Creation KO
				if (! empty($objProductbonus->errors)) setEventMessages(null, $objProductbonus->errors, 'errors');
				else  setEventMessages($objProductbonus->error, null, 'errors');
				$action='create';
			}
		}
		else
	...
	if ($action == 'update')
	{
		...
		if (! $error)
		{
			$result=$objProductbonus->update($user);
			if ($result > 0)
			{
				$action='view';
			}
	...
	{
		$result=$objProductbonus->delete($user);
		if ($result > 0)
		{
			// Delete OK
			setEventMessages("RecordDeleted", null, 'mesgs');
			header("Location: ".dol_buildpath('/productext/bonus/card.php?id='.$id,1));
			exit;
		}
		else
		{
	...
	{
		$objProductbonus->status = 1;
		$objProductbonus->fk_user_val = $user->id;
		$objProductbonus->datem = $now;
		$objProductbonus->datev = $now;
		$objProductbonus->tms = $now;
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/productext/bonus/card.php?id='.$id.'&idr='.$idr,1));
			exit;
		}
		else
		{
...
{
	require_once DOL_DOCUMENT_ROOT.'/productext/bonus/tpl/list.tpl.php';
}
else
{
	require_once DOL_DOCUMENT_ROOT.'/productext/bonus/tpl/card.tpl.php';
}
