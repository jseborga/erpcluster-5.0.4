...
if ($id > 0 || ! empty($ref))
{
	$ret = $object->fetch($id,is_null($ref)?null:$ref);	
...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->projet->creer)
	{
		...
		if (! $error)
		{
			...
			if (!$error)
			{
				//creamos las tareas por defecto si esta habilitado
				if ($conf->global->MONPROJET_CREATE_TASK_DEFAULT)
				{
					$aTasks = explode('|',$conf->global->MONPROJET_CREATE_TASK_DEFAULT);
					foreach ((array) $aTasks AS $j => $aTask)
					{
						include DOL_DOCUMENT_ROOT.'/monprojet/include/taskdefault.inc.php';
					}
				}
			}
			if (GETPOST('origin') && GETPOST('originid'))
			{
				//actualizamos el contrato con el proyecto creado
				$contrat = new Contrat($db);
				$contrat->fetch(GETPOST('originid'));
				$contrat->setProject($object->id);
			}
			if (! $error)
			{
				$db->commit();
...
if ($action == 'create' && $user->rights->projet->creer)
{
	 ...
	 * Create
	 */
	if (GETPOST('originid') && GETPOST('origin'))
	{
		if (GETPOST('origin') == 'contrat')
		{
			$element = 'contrat';
			$subelement = 'contrat';
		}
		...
		$extralabelsc = $extrafields->fetch_name_optionals_label($objectsrc->table_element);

		$objectsrc->fetch(GETPOST('originid'));
		$objectsrc->fetch_optionals($objectsrc->id,$extralabelsc);
		...
		$socid = $soc->id;
		$_GET['title'] = $objectsrc->note_public;
		$date_start = '';
		$date_end = '';

		foreach ($objectsrc->lines AS $j => $line)
		{
			if (empty($date_start)) $date_start = $line->date_ouverture_prevue;
			if (empty($date_end)) $date_end = $line->date_fin_validite;
			if ($line->date_ouverture_prevue <= $date_start) $date_start = $line->date_ouverture_prevue;
			if ($line->date_fin_validite >= $date_end) $date_end = $line->date_fin_validite;
		}
	}
