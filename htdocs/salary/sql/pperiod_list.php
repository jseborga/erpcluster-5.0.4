...
/**
 *   	\file       salary/pperiod_list.php
 *		\ingroup    salary
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-01-06 16:27
 */
...
dol_include_once('/salary/class/pperiod.class.php');
...
$search_mes=GETPOST('search_mes','int');
...
$hookmanager->initHooks(array('pperiodlist'));
...
't.mes'=>array('label'=>$langs->trans("Fieldmes"), 'checked'=>1),
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_period_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_mes) $sql.= natural_search("mes",$search_mes);
...
if ($search_mes != '') $params.= '&amp;search_mes='.urlencode($search_mes);
if ($search_anio != '') $params.= '&amp;search_anio='.urlencode($search_anio);
...
if (! empty($arrayfields['t.mes']['checked'])) print_liste_field_titre($arrayfields['t.mes']['label'],$_SERVER['PHP_SELF'],'t.mes','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.mes']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_mes" value="'.$search_mes.'" size="10"></td>';
