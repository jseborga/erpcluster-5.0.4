...
if (!$user->rights->almacen->transfin->write) accessforbidden();
...
$lTransfer = false;
if (count($itemTransf) >0) $lTransfer = true;
if ($cancel) $action='';
if ($cancel)
{
...
if ($action == "confirm_transfert_stock" && $_REQUEST['confirm'] == 'yes')
{
	...
	if (! $error)
	{
		if ($idr <> $idd)
		{
			...
			$objectdoc->fk_source = $transf['fk_source']+0;
...
if ($action == 'transfert_stock')
{
	...
	$transf['fk_source'] = GETPOST('fk_source');
...
if ($action == "create" || $action=='endt' || $action == 'edit')
{
	...
	print_fiche_titre($langs->trans('CreateMovementEntry').' '.$period_year);
	...
	if ($action == 'create' && $lAddnew)
	{
		...
		print '<input type="text" style="border:none;" size="80%" id="labelproduct" name="labelproduct" value="" readonly>';
		print '</td>';
		$var= !$var;
		print "<tr $bc[$var]>";
		print '<td>'.$langs->trans('Qty');
		print '</td>';
		print '<td>';
		print '<input type="number" id="nbpiece" min="0" step="any" name="nbpiece" value="'.GETPOST("nbpiece").'" required>';
		...
		{
			print '<td>'.$langs->trans("P.U.").' '.$langs->trans('Neto').'</td>';
			print '<td>';
			print '<input type="number" name="price" step="any" min="'.$min.'" value="'.GETPOST("price").'" required>';
			print '</td>';
		}
	...
	if ($user->rights->almacen->transfin->write && $action != 'endt')
	{
	...
	if (count($aItemRow)>0 && $action=="endt")
	{
		...
		print '<tr>';
		print '<td width="10%" class="fieldrequired">'.$langs->trans("WarehouseEntryshort").'</td><td>';
		print $formproduct->selectWarehouses(($_GET["dwid"]?$_GET["dwid"]:(GETPOST('id_entrepot_source')?GETPOST('id_entrepot_source'):$idEntrepotdefault)),'id_entrepot_source','',1,0,0,'',0,0,array(),'',$aExcluded);
		print '</td>';
		print '</tr>';
		print '<tr>';
		...
		{
			print '<tr>';
			print '<td>'.$langs->trans('Departamentshort').'/'.$langs->trans("Executing Units short").'</td>';
			print '<td colspan="4">';
		...
		{
			print '<tr>';
			print '<td>'.$langs->trans('Foundingsourceshort').'</td>';
			print '<td colspan="4">';
			print $form->select_founding_source(GETPOST('fk_source'),'fk_source','',0,1,'');
			print '</td>';
			print '</tr>';
		}
		print '<tr>';
		...
		print '<td colspan="4">';
		print '<input type="text" name="label" size="25" value="'.GETPOST("label").'">';
		print '</td>';
		print '</tr>';
