...
/**
 *   	\file       salary/psalaryhistory_list.php
 *		\ingroup    salary
...
$hookmanager->initHooks(array('psalaryhistorylist'));
...
't.fk_salary_present'=>array('label'=>$langs->trans("Fieldfk_salary_present"), 'checked'=>1),
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_salary_history_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_fk_salary_present) $sql.= natural_search("fk_salary_present",$search_fk_salary_present);
...
if ($search_fk_salary_present != '') $params.= '&amp;search_fk_salary_present='.urlencode($search_fk_salary_present);
...
if (! empty($arrayfields['t.fk_salary_present']['checked'])) print_liste_field_titre($arrayfields['t.fk_salary_present']['label'],$_SERVER['PHP_SELF'],'t.fk_salary_present','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_salary_present']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_salary_present" value="'.$search_fk_salary_present.'" size="10"></td>';
