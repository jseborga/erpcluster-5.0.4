...
// Ajout entrepot
if ($action == 'add' && $user->rights->purchase->conv->crear)
{
	//creamos
...
// Delete warehouse
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->purchase->conv->del)
{
...
if ($id)
{
	...
	print $formconfirm;
	dol_fiche_head();
	if ($user->rights->purchase->conv->crear)
	{
	...
	foreach ((array) $object->lines AS $i => $line)
	{
		...
		else
		{
			...
			if ($user->rights->purchase->conv->del)
			{
	...
	if ($user->rights->purchase->conv->crear && $action != 'edit')
	{
		print '<tr>';
		print '<td>';
		if (! empty($conf->global->PRODUCT_USE_UNITS))
