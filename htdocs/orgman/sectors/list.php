...
/**
 *   	\file       poa/cpartida_list.php
 *		\ingroup    poa
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-31 10:52
 */
...
dol_include_once('/orgman/class/csectorsext.class.php');
...
$id			= GETPOST('id','int');
$code_father		= GETPOST('code_father','alpha');
...
$search_sigla=strtoupper(GETPOST('search_sigla','alpha'));
$search_label=GETPOST('search_label','alpha');
$search_detail=strtoupper(GETPOST('search_detail','alpha'));
$search_code_father=GETPOST('search_code_father','int');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'poasectorslist';
...
$arrayfields=array(
	...
	't.code_father'=>array('label'=>$langs->trans("Fieldcode_father"), 'checked'=>1),
...
$object=new Csectorsext($db);
if (($id > 0 || ! empty($ref)) && $action != 'add')
{
	$result=$object->fetch($id,$ref);
	if ($result < 0) dol_print_error($db);
}
$objecttmp = new Csectors($db);

// Initialize technical object to manage hooks of modules. Note that conf->hooks_modules contains array array
$hookmanager->initHooks(array('cpartida'));
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/sectors/list.php',1);
	...
	if ($action == 'add')
	{
		...
		$code_father = GETPOST('code_father');
		$res = $objecttmp->fetch(0,$code_father,$period_year);
		$object->entity=$conf->entity;
		$object->period_year=$period_year;
		$object->ref=GETPOST('ref','alpha');
		$object->sigla=GETPOST('sigla','alpha');
		$object->level = 1;
		if (!empty($code_father))
		{
			$object->ref=$code_father.'.'.GETPOST('ref','alpha');
			$object->level = $objecttmp->level +1;
		}
		$object->label=GETPOST('label','alpha');
		$object->detail=GETPOST('detail','alpha');
		$object->code_father=$code_father;
		$object->type=substr($object->code,0,1);
	...
	if ($action == 'update')
	{
		...
		if (empty($object->sigla))
		{
			$error++;
			setEventMessages($langs->transnoentitiesnoconv("ErrorFieldRequired",$langs->transnoentitiesnoconv("Sigla")), null, 'errors');
		}
		if (empty($object->label))
		{
			$error++;
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/sectors/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Clasificador de Sectores Económicos');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_sectors as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_partida_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE period_year = ".$period_year;
...
if ($search_code_father) $sql.= natural_search("code_father",$search_code_father);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/orgman/partida/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.code_father']['checked'])) print_liste_field_titre($arrayfields['t.code_father']['label'],$_SERVER['PHP_SELF'],'t.code_father','',$params,'',$sortfield,$sortorder);
...
if ($action != 'create' && $action != 'edit')
{
	...
	if (! empty($arrayfields['t.code_father']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_code_father" value="'.$search_code_father.'" size="10"></td>';
...
$var=true;
$totalarray=array();
$filterstatic = " AND t.period_year = ".$period_year;
$objecttmp->fetchAll('ASC', 'ref', 0, 0, array('entity'=>$conf->entity), 'AND',$filterstatic);
$options = '<option value="-1">'.$langs->trans('Selectfunctions').'</option>';
$indent = '';
$idFather = 0;
$aPartida = array();
$linesfather = $objecttmp->lines;
foreach ($objecttmp->lines AS $j => $line)
{
	$aPartida[$line->id] = $line->code;
	if ($line->code_father <=0) $indent = '';
	else
	{
		if ($idFather != $line->code_father)
		{
			$indent.= '&nbsp;&nbsp;';
			$idFather = $line->code_father;
		}
	}
	$selected = '';
	if ($action == 'edit' && $id == $line->id) $selected = ' selected ';
	if (GETPOST('code_father') == $line->sigla) $selected =' selected';
	$options.= '<option value="'.$line->id.'" '.$selected.'>'.$indent.$line->code.' - '.$line->label.'</option>';
}

if ($action == 'delete') {
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Deletesectors'), $langs->trans('ConfirmDeletesectors'), 'confirm_delete', '', 1, 2);
	print $formconfirm;
}
if ($action == 'create')
{
	...
	if (! empty($arrayfields['t.ref']['checked']))
	{
		$sigla = '';
		if (!empty($code_father))
			$sigla = GETPOST('code_father');
		print '<td nowrap>'.($sigla?$sigla.'.':'').'<input type="text" name="ref" value="'.GETPOST('ref').'" size="3">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.sigla']['checked']))
	{
		print '<td nowrap>'.'<input type="text" name="sigla" value="'.GETPOST('sigla').'" size="7">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	...
	if (! empty($arrayfields['t.code_father']['checked']))
	{
		if (!empty($code_father))
		{
			$res = $objecttmp->fetch(0,$code_father,$period_year);
			if ($res>0)
			{
				print '<td>'.$objecttmp->sigla.'-'.$objecttmp->label.'</td>';
				print '<input type="hidden" name="code_father" value="'.$code_father.'">';
			}
			else
				print '<td></td>';
	...
	print '&nbsp;<a href="'.$_SERVER['PHP_SELF'].'">'.img_picto($langs->trans('Return'),DOL_URL_ROOT.'/orgman/img/return','',1).'</a>';
	print '</td>';
	print '</tr>';
}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		$object->id = $obj->rowid;
		$object->ref = $obj->ref;
		$object->sigla = $obj->sigla;
		$object->label = $obj->label;
		$filterfather = " AND t.code_father = '".$obj->ref."'";
		$resson = $objecttmp->fetchAll('','',0,0,array(1=>1),'AND',$filterfather);
		...
		if ($action == 'edit' && $id == $obj->rowid)
		{
			...
			if (! empty($arrayfields['t.ref']['checked']))
			{
				print '<td nowrap>'.($obj->code_father?$obj->code_father.'.':'').'<input type="text" name="ref" value="'.substr($obj->ref,strlen($obj->code_father)+1,30).'" size="3">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.sigla']['checked']))
			{
				print '<td nowrap>'.'<input type="text" name="sigla" value="'.$obj->sigla.'" size="7">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked']))
			{
			...
			if (! empty($arrayfields['t.code_father']['checked']))
			{
				...
				foreach ($linesfather AS $j => $line)
				{
					...
					$selected = '';
					if ($action == 'edit' && $obj->code_father == $line->ref) $selected = ' selected ';
					$options.= '<option value="'.$line->ref.'" '.$selected.' title="'.$line->label.'">'.$indent.$line->ref.'-'.$line->sigla.' - '.dol_trunc($line->label,40).'</option>';
				}

				print '<td>'.'<select name="code_father">'.$options.'</select>'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.type']['checked']))
			{
		...
		else
		{
			...
			{
				$act = '';
				if ($user->rights->orgman->sec->write) $act = '&action=edit';
				print '<td>'.$object->getNomUrladd(1,'', 0, 24, '',$act).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.sigla']['checked']))
			{
			...
			if (! empty($arrayfields['t.code_father']['checked']))
			{
				if (!empty($obj->code_father))
				{
					$objecttmp->fetch(0,$obj->code_father,$obj->period_year);
		...
		// Action column
			...
			{
				if ($user->rights->orgman->sec->del && !$resson)
					print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$obj->rowid.'&action=delete">'.img_picto($langs->trans('Delete'),'delete').'</a>';
				if ($user->rights->orgman->sec->write)
					print '<a href="'.$_SERVER['PHP_SELF'].'?code_father='.$obj->ref.'&action=create">'.img_picto($langs->trans('New(ao)').' '.$objCname->label,'rightarrow').'</a>';
			}
...
if (empty($reshook))
{
	if ($user->rights->orgman->fun->write)
	{
