...
class pdf_salesareaadd extends ModelePDFContab
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->contab->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->contab->dir_output."/contab/".$period_year.'/salesareaadd';
				$file = $dir . "/report_".$object->refDepartament.".pdf";
			}
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					$pdf->MultiCell($this->posxlabe-$this->posxrefe-1, 1, $line['label'], 0, 'C',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxlabe, $curY);
					$pdf->MultiCell($this->posxfech-$this->posxlabe-1, 1, dol_print_date($line['fecha'],'day'), 0, 'R',0);
					$aPosY[] =$pdf->GetY();
					$pdf->SetXY($this->posxfech, $curY);
					$pdf->MultiCell($this->posxunid-$this->posxfech-1, 1,$line['unidad'], 0, 'R',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxunid, $curY);
					$pdf->MultiCell($this->posxresp-$this->posxunid-1, 1, $line['respon'], 0, 'R',0);
					$aPosY[] = $pdf->GetY();
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxresp-$this->posxunid-1,2, $outputlangs->transnoentities("Solicitante"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('De')." : " . dol_print_date($aReporte[2],'day'), '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('A')." : " . dol_print_date($aReporte[3],'day'), '', 'R');

		//Dato
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $langs->trans('Departament')." : " . $aReporte[4], '', 'R');
	...
	function pdf_pie_total(&$pdf, $object, $posy, $outputlangs,$marge_gauche, $page_largeur,$marge_droite,$page_hauteur,$type_page=0,$bottomlasttab)
	{
		...
		$pdf->SetFont('times', '',10);
			...
			$pdf->SetXY($this->posxunid, $curY);
			$pdf->MultiCell($this->posxresp-$this->posxunid-1, 1, $langs->trans("Total")." : ", 1, 'R',0);
			$pdf->SetXY($this->posxresp, $curY);
