 ...
 * @see CommonObject
 */
class Mjobs extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'mjobs';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'm_jobs';
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_work_request)) {
			 $this->fk_work_request = trim($this->fk_work_request);
		}
		...
		if (isset($this->fk_departament_assign)) {
			 $this->fk_departament_assign = trim($this->fk_departament_assign);
		}
		if (isset($this->fk_user_assign)) {
			 $this->fk_user_assign = trim($this->fk_user_assign);
		}
		if (isset($this->speciality_assign)) {
			 $this->speciality_assign = trim($this->speciality_assign);
		}
		if (isset($this->description_assign)) {
			 $this->description_assign = trim($this->description_assign);
		}
		...
		if (isset($this->speciality_job)) {
			 $this->speciality_job = trim($this->speciality_job);
		}
		if (isset($this->typemant)) {
			 $this->typemant = trim($this->typemant);
		}
		if (isset($this->description_job)) {
			 $this->description_job = trim($this->description_job);
		}
		if (isset($this->group_task)) {
			 $this->group_task = trim($this->group_task);
		}
		if (isset($this->task)) {
		...
		if (isset($this->image_fin)) {
			 $this->image_fin = trim($this->image_fin);
		}
		...
		$sql .= ' '.(! isset($this->fk_departament_assign)?'NULL':$this->fk_departament_assign).',';
		...
		$sql .= ' '.(! isset($this->group_task)?'NULL':$this->group_task).',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fk_departament_assign = $obj->fk_departament_assign;
				...
				$this->group_task = $obj->group_task;
				$this->task = $obj->task;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->fk_work_request = $obj->fk_work_request;
				...
				$line->fk_departament_assign = $obj->fk_departament_assign;
				$line->fk_user_assign = $obj->fk_user_assign;
				$line->date_assign = $this->db->jdate($obj->date_assign);
				$line->speciality_assign = $obj->speciality_assign;
				$line->description_assign = $obj->description_assign;
				...
				$line->speciality_job = $obj->speciality_job;
				$line->typemant = $obj->typemant;
				$line->description_job = $obj->description_job;
				$line->group_task = $obj->group_task;
				$line->task = $obj->task;
				$line->image_ini = $obj->image_ini;
				$line->image_fin = $obj->image_fin;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_departament_assign = '.(isset($this->fk_departament_assign)?$this->fk_departament_assign:"null").',';
		...
		$sql .= ' group_task = '.(isset($this->group_task)?$this->group_task:"null").',';
		$sql .= ' task = '.(isset($this->task)?$this->task:"null").',';
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Workorder") . '</u>';
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;

        $url = DOL_URL_ROOT.'/mant/jobs/'.'card.php?id='.$this->id;
