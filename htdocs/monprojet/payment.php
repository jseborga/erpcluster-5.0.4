...
$taskresource = new Proyecttaskresource($db);
	...
	foreach ((array) $arraydeduc AS $j => $data)
	{
		...
		if ($user->rights->monprojet->payp->mod)
		{
			...
			else
			{
				...
				{
					if (!$idd)
						print price($objpayde->amount).' '.'<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idd='.$objpayde->id.'&action=editd">'.img_picto($langs->trans('Edit'),'edit').'</a>';
				}
				else
				{
					if (!$code)
...
if ($idp)
{
	...
	// Fields title
		...
		if ($user->rights->monprojet->payp->rep && $objpay->statut == 2)
			print '<a class="butAction" href="'.DOL_URL_ROOT.'/monprojet/tasksexcel.php'.'?id='.$id.'&idpay='.$objpay->id.'">'.$langs->trans("Excel").'</a>';
		else
