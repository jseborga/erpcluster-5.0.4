...
$search_fk_origin=GETPOST('search_fk_origin','alpha');
$search_percent_origin=GETPOST('search_percent_origin','int');
...
$arrayfields=array(
	...
	't.amount_noprod'=>array('label'=>$langs->trans("Fieldamountnoprod"), 'align'=>'align="right"', 'checked'=>1),
	't.amount'=>array('label'=>$langs->trans("Fieldamountprod"), 'align'=>'align="right"', 'checked'=>1),
	't.work_hours'=>array('label'=>$langs->trans("Fieldtotal"), 'align'=>'align="right"', 'checked'=>1),
	...
	//'t.status'=>array('label'=>$langs->trans("Fieldstatus"), 'align'=>'align="right"', 'checked'=>1),
...
//vamos a buscar y sumar cuanto se utilizao por cada recursos
$sql = " SELECT t.rowid FROM ".MAIN_DB_PREFIX."budget_task AS t WHERE t.fk_budget = ".$object->id;

$filter = " AND t.fk_budget_task IN (".$sql.")";
...
if ($res > 0)
{
	...
	{
		$aSumquant[$line->fk_product_budget]+= $line->quant;
		//$aProductitem[$line->fk_product_budget][$line->fk_budget_task]['quant']+=$line->quant;
		//buscamos el item del proyecto
		$objBudgettask->fetch($line->fk_budget_task);
		$aProductitem[$line->fk_product_budget][$line->fk_budget_task]['quant']+=$line->quant;
		$aProductitem[$line->fk_product_budget][$line->fk_budget_task]['ref']=$objBudgettask->ref;
	}
}
...
if (! empty($arrayfields['t.performance']['checked']))
{
	//print '<td class="liste_titre" '.$arrayfields['t.performance']['align'].'><input type="text" class="flat" name="search_performance" value="'.$search_performance.'" size="10"></td>';
...
if (! empty($arrayfields['t.amount']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.amount']['align'].'></td>';
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'status')
				{
					$objProductbudget->status = $obj->$key2;
					$obj->$key2 = $objProductbudget->getLibStatut(6);
				}
				if ($key2 == 'performance')
				{
					$cLink = '';
					if (is_array($aProductitem[$obj->rowid]) && count($aProductitem[$obj->rowid])>0)
					{
						foreach ($aProductitem[$obj->rowid] AS $k => $aLink)
						{
							if (!empty($cLink)) $cLink.='; ';
							$cLink.= $aLink['ref'].' = '.$aLink['quant'];
						}
					}
					$obj->$key2 = '<a href="" title="'.$cLink.'" >'.$aSumquant[$obj->rowid].'</a>';
				}
				...
				if ($key2 == 'work_hours')
				{
					$nprod = price2num($aSumquant[$obj->rowid] * $obj->percent_prod * $obj->amount / 100,$general->decimal_total);
					//$nprod = price2num($lineb->quant * $lineb->percent_prod * $lineb->amount / 100,$general->decimal_total);
					$nnprod = price2num($aSumquant[$obj->rowid] * (100-$obj->percent_prod) * $obj->amount_noprod / 100,$general->decimal_total);
...
if ($user->rights->budget->budr->fact)
{
	print '<div class="tabsAction">';
	print '<a class="butAction" href="' . dol_buildpath('/budget/budget/resources.php?id='.$id,1).'&action=changefactor">' . $langs->trans('Changepricesbyfactor') . '</a>';
	print '</div>';
}
...
print '<form id="formidr" name="formidr"  action="'.DOL_URL_ROOT.'/budget/budget/resources.php'.'" method="POST">';
...
print '<div class="modal-content">';
print '<form id="formid" name="formid" action="'.DOL_URL_ROOT.'/budget/budget/resources.php'.'" method="POST">';
print '<input type="hidden" name="id" value="'.$id.'">';
print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
