...
/**
 * \file    mant/mjobsorder.class.php
 * \ingroup mant
...
class Mjobsorder extends CommonObject
{
	 ...
	 */
	public $element = 'mjobsorder';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'm_jobs_order';

	/**
	 * @var MjobsorderLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("mjobsorder", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new MjobsorderLine();

				$line->id = $obj->rowid;
				
				$line->fk_jobs = $obj->fk_jobs;
				$line->fk_product = $obj->fk_product;
				$line->order_number = $obj->order_number;
