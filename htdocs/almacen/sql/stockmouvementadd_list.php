...
/**
 *   	\file       almacen/stockmouvementadd_list.php
 *		\ingroup    almacen
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-30 11:27
 */
...
dol_include_once('/almacen/class/stockmouvementadd.class.php');
...
$search_fk_stock_mouvement_doc=GETPOST('search_fk_stock_mouvement_doc','int');
...
$search_value_peps=GETPOST('search_value_peps','alpha');
$search_value_ueps=GETPOST('search_value_ueps','alpha');
...
$hookmanager->initHooks(array('stockmouvementaddlist'));
...
$object=new Stockmouvementadd($db);
...
't.fk_stock_mouvement_doc'=>array('label'=>$langs->trans("Fieldfk_stock_mouvement_doc"), 'checked'=>1),
...
't.value_peps'=>array('label'=>$langs->trans("Fieldvalue_peps"), 'checked'=>1),
't.value_ueps'=>array('label'=>$langs->trans("Fieldvalue_ueps"), 'checked'=>1),
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."stock_mouvement_add_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_stock_mouvement) $sql.= natural_search("fk_stock_mouvement",$search_fk_stock_mouvement);
if ($search_fk_stock_mouvement_doc) $sql.= natural_search("fk_stock_mouvement_doc",$search_fk_stock_mouvement_doc);
...
if ($search_value_peps) $sql.= natural_search("value_peps",$search_value_peps);
if ($search_value_ueps) $sql.= natural_search("value_ueps",$search_value_ueps);
...
if ($search_fk_stock_mouvement_doc != '') $params.= '&amp;search_fk_stock_mouvement_doc='.urlencode($search_fk_stock_mouvement_doc);
...
if ($search_value_peps != '') $params.= '&amp;search_value_peps='.urlencode($search_value_peps);
if ($search_value_ueps != '') $params.= '&amp;search_value_ueps='.urlencode($search_value_ueps);
...
if (! empty($arrayfields['t.fk_stock_mouvement_doc']['checked'])) print_liste_field_titre($arrayfields['t.fk_stock_mouvement_doc']['label'],$_SERVER['PHP_SELF'],'t.fk_stock_mouvement_doc','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.value_peps']['checked'])) print_liste_field_titre($arrayfields['t.value_peps']['label'],$_SERVER['PHP_SELF'],'t.value_peps','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.value_ueps']['checked'])) print_liste_field_titre($arrayfields['t.value_ueps']['label'],$_SERVER['PHP_SELF'],'t.value_ueps','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_stock_mouvement_doc']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_stock_mouvement_doc" value="'.$search_fk_stock_mouvement_doc.'" size="10"></td>';
...
if (! empty($arrayfields['t.value_peps']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_value_peps" value="'.$search_value_peps.'" size="10"></td>';
if (! empty($arrayfields['t.value_ueps']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_value_ueps" value="'.$search_value_ueps.'" size="10"></td>';
