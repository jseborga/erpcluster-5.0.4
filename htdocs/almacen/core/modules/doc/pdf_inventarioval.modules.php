 ...
 */
class pdf_inventarioval extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
	...
	// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		...
		if ($conf->almacen->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$heightforfooter = $this->marge_basse + 20;	//2  Height reserved to output the footer (value include bottom margin)
				...
				foreach ((array) $inventory AS $i => $lines)
				{
					...
					$pdf->MultiCell($this->posxsaa-$this->posxuni-1, 3, $unit, 0, 'L',0);
					// balance fisico
					$bal = $lines['balance'];
					$pdf->SetXY($this->posxsal, $curY);
					$pdf->MultiCell($this->posxsalv-$this->posxsal-1, 3, price(price2num($bal,'MT')), 0, 'R',0);
	...
	*/
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		// Output Rect
		$this->printRect($pdf,$this->marge_gauche, $tab_top-5, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $tab_height+5, $hidetop, $hidebottom);	// Rect prend une longueur en 3eme param et 4eme param
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxsal-$this->posxuni-1,2, $outputlangs->transnoentities("UM"),'','C');
		}
		$pdf->line($this->posxsal-1, $tab_top, $this->posxsal-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxsal-1, $tab_top+1);
			$pdf->MultiCell($this->posxsalv-$this->posxsal-1,2, $outputlangs->transnoentities("Saldo Fisico"),'','C');
		}
		$pdf->line($this->posxsalv, $tab_top, $this->posxsalv, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxsalv-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxsalv,2, $outputlangs->transnoentities("Saldo Valorado"),'','R');
		}
	...
	*/
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price(price2num($total_ttc,'MT')), 0, 'R', 1);
