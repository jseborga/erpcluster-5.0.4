...
class pdf_fractalrec extends ModelePDFFactures
{
	var $db;
	var $name;
	var $um;
  ...
  // Objet societe qui emet
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "fractalrec";
		$this->description = $langs->trans('Recibo');
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i=0; $i < count($objFactura->lines) ; $i++)
				{
					...
					$pdf->MultiCell($this->posxperi-$this->posxini-1,2, "Desde ".dol_print_date($objFactura->lines[$i]->date_start,'day')." Hasta ".dol_print_date($objFactura->lines[$i]->date_end,'day'),'','L');
					$nexYperi = $pdf->GetY();

					//Especificacion del Inmueble
					$pdf->SetXY($this->posxperi-1, $posy);
					$pdf->MultiCell($this->posxespe-$this->posxperi-1,2, $objFactura->lines[$i]->description,'','L');
					$nexYdesc = $pdf->GetY();

					//$sumaParcial = $objFactura->lines[$i]->price*$objFactura->lines[$i]->qty;//fk_unit
					$sumaParcial = $objFactura->lines[$i]->total_ttc;
					//Sub Total
					$pdf->SetXY($this->posxespe-1, $posy);
					$pdf->MultiCell($this->posxfin-$this->posxespe-1,2, price(price2num($objFactura->lines[$i]->total_ttc),'MT'),'','R');
					$sumaTotal = $sumaTotal + $sumaParcial;

					//if($nexYdesc > $posy ){
					if($nexYdesc > $nexYperi){
						$posy = $nexYdesc;
					}else{
						$posy = $nexYperi;
					}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		//Cabecera del Recibo
			$pdf->SetXY($this->posxini-4, 71);
		//$pdf->SetDrawColor(174, 214, 241);
			$pdf->SetFillColor(174, 214, 241);
		//$pdf->MultiCell(55, 5, '[LEFT] '.$txt, 1, 'L', 1, 0, '', '', true);
		//$pdf->MultiCell($this->marge_gauche + $this->marge_droite,2, $outputlangs->transnoentities("DESCRIPCION"),'','C', 1, 0, '', '', true);
			$pdf->MultiCell(190,2, $outputlangs->transnoentities("DESCRIPCION"),'','C', 1, 0, '', '', true);
			...
			$pdf->MultiCell($this->posxperi-$this->posxini-1,2, $outputlangs->transnoentities("PERIODO"),'','C');
			/*Linea de la columna*/
			$pdf->line($this->posxperi-1, 76, $this->posxperi-1, $tab_height+76);

			$pdf->SetXY($this->posxperi-1, 77);
			$pdf->MultiCell($this->posxespe-$this->posxperi-1,2, $outputlangs->transnoentities("ESPECIFICACION DEL INMUEBLE"),'','C');
			/*Linea de la columna*/
			$pdf->line($this->posxespe-1, 76, $this->posxespe-1, $tab_height+76);

			$pdf->SetXY($this->posxespe-1, 77);
			$pdf->MultiCell($this->posxfin-$this->posxespe-1,2, $outputlangs->transnoentities("SUBTOTAL"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$title=$outputlangs->transnoentities("RECIBO DE ALQUILER");
	...
	function _pagefootfractalqr(&$pdf,$object,$outputlangs,$hidefreetext=0,$sumaTotal)
	{
		...
		$pdf->SetFont('','',10);
		$pdf->SetXY($this->posxperi-1, 205);
		$pdf->MultiCell($this->posxespe-$this->posxperi-1,2,$outputlangs->transnoentities("Total Bs. "),'','R');

		$pdf->SetXY($this->posxespe-1, 205);
		$pdf->MultiCell($this->posxfin-$this->posxespe-4,2,  price($sumaTotal),'','R');
