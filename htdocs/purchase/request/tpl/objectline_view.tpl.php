...
<tr <?php echo 'id="row-'.$line->id.'" '.$bcdd[$var]; ?>>
	...
	if($conf->global->PRODUCT_USE_UNITS)
	{
		...
		$label = $langs->trans($line->getLabelOfUnit());
		if ($label !== '') {
	...
	<?php
		...
		print '</td>';
		print '<td align="right" class="nowrap">';
			print price(price2num($line->price*$line->qty,'MU'));
		print '</td>';

	?>	

	<?php 
	//vamos a verificar las condiciones
	if ($user->rights->purchase->req->mod)
		$object_rights->creer = true;
	else
		$object_rights->creer = false;
	...
	<td class="linecoldelete" align="center"><?php $coldisplay++; ?>
		<?php
		if ($conf->global->PURCHASE_INTEGRATED_POA && $this->originid>0)
			$disableremove = true;
