 ...
 */
class mod_mant_numberone extends ModeleNumRefMant
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefixticket='TICK-';
	var $prefixcreditnote='TIAV';
	var $error='';
	var $nom='Opcion numeracion 1';
	...
	function info()
	{
		global $langs;
		$langs->load("mant");
		return $langs->trans('Number one ticket',$this->prefixticket,$this->prefixcreditnote);
	}
	...
	function getExample()
	{
	  return $this->prefixticket.substr(date('Y'),2,2).(strlen(date('m'))==1?'0'.date('m'):date('m'))."-0001";
	}
	...
	function canBeActivated()
	{
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."m_work_request";
		//$sql.= " WHERE ref LIKE '".$this->prefixticket."____-%'";
		$sql.= " WHERE ref LIKE '".$this->prefixticket.$fayymm."-%'";
	...
	function getNextValue($objsoc,$facture,$mode='next')
	{
	  ...
	  $sql.= " AND entity = ".$conf->entity;
	  $resql=$db->query($sql);
	  dol_syslog("mod_mant_numberone::getNextValue sql=".$sql);
	  if ($resql)
	    {
	    ...
	    {
	      dol_syslog("mod_mant_numberone::getNextValue sql=".$sql, LOG_ERR);
	      return -1;
	    }
	  ...
	  if ($mode == 'last')
	  ...
	  else if ($mode == 'next')
	    {
	      ...
	      dol_syslog("mod_mant_numberone::getNextValue return ".$prefix.$yymm."-".$num);
