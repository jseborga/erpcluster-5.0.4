...
/**
 *   	\file       assets/cassetsdoc_list.php
 *		\ingroup    assets
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'assetscassetsdoclist';
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_assets_doc as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_assets_doc_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/cassetsdoc/card.php?id='.$id);
	exit;
}
...
print '</form>'."\n";
	...
	if ($user->rights->assets->teach->creer)
		print '<a class="butAction" href="'.DOL_URL_ROOT.'/assets/cassetsdoc/card.php?action=create">'.$langs->trans("Create").'</a>';
	else
