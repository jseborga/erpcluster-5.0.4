...
/**
 * \file    orgman/caltitude.class.php
 * \ingroup orgman
 ...
 * @see CommonObject
 */
class Caltitude extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'caltitude';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'c_altitude';

	/**
	 * @var CaltitudeLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("caltitude", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Caltitude") . '</u>';
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;
        $label.= '<br>';
        $label.= '<b>' . $langs->trans('Label') . ':</b> ' . $this->label;

        $url = DOL_URL_ROOT.'/orgman/altitude/'.'card.php?id='.$this->id;
