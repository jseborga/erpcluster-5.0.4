...
require_once DOL_DOCUMENT_ROOT.'/purchase/class/paiementfournext.class.php';
...
$objCtypefacture = new Ctypefacture($db);
	...
	if ($action == 'confirm_paiement' && $confirm == 'yes')
	{
	    ...
	    if (! $error)
	    {
	        ...
	        if (! $error)
	        {
	            ...
	            if ($invoiceid > 0) $loc = DOL_URL_ROOT.'/purchase/facture/card.php?facid='.$invoiceid;
    ...
    if ($resql)
    {
        ...
        if ($num)
        {
			...
			if (empty($reshook))
			{
	             ...
	             */
	            $sql = 'SELECT f.rowid as facid, f.ref, f.ref_supplier, f.total_ht, f.total_ttc, f.datef as df, f.localtax1, f.localtax2';
	            ...
	            if ($resql)
	            {
	                ...
	                if ($num > 0)
	                {
	                    ...
	                    while ($i < $num)
	                    {
	                        ...
	                        //obtenemos la tabla adicional
	                        $resadd = $objectadd->fetch(0,$objp->facid);
                            $retention = 0;
                            if ($resadd==1)
                            {
                                $resctype = $objCtypefacture->fetch(0,$objectadd->code_facture);
                                if ($resctype)
                                {
	                        ...
	                        if ($objectadd->fk_facture_fourn == $objp->facid)
	                        {
	                        	$invoicesupplierstatic->nfiscal = $objectadd->nfiscal;
	                        	$invoicesupplierstatic->num_autoriz = $objectadd->num_autoriz;
	                        }
	                        print $invoicesupplierstatic->getNomUrl(1);
	                        print '</td>';
	                        print '<td>'.'<span class="badge">F</span>'.$invoicesupplierstatic->nfiscal.' '.'<span class="badge">A</span>'.$invoicesupplierstatic->num_autoriz.'</td>';
	                        if ($objp->df > 0 )
	                        {
                            ...
                            $totalttc = $objp->total_ttc;
                            if ($retention)
                            {

                                 $totalttc = $objp->total_ttc -$objp->localtax1-$objp->localtax2;
                             }
	                        print '<td align="right">'.price($totalttc).'</td>';
	                        print '<td align="right">'.price($objp->am).'</td>';
	                        print '<td align="right">'.price($totalttc - $objp->am).'</td>';
	                        print '<td align="center">';
	                        $namef = 'amount_'.$objp->facid;
                            $retentionf = 'retention_'.$objp->facid;
							if(!empty($conf->global->FAC_AUTO_FILLJS))
                            ...
                            print '<input type="hidden" name="'.$retentionf.'" value="'.$retention.'">';
							print "</td></tr>\n";
	                        $total+=$objp->total_ht;
...
if (empty($action))
{
    ...
    if ($resql)
    {
        ...
        while ($i < min($num,$limit))
        {
            ...
            // Ref payment
            print '<td class="nowrap"><a href="'.DOL_URL_ROOT.'/purchase/paiement/card.php?id='.$objp->pid.'">'.img_object($langs->trans('ShowPayment'),'payment').' '.$objp->pid.'</a></td>';
