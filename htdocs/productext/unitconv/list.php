...
/**
 *   	\file       purchase/unitconv_list.php
 *		\ingroup    purchase
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-10-17 11:48
 */
...
dol_include_once('/productext/class/unitconv.class.php');
...
$search_fk_unit_ext=GETPOST('search_fk_unit_ext','int');
$search_fk_unit=GETPOST('search_fk_unit','int');
$search_fc=GETPOST('search_fc','alpha');
$search_type_fc=GETPOST('search_type_fc','alpha');
...
$hookmanager->initHooks(array('unitconvlist'));
...
't.fk_unit_ext'=>array('label'=>$langs->trans("Fieldfk_unit_ext"), 'checked'=>1),
't.fk_unit'=>array('label'=>$langs->trans("Fieldfk_unit"), 'checked'=>1),
't.fc'=>array('label'=>$langs->trans("Fieldfc"), 'checked'=>1),
't.type_fc'=>array('label'=>$langs->trans("Fieldtype_fc"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."unit_conv as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."unit_conv_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_product) $sql.= natural_search("fk_product",$search_fk_product);
if ($search_fk_unit_ext) $sql.= natural_search("fk_unit_ext",$search_fk_unit_ext);
if ($search_fk_unit) $sql.= natural_search("fk_unit",$search_fk_unit);
if ($search_fc) $sql.= natural_search("fc",$search_fc);
if ($search_type_fc) $sql.= natural_search("type_fc",$search_type_fc);
...
if ($search_fk_unit_ext != '') $params.= '&amp;search_fk_unit_ext='.urlencode($search_fk_unit_ext);
if ($search_fk_unit != '') $params.= '&amp;search_fk_unit='.urlencode($search_fk_unit);
if ($search_fc != '') $params.= '&amp;search_fc='.urlencode($search_fc);
if ($search_type_fc != '') $params.= '&amp;search_type_fc='.urlencode($search_type_fc);
...
if (! empty($arrayfields['t.fk_unit_ext']['checked'])) print_liste_field_titre($arrayfields['t.fk_unit_ext']['label'],$_SERVER['PHP_SELF'],'t.fk_unit_ext','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fc']['checked'])) print_liste_field_titre($arrayfields['t.fc']['label'],$_SERVER['PHP_SELF'],'t.fc','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.type_fc']['checked'])) print_liste_field_titre($arrayfields['t.type_fc']['label'],$_SERVER['PHP_SELF'],'t.type_fc','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_unit_ext']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_unit_ext" value="'.$search_fk_unit_ext.'" size="10"></td>';
...
if (! empty($arrayfields['t.fc']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fc" value="'.$search_fc.'" size="10"></td>';
if (! empty($arrayfields['t.type_fc']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_type_fc" value="'.$search_type_fc.'" size="10"></td>';
