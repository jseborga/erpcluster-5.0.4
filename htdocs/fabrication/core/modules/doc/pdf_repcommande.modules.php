 ...
 */
class pdf_repcommande extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
  ...
  // Objet societe qui emet
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "repcommande";
		$this->description = $langs->trans('PDFReportCommandeDescription');
	...
	*/
	...
	// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		...
		$outputlangs->load("commande");
		$outputlangs->load("sales@sales");
		$aKardex = unserialize($_SESSION['aCommanderep']);

		if ($conf->fabrication->dir_output)
		{
			...
			if ($object->specimen)
			{
				$dir = $conf->fabrication->dir_output;
				$file = $dir . "/SPECIMEN.pdf";
			}
			else
			{
				$now = $this->db->idate(dol_now());
				$dir = $conf->fabrication->dir_output .'/rep';
			...
			if (file_exists($dir))
			{
				...
				//for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$pdf->MultiCell(0, 0, $ref, 0, 'L',0); 

					//entrepot
					$entrepot = $lines['entrepot'];
					$pdf->SetXY($this->posxentr, $curY);
					$pdf->MultiCell(0, 0, $entrepot, 0, 'L',0); 

					//date
					$date = dol_print_date($lines['date'],'dayhour');
					$pdf->SetXY($this->posxdate, $curY);
					$pdf->MultiCell($this->posxprod-$this->posxdate-1, 0, $date, 0, 'C',0); 
					//$pdf->MultiCell($this->posxprod-$this->posxdate-1,2, $outputlangs->transnoentities("Date"),'','C');

					//product
					$product = $lines['label'];
					$pdf->SetXY($this->posxprod, $curY);
					$pdf->MultiCell($this->posxqty-$this->posxprod-1, 0, $product, 0, 'L',0); 

					//qty
					$qty = $lines['qty'];
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->posxdesc-$this->posxqty-1, 0, $qty, 0, 'R',0); 

					$npri = dol_string_nohtmltag(strtolower($lines['note_private']),false,'UTF-8');
					$npri = $langs->trans('Nota privada').': '.$npri;

					$np = dol_string_nohtmltag(strtolower($lines['note_public']),false,'UTF-8');
					$np = $langs->trans('Nota pública').': '.$np;
					$notetoshow=dol_concatdesc($npri, $np);
					$desc=dol_string_nohtmltag(strtolower($lines['description']),false,'UTF-8');
					$desc = trim($desc);
					$desc = preg_replace("[\n|\r|\n\r]", " ", $desc);

					$notetoshow=dol_concatdesc($notetoshow, $desc);

					//desc
					//$np = dol_string_nohtmltag(strtolower($lines['note_private']),false,'UTF-8');
					//$desc = $lines['description'];
					//$desc = 'texto';
					//$pdf->SetXY($this->posxdesc, $curY);
					//$pdf->MultiCell($this->posxnpu-$this->posxdesc-1, 0, $np, 0, 'L',0, 1, '' ,'', false,3,false,true,0,'T');
					//$pdf->MultiCell(80, 5, $txt."\n", 1, 'J', 1, 1, '' ,'', true);
					//note public
					//$np = $lines['note_public'];
					//$np = dol_string_nohtmltag(strtolower($lines['note_public']),false,'UTF-8');
					//$pdf->SetXY($this->posxnpu, $curY);
					//$pdf->MultiCell($this->posxnpr-$this->posxnpu-1, 0, $np, 0, 'L',0, 1, '' ,'', false,3,false,true,0,'T');
					...
					$desc = preg_replace("[\n|\r|\n\r]", " ", $desc);
					$pdf->SetXY($this->posxdesc, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxdesc, 0, $notetoshow, 0, 'L',0, 1, '' ,'', false,3,false,true,0,'T');
	...
	*/
	...
	// line prend une position y en 2eme param et 4eme param
		...
		{	
			...
			$pdf->MultiCell($this->posxdate-$this->posxentr-1,2, $outputlangs->transnoentities("Almacen"),'','L');
		}
		...
		if (empty($hidetop))
		{	
			$pdf->SetXY($this->posxprod-1, $tab_top+1);
			$pdf->MultiCell($this->posxqty-$this->posxprod-1,2, $outputlangs->transnoentities("Product"),'','C');
		}
		$pdf->line($this->posxqty-1, $tab_top, $this->posxqty-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{	
			$pdf->SetXY($this->posxqty-1, $tab_top+1);
			$pdf->MultiCell($this->posxdesc-$this->posxqty-1,2, $outputlangs->transnoentities("Q"),'','C');
		}
		//$pdf->line($this->posxdesc-1, $tab_top, $this->posxdesc-1, $tab_top + $tab_height);
		//if (empty($hidetop))
		//{	
		//	$pdf->SetXY($this->posxdesc-1, $tab_top+1);
		//	$pdf->MultiCell($this->posxnpu-$this->posxdesc-1,2, $outputlangs->transnoentities("Nota Privada"),'','C');
		//}

		//$pdf->line($this->posxnpu-1, $tab_top, $this->posxnpu-1, $tab_top + $tab_height);
		//if (empty($hidetop))
		//{	
		//	$pdf->SetXY($this->posxnpu-1, $tab_top+1);
		//	$pdf->MultiCell($this->posxnpr-$this->posxnpu-1,2, $outputlangs->transnoentities("Nota Publica"),'','C');
		//}
		...
		{
			$pdf->SetXY($this->posxdesc-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxdesc,2, $outputlangs->transnoentities("Descripción"),'','C');
		}
	}
	...
	*/
	...
	//    echo $posx.' '.$posy;exit;
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Lista de pedidos");
