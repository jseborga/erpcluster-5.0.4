 ...
 *				Initialy built by build_class_from_table on 2015-03-16 16:56
 */
...
class Poaprocesscontrat extends CommonObject
{
	...
	var $element='poaprocesscontrat';			//!< Id that identify managed objects
	var $table_element='poaprocesscontrat';		//!< Name of table without prefix where object is stored
	...
	var $date_provisional='';
	var $date_final='';
	var $date_nonconformity='';
	var $nonconformity;
	var $motif;
	var $fk_user_create;
	var $tms='';
	...
	function create($user, $notrigger=0)
	{
		...
		if (isset($this->nonconformity)) $this->nonconformity=trim($this->nonconformity);
		if (isset($this->motif)) $this->motif=trim($this->motif);
		...
		$sql.= "date_provisional,";
		$sql.= "date_final,";
		$sql.= "date_nonconformity,";
		$sql.= "nonconformity,";
		$sql.= "motif,";
		$sql.= "fk_user_create,";
		$sql.= "statut";
		...
		$sql.= " ".(! isset($this->date_provisional) || dol_strlen($this->date_provisional)==0?'NULL':$this->db->idate($this->date_provisional)).",";
		$sql.= " ".(! isset($this->date_final) || dol_strlen($this->date_final)==0?'NULL':$this->db->idate($this->date_final)).",";
		$sql.= " ".(! isset($this->date_nonconformity) || dol_strlen($this->date_nonconformity)==0?'NULL':$this->db->idate($this->date_nonconformity)).",";
		$sql.= " ".(! isset($this->nonconformity)?'NULL':"'".$this->nonconformity."'").",";
		$sql.= " ".(! isset($this->motif)?'NULL':"'".$this->db->escape($this->motif)."'").",";
	 ...
	 */
	function fetch($id,$fk_poa_process='',$fk_contrat='')
	{
		global $langs;
		$sql = "SELECT";
		$sql.= " t.rowid,";
		...
		$sql.= " t.date_provisional,";
		$sql.= " t.date_final,";
		$sql.= " t.date_nonconformity,";
		$sql.= " t.nonconformity,";
		$sql.= " t.motif,";
		...
		else if ($fk_poa_process>0 && $fk_contrat>0)
		{
		...
		if ($resql)
		{
			if ($this->db->num_rows($resql))
			{
				...
				$this->date_provisional = $this->db->jdate($obj->date_provisional);
				$this->date_final = $this->db->jdate($obj->date_final);
				$this->date_nonconformity = $this->db->jdate($obj->date_nonconformity);
				$this->nonconformity = $obj->nonconformity;
	...
	function update($user=0, $notrigger=0)
	{
		...
		$sql.= " date_provisional=".(dol_strlen($this->date_provisional)!=0 ? "'".$this->db->idate($this->date_provisional)."'" : 'null').",";
		$sql.= " date_final=".(dol_strlen($this->date_final)!=0 ? "'".$this->db->idate($this->date_final)."'" : 'null').",";
		$sql.= " date_nonconformity=".(dol_strlen($this->date_nonconformity)!=0 ? "'".$this->db->idate($this->date_nonconformity)."'" : 'null').",";
		$sql.= " nonconformity=".(isset($this->nonconformity)?$this->nonconformity:"null").",";
		$sql.= " motif=".(isset($this->motif)?"'".$this->db->escape($this->motif)."'":"null").",";
	...
	function initAsSpecimen()
	{
		...
		$this->date_provisional='';
		$this->date_final='';
		$this->date_nonconformity='';
		$this->nonconformity='';
		$this->motif='';
		$this->fk_user_create='';
		$this->tms='';
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				while ($i < $num)
				{
					...
					$objnew->date_provisional = $this->db->jdate($obj->date_provisional);
					$objnew->date_final = $this->db->jdate($obj->date_final);
					$objnew->date_nonconformity = $this->db->jdate($obj->date_nonconformity);
					$objnew->nonconformity = $obj->nonconformity;
					$objnew->motif = $obj->motif;
	...
	/**
	 *  Load object in memory from the database
	 *  Busca los contratos registrados para el preventivo principal
	 *  @param	int		$fk_poa_process    Id object
	 *  @return int          	<0 if KO, >0 if OK
	 */
	function getlist2($fk_poa_prev)
	{
		global $langs;
		$sql = "SELECT";
		$sql.= " t.rowid,";
		...
		$sql.= " WHERE p.fk_poa_prev = ".$fk_poa_prev;
		$sql.= " AND t.statut = 1";
	  //solo los validos
	 ...
	 *  @return int          	<0 if KO, >0 if OK
	 */
	function getlist_contrat($fk_poa_process)
	{
		global $langs;
		$sql = "SELECT";
		$sql.= " t.fk_contrat";
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."poa_process_contrat as t";
		$sql.= " WHERE t.fk_poa_process = ".$fk_poa_process;
		dol_syslog(get_class($this)."::getlist_contrat sql=".$sql, LOG_DEBUG);
		$resql=$this->db->query($sql);
		$this->aContrat = array();
		if ($resql)
		{
			...
			if ($num)
			{
				...
				{
					$obj = $this->db->fetch_object($resql);
					$this->aContrat[$obj->fk_contrat] = $obj->fk_contrat;
					$i++;
				}
				$this->db->free($resql);
				return 1;
		...
		{
			$this->error="Error ".$this->db->lasterror();
			dol_syslog(get_class($this)."::getlist_contrat ".$this->error, LOG_ERR);
			return -1;
		}
	}
	 ...
	 *  @return int          	<0 if KO, >0 if OK
	 */
	function getlist_contrat2($fk_poa_process,$statutppc=1)
	{
		global $langs;
		$sql = "SELECT";
		$sql.= " t.fk_contrat,";
		...
		$sql.= " AND p.statut >= 0 ";
	  //revisar rqc
		$sql.= " AND t.statut in(".$statutppc.")";
	  //1=activo, 2=anulado
		dol_syslog(get_class($this)."::getlist_contrat2 sql=".$sql, LOG_DEBUG);
		$resql=$this->db->query($sql);
		$this->aContrat = array();
		if ($resql)
		{
			...
			if ($num)
			{
				...
				{
					$obj = $this->db->fetch_object($resql);
					$this->aContrat[$obj->fk_contrat] = $obj->rowid;
					$i++;
				}
				$this->db->free($resql);
				return 1;
		...
		{
			$this->error="Error ".$this->db->lasterror();
			dol_syslog(get_class($this)."::getlist_contrat2 ".$this->error, LOG_ERR);
			return -1;
		}
	}
