...
/**
 * \file 	htdocs/sales/compta/facture.php
 * \ingroup facture
...
require_once DOL_DOCUMENT_ROOT . '/sales/lib/sales.lib.php';
...
$vfiscal         = new Vfiscalext($db);
$vdosing         = new Vdosingext($db);
$subsidiary      = new Subsidiaryext($db);
$tvadef          = new Tvadefext($db);
$objectdetfiscal = new Facturedetfiscalext($db);

if ($conf->fiscal->enabled)
{
	if (empty($conf->global->FISCAL_CODE_FACTURE_SALES))
		setEventMessages($langs->trans('Falta configurar el codigo de factura para venta'),null,'warnings');
}
// Load object
if ($id > 0 || ! empty($ref)) {
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->facture->supprimer) {
		...
		if ($result > 0) {
			header('Location: ' . DOL_URL_ROOT . '/sales/compta/facture/list.php');
			exit();
		} else {
			setEventMessages($object->error, $object->errors, 'errors');
			$action='';
	...
	if ($action == 'confirm_deleteline' && $confirm == 'yes' && $user->rights->facture->creer)
	{
		...
		$filter = " AND t.fk_facturedet = ".GETPOST('lineid');
		...
		if (!$error)
		{
			...
			// Define output language
				...
				//agregamos el tipo de dosificacion para el nombre de factura
					$res = $vfiscal->fetch(0,$object->id);
					if ($res > 0)
						$vdosing->fetch($vfiscal->fk_dosing);
					$object->typedosing=$vdosing->type;
					$object->nametypedosing=get_name_typebill($vdosing->type);
	...
	if ($action == 'set_thirdparty' && $user->rights->facture->creer)
	{
		$object->fetch($id);
	...
	if ($action == 'classin' && $user->rights->facture->creer)
	{
		$object->fetch($id);
		$object->setProject($_POST['projectid']);
	}

	if ($action == 'setmode' && $user->rights->facture->creer)
	{
		$object->fetch($id);
	...
	// Multicurrency Code
	if ($action == 'setmulticurrencycode' && $user->rights->facture->creer) {
		$result = $object->setMulticurrencyCode(GETPOST('multicurrency_code', 'alpha'));
	}

	// Multicurrency rate
	if ($action == 'setmulticurrencyrate' && $user->rights->facture->creer) {
		$result = $object->setMulticurrencyRate(price2num(GETPOST('multicurrency_tx')));
	}

	if ($action == 'setinvoicedate' && $user->rights->facture->creer)
	{
		$object->fetch($id);
	...
	if ($action == 'setdate_pointoftax' && $user->rights->facture->creer)
	{
		$object->fetch($id);
	...
	if ($action == 'setpaymentterm' && $user->rights->facture->creer)
	{
		$object->fetch($id);
	...
	if ($action == 'setrevenuestamp' && $user->rights->facture->creer)
	{
		$object->fetch($id);
	...
	// bank account
	if ($action == 'setbankaccount' && $user->rights->facture->creer)
	{
	...
	if ($action == 'confirm_valid' && $confirm == 'yes' && ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->facture->creer)) || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->facture->invoice_advance->validate))))
	{
		...
		if ($object->type != Facture::TYPE_DEPOSIT && !empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change ) {
			...
			//revisamos si esta activo STOCK_ALLOW_NEGATIVE_TRANSFER
			if (!$conf->global->STOCK_ALLOW_NEGATIVE_TRANSFER)
			{
				$objProduct = new Product($db);
				//sin un producto no tiene saldo se manda mensaje de error
				foreach ($object->lines AS $j => $line)
				{
					if ($line->fk_product>0)
					{
						...
						if ($objProduct->stock_warehouse[$idwarehouse]->real <=0)
						{
							$error++;
							setEventMessages($langs->trans('Theproduct').' '.$objProduct->ref.' '.$objProduct->label.' '.$langs->trans('Nobalanceinthewarehouse'),null,'errors');
						}
		...
		if (GETPOST('fk_dosing')<=0)
		{
			$error ++;
			setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentitiesnoconv("Dosing")), null, 'errors');
			$action = '';
		}
			//dependiendo del tipo de dosificación
		else
		{
			$model_facture = 'fractal';
				//verificamos si es recibo de alquiler, debe tener fecha de inicio y fin

			$resd = $vdosing->fetch(GETPOST('fk_dosing'));


			if ($resd > 0)
			{
				if ($vdosing->type == 2) $model_facture = 'fractalser';
				if ($vdosing->type == 3)
				{
					$model_facture = 'fractalrec';
					$object->fetch_lines();
					$lines = $object->lines;
					foreach ($lines AS $j => $line)
					{
						if ((empty($line->date_start) || is_null($line->date_start)) ||(empty($line->date_end) || is_null($line->date_end)))
						{
							$error++;
							setEventMessages($langs->trans('Recordrentalreceiptdates'),null,'errors');
						}
					}
				}
			}
		...
		{
			$db->begin();
			$result = $object->validateadd($user, '', $idwarehouse);
				//generamos la factura automatica o manual
			if ($result >=0)
			{
					//verificamos que tipo de factura es
				$vdosing->fetch(GETPOST('fk_dosing','int'));
				if ($vdosing->lote == 2) $modseller = 2;
				else $modseller = 1;
				$numfactsel = GETPOST('invoicenumber');
				$numautsel = $vdosing->num_autoriz;
				$nTotalTtc = $object->total_ttc;
				$nTotalTva = $object->total_tva;
				$fk_dosing = GETPOST('fk_dosing');
				$thirdpartyid = $object->socid;
				$fk_tva 	= 13;
					//revisar
				$now = dol_now();
				$nit = GETPOST('nit','int')+0;
				if ($nit == 0)
					$razsoc = $langs->trans('Sinnombre');
				else
					$razsoc = GETPOST('razsoc');
				if($vdosing->lote == 1 && empty($numfactsel))
				{
					$error++;
					setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentitiesnoconv("Invoicenumber")), null, 'errors');
					$action = '';
				}
				if($nit<0)
				{
				...
				{
					$error++;
					setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentitiesnoconv("Razsoc")), null, 'errors');
					$action = '';
				}
				if (!$error)
				{
					include DOL_DOCUMENT_ROOT.'/fiscal/include/reg_vfiscal.inc.php';
					if ($error)
					{
						$error++;
						setEventMessages($vfiscal->error,$vfiscal->errors,'errors');
					}
				}
			}
			if (!$error)
			{
				...
				$object->modelpdf = $model_facture;
				$res = $object->update_modelpdf($user);
				if ($res<=0)
				{
					$error++;
					setEventMessages($object->error,$object->errors,'errors');
	...
	if ($action == 'add' && $user->rights->facture->creer)
	{
		...
		// Replacement invoice
		if (
			$_POST['type'] == Facture::TYPE_REPLACEMENT)
		{
		...
		// Credit note invoice
		if (
			$_POST['type'] == Facture::TYPE_CREDIT_NOTE)
		{
		...
		if (
			($_POST['type'] == Facture::TYPE_STANDARD || $_POST['type'] == Facture::TYPE_DEPOSIT) && GETPOST('fac_rec') > 0)
		{
			$dateinvoice = dol_mktime(12, 0, 0, $_POST['remonth'], $_POST['reday'], $_POST['reyear']);
		...
		if (
			($_POST['type'] == Facture::TYPE_STANDARD || $_POST['type'] == Facture::TYPE_DEPOSIT || $_POST['type'] == Facture::TYPE_PROFORMA || ($_POST['type'] == Facture::TYPE_SITUATION && empty($_POST['situations']))) && GETPOST('fac_rec') <= 0)
		{
			...
			if (! $error)
			{
				...
				$object->ref_int			= $_POST['ref_int'];
				$object->modelpdf			= $_POST['model'];
				$object->fk_project			= ($idpay>0?$_POST['fk_projet']:$_POST['projectid']);
				$object->cond_reglement_id	= ($_POST['type'] == 3?1:$_POST['cond_reglement_id']);
				$object->mode_reglement_id	= $_POST['mode_reglement_id'];
				$object->fk_account         = GETPOST('fk_account', 'int');
				$object->amount				= $_POST['amount'];
				...
				if (
					GETPOST('type') == Facture::TYPE_SITUATION)
				{
						...
						if ($_POST['type'] == Facture::TYPE_DEPOSIT)
							{
								...
								else
								{
									...
									if ($result > 0)
									{
										...
										{
											$qualified=1;
											if (empty($lines[$i]->qty)) $qualified=0;
										// We discard qty=0, it is an option
											if (! empty($lines[$i]->special_code)) $qualified=0;
									...
									for ($i=0;$i<$num;$i++)
									{
										...
										// View third's localtaxes for now
											...
											if ($conf->fiscal->enabled)
											{
												...
												{
													$productadd = new Product($db);
													$productadd->fetch($lines[$i]->fk_product);
												}
												...
												if ($conf->global->PRICE_TAXES_INCLUDED)
												{
													$price_base_type = 'TTC';
													$pu = $lines[$i]->price;
													if ($lines[$i]->element =='contratdet')
													{
														$pu = price2num($lines[$i]->total_ttc / $lines[$i]->qty,'MU');
														if ($num == 1 && $ppay == 1)
														{
															$ns = GETPOST('ns');
															if ($ns > 0)
															{
																//se debe reemplazar el valor de la linea por el valor recibido de pago
																$pu = price2num($ns / $lines[$i]->qty,'MU');
															}
														}
												...
												$tvatx = $lines[$i]->tva_tx;
												$result = $object->addlineadd($desc, $lines[$i]->subprice, $lines[$i]->qty, $lines[$i]->tva_tx, $localtax1_tx, $localtax2_tx, $lines[$i]->fk_product, $lines[$i]->remise_percent, $date_start, $date_end, 0, $lines[$i]->info_bits, $lines[$i]->fk_remise_except, $price_base_type, $lines[$i]->price, $product_type, $lines[$i]->rang, $lines[$i]->special_code, $object->origin, $lines[$i]->rowid, $fk_parent_line, $lines[$i]->fk_fournprice, $lines[$i]->pa_ht, $label, $array_options, $lines[$i]->situation_percent, $lines[$i]->fk_prev_id, $lines[$i]->fk_unit,$lines[$i]);
											...
											//agregamos en detfiscal
												...
												{
													$objectdetfiscal->fk_facturedet = $idligne;
													$objectdetfiscal->code_tva = $code;
													$objectdetfiscal->tva_tx = $tvatx;
			...
			if (
				GETPOST('type') == Facture::TYPE_SITUATION && (!empty($_POST['situations'])))
			{
			...
			//si viene de un pago de proyecto actualizamos el id
			if (!$error)
			{
				if ($idpay>0 && $ppay)
				{
					$objProjetpayment = new Projetpaymentext($db);
					$res = $objProjetpayment->fetch($idpay);
					if ($res>0)
					{
						$objProjetpayment->fk_facture = $id;
						$objProjetpayment->fk_user_mod = $user->id;
						$res = $objProjetpayment->update($user);
						if ($res<=0)
						{
							$error++;
							setEventMessages($objProjetpayment->error,$objProjetpayment->errors,'errors');
						}
					}
				}
			}
	...
	// Add a new line
		...
		// Unit price can be 0 but not ''
			...
			// Replaces $fk_unit with the product's
				...
				if ($conf->fiscal->enabled)
				{
					...
					if (! empty($price_min) && (price2num($lines->subprice) * (1 - price2num($remise_percent) / 100) < price2num($price_min))) {
						$mesg = $langs->trans("CantBeLessThanMinPrice", price(price2num($price_min, 'MU'), 0, $langs, 0, 0, - 1, $conf->currency));
						setEventMessages($mesg, null, 'errors');
						$error++;
					}

					$result = $object->addlineadd($desc, $lines->subprice, $qty, $lines->tva_tx, $lines->localtax1_tx, $lines->localtax2_tx, $idprod, $lines->remise_percent, $date_start, $date_end, 0, $info_bits, $fk_remise_except, $price_base_type, $lines->price, $type, -1, $special_code, 0, 0, GETPOST('fk_parent_line'), $fournprice, $buyingprice, $label, $array_options, GETPOST('progress'), '', $fk_unit,$lines);
		...
		if (! $error)
		{
			if ($conf->fiscal->enabled)
			{
				...
				if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT) && $price_min && (price2num($lines->subprice) * (1 - price2num(GETPOST('remise_percent')) / 100) < price2num($price_min))) {
					setEventMessages($langs->trans("CantBeLessThanMinPrice", price(price2num($price_min, 'MU'), 0, $langs, 0, 0, - 1, $conf->currency)), null, 'errors');
					$error ++;
				}

				$result = $object->updatelineadd(GETPOST('lineid'), $description, $lines->subprice, $qty, $lines->remise_percent, $date_start, $date_end, $lines->tva_tx, $lines->localtax1tx, $lines->localtax2tx, $price_base_type, $info_bits, $type, GETPOST('fk_parent_line'), 0, $fournprice, $buyingprice, $label, $special_code, $array_options, GETPOST('progress'), GETPOST('fk_unit'),$lines);
			}
			else
			{
...
llxHeader('', $langs->trans('InvoiceCustomer'), 'EN:Customers_Invoices|FR:Factures_Clients|ES:Facturas_a_clientes');
	...
	//recuperamos las dosificaciones
$filterstatic = " AND t.status = 1 AND t.active = 1";
$nbdosing = $vdosing->fetchAll('ASC', 'series', 0,0,array(1=>1),'AND', $filterstatic);
$aDosing = array();
if ($nbdosing>0)
{
	if ($nbdosing>1)
		$optionsdosing = '<option value="0">'.$langs->trans('Select').'</option>';
	foreach ($vdosing->lines AS $j => $line)
	{
		$typebill = select_typebill($line->type,'type','',0,0,1);
		$subsidiary->fetch($line->fk_subsidiaryid);
		$optionsdosing.= '<option value="'.$line->id.'">'.$line->series.' - '.($line->lote==2?$langs->trans('Automatic'):$langs->trans('Manual')).' - '.$subsidiary->ref.'</option>';
		$aDosing[$line->id] = $line->series.' '.($line->lote==2?$langs->trans('Automatic'):$langs->trans('Manual')).' - '.$subsidiary->ref.' - '.$typebill;
	}
}
...
if ($action == 'create')
{
	...
	if (! empty($origin) && ! empty($originid))
	{
		...
		else
		{
			...
			if ($element == 'fichinter') {
				$element = 'sales';
				$subelement = 'fichinterext';
			}
			if ($element == 'commande') {
				$element = 'sales';
				$subelement = 'commandeext';
			}
	...
	print '<input type="hidden" name="fk_projet" value="' . $projectid . '">';

	print '<input type="hidden" name="ns" value="' . GETPOST('ns') . '">';
	print '<input type="hidden" name="ppay" value="' . GETPOST('ppay') . '">';
	print '<input type="hidden" name="idpay" value="' . GETPOST('idpay') . '">';
	...
	else
	{
		...
		if (empty($origin) && empty($originid) && GETPOST('fac_rec','int') > 0)
		{
			...
			$dateinvoice = $invoice_predefined->date_when;
			// To use next gen date by default later
	...
	//if ($conf->fiscal->enabled)
	//{
	//	print '<tr><td class="fieldrequired">'.$langs->trans('Series').'</td><td>';
	//	if ($nbdosing>0)
	//	{
	//		print '<select name="fk_dosing">'.$optionsdosing.'</select>';
	//	}
	//	else
	//		print $langs->trans('Error, es necesario las dosificaciones para las facturas');
	//	print '</td></tr>';
	//}
	...
	// Private note
							...
							if (! empty($origin) && ! empty($originid) && is_object($objectsrc))
							{
								...
								foreach ($lines AS $j => $line)
								{
									...
									echo '<hr>suma '.$total_ttc += $line->total_ttc;
								}
								if ($objectsrc->element == 'contrat')
								{
									$objectsrc->total_ht = $total_ht;
									$objectsrc->total_ttc = $total_ttc;
									$objectsrc->total_tva = $total_tva;
								}
								...
								print '<tr><td>' . $langs->trans($newclassname) . '</td><td colspan="2">' . ($classname=='Expedition'?$objectsrc->getNomUrl(1):$objectsrc->getNomUrladd(1));
		...
		// Localtax2
								...
								if (GETPOST('idpay')>0)
								{
									print '<tr><td>' . $langs->trans('TotalTTC') . '</td><td colspan="2">' . price(GETPOST('ns')) . "</td></tr>";
								}
								else
								{
	...
	// Show origin lines
							if (! empty($origin) && ! empty($originid) && is_object($objectsrc)) {
								...
								($classname=='Expedition'?$objectsrc->printOriginLinesList:$objectsrc->printOriginLinesListadd);
	...
	$head = sales_facture_prepare_head($object);
	...
	if ($action == 'delete')
	{
		...
		if ($action == 'valid')
		{
			...
			if ($object->type != Facture::TYPE_DEPOSIT && ! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change)
				...
				if ($object->type != Facture::TYPE_CREDIT_NOTE && $object->total_ttc < 0)
					// Can happen only if $conf->global->FACTURE_ENABLE_NEGATIVE is on
					{
						$text .= '<br>' . img_warning() . ' ' . $langs->trans("ErrorInvoiceOfThisTypeMustBePositive");
					}
					if ($object->type == Facture::TYPE_STANDARD && $object->total_ttc > 0)
						{
							$label = $langs->trans('titledosifautomatic');
							$value = $langs->trans('messagedosifautomatic');

							//$formquestion = array(
							//	'text' => $langs->trans("Selectserie"),
							//	array('type' => 'select', 'name' => 'fk_dosing', 'label' => $langs->trans("Dosing"), 'values' =>$aDosing),
							//	array('type' => 'other','name' => 'idwarehouse','label' => $label,'value' => $value),
							//	array('type' => 'text', 'name' => 'invoicenumber', 'label' => $langs->trans("Invoicenumber"), 'value' =>''),
							//	array('type' => 'text', 'name' => 'nit', 'label' => $langs->trans("NIT"), 'value' =>$soc->tva_intra),
							//	array('type' => 'text', 'name' => 'razsoc', 'label' => $langs->trans("Name"), 'value' =>$soc->nom),);
							$formquestion[] = array('type' => 'select', 'name' => 'fk_dosing', 'label' => $langs->trans("Dosing"), 'values' =>$aDosing);
							$formquestion[] = array('type' => 'other','name' => 'idwarehouse','label' => $label,'value' => $value);
							$formquestion[] = array('type' => 'text', 'name' => 'invoicenumber', 'label' => $langs->trans("Invoicenumber"), 'value' =>'');
							$formquestion[] = array('type' => 'text', 'name' => 'nit', 'label' => $langs->trans("NIT"), 'value' =>$soc->tva_intra);
							$formquestion[] = array('type' => 'text', 'name' => 'razsoc', 'label' => $langs->trans("Name"), 'value' =>$soc->nom);
							//$formquestion = array('text' => $langs->trans("Selectserie"),$formquestionadd);
						}
						$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?facid=' . $object->id, $langs->trans('ValidateBill'), $text, 'confirm_valid', $formquestion, (($object->type != Facture::TYPE_CREDIT_NOTE && $object->total_ttc < 0) ? "no" : "yes"), 2,400);
					}
	...
	$linkback = '<a href="' . DOL_URL_ROOT . '/sales/compta/facture/list.php' . (! empty($socid) ? '?socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	if ($action == 'editthirdparty') {
		...
		print ' &nbsp; ';
		print '(<a href="' . DOL_URL_ROOT . '/sales/compta/facture/list.php?socid=' . $object->socid . '">' . $langs->trans('OtherBills') . '</a>';
		// Outstanding Bill
		$outstandingBills = $soc->get_OutstandingBill();
		print ' - ' . $langs->trans('CurrentOutstandingBill') . ': ';
		print price($outstandingBills, '', $langs, 0, 0, - 1, $conf->currency);
	...
	$addcreditnote = '<a href="' . DOL_URL_ROOT . '/sales/compta/facture.php?action=create&socid=' . $soc->id . '&type=2&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?facid=' . $object->id . '">' . $langs->trans("AddCreditNote") . '</a>';
	...
	// Date invoice
		...
		print '</td>';
		if (
			$object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && ! empty($object->brouillon) && $user->rights->facture->creer && empty($conf->global->FAC_FORCE_DATE_VALIDATION))
		{
		...
		if
			($object->type == Facture::TYPE_SITUATION && !empty($conf->global->INVOICE_USE_SITUATION))
		{
			...
			($object->type != Facture::TYPE_CREDIT_NOTE)
		{
			...
			if ($resql)
			{
				...
				while ($i < $num)
				{
					...
					print '<tr><td colspan="' . $nbcols . '" align="right">';
					if
						($invoice->type == Facture::TYPE_CREDIT_NOTE) print $langs->trans("CreditNote") . ' ';
					if
						($invoice->type == Facture::TYPE_DEPOSIT) print $langs->trans("Deposit") . ' ';
					...
					$i ++;
					if
						($invoice->type == Facture::TYPE_CREDIT_NOTE) $creditnoteamount += $obj->amount_ttc;
					if
						($invoice->type == Facture::TYPE_DEPOSIT) $depositamount += $obj->amount_ttc;
				}
			} else {
				dol_print_error($db);
			}
	...
	if (!empty($conf->incoterm->enabled))
	{
		...
		if ($user->rights->facture->creer) print '<a href="'.DOL_URL_ROOT.'/sales/compta/facture.php?facid='.$object->id.'&action=editincoterm">'.img_edit().'</a>';
		else print '&nbsp;';
		print '</td></tr></table>';
		print '</td>';
		print '<td colspan="3">';
	...
	// Lines
	$result = $object->getLinesArrayadd();
	...
	if (! empty($object->lines))
		$ret = $object->printObjectLinesadd($action, $mysoc, $soc, $lineid, 1);
	...
	if ($object->statut == 0 && $user->rights->facture->creer && $action != 'valid' && $action != 'editline' && ($object->is_first() || !$object->situation_cycle_ref))
	{
		if ($action != 'editline')
		{
			...
			$object->formAddObjectLineadd(1, $mysoc, $soc);
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline')
	{
		...
		if (empty($reshook)) {
			...
			{
				if (! $objectidnext && $object->close_code != 'replaced' && $objfiscal->status != 2)
				// Not replaced by another invoice
				{
			...
			if ($object->type != Facture::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0 && $user->rights->facture->paiement) {
				if ($objectidnext) {
					print '<div class="inline-block divButAction"><span class="butActionRefused" title="' . $langs->trans("DisabledBecauseReplacedInvoice") . '">' . $langs->trans('Register payment') . '</span></div>';
				} else {
					if ($resteapayer == 0) {
						print '<div class="inline-block divButAction"><span class="butActionRefused" title="' . $langs->trans("DisabledBecauseRemainderToPayIsZero") . '">' . $langs->trans('Register payment') . '</span></div>';
					} else {
						print '<div class="inline-block divButAction"><a class="butAction" href="paiement.php?facid=' . $object->id . '&amp;action=create&amp;accountid='.$object->fk_account.'">' . $langs->trans('Register payment') . '</a></div>';
					}
				}
			}
	...
	// Select mail models is same action as presend
		...
		$genallowed = $user->rights->sales->pdf->write;
		$delallowed = $user->rights->sales->pdf->del;
