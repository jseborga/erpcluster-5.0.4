 ...
 */
class pdf_fractal extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					// VAT Rate
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN) && $abc)
					{
					...
					{
						$up_excl_tax = price($object->lines[$i]->price);
					...
					if ($conf->fiscal->enabled)
					{
						if ($object->lines[$i]->remise)
						{
							$pdf->SetXY($this->posxdiscount-2, $curY);
							$remise_percent = price($object->lines[$i]->remise);
							$pdf->MultiCell($this->postotalht-$this->posxdiscount+2, 3, $remise_percent, 0, 'R');
						}
						elseif ($object->lines[$i]->remise_percent)
						{
							$pdf->SetXY($this->posxdiscount-2, $curY);
					...
					// Total HT line
						$total_excl_tax = price($object->lines[$i]->total_ttc);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if($conf->global->PRODUCT_USE_UNITS) {
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell($this->posxdiscount - $this->posxunit - 1, 2, $outputlangs->transnoentities("Un."), '',
					'C');
			}
		}
		...
		if (empty($hidetop))
		{
			...
			if ($conf->fiscal->enabled)
				$pdf->MultiCell(30,2, $outputlangs->transnoentities("Value"),'','C');
			else
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Quotation");
