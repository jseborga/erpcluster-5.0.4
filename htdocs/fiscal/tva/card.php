...
require_once DOL_DOCUMENT_ROOT.'/fiscal/class/tvadet.class.php';
...
$result = restrictedArea($user,'fiscal','','','tax');
//if (!$user->rights->fiscal->tax->read) accessforbidden();

$object = new Tva($db);
$objectadd = new Tvaadd($db);
$objectdet = new Tvadet($db);
...
if ($action == 'setdatev' && $user->rights->fiscal->tax->write)
{
	$object->fetch($id);
...
if ($action == 'add' && $_POST["cancel"] <> $langs->trans("Cancel"))
{
	...
	if (! $error)
	{
		...
		if (!$error)
		{
			$objectadd->fk_tva = $ret;
			$objectadd->fk_type_tva = GETPOST('fk_type_tva');
			$objectadd->fk_user_create = $user->id;
			$objectadd->fk_user_mod = $user->id;
			$objectadd->datec = dol_now();
			$objectadd->datem = dol_now();
...
if ($action == 'delete')
{
	...
	if ($object->rappro == 0)
	{
		...
		if ($ret > 0)
		{
			//buscamos si tiene tvadet
			$filter = " AND t.fk_tva = ".$id;
...
if ($code_tva)
{
 	$result = $objTypetva->fetch(0,$code_tva,$mysoc->country_id);
	if ($result <= 0)
	{
		dol_print_error($db);
		exit;
...
if ($action == 'create')
{
	...
	print '<input type="hidden" name="fk_type_tva" value="'.$objTypetva->id.'">';
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Amount").'</td><td><input type="number" min="0" step="any" name="amount" size="10" value="'.GETPOST("amount").'"></td></tr>';
	...
	print '</table>';
	//cargamos si existe el detalle de ctypetvadet
	$filter = " AND t.fk_type_tva = ".$objTypetva->id;
...
if ($id)
{
	...
	if ($object->rappro == 0)
	{
		if (! empty($user->rights->fiscal->tax->del))
		{
