...
/**
 *   	\file       advance/paiementfournadvance_list.php
 *		\ingroup    advance
...
dol_include_once('/advance/class/paiementfournadvance.class.php');
...
$extralabels = $extrafields->fetch_name_optionals_label('advance');
...
't.fk_paiement'=>array('label'=>$langs->trans("Fieldfk_paiement"), 'checked'=>1),
...
't.multicurrency_amount'=>array('label'=>$langs->trans("Fieldmulticurrency_amount"), 'checked'=>1),
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/advance/list.php',1));
			exit;
		}
		else
		{
...
if ($search_fk_paiement) $sql.= natural_search("fk_paiement",$search_fk_paiement);
if ($search_num_paiement) $sql.= natural_search("num_paiement",$search_num_paiement);
...
if ($search_multicurrency_amount) $sql.= natural_search("multicurrency_amount",$search_multicurrency_amount);
...
if ($search_fk_paiement != '') $params.= '&amp;search_fk_paiement='.urlencode($search_fk_paiement);
if ($search_num_paiement != '') $params.= '&amp;search_num_paiement='.urlencode($search_num_paiement);
...
if ($search_multicurrency_amount != '') $params.= '&amp;search_multicurrency_amount='.urlencode($search_multicurrency_amount);
...
if (! empty($arrayfields['t.fk_paiement']['checked'])) print_liste_field_titre($arrayfields['t.fk_paiement']['label'],$_SERVER['PHP_SELF'],'t.fk_paiement','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.num_paiement']['checked'])) print_liste_field_titre($arrayfields['t.num_paiement']['label'],$_SERVER['PHP_SELF'],'t.num_paiement','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.multicurrency_amount']['checked'])) print_liste_field_titre($arrayfields['t.multicurrency_amount']['label'],$_SERVER['PHP_SELF'],'t.multicurrency_amount','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_paiement']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_paiement" value="'.$search_fk_paiement.'" size="10"></td>';
if (! empty($arrayfields['t.num_paiement']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_num_paiement" value="'.$search_num_paiement.'" size="10"></td>';
...
if (! empty($arrayfields['t.multicurrency_amount']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_multicurrency_amount" value="'.$search_multicurrency_amount.'" size="10"></td>';
