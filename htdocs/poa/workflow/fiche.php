...
$id        = GETPOST("id");
$idp       = GETPOST("idp"); //preventivo original
$idu       = GETPOST("idu");
$idw       = GETPOST("idw");
	 ...
	 if ($action <> 'edit' && $action <> 're-edit')
	   {
		 ...
		 $db->begin();
		 //cambiando el preventivo a statut 2
	     ...
	     if (!empty($idp) && $idp != $object->fk_poa_prev)
	       {
		 $objprev->fetch($idp);
		 // preventive presente gestion
		 print '<tr><td width="20%">'.$langs->trans('Preventivepresent').'</td><td colspan="7">';
		 print $objprev->nro_preventive.'/'.$objprev->gestion.' : '.$objprev->label;
		 print '</td>';
		 print '</tr>';
	     ...
	     print '</td>';
	     print '</tr>';
	     //buscamos el proceso del preventivo
	     $group = '';
	     if ($objproc->fetch_prev($object->fk_poa_prev)>0)
	       {
		 $group = $objproc->fk_type_con;
		 $aTypecontrat = fetch_tables($objproc->fk_type_con);
		 print '<tr><td>'.$langs->trans('Typecontracting').'</td><td colspan="7">';
		 print $aTypecontrat['label'];
		 print '</td>';
		 print '</tr>';
	       }
	     ...
	     print '</table>';
	     //para registro de envio modo grafico
	     if ($action == 'createtrans')
	       {
		 if ($user->admin || 
		     ($user->rights->poa->work->tra && $aUserArea[$codearea]))
		   {
		     include_once DOL_DOCUMENT_ROOT.'/poa/workflow/tpl/addtrans.tpl.php';
		   }
	       }
		 ...
		 if ($user->rights->poa->work->fin)
		   {
			 ...
			 (!$user->admin && $aUserArea[$codearea]) && $user->id == $objprev->fk_user_create)
		       {
			 $sequen++;
			 print "<a class=\"butAction\" href=\"fiche.php?id=".$id.'&idp='.$idp.'&codearea='.$codearea.'&sequen='.$sequen."&action=createfin\">".$langs->trans("Finalize")."</a>";
		       }
		     // if ($user->admin)
		 ...
		 if ($user->rights->poa->work->tra)
		   {
		     ...
		     if ($user->admin || 
			 (!$user->admin && $aUserArea[$codearea]) && $user->id == $objprev->fk_user_create)
		       print "<a class=\"butAction\" href=\"fiche.php?id=".$id.'&idp='.$idp."&action=createtransf\">".$langs->trans("Send")."</a>";
		   }
		 
		 if ($user->admin || 
		     ($user->rights->poa->work->mod && count($objworkd->array) <= 0))
		   print "<a class=\"butAction\" href=\"fiche.php?action=edit&id=".$object->id.'&idp='.$idp."\">".$langs->trans("Modify")."</a>";
		 else
		   print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Modify")."</a>";
		 
		 if ($user->rights->poa->work->del && count($objworkd->array) <=0 && $object->statut == 0)
		   print "<a class=\"butActionDelete\" href=\"fiche.php?action=delete&id=".$object->id.'&idp='.$idp."\">".$langs->trans("Delete")."</a>";
		 else
		 ...
		 print '</div>';
		 //select
		 print poa_grafic_select($group,$arrayl,$arrayc,$arrayk,$lastcode,$object->id,$idp);
