...
//$display = 'block';
print '<div id="'.$tagidp.'" class="modal fade in" tabindex="-1" role="dialog" style="display: '.$display.'; margin-top:0px;" data-width="760;" aria-hidden="false">';
...
if ($idProcess>0 && $idrcreg > 0)
{
	if ($user->rights->poa->op->rp)
	{
		...
		print '<input type="hidden" name="action" value="updaterp">';
	...
	//buscamos el registro en poa process contrat
		...
		if ($objpcon->date_provisional > 0)
			$aDate = dol_getdate($objpcon->date_provisional);
		$dateop = $aDate['year'].'-'.$aDate['mon'].'-'.$aDate['mday'];
	//date order proceed
		print '<div class="form-group">';
		print '<label class="col-sm-3 control-label" for="socid">'.$langs->trans('Provisional').'</label>';
		print '<div class="col-xs-8">';
		print '<div class="well well-sm">';
		print '<div class="input-group date" id="datepicinixx">';
		print '<input type="text" name="di_" id="datepicinip" class="flat" value="'.$dateop.'" readonly/>';
