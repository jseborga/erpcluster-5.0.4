...
if (! $user->rights->monprojet->eje->leer) accessforbidden();
...
if ($action == 'update' && ! $_POST["cancel"] && $user->rights->monprojet->eje->crear)
{
	$error=0;
	...
	//recargamos los valores
		...
		$_POST['options_unit_declared'] = $object->array_options['options_unit_declared'];
	...
	$objecttaskadd->unit_ejecuted = $_POST['options_unit_ejecuted'];
	$objecttaskadd->fk_user_mod = $user->id;
	$objecttaskadd->detail_close = $_POST['options_detail_close'];
...
if ($action == 'reopen' && ! $_POST["cancel"] && $user->rights->monprojet->eje->crear)
{
	$error=0;
	if (! $error)
	{
		...
		$object->fetch($id,$ref);
		$objadd->fetch($id,$ref);
		$res = $objadd->update_statut($id,0);
		if (!$res > 0)
			$error;
	// if (! $error)
	//   {
	//     $object->fk_statut = 0; //open
	//     $result=$objadd->update($user,1);
	//     if ($result < 0)
	//       {
	// 	$error++;
	// 	setEventMessages($object->error,$object->errors,'errors');
...
if ($action == 'edit' && $user->rights->projet->creer)
{
	...
	dol_fiche_head($head, 'task_close', $langs->trans("Task"),0,'projecttask');
...
if ($action != 'edit')
{
	...
	print '<div class="tabsAction">';
	// Modify
	if (!$lDisabled && $user->rights->monprojet->eje->crear && $projectstatic->statut == 1)
	{
		if ($object->fk_statut == 0)
			print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=edit&amp;withproject='.$withproject.'">'.$langs->trans('Closetask').'</a>';
		else
			print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=reopen&amp;withproject='.$withproject.'">'.$langs->trans('Opentask').'</a>';
	}
	else
	{
		print '<a class="butActionRefused" href="#" title="'.$langs->trans("NotAllowed").'">'.$langs->trans('Register').'</a>';
	}
