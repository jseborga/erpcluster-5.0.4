...
/**
 *   	\file       priceunits/productpricebudget_list.php
 *		\ingroup    priceunits
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-10-26 08:38
 */
...
dol_include_once('/priceunits/class/productpricebudget.class.php');
...
$hookmanager->initHooks(array('productpricebudgetlist'));
...
$object=new Productpricebudget($db);
...
't.work_hours'=>array('label'=>$langs->trans("Fieldwork_hours"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."product_price_budget as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."product_price_budget_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_work_hours != '') $params.= '&amp;search_work_hours='.urlencode($search_work_hours);
...
if (! empty($arrayfields['t.work_hours']['checked'])) print_liste_field_titre($arrayfields['t.work_hours']['label'],$_SERVER['PHP_SELF'],'t.work_hours','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.work_hours']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_work_hours" value="'.$search_work_hours.'" size="10"></td>';
