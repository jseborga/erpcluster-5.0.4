...
/**
 *   	\file       promotion/promotions_list.php
 *		\ingroup    promotion
...
$search_fk_type_promotion=GETPOST('search_fk_type_promotion','alpha');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'promotionlist';
...
$aActive = array(0=>$langs->trans('Deactive'),1=>$langs->trans('Active'));
...
$arrayfields=array(
	...
	't.fk_type_promotion'=>array('label'=>$langs->trans("Fieldfk_type_promotion"), 'checked'=>1),
...
$parameters=array();
$reshook=$hookmanager->executeHooks('doActions',$parameters,$objPromotion,$action);    // Note that $action and $objPromotion may have been modified by some hooks
...
$help_url='';
$title = $langs->trans('Promotionlist');
...
$sql .= " t.tms,";
$sql .= " t.status, ";
$sql .= " p.ref AS reftype, ";
$sql .= " p.label AS labeltype ";
...
// Add fields from hooks
	$parameters=array();
$reshook=$hookmanager->executeHooks('printFieldListSelect',$parameters);    // Note that $action and $objPromotion may have been modified by hook
$sql.=$hookmanager->resPrint;
$sql.= " FROM ".MAIN_DB_PREFIX."promotions as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."c_type_promotion as p ON t.fk_type_promotion = p.rowid";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."promotions_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_type_promotion) $sql.= natural_search(array("fk_type_promotion","p.ref","p.label"),$search_fk_type_promotion);
...
$parameters=array();
$reshook=$hookmanager->executeHooks('printFieldListWhere',$parameters);    // Note that $action and $objPromotion may have been modified by hook
$sql.=$hookmanager->resPrint;
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/promotions/card.php?id='.$id);
	exit;
}
...
if ($search_fk_type_promotion != '') $param.= '&amp;search_fk_type_promotion='.urlencode($search_fk_type_promotion);
...
$parameters=array();
$reshook=$hookmanager->executeHooks('printFieldPreListTitle',$parameters);    // Note that $action and $objPromotion may have been modified by hook
...
if (! empty($arrayfields['t.fk_type_promotion']['checked'])) print_liste_field_titre($arrayfields['t.fk_type_promotion']['label'],$_SERVER['PHP_SELF'],'t.fk_type_promotion','',$params,'',$sortfield,$sortorder);
...
$reshook=$hookmanager->executeHooks('printFieldListTitle',$parameters);    // Note that $action and $objPromotion may have been modified by hook
...
if (! empty($arrayfields['t.qty']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_qty" value="'.$search_qty.'" size="5"></td>';
...
if (! empty($arrayfields['t.fk_type_promotion']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_type_promotion" value="'.$search_fk_type_promotion.'" size="10"></td>';
...
// Fields from hook
$parameters=array('arrayfields'=>$arrayfields);
$reshook=$hookmanager->executeHooks('printFieldListOption',$parameters);    // Note that $action and $objPromotion may have been modified by hook
print $hookmanager->resPrint;
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'ref')
				{
					$objPromotion->id = $obj->rowid;
					$objPromotion->fk_product = $obj->fk_product;
					$objPromotion->ref = $obj->ref;
					$objPromotion->label = $obj->label;
					$obj->$key2 = $objPromotion->getNomUrltpl();
				}
				if ($key2 == 'active')
				{
					$img = 'switch_off';
				...
				if ($key2 == 'status')
				{
					$objPromotion->status = $obj->$key2;
					$obj->$key2 = $objPromotion->getLibStatut(3);
				}
		...
		$reshook=$hookmanager->executeHooks('printFieldListValue',$parameters);    // Note that $action and $objPromotion may have been modified by hook
		print $hookmanager->resPrint;
		// Date creation
		if (! empty($arrayfields['t.datec']['checked']))
		{
...
$reshook=$hookmanager->executeHooks('printFieldListFooter',$parameters);    // Note that $action and $objPromotion may have been modified by hook
print $hookmanager->resPrint;
...
if ($user->rights->promotion->prom->write)
	print '<a class="butAction" href="card.php?id='.$id.'&action=create">'.$langs->trans("New").'</a>';
else
