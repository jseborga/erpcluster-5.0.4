...
/**
 *   	\file       poa/poaareauser_list.php
 *		\ingroup    poa
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-05-24 08:27
 */
...
dol_include_once('/poa/class/poaareauser.class.php');
...
$hookmanager->initHooks(array('poaareauserlist'));
...
't.privilege'=>array('label'=>$langs->trans("Fieldprivilege"), 'checked'=>1),
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."poa_area_user_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if (! empty($arrayfields['t.privilege']['checked'])) print_liste_field_titre($arrayfields['t.privilege']['label'],$_SERVER['PHP_SELF'],'t.privilege','',$param,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_user']['checked'])) print '<td>'.$obj->fk_user.'</td>';
if (! empty($arrayfields['t.active']['checked'])) print '<td>'.$obj->active.'</td>';
if (! empty($arrayfields['t.privilege']['checked'])) print '<td>'.$obj->privilege.'</td>';
