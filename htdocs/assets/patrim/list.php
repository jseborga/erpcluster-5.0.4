...
/**
 *   	\file       assets/cassetspatrim_list.php
 *		\ingroup    assets
...
dol_include_once('/assets/class/cassetspatrim.class.php');
...
$hookmanager->initHooks(array('cassetspatrimlist'));
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/assets/patrim/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/assets/patrim/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Assetclasification');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_assets_patrim as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_assets_patrim_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($resql)
{
    ...
    if ($action == 'delete') {
    	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Deletepatrim'), $langs->trans('ConfirmDeletepatrim'), 'confirm_delete', '', 0, 1);
    	print $formconfirm;
    }
