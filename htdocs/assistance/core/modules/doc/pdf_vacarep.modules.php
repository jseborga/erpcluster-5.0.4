...
class pdf_vacarep extends ModelePDFAssistance
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->assistance->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->assistance->dir_output . "/licvac/".$period_year.'/report';
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aVacacion as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-1);
						...
						$pdf->MultiCell($this->posxref-$this->posxini-1, 1, $line['ref'], 0, 'L',0);
						
						$pdf->SetXY($this->posxref, $curY);
						$pdf->MultiCell($this->posxci-$this->posxref-1, 1, $line['ci'], 0, 'L',0);
						
						$pdf->SetXY($this->posxci, $curY);
						$pdf->MultiCell($this->posxpat-$this->posxci-1, 1, $line['paterno'], 0, 'L',0);
						
						$pdf->SetXY($this->posxpat, $curY);
						$pdf->MultiCell($this->posxmat-$this->posxpat-1, 1, $line['materno'], 0, 'L',0);
						
						$pdf->SetXY($this->posxmat, $curY);
						$pdf->MultiCell($this->posxnom-$this->posxmat-1, 1,$line['nombre'], 0, 'L',0);
						
						$pdf->SetXY($this->posxnom, $curY);
						$pdf->MultiCell($this->posxfsol-$this->posxnom-1, 1, $line['campoUno'], 0, 'C',0);
						
						$pdf->SetXY($this->posxfsol, $curY);
						$pdf->MultiCell($this->posxdsol-$this->posxfsol-1, 1, $line['campoDos'], 0, 'C',0);
						
						$pdf->SetXY($this->posxdsol, $curY);
						$pdf->MultiCell($this->posxtdva-$this->posxdsol-1, 1, $line['diasAsig'], 0, 'C',0);
						
						$pdf->SetXY($this->posxtdva, $curY);
						$pdf->MultiCell($this->posxtdvu-$this->posxtdva-1, 1, $line['diasUtil'], 0, 'C',0);
						
						$pdf->SetXY($this->posxtdvu, $curY);
						$pdf->MultiCell($this->posxsaldo-$this->posxtdvu-1, 1, $line['saldo'], 0, 'C',0);

						$pdf->SetXY($this->posxsaldo, $curY);
						$pdf->MultiCell($this->posxfin-$this->posxsaldo-1, 1, $line['estado'], 0, 'C',0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		*/
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxci-$this->posxref-1,2, $outputlangs->transnoentities("CI"),'','C');
			$pdf->line($this->posxci-1, $tab_top, $this->posxci-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxci-1, $tab_top+3);
			$pdf->MultiCell($this->posxpat-$this->posxci-1,2, $outputlangs->transnoentities("Paterno"),'','C');
			$pdf->line($this->posxpat-1, $tab_top, $this->posxpat-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxpat-1, $tab_top+2);
			$pdf->MultiCell($this->posxmat-$this->posxpat-1,2, $outputlangs->transnoentities("Materno"),'','C');
			$pdf->line($this->posxmat-1, $tab_top, $this->posxmat-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxmat-1, $tab_top+2);
			$pdf->MultiCell($this->posxnom-$this->posxmat-1,2, $outputlangs->transnoentities("Nombres"),'','C');
			$pdf->line($this->posxnom-1, $tab_top, $this->posxnom-1, $tab_top + $tab_height);

			if($aDatosAdd['fk_member']!= (-1)){
				$pdf->SetXY($this->posxnom-1, $tab_top+2);
				$pdf->MultiCell($this->posxfsol-$this->posxnom-1,2, $outputlangs->transnoentities("Fecha de Ingreso"),'','C');
				$pdf->line($this->posxfsol-1, $tab_top, $this->posxfsol-1, $tab_top + $tab_height);
			}else{
				$pdf->SetXY($this->posxnom-1, $tab_top+2);
				$pdf->MultiCell($this->posxfsol-$this->posxnom-1,2, $outputlangs->transnoentities("Fecha Solicitud"),'','C');
				$pdf->line($this->posxfsol-1, $tab_top, $this->posxfsol-1, $tab_top + $tab_height);
			}

			if($aDatosAdd['fk_member']!= (-1)){
				$pdf->SetXY($this->posxfsol-1, $tab_top+2);
				$pdf->MultiCell($this->posxdsol-$this->posxfsol-1,2, $outputlangs->transnoentities("Gestion"),'','C');
				$pdf->line($this->posxdsol-1, $tab_top, $this->posxdsol-1, $tab_top + $tab_height);
			}else{
				$pdf->SetXY($this->posxfsol-1, $tab_top+2);
				$pdf->MultiCell($this->posxdsol-$this->posxfsol-1,2, $outputlangs->transnoentities("Dias Solicitados"),'','C');
				$pdf->line($this->posxdsol-1, $tab_top, $this->posxdsol-1, $tab_top + $tab_height);
			}
			
			$pdf->SetXY($this->posxdsol-1, $tab_top+2);
			$pdf->MultiCell($this->posxtdva-$this->posxdsol-1,2, $outputlangs->transnoentities("Total dias Vacacion Asisgnados"),'','C');
			$pdf->line($this->posxtdva-1, $tab_top, $this->posxtdva-1, $tab_top + $tab_height);
			
			$pdf->SetXY($this->posxtdva-1, $tab_top+2);
			$pdf->MultiCell($this->posxtdvu-$this->posxtdva-1,2, $outputlangs->transnoentities("Total dias Vacacion Utilizados"),'','C');
			$pdf->line($this->posxtdvu-1, $tab_top, $this->posxtdvu-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxtdvu-1, $tab_top+2);
			$pdf->MultiCell($this->posxsaldo-$this->posxtdvu-1,2, $outputlangs->transnoentities("Saldo"),'','C');
			$pdf->line($this->posxsaldo-1, $tab_top, $this->posxsaldo-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxsaldo-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxsaldo-1,2, $outputlangs->transnoentities("Estado"),'','C');
			//$pdf->line($this->posxci-1, $tab_top, $this->posxci-1, $tab_top + $tab_height);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$titulo = "LISTADO DE VACACIONES SOLICITADAS";
		...
		if($aDatosAdd['fk_member']!= (-1)){
			...
			//$title=$outputlangs->transnoentities("Determinacion de Servicios");
			//$title=$outputlangs->transnoentities($titulo);
			$pdf->MultiCell(270, 3, "DE : ".$aDatosAdd['nomMember'], '', 'L');
		}else{
			//Formulario
			$posy+=14;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DESDE FECHA")." : ". dol_print_date($aDatosAdd['fecha_a'],'daytext'), '', 'R');

			//Fecha de elaborarion
			$posy+=7;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("HASTA FECHA")." : " . dol_print_date($aDatosAdd['fecha_b'],'daytext'), '', 'R');
