 ...
 */
abstract class ModelePDFFinInt extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleFinInt extends CommonDocGenerator
{
	var $error='';
...
abstract class ModeleNumRefFinInt
{
	var $error='';
 ...
 */
function finint_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false)
{
	...
	{
	    if (! empty($conf->global->FININT_CASHAPPROVED_PDF))
	    {
	        $modele = $conf->global->FININT_CASHAPPROVED_PDF;
	    }
	    else
	    {
	        $modele = 'crabe';
	...
	foreach($dirmodels as $reldir)
	{
    	foreach(array('doc','pdf') as $prefix)
    	{
	        ...
	        $file=dol_buildpath($reldir."finint/core/modules/doc/".$file,0);
    		if (file_exists($file))
    		{
    			$filefound=1;
	...
	if ($filefound)
	{
		...
		if ($obj->write_file($object, $outputlangs, $srctemplatepath, $hidedetails, $hidedesc, $hideref, $hookmanager) > 0)
		{
			...
			$result=$interface->run_triggers('FININT_CASHAPPROVED',$object,$user,$langs,$conf);
		...
		{
			$outputlangs->charset_output=$sav_charset_output;
			dol_print_error($db,"finint_pdf_create Error: ".$obj->error);
			return -1;
		}
