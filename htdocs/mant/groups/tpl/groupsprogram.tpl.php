...
$arrayfields=array(
	...
	't.type'=>array('label'=>$langs->trans("Type"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."m_groups_program as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."m_type_repair AS r ON t.fk_type_repair = r.rowid ";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."m_groups_program_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE t.fk_group = ".$fk_group;
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_group) $sql.= natural_search("fk_group",$search_fk_group);
if ($search_fk_type_repair) $sql.= natural_search("fk_type_repair",$search_fk_type_repair);
if ($search_fk_parent_previous) $sql.= natural_search("fk_parent_previous",$search_fk_parent_previous);
...
print '<form name="Form" id="Form" method="POST" action="'.$_SERVER["PHP_SELF"].'">';
...
//
if (! empty($arrayfields['t.fk_group']['checked'])) print_liste_field_titre($arrayfields['t.fk_group']['label'],$_SERVER['PHP_SELF'],'t.fk_group','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_parent_previous']['checked'])) print_liste_field_titre($arrayfields['t.fk_parent_previous']['label'],$_SERVER['PHP_SELF'],'t.fk_parent_previous','',$params,'',$sortfield,$sortorder);
...
// Fields title search
	print '<tr class="liste_titre">';
//
	if (! empty($arrayfields['t.fk_group']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_group" value="'.$search_fk_group.'" size="10"></td>';
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<select id="fk_type_repair" name="fk_type_repair">'.$options.'</select>'.'</td>';
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		$var = !$var;
		// Show here line of result
		if ($obj->fk_type_repair == $fkProgramc)
			print '<tr class="selmarkc">';
		elseif ($obj->fk_type_repair == $fkPrograme)
		...
		if ($action == 'edit' && $idr == $obj->rowid)
		{
			...
			{
				$selected = '';
				if ($line->id == $obj->fk_type_repair) $selected = ' selected';
			...
			{
				print '<td>';
				print $form->selectarray('fk_parent_previous',$aMEPTypeRepair,$obj->fk_parent_previous,1);
				print '</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.type']['checked']))
			{
				print '<td>'.$form->selectarray('type',$aType,$obj->type,1).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.accountant']['checked']))
			{
		...
		// Action column
			...
			{
				if (!$editdisabled)
					print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$obj->rowid.'&action=edit">'.img_picto($langs->trans('Edit'),'edit').'</a>';
			}
			print '</td>';
			if (! $i) $totalarray['nbfield']++;
		}
