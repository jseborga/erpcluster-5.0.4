...
class Taskext extends Task
{
	...
	*/
	 ...
	 */
	function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false,$filteradd='')
	{
	...
	function getTasksArray($usert=0, $userp=0, $projectid=0, $socid=0, $mode=0, $filteronprojref='', $filteronprojstatus=-1, $morewherefilter='',$filteronprojuser=0,$filterontaskuse=0,$limit=0,$ofset=1,$filterdate=0,$idpay='',$modepay=0)
	{
		...
		{
			require_once DOL_DOCUMENT_ROOT.'/request/class/request.class.php';
			require_once DOL_DOCUMENT_ROOT.'/request/class/requestitem.class.php';
			$request = new Request($this->db);
			$requestitem = new Requestitem($this->db);
		}
				...
				if (! $error)
				{
					...
					if ($filterdate)
					{
						...
						if ($lFilter)
						{
							...
							if ($this->db->jdate($obj->date_start) <= $datee)
								$lAdd = true;
							else
								$lAdd = false;
						}
					...
					if ($lAdd )
					{
						...
						if (empty($this->date_ini)) $this->date_ini = $this->db->jdate($obj->date_start);
						else
						{
						...
						//$objNewtask = new Taskext($this->db);
						//$objNewtask->array_options['options_fk_unit'] = $obj->fk_unit;

						//$unit = $objNewtask->getLabelOfUnit();
						$unit = '';
						if (!empty($unit)) {
							$tasks[$i]->array_options['options_unit'] = $langs->trans($unit);
						}
						//$unitc = $objNewtask->getLabelOfUnit('code');
						$unitc = '';
						if (!empty($unitc)) {
	...
	static function showphoto($imageview,$document,$task_time,$modulepart, $object, $projectstatic,$width=100, $height=0, $caneditfield=0, $cssclass='photowithmargin', $imagesize='', $addlinktofullsize=1, $cache=0,$docext='')
	{
		global $conf,$langs;
		...
		$file = $projectstatic->ref.'/'.$object->ref.'/'.$id.'/';
			//$dirfile= '/'.$projectstatic->ref.'/'.$object->ref.'/'.$id;
		$originalfile = $projectstatic->ref.'/'.$object->ref.'/';
		...
		if ($dir)
		{
			...
			if ($file && file_exists($dir."/".$file))
			{
				...
				$ret.='<img alt="'.$docext.'" id="photologo'.(preg_replace('/[^a-z]/i','_',$file)).'" class="'.$cssclass.'" '.($width?' width="'.$width.'"':'').($height?' height="'.$height.'"':'').' src="'.DOL_URL_ROOT.'/viewimage.php?modulepart='.$modulepart.'&entity='.$entity.'&file='.urlencode($file).'&cache='.$cache.'">';
				if ($addlinktofullsize) $ret.='</a>';
			}
			else if ($altfile && file_exists($dir."/".$altfile))
			{
				if ($addlinktofullsize) $ret.='<a href="'.DOL_URL_ROOT.'/viewimage.php?modulepart='.$modulepart.'&entity='.$entity.'&file='.urlencode($originalfile).'&cache='.$cache.'" target="_blank">';
