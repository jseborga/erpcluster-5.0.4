...
't.fk_country'=>array('label'=>$langs->trans("Fieldfk_country"), 'checked'=>0),
't.fk_region'=>array('label'=>$langs->trans("Fieldfk_region"), 'checked'=>0),
...
if ($search_type != -1) $sql.= natural_search("type",$search_type);
...
if (! empty($arrayfields['t.fk_region']['checked'])) print_liste_field_titre($arrayfields['t.fk_region']['label'],$_SERVER['PHP_SELF'],'t.fk_region','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.type']['checked']))
{
	 ...
	 $aYesno=array(0=>$langs->trans('Not'),1=>$langs->trans('Yes'));
	 print $form->selectarray('search_type',$aYesno,$search_type,1);
	 print '</td>';
}
...
if (! empty($arrayfields['t.fk_region']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_region" value="'.$search_fk_region.'" size="10"></td>';
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				{
					if (empty($obj->$key2))
						$obj->$key2 = $langs->trans('Not');
					else
						$obj->$key2 = $langs->trans('Yes');
				}
				if ($key2 == 'date_year')
				{
					if (empty($obj->$key2))
...
if (empty($reshook))
{
	...
	{
		print '&nbsp;<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/orgman/holiday/card.php'.'?action=create">'.$langs->trans("New").'</a></div>'."\n";
	}
}
print '</div>'."\n";
