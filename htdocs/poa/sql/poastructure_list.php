...
$search_fk_poa_objetive=GETPOST('search_fk_poa_objetive','int');
$search_fk_father=GETPOST('search_fk_father','int');
$search_fk_area=GETPOST('search_fk_area','int');
$search_fk_area_ej=GETPOST('search_fk_area_ej','int');
...
't.fk_poa_objetive'=>array('label'=>$langs->trans("Fieldfk_poa_objetive"), 'checked'=>1),
't.fk_father'=>array('label'=>$langs->trans("Fieldfk_father"), 'checked'=>1),
't.fk_area'=>array('label'=>$langs->trans("Fieldfk_area"), 'checked'=>1),
't.fk_area_ej'=>array('label'=>$langs->trans("Fieldfk_area_ej"), 'checked'=>1),
...
$search_statut='';
    ...
    $permtoread = $user->rights->poastructure->read;
    $permtodelete = $user->rights->poastructure->delete;
    $uploaddir = $conf->poastructure->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."poa_structure_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_poa_objetive) $sql.= natural_search("fk_poa_objetive",$search_fk_poa_objetive);
...
if ($search_fk_area_ej) $sql.= natural_search("fk_area_ej",$search_fk_area_ej);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/poastructure/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.fk_poa_objetive']['checked'])) print_liste_field_titre($arrayfields['t.fk_poa_objetive']['label'],$_SERVER['PHP_SELF'],'t.fk_poa_objetive','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_area_ej']['checked'])) print_liste_field_titre($arrayfields['t.fk_area_ej']['label'],$_SERVER['PHP_SELF'],'t.fk_area_ej','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_poa_objetive']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_poa_objetive" value="'.$search_fk_poa_objetive.'" size="10"></td>';
...
if (! empty($arrayfields['t.fk_area_ej']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_area_ej" value="'.$search_fk_area_ej.'" size="10"></td>';
