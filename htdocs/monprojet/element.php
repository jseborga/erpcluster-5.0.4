...
'propal'=>array(
	...
	'table'=>'propal',
    'datefieldname'=>'datep',
    'urlnew'=>DOL_URL_ROOT.'/sales/propal/card.php?action=create&origin=project&originid='.$id.'&socid='.$socid,
    'lang'=>'propal',
    'buttonnew'=>'AddProp',
...
'order'=>array(
    ...
    'urlnew'=>DOL_URL_ROOT.'/sales/commande/card.php?action=create&projectid='.$id.'&socid='.$socid,
    'lang'=>'orders',
...
'invoice'=>array(
	...
	'table'=>'facture',
	'datefieldname'=>'datef',
    'urlnew'=>DOL_URL_ROOT.'/sales/compta/facture.php?action=create&projectid='.$id.'&socid='.$socid,
    'lang'=>'bills',
...
'order_supplier'=>array(
	...
	'datefieldname'=>'date_commande',
    'urlnew'=>DOL_URL_ROOT.'/purchase/commande/card.php?action=create&projectid='.$id.'&socid='.$socid,
    'lang'=>'suppliers',
...
'invoice_supplier'=>array(
	...
	'class'=>'FactureFournisseurext',
	'margin'=>'minus',
	'table'=>'facture_fourn',
	'datefieldname'=>'datef',
    'urlnew'=>DOL_URL_ROOT.'/purchase/facture/card.php?action=create&projectid='.$id.'&socid='.$socid,
    'lang'=>'suppliers',
...
'contract'=>array(
    ...
    'urlnew'=>DOL_URL_ROOT.'/sales/contrat/card.php?action=create&projectid='.$id.'&socid='.$socid,
    'lang'=>'contracts',
...
'intervention'=>array(
	...
	'disableamount'=>1,
    'urlnew'=>DOL_URL_ROOT.'/sales/fichinter/card.php?action=create&origin=project&originid='.$id.'&socid='.$socid,
    'lang'=>'interventions',
...
if ($conf->purchase->enabled)
{
	$listofreferent['order_supplier']=array(
	'name'=>"SuppliersOrders",
	'title'=>"ListSupplierOrdersAssociatedProject",
	'class'=>'FournisseurCommandeext',
	'table'=>'commande_fournisseur',
	'datefieldname'=>'date_commande',
    'urlnew'=>DOL_URL_ROOT.'/purchase/commande/card.php?action=create&projectid='.$id.'&socid='.$socid,
    'lang'=>'suppliers',
...
foreach ($listofreferent as $key => $value)
{
	...
	// If this element must be included into profit calculation ($margin is 'minus' or 'plus')
	{
		...
		if (is_array($elementarray) && count($elementarray)>0)
		{
			...
			for ($i = 0; $i < $num; $i++)
			{
				...
				else
				{
					...
					{
						if ($classname == 'ActionComm')
							print $element->getNomUrl(1);
						else
							print $element->getNomUrladd(1);
					}
...
//agregar aqui manualmente de almacenes
//
$filterstatic = " AND t.fk_projet = ".$id;
$solalmacen = new Solalmacenext($db);
$solalmacen->fetchAll('DESC', 'date_creation', 0, 0,array(1=>1),'AND', $filterstatic);

print '<table class="centpercent notopnoleftnoright" summary="" style="margin-bottom: 2px;">';
print '<tr>';
print '<td>'.$langs->trans('Listado de pedidos al almacen vinculados al proyecto').'</td>';
print '</tr>';
print '<tr class="liste_titre">';
print '<th>'.$langs->trans('Ref').'</th>';
print '<th>'.$langs->trans('Solicitante').'</th>';
print '<th>'.$langs->trans('Date').'</th>';
print '<th>'.$langs->trans('Statut').'</th>';
print '</tr>';
$objuser = new User($db);
$lines = $solalmacen->lines;
foreach ((array) $lines AS $j => $line)
{
	$solalmacen->id = $line->id;
	$solalmacen->ref = $line->ref;
	$solalmacen->statut = $line->statut;
	$objuser->fetch($line->fk_user);
	$var=!$var;
	print "<tr $bc[$var]>";
	print '<td>'.$solalmacen->getNomUrl(1).'</td>';
	print '<td>'.$objuser->getNomUrl(1).'</td>';
	print '<td>'.dol_print_date($line->date_creation,'day').'</td>';
	print '<td>'.$solalmacen->getLibStatut(4).'</td>';
	print '</tr>';
}
print '</table>';
