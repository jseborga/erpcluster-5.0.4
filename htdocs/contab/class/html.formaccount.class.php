...
class Formaccount extends Form
{
	 ...
	 */
	function select_account($selectid, $htmlname = 'account', $showempty = 0, $event = array(), $select_in = 0, $select_out = 0, $morecss='maxwidth300 maxwidthonsmartphone', $usecache='',$filter)
	{
		global $conf;
		...
		else
		{
			...
			$sql .= " INNER JOIN ".MAIN_DB_PREFIX."accounting_account_add as ta ON ta.fk_accounting_account = aa.rowid";
	 ...
	 * @return string                      String with HTML select
	 */
	function select_accountaux($selectid, $htmlname = 'account', $showempty = 0, $event = array(), $select_in = 0, $select_out = 0, $morecss='maxwidth300 maxwidthonsmartphone', $usecache='',$filter)
	{
		global $conf;
		...
		else
		{
    		...
    		$sql = "SELECT DISTINCT aux.ref AS account_number, aux.label, aux.rowid, aa.fk_pcg_version ";
    		$sql .= " FROM " . MAIN_DB_PREFIX . "accounting_account_aux as aux";
    		$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "accounting_account as aa ON aux.fk_accounting_account = aa.rowid";
    		$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "accounting_system as asy ON aa.fk_pcg_version = asy.pcg_version";
			//$sql .= " INNER JOIN ".MAIN_DB_PREFIX."accounting_account_add as ta ON ta.fk_accounting_account = aa.rowid";
    		$sql .= " AND asy.rowid = " . $conf->global->CHARTOFACCOUNTS;
    		...
    		if ($filter)
    			$sql.= $filter;
    		$sql .= " ORDER BY aux.ref";

    		dol_syslog(get_class($this) . "::select_accountaux", LOG_DEBUG);
    		$resql = $this->db->query($sql);
