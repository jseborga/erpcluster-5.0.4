...
/**
 *   	\file       priceunits/puformulasdet_list.php
 *		\ingroup    priceunits
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-10-25 08:49
 */
...
dol_include_once('/priceunits/class/puformulasdet.class.php');
...
$search_ref_formula=GETPOST('search_ref_formula','alpha');
$search_fk_operator=GETPOST('search_fk_operator','int');
$search_type=GETPOST('search_type','alpha');
$search_changefull=GETPOST('search_changefull','alpha');
...
$hookmanager->initHooks(array('puformulasdetlist'));
...
't.ref_formula'=>array('label'=>$langs->trans("Fieldref_formula"), 'checked'=>1),
't.fk_operator'=>array('label'=>$langs->trans("Fieldfk_operator"), 'checked'=>1),
't.type'=>array('label'=>$langs->trans("Fieldtype"), 'checked'=>1),
't.changefull'=>array('label'=>$langs->trans("Fieldchangefull"), 'checked'=>1),
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pu_formulas_det_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_ref_formula) $sql.= natural_search("ref_formula",$search_ref_formula);
...
if ($search_changefull) $sql.= natural_search("changefull",$search_changefull);
...
if ($search_ref_formula != '') $params.= '&amp;search_ref_formula='.urlencode($search_ref_formula);
...
if ($search_changefull != '') $params.= '&amp;search_changefull='.urlencode($search_changefull);
...
if (! empty($arrayfields['t.ref_formula']['checked'])) print_liste_field_titre($arrayfields['t.ref_formula']['label'],$_SERVER['PHP_SELF'],'t.ref_formula','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_operator']['checked'])) print_liste_field_titre($arrayfields['t.fk_operator']['label'],$_SERVER['PHP_SELF'],'t.fk_operator','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.changefull']['checked'])) print_liste_field_titre($arrayfields['t.changefull']['label'],$_SERVER['PHP_SELF'],'t.changefull','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.ref_formula']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ref_formula" value="'.$search_ref_formula.'" size="10"></td>';
...
if (! empty($arrayfields['t.changefull']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_changefull" value="'.$search_changefull.'" size="10"></td>';
