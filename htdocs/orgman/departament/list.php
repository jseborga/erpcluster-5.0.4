...
/**
 *   	\file       orgman/pdepartament_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-04-02 12:28
 */
...
$search_fk_father=GETPOST('search_fk_father','alpha');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmanDepartamentlist';
...
$arrayfields=array(
	...
	't.fk_father'=>array('label'=>$langs->trans("Fieldfk_father"), 'align'=>'align="left"', 'checked'=>1),
	't.fk_user_resp'=>array('label'=>$langs->trans("Fieldfk_user_resp"), 'align'=>'align="left"', 'checked'=>1),
...
if (empty($reshook))
{
	...
	//$permtoread = $user->rights->pdepartament->read;
	//$permtodelete = $user->rights->pdepartament->delete;
	//$uploaddir = $conf->pdepartament->dir_output;
...
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."adherent AS u ON t.fk_user_resp = u.rowid ";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_departament AS d ON t.fk_father = d.rowid ";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_departament_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE t.entity = ".$conf->entity;

if ($sref)
{
	$sql.= " AND t.ref like '%".$sref."%'";
}
if ($sall)
{
	$sql.= " AND (u.lastname like '%".$sall."%' OR u.firstname like '%".$sall."%' OR u.email like '%".$sall."%')";
}
...
if ($search_fk_father) $sql.= natural_search(array("t.fk_father","d.ref","d.label"),$search_fk_father);
if ($search_fk_user_resp) $sql.= natural_search(array("t.fk_user_resp","u.firstname","u.lastname"),$search_fk_user_resp);
//if ($search_fk_user_resp) $sql.= natural_search(array("fk_user_resp",$search_fk_user_resp);
...
if ($search_active!=9) $sql.= natural_search("t.active",$search_active);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/pdepartament/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.fk_father']['checked'])) print_liste_field_titre($arrayfields['t.fk_father']['label'],$_SERVER['PHP_SELF'],'t.fk_father','',$params,$arrayfields['t.fk_father']['align'],$sortfield,$sortorder);

if (! empty($arrayfields['t.fk_user_resp']['checked'])) print_liste_field_titre($arrayfields['t.fk_user_resp']['label'],$_SERVER['PHP_SELF'],'t.fk_user_resp','',$params,$arrayfields['t.fk_user_resp']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_father']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_father']['align'].'><input type="text" class="flat" name="search_fk_father" value="'.$search_fk_father.'" size="10"></td>';
if (! empty($arrayfields['t.fk_user_resp']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_user_resp']['align'].'><input type="text" class="flat" name="search_fk_user_resp" value="'.$search_fk_user_resp.'" size="10"></td>';
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'fk_father')
				{
					...
					{
						print '<td>'.$objectf->getNomUrl(1,'','','','',1).'</td>';
					}
					else
					{
						print '<td>&nbsp;</td>';
				...
				if ($key2 == 'fk_user_resp')
				{
					if ($obj->$key2 >0)
					{
						$objAdherent->id = $obj->fk_user_resp;
						$objAdherent->ref = $obj->fk_user_resp;
				...
				if($key2!='fk_father' && $key2!='fk_user_resp')
