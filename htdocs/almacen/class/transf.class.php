	...
	*/
	...
	function add_transfer_ok($user, $id_entrepot, $nbpiece, $movement, $label='', $price=0, $idtemp='',$fk_type_mov=0,$datem='')
	{
		if ($id_entrepot)
		{
			...
			$op[0] = "+".trim($nbpiece);
			$op[1] = "-".trim($nbpiece);
			$movementstock=new MouvementStockext($this->db);
			//origin
			...
			$result=$movementstock->_createadd($user,$this->id,$id_entrepot,$op[$movement],$movement,$price,$label,'',$datem,'','',false);
			if ($result >= 0)
			{
				$this->db->commit();
				return $result;
