...
/**
 *   	\file       almacen/solalmacenlog_list.php
 *		\ingroup    almacen
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-06-16 10:15
 */
...
$search_fk_solalmacen=GETPOST('search_fk_solalmacen','int');
...
$sql.= " FROM ".MAIN_DB_PREFIX."sol_almacen_log as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."sol_almacen_log_extrafields as ef on (t.rowid = ef.fk_object)";
//$sql.= " WHERE 1 = 1";
$sql.= " WHERE t.fk_solalmacen = ".$id;

if ($search_fk_solalmacen) $sql.= natural_search("fk_solalmacen",$search_fk_solalmacen);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/solalmacenlog/card.php?id='.$id);
	exit;
}
...
//
if (! empty($arrayfields['t.fk_solalmacen']['checked'])) print_liste_field_titre($arrayfields['t.fk_solalmacen']['label'],$_SERVER['PHP_SELF'],'t.fk_solalmacen','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.datec']['checked']))  print_liste_field_titre($arrayfields['t.datec']['label'],$_SERVER["PHP_SELF"],"t.datec","",$param,'align="left" class="nowrap"',$sortfield,$sortorder);
...
// Fields title search
	print '<tr class="liste_titre">';
//
	if (! empty($arrayfields['t.fk_solalmacen']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_solalmacen" value="'.$search_fk_solalmacen.'" size="10"></td>';
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		{
			print '<td>'.$langs->trans($obj->description).'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
