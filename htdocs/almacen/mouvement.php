...
if ($day > 0 && $month > 0 && $year > 0)
{
	...
	$search_dateini = dol_mktime(23,59,59,$aDate['month'],$aDate['day'],$aDate['year']);
	$datestr = (strlen($day)==1?'0'.$day:$day).'/'.(strlen($month)==1?'0'.$month:$month).'/'.$year.' 23:59:59';

	$search_datefin = dol_stringtotime($datestr);
	$search_datefin = dol_mktime(23,59,59,$month,$day,$year);
}
...
if ($action == 'excel')
{
	...
	*/
	...
	if ($conf->global->ALMACEN_FILTER_YEAR && $typemethod>0)
