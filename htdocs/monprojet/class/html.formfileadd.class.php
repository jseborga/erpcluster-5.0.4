...
class FormFileAdd extends FormFile
{
    var $db;
    var $error;
     ...
     */
    function form_attach_new_fileadd($url, $title='', $addcancel=0, $sectionid=0, $perm=1, $size=50, $object='', $options='', $useajax=1, $savingdocmask='', $linkfiles=1, $htmlname='formuserfile',$objecttype='project')
    {
        ...
        else
       {
            ...
            if ($linkfiles)
            {
	            ...
	            $out .= '<input type="hidden" name="objecttype" value="' . $objecttype . '">';
     ...
     */
    function show_documentsadd($modulepart,$modulesubdir,$filedir,$urlsource,$genallowed,$delallowed=0,$modelselected='',$allowgenifempty=1,$forcenomultilang=0,$iconPDF=0,$maxfilenamelength=28,$noform=0,$param='',$title='',$buttonlabel='',$codelang='')
    {
        $this->numoffiles=0;
        print $this->showdocumentsadd($modulepart,$modulesubdir,$filedir,$urlsource,$genallowed,$delallowed,$modelselected,$allowgenifempty,$forcenomultilang,$iconPDF,$maxfilenamelength,$noform,$param,$title,$buttonlabel,$codelang);
     ...
     */
    function showdocumentsadd($modulepart,$modulesubdir,$filedir,$urlsource,$genallowed,$delallowed=0,$modelselected='',$allowgenifempty=1,$forcenomultilang=0,$iconPDF=0,$maxfilenamelength=28,$noform=0,$param='',$title='',$buttonlabel='',$codelang='',$morepicto='')
    {
     ...
     */
    function getDocumentsLinkadd($modulepart, $modulesubdir, $filedir,array $lines = array(),$code='',$useimg='',$usepath='',$filter=1)
    {
    	...
    	$this->numoffiles=0;
	$excludefilter = '\.meta$|\.png$';
	$excludefilter = '\.meta$';
	if ($filter)
	  $filter = preg_quote($modulesubdir.'.pdf','/');
	else
	  $filter = '';
    	$file_list=dol_dir_list($filedir, 'all', 0, $filter, $excludefilter);
    	// For ajax treatment
    	...
    	if (! empty($file_list))
	  {
		...
		$aNome = explode('.',$file['name']);
		$idrow = $aNome[0];
		if ($usepath)
		  $relativepath=$usepath.'/'.$file['name'];
		else
		  if ($modulesubdir) $relativepath=$modulesubdir."/".$file["name"];	// Cas propal, facture...
		// Autre cas
		...
		if (preg_match('/text/',$mime)) $out.= ' target="_blank"';
		$out.= '>';
		$title = select_attachment($lines[$idrow]->code,'','',0,1);
		$out.= img_print($file["name"],$title,$modulepart,$useimg);
     ...
     */
	function list_of_documentsadd($filearray,$object,$modulepart,$param='',$forcedownload=0,$relativepath='',$permtodelete=1,$useinecm=0,$textifempty='',$maxlength=0,$title='',$url='', $showrelpart=0)
	{
		...
		else
		{
			...
			foreach($filearray as $key => $file)      // filearray must be only files here
			{
						...
						&& ! preg_match('/\.meta$/i',$file['name']))
				{
					...
					print '<a data-ajax="false" href="'.DOL_URL_ROOT.'/monprojet/documentview.php?modulepart='.$modulepart;
     ...
     */
    public function listOfLinksadd($object, $permtodelete=1, $action=null, $selected=null, $param='',$objecttype='project')
    {
        ...
        $res = $link->fetchAll($links, $objecttype, $object->id, $sortfield, $sortorder);
