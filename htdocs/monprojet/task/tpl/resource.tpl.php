...
$filterstatic.= " AND t.fk_projet_task = ".$fk_task_parent;
...
foreach((array) $objectptr->lines AS $l => $lineb)
{
	...
	if (GETPOST('idreg') == $lineb->id && ($action == 'editres' || $action == 'createaddr'))
	{
		...
		if ($newdata->objectdet == 'product')
		{
			...
			$fk_product_task = $newdata->fk_objectdet;
		}
		if ( $projectstatic->statut == 1 && $object->fk_statut < 2)
			include DOL_DOCUMENT_ROOT.'/monprojet/task/tpl/add_resource.tpl.php';
	}
	else
	{
		print "<tr $bc[$var] id=".'"'.$lineb->id.'"'.">";
		$aColor[0] = substr($categorie->color,0,2);
		$aColor[1] = substr($categorie->color,2,2);
		$aColor[2] = substr($categorie->color,4,2);
		$textcolor = '#'.$aColor[2].$aColor[2].$aColor[1];
		print '<td align="center" class="none">'.dol_print_date($lineb->date_resource,'day').'</td>';
		print '<td style="background-color:#'.$categorie->color.'; color:'.$textcolor.';">'.$categorie->label.'</td>';
		...
		elseif ($lineb->object == 'assets')
		{
			$assets->fetch($lineb->fk_object);
			print '<td>'.$assets->getNomurl(1).'</td>';
		}
		else
		{
			print '<td></td>';
		...
		print '<td align="right"><div class="text" id="quant-'.$lineb->id.'">'.price($lineb->quant).'</div></td>';
		...
		if ($user->rights->budget->budr->mod  && $projectstatic->statut == 1 && $object->fk_statut < 2)
		...
		if ($user->rights->budget->budr->del  && $projectstatic->statut == 1 && $object->fk_statut < 2)
