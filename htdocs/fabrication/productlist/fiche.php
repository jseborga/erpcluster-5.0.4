...
require_once DOL_DOCUMENT_ROOT.'/fabrication/class/productlist.class.php';
...
// Ajout entrepot
if ($action == 'updateqty'  && $user->rights->fabrication->mat->write)
{
...
if ($action == 'addlines' && $user->rights->fabrication->mat->write)
{
...
// Ajout entrepot
if ($action == 'addlinesson' && $user->rights->fabrication->mat->write)
{
...
// Delete warehouse
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->fabrication->mat->del)
{
	$objectl = new Productlist($db);
...
if ($action == 'update' && $user->rights->fabrication->mat->write && $_POST["cancel"] <> $langs->trans("Cancel"))
{
...
$tab = 'material';
dol_fiche_head($head, $tab, $langs->trans("Fabrication"),0,($object->public?'projectpub':'project'));

if ($action == 'create' && $user->rights->fabrication->mat->write)
{
  ...
  // 	    {
  // 	      $var=!$var;
  // 	      $objfabcom->formAddPredefinedProduct_lm(0,$mysoc,$soc,$hookmanager);
  // 	    }
  // 	}

  //     $parameters=array();
  //     $reshook=$hookmanager->executeHooks('formAddObjectLine',$parameters,$objfabcom,$action);    // Note that $action and $object may have been modified by hook
  //   }
  // print '</table>';
  // print '</td>';
  // print '</tr>';
...
else
{
	if ($id>0)
	{
	  ...
	  //unit
			...
			if ($user->rights->fabrication->mat->write && $action != 'editqty')
				print $qty_father.' '.'<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&action=editqty">'.img_picto($langs->trans('Edit'),'edit').'</a>';
			elseif ($id && $user->rights->fabrication->mat->write && $action == 'editqty')
			{
				print '<form action="'.$_SERVER['PHP_SELF'].'" method="post">';
				print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
			...
			if ($action == '')
			{
				...
				if (($object->statut==1 || $object->statut==0 ) && $user->rights->fabrication->mat->write)
				{
					if ($lCreate)
		  ...
		  // 	print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Delete")."</a>";
			...
			if ($lCreate)
			{
				...
				if ($action == 'createnew')
				{
					...
					$objfabcom->qty_father = $qty_father;
					if ($conf->global->MAIN_FEATURES_LEVEL > 1)
					{
						// Add free or predefined products/services
						$object->formAddObjectLine(1,$mysoc,$soc,$hookmanager);
					...
					else
					{
						...
						{
							$var=!$var;
							$objfabcom->formAddPredefinedProductSon(0,$mysoc,$soc,$hookmanager);
						}
					}
				...
				if ($resql)
				{
					...
					while ($i < $num)
					{
						...
						print '<td align="center">'.$unit.'</td>';
						print '<td align="right">'.$objp->qty_son.'</td>';
						//		  print '<td align="center">'.$objp->statut.'</td>';
						if ($user->rights->fabrication->mat->del)
							print '<td align="right"><a href="'.DOL_URL_ROOT.'/fabrication/productlist/fiche.php?action=delete&id='.$objp->fk_product_father.'&idr='.$objp->rowid.'">'.img_picto($langs->trans("Clearmaterial"),'delete').'</a></td>';
						else
							print '<td>&nbsp;</td>';
						print "</tr>";
						$i++;
