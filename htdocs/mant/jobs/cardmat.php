...
if ($conf->almacen->enabled)
{
	$objSol = new Solalmacenext($db);
	$objSoldet = new Solalmacendetext($db);
}
...
$hookmanager->initHooks(array('mjobsresource'));
...
$parameters=array();
$reshook=$hookmanager->executeHooks('doActions',$parameters,$objJobsresource,$action);    // Note that $action and $objJobsresource may have been modified by some hooks
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/mant/jobs/cardmat.php?id='.$id,1);
			header("Location: ".$urltogo);
			exit;
		}
		if ($id > 0 || ! empty($ref)) $ret = $objJobsresource->fetch($id,$ref);
		$action='';
	}
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/mant/list.php',1);
			header("Location: ".$urltogo);
			exit;
		}
		...
		if ($fk_sol_almacendet>0)
		{
			$objSolalmacendet->fetch($fk_sol_almacendet);
			$fk_product = $objSolalmacendet->fk_product+0;
		}
		$objJobsresource->fk_jobs=GETPOST('id','int');
		$objJobsresource->ref=GETPOST('ref','alpha');
		$objJobsresource->fk_sol_almacen=GETPOST('fk_sol_almacen','int');
		$objJobsresource->fk_sol_almacendet=GETPOST('fk_sol_almacendet','int');
		$objJobsresource->fk_jobs_program=GETPOST('fk_jobs_program','int');
		$objJobsresource->fk_product=$fk_product+0;
		$objJobsresource->dater = $dater;
		$objJobsresource->type_cost=GETPOST('type_cost','alpha');
		$objJobsresource->description=GETPOST('description','alpha');
		$objJobsresource->quant=GETPOST('quant','alpha');
		$objJobsresource->fk_unit=GETPOST('fk_unit','int');
		$objJobsresource->price=GETPOST('price','alpha');
		$objJobsresource->fk_user_create=$user->id;
		$objJobsresource->fk_user_mod=$user->id;
		$objJobsresource->datec = $now;
		$objJobsresource->datem = $now;
		$objJobsresource->status=1;

		if (empty($objJobsresource->ref))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Ref")), null, 'errors');
		}
		if ($objJobsresource->quant<=0)
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Quant")), null, 'errors');
		}

		if (! $error)
		{
			$result=$objJobsresource->create($user);
			if ($result > 0)
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/mant/jobs/cardmat.php?id='.$id,1);
			...
			{
				// Creation KO
				if (! empty($objJobsresource->errors)) setEventMessages(null, $objJobsresource->errors, 'errors');
				else  setEventMessages($objJobsresource->error, null, 'errors');
				$action=$actionant;
			}
		}
		else
	...
	if ($action == 'update')
	{
		...
		if (! $error)
		{
			$result=$objJobsresource->update($user);
			if ($result > 0)
			{
				$action='view';
			}
	...
	{
		$result=$objJobsresource->delete($user);
		if ($result > 0)
		{
			// Delete OK
			setEventMessages("RecordDeleted", null, 'mesgs');
			header("Location: ".dol_buildpath('/mant/jobs/cardmat.php?id='.$id,1));
			exit;
		}
		else
		{
...
if ($action == 'addadvance' && $user->rights->mant->jobs->upjobs )
{
	$actionant = GETPOST('actionant');
	...
	$objectadvance->fk_jobs_program = 0;
	...
	$objectadvance->used_time = GETPOST('used_time');
...
if ($id||$ref)
{
	...
	dol_fiche_head($head, 'mat', $langs->trans("Jobs"), 0, 'mant');
	...
	if ($action == '')
	{
		...
		{
			//print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=advance&amp;id='.$object->id.'">'.$langs->trans('Reportadvance').'</a>';
		}
	...
	if ($object->status == 3 || $object->status == 4 || $object->status == 5 || $object->status == 6)
	{
		if ($action == 'advance')
		{
			...
			if ($object->status ==4)
				include_once DOL_DOCUMENT_ROOT.'/mant/jobs/tpl/work_advance.tpl.php';
		}
		include_once DOL_DOCUMENT_ROOT.'/mant/jobs/tpl/work_material_list.tpl.php';
	}
