...
/**
 *   	\file       poa/partidaproduct_list.php
 *		\ingroup    poa
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-31 12:01
 */
...
/* codLaiwett Aqui incluimos mi codigo para que puedda heredar */
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'partidaproductlist';
...
$arrayfields=array(
	...
	'p.code_partida'=>array('label'=>$langs->trans("Fieldcodepartida"), 'checked'=>1),
	'p.label'=>array('label'=>$langs->trans("Fieldlabel"), 'checked'=>1),
...
$objectdet=new Partidaproductext($db);
if (($idr > 0 || ! empty($ref)) && $action != 'add')
{
	$result=$objectdet->fetch($idr,$ref);
...
$sql.= " FROM ".MAIN_DB_PREFIX."partida_product as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."product AS p ON t.fk_product = p.rowid ";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."partida_product_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE t.code_partida = '".$object->code."'";
...
if ($action == 'createprod')
	print '<input type="hidden" name="action" value="addprod">';
...
Nota al parecer se debe llenatr todos los datos que te pide la funcion fromconfirm */
if ($action == 'deleteprod') {

	$idr = GETPOST('idr');
		  //echo 'aqui lo salvado '.$idrr;

	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id.'&idr='.$idr,
...
// Fields title
// Aqui se muestra la Cabezera de la tabla que se lista
print '<tr class="liste_titre">';
//
if (! empty($arrayfields['t.code_partida']['checked'])) print_liste_field_titre($arrayfields['t.code_partida']['label'],$_SERVER['PHP_SELF'],'t.code_partida','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['p.ref']['checked'])) print_liste_field_titre($arrayfields['p.ref']['label'],$_SERVER['PHP_SELF'],'p.ref','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['p.label']['checked'])) print_liste_field_titre($arrayfields['p.label']['label'],$_SERVER['PHP_SELF'],'p.label','',$params,'',$sortfield,$sortorder);
...
/* Pruebas Laiwett para aumentar */
if (! empty($arrayfields['t.active']['checked'])) print_liste_field_titre($arrayfields['']['label'],$_SERVER['PHP_SELF'],'t.active','',$params,'',$sortfield,$sortorder);
...
if ($action != 'createprod' && $action != 'editprod')
{
...
//
	...
	if (! empty($arrayfields['p.ref']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ref" value="'.$search_ref.'" size="10"></td>';
	if (! empty($arrayfields['p.label']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_label" value="'.$search_label.'" size="10"></td>';
	...
	/* Hoy codLaiwett antes de agregar el nuevos productos*/
...
if ($action == 'createprod')
{
	...
	{
		print '<td colspan="2">';
		//$form->select_produits(GETPOST('fk_product'),'fk_product', '', 0,0,1,2,'',1,array(),0,'1',0,'',0,'');
		//$form->select_produits_v('', 'idprod', '', 0,0,1,2,'',0,array(),0,'','product',0);
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if (! empty($arrayfields['t.fk_product']['checked']))
		{
			/* Estudiar esto manejo de objetos */
			$product->fetch($obj->fk_product);
			//print '<td>'.$obj->fk_product.'</td>';
		...
		{
			// codLaiwett para los productos
			$img = 'switch_off';
			if ($obj->active) $img = 'switch_on';
			$img = 'switch_off';
			if ($obj->active) $img = 'switch_on';
				//
			print '<td><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;idr='.$obj->fk_product.'&amp;action=checkingPro">'.img_picto('',$img).'</a></td>';
			if (! $i) $totalarray['nbfield']++;
		}
		...
		/* Hoy codLaiwett */
		/* Aqui modificare para ver si ocupa el campo delete*/
		//if ($user->rights->teacher->del)
		//{
		print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;idr='.$obj->fk_product.'&amp;action=deleteprod">'.img_picto('','delete').'</a>';
		//}
		print '</td>';
		if (! $i) $totalarray['nbfield']++;
...
if (empty($reshook))
{
	...
	{
	print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&action=createprod">'.$langs->trans("Newproduct").'</a></div>'."\n";
	}
}
print '</div>'."\n";
