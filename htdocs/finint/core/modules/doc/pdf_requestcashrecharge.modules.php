 ...
 */
class pdf_requestcashrecharge extends ModelePDFFinint
{
	var $db;
	var $name;
	var $um;
  ...
  var $emetteur;	// Objet societe qui emet
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Requestcashrecharge";
		$this->description = $langs->trans('PDFRequestCashApprovedDescription');
	...
	// Define position of columns
		$this->posxlab=$this->marge_gauche+1;
		$this->posxval=100;
	...
	*/
	...
	// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		...
		if ($conf->finint->dir_output)
		{
			...
			$objRequest = new Requestcashext($this->db);
			$objRequest->fetch($object->fk_request_cash_dest);
			$object->ref_recharge = $object->ref;
			$object->ref = $objRequest->ref;
			...
			else
			{
				...
				$file = $dir . "/" . $objectref.'recharge'.'_'.$object->ref_recharge . ".pdf";
			}
			if (! file_exists($dir))
			{
				if (dol_mkdir($dir) < 0)
		...
		//150
				...
				//vamos a mostrar los datos de la recarga
				...
				$showpricebeforepagebreak=1;
					...
					//label_recharge
				$pdf->SetXY($this->posxlab, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $langs->transnoentities('Ref'), 0, 'L',0); 

					//ref_recharge
				$pdf->SetXY($this->posxval, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $object->ref_recharge, 0, 'L',0); 

				$curY+=5;
				$nexY+=5;
					//label_recharge
				$pdf->SetXY($this->posxlab, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $langs->transnoentities('Date'), 0, 'L',0); 

					//ref_recharge
				$pdf->SetXY($this->posxval, $curY);
				$pdf->MultiCell($this->posxval-1, 3, dol_print_date($object->dateo,'day'), 0, 'L',0); 

				$curY+=5;
				$nexY+=5;
					//label_recharge
				$pdf->SetXY($this->posxlab, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $langs->transnoentities('Document'), 0, 'L',0); 

					//ref_recharge
				$pdf->SetXY($this->posxval, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $object->nro_chq, 0, 'L',0); 

				$curY+=5;
				$nexY+=5;
					//label_recharge
				$pdf->SetXY($this->posxlab, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $langs->transnoentities('Amount'), 0, 'L',0); 

					//ref_recharge
				$pdf->SetXY($this->posxval, $curY);
				$pdf->MultiCell($this->posxval-1, 3, price(price2num($object->amount,'MT')), 0, 'L',0); 

				$curY+=5;
				$nexY+=5;
					//label_recharge
				$pdf->SetXY($this->posxlab, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $langs->transnoentities('Detail'), 0, 'L',0); 

					//ref_recharge
				$pdf->SetXY($this->posxval, $curY);
				$pdf->MultiCell($this->posxval-1, 3, $object->detail, 0, 'L',0); 
...
//exit;
   ...
   *  @return	void
	...
	//    echo $posx.' '.$posy;exit;
  		...
  		$pdf->SetTextColor(0,0,60);
  		$title=$outputlangs->transnoentities("RECHARGERESOURCES");
