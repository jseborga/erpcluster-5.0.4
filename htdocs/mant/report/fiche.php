...
if ($action == 'excel')
{
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('A1',html_entity_decode($langs->trans('Workorders')));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('A5',html_entity_decode($langs->trans('Status')));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('B'.$line,html_entity_decode($langs->trans("Fielddate_create")));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('K'.$line,html_entity_decode($langs->trans("Technicians")));
	$objPHPExcel->getActiveSheet()->SetCellValue('L'.$line,html_entity_decode($langs->trans("Typerepair")));



	$objPHPExcel->getActiveSheet()->getStyle('A'.$line.':L'.$line)->applyFromArray(
		array(
			'font'    => array(
				'bold'      => true
			),
	...
	foreach ( (array)$aReporte[1] as $i => $row)
	{
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$line,$row['l']);
	...
	$line--;
	$objPHPExcel->getActiveSheet()->getStyle('A8:L'.$line)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
else
{
	if ($date_ini && $date_fin)
	{
		...
		// print_liste_field_titre($langs->trans("Speciality"),'','','','','align="left"');
			 ...
			 //internos
				...
				//verificamos el avance
					...
					if ($resprog>0)
					{
						...
						{
							if (!empty($description_job)) $description_job.= '; ';
				...
				//$aLineas[$i]['o']=select_typemant($obj->typemant,'','',0,1);
						$aLineas[$i]['l']=$objTyperepair->label;
					}
					print '</td>';
				//print '<td align="left">';
			...
			$aReporte = array(1=>$aLineas,2=>$dateini,3=>$datefin,4=>$alevel[$level]);

			$_SESSION['aReporte'] = serialize($aReporte);
			print "<div class=\"tabsAction\">\n";
			print '<a class="butAction" href="'.DOL_URL_ROOT.'/mant/report/fiche.php?action=excel">'.$langs->trans('Spreadsheet').'</a>';
			print '</div>';
		//}
	}
}
