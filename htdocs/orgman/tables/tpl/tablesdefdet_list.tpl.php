...
/**
 *   	\file       orgman/tablesdefdet_list.php
 *		\ingroup    orgman
...
$aActive = array(0=>$langs->trans('Not'),1=> $langs->trans('yes'));
$aStatus = array(9=>$langs->trans('All'),0=>$langs->trans('Unactivated'),1=> $langs->trans('Activated'));

// Initialize technical object to manage context to save list fields
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmantablesdefdetlist';
...
$arrayfields=array(
	...
	't.fk_table_def'=>array('label'=>$langs->trans("Fieldfk_table_def"), 'align'=>'align="left"', 'checked'=>0),
	...
	't.range_ini'=>array('label'=>$langs->trans("Fieldrange_ini"), 'align'=>'align="left"', 'checked'=>($object->with_limit?1:0)),
	't.range_fin'=>array('label'=>$langs->trans("Fieldrange_fin"), 'align'=>'align="left"', 'checked'=>($object->with_limit?1:0)),
...
$help_url='';
$title = $langs->trans('Tablesdefdet');
...
$sql.= " FROM ".MAIN_DB_PREFIX."tables_def_det as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."tables_def_det_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
$sql.= " AND t.fk_table_def = ".$id;
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_table_def) $sql.= natural_search("fk_table_def",$search_fk_table_def);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/tablesdefdet/card.php?id='.$id);
	exit;
}
...
//
if (! empty($arrayfields['t.fk_table_def']['checked'])) print_liste_field_titre($arrayfields['t.fk_table_def']['label'],$_SERVER['PHP_SELF'],'t.fk_table_def','',$params,$arrayfields['t.fk_table_def']['align'],$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
//
if (! empty($arrayfields['t.fk_table_def']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_table_def']['align'].'><input type="text" class="flat" name="search_fk_table_def" value="'.$search_fk_table_def.'" size="10"></td>';
...
if (! empty($arrayfields['t.active']['checked']))
{
	...
	print $form->selectyesno('search_active',$search_active,1);
	print '</td>';
}
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'ref')
				{
					$objectdet->fk_table_def = $obj->fk_table_def;
					$objectdet->id = $obj->rowid;
					$objectdet->ref = $obj->ref;
					$objectdet->label = $obj->label;
					$obj->$key2 = $objectdet->getNomUrltpl();
				}
				if ($key2 == 'active')
				{
					$img = 'switch_off';
				...
				if ($key2 == 'status')
				{
					$objectdet->status = $obj->$key2;
					$obj->$key2 = $objectdet->getLibStatut(3);
				}
...
if ($object->status == 0)
	print '&nbsp;<a class="butAction" href="' . dol_buildpath('/orgman/tables/carddet.php',1).'?id='.$id.'&action=create">' . $langs->trans('New') . '</a>';
print '</div>';