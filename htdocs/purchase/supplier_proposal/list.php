...
{
	$objectid=$socid;
	$module='societe,purchase,supplier_proposal';
	$dbtable='&societe';
}
//$result = restrictedArea($user, $module, $objectid, $dbtable);
//$result = restrictedArea($user, 'supplier_proposal', $id);
if (!$user->rights->purchase->sup->lire) accessforbidden();
...
if ($result)
{
	$objectstatic=new SupplierProposalext($db);
	...
	while ($i < min($num,$limit))
	{
		...
		print $objectstatic->getNomUrladd(1);
		print '</td>';
