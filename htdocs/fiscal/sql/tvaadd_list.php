...
/**
 *   	\file       fiscal/tvaadd_list.php
 *		\ingroup    fiscal
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-05-11 19:18
 */
...
dol_include_once('/fiscal/class/tvaadd.class.php');
...
$search_fk_type_tva=GETPOST('search_fk_type_tva','int');
...
't.fk_type_tva'=>array('label'=>$langs->trans("Fieldfk_type_tva"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->tvaadd->read;
    $permtodelete = $user->rights->tvaadd->delete;
    $uploaddir = $conf->tvaadd->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."tva_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."tva_add_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_tva) $sql.= natural_search("fk_tva",$search_fk_tva);
if ($search_fk_type_tva) $sql.= natural_search("fk_type_tva",$search_fk_type_tva);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/tvaadd/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.fk_type_tva']['checked'])) print_liste_field_titre($arrayfields['t.fk_type_tva']['label'],$_SERVER['PHP_SELF'],'t.fk_type_tva','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_type_tva']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_type_tva" value="'.$search_fk_type_tva.'" size="10"></td>';
