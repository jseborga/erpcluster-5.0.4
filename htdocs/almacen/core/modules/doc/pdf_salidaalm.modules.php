 ...
 */
class pdf_salidaalm extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
	...
	*/
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->almacen->dir_output)
		{
			...
			$aData = $object->fetchlines(" fk_entrepot = ".$object->fk_entrepot_to);
			...
			if (file_exists($dir))
			{
				...
				//for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$unit = $productunit->getLabelOfUnit('short');
	...
	*/
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxuni-$this->posxdesc-1,2, $outputlangs->transnoentities("Desc"),'','L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxsen-$this->posxuni-1,2, $outputlangs->transnoentities("Unit"),'','R');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxbal-$this->posxsen-1,2, $outputlangs->transnoentities("Output"),'','R');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxbal,2, $outputlangs->transnoentities("Confirms"),'','R');
		}
	}
	...
	*/
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		if ($object->fk_entrepot_to)
		{
			...
			$pdf->SetXY($posxx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("A")." : " . $entrepot->libelle, '', 'R');
		}
		//label
		$posy+=4;
		$pdf->SetXY($posxx,$posy);
