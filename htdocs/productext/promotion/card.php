...
require_once DOL_DOCUMENT_ROOT.'/productext/class/promotionsext.class.php';
require_once DOL_DOCUMENT_ROOT.'/productext/class/promotionsdet.class.php';
require_once DOL_DOCUMENT_ROOT.'/productext/class/ctypepromotion.class.php';
...
$objPromotion = new Promotionsext($db);
$objPromotiondet = new Promotionsdet($db);
$objCtypepromotion = new Ctypepromotion($db);
$objUser = new User($db);
...
if ($idr > 0)
{
	$objPromotion->fetch($idr);
}
if ($idrd > 0)
{
	$objPromotiondet->fetch($idrd);
}
...
if (empty($reshook))
{
	...
	if ($idr) $ret = $objPromotion->fetch($idr);
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/promotion/promotion/card.php?id='.$id,1);
		...
		$objPromotion->entity=$object->entity;
		$objPromotion->ref=GETPOST('ref','alpha');
		$objPromotion->label=GETPOST('label','alpha');
		$objPromotion->detail=GETPOST('detail','alpha');
		$objPromotion->fk_product=$id;
		$objPromotion->fk_type_promotion=GETPOST('fk_type_promotion','int');
		$objPromotion->qty=GETPOST('qty','int');
		$objPromotion->active=GETPOST('active','int');
		$objPromotion->fk_user_create=$user->id;
		$objPromotion->fk_user_mod=$user->id;
		$objPromotion->datec = $now;
		$objPromotion->datem = $now;
		$objPromotion->tms = $now;
		$objPromotion->status=0;



		if (empty($objPromotion->ref))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Ref")), null, 'errors');
		}
		if (empty($objPromotion->label))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Label")), null, 'errors');
		}
		if ($objPromotion->fk_type_promotion<=0)
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Fieldfk_type_promotion")), null, 'errors');
		}
		if (empty($objPromotion->qty) || $objPromotion->qty <=0)
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Fieldqty")), null, 'errors');
		}

		if (! $error)
		{
			$result=$objPromotion->create($user);
			if ($result > 0)
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/promotion/promotion/card.php?id='.$id.'&idr'.$result,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
				// Creation KO
				if (! empty($objPromotion->errors)) setEventMessages(null, $objPromotion->errors, 'errors');
				else  setEventMessages($objPromotion->error, null, 'errors');
				$action='create';
			}
		}
		else
	...
	if ($action == 'update')
	{
		...
		if (! $error)
		{
			$result=$objPromotion->update($user);
			if ($result > 0)
			{
				$action='view';
			}
	...
	{
		$objPromotion->status = 1;
		$objPromotion->fk_user_mod = $user->id;
		$objPromotion->fk_user_val = $user->id;
		$objPromotion->datem = $now;
		$objPromotion->datev = $now;
		$result=$objPromotion->update($user);
		if ($result > 0)
		{
			// Delete OK
			setEventMessages("RecordValidate", null, 'mesgs');
			header("Location: ".$_SERVER['PHP_SELF'].'?id='.$id,1);
			exit;
		}
		else
		{
	...
	if ($action == 'confirm_novalidate')
	{
		$objPromotion->status = 0;
		$objPromotion->fk_user_mod = $user->id;
		$objPromotion->fk_user_val = 0;
		$objPromotion->datem = $now;
		$objPromotion->datev = 'NULL';
		$result=$objPromotion->update($user);
		if ($result > 0)
		{
			// Delete OK
	...
	{
		$result=$objPromotion->delete($user);
		if ($result > 0)
		{
			// Delete OK
			setEventMessages("RecordDeleted", null, 'mesgs');
			header("Location: ".dol_buildpath('/promotion/promotion/card.php?id='.$id,1));
			exit;
		}
		else
		{
...
else
{
	...
	if ($object->id > 0)
	{
		...
		dol_fiche_head($head, 'promotion', $titre, 0, $picto);
...
if ((empty($idr) || $idr == 0) && (empty($action) || $action == 'view' || $action == 'list'))
{
	require_once DOL_DOCUMENT_ROOT.'/productext/promotion/tpl/list.tpl.php';
}
else
{
	require_once DOL_DOCUMENT_ROOT.'/productext/promotion/tpl/card.tpl.php';
}
