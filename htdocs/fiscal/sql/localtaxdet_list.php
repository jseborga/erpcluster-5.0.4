...
/**
 *   	\file       fiscal/localtaxdet_list.php
 *		\ingroup    fiscal
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-05-11 15:00
 */
...
dol_include_once('/fiscal/class/localtaxdet.class.php');
...
$search_fk_localtax=GETPOST('search_fk_localtax','int');
$search_fk_typetvadet=GETPOST('search_fk_typetvadet','int');
...
't.fk_localtax'=>array('label'=>$langs->trans("Fieldfk_localtax"), 'checked'=>1),
't.fk_typetvadet'=>array('label'=>$langs->trans("Fieldfk_typetvadet"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->localtaxdet->read;
    $permtodelete = $user->rights->localtaxdet->delete;
    $uploaddir = $conf->localtaxdet->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."localtaxdet as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."localtaxdet_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_localtax) $sql.= natural_search("fk_localtax",$search_fk_localtax);
if ($search_fk_typetvadet) $sql.= natural_search("fk_typetvadet",$search_fk_typetvadet);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/localtaxdet/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_localtax']['checked'])) print_liste_field_titre($arrayfields['t.fk_localtax']['label'],$_SERVER['PHP_SELF'],'t.fk_localtax','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_typetvadet']['checked'])) print_liste_field_titre($arrayfields['t.fk_typetvadet']['label'],$_SERVER['PHP_SELF'],'t.fk_typetvadet','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_localtax']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_localtax" value="'.$search_fk_localtax.'" size="10"></td>';
if (! empty($arrayfields['t.fk_typetvadet']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_typetvadet" value="'.$search_fk_typetvadet.'" size="10"></td>';
