 ...
 */
class mod_finint_ubuntuborendcash extends ModeleNumRefFinInt
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefixreq='REN';
	var $prefixnote='AV';
	var $error='';
	var $nom='ubuntuborendcash';
	...
	function info()
	{
		global $langs;
		$langs->load("finint");
		return $langs->trans('UbuntuborendcashNumRefModelDesc',$this->prefixreq,$this->prefixnote);
	}
	...
	function getExample()
	{
		return $this->prefixreq."1601-0001";
	}
	...
	function canBeActivated()
	{
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."request_cash_deplacement";
	...
	function getNextValue($objsoc,$finint,$mode='next')
	{
		...
		$sql = "SELECT MAX(SUBSTRING(ref FROM ".$posindice.")) as max";	// This is standard SQL
		$sql.= " FROM ".MAIN_DB_PREFIX."request_cash_deplacement";
		$sql.= " WHERE ref LIKE '".$prefix.$yymm."-%'";
		$sql.= " AND entity = ".$conf->entity;

		$resql=$db->query($sql);
		dol_syslog("mod_finint_ubuntuborendcash::getNextValue sql=".$sql);
		if ($resql)
		{
		...
		{
			dol_syslog("mod_finint_ubuntuborendcash::getNextValue sql=".$sql, LOG_ERR);
			exit;
			return -1;
		}
		...
		else if ($mode == 'next')
		{
			...
			dol_syslog("mod_finint_ubuntuborendcash::getNextValue return ".$prefix.$yymm."-".$num);
