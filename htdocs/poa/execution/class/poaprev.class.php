 ...
 *				Initialy built by build_class_from_table on 2015-06-01 12:11
 */
...
class Poaprev extends CommonObject
{
	...
	var $element='poa_prev';			//!< Id that identify managed objects
	...
	elseif ($nro)
	  {
	    $sql.= " WHERE t.nro_preventive = ".$nro;
     ...
     *  @param  string  $gestion        Filter gestion
     *  @param  strimg  $fk_area        FIlter fk_area
     *  @param  string  $statut         0=Pendiente; 1=Aprobado 2=Comprometido; 3=Devengado; T= mayores a 1
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
    function getlist($gestion,$fk_user='',$fk_area=0,$search='')
    {
      global $langs;
        $sql = "SELECT";
	$sql.= " t.rowid,";
	...
	{
	  $sql.= " AND ( ";
	  $sql.= " t.nro_preventive LIKE '%".$search."%' ";
	  $sql.= " OR t.label LIKE '%".$search."%' ";
	  $sql.= " OR t.pseudonym LIKE '%".$search."%' ";
	  $sql.= ")";
	}
      $sql.= " AND t.statut > 0";
   ...
   /**
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."poa_prev as t";
	$sql.= " INNER JOIN ".MAIN_DB_PREFIX."poa_prev_seg AS s ON t.rowid = s.fk_prev";
	$sql.= " WHERE s.fk_prev_ant = ".$id;
	$sql.= " AND t.statut > 0";
	dol_syslog(get_class($this)."::getlistant sql=".$sql, LOG_DEBUG);
        $resql=$this->db->query($sql);
	$this->arraya = array();
        if ($resql)
	  {
        ...
        {
      	    $this->error="Error ".$this->db->lasterror();
            dol_syslog(get_class($this)."::getlistant ".$this->error, LOG_ERR);
            return -1;
        }
    }
