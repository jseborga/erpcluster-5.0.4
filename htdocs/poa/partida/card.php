...
dol_include_once('/poa/class/cpartida.class.php');
dol_include_once('/poa/class/partidaproduct.class.php');
dol_include_once('/poa/class/productext.class.php');
...
if (empty($reshook))
{
	...
	include DOL_DOCUMENT_ROOT.'/poa/partida/inc/abm_partidaproduct.inc.php';
	...
	if ($action == 'add')
	{
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/partida/card.php?id'.$result,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/poa/partida/listgas.php',1));
			exit;
		}
		else
		{
...
foreach ($objecttmp->lines AS $j => $line)
{
	...
	if ($line->fk_father <=0) $indent = '';
	else
	{
		if ($idFather != $line->fk_father)
		{
			$indent.= '&nbsp;&nbsp;';
			$idFather = $line->fk_father;
		}
	}
	$selected = '';
	if ($action == 'edit' && $line->id == $object->fk_father) $selected = ' selected ';
...
if (($id || $ref) && $action == 'edit')
{
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldfk_father").'</td><td>';
	print '<select name="fk_father">'.$options.'</select>';
	print '</td></tr>';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	{
		$objecttmp->fetch($object->fk_father);
		print '<tr><td>'.$langs->trans("Fieldfk_father").'</td><td>'.$objecttmp->getNomUrl(1).'</td></tr>';
	}
