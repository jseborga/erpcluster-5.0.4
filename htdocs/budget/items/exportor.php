...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		...
		if ($res > 0)
		{
			...
			foreach ($lines AS $j => $line)
			{
				$res = $objItemsproduct->procedure_calc($line->id,$type_structure, $fk_region,$fk_sector,true);
				//if ($res>0)
				//{
			...
			$objPHPExcel->getActiveSheet()->setTitle("Análisis de precio unitario");
			...
			foreach ($aReport AS $j => $aDatatask)
			{
				foreach ($aDatatask AS $fk_task => $lines)
				{
					...
					//$objPHPExcel->getActiveSheet()->getStyle('A'.$lin)->getFont()->setName('Arial');
					//$objPHPExcel->getActiveSheet()->getStyle('A'.$lin)->getFont()->setSize(12);
					//$objPHPExcel->getActiveSheet()->getStyle('A'.$lin)->getFont()->setBold(true);
					$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':G'.$lin)->applyFromArray(array('font' => array( 'bold'  => true,'color' => array('rgb' => 'ff0000'),'size'  => 13,'name'  => 'Arial')));

					$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,html_entity_decode($cTitle));
					...
					// ProjectO
					$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,html_entity_decode($langs->trans("Project").':'));
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$lin,html_entity_decode($cTitleproject.' '.$cTitleregion));
					$objPHPExcel->getActiveSheet()->mergeCells('B'.$lin.':G'.$lin);
					$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':C'.$lin)->applyFromArray($styleTitle);
					$lin++;
					//actovotu
					$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,html_entity_decode($langs->trans("Activity").':'));
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$lin,html_entity_decode($object->ref.' '.$object->detail));
					$objPHPExcel->getActiveSheet()->mergeCells('B'.$lin.':G'.$lin);
					$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':C'.$lin)->applyFromArray($styleTitle);
					$lin++;
					$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,html_entity_decode($langs->trans("Unit")));
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$lin,$unit);
					$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':C'.$lin)->applyFromArray($styleTitle);
					$lin++;
					$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,html_entity_decode($langs->trans("Quantity")));
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$lin,($objectdet->quant>0?$objectdet->quant:1));
					$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':C'.$lin)->applyFromArray($styleTitle);
					$lin++;
					...
					$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,html_entity_decode($langs->trans("Currencybase")));
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$lin,$moneda);
					$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':C'.$lin)->applyFromArray($styleTitle);
					$lin++;
					$linhead= $lin;
					//armamos titulos
					$objPHPExcel->getActiveSheet()->setCellValue('E'.$lin,$langs->trans("Priceunit"));
					$objPHPExcel->getActiveSheet()->mergeCells('E'.$lin.':F'.$lin);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('D'.$lin,'% Prod.');
					$objPHPExcel->getActiveSheet()->setCellValue('E'.$lin,$langs->trans("Improductive"));
					$objPHPExcel->getActiveSheet()->setCellValue('F'.$lin,$langs->trans("Productive"));
					$objPHPExcel->getActiveSheet()->setCellValue('G'.$lin,$langs->trans("Pricetotal"));

					$linant=$lin-1;
					$objPHPExcel->getActiveSheet()->getStyle('A'.$linant.':G'.$lin)->applyFromArray($styleTitleline);
					$lin++;
						...
						foreach ($aData AS $nom => $row)
						{
							foreach ($row AS $nReg => $value)
							{
								if ($nom == 'datag')
								{
									...
									$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':G'.$lin)->applyFromArray($styleDatag);
								}
								if ($nom == 'data')
								{
									if ($lFormula)
									{
										...
										if ($nReg==7)
										{
											...
											$objPHPExcel->getActiveSheet()->setCellValue($aColumn[$nReg].$lin,'='.$cel);
											if (strlen($cel)>0)
											$objPHPExcel->getActiveSheet()->getCell($aColumn[$nReg].$lin)->getCalculatedValue(true);
										}
										else
								...
								if ($nom == 'partial')
								{
									if ($lFormula)
									{
										if ($nReg == 7)
										{
											...
											$objPHPExcel->getActiveSheet()->setCellValue($aColumn[$nReg].$lin,'='.$cel);
											if (strlen($cel)>0)
											$objPHPExcel->getActiveSheet()->getCell($aColumn[$nReg].$lin)->getOldCalculatedValue(true);
										}
										else
									...
									$objPHPExcel->getActiveSheet()->mergeCells('A'.$lin.':E'.$lin);
									$objPHPExcel->getActiveSheet()->getStyle('A'.$lin)->applyFromArray($estilo);
								}
								if ($nom == 'total')
								{
									if ($lFormula)
									...
									$objPHPExcel->getActiveSheet()->mergeCells('A'.$lin.':E'.$lin);
									$objPHPExcel->getActiveSheet()->getStyle('A'.$lin)->applyFromArray($estilo);
									$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':G'.$lin)->applyFromArray($styleTotal);
								}
								if ($nom == 'totalpu')
								{
									if ($lFormula)
									...
									$objPHPExcel->getActiveSheet()->mergeCells('A'.$lin.':F'.$lin);
									$objPHPExcel->getActiveSheet()->getStyle('A'.$lin.':G'.$lin)->applyFromArray($styleTotalpu);
					...
					$linbutton = $lin-1;
					$objPHPExcel->getActiveSheet()->getStyle('A'.$linhead.':G'.$linbutton)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					//$lin++;
					//vamos a mostrar el valor del precio unitario
					//$cNumtext = $langs->trans('Son').': '.num2texto(price2num($valuepu,'MT'),'');
					//$cNumtext.= ' '.$moneda;
					///$objPHPExcel->getActiveSheet()->setCellValue('A'.$lin,$cNumtext);
					//$objPHPExcel->getActiveSheet()->mergeCells('A'.$lin.':G'.$lin);
					$lin++;
				}
				$lin++;
