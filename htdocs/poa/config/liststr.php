...
/**
 *   	\file       poa/cnamestructure_list.php
 *		\ingroup    poa
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-28 15:02
 */
...
if (!$user->rights->poa->teacher->lire) accessforbidden();
...
$hookmanager->initHooks(array('cnamestructure'));
...
$arrayfields=array(
	't.code'=>array('label'=>$langs->trans("Fieldorder"), 'checked'=>1),
...
$object=new Cnamestructure($db);
if (($id > 0 || ! empty($code)) && $action != 'add')
{
	$result=$object->fetch($id,$code);
...
if (empty($reshook))
{
	if ($cancel) 
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/config/liststr.php',1);
			header("Location: ".$urltogo);
			exit;
		}		
		if ($id > 0 || ! empty($code)) $ret = $object->fetch($id,$code);
		$action='';
	}
...
$help_url='';
$title = $langs->trans('Namestructure');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_name_structure as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_name_structure_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/cnamestructure/card.php?id='.$id);
	exit;
}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if ($action == 'edit' && $id == $obj->rowid)
		{
			...
			{
				print '<td>'.'<input type="number" min="1" class="flat" name="code" value="'.$obj->code.'">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked'])) 
			{
				print '<td>'.'<input type="text" class="flat" name="label" value="'.$obj->label.'">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
