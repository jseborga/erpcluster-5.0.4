...
require_once DOL_DOCUMENT_ROOT.'/productext/class/promotionsdetext.class.php';
...
$objPromotiondet = new Promotionsdetext($db);
...
$now = dol_now();
$parameters=array();
$reshook=$hookmanager->executeHooks('doActions',$parameters,$objPromotiondet,$action);    // Note that $action and $objPromotiondet may have been modified by some hooks
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/promotion/list.php',1);
			header("Location: ".$urltogo);
			exit;
		}
		if ($id > 0 || ! empty($ref)) $ret = $objPromotiondet->fetch($id,$ref);
		$action='';
	}
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/promotion/promotion/carddet.php?id='.$id.'&idr='.$idr,1);
			header("Location: ".$urltogo);
			exit;
		}
		...
		$objPromotiondet->fk_promotions=$idr;
		$objPromotiondet->fk_product=GETPOST('fk_product','int');
		$objPromotiondet->saleprice=GETPOST('saleprice','int');
		if (empty($objPromotiondet->saleprice)) $objPromotiondet->saleprice=0;
		$objPromotiondet->qty=GETPOST('qty','int');
		$objPromotiondet->price=GETPOST('price','int');
		if (empty($objPromotiondet->price)) $objPromotiondet->price=0;
		$objPromotiondet->subprice=GETPOST('subprice','int');
		if(empty($objPromotiondet->subprice)) $objPromotiondet->subprice=0;
		$objPromotiondet->fk_user_create=$user->id;
		$objPromotiondet->fk_user_mod=$user->id;
		$objPromotiondet->datec = $now;
		$objPromotiondet->datem = $now;
		$objPromotiondet->tms = $now;
		$objPromotiondet->status=1;

		if (empty($objPromotiondet->qty))
		{
			$error++;
		...
		if (! $error)
		{
			$result=$objPromotiondet->create($user);
			if ($result > 0)
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/promotion/promotion/carddet.php?id='.$id.'&idr='.$idr,1);
			...
			{
				// Creation KO
				if (! empty($objPromotiondet->errors)) setEventMessages(null, $objPromotiondet->errors, 'errors');
				else  setEventMessages($objPromotiondet->error, null, 'errors');
				$action='create';
			}
		}
		else
	...
	//$objPromotiondet->fk_product=GETPOST('fk_product','int');
		...
		{
			$error++;
			setEventMessages($langs->transnoentitiesnoconv("ErrorFieldRequired",$langs->transnoentitiesnoconv("Fieldqty")), null, 'errors');
		}

		if (! $error)
		{
			$result=$objPromotiondet->update($user);
			if ($result > 0)
			{
				$action='view';
			}
	...
	if ($action == 'confirm_delete')
	{
		...
		$result=$objPromotiondet->delete($user);
		if ($result > 0)
		{
			// Delete OK
			setEventMessages("RecordDeleted", null, 'mesgs');
			header("Location: ".dol_buildpath('/promotion/promotion/carddet.php?id='.$id.'&idr='.$idr,1));
			exit;
		}
		else
		{
...
require_once DOL_DOCUMENT_ROOT.'/productext/promotion/tpl/carddet.tpl.php';
