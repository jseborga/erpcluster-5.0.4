...
/**
 *   	\file       orgman/pcharge_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-06-20 21:33
 */
...
// Must be include, not include_once  // Must be include, not

$hookmanager->initHooks(array('pcharge'));
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_charge_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/pcharge/card.php?id='.$id);
	exit;
}
...
$totalarray=array();
if ($action == 'delete') {
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteCharge'), $langs->trans('ConfirmDeleteCharge'), 'confirm_delete', '', 0, 1);
	print $formconfirm;
}
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<input type="number" min="0" step="any" name="amount" value="'.GETPOST('amount').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if ($action == 'edit' && $id == $obj->rowid)
		{
			...
			if (! empty($arrayfields['t.amount']['checked']))
			{
				print '<td>'.'<input type="number" min="0" step="any" name="amount" value="'.(GETPOST('amount')?GETPOST('amount'):$obj->amount).'">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
		...
		else
		{
			if (! empty($arrayfields['t.ref']['checked']))
			{
				print '<td>'.$object->getNomUrl(0,'',0,24,'',($user->rights->orgman->char->write?"&action=edit&page=".$page.'&sortfield='.$sortfield.'&sortorder='.$sortorder:"")).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked']))
			{
			...
			// Action column
			print '<td>';
			if ($user->rights->orgman->char->del)
			{
