...
/**
 *   	\file       almacen/stockprogram_list.php
 *		\ingroup    almacen
...
$search_fk_entrepot=GETPOST('search_fk_entrepot','alpha');
...
$arrayfields=array(
	...
	't.fk_user_val'=>array('label'=>$langs->trans("Fieldfk_user_val"), 'checked'=>1),
...
$help_url='';
$title = $langs->trans('Transferproduct');
...
$sql.= " FROM ".MAIN_DB_PREFIX."stock_program as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."entrepot AS e ON t.fk_entrepot = e.rowid ";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."user AS uc ON t.fk_user_create = uc.rowid ";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."user AS um ON t.fk_user_mod = um.rowid ";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."stock_program_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_ref) $sql.= natural_search("ref",$search_ref);
if ($search_fk_entrepot) $sql.= natural_search(array("fk_entrepot",'e.lieu'),$search_fk_entrepot);
if ($search_fk_user_create) $sql.= natural_search(array("fk_user_create",'uc.lastname','uc.firstname'),$search_fk_user_create);
if ($search_fk_user_mod) $sql.= natural_search(array("fk_user_mod",'um.lastname','um.firstname'),$search_fk_user_mod);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/stockprogram/card.php?id='.$id);
	exit;
}
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'fk_entrepot')
				{
					$objEntrepot->fetch($obj->$key2);
					$obj->$key2 = $objEntrepot->getNomUrl(1);
				}
				if ($key2 == 'active')
				{
					$img = 'switch_off';
				...
				if ($key2 == 'status')
				{
					...
					$obj->$key2 = $object->getLibStatutadd(6);
				}
				if ($key2 == 'fk_user_create' || $key2 == 'fk_user_mod' || $key2 == 'fk_user_val')
				{
					if ($obj->$key2>0)
					{
