...
//$result = restrictedArea($user, 'purchase&fournisseur', $orderid, '', 'commande');
$result = restrictedArea($user, 'purchase',$socid,'');
	...
	if ($resql)
	{
		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				print '<a href="'.DOL_URL_ROOT.'/purchase/commande/card.php?id='.$obj->rowid."\">".img_object($langs->trans("ShowOrder"),"order").' '.$obj->ref."</a></td>";
				print '<td>';
				print $societe->getNomUrl(1,'',16).'</td></tr>';
				$i++;
			}
		}
		print "</table><br>";
...
if ($resql)
{
	...
	if ($num)
	{
		...
		while ($i < $num)
		{
			...
			print '<a href="'.DOL_URL_ROOT.'/purchase/commande/card.php?id='.$obj->rowid.'">'.$commandestatic->ref.'</a>';;
