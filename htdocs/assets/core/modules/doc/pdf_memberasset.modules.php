...
class pdf_memberasset extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
				...
				foreach ( (array)$aReportdetasset as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-1);
						...
						if(empty($line['Codigo'])){
							...
							$pdf->MultiCell($this->posxeti-$this->posxcod-1, 1, $line['Etiqueta'], 0, 'L',0);
						}else{
							...
							$pdf->SetXY($this->posxcod, $curY);
							$pdf->MultiCell($this->posxeti-$this->posxcod-1, 1, dol_trunc($line['Etiqueta'],36), 0, 'L',0);
							$nexYaux = $pdf->GetY();

							$pdf->SetXY($this->posxeti, $curY);
							$pdf->MultiCell($this->posxfead-$this->posxeti-1, 1, dol_print_date($line['FechaAdquisicion'],"day"), 0, 'L',0);

							$pdf->SetXY($this->posxfead, $curY);
							$pdf->MultiCell($this->posxcos-$this->posxfead-1, 1, price($line['costo']), 0, 'R',0);
							...
							$pdf->SetXY($this->posxfeas, $curY);
							$pdf->MultiCell($this->posxres-$this->posxfeas-1, 1, $line['Responsable'], 0, 'C',0);

							$pdf->SetXY($this->posxres, $curY);
							$pdf->MultiCell($this->posxcon-$this->posxres-1, 1, $line['Condicion'], 0, 'C',0);

							$pdf->SetXY($this->posxcon, $curY);
							$pdf->MultiCell($this->posxfin-$this->posxcon-1, 1, $line['Estado'], 0, 'C',0);
							//$nexYend = $pdf->GetY();
							$nro++;
						}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		*/
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxeti-$this->posxcod-1,2, $outputlangs->transnoentities("Label"),'','C');
			...
			$pdf->SetXY($this->posxfeas-1, $tab_top+2);
			$pdf->MultiCell($this->posxres-$this->posxfeas-1,2, $outputlangs->transnoentities("Responsable"),'','C');
			...
			$pdf->MultiCell($this->posxfin-$this->posxcon-1,2, $outputlangs->transnoentities("Status"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		if(empty($aExtras ['nombre'])){
			$titulo = "TODOS LOS DEPARTAMENTOS CON RESPONSABLE";
		}else{
		...
		$title=$outputlangs->transnoentities("INVENTARIO ORDENADO POR MIEMBRO Y DEPARTAMENTO");
