...
/**
 *   	\file       multicurren/csindexescountry_card.php
 *		\ingroup    multicurren
...
$hookmanager->initHooks(array('csindexescountry'));
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/multicurren/list.php',1);
...
print '<tr><td class="fieldrequired">'.$langs->trans("Fieldstatus").'</td><td>$object->status</td></tr>';
	...
	if (empty($reshook))
	{
		if ($user->rights->multicurren->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>'."\n";
		}

		if ($user->rights->multicurren->delete)
		{
