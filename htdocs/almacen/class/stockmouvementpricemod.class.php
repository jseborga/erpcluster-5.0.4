...
class Stockmouvementpricemod extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		$sql .= ' '.(! isset($this->date_closed) || dol_strlen($this->date_closed)==0?'NULL':"'".$this->db->idate($this->date_closed)."'").',';
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->date_closed = $this->db->jdate($obj->date_closed);
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' date_closed = '.(! isset($this->date_closed) || dol_strlen($this->date_closed) != 0 ? "'".$this->db->idate($this->date_closed)."'" : 'null').',';
