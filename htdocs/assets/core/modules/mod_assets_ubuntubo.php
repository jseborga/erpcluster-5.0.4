 ...
 */
class mod_assets_ubuntubo extends ModeleNumRefAssets
{
	...
	function info()
	{
		...
		return $langs->trans('UbuntuboNumRefModelDesc1',$this->prefixinvoice,$this->prefixcreditnote);
	}
	...
	function getExample()
	{
		return $this->prefixinvoice."ACT-0501-0001";
	}
	...
	function canBeActivated()
	{
		...
		//$nPosindice=$conf->global->ASSETS_INDICE;
		$nPosindice= strlen($conf->global->ASSETS_PREFIX)+7;
		if ($conf->global->ASSETS_PREFIX)
		  $this->prefixasset = $conf->global->ASSETS_PREFIX;
		$nLenprefix = STRLEN($this->prefixasset);
	...
	function getNextValue($objsoc,$facture,$mode='next')
	{
	  ...
	  $prefix=$this->prefixasset.$objsoc->type_group.'-'; //prefijo y type_group
	  $nPosindice= strlen($prefix)+6;
	  //$nPosindice=$conf->global->ASSETS_INDICE;
	  $prefixasset = $prefix;
	  ...
	  $sql.= " AND entity = ".$conf->entity;
	  $resql=$db->query($sql);
	  dol_syslog("mod_assets_ubuntubo::getNextValue sql=".$sql);
	  if ($resql)
	    {
	    ...
	    {
	      dol_syslog("mod_assets_ubuntubo::getNextValue sql=".$sql, LOG_ERR);
	      return -1;
	    }
	  ...
	  if ($mode == 'last')
	    {
	      ...
	      $sql.= " FROM ".MAIN_DB_PREFIX."assets";
	      $sql.= " WHERE ref LIKE '".$prefix.$cText.$num."'";
	      $sql.= " AND entity = ".$conf->entity;
	    ...
	    {
	      $date=$objsoc->date_adq;	// This is invoice date (not creation date)
	      if (empty($date)) $date = dol_now();
	      $yymm = strftime("%y%m",$date);
	      $num = sprintf("%04s",$max+1);
	      
	      dol_syslog("mod_assets_ubuntubo::getNextValue return ".$prefix.$yymm."-".$num);
