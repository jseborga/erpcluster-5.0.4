...
if ($id && (empty($action) || $action == 'view' || $action == 'delete' || $action == 'verupdate'))
{
	...
	if (empty($reshook))
	{
		...
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->fk_budget.'&amp;idr='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>'."\n";
		}

		if ($user->rights->budget->bpri->del)
		{
			print '<div class="inline-block divButAction"><a class="butActionDelete" href="'.$_SERVER["PHP_SELF"].'?id='.$object->fk_budget.'&amp;idr='.$object->id.'&amp;action=delete">'.$langs->trans('Delete').'</a></div>'."\n";
		}
	}
	print '</div>'."\n";
	...
	//$somethingshown=$form->showLinkedObjectBlock($pobject);
	//$linktoelem = $form->showLinkToObjectBlock($pobject);
