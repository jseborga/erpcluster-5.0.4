...
class pdf_equipmentjobs extends ModelePDFMant
{
	var $db;
	var $name;
	var $um;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->mant->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->mant->dir_output."/mant/".$period_year.'/equipmentjobs';
				$file = $dir . "/equipmentjobs.pdf";
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					$pdf->MultiCell($this->posxequi-$this->posxini-1, 1, $line['ref'], 0, 'L',0);
						...
						//$pdf->MultiCell($this->posxacti-$this->posxequi-1, 1, $line['label'], 0, 'L',0);
						//$aPosY[] =$pdf->GetY();
					...
					$pdf->MultiCell($this->posxinic-$this->posxorde-1, 1, dol_print_date($line['date_ini'],'day'), 0, 'L',0);

					$pdf->SetXY($this->posxinic, $curY);
					$pdf->MultiCell($this->posxfini-$this->posxinic-1, 1, dol_print_date($line['date_fin'],'day'), 0, 'L',0);

					$pdf->SetXY($this->posxfini, $curY);
					$pdf->MultiCell($this->posxmoti-$this->posxfini-1, 1, $line['problema'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxmoti, $curY);
					$pdf->MultiCell($this->posxacci-$this->posxmoti-1, 1, $line['solucion'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();

					$pdf->SetXY($this->posxacci, $curY);
					$pdf->MultiCell($this->posxfin-$this->posxacci-1, 1, html_entity_decode($line['estado']), 0, 'L',0);
		...
		//para imprimir una linea restamos el tabtop
			...
			//$pdf->MultiCell($this->posxacti-$this->posxequi-1,2, $outputlangs->transnoentities("Assets"),'','C');
			//$pdf->line($this->posxacti-1, $tab_top, $this->posxacti-1, $tab_top + $tab_height);
						...
						$pdf->MultiCell($this->posxinic-$this->posxorde-1,2, $outputlangs->transnoentities("Dateini"),'','C');
						$pdf->line($this->posxinic-1, $tab_top, $this->posxinic-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxinic-1, $tab_top+2);
						$pdf->MultiCell($this->posxfini-$this->posxinic-1,2, $outputlangs->transnoentities("Datefin"),'','C');
						$pdf->line($this->posxfini-1, $tab_top, $this->posxfini-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxfini-1, $tab_top+2);
						$pdf->MultiCell($this->posxmoti-$this->posxfini-1,2, $outputlangs->transnoentities("Problem"),'','C');
						$pdf->line($this->posxmoti-1, $tab_top, $this->posxmoti-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxmoti-1, $tab_top+2);
						$pdf->MultiCell($this->posxacci-$this->posxmoti-1,2, $outputlangs->transnoentities("Actiontaken"),'','C');
						$pdf->line($this->posxacci-1, $tab_top, $this->posxacci-1, $tab_top + $tab_height);

						$pdf->SetXY($this->posxacci-1, $tab_top+2);
						$pdf->MultiCell($this->posxfin-$this->posxacci-1,2, $outputlangs->transnoentities("Status"),'','C');
		...
		// Logo
			...
			// width=0 (auto)
					...
					$titulo = "ORDEN DE TRABAJO POR EQUIPOS";
		...
		//Lado Derecho
					...
					$pdf->SetXY($posx,$posy);
					$pdf->SetTextColor(0,0,60);
					$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Datefin")." : " . dol_print_date($aReporte[3],'day'), '', 'R');

					//Equipo
					$posy+=4;
					$pdf->SetXY($posx,$posy);
					$pdf->SetTextColor(0,0,60);
					if(empty($object->ref_equipment)){
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Equipment")." : TODOS LOS EQUIPOS" , '','R');
					}else{
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Equipment")." : " . $object->ref_equipment, '','R');
					}
					$posy+=4;
					$pdf->SetXY($posx,$posy);
					$pdf->SetTextColor(0,0,60);
					if(empty($object->ref_equipment)){
						...
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Equipment")." : " . $object->ref_equipment.' '.$object->label_equipment, '','R');
					}
				}
