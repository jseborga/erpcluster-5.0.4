...
if (empty($reshook))
{
	...
	if (! $error && $massaction == 'confirm_presend')
	{
		...
		if (! $error)
		{
			...
			$objecttmp=new Factureext($db);
			$listofobjectid=array();
			$listofobjectthirdparties=array();
			$listofobjectref=array();
			foreach($toselect as $toselectid)
			{
				$objecttmp=new Factureext($db);	// must create new instance because instance is saved into $listofobjectref array for future use
			...
			foreach ($listofobjectthirdparties as $thirdpartyid)
			{
				...
				foreach($listofobjectref[$thirdpartyid] as $objectid => $object)
				{
					...
					if ($object->statut != Factureext::STATUS_VALIDATED)
					{
						$nbignored++;
...
$facturestatic=new Factureext($db);
    ...
    if ($num > 0)
    {
        ...
        while ($i < min($num,$limit))
        {
    		...
    		if (! empty($arrayfields['f.facnumber']['checked']))
    		{
                ...
                print $facturestatic->getNomUrladd(1,'',200,0,$notetoshow);
                print $obj->increment;
                print '</td>';
