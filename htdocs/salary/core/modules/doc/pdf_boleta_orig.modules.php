 ...
 */
class pdf_boleta_orig extends ModelePDFSalary
{
	var $db;
	var $name;
	var $description;
  ...
  // Minimum version of PHP required by module
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "boleta";
		$this->description = $langs->trans('PDFBoletaOrigDescription');
						...
						if ($resql)
						{
							...
							if ($num)
							{
								...
								$nexY = $tab_top + 7;
									//titulos
								$pdf->SetFont('','', $default_font_size - 1);
								$pdf->writeHTMLCell(190, 3, $this->posxdes+1, $tab_top-2, $langs->trans('Concept'), 0, 1);
