...
/**
 *   	\file       purchase/supplierproposaladd_list.php
 *		\ingroup    purchase
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-04-27 16:44
 */
...
$search_fk_supplier_proposal=GETPOST('search_fk_supplier_proposal','int');
...
't.fk_supplier_proposal'=>array('label'=>$langs->trans("Fieldfk_supplier_proposal"), 'checked'=>1),
...
$object=new Supplierproposaladd($db);
...
$search_status='';
    ...
    $permtoread = $user->rights->supplierproposaladd->read;
    $permtodelete = $user->rights->supplierproposaladd->delete;
    $uploaddir = $conf->supplierproposaladd->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."supplier_proposal_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."supplier_proposal_add_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_supplier_proposal) $sql.= natural_search("fk_supplier_proposal",$search_fk_supplier_proposal);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/supplierproposaladd/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_supplier_proposal']['checked'])) print_liste_field_titre($arrayfields['t.fk_supplier_proposal']['label'],$_SERVER['PHP_SELF'],'t.fk_supplier_proposal','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_supplier_proposal']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_supplier_proposal" value="'.$search_fk_supplier_proposal.'" size="10"></td>';
