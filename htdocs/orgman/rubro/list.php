...
$search_code_father=GETPOST('search_code_father','alpha');
...
if (empty($reshook))
{
	...
	$objectclass='Crubro';
	$objectlabel='Rubro';
	$permtoread = $user->rights->orgman->rub->lire;
	$permtodelete = $user->rights->orgman->rub->del;
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/rubro/list.php',1);
			header("Location: ".$urltogo);
			exit;
		}
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/rubro/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Income categories');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_rubro as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_rubro_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/poa/rubro/card.php?id='.$id);
	exit;
}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		else
		{
			...
			if (! empty($arrayfields['t.detail']['checked']))
			{
				print '<td>'.dol_trunc($obj->detail,50).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.code_father']['checked']))
			{
...
if (empty($reshook))
{
	if ($user->rights->orgman->rub->write)
	{
