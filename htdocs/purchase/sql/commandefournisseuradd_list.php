...
/**
 *   	\file       purchase/commandefournisseuradd_list.php
 *		\ingroup    purchase
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-06-30 16:57
 */
...
dol_include_once('/purchase/class/commandefournisseuradd.class.php');
...
$search_fk_commande_fournisseur=GETPOST('search_fk_commande_fournisseur','int');
...
$search_ref_contrat=GETPOST('search_ref_contrat','alpha');
$search_term=GETPOST('search_term','int');
$search_ref_term=GETPOST('search_ref_term','int');
$search_type=GETPOST('search_type','alpha');
$search_advance=GETPOST('search_advance','int');
$search_order_proceed=GETPOST('search_order_proceed','int');
$search_designation_fiscal=GETPOST('search_designation_fiscal','int');
$search_designation_supervisor=GETPOST('search_designation_supervisor','int');
...
't.fk_commande_fournisseur'=>array('label'=>$langs->trans("Fieldfk_commande_fournisseur"), 'checked'=>1),
...
't.ref_contrat'=>array('label'=>$langs->trans("Fieldref_contrat"), 'checked'=>1),
't.term'=>array('label'=>$langs->trans("Fieldterm"), 'checked'=>1),
't.ref_term'=>array('label'=>$langs->trans("Fieldref_term"), 'checked'=>1),
't.type'=>array('label'=>$langs->trans("Fieldtype"), 'checked'=>1),
't.advance'=>array('label'=>$langs->trans("Fieldadvance"), 'checked'=>1),
't.order_proceed'=>array('label'=>$langs->trans("Fieldorder_proceed"), 'checked'=>1),
't.designation_fiscal'=>array('label'=>$langs->trans("Fielddesignation_fiscal"), 'checked'=>1),
't.designation_supervisor'=>array('label'=>$langs->trans("Fielddesignation_supervisor"), 'checked'=>1),
...
$object=new Commandefournisseuradd($db);
...
$search_fk_commande_fournisseur='';
$search_object='';
$search_fk_object='';
...
$search_type='';
$search_advance='';
$search_order_proceed='';
$search_designation_fiscal='';
$search_designation_supervisor='';
$search_fk_poa_prev='';
$search_code_facture='';
...
$search_status='';
    ...
    $permtoread = $user->rights->commandefournisseuradd->read;
    $permtodelete = $user->rights->commandefournisseuradd->delete;
    $uploaddir = $conf->commandefournisseuradd->dir_output;
...
$sql.= " t.rowid,";
		...
		$sql .= " t.advance,";
		$sql .= " t.order_proceed,";
		$sql .= " t.designation_fiscal,";
		$sql .= " t.designation_supervisor,";
		$sql .= " t.date_ini,";
		$sql .= " t.date_fin,";
		$sql .= " t.fk_poa_prev,";
		$sql .= " t.code_facture,";
...
$sql.= " FROM ".MAIN_DB_PREFIX."commande_fournisseur_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."commande_fournisseur_add_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_commande_fournisseur) $sql.= natural_search("fk_commande_fournisseur",$search_fk_commande_fournisseur);
...
if ($search_ref_contrat) $sql.= natural_search("ref_contrat",$search_ref_contrat);
if ($search_term) $sql.= natural_search("term",$search_term);
if ($search_ref_term) $sql.= natural_search("ref_term",$search_ref_term);
if ($search_type) $sql.= natural_search("type",$search_type);
if ($search_advance) $sql.= natural_search("advance",$search_advance);
if ($search_order_proceed) $sql.= natural_search("order_proceed",$search_order_proceed);
if ($search_designation_fiscal) $sql.= natural_search("designation_fiscal",$search_designation_fiscal);
if ($search_designation_supervisor) $sql.= natural_search("designation_supervisor",$search_designation_supervisor);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/commandefournisseuradd/card.php?id='.$id);
    exit;
}
...
// 
if (! empty($arrayfields['t.fk_commande_fournisseur']['checked'])) print_liste_field_titre($arrayfields['t.fk_commande_fournisseur']['label'],$_SERVER['PHP_SELF'],'t.fk_commande_fournisseur','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.ref_contrat']['checked'])) print_liste_field_titre($arrayfields['t.ref_contrat']['label'],$_SERVER['PHP_SELF'],'t.ref_contrat','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.term']['checked'])) print_liste_field_titre($arrayfields['t.term']['label'],$_SERVER['PHP_SELF'],'t.term','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.ref_term']['checked'])) print_liste_field_titre($arrayfields['t.ref_term']['label'],$_SERVER['PHP_SELF'],'t.ref_term','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.advance']['checked'])) print_liste_field_titre($arrayfields['t.advance']['label'],$_SERVER['PHP_SELF'],'t.advance','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.order_proceed']['checked'])) print_liste_field_titre($arrayfields['t.order_proceed']['label'],$_SERVER['PHP_SELF'],'t.order_proceed','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.designation_fiscal']['checked'])) print_liste_field_titre($arrayfields['t.designation_fiscal']['label'],$_SERVER['PHP_SELF'],'t.designation_fiscal','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.designation_supervisor']['checked'])) print_liste_field_titre($arrayfields['t.designation_supervisor']['label'],$_SERVER['PHP_SELF'],'t.designation_supervisor','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_commande_fournisseur']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_commande_fournisseur" value="'.$search_fk_commande_fournisseur.'" size="10"></td>';
...
if (! empty($arrayfields['t.ref_contrat']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ref_contrat" value="'.$search_ref_contrat.'" size="10"></td>';
if (! empty($arrayfields['t.term']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_term" value="'.$search_term.'" size="10"></td>';
if (! empty($arrayfields['t.ref_term']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ref_term" value="'.$search_ref_term.'" size="10"></td>';
...
if (! empty($arrayfields['t.advance']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_advance" value="'.$search_advance.'" size="10"></td>';
if (! empty($arrayfields['t.order_proceed']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_order_proceed" value="'.$search_order_proceed.'" size="10"></td>';
if (! empty($arrayfields['t.designation_fiscal']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_designation_fiscal" value="'.$search_designation_fiscal.'" size="10"></td>';
if (! empty($arrayfields['t.designation_supervisor']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_designation_supervisor" value="'.$search_designation_supervisor.'" size="10"></td>';
