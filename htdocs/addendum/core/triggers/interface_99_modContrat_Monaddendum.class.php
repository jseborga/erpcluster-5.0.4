...
class InterfaceMonaddendum
{
	...
	function run_trigger($action,$object,$user,$langs,$conf)
	{
		...
		{
	  //verificamos los montos de contrato segun la variable
	  //cuando se valida damos una opcion para seleccionar el contrato padre
			$_SESSION['urladd'] = $_SERVER['PHP_SELF'];

			header("Location: ".DOL_URL_ROOT.'/addendum/fiche.php?cid='.$object->id.'&action=confirm_add&backtopage='.$_SERVER['PHP_SELF']);
			exit;
	  // $langs->load("addendum@addendum");

	  // require_once DOL_DOCUMENT_ROOT.'/core/class/html.form.class.php';
	  // $form = new Form($db);
	  // $ret=$form->formconfirm(DOL_URL_ROOT.'/addendum/fiche.php'."?cid=".$object->id,
	  // 			  $langs->trans("Contracts"),
	  // 			  $langs->trans("Linktocontractfhater").' '.$object->ref,
	  // 			  "confirm_add_contrat",
	  // 			  '',
	  // 			  0,
	  // 			  0);
	  ...
	  // print $ret;
		...
		{
			dol_syslog("Trigger '".$this->name."' for action '$action' launched by ".__FILE__.". id=".$object->id);
			//agregamos la modificacion de acuerdo al iva
			$defvalue = $conf->global->ADDENDUM_REG_VALUE_CONTRAT;
			if ($defvalue)
			{
				$object->fetch_lines();
				foreach ((array) $object->lines AS $j => $obj1)
				{
					$objnew = new ContratLigne($this->db);
					$objnew->fetch($obj1->id);
					$total_ttc = $objnew->total_ttc;
					$total_ht = $objnew->total_ht;
					$tva_tx = $objnew->tva_tx;
					//reemplazamos
					$objnew->total_ttc = $total_ht;
					$objnew->total_ht = price2num($total_ht / (1+($tva_tx/100)));
					$objnew->total_tva = $objnew->total_ttc - $objnew->total_ht;
					$res = $objnew->update_total();
					if (!$res >0)
						$err++;
				}
				if (empty($err))
		...
		elseif ($action == 'PROJECT_BUILDDOC')
		{
			dol_syslog("Trigger '".$this->name."' for action '$action' launched by ".__FILE__.". id=".$object->id);
		}
