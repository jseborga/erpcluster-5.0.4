...
$server->wsdl->addComplexType(
	...
	array(
		...
		'name_societe' => array('name'=>'name_societe','type'=>'xsd:string'),
	...
	if (! $error)
	{
			...
			|| ($fuser->rights->user->self->creer && $ref_ext && $ref_ext==$fuser->ref_ext))
		{
			...
			if ($result > 0)
			{
				...
				{
					$ressoc = $soc->fetch( $user->societe_id );
					$user->name_societe = $soc->name;
				}
				else
					$user->name_societe= '';
				// Create
				...
				$objectresp = array(
					...
					'user'=>array(
						...
						'name_societe' => $user->name_societe,
...
function setUserPassword($authentication,$shortuser) {
	...
	if (! $error)
	{
		...
		if ($fuser->rights->user->user->password || $fuser->rights->user->self->password)
		{
			...
			if($res)
			{
				$res = $userstat->setPassword($userstat,$shortuser['password'],1);
				if($res)
				{
