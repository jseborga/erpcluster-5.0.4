...
/**
 * \file    salary/pformulasdet.class.php
 * \ingroup salary
 ...
 * @see CommonObject
 */
class Pformulasdet extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'pformulasdet';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'p_formulas_det';

	/**
	 * @var PformulasdetLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->changefull)) {
			 $this->changefull = trim($this->changefull);
		}
		if (isset($this->nmonth)) {
			 $this->nmonth = trim($this->nmonth);
		}
		if (isset($this->andor)) {
			 $this->andor = trim($this->andor);
		}
		...
		$sql .= ' '.(! isset($this->changefull)?'NULL':"'".$this->db->escape($this->changefull)."'").',';
		$sql .= ' '.(! isset($this->nmonth)?'NULL':$this->nmonth).',';
		$sql .= ' '.(! isset($this->andor)?'NULL':$this->andor).',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("pformulasdet", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->type = $obj->type;
				$this->changefull = $obj->changefull;
				$this->nmonth = $obj->nmonth;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->type = $obj->type;
				$line->changefull = $obj->changefull;
				$line->nmonth = $obj->nmonth;
				$line->andor = $obj->andor;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' changefull = '.(isset($this->changefull)?"'".$this->db->escape($this->changefull)."'":"null").',';
		$sql .= ' nmonth = '.(isset($this->nmonth)?$this->nmonth:"null").',';
		$sql .= ' andor = '.(isset($this->andor)?$this->andor:"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Pformulasdet($this->db);
