<?php
/* Copyright (C) 2016      Ramiro Queso       <ramiro@ubuntu-bo.com>
 *
...
// Add new contact
if ($action == 'addcontact' && $user->rights->budget->bud->write)
{
	$result = 0;
...
if ($action == 'swapstatut' && $user->rights->budget->bud->write)
{
	if ($object->fetch($id))
	{
...
// Efface un contact
if (($action == 'deleteline' || $action == 'deletecontact') && $user->rights->budget->bud->write)
{
	$object->fetch($id);
...
$title=$langs->trans("BudgetContact").' - '.$object->ref.' '.$object->name;
...
if ($id > 0 || ! empty($ref))
{
	$object->fetch($id,(!empty($ref)?$ref:null));
	// To verify role of users
	//$userAccess = $object->restrictedProjectArea($user,'read');
	$userWrite  = $object->restrictedBudgetArea($user,'write');
	//$userDelete = $object->restrictedProjectArea($user,'delete');
	//print "userAccess=".$userAccess." userWrite=".$userWrite." userDelete=".$userDelete;

	$head = budget_prepare_head($object,$user);
	dol_fiche_head($head, 'Contact', $langs->trans("Budget"), 0, ($object->public?'projectpub':'budget'));
	...
	*/
	...
	$linkback = '<a href="'.DOL_URL_ROOT.'/budget/list.php">'.$langs->trans("BackToList").'</a>';

	// Ref
	print '<tr><td width="30%">'.$langs->trans('Ref').'</td><td colspan="3">';
	// Define a complementary filter for search of next/prev ref.
	if (! $user->rights->budget->budc->lire)
	{
		$objectsListId = $object->getBudgetAuthorizedForUser($user,$mine,0);
