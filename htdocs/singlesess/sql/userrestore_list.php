...
/**
 *   	\file       singlesess/userrestore_list.php
 *		\ingroup    singlesess
...
$hookmanager->initHooks(array('userrestorelist'));
...
't.coderest'=>array('label'=>$langs->trans("Fieldcoderest"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."user_restore as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."user_restore_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_user) $sql.= natural_search("fk_user",$search_fk_user);
if ($search_coderest) $sql.= natural_search("coderest",$search_coderest);
...
if ($search_coderest != '') $params.= '&amp;search_coderest='.urlencode($search_coderest);
...
if (! empty($arrayfields['t.coderest']['checked'])) print_liste_field_titre($arrayfields['t.coderest']['label'],$_SERVER['PHP_SELF'],'t.coderest','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.coderest']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_coderest" value="'.$search_coderest.'" size="10"></td>';
