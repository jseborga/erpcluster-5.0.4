...
/**
 *   	\file       orgman/pholiday_list.php
 *		\ingroup    orgman
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-09-18 14:09
 */
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'orgmanlist';
...
$arrayfields=array(
	...
	't.date_day'=>array('label'=>$langs->trans("Fielddate_day"), 'checked'=>1),
	't.date_month'=>array('label'=>$langs->trans("Fielddate_month"), 'checked'=>1),
	't.date_year'=>array('label'=>$langs->trans("Fielddate_year"), 'checked'=>1),
	't.type'=>array('label'=>$langs->trans("Fieldunique"), 'checked'=>1),
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/list.php',1);
...
$sql.= " FROM ".MAIN_DB_PREFIX."p_holiday as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."p_holiday_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_date_day) $sql.= natural_search("date_day",$search_date_day);
if ($search_date_month) $sql.= natural_search("date_month",$search_date_month);
if ($search_date_year) $sql.= natural_search("date_year",$search_date_year);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/pholiday/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.date_day']['checked'])) print_liste_field_titre($arrayfields['t.date_day']['label'],$_SERVER['PHP_SELF'],'t.date_day','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.date_month']['checked'])) print_liste_field_titre($arrayfields['t.date_month']['label'],$_SERVER['PHP_SELF'],'t.date_month','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.date_year']['checked'])) print_liste_field_titre($arrayfields['t.date_year']['label'],$_SERVER['PHP_SELF'],'t.date_year','',$params,'',$sortfield,$sortorder);
...
//
	...
	if (! empty($arrayfields['t.date_day']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_date_day" value="'.$search_date_day.'" size="10"></td>';
	if (! empty($arrayfields['t.date_month']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_date_month" value="'.$search_date_month.'" size="10"></td>';
	if (! empty($arrayfields['t.date_year']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_date_year" value="'.$search_date_year.'" size="10"></td>';
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<input type="text" name="ref" value="'.$obj->ref.'" maxlength="30"></td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.label']['checked']))
	{
		print '<td>'.'<input type="text" name="label" value="'.$obj->label.'" maxlength="200"></td>';
		if (! $i) $totalarray['nbfield']++;
	}

	if (! empty($arrayfields['t.date_day']['checked']))
	{
		print '<td>'.'<input type="number" min="1" max="'.$max.'" name="date_day" value="'.GETPOST('date_day').'"></td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.date_month']['checked']))
	{
		print '<td>'.'<input type="number" min="1" max="12" name="date_month" value="'.GETPOST('date_month').'"></td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.date_year']['checked']))
	{
		print '<td>'.'<input type="number" min="1" max="9999" name="date_year" value="'.GETPOST('date_year').'"></td>';
		if (! $i) $totalarray['nbfield']++;
	}
	...
	print '<td>';
	print '<input type="submit" class="butAction" value="'.$langs->trans('Keep').'">';
	print '</td>';
	print '</tr>';
}
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if (! empty($arrayfields['t.date_day']['checked']))
		{
			print '<td>'.$obj->date_day.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.date_month']['checked']))
		{
			print '<td>'.$obj->date_month.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.date_year']['checked']))
		{
			if ($obj->type ==1)
				print '<td>'.$obj->date_year.'</td>';
			else
				print '<td>'.'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.type']['checked']))
		{
			print '<td>'.($obj->type?$langs->trans('Yes'):$langs->trans('No')).'</td>';
			if (! $i) $totalarray['nbfield']++;
		}
		if (! empty($arrayfields['t.fk_user_create']['checked']))
		{
