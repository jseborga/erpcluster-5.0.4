...
/**
 * \file    monprojet/projetadd.class.php
 * \ingroup monprojet
 ...
 * @see CommonObject
 */
class Projetadd extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'projetadd';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'projet_add';

	/**
	 * @var ProjetaddLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->programmed)) {
			 $this->programmed = trim($this->programmed);
		}
		if (isset($this->fk_contracting)) {
			 $this->fk_contracting = trim($this->fk_contracting);
		}
		if (isset($this->fk_supervising)) {
			 $this->fk_supervising = trim($this->fk_supervising);
		}
		if (isset($this->use_resource)) {
			 $this->use_resource = trim($this->use_resource);
		}
		...
		$sql .= ' '.(! isset($this->programmed)?'NULL':"'".$this->db->escape($this->programmed)."'").',';
		$sql .= ' '.(! isset($this->fk_contracting)?'NULL':$this->fk_contracting).',';
		$sql .= ' '.(! isset($this->fk_supervising)?'NULL':$this->fk_supervising).',';
		$sql .= ' '.(! isset($this->use_resource)?'NULL':"'".$this->db->escape($this->use_resource)."'").',';
	...
	public function fetch($id, $fk=0)
	{
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->programmed = $obj->programmed;
				$this->fk_contracting = $obj->fk_contracting;
				$this->fk_supervising = $obj->fk_supervising;
				$this->use_resource = $obj->use_resource;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' programmed = '.(isset($this->programmed)?"'".$this->db->escape($this->programmed)."'":"null").',';
		$sql .= ' fk_contracting = '.(isset($this->fk_contracting)?$this->fk_contracting:"null").',';
		$sql .= ' fk_supervising = '.(isset($this->fk_supervising)?$this->fk_supervising:"null").',';
		$sql .= ' use_resource = '.(isset($this->use_resource)?"'".$this->db->escape($this->use_resource)."'":"null").',';
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='')
	{
        ...
        $label = '<u>' . $langs->trans("Monprojet") . '</u>';
        $label.= '<div width="100%">';
