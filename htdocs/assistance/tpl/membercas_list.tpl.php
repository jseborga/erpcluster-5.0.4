...
/**
 *   	\file       assistance/membercas_list.php
 *		\ingroup    assistance
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-09-12 10:49
 */
...
$search_number_year=GETPOST('search_number_year','int');
$search_number_month=GETPOST('search_number_month','int');
$search_number_day=GETPOST('search_number_day','int');
...
't.number_year'=>array('label'=>$langs->trans("Fieldnumber_year"), 'checked'=>1),
't.number_month'=>array('label'=>$langs->trans("Fieldnumber_month"), 'checked'=>1),
't.number_day'=>array('label'=>$langs->trans("Fieldnumber_day"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->membercas->read;
    $permtodelete = $user->rights->membercas->delete;
    $uploaddir = $conf->membercas->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."member_cas as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."member_cas_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_member) $sql.= natural_search("fk_member",$search_fk_member);
if ($search_number_year) $sql.= natural_search("number_year",$search_number_year);
if ($search_number_month) $sql.= natural_search("number_month",$search_number_month);
if ($search_number_day) $sql.= natural_search("number_day",$search_number_day);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/membercas/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.number_year']['checked'])) print_liste_field_titre($arrayfields['t.number_year']['label'],$_SERVER['PHP_SELF'],'t.number_year','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.number_month']['checked'])) print_liste_field_titre($arrayfields['t.number_month']['label'],$_SERVER['PHP_SELF'],'t.number_month','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.number_day']['checked'])) print_liste_field_titre($arrayfields['t.number_day']['label'],$_SERVER['PHP_SELF'],'t.number_day','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.number_year']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_number_year" value="'.$search_number_year.'" size="10"></td>';
if (! empty($arrayfields['t.number_month']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_number_month" value="'.$search_number_month.'" size="10"></td>';
if (! empty($arrayfields['t.number_day']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_number_day" value="'.$search_number_day.'" size="10"></td>';
...
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
    ...
    print $formfile->showdocuments('massfilesarea_assistance','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
