...
$colmes = 7 + $monActual - 1;
$colmesf = 14 - $colmes + 1;
$htmlcol = '<colgroup>';
$htmlcol.= '<col span="'.$colmes.'" >';
$htmlcol.= '<col style="background-color:#CFCDFF;">';
$htmlcol.= '<col span="'.$colmesf.'" >';
$htmlcol.= '</colgroup>';
...
if (isset($_POST['search_all']) || isset($_GET['search_all']))
{
	...
	//eliminamos cualquier otra busqueda
	unset($aDetail['search_login']);
	unset($aDetail['search_all']);
	$search_all = GETPOST('search_all');
	if (!empty($search_all))
		$aDetail['search_all'] = $search_all;
	else
		unset($aDetail['search_all']);
...
//quitar cuando ya esta funcionando
...
if ($respoa)
{
	...
	if (!isset($_SESSION['aPartida'])) $_SESSION['aPartida'] = get_partida($period_year);
	...
	else
	{
		...
		if ($objap->period_year == $period_year) $nVersion = $objap->version + 1;
		else $nVersion = 1;
	}
	...
	print '</form>';
	print '<table id="data1" class="table table-bordered table-hover table-striped" role="grid">';
	print $htmlcol;
	//fin menu seleccion
	...
	if ($num)
	{
		...
		while ($i < $num)
		{
			...
			foreach ((array) $objactl->array AS $jl => $objppl)
			{
				...
				//preventivos
				//obtenemos el preventivo de la period_year
				...
				if (count($objprocc->aContrat)>0 && $fk_proc_pri >0)
				{
					foreach ((array) $objprocc->aContrat AS $fk_c => $value)
					{
						...
						if ($objcon->id == $fk_c && $fk_c>0)
						{
							...
							//revisamos filtro por contrato
							include DOL_DOCUMENT_ROOT.'/poa/poa/lib/filtercontrat.lib.php';
						}
					}
				}
				//devengados
					...
					else
					{
						...
						foreach ((array) $aActplanif AS $cCode => $dPlan)
						{
							if ($lLoop)
							{
								//armamos lo planificado del mes
								$aDateplan = dol_getdate($dPlan);
								if ($aDateplan['year'] == $_SESSION['period_year'])
								{
									if ($aDateplan['mon']>0)
									{
			...
			$n_search = 0;
				//principal
			foreach ((array) $aActivity[$obj->id] AS $j1 => $aP)
				//$j1 es el id de la actividad
			{
				$n_search= 0;
				$aActivityf_ = array();
				foreach ((array) $aP AS $j2 => $val)
					//$j2 es el search
				{
					if ($val)
					{
			...
			if ($aPoaf[$obj->id])
			{
				...
				//preparamos la edicion del insumos
				$objectpoa = new Poapoa($db);
				$objectpoa->fetch($obj->id);
				include DOL_DOCUMENT_ROOT.'/poa/poa/tpl/fiche_poa.tpl.php';
			}
			$i++;
				//print '</tr>';
		}
	...
	if ($action != 'create' || $action != 'edit')
	{
		...
		{
			print '<button class="btn btn-primary btn-lg bg-primary" href="#fichepoa" role="button" data-toggle="modal">'.$langs->trans('New').'</button>';
		...
		if ($user->rights->poa->exp)
			print '<a class="btn btn-app" href="fiche_excel.php"><i class="fa fa-file-excel-o"></i>'.$langs->trans("Excel").'</a>';
		else
			print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Excel")."</a>";
				//registro nuevo poa
		include DOL_DOCUMENT_ROOT.'/poa/poa/tpl/fiche_poanew.tpl.php';
	...
	print '<form method="POST" action="'.DOL_URL_ROOT.'/poa/ind.php'.'">';
	print '<input type="hidden" name="a_s" value="1">';
	print '<button class="botonF1">';
	print '<span>'.$langs->trans('Prin').'</span>';
	print '</button>';
	print '</form>';
