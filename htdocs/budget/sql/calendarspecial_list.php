...
/**
 *   	\file       budget/calendarspecial_list.php
 *		\ingroup    budget
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-03-01 19:05
 */
...
$search_type_date=GETPOST('search_type_date','int');
$search_working_day_hour=GETPOST('search_working_day_hour','alpha');
...
$hookmanager->initHooks(array('calendarspeciallist'));
...
't.type_date'=>array('label'=>$langs->trans("Fieldtype_date"), 'checked'=>1),
't.working_day_hour'=>array('label'=>$langs->trans("Fieldworking_day_hour"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."calendar_special as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."calendar_special_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_type_date) $sql.= natural_search("type_date",$search_type_date);
if ($search_working_day_hour) $sql.= natural_search("working_day_hour",$search_working_day_hour);
...
if ($search_type_date != '') $params.= '&amp;search_type_date='.urlencode($search_type_date);
if ($search_working_day_hour != '') $params.= '&amp;search_working_day_hour='.urlencode($search_working_day_hour);
...
if (! empty($arrayfields['t.type_date']['checked'])) print_liste_field_titre($arrayfields['t.type_date']['label'],$_SERVER['PHP_SELF'],'t.type_date','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.working_day_hour']['checked'])) print_liste_field_titre($arrayfields['t.working_day_hour']['label'],$_SERVER['PHP_SELF'],'t.working_day_hour','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.type_date']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_type_date" value="'.$search_type_date.'" size="10"></td>';
if (! empty($arrayfields['t.working_day_hour']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_working_day_hour" value="'.$search_working_day_hour.'" size="10"></td>';
