...
//$display = 'block';
print '<div id="'.$tagid.'" class="modal modal-warning fade in" tabindex="-1" role="dialog" style="display: '.$display.'; margin-top:0px;" data-width="760;" aria-hidden="false">';
...
if ($idProcess>0 && $idrcreg > 0)
{
	...
	if ($objpcon->date_order_proceed > 0)
		$aDate = dol_getdate($objpcon->date_order_proceed);
	$dateop = $aDate['year'].'-'.$aDate['mon'].'-'.$aDate['mday'];
	//date order proceed
	print '<div class="form-group">';
	print '<label class="col-sm-3 control-label" for="socid">'.$langs->trans('Orderproceed').'</label>';
	print '<div class="col-xs-8">';
	print '<div class="well well-sm">';
	print '<div class="input-group date" id="datepicinixx">';
	print '<input type="text" name="di_" id="datepicini" class="flat" value="'.$dateop.'" readonly/>';
