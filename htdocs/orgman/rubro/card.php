...
$object = new Crubro($db);
$objecttmp = new Crubro($db);
...
$hookmanager->initHooks(array('crubro'));
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/rubro/list.php',1);
	...
	//include DOL_DOCUMENT_ROOT.'/poa/rubro/inc/abm_partidaproduct.inc.php';
	...
	if ($action == 'add')
	{
		...
		$object->code_father=GETPOST('code_father','int');
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/rubro/card.php?id'.$result,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	if ($action == 'update')
	{
		...
		if ($object->code_father)
		{
			$res = $objecttmp->fetch($object->code_father);
			if ($res==1)
				$object->code_father = $objecttmp->code;
		}
		if (! $error)
		{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/poa/rubro/list.php',1));
			exit;
		}
		else
		{
...
foreach ($objecttmp->lines AS $j => $line)
{
	...
	$selected = '';
	if ($action == 'edit' && $line->code == $object->code_father) $selected = ' selected ';
...
{
	print load_fiche_titre($langs->trans("Rubro"));
	...
	// print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td><input class="flat" type="text" size="36" name="label" value="'.$label.'"></td></tr>';
	//
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldperiod_year").'</td><td><input class="flat" type="text" name="period_year" value="'.(GETPOST('period_year')?GETPOST('period_year'):$object->period_year).'"></td></tr>';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'partida', $langs->trans("Rubro"), 0, 'partida');

	print load_fiche_titre($langs->trans("Rubros"));
	...
	{
		$objecttmp->fetch(0,$object->code_father,$object->period_year);
		print '<tr><td>'.$langs->trans("Fieldcode_father").'</td><td>'.$objecttmp->getNomUrl(1).'</td></tr>';
	}
	...
	//mostramos la realcion con productos
		...
		//include DOL_DOCUMENT_ROOT.'/poa/partida/tpl/partidaproduct.tpl.php';
