 ...
 */
class pdf_inputalm extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Ingreso almacen";
		$this->description = $langs->trans('PDFInputAlmDescription');
		...
		$this->posxnum=$this->marge_gauche+1;
		$this->posxcod=19;
		$this->posxdesc=50;
		//$this->posxsaa=145;
		$this->posxuni=131;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->almacen->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->almacen->dir_output . "/" . $object->ref;
			...
			if (file_exists($dir))
			{
				...
				*/
				...
				if ($object->label)
				{
					...
					$pdf->writeHTMLCell(190, 3, $this->posxnum-1, $tab_top-2, $langs->trans('Note'), 0, 1);
					$pdf->writeHTMLCell(0, 3, $this->posxnum+8, $tab_top-2, dol_htmlentitiesbr($object->label), 0, 1);
					$nexY = $pdf->GetY();
				...
				//for ($i = 0; $i < 30; $i++)
				{
					...
					$pdf->MultiCell(5, 3, $cont, 0, 'R',0);

					// codigo
					$curX = $this->posxcod-1;
					$showpricebeforepagebreak=1;
					$product->fetch($lines->fk_product);
					...
					$pdf->MultiCell($this->posxuni+1-$this->posxdesc-1, 3, $desc, 0, 'L',0);

					// unit
					//$unit = $product->getLabelOfUnit('short');
					$unit = $product->getLabelOfUnit();
					$pdf->SetXY($this->posxuni, $curY);
					$pdf->MultiCell($this->posxsen-$this->posxuni-1, 3, $outputlangs->transnoentities($unit), 0, 'L',0);
					...
					// pu
					$pu = price(price2num($lines->price,'MT'));
					//modificado para emav
					$aPu = explode('.',$lines->price);
					$cEntero = $aPu[0];
					$cDecimal = substr($aPu[1],0,2);
					$pu = $cEntero.'.'.$cDecimal;
					$pdf->SetXY($this->posxpu, $curY);
					$pdf->MultiCell($this->posxbal-$this->posxpu-1, 3, $pu, 0, 'R',0);

					// balance
					$bal = price(price2num($lines->value*$lines->price,'MT'));
					if ($lines->statut==2)
						$bal = price(price2num($lines->quant*$lines->price,'MT'));
					$pdf->SetXY($this->posxbal, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxbal, 3, $bal, 0, 'R',0);

					if ($lines->statut==2)
						$sumValor += price2num($lines->quant*$lines->price,'MT');
					else
						$sumValor += price2num($lines->value*$lines->price,'MT');
				...
				//$this->_tableau_total($pdf, $object, $posy, $outputlangs);
				//$type_page=1;
				...
				$posym=$marginwithfooter+0;
				$bottomlasttab-=12;
				pdf_pie_page_mod7($pdf, $object, $bottomlasttab, $outputlangs,$this->marge_gauche, $this->page_largeur,$this->marge_droite,$this->page_hauteur,$this->type_page=0,$bottomlasttab);
	...
	*/
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxcod-$this->posxnum-1,2, $outputlangs->transnoentities("Nro"),'','C');
		}
		...
		$pdf->line($this->posxuni-1, $tab_top, $this->posxuni-1, $tab_top + $tab_height-18);
		if (empty($hidetop))
		{
		...
		$pdf->line($this->posxsen-1, $tab_top, $this->posxsen-1, $tab_top + $tab_height-18);
		if (empty($hidetop))
		{
		...
		$pdf->line($this->posxpu-1, $tab_top, $this->posxpu-1, $tab_top + $tab_height-18);
		if (empty($hidetop))
		{
		...
		$pdf->line($this->posxbal, $tab_top, $this->posxbal, $tab_top + $tab_height-18);
		if (empty($hidetop))
		{
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$title=$outputlangs->transnoentities("Ingreso de Materiales");
		...
		if ($object->entrepot)
		{
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Warehouse")." : " . $outputlangs->convToOutputCharset($object->entrepot), '', 'R');
			$posy+=1;
		}
	...
	*/
	...
	function _tableau_total(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->SetTextColor(0,0,60);
		$pdf->line($this->marge_gauche, $posy, ($this->page_largeur-$this->marge_droite)/2-20, $posy);
		$pdf->line(($this->page_largeur-$this->marge_droite)/2+20, $posy, $this->page_largeur-$this->marge_droite, $posy);
