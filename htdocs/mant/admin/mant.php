...
if ($action == 'specimen')
{
	...
	{
		$file=dol_buildpath($reldir."mant/core/modules/doc/pdf_".$modele.".modules.php",0);
		if (file_exists($file))
		{
			$filefound=1;
...
if ($action == 'set')
{
	...
	$res = dolibarr_set_const($db,"MANT_EQUIPMENT_INTEGRATED_WITH_ASSET",GETPOST('MANT_EQUIPMENT_INTEGRATED_WITH_ASSET','int'),'yesno',0,'',$conf->entity);
	$res = dolibarr_set_const($db,"MANT_SEND_EMAIL",GETPOST('MANT_SEND_EMAIL','int'),'yesno',0,'',$conf->entity);
	$res = dolibarr_set_const($db,"MANT_LINK_WEB",GETPOST('MANT_LINK_WEB','alpha'),'chaine',0,'',$conf->entity);
...
if ($action == 'del')
{
	...
	if ($db->query($sql))
	{
		if ($conf->global->ALMACEN_ADDON_PDF == "$value") dolibarr_del_const($db, 'MANT_ADDON_PDF',$conf->entity);
	}
}
...
if ($action == 'setdoc')
{
	...
	if (dolibarr_set_const($db, "MANT_ADDON_PDF",$value,'chaine',0,'',$conf->entity))
	{
...
if ($action == 'setmod')
{
  ...
  // par appel methode canBeActivated
	...
	dolibarr_set_const($db, "MANT_ADDON",$value,'chaine',0,'',$conf->entity);
}
...
llxHeader("",$langs->trans("MantSetup"));

$form=new Form($db);

$linkback='<a href="'.DOL_URL_ROOT.'/admin/modules.php">'.$langs->trans("BackToModuleList").'</a>';
print_fiche_titre($langs->trans("MantSetup"),$linkback,'setup');
print '<br>';
...
$var=!$var;
print '<tr '.$bc[$var].'><td width=\"50%\">'.$langs->trans("Equipos integrados totalmente con Activos").'</td>';
print '<td colspan="2">';
print $form->selectyesno('MANT_EQUIPMENT_INTEGRATED_WITH_ASSET',$conf->global->MANT_EQUIPMENT_INTEGRATED_WITH_ASSET,1);
print '</td></tr>';

$var=!$var;
print '<tr '.$bc[$var].'><td width=\"50%\">'.$langs->trans("Habilitar envío de correos en mantenimiento").'</td>';
print '<td colspan="2">';
print $form->selectyesno('MANT_SEND_EMAIL',$conf->global->MANT_SEND_EMAIL,1);
print '</td></tr>';

$var=!$var;
print '<tr '.$bc[$var].'><td width=\"50%\">'.$langs->trans("Nombre del dominio o link de la pagina web para incluir en los correos").'</td>';
print '<td colspan="2">';
print '<input type="text" name="MANT_LINK_WEB" value="'.$conf->global->MANT_LINK_WEB.'" size="30">';
print '</td></tr>';
...
print_titre($langs->trans("MantNumberingModules"));
print '<table class="noborder" width="100%">';
print '<tr class="liste_titre">';
...
foreach ($dirmodels as $reldir)
{
	$dir = dol_buildpath($reldir."mant/core/modules/");
	if (is_dir($dir))
	{
		$handle = opendir($dir);
		if (is_resource($handle))
		{
			...
			{
				if (substr($file, 0, 9) == 'mod_mant_' && substr($file, dol_strlen($file)-3, 3) == 'php')
				{
					$file = substr($file, 0, dol_strlen($file)-4);

					require_once DOL_DOCUMENT_ROOT ."/mant/core/modules/".$file.'.php';
			  ...
			  // Show example of numbering module
						...
						if ($conf->global->MANT_ADDON == $file)
						{
 ...
 */
print_titre($langs->trans("MantModelModule"));
...
foreach ($dirmodels as $reldir)
{
	...
	{
		$dir = dol_buildpath($reldir."/mant/core/modules".$valdir);
		...
		if (is_dir($dir))
		{
			...
			if (is_resource($handle))
			{
				...
				foreach($filelist as $file)
				{
				  ...
				  // Defaut
								print '<td align="center">';
								if ($conf->global->MANT_ADDON_PDF == $name)
								{
