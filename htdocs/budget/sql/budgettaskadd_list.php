...
/**
 *   	\file       budget/budgettaskadd_list.php
 *		\ingroup    budget
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-02-13 18:13
 */
...
dol_include_once('/budget/class/budgettaskadd.class.php');
...
$search_complementary=GETPOST('search_complementary','alpha');
$search_unit_budget=GETPOST('search_unit_budget','alpha');
$search_unit_amount=GETPOST('search_unit_amount','alpha');
$search_total_amount=GETPOST('search_total_amount','alpha');
...
$hookmanager->initHooks(array('budgettaskaddlist'));
...
't.complementary'=>array('label'=>$langs->trans("Fieldcomplementary"), 'checked'=>1),
...
't.total_amount'=>array('label'=>$langs->trans("Fieldtotal_amount"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."budget_task_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."budget_task_add_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_complementary) $sql.= natural_search("complementary",$search_complementary);
...
if ($search_total_amount) $sql.= natural_search("total_amount",$search_total_amount);
...
if ($search_complementary != '') $params.= '&amp;search_complementary='.urlencode($search_complementary);
...
if ($search_total_amount != '') $params.= '&amp;search_total_amount='.urlencode($search_total_amount);
...
if (! empty($arrayfields['t.total_amount']['checked'])) print_liste_field_titre($arrayfields['t.total_amount']['label'],$_SERVER['PHP_SELF'],'t.total_amount','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.complementary']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_complementary" value="'.$search_complementary.'" size="10"></td>';
...
if (! empty($arrayfields['t.total_amount']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_total_amount" value="'.$search_total_amount.'" size="10"></td>';
