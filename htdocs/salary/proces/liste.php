...
/**
 *      \file       htdocs/salary/proces/liste.php
 *      \ingroup    Salary proces
 *      \brief      Page liste Salary proces
 */
...
if (!$user->rights->salary->proces->lire)
  accessforbidden();
...
$sql  = "SELECT p.rowid AS id, p.ref, p.label, p.state ";
$sql.= " FROM ".MAIN_DB_PREFIX."p_proces as p ";
...
if ($result)
{
	...
	llxHeader("",$langs->trans("Liste proces"),$help_url);

	print_barre_liste($langs->trans("Liste proces"), $page, "liste.php", "", $sortfield, $sortorder,'',$num);
	  ...
	  while ($i < min($num,$limit))
	    {
	      ...
	      $var=!$var;
	      print "<tr $bc[$var]>";
	      print '<td><a href="fiche.php?id='.$obj->id.'">'.img_object($langs->trans("Ref"),'proces').' '.$obj->ref.'</a></td>';
