...
/**
 *   	\file       budget/ctypeitem_card.php
 *		\ingroup    budget
...
$hookmanager->initHooks(array('ctypeitem'));
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/budget/ctypeitem/list.php',1);
	...
	if ($action == 'add')
	{
		...
		$object->code=dol_string_nospecial(trim(GETPOST('code','alpha')));
		...
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Cod")), null, 'errors');
		}
	...
	{
		//buscamos si se estea usando
		$objItems = new Itemsext($db);
		$filter = " AND t.fk_type_item = ".$id;
		...
		if (empty($res))
		{
			...
			// Delete OK
				...
				header("Location: ".dol_buildpath('/budget/ctypeitem/list.php',1));
				exit;
			}
			else
			{
		...
		{
			$error++;
			setEventMessages($langs->trans('Thereisrecordrelated').' '.$langs->trans('in').' ITEMS',null,'errors');
		}
	}
}
...
$aActive=array();
$aActive[]=($langs->trans('Not'));
$aActive[]=($langs->trans('Yes'));


$optionsac = '';
foreach ($aActive AS $j => $line)
{
	$selected = '';
	if((GETPOST('active')?GETPOST('active'):$object->active)==$j)$selected = 'selected';
		$optionsac.= '<option value="'.$j.'" '.$selected.' >'.$line.'</option>';
	}
...
// Part to create
	if ($action == 'create')
	{
		print load_fiche_titre($langs->trans("Ctypeitem"));
	...
	//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldentity").'</td><td><input class="flat" type="text" name="entity" value="'.GETPOST('entity').'"></td></tr>';
		print '<tr><td class="fieldrequired">'.$langs->trans("Fieldcode").'</td><td><input class="flat" type="text" name="code" value="'.GETPOST('code').'" maxlength="30"></td></tr>';
	...
	//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td><input class="flat" type="text" name="active" value="'.GETPOST('active').'"></td></tr>';
		...
		print '<tr>';
		print '<td width="20%" class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>';
		print '<select name="active" value="'.$active.'">'.$optionsac.'</select>';;
		print '</td>';
		print '</tr>';
...
// Part to edit record
	...
	//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldentity").'</td><td><input class="flat" type="text" name="entity" value="'.$object->entity.'"></td></tr>';
		print '<tr><td class="fieldrequired">'.$langs->trans("Fieldcode").'</td><td><input class="flat" type="text" name="code" value="'.$object->code.'" maxlength="30"></td></tr>';
	...
	//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td><input class="flat" type="text" name="active" value="'.$object->active.'"></td></tr>';
		...
		print '<tr>';
		print '<td width="20%" class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>';
		print '<select name="active" value="'.$object->active.'">'.$optionsac.'</select>';;
		print '</td>';
		print '</tr>';
...
// Part to show record
	...
	//print '<tr><td class="fieldrequired">'.$langs->trans("Fieldentity").'</td><td>'.$object->entity.'</td></tr>';
		...
		if($object->active==1)
		{
			...
			print '<tr><td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>'.$langs->trans("Yes").'</td></tr>';
		}
		else
		{

			print '<tr><td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>'.$langs->trans("Not").'</td></tr>';
		}
	...
	if (empty($reshook))
	{
		...
		print '<div class="inline-block divButAction"><a class="butAction" href="'.dol_buildpath('/budget/ctypeitem/list.php',1).'">'.$langs->trans("Return").'</a></div>'."\n";
	...
	//$linktoelem = $form->showLinkToObjectBlock($object, null, array('ctypeitem'));
