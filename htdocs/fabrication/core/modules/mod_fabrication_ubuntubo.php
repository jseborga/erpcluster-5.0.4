 ...
 */
class mod_fabrication_ubuntubo extends ModeleNumRefFabrications
{
	var $version='dolibarr';		// 'development', 'experimental', 'dolibarr'
	var $prefixinvoice='PR';
	...
	function canBeActivated()
	{
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."fabrication";
	...
	function getNextValue($objsoc,$facture,$mode='next')
	{
		...
		$resql=$db->query($sql);
		dol_syslog("mod_fabrication_ubuntubo::getNextValue sql=".$sql);
		if ($resql)
		{
		...
		{
			dol_syslog("mod_fabrication_ubuntubo::getNextValue sql=".$sql, LOG_ERR);
			return -1;
		}
		...
		if (empty($date)) $date = date("Y-m-d");
    		...
    		dol_syslog("mod_fabrication_ubuntubo::getNextValue return ".$prefix.$yymm."-".$num);
