...
abstract class ModeleSalary extends CommonDocGenerator
{
	var $error='';
 ...
 */
abstract class ModeleNumRefSalary
{
	var $error='';
 ...
 */
function salary_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false)
{
  ...
  // Positionne le modele sur le nom du modele a utiliser
	...
	{
		if (! empty($conf->global->SALARY_ADDON_PDF))
		{
			$modele = $conf->global->SALARY_ADDON_PDF;
		}
		else
		{
			$modele = 'crabe';
  ...
  // Search template files
	...
	foreach($dirmodels as $reldir)
	{
	  ...
	  // On verifie l'emplacement du modele
			$file=dol_buildpath($reldir."salary/core/modules/doc/".$file,0);
			if (file_exists($file))
			{
				$filefound=1;
  ...
  // Charge le modele
	if ($filefound)
	{
	  ...
	  // Fin appel triggers
		...
		{
			$outputlangs->charset_output=$sav_charset_output;
			dol_print_error($db,"salary_pdf_create Error: ".$obj->error);
			return -1;
		}
	}
	else
 ...
 *	@return int        					<0 if KO, >0 if OK
 */
function boleta_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false,$aList=array())
{
	global $conf,$user,$langs;
	$langs->load("bills");
  ...
  // Charge le modele
	if ($filefound)
	{
	  ...
	  // output format that does not support UTF8.
		$sav_charset_output=$outputlangs->charset_output;
		if ($obj->write_file($object, $outputlangs, $srctemplatepath, $hidedetails, $hidedesc, $hideref, $hookmanager,$aList) > 0)
		{
			$outputlangs->charset_output=$sav_charset_output;
	  ...
	  // Appel des triggers
			include_once DOL_DOCUMENT_ROOT . '/core/class/interfaces.class.php';
			$interface=new Interfaces($db);
			$result=$interface->run_triggers('SALARY_BOLETADOC',$object,$user,$langs,$conf);
