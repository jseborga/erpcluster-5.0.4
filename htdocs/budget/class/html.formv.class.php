...
class Formv extends Form
{
	 ...
	 *  @return		void
	 */
	function select_produits_v($selected='', $htmlname='productid', $filtertype='', $limit=20, $price_level=0, $status=1, $finished=2, $selected_input_value='', $hidelabel=0, $ajaxoptions=array(),$socid=0,$action='',$element='product')
	{
		global $langs,$conf;

		include_once(DOL_DOCUMENT_ROOT.'/budget/tpl/frames.tpl.php');
		...
		if (! empty($conf->use_javascript_ajax) && ! empty($conf->global->PRODUIT_USE_SEARCH_TO_SELECT))
		{
			...
			print ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.'/budget/ajax/products.php', $urloption, $conf->global->PRODUIT_USE_SEARCH_TO_SELECT, 0, $ajaxoptions);
	 ...
	 *  @return		void
	 */
    function select_items_v($selected='', $htmlname='productid', $filtertype='', $limit=20, $price_level=0, $status=1, $finished=2, $selected_input_value='', $hidelabel=0, $ajaxoptions=array(),$socid=0,$action='')
    {
    	global $langs,$conf;
    	...
    	if (! empty($conf->use_javascript_ajax) && ! empty($conf->global->PRODUIT_USE_SEARCH_TO_SELECT))
    	{
			...
			// mode=1 means customers products
    		$urloption='htmlname='.$htmlname.'&outjson=1&price_level='.$price_level.'&type='.$filtertype.'&mode=1&status='.$status.'&finished='.$finished.'&selected='.$selected;
			//Price by customer
    		if (! empty($conf->global->PRODUIT_CUSTOMER_PRICES) && !empty($socid)) {
    			$urloption.='&socid='.$socid;
    		}
    	...
    	{
    		print $this->select_items_list_v($selected,$htmlname,$filtertype,$limit,$price_level,'',$status,$finished,0,$socid);
    	}
    }	
     ...
     *  @return     array    				Array of keys for json
     */
    function select_items_list_v($selected='',$htmlname='itemid',$filtertype='',$limit=20,$price_level=0,$filterkey='',$status=1,$finished=2,$outputmode=0,$socid=0)
    {
    	global $langs,$conf,$user,$db;
    ...
    private function constructProductListOption_v(&$objp, &$opt, &$optJson, $price_level, $selected)
    {
    	...
    	if ($objp->budget) $opt.= $langs->trans('Proyectresource').' -> ';
    	$opt.= $objp->ref.' - '.dol_trunc($label,32).' - ';
    	$opt.= $objp->description.' - ';

    	$objRef = $objp->ref;
    	if (! empty($filterkey) && $filterkey != '') $objRef=preg_replace('/('.preg_quote($filterkey).')/i','<strong>$1</strong>',$objRef,1);
    	if ($objp->budget) $outval.= $langs->trans('Proyectresource').' -> ';
