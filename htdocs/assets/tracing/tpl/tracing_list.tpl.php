...
$search_fk_user_resp=GETPOST('search_fk_user_resp','alpha');
$search_been=GETPOST('search_been','alpha');
$search_description=GETPOST('search_description','alpha');
$search_fk_user_create=GETPOST('search_fk_user_create','int');
$search_fk_user_mod=GETPOST('search_fk_user_mod','int');
$search_dater=GETPOST('search_fk_user_mod','alpha');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'assetstracing_list';
...
't.fk_user_resp'=>array('label'=>$langs->trans("Fieldfk_user_resp"), 'checked'=>1),
...
't.dater'=>array('label'=>$langs->trans("Fielddater"), 'checked'=>1),
...
$objectTracing=new Assetstracing($db);
if (($id > 0 || ! empty($ref)) && $action != 'add')
{
	//$result=$objectTracing->fetch($id,$ref);
...
$parameters=array();
$reshook=$hookmanager->executeHooks('doActions',$parameters,$objectTracing,$action);    // Note that $action and $object may have been modified by some hooks
...
//echo "TPL ID : ".$idAux."<br>";
$form=new Form($db);

//$help_url="EN:Module_Customers_Orders|FR:Module_Commandes_Clients|ES:Módulo_Pedidos_de_clientes";
$help_url='';
$title = $langs->trans('Tracinglist');
...
$sql.= " FROM ".MAIN_DB_PREFIX."assets_tracing as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."c_assets_been as b ON b.rowid = t.been";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."adherent as a ON a.rowid = t.fk_user_resp";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."assets_tracing_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
$sql.= " AND t.fk_asset = ".$id;
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_asset) $sql.= natural_search("fk_asset",$search_fk_asset);
if ($search_fk_user_resp) $sql.= natural_search("a.lastname",$search_fk_user_resp);
if ($search_been) $sql.= natural_search("b.label",$search_been);
...
if ($search_dater) $sql.= natural_search("dater",$search_dater);
...
if (! empty($arrayfields['t.fk_user_resp']['checked'])) print_liste_field_titre($arrayfields['t.fk_user_resp']['label'],$_SERVER['PHP_SELF'],'t.fk_user_resp','',$params,'',$sortfield,$sortorder);
...
if($resBeen){
	$opcBeen .="<option value=-1 selected></option>";
	foreach ($objCassetsbeen->lines as $key => $value) {

		if($value->label == $search_been){
			$opcBeen .="<option value=".$value->label." selected>".$value->label."</option>";
		}else{
			$opcBeen .="<option value=".$value->label." >".$value->label."</option>";
		}
	}
}
print '<tr class="liste_titre">';
...
if (! empty($arrayfields['t.fk_user_resp']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_user_resp" value="'.$search_fk_user_resp.'" size="10"></td>';
...
if (! empty($arrayfields['t.been']['checked'])) print '<td class="liste_titre"><select name="search_been">'.$opcBeen.'</select></td>';
...
while ($i < min($num, $limit))
{
	...
	{
		$var = !$var;
		$objectAsstra->status = $obj->status;
		...
		foreach ($arrayfields as $key => $value)
		{
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'ref')
				{
					$objectTracing->id = $obj->rowid;
					$objectTracing->ref = $obj->ref;
					$objectTracing->label = $obj->label;
					$obj->$key2 = $objectTracing->getNomUrl();
					$aLineas[$i]['ref'] = $obj->ref;
				}
				if ($key2 == 'fk_user_resp'){
					$obj->$key2 = $obj->lastname.' '.$obj->firstname;
					$aLineas[$i]['nombre'] = $obj->lastname.' '.$obj->firstname;
				}
				...
				if ($key2 == 'dater'){
					$obj->$key2 = dol_print_date($db->jdate($obj->dater),'day');
					$aLineas[$i]['fecha'] = $obj->dater;
				}
				...
				{
					$act = 'validate';
					if ($obj->status) $act = 'novalidate';
					$obj->$key2 = '<a href="'.$_SERVER["PHP_SELF"].'?idr='.$obj->rowid.'&amp;action='.$act.'&amp;id='.$id.'">'.$objectAsstra->getLibStatut(6).'</a>';
					$aLineas[$i]['estado'] = $objectAsstra->getLibStatut(0);
				}
		...
		print '<td class="nowrap">';
		//print '<div class="class="inline-block"><a class="" href="'.$_SERVER["PHP_SELF"].'?idEdit='.$obj->rowid.'&amp;action=edit &amp;view=3 &amp;id='.$idAux.'">'.img_picto('','edit').'</a>';
		if ($user->rights->assets->tra->write)
			print '<div class="class="inline-block"><a class="" href="'.$_SERVER["PHP_SELF"].'?idEdit='.$obj->rowid.'&amp;action=edit&amp;id='.$id.'">'.img_picto('','edit').'</a>';
		if ($user->rights->assets->tra->del)
			print '<a class="" href="'.$_SERVER["PHP_SELF"].'?idr='.$obj->rowid.'&amp;action=delete&amp;id='.$id.'">'.img_picto('','delete').'</a></div>';
		print '</td>';
...
$_SESSION['idtpl'] = serialize($id);

print '<div class="tabsAction">'."\n";
if ($user->rights->assets->tra->write)
	print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&action=create&view=2">'.$langs->trans("Createnew").'</a></div>'."\n";
...
print '</div>';
 ...
 $aReporte = array(1=>$aLineas, 2=>$aCabecera);
 ...
 $filename='assets/'.$period_year.'/tracing/'.$aCabecera['ref'];
 $filedir=$conf->assets->dir_output.'/assets/'.$period_year.'/tracing/'.$aCabecera['ref'];
 $modelpdf = "tracing";
