...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield="t.sequen"; // Set here default search field
...
//if ($type=='copiesplane')
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'budgetincidents'.$type.'tpllist';
...
$sql.= " FROM ".MAIN_DB_PREFIX."budget_incidents_det as t";
...
$sql.= " WHERE 1 = 1";
$sql.= " AND t.fk_budget_incident = ".$idr;
if (empty($typetwo))
	$sql.= " AND t.type = '".$type."'";
else
...
if ((!empty($typetwo) && $subaction == $typetwo && $action == 'createtwo') || (empty($typetwo) && empty($subaction) && $action=='createline'))
{
	$var = !$var;
	...
	foreach ($arrayfields as $key => $value) {
		if (!empty($arrayfields[$key]['checked'])) {
			...
			elseif ($key2=='sequen')
			{
				...
				$filter = " AND t.fk_incident = ".$objBudgetincidents->id;
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		// Show here line of result
		print '<tr '.$bc[$var].'>';
		if ($action == 'editline' && $idrd== $obj->rowid)
		{
			// LIST_OF_TD_FIELDS_LIST
			$lAddbutton=true;
			foreach ($arrayfields as $key => $value) {
			...
			foreach ($arrayfields as $key => $value) {
				...
				//$key2 = str_replace('t.', '', $key);
					...
					if ($key2 == 'ref')
					{
						...
						$obj->$key2 = $objBudgetincidents->getNomUrl();
					}
					if ($key2 == 'active')
					{
						$img = 'switch_off';
					...
					if ($key2=='value_one' || $key2=='value_two' || $key2=='value_three'|| $key2=='value_four'|| $key2=='value_five' || $key2=='value_six' || $key2=='value_seven')
					{
						...
						//calculo ponderado
							...
							if($objBudgetincidents->status==0 && $lUpdate)
							{
						...
						{
							if ($objBudgetincidents->cost_direct>0)
								$obj->$key2 = $obj->value_one * $obj->value_two / $objBudgetincidents->cost_direct*100;
							//vamos a actualziar
							if ($objBudgetincidents->status==0 && $lUpdate)
							{
								//vamos a actualizar los valores
						...
						elseif($type=='propossal'|| $type=='legaldoc')
						{
							...
							if($key2=='value_five')
							{
								$obj->$key2 = ($obj->value_one * $obj->value_two * $obj->value_four)/$objBudgetincidents->cost_direct*100;
								//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
									//vamos a actualizar los valores
						...
						elseif($type=='guarantees')
						{
							if ($key2=='value_two')
							{
								$obj->$key2 = $obj->value_one * $objBudgetincidents->cost_direct;
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
							...
							if ($key2=='value_four')
							{
								$obj->$key2 = $obj->value_one * $objBudgetincidents->cost_direct * $obj->value_three * $objBudgetincidents->commission;
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
							...
							if ($key2=='value_five')
							{
								$obj->$key2 = price2num(($obj->value_one * $objBudgetincidents->cost_direct * $obj->value_three * $objBudgetincidents->commission)/$objBudgetincidents->cost_direct*100,5);
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
						...
						elseif($type=='operation' || $type=='administrative')
						{
							...
							if ($key2=='value_four')
							{
								$obj->$key2 = $obj->value_one * $obj->value_two*$objBudgetincidents->time_duration;
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
							...
							if ($key2=='value_five')
							{
								$obj->$key2 = $obj->value_one * $obj->value_two*$objBudgetincidents->time_duration/$objBudgetincidents->cost_direct*100;
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
					...
					//elseif($type=="mobilization")
						elseif($type=="mobilization" || $type=='traffic' || $type=='risk')
						{
							...
							if ($key2=='value_three')
							{
								$obj->$key2 = $obj->value_one * 12 * $objBudgetincidents->time_duration;
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
							...
							if ($key2=='value_four')
							{
								$obj->$key2 = $obj->value_one * 12 * $objBudgetincidents->time_duration / $objBudgetincidents->cost_direct * 100;
							//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
								//vamos a actualizar los valores
						...
						elseif($type=="costmo")
						{
							...
							if ($key2=='value_seven')
							{
								if ($objBudgetincidents->exchange_rate>0)
									$obj->$key2 = ((($aPoliticsalarymonth[$obj->label]+($aPoliticsalarymonth[$obj->label]*$nBenesoc/100)+$nOccupational)+($aPoliticsalarymonth[$obj->label]+($aPoliticsalarymonth[$obj->label]*$nBenesoc/100)+$nOccupational)*(13/(100-13)))/(25*8))/$objBudgetincidents->exchange_rate;
								//vamos a actualziar
								if ($objBudgetincidents->status==0 && $lUpdate)
								{
									//vamos a actualizar los valores
					...
					if ($key2=='quantity')
					{
						...
						{
							$align='right';
							$obj->$key2 = $objBudgetincidents->njobs*$obj->value_one/100;
							$obj->$key2 = price2num($obj->$key2,'MT');
							$aTotal[$key2]+=$obj->$key2;
						}
					}
	...
	if (empty($action))
	{
		...
		print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$obj->rowid.'&action=deleteline">'.img_picto('','delete').'</a>';
	}
	print '</td>';
	if (! $i) $totalarray['nbfield']++;
...
if ($type=='antiquity')
{
	...
	if ($objBudgetincidents->ponderation>0) $incident=$amountanual / $objBudgetincidents->ponderation;
	$obj = new Stdclass();
			// Show here line of result
	$var = !$var;
	print '<tr '.$bc[$var].'>';
...
{
	if (empty($action))
		print '<a class="butAction" href="' . dol_buildpath('/budget/budget/incidents.php',1).'?id='.$object->id.'&idr='.$idr.'&type='.$type.'&action='.($typetwo?'createtwo':'createline').''.($typetwo?'&subaction='.$typetwo:'').'">' . $langs->trans('New') . '</a>';
}
print '</div>';