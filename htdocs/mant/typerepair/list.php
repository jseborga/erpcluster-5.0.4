...
/**
 *   	\file       mant/mtyperepair_list.php
 *		\ingroup    mant
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'manttyperepairlist';
...
$arrayfields=array(
	...
	't.fk_type_mant'=>array('label'=>$langs->trans("Fieldfk_type_mant"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."m_type_repair as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."m_type_repair_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_type_mant) $sql.= natural_search("fk_type_mant",$search_fk_type_mant);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/mtyperepair/card.php?id='.$id);
	exit;
}
...
if ($search_field1 != '') $param.= '&amp;search_field1='.urlenref($search_field1);
if ($search_field2 != '') $param.= '&amp;search_field2='.urlenref($search_field2);
...
if (! empty($arrayfields['t.fk_type_mant']['checked'])) print_liste_field_titre($arrayfields['t.fk_type_mant']['label'],$_SERVER['PHP_SELF'],'t.fk_type_mant','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_type_mant']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_type_mant" value="'.$search_fk_type_mant.'" size="10"></td>';
...
if (empty($reshook))
{
	...
	{
		print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/mant/typerepair/card.php?action=create">'.$langs->trans("New").'</a></div>'."\n";
	}
}
print '</div>'."\n";
