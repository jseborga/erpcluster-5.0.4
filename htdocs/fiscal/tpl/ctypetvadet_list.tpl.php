...
/**
 *   	\file       fiscal/ctypetvadet_list.php
 *		\ingroup    fiscal
...
$arrayfields=array(
	...
	't.type'=>array('label'=>$langs->trans("Principal"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_type_tvadet as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_type_tvadet_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE t.fk_type_tva = ".$id;
...
if ($abc && $action != 'create' && $action != 'edit')
{
// Fields title search
	print '<tr class="liste_titre">';
//
	...
	if ($obj)
	{
		...
		// Show here line of result
    		...
    		if ($action == 'edit' && $idr == $obj->rowid)
    		{
    			...
    			{
    				print '<td>'.$form->selectyesno('type',$obj->type,1).'</td>';
    				if (! $i) $totalarray['nbfield']++;
    			}
    			if (! empty($arrayfields['t.status']['checked']))
    			{
    				print '<td>'.$form->selectyesno('status',$obj->status,1).'</td>';
    				if (! $i) $totalarray['nbfield']++;
    			}
...
if ($action == 'create')
{
	...
	if (! empty($arrayfields['t.type']['checked']))
	{
		if ($lPrincipal)
		{
			print '<td>'.$form->selectyesno('type',GETPOST('type'),1).'</td>';
		}
		else
		{
