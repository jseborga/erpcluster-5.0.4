...
if ($result)
  {
    ...
    //verificamos la version pendiente de aprobacion
    $objrefo->fetch_version($gestion);
    if ($obj_ref->gestion == $gestion)
      {
	$lVersion = true;
	$nVersion = $obj_ref->version;
...
<?php
	...
	if ($numCol[7]==true)
	  {
	    print '<div id="amount" class="left title">';
	    print $langs->trans("Approved budget").' '.$nVersionAp;
	    print '</div>';
	  }
	else
	  {
	...
	$aHtml = array();
		...
		//reformulado pendiente
		$nReform = $aOf[$obj->fk_structure][$obj->id][$obj->partida];
	    	print '<div id="amount" '.$newClase.'">'.($nPresup>0?price(price2num($nReform/$nPresup*100,'MT')):'').'</div>';
	      }
