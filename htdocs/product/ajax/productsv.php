...
$stockcero = GETPOST('stockcero','int');
/*
 * View
 */
...
if (! empty($action) && $action == 'fetch' && ! empty($id))
{
	...
	if ($ret > 0) {
		...
		$outjson = array('ref' => $outref,'label' => $outlabel,'desc' => $outdesc,'type' => $outtype,'unit'=>$unit,'price_ht' => $outprice_ht,'price_ttc' => $outprice_ttc,'pricebasetype' => $outpricebasetype,'tva_tx' => $outtva_tx,'qty' => $outqty,'discount' => $outdiscount);
	}
	...
	if (empty($mode) || $mode == 1) {
		$arrayresult = $form->select_produits_list_v("", $htmlname, $type, "", $price_level, $searchkey, $status, 2, $outjson, $socid,$fk_entrepot,$stockcero);
	} elseif ($mode == 2) {
