...
function createThirdParty($authentication,$thirdparty)
{
	...
	if (! $error)
	{
		...
		//vamos a codificar segun la seleccon
	   // Load object modCodeTiers
		...
		//$tmpcodecli=$object->code_client;
		if (empty($tmpcodec) && ! empty($modCodeClient->code_auto)) $tmpcodec=$modCodeClient->getNextValue($newobject,0);

			//$tmpcodef=$object->code_fournisseur;
		if (empty($tmpcodef) && ! empty($modCodeFournisseur->code_auto)) $tmpcodef=$modCodeFournisseur->getNextValue($newobject,1);
		...
		$newobject->code_client=$tmpcodec;
		$newobject->code_fournisseur=$tmpcodef;
...
function getListOfThirdParties($authentication,$filterthirdparty)
{
	...
	if (! $error)
	{
		$sql  = "SELECT s.rowid as socRowid, s.nom as ref, s.ref_ext, s.address, s.zip, s.town, c.label as country, s.tva_intra, s.phone, s.fax, s.url, extra.*";
