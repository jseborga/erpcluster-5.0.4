...
/**
 * \file    budget/budgettaskproduct.class.php
 * \ingroup budget
...
/**
 * Class Budgettaskproduct
 *
 * Put here description of your class
 *
 * @see CommonObject
 */
class Budgettaskproduct extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'budgettaskproduct';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'budget_task_product';

	/**
	 * @var BudgettaskproductLine[] Lines
	 */
	public $lines = array();
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("budgettaskproduct", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new BudgettaskproductLine();
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Budgettaskproduct($this->db);
