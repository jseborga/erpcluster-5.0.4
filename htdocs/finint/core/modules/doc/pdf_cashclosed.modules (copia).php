 ...
 */
class pdf_acashclosed extends ModelePDFRequest
{
	var $db;
	var $name;
	var $um;
  ...
  // Objet societe qui emet
	...
	*/
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Cashclosed";
		$this->description = $langs->trans('PDFCashclosedDescription');
	...
	*/
	...
	// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		...
		if ($conf->request->dir_output)
		{
			...
			//recuperamos la informacion line
			$object->fetch_lines_io();
			...
			else
			{
				...
				$dir = $conf->request->dir_output . "/" . $objectref;
				$file = $dir . "/" . $objectref .'_end'. ".pdf";
			}
			if (! file_exists($dir))
			{
				if (dol_mkdir($dir) < 0)
			...
			if (file_exists($dir))
			{
				...
				$objrcd = new Requestcashdeplacementext($db);
				$nblignes = count($object->linesio);
				...
				// column titles
				$header = array($outputlangs->transnoentities("Date"), $outputlangs->transnoentities("Detail"), $outputlangs->transnoentities("Qty"), $outputlangs->transnoentities("Unit"),$outputlangs->transnoentities("Input"),$outputlangs->transnoentities("Output"),$outputlangs->transnoentities("Balance"));

				$this->ColoredTable($pdf, $header,$object->linesio);

				// Loop on each lines entradas
				$nbligness = 0;
				for ($i = 0; $i < $nbligness; $i++)
				{
					...
					//date
					$date = dol_print_date($object->linesio[$i]->dateo,'day');
					$pdf->SetXY($this->posxdate, $curY);
					$pdf->MultiCell($this->posxdate+2, 0, $date, 0, 'C',0); 
					// detail
					$desc = $object->linesio[$i]->detail;
					$pdf->SetXY($this->posxdet, $curY);
					$pdf->MultiCell($this->posxdet+47, 0, $desc."\n", 0, 'J',0,0); 
					//doc
					$doc= 'vac '.$object->linesio[$i]->fk_bank;
					if ( $object->linesio[$i]->fk_bank)
					{
						$objbank->fetch($object->linesio[$i]->fk_bank);
						$doc = $objbank->num_chq;
					}
					$pdf->SetXY($this->posxdoc, $curY);
					$pdf->MultiCell($this->posxdoc-1, 0, $doc, 0, 'L',0); 
					// qty
					$qty = price2num($object->linesio[$i]->quant);
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->posxqty-1, 0, $qty, 0, 'L',0); 

					//uni
					$idr = $object->linesdet[$i]->id;
					$objrcd->fetch($idr);
					$unit = $objrcd->getLabelOfUnit();
					$pdf->SetXY($this->posxuni, $curY);
					$pdf->MultiCell($this->posxuni-1, 0, $unit, 0, 'L',0); 

					//ing
					$ing = 0;
					if ($object->linesio[$i]->concept == 'banktransfert')
						$ing = price2num($object->linesio[$i]->amount);
					$pdf->SetXY($this->posxinp, $curY);
					$pdf->MultiCell($this->posxinp-1, 0, $ing, 0, 'R',0); 

					//out
					$out = 0;
					if ($object->linesio[$i]->concept == 'deplacement')
						$out = price2num($object->linesio[$i]->amount);
					$pdf->SetXY($this->posxout, $curY);
					$pdf->MultiCell($this->posxout-1, 0, $out, 0, 'R',0); 
					...
					$pdf->SetXY($this->posxdet, $curY);
					$pdf->MultiCell($this->posxdet-1, 0, $desc, 0, 'L',0); 

					// total
					$app = price(price2num($saldo,'MT'));
					$pdf->SetXY($this->posxtot, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxtot, 0, $app, 0, 'R'); 
...
//exit;
	...
	// Rect prend une longueur en 3eme param et 4eme param
  		...
  		if (empty($hidetop))
  		{
			...
			// line prend une position y en 2eme param et 4eme param
  			...
  			$pdf->MultiCell($this->posxdate-1,2, $outputlangs->transnoentities("Date"),'','C');
  		}
  		//detail
  		...
  		{	
  			...
  			$pdf->MultiCell($this->posxdet-1,2, $outputlangs->transnoentities("Detail"),'','L');
  		}
  		//doc
  		...
  		{	
  			...
  			$pdf->MultiCell($this->posxdoc-1,2, $outputlangs->transnoentities("Doc"),'','L');
  		}
  		//qty
  		...
  		{	
  			...
  			$pdf->MultiCell($this->posxqty-1,2, $outputlangs->transnoentities("Qty"),'','L');
  		}
  		//unid
  		...
  		{	
  			...
  			$pdf->MultiCell($this->posxing-1,2, $outputlangs->transnoentities("Input"),'','L');
  		}
  		//output
  		...
  		{	
  			...
  			$pdf->MultiCell($this->posxout-1,2, $outputlangs->transnoentities("Output"),'','L');
  		}
  		...
  		if (empty($hidetop))
  		{
  			...
  			$pdf->MultiCell($this->posxtot,2, $outputlangs->transnoentities("Balance"),'','C');
  		}
  	}
   ...
   *      @return	int								Return height of bottom margin including footer text
	...
	public function ColoredTable(&$pdf,$header,$data) 
	{
        ...
        // Header
		$w = array(40, 35, 40, 45);
		$w = array($this->posxdate, $this->det, $this->doc, $this->qty,$this->uni,$this->inp,$this->out, $this->tot);
		...
		{
			$pdf->Cell($this->posxdate, 6, dol_print_date($data[$i]->dateo,'day'), 'LR', 0, 'L', $fill);
			$pdf->Cell($this->det, 6, $data[$i]->detail, 'LR', 0, 'L', $fill);
			$pdf->Cell($this->doc, 6, $data[$i]->nro_chq, 'LR', 0, 'L', $fill);
			$pdf->Cell($this->qty, 6, $data[$i]->quant, 'LR', 0, 'L', $fill);
			$pdf->Cell($this->uni, 6, $data[$i]->fk_unit, 'LR', 0, 'L', $fill);
			$pdf->Cell($this->inp, 6, $data[$i]->input, 'LR', 0, 'L', $fill);
			$pdf->Cell($this->out, 6, $data[$i]->output, 'LR', 0, 'L', $fill);
			$pdf->Cell($this->tot, 6, $data[$i]->total, 'LR', 0, 'L', $fill);
