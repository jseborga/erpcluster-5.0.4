...
/**
 *   	\file       budget/incidents_list.php
 *		\ingroup    budget
...
if (!$user->rights->budget->tea->leer)
	accessforbidden();

// Initialize technical object to manage context to save list fields
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'budgetincidentslist';
...
$arrayfields=array(
	...
	't.fk_region'=>array('label'=>$langs->trans("Fieldfk_region"), 'align'=>'align="left"', 'checked'=>1),
	't.incident'=>array('label'=>$langs->trans("Fieldincident"), 'align'=>'align="left"', 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."incidents as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."c_region_geographic as r ON t.fk_region = r.rowid";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."incidents_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/incidents/card.php?id='.$id);
	exit;
}
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'fk_region')
				{
					$resreg = $objCregiongeographic->fetch($obj->fk_region);
					if ($resreg==1)
...
{
	print '<a class="butAction" href="' . dol_buildpath('/budget/incidents/card.php',1).'?action=create">' . $langs->trans('New') . '</a>';
}
print '</div>';
