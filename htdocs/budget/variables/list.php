...
/**
 *   	\file       budget/puvariables_list.php
 *		\ingroup    budget
...
if (empty($reshook))
{
	...
	//$permtoread = $user->rights->puvariables->read;
	//$permtodelete = $user->rights->puvariables->delete;
	//$uploaddir = $conf->puvariables->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."pu_variables as t";
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pu_variables_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_unit) $sql.= natural_search(array("t.fk_unit","c.label"),$search_fk_unit);
if ($search_fk_user_create) $sql.= natural_search(array("t.fk_user_create","u.firstname","u.lastname"),$search_fk_user_create);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/puvariables/card.php?id='.$id);
	exit;
}
...
if ($search_fk_user_create != '') $param.= '&amp;search_fk_user_create='.urlencode($search_fk_user_create);
if ($search_fk_user_mod != '') $param.= '&amp;search_fk_user_mod='.urlencode($search_fk_user_mod);
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'status')
				{
					...
					$obj->$key2 = $object->getLibStatut(6);
				}
				...
				if ($key2 == 'fk_unit')
				{
					$obj->$key2 = $obj->labelunit;
				}

				print '<td '.(!empty($arrayfields[$key]['align'])?$arrayfields[$key]['align']:'').'>' . $obj->$key2 . '</td>';
				if (!$i)
					$totalarray['nbfield'] ++;
			}
		}
...
print '<a class="butAction" href="' . dol_buildpath('/budget/variables/card.php',1).'?action=create">' . $langs->trans('Create') . '</a>';
print '</div>';
