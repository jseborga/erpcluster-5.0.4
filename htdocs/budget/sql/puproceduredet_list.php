...
/**
 *   	\file       priceunits/puproceduredet_list.php
 *		\ingroup    priceunits
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2016-10-24 10:25
 */
...
dol_include_once('/priceunits/class/puproceduredet.class.php');
...
$search_fk_pu_procedure=GETPOST('search_fk_pu_procedure','int');
$search_sequen=GETPOST('search_sequen','int');
$search_ref_procedure=GETPOST('search_ref_procedure','alpha');
...
$hookmanager->initHooks(array('puproceduredetlist'));
...
't.fk_pu_procedure'=>array('label'=>$langs->trans("Fieldfk_pu_procedure"), 'checked'=>1),
't.sequen'=>array('label'=>$langs->trans("Fieldsequen"), 'checked'=>1),
't.ref_procedure'=>array('label'=>$langs->trans("Fieldref_procedure"), 'checked'=>1),
...
$search_fk_pu_procedure='';
$search_sequen='';
$search_ref_procedure='';
$search_detail='';
$search_formula='';
...
$sql.= " FROM ".MAIN_DB_PREFIX."pu_procedure_det as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pu_procedure_det_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_pu_procedure) $sql.= natural_search("fk_pu_procedure",$search_fk_pu_procedure);
if ($search_sequen) $sql.= natural_search("sequen",$search_sequen);
if ($search_ref_procedure) $sql.= natural_search("ref_procedure",$search_ref_procedure);
...
if ($search_fk_pu_procedure != '') $params.= '&amp;search_fk_pu_procedure='.urlencode($search_fk_pu_procedure);
if ($search_sequen != '') $params.= '&amp;search_sequen='.urlencode($search_sequen);
if ($search_ref_procedure != '') $params.= '&amp;search_ref_procedure='.urlencode($search_ref_procedure);
...
if ($search_status != '') $params.= '&amp;search_status='.urlencode($search_status);
    ...
    // 
if (! empty($arrayfields['t.fk_pu_procedure']['checked'])) print_liste_field_titre($arrayfields['t.fk_pu_procedure']['label'],$_SERVER['PHP_SELF'],'t.fk_pu_procedure','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.ref_procedure']['checked'])) print_liste_field_titre($arrayfields['t.ref_procedure']['label'],$_SERVER['PHP_SELF'],'t.ref_procedure','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.status']['checked'])) print_liste_field_titre($arrayfields['t.status']['label'],$_SERVER['PHP_SELF'],'t.status','',$params,'',$sortfield,$sortorder);
	...
	print '<tr class="liste_titre">';
	// 
if (! empty($arrayfields['t.fk_pu_procedure']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_pu_procedure" value="'.$search_fk_pu_procedure.'" size="10"></td>';
...
if (! empty($arrayfields['t.ref_procedure']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_ref_procedure" value="'.$search_ref_procedure.'" size="10"></td>';
