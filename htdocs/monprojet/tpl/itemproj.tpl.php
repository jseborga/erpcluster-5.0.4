...
$arrayfields=array(
	...
	't.country_id'=>array('label'=>$langs->trans("Fieldfk_country"), 'checked'=>1),
...
$filterstatic.= " AND t.fk_projet = 0";
...
$head = monprojetgroup_prepare_head($object);
dol_fiche_head($head, $tab, $langs->trans("Groups"),0,($object->public?'projectpub':'project'));
...
$filteradd.= " AND t.fk_task_parent = ".$tasksel;
$res = $taskadd->fetchAll($sortorder='', $sortfield='', 0, 0, $filter, 'AND',$filterstatic,false,$filteradd);
...
if ($res>0 || $lSearch)
{
	...
	$params='&amp;id='.$object->id.'&amp;action=supplies';
	...
	print '<input type="hidden" name="action" value="supplies">';
	...
	//	while ($i < min($num, $limit))
	{
		if ($obj)
		{
			...
			if (! empty($arrayfields['t.country_id']['checked'])) 
			{
				print '<td>'.getCountry($obj->country_id,0,$db).'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			...
			if (! empty($arrayfields['t.type']['checked'])) 
			{
            	...
            	print '<td>';
                print $form->editfieldval("Type",'fk_product_type',$obj->type,$obj,$user->rights->produit->creer||$user->rights->service->creer,$typeformat);
                print '</td>';
