...
/**
 *   	\file       productext/productbonus_list.php
 *		\ingroup    productext
...
't.fk_bonus_type'=>array('label'=>$langs->trans("Fieldfk_bonus_type"), 'align'=>'align="left"', 'checked'=>1),
...
$help_url='';
$title = $langs->trans('Productbonuslist');
...
$sql.= " FROM ".MAIN_DB_PREFIX."product_bonus as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."product_bonus_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_bonus_type) $sql.= natural_search("fk_bonus_type",$search_fk_bonus_type);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/productbonus/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.fk_bonus_type']['checked'])) print_liste_field_titre($arrayfields['t.fk_bonus_type']['label'],$_SERVER['PHP_SELF'],'t.fk_bonus_type','',$params,$arrayfields['t.fk_bonus_type']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_bonus_type']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_bonus_type']['align'].'><input type="text" class="flat" name="search_fk_bonus_type" value="'.$search_fk_bonus_type.'" size="10"></td>';
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'ref')
				{
					$objProductbonus->fk_product = $obj->fk_product;
					$objProductbonus->id = $obj->rowid;
					$objProductbonus->ref = $obj->ref;
					$objProductbonus->label = $obj->label;
					$obj->$key2 = $objProductbonus->getNomUrl();
				}
				if ($key2 == 'active')
				{
					$img = 'switch_off';
