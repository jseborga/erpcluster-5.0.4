...
/** Descripcion
 *	El siguiente modelo se refiere a la plantilla de facturas
 */
...
class pdf_fractal extends ModelePDFFactures
{
	var $db;
	var $name;
	var $um;
  ...
  // Objet societe qui emet
	...
	*/
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i=0; $i < count($objFactura->lines) ; $i++) {
					...
					/*CONSULTA PARA BUSCAR EL CODIGO*/
					$idPro = $objFactura->lines[$i]->fk_product;
					if(!empty($idPro)){
						$rP = $objProduct->fetch($idPro);
						if($rP > 0){
							$codProduc = $objProduct->ref;
						}else{
							$codProduc ="NN";
						}
					}else{
					...
					$pdf->MultiCell($this->posxcon-$this->posxini-1,2, $codProduc,'','L');

					//Cantidad
					$pdf->SetXY($this->posxcodi-1, $posy);
					$pdf->MultiCell($this->posxconc-$this->posxcodi-1,2, $objFactura->lines[$i]->qty,'','C');

					$description = '';
					if ($objFactura->lines[$i]->fk_product>0)
					{
						$objProduct->fetch($objFactura->lines[$i]->fk_product);
						$description = $objProduct->label;
					}
					if (!empty($objFactura->lines[$i]->description)) $description.= (!empty($description)?' : ':'').$objFactura->lines[$i]->description;

					//Detalle
					$pdf->SetXY($this->posxconc-1, $posy);
					$pdf->MultiCell($this->posxpreu-$this->posxconc-1,2, $description,'','L');
					$nexYdesc = $pdf->GetY();

					//Precio Unitario
					$pdf->SetXY($this->posxpreu-1, $posy);
					$pdf->MultiCell($this->posxtota-$this->posxpreu-1,2, price($objFactura->lines[$i]->price),'','R');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
	 ...
	 // Output Rect
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxcodi-$this->posxini-1,2, $outputlangs->transnoentities("Code"),'','C');
			/*Linea de la columna*/
			$pdf->line($this->posxcodi-1, 76, $this->posxcodi-1, $tab_height+76);

			$pdf->SetXY($this->posxcodi-1, 77);
			$pdf->MultiCell($this->posxconc-$this->posxcodi-1,2, $outputlangs->transnoentities("Cantidad"),'','C');
			/*Linea de la columna*/
			$pdf->line($this->posxconc-1, 76, $this->posxconc-1, $tab_height+76);

			$pdf->SetXY($this->posxconc-1, 77);
			$pdf->MultiCell($this->posxpreu-$this->posxconc-1,2, $outputlangs->transnoentities("Detail"),'','C');
			/*Linea de la columna*/
			$pdf->line($this->posxpreu-1, 76, $this->posxpreu-1, $tab_height+76);

			$pdf->SetXY($this->posxpreu-1, 77);
			$pdf->MultiCell($this->posxtota-$this->posxpreu-1,2, $outputlangs->transnoentities("Precio Unitario"),'','C');
			/*Linea de la columna*/
	...
	{
		return pdf_pagefoot($pdf,$outputlangs,'SALES_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,0,$hidefreetext);
