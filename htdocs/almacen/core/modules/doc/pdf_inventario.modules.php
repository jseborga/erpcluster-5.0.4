 ...
 */
class pdf_inventario extends ModelePDFAlmacen
{
	var $db;
	var $name;
	var $um;
	...
	*/
	function __construct($db)
	{
		...
		$this->description = $langs->trans('PDFInventarioDescription');
		...
		if ($conf->almacen->dir_output)
		{
			...
			else
			{
				...
				if ($object->id >0)
				{
					...
					$object->ref=dol_sanitizeFileName($objecten->libelle).($object->yesnoprice?'_val':'');
					$objectref = dol_sanitizeFileName($object->ref).($object->yesnoprice?'_val':'');
					$dir = $conf->almacen->dir_output . "/" . $objectref.'/'.$period_year;
				}
				else
				{
			...
			if (file_exists($dir))
			{
				...
				foreach ((array) $inventory AS $i => $lines)
				{
					...
					$pdf->MultiCell($this->posxinp-$this->posxsaa-1, 3, price2num($saa,'MU'), 0, 'R',0);

					// input
					$inp = $lines['input'];
					$pdf->SetXY($this->posxinp, $curY);
					$pdf->MultiCell($this->posxout-$this->posxinp-1, 3, price2num($inp,'MU'), 0, 'R',0);

					// output
					$out = $lines['output'];
					$pdf->SetXY($this->posxout, $curY);
					$pdf->MultiCell($this->posxbal-$this->posxout-1, 3, price2num($out,'MU'), 0, 'R',0);

					// balance
					$bal = $lines['balance'];
					$pdf->SetXY($this->posxbal, $curY);
					$pdf->MultiCell($this->posxcco-$this->posxbal-1, 3, price2num($bal,'MU'), 0, 'R',0);

					// cco
					$pdf->SetXY($this->posxcco, $curY);
					$pdf->MultiCell($this->posxdif-$this->posxcco-1, 3, '  ', 0, 'L',0);

					// dif
					$pdf->SetXY($this->posxdif, $curY);
					$pdf->MultiCell($this->posxobs-$this->posxdif-1, 3, '  ', 0, 'L',0);
					// obs
					$pdf->SetXY($this->posxobs, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxobs, 3, '  ', 0, 'L',0);
	...
	*/
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxsaa-$this->posxuni-1,2, $outputlangs->transnoentities("Unidad"),'','C');
		}

		$pdf->line($this->posxsaa-1, $tab_top, $this->posxsaa-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxbal-$this->posxout-1,2, $outputlangs->transnoentities("Output"),'','R');
		}
		$pdf->line($this->posxbal, $tab_top, $this->posxbal, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxbal-1, $tab_top+1);
			$pdf->MultiCell($this->posxcco-$this->posxbal-1,2, $outputlangs->transnoentities("Cant.Sistema"),'','C');
		}
		$pdf->line($this->posxcco, $tab_top, $this->posxcco, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxcco-1, $tab_top+1);
			$pdf->MultiCell($this->posxdif-$this->posxcco-1,2, $outputlangs->transnoentities("Cant.conteo"),'','C');
		}
		$pdf->line($this->posxdif, $tab_top, $this->posxdif, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxdif-1, $tab_top+1);
			$pdf->MultiCell($this->posxobs-$this->posxdif-1,2, $outputlangs->transnoentities("Diferencia"),'','C');
		}

		$pdf->line($this->posxobs, $tab_top, $this->posxobs, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxobs-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxobs,2, $outputlangs->transnoentities("Observación"),'','C');
		}
	}
	...
	*/
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Inventario Físico");
