...
if (! $user->rights->monprojet->contact->leer) accessforbidden();

$object = new Task($db);
$objectadd = new Taskext($db);
...
// Add new contact
if ($action == 'addcontact' && $user->rights->monprojet->contact->crear)
{
...
if ($id > 0 || ! empty($ref))
{
	if ($object->fetch($id, $ref) > 0)
	{
		...
		if (! GETPOST('withproject') || empty($projectstatic->id))
		{
			...
			$objectadd->next_prev_filter=" fk_projet = ".$projectstatic->id;
		}
		print $form->showrefnavadd($objectadd,'id',$linkback,1,'ref','ref','',$param);
		print '</td></tr>';
...
*/
		...
		if ($action != 'editline' && $user->rights->monprojet->contact->crear)
		{
			...
			$var = false;
			if ($user->rights->monprojet->contact->crear && $object->fk_statut == 0)
			{
