...
$objUser = new User($db);
$objIncidentsdet = new Incidentsdet($db);
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/budget/incidents/inactivity.php?id='.$id,1);
...
if ($object->id > 0)
{
	...
	dol_fiche_head($head, 'inactivity', $langs->trans("Incidents"),0,'incidents');
	...
	print '<td align="right">'.$object->day_year.'</td>';
	print '</tr>';
	$var= !$var;
	print '<tr '.$bc[$var].'>';
	print '<td>'.$langs->trans('Fieldday_efective').'</td>';
	$day_efective = $object->day_year - $aTotal['value_one'];
	...
	if ($object->status==0)
	{
		...
		if (!$error)
		{
			...
			//vamos a actualizar el total de incidencia
			$filter = " AND t.fk_incident = ".$object->id;
			$res = $objIncidentsres->fetchAll('','',0,0,array(),'AND',$filter);
			if ($res>0)
			{
				$linesres = $objIncidentsres->lines;
				foreach ($linesres AS $j => $lineres)
					$sumIncident+= $lineres->incident;
			}
		}
		//vamos a actualizar los valores
		$object->day_efective=$day_efective;
		$object->day_journal = $day_journal;
		$object->incident = $sumIncident;
