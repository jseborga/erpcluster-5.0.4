...
/**
 * \file    almacen/contabperiodo.class.php
 * \ingroup almacen
...
class Contabperiodo extends CommonObject
{
	 ...
	 */
	public $element = 'contabperiodo';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'contab_periodo';

	/**
	 * @var ContabperiodoLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->period_month)) {
			 $this->period_month = trim($this->period_month);
		}
		...
		if (isset($this->status_af)) {
			 $this->status_af = trim($this->status_af);
		}
		if (isset($this->status_al)) {
			 $this->status_al = trim($this->status_al);
		}
		...
		$sql .= ' '.(! isset($this->status_af)?'NULL':$this->status_af).',';
		$sql .= ' '.(! isset($this->status_al)?'NULL':$this->status_al);
	 ...
	 */
	public function fetch($id, $month=0,$year=0)
	{
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		if ($month > 0 && $year > 0) {
			$sql .= ' WHERE t.period_month = ' . $month;
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->status_af = $obj->status_af;
				$this->status_al = $obj->status_al;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' status_af = '.(isset($this->status_af)?$this->status_af:"null").',';
		$sql .= ' status_al = '.(isset($this->status_al)?$this->status_al:"null");
