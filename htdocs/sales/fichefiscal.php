...
if ($action =='updatevfiscal' && $user->rights->sales->fac->edit)
{
	//recibimos valores
	$facid = GETPOST('facid');
	$idr = GETPOST('idr');
...
// Delete invoice
  ...
  //cambiamos el estado de vfiscal
	...
	if ($objvfiscal->fk_facture == $id)
	{
	  ...
	  echo '<br>resfis '.  $result = $objvfiscal->update($user);
	  if ($result <= 0) $error++;
	}
  ...
  //eliminamos los pagos
	...
	foreach ($_SESSION['paimentid'] AS $e => $idpai)
	{
	  //eliminamos en la tabla paiement_facture;
		...
		echo '<hr>res '.$result = $objpaiement->delete();
		if ($result <=0)
			$error++;
	}
  ...
  //  echo '<hr>'.$error;
	if (empty($error)) {
		$db->commit();
		header('Location: ' . DOL_URL_ROOT . '/ventas/list.php');
		exit();
	} else {
		$db->rollback();
...
// confirm_re_annulled
	...
	// $object->statut = 3;
	$result = $object->delete($user, 1, $idwarehouse);
	if ($result <=0)
	{
		$error++;
		setEventMessages($object->error,$object->errors,'errors');
	...
	//cambiamos el estado de vfiscal
	$objvfiscal = new Vfiscalext($db);
	$resv = 	$objvfiscal->fetch('',$id);
	//echo '<hr>'.	$objvfiscal->fk_facture.' == '.$id;
	if ($objvfiscal->fk_facture == $id)
	{
	...
	if (empty($error)) {
		$db->commit();
		header('Location: ' . DOL_URL_ROOT . '/sales/list.php');
		exit();
	} else {
		$db->rollback();
		echo '<hr>no hace nada';
		setEventMessage($object->error, 'errors');
		$action='';
	}
}
...
$parameters = array('objectsrc' => $objectsrc,'colspan' => ' colspan="3"');
	...
	$head = sales_facturefiscal_prepare_head($object);
	...
	if ($action == 'confirm_annulled') {
	  ...
	  //confirmamos el abandono
		...
		//echo '<hr>anulacion '.$object->type.' != '.Facture::TYPE_DEPOSIT.' && '.! empty($conf->global->STOCK_CALCULATE_ON_BILL).' && '.$qualified_for_stock_change.' && '.$object->statut.' >= 1';
		if ($object->type != Facture::TYPE_DEPOSIT && ! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change && $object->statut >= 1) {
			$langs->load("stocks");
			require_once DOL_DOCUMENT_ROOT . '/product/class/html.formproduct.class.php';
			$formproduct = new FormProduct($db);
...
echo '<hr>action '.$action;
		...
		if ($objectidnext && $statusreplacement == 0) {
			...
			$formquestion = array('text' => $langs->trans("ConfirmCancelBillQuestion"),
				...
				array('type' => 'text',
					...
					'value' => $langs->trans('Manual cancellation'),
					'size' => '100'));
	...
	if ($result) {
		...
		if ($num > 0) {
			while ($i < $num) {
				...
				print '&nbsp;&nbsp;';
				print '<a href="' . DOL_URL_ROOT . '/ventas/fichefiscal.php?facid='.$id.'&idp=' . $objp->rowid.'&action=delete_paiement' . '">' . img_picto($langs->trans('Deletepaiement'), 'delete') . '</a></td>';
	...
	else
	{
		...
		if ($user->rights->sales->fac->edit && $lDosing)
		{
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline')
	{
		...
		if (empty($reshook)) {
			...
			{
				if (! $objectidnext && $object->close_code != 'replaced' && $objfiscal->status != 2) 				// Not replaced by another invoice
				{
			...
			// Classify 'closed not completely paid' (possible si validee et pas encore classee payee)
			if ($object->statut == 1 && $object->paye == 0 && $resteapayer > 0 && $user->rights->sales->fac->nul)
			{
