...
<tr class="liste_titre nodrag nodrop">
	...
	<!-- <td align="right"><span id="<?php echo ($conf->global->FISCAL_PRICE_TAXES_INCLUDED?'title_vatx':'title_vat'); ?>"><?php echo $langs->trans('VAT'); ?></span></td> -->
	<?php if (!$conf->global->FISCAL_PRICE_TAXES_INCLUDED) { ?>
	<td align="right"><span id="<?php echo ($conf->global->FISCAL_PRICE_TAXES_INCLUDED?'title_up_htx':'title_up_ht'); ?>"><?php echo $langs->trans('PriceUHT'); ?></span></td>
	<?php } else {  ?>
	<td align="right"><span id="<?php echo ($conf->global->FISCAL_PRICE_TAXES_INCLUDED?'title_up_ttcx':'title_up_ttc'); ?>"><?php echo $langs->trans('PriceUTTC'); ?></span></td>
	<?php } ?>
...
?>
	...
	if (! empty($conf->product->enabled) || ! empty($conf->service->enabled))
	{
		...
		if (empty($senderissupplier))
		{
			$form->select_produits_v(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, 1, 2, '', 1, array(),$buyer->id);
		}
		else
		{
			$ajaxoptions=array(
	...
	//if (!$conf->global->FISCAL_PRICE_TAXES_INCLUDED)
	//{
	...
	<?php if (!$conf->global->FISCAL_PRICE_TAXES_INCLUDED) { ?>
	<td align="right">
	<input type="text" size="5" name="price_ht" id="<?php echo ($conf->global->FISCAL_PRICE_TAXES_INCLUDED?'price_htx':'price_ht'); ?>" class="flat" value="<?php echo (isset($_POST["price_ht"])?$_POST["price_ht"]:''); ?>">
	</td>
	<?php } else { ?>
	
	<td align="right">
	<input type="text" size="5" name="price_ttc" id="<?php echo ($conf->global->FISCAL_PRICE_TAXES_INCLUDED?'price_ttcx':'price_ttc'); ?>" class="flat" value="<?php echo (isset($_POST["price_ttc"])?$_POST["price_ttc"]:''); ?>">
	</td>
	<?php }?>
