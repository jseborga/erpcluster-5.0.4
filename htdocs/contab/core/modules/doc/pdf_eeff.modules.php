...
class pdf_eeff extends ModelePDFContab
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->contab->dir_output)
		{
			...
			$aReport = unserialize($_SESSION['aReport']);
			$aDimension = unserialize($_SESSION['aDimension']);
			$aDatos = unserialize($_SESSION['aDatos']);
			$column = unserialize($_SESSION['column_vision']);
			...
			else
			{
				...
				$dir = $conf->contab->dir_output . "/contab/".$period_year.'/eeff';
				$file = $dir . "/EEFF_".$aDatos['ref'].".pdf";
			}
			//echo $file;
			if (! file_exists($dir))
			{
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReport as $j => $line)
				{
					$curY = $nexY;
					$pdf->SetFont('','', $default_font_size-1);
					...
					for ($c=0; $c < $nN ; $c++)
					{
						$pdf->SetXY($aDimension[$c], $curY);
						if($sw == 1)
						{
							//IMPRESION EN NEGRILLA
							$pdf->SetFont('','B', $default_font_size-1);
							if($i < $nVnum){
									//<3
										//$cCharset = explode("/",$line[$i]);
										//$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]+1), 1, $cCharset[0], 0, 'L',0);
								$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]+1), 1, $line[$i], 0, 'L',0);
								$nNewNexy = $pdf->GetY();
							}else{
								if(!empty(price2num($line[$i],'MT')))
									$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]+1), 1, price(price2num($line[$i],'MT')), 0, 'R',0);
								else
									$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]-1), 1, '', 0, 'L',0);
							}
						}
						else
						{
							...
							else
							{
								if(!empty(price2num($line[$i],'MT')))
									...
									$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]+1), 1, '', 0, 'R',0);
							}
						}
						$i++;
					}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		$aHeader = unserialize($_SESSION['aHeader']);
		$aDimension = unserialize($_SESSION['aDimension']);
		$aReport = unserialize($_SESSION['aReport']);
		$column = unserialize($_SESSION['column_vision']);
		...
		*/
		...
		if (empty($hidetop))
		{
			...
			for ($c=0; $c < $nN ; $c++)
			{
				$pdf->SetXY($aDimension[$c]-2, $tab_top+3);
				if ($i<2)
					$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]-1), 1, $aHeader[$i], 0, 'C',0);
				elseif($i>=2 && $i <$nN)
					$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]-1), 1, $outputlangs->transnoentities('Partial'), 0, 'C',0);
				else
					$pdf->MultiCell($aDimension[$c+1]-($aDimension[$c]-1), 1, $outputlangs->transnoentities('Total'), 0, 'C',0);
				if($c != 0){
					$pdf->line($aDimension[$c]-1, $tab_top, $aDimension[$c]-1, $tab_top + $tab_height);
				}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		if($object->statut==0 && (! empty($conf->global->CONTAB_DRAFT_WATERMARK)) )
		{
		...
		$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxdate, 3, $title, '', 'C');

		$pdf->SetFont('','B',$default_font_size+1);

		//Fecha de elaborarion
		$title = $outputlangs->transnoentities("Tothe")." : ". dol_print_date($aDatos['dateend'],'day');
		$posy+=6;
		$pdf->SetXY($this->posxini +2,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell( $this->page_largeur-$this->marge_droite-$this->posxdate, 3, $title, '', 'C');
