...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/mant/groups/program.php?id='.$id,1);
			header("Location: ".$urltogo);
			exit;
		}
		...
		$objectp->fk_group=$id;
		$fk_parent_previous = GETPOST('fk_parent_previous','int');
		if ($fk_parent_previous<=0) $fk_parent_previous = 0;
		$objectp->fk_parent_previous=$fk_parent_previous+0;
		$objectp->fk_type_repair=GETPOST('fk_type_repair','int');
		$objectp->type=GETPOST('type','alpha');
		...
		if ($objectp->fk_type_repair<=0)
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Fieldfk_type_repair")), null, 'errors');
		}
...
if ($object->id > 0)
{
	...
	if ($action == 'create' || $action == 'edit')
		include DOL_DOCUMENT_ROOT.'/mant/groups/tpl/card.tpl.php';		
	else
