 ...
 *					Initialy built by build_class_from_table on 2017-09-11 09:47
 */
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'assistance_membervacationlist';
...
if (($idr > 0) && $action != 'add')
{
	$result=$objMembervacation->fetch($id,$ref);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/membervacation/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.status']['checked'])) print_liste_field_titre($arrayfields['t.status']['label'],$_SERVER['PHP_SELF'],'t.status','',$params,'align="right"',$sortfield,$sortorder);
...
//
	...
	if (! empty($arrayfields['t.status']['checked'])) print '<td class="liste_titre" align="right"><input type="text" class="flat" name="search_status" value="'.$search_status.'" size="10"></td>';
...
if ($action == 'create')
{
	...
	{
		print '<td>'.'<input type="number" min="0" name="period_year" value="'.GETPOST('period_year').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['t.days_assigned']['checked']))
	{
		print '<td>'.'<input type="number" min="0" name="days_assigned" value="'.GETPOST('days_assigned').'">'.'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
