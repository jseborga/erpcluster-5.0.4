					...
					if ($id > 0)
					{
						...
						else
						{
							if ($result > 0)
							{
								...
								if (!$error && $abc)
								{
									//registramos en la tabla v_fiscal
									$vfiscal->entity = $object->entity;
									$vfiscal->nfiscal = '(PROV)'.$id;
									$vfiscal->serie = '000';
									$vfiscal->fk_dosing = GETPOST('fk_dosing');
									$vfiscal->fk_facture = $id;
									$vfiscal->fk_cliepro = $object->socid;
									$vfiscal->nit = GETPOST('nit');
									$vfiscal->razsoc = GETPOST('razsoc');
									$vfiscal->date_exp = $dateinvoice;
									$vfiscal->type_op = 1;
									$vfiscal->num_autoriz = '000';
									$vfiscal->cod_control = '0';
									$vfiscal->baseimp1 = $aData['total_ttc'];
									$vfiscal->aliqimp1 = $tvatx;
									$vfiscal->valimp1 = $aData['total_tva'];
									$vfiscal->amount_payment = 0;
									$vfiscal->amount_balance = 0;
									$vfiscal->date_create = dol_now();
									$vfiscal->date_mod = dol_now();
									$vfiscal->tms = dol_now();
									$vfiscal->fk_user_create = $user->id;
									$vfiscal->fk_user_mod = $user->id;
									$vfiscal->status = 0;
									$res = $vfiscal->create($user);
									if ($res <=0)
									{
										$error++;
										setEventMessages($vfiscal->error,$vfiscal->errors,'errors');
...
if ($nbdosing>0)
{
	...
	foreach ($vdosing->lines AS $j => $line)
	{
		...
		$aDosing[$line->id] = $line->series.' '.($line->lote==2?$langs->trans('Automatic'):$langs->trans('Manual')).' - '.$subsidiary->ref;
	}
}
...
if ($action == 'create')
{
	...
	if (! empty($origin) && ! empty($originid) && is_object($objectsrc))
	{
		...
		print '<tr><td>' . $langs->trans($newclassname) . '</td><td colspan="2">' . $objectsrc->getNomUrladd(1);
	...
	if (! empty($origin) && ! empty($originid) && is_object($objectsrc)) {
		...
		$objectsrc->printOriginLinesListadd();
...
else if ($id > 0 || ! empty($ref))
{
	...
	if ($action == 'valid')
	{
		...
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?facid=' . $object->id, $langs->trans('ValidateBill'), $text, 'confirm_valid', $formquestion, (($object->type != Facture::TYPE_CREDIT_NOTE && $object->total_ttc < 0) ? "no" : "yes"), 2,300);
	}
