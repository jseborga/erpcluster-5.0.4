...
/**
 *  \file       dev/skeletons/cscurrencytype.class.php
 *  \ingroup    mymodule othermodule1 othermodule2
 *  \brief      This file is an example for a CRUD class file (Create/Read/Update/Delete)
 *				Initialy built by build_class_from_table on 2014-07-06 12:56
 */
 ...
 */
class Cscurrencytype extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	var $element='cscurrencytype';			//!< Id that identify managed objects
	var $table_element='cscurrencytype';		//!< Name of table without prefix where object is stored
    ...
    function create($user, $notrigger=0)
    {
		...
		if (isset($this->registry)) $this->registry=trim($this->registry);
		if (isset($this->order_currency)) $this->order_currency=trim($this->order_currency);
        ...
        // Insert request
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."cs_currency_type(";
		...
		$sql.= " ".(! isset($this->registry)?'NULL':"'".$this->registry."'").",";
		$sql.= " ".(! isset($this->order_currency)?'NULL':"'".$this->order_currency."'").",";
		...
		if (! $error)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."cs_currency_type");
    ...
    function fetch($id)
    {
        ...
        if ($resql)
        {
            if ($this->db->num_rows($resql))
            {
				...
				$this->order_currency = $obj->order_currency;
    ...
    function update($user=0, $notrigger=0)
    {
        ...
        // Update request
        $sql = "UPDATE ".MAIN_DB_PREFIX."cs_currency_type SET";
        
		$sql.= " entity=".(isset($this->entity)?$this->entity:"null").",";
		$sql.= " ref=".(isset($this->ref)?"'".$this->db->escape($this->ref)."'":"null").",";
		$sql.= " registry=".(isset($this->registry)?$this->registry:"null").",";
		$sql.= " order_currency=".(isset($this->order_currency)?$this->order_currency:"null").",";
	...
	function delete($user, $notrigger=0)
	{
		...
		if (! $error)
		{
    		$sql = "DELETE FROM ".MAIN_DB_PREFIX."cs_currency_type";
    		$sql.= " WHERE rowid=".$this->id;
	...
	function createFromClone($fromid)
	{
		...
		$object=new Cscurrencytype($this->db);
