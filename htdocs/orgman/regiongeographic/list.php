...
/**
 *   	\file       orgman/cregiongeographic_list.php
 *		\ingroup    orgman
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'cregiongeographiclist';
...
if (empty($reshook))
{
	...
	// Mass actions
	$objectclass='Cregiongeographic';
	$objectlabel='Cregiongeographic';
	$permtoread = $user->rights->orgman->regg->read;
	$permtodelete = $user->rights->orgman->regg->del;
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_region_geographic as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_region_geographic_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/cregiongeographic/card.php?id='.$id);
	exit;
}
...
if (empty($reshook))
{
	...
	{
		print '&nbsp;<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/orgman/regiongeographic/card.php?action=create">'.$langs->trans("New").'</a></div>'."\n";
	}
}
print '</div>'."\n";
