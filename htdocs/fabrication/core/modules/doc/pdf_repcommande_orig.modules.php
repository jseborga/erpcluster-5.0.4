  ...
  // Objet societe qui emet
		...
		if ($conf->fabrication->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				//for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$desc = trim($desc);
					$desc = preg_replace("[\n|\r|\n\r]", " ", $desc);
					$pdf->SetXY($this->posxnpr, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxnpr, 0, $desc, 0, 'L',0, 1, '' ,'', false,3,false,true,0,'T');
	...
	*/
	...
	// line prend une position y en 2eme param et 4eme param
		...
		$pdf->line($this->posxnpr, $tab_top, $this->posxnpr, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxnpr-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxnpr,2, $outputlangs->transnoentities("Descripción"),'','C');
		}
	}
