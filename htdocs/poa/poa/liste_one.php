...
<?php
	...
	$aHtml = array();
		...
		(!empty(STRTOUPPER($search_user)) && $filteruser===false))
	      {
		$lContinue = false;
		$ii--;
	      }
	    ...
	    $objprevl->getlist_poa($obj->id);
	    $lisPrev.= '<div id="pre'.$obj->id.'" style="display:none;">';
	    $newClase2 = $newClase;
	    foreach ((array) $objprevl->array AS $objppl)
	      {
		$newClase = ' class="left"  style="background-color:#a9f5d0;"';
		$lisPrev.= '<div>';
		$lisPrev.= '<div id="meta" '.$newClase.'">'.'<a href="'.DOL_URL_ROOT.'/poa/execution/fiche.php?id='.$objppl->previd.'" title="'.$objppl->label.'">'.'PR-'.$objppl->nro_preventive.'</a>'.'</div>';
		if ($numCol[1] || $numCol[2])
		  {
		    $lisPrev.= '<div id="pseudo" '.$newClase.'">';
		    $lisPrev.=  (strlen($objppl->label)>30?'<a href="#" title="'.$objppl->label.'">'.substr($objppl->label,0,30).'...</a>':(empty($objppl->pseudonym)?'&nbsp;':$objppl->pseudonym));
		    $lisPrev.=  '</div>';
		  }
		$lisPrev.= '<div id="partida" '.$newClase.'">&nbsp;</div>'; //partida
		$lisPrev.= '<div id="amount" '.$newClase.'">&nbsp;</div>';//presupuesto
		...
		if ($opver == 1)
		  {
		    $lisPrev.= '<div id="amountone" '.$newClase.'">&nbsp;'.'</div>';
		    $lisPrev.= '<div id="amountone" '.$newClase.'">&nbsp;'.'</div>';
		    $lisPrev.= '<div id="amountone" '.$newClase.'">&nbsp;'.'</div>';
		    $lisPrev.= '<div id="amountone" '.$newClase.'">&nbsp;'.'</div>';
		    $lisPrev.= '<div id="amountone" '.$newClase.'">&nbsp;'.'</div>';
		...
		if($numCol[9] || $numCol[10] || $numCol[15])
		  {
		    $lisPrev.= '<div id="amount" '.$newClase.'">'.price(price2num($objppl->amount,'MT')).'</div>';
		    
		  }
		//comprometidos	
		//rqcc
		$objcomp->getsum_prev_str_part($objppl->previd,$obj->fk_structure,$obj->id, $objppl->partida);
		...
		//deventados
		$objdeve->getsum_prev_str_part($objppl->previd,$obj->fk_structure,$obj->id, $objppl->partida);
		...
		foreach ((array) $objpac->array AS $k => $objpaclist)
		  {
		    ...
		    if ($opver == 1)
		      {
			$lisPac.= '<div id="amountone" '.$newClase.'">&nbsp;</div>';
			$lisPac.= '<div id="amountone" '.$newClase.'">&nbsp;</div>';
			$lisPac.= '<div id="amountone" '.$newClase.'">&nbsp;</div>';
			$lisPac.= '<div id="amountone" '.$newClase.'">&nbsp;</div>';
			$lisPac.= '<div id="amountone" '.$newClase.'">&nbsp;</div>';
