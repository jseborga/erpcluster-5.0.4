...
/**
 *   	\file       multicurren/cscurrencytype_list.php
 *		\ingroup    multicurren
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-01-24 11:49
 */
...
$search_registry=GETPOST('search_registry','int');
$search_order_currency=GETPOST('search_order_currency','int');
...
$hookmanager->initHooks(array('cscurrencytypelist'));
...
//'t.entity'=>array('label'=>$langs->trans("Fieldentity"), 'checked'=>1),
	...
	't.registry'=>array('label'=>$langs->trans("Fieldregistry"), 'checked'=>1),
	't.order_currency'=>array('label'=>$langs->trans("Fieldorder_currency"), 'checked'=>1),
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/multicurren/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Currencytype');
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."cs_currency_type_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_registry) $sql.= natural_search("registry",$search_registry);
if ($search_order_currency) $sql.= natural_search("order_currency",$search_order_currency);
...
//if ($search_entity != '') $params.= '&amp;search_entity='.urlencode($search_entity);
	...
	if ($search_registry != '') $params.= '&amp;search_registry='.urlencode($search_registry);
	if ($search_order_currency != '') $params.= '&amp;search_order_currency='.urlencode($search_order_currency);
...
//if (! empty($arrayfields['t.entity']['checked'])) print_liste_field_titre($arrayfields['t.entity']['label'],$_SERVER['PHP_SELF'],'t.entity','',$params,'',$sortfield,$sortorder);
    ...
    if (! empty($arrayfields['t.registry']['checked'])) print_liste_field_titre($arrayfields['t.registry']['label'],$_SERVER['PHP_SELF'],'t.registry','',$params,'',$sortfield,$sortorder);
    if (! empty($arrayfields['t.order_currency']['checked'])) print_liste_field_titre($arrayfields['t.order_currency']['label'],$_SERVER['PHP_SELF'],'t.order_currency','',$params,'',$sortfield,$sortorder);
    ...
    if (! empty($arrayfields['t.registry']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_registry" value="'.$search_registry.'" size="10"></td>';
    if (! empty($arrayfields['t.order_currency']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_order_currency" value="'.$search_order_currency.'" size="10"></td>';
    ...
    while ($i < min($num, $limit))
    {
    	...
    	if ($obj)
    	{
    		...
    		if (! empty($arrayfields['t.registry']['checked']))
    		{
    			print '<td>'.$aRegistry[$obj->registry].'</td>';
    			if (! $i) $totalarray['nbfield']++;
    		}
    		if (! empty($arrayfields['t.order_currency']['checked']))
    		{
    			print '<td>'.$obj->order_currency.'</td>';
    			if (! $i) $totalarray['nbfield']++;
    		}
    		...
    		{
    			print '<td>'.$object->LibStatut($object->status,1).'</td>';
    			if (! $i) $totalarray['nbfield']++;
    		}
        	// Extra fields
    		if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label))
  ...
  $parameters=array('sql' => $sql);
	...
	if ($user->rights->multicurren->curr->creer)
