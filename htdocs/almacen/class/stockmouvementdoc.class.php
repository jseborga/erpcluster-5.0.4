...
class Stockmouvementdoc extends CommonObject
{
	...
	function getNomUrl($withpicto=0, $option='', $notooltip=0, $maxlen=24, $morecss='',$type=0)
	{
        ...
        $label = '<u>' . $langs->trans("Stockmouvementdoc") . '</u>';
        $label.= '<div width="100%">';
        $label.= '<b>' . $langs->trans('Ref') . ':</b> ' . $this->ref;

        if($type == 1)
	    	$link = '<a href="'.DOL_URL_ROOT.'/almacen/mouvement/card.php?id='.$this->id.'"';
        elseif($type == 2)
	    	$link = '<a href="'.DOL_URL_ROOT.'/almacen/transferencia/card.php?id='.$this->id.'"';
        else
	        $link = '<a href="'.DOL_URL_ROOT.'/almacen/transferencia/fiche.php?id='.$this->id.'"';
	 ...
	 *  @return string 			       	Label of status
	 */
	function LibStatutx($status,$mode=0)
	{
		global $langs;
