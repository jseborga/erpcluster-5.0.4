...
/**
 *   	\file       assets/assetsdoc_list.php
 *		\ingroup    assets
...
$search_labeldoc=GETPOST('search_labeldoc','alpha');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'assetsdoclist';
...
$arrayfields=array(
	...
	't.fk_cassetdoc'=>array('label'=>$langs->trans("Fieldfk_cassetdoc"), 'checked'=>1),
	'd.label'=>array('label'=>$langs->trans("Fieldlabeldoc"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."assets_doc as t";
$sql.= " INNER JOIN ".MAIN_DB_PREFIX."c_assets_doc as d ON t.fk_cassetdoc = d.rowid";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."assets_doc_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
$sql.= " AND t.fk_asset = ".$id;
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_asset) $sql.= natural_search("fk_asset",$search_fk_asset);
if ($search_fk_cassetdoc) $sql.= natural_search("fk_cassetdoc",$search_fk_cassetdoc);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/assetsdoc/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.fk_cassetdoc']['checked'])) print_liste_field_titre($arrayfields['t.fk_cassetdoc']['label'],$_SERVER['PHP_SELF'],'t.fk_cassetdoc','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['d.label']['checked'])) print_liste_field_titre($arrayfields['d.label']['label'],$_SERVER['PHP_SELF'],'d.label','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_cassetdoc']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_cassetdoc" value="'.$search_fk_cassetdoc.'" size="10"></td>';
if (! empty($arrayfields['d.label']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_labeldoc" value="'.$search_labeldoc.'" size="10"></td>';
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				$aKey = explode('.',$key);
				$key2 = $aKey[1];
				if ($key2 == 'fk_cassetdoc')
				{
					$objCassetsdoc->fetch($obj->$key2);
					$obj->$key2 = '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$obj->rowid.'">'.$objCassetsdoc->label.'</a>';
				}
				if ($key2 == 'active')
				{
					$img = 'switch_off';
		...
		{
			print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$id.'&idr='.$obj->rowid.'&action=deletedoc">'.img_picto($langs->trans('Delete'),'delete').'</a>';
		}
		print '</td>';
		if (! $i) $totalarray['nbfield']++;
...
print '</form>'."\n";
	...
	if ($user->rights->assets->doc->write)
		print '<a class="butAction" href="'.DOL_URL_ROOT.'/assets/assets/fiche.php?id='.$id.'&tab='.$tab.'&action=createdoc">'.$langs->trans("Create").'</a>';
	else
