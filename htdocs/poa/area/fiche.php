...
if ($action == 'add' && $user->rights->poa->area->crear)
{
	...
	if (empty($error))
	{
		...
		if ($id > 0)
		{
			header("Location: fiche.php?id=".$id.'&dol_hide_leftmenu=1');
			exit;
		}
		$action = 'create';
...
if ($action == 'adduser' && $user->rights->poa->area->crear)
{
	...
	$error = 0;
	$objuser->fk_area = GETPOST('id');
	$objuser->fk_user = GETPOST('fk_user');
	$objuser->date_create = dol_now();
	$objuser->tms = dol_now();
	$objuser->active  = 1;
	$objuser->privilege = 0;
	...
	if (empty($error))
	{
		if ($objuser->create($user)>0)
		{
			header("Location: fiche.php?id=".$id);
			exit;
		}
		$action = 'create';
		echo $mesg='<div class="error">'.$objuser->error.'</div>';
		exit;
	}
	else
	{
...
if ($action == 'confirm_delete' && $_REQUEST["confirm"] == 'yes' && $user->rights->poa->area->del)
{
	...
	if ($result > 0)
	{
		header("Location: ".DOL_URL_ROOT.'/poa/area/liste.php?dol_hide_leftmenu=1');
		exit;
	}
	else
	{
...
llxHeader("",$langs->trans("Area"),$help_url,'','','',$aArrjs,$aArrcss);
...
if ($action == 'create' && $user->rights->poa->area->crear)
{
	...
	print '<a class="butAction" href="'.DOL_URL_ROOT.'/poa/area/liste.php?dol_hide_leftmenu=1">'.$langs->trans("Return").'</a>';
	print '</div>';
}
else
{
		 ...
		 if (count($objuser->array) > 0)
		 {
			foreach((array) $objuser->array AS $j => $objus)
			{
			 ...
			 //privileges
				...
				if ($user->rights->poa->area->crear)
				{
					print '<a href="fiche.php?id='.$id.'&idr='.$objus->id.'&dol_hide_leftmenu=1&action='.($objus->privilege==0?'privadmin':($objus->privilege==1?'privuser':($objus->privilege==2?'privvisit':'notpriv'))).'">'.$objuser->LibStatut($objus->privilege,4,0).'</a>';
				}
				else
				...
				if ($user->rights->poa->area->crear)
				{
					print '<a href="'.DOL_URL_ROOT.'/poa/area/fiche.php?id='.$id.'&dol_hide_leftmenu=1&idus='.$objus->id.'&action=deluser'.'">'.img_picto($langs->trans('Delete'),'delete').'</a>';
				}
				print '</td></tr>';
