...
/**
 *   	\file       poa/poaprev_list.php
 *		\ingroup    poa
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-04-04 10:36
 */
...
dol_include_once('/poa/class/poaprev.class.php');
...
$search_fk_poa_activity=GETPOST('search_fk_poa_activity','int');
...
$search_nro_preventive=GETPOST('search_nro_preventive','int');
...
't.fk_poa_activity'=>array('label'=>$langs->trans("Fieldfk_poa_activity"), 'checked'=>1),
...
't.nro_preventive'=>array('label'=>$langs->trans("Fieldnro_preventive"), 'checked'=>1),
...
$search_fk_pac='';
$search_fk_area='';
$search_fk_poa_activity='';
$search_origin='';
$search_originid='';
$search_code_requirement='';
$search_label='';
$search_pseudonym='';
$search_nro_preventive='';
$search_amount='';
$search_priority='';
...
$search_active='';
    ...
    $permtoread = $user->rights->poaprev->read;
    $permtodelete = $user->rights->poaprev->delete;
    $uploaddir = $conf->poaprev->dir_output;
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."poa_prev_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_fk_poa_activity) $sql.= natural_search("fk_poa_activity",$search_fk_poa_activity);
...
if ($search_nro_preventive) $sql.= natural_search("nro_preventive",$search_nro_preventive);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/poaprev/card.php?id='.$id);
    exit;
}
...
if (! empty($arrayfields['t.gestion']['checked'])) print_liste_field_titre($arrayfields['t.gestion']['label'],$_SERVER['PHP_SELF'],'t.gestion','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_pac']['checked'])) print_liste_field_titre($arrayfields['t.fk_pac']['label'],$_SERVER['PHP_SELF'],'t.fk_pac','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_poa_activity']['checked'])) print_liste_field_titre($arrayfields['t.fk_poa_activity']['label'],$_SERVER['PHP_SELF'],'t.fk_poa_activity','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.code_requirement']['checked'])) print_liste_field_titre($arrayfields['t.code_requirement']['label'],$_SERVER['PHP_SELF'],'t.code_requirement','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.nro_preventive']['checked'])) print_liste_field_titre($arrayfields['t.nro_preventive']['label'],$_SERVER['PHP_SELF'],'t.nro_preventive','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_poa_activity']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_poa_activity" value="'.$search_fk_poa_activity.'" size="10"></td>';
...
if (! empty($arrayfields['t.nro_preventive']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_nro_preventive" value="'.$search_nro_preventive.'" size="10"></td>';
