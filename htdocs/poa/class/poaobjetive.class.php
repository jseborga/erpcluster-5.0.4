...
/**
 * \file    poa/poaobjetive.class.php
 * \ingroup poa
 ...
 * @see CommonObject
 */
class Poaobjetive extends CommonObject
{
	/**
	 * @var string Id to identify managed objects
	 */
	public $element = 'poaobjetive';
	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'poa_objetive';

	/**
	 * @var PoaobjetiveLine[] Lines
	 */
	public $lines = array();
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (isset($this->fk_poa_strategic)) {
			 $this->fk_poa_strategic = trim($this->fk_poa_strategic);
		}
		...
		if (isset($this->expected_result)) {
			 $this->expected_result = trim($this->expected_result);
		}
		if (isset($this->unit)) {
		...
		$sql .= ' '.(! isset($this->fk_poa_strategic)?'NULL':$this->fk_poa_strategic).',';
		...
		$sql .= ' '.(! isset($this->expected_result)?'NULL':"'".$this->db->escape($this->expected_result)."'").',';
	...
	public function fetch($id, $ref = null)
	{
		...
		if (! empty($conf->multicompany->enabled)) {
		    $sql .= " AND entity IN (" . getEntity("poaobjetive", 1) . ")";
		}
		if (null !== $ref) {
			$sql .= ' AND t.ref = ' . '\'' . $ref . '\'';
		} else {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->fk_poa_strategic = $obj->fk_poa_strategic;
				...
				$this->expected_result = $obj->expected_result;
				$this->unit = $obj->unit;
				$this->pos = $obj->pos;
	...
	public function fetchAll($sortorder='', $sortfield='', $limit=0, $offset=0, array $filter = array(), $filtermode='AND',$filterstatic='',$lView=false)
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new PoaobjetiveLine();

				$line->id = $obj->rowid;
				
				$line->fk_poa_strategic = $obj->fk_poa_strategic;
				...
				$line->expected_result = $obj->expected_result;
				$line->unit = $obj->unit;
				$line->pos = $obj->pos;
	...
	public function update(User $user, $notrigger = false)
	{
		...
		$sql .= ' fk_poa_strategic = '.(isset($this->fk_poa_strategic)?$this->fk_poa_strategic:"null").',';
		...
		$sql .= ' expected_result = '.(isset($this->expected_result)?"'".$this->db->escape($this->expected_result)."'":"null").',';
	...
	public function createFromClone($fromid)
	{
		...
		global $user;
		$error = 0;
		$object = new Poaobjetive($this->db);
