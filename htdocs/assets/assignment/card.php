...
if ($action == 'add' && $user->rights->assets->alloc->crear)
{
	...
	{
		$error++;
		$mesg.='<div class="error">'.$langs->trans("ErrorPropertyLocationProjectisrequired").'</div>';
	}
	if (empty($error))
	{
...
if ($action == 'addassign' && $user->rights->assets->alloc->crear)
{
	...
	if ($result > 0)
	{
		...
		if (empty($error))
		{
			...
			{
				$db->rollback();
				setEventMessages($objassigndet->error,$objassigndet->errors,'error');
...
if ($action == 'confirm_applyfor' && $_REQUEST["confirm"] == 'yes' && $user->rights->assets->alloc->sol)
{
	...
	if ($object->fetch($id))
	{
		...
		if ($numref == -1)
		{
			setEventMessages($langs->trans('Assignment is not coded correctly, check'),null,'errors');
			$errror++;
			$action='';
		}
		else
...
if ($user->rights->assets->alloc->crear && ($action == 'create' || $action == 'createf'))
{
	...
	//date assignment
	print '<tr><td class="fieldrequired">'.$langs->trans('Date assignment').'</td><td colspan="2">';
	if ($user->admin)
	{
	...
	if (GETPOST('origin'))
	{
		...
		if ($action == 'createf')
		{
			...
			print '</td></tr>';
			//solicitar a
			print '<tr><td class="fieldrequired">xx'.$langs->trans('Transferto').'</td><td colspan="2">';
		...
		else
		{
			...
			{
				//print $form->select_dolusers((GETPOST('fk_user')?GETPOST('fk_user'):$fk_user),'fk_user',1,$exclude,0,$include);
				print $form->select_member((GETPOST('fk_user')?GETPOST('fk_user'):$fk_user), 'fk_user', $filter='', 1,0,0,array(),0);
			}
			print '</td></tr>';
			...
			if ($action == 'createf') $fk_user_from = $objtmp->fk_user;
			//print $form->select_dolusers((GETPOST('fk_user_from')?GETPOST('fk_user_from'):$fk_user_from),'fk_user_from',1,$exclude,0,$include);
			print $form->select_member((GETPOST('fk_user_from')?GETPOST('fk_user_from'):$fk_user_from), 'fk_user_from', $filter='', 1,0,0,array(),0);
			print '</td></tr>';
		}
	}
	else
	{
		...
		//print $form->select_dolusers($selected,'fk_user',1,$exclude,0,$include);
	...
	print '<tr><td >'.$langs->trans('Typeassignment').' '.$action.'</td><td colspan="2">';
	if ($action != 'createf')
	...
	if ($id || $_GET['id'])
	{
		...
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			if ($action == '')
			{
				...
				{
					if ($object->status == 0 && (($user->rights->assets->alloc->lall && $user->rights->assets->alloc->sol && count($objassigdact->array) > 0)  || ($user->rights->assets->alloc->sol && $user->id == $object->fk_user && $object->origin && count($objassigdact->array) > 0)))
						print "<a class=\"butAction\" href=\"fiche.php?action=applyfor&id=".$object->id."\">".$langs->trans("Applyfor")."</a>";
					else
						print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Requested")."</a>";
				}
				...
				if ($object->status == 3 && (($user->rights->assets->alloc->lall && $user->rights->assets->alloc->lib && count($objassigdact->array) > 0)  || ($user->rights->assets->alloc->lib && $user->id == $object->fk_user && count($objassigdact->array) > 0)))
					if ($lLibre)
						print "<a class=\"butAction\" href=\"fiche.php?action=liberate&id=".$object->id."\">".$langs->trans("Leave free")."</a>";
					else
						print "<a class=\"butActionRefused\" href=\"#\">".$langs->trans("Leave free")."</a>";
				...
				{
					$lApp = false;
					if (($user->rights->assets->alloc->lall && $object->type_assignment) || ($object->fk_user == $user->id))
						$lApp = true;
					if ($lApp)
					{
						print '<center>';
		 ...
		 //adherent
				...
				if ($user->admin)
					//print $form->select_dolusers($object->fk_user,'fk_user',1,$exclude,0,$include);
		 ...
		 //location
				...
				//print $objloc->select_location($object->fk_location,'fk_location','',40,1,$object->fk_property);
				print '</td></tr>';
