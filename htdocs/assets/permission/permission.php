 ...
 * Actions
 */
if ($action == 'adduser' && $user->rights->assets->teach->creer)
{
	$error = 0;
...
if ($action == 'deleteuser' && $user->rights->assets->teach->del)
{
	$error = 0;
	$obj = new Mpropertyuser($db);
		...
		$parameters=array();
			...
			if ($resql)
			{
				...
				while ($i < $num)
				{
					...
					{
						$objnew = $objp;
						include DOL_DOCUMENT_ROOT.'/assets/permission/tpl/permission.tpl.php';	    
					}
					else
					{
					//print '<td>'.dol_print_date($objp->datem).'</td>';
						...
						if ($user->rights->almacen->entr->del)
			...
			if (empty($action))
			{
				if ($user->rights->almacen->entr->crear)
