 ...
 */
class pdf_reportodt extends ModelePDFMant
{
	var $db;
	var $name;
	var $description;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		$aReportdet = unserialize($_SESSION['aReportodt']);
		...
		if ($conf->poa->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				foreach ((array) $aReportdet AS $i => $lines)
				{
					...
					$pdf->MultiCell($this->posxref-$this->posxnum-1, 5, $j, 0, 'L',0);

					//Referencia
					$ref = $lines['ref'];
					$pdf->SetXY($this->posxref, $curY);
					$pdf->MultiCell($this->posxtic-$this->posxref-1, 5, $ref, 0, 'L',0);

					//ticket
					$ticket = $lines['ticket'];
					$pdf->SetXY($this->posxtic, $curY);
					$pdf->MultiCell($this->posxdate-$this->posxtic-1, 5, $ticket, 0, 'L',0);

					//date
					$date = $lines['date'];
					$pdf->SetXY($this->posxdate, $curY);
					$pdf->MultiCell($this->posxinm-$this->posxdate-1, 5, $date, 0, 'L',0);
					...
					$pdf->MultiCell($this->posxpro-$this->posxloc-1, 5, $local, 0, 'L',0);

					//problema
					$Problema = $lines['Problema'];
					$pdf->SetXY($this->posxpro, $curY);
					$pdf->MultiCell($this->posxusu-$this->posxpro-1, 5, $Problema, 0, 'L',0);

					//Usuario
					$Usuario = $lines['Usuario'];
					$pdf->SetXY($this->posxusu, $curY);
					$pdf->MultiCell($this->posxasig-$this->posxusu-1, 5, $Usuario, 0, 'L',0,0,'','',true);
					//Asignado
					$Asignado = $lines['Asignado'];
					$pdf->SetXY($this->posxasig, $curY);
					$pdf->MultiCell($this->posxresp-$this->posxasig-1, 5, $Asignacion, 0, 'L',0,0,'','',true);
					//Responsable
					$Responsable = $lines['responsable'];
					$pdf->SetXY($this->posxresp, $curY);
					$pdf->MultiCell($this->posxstatus-$this->posxresp-1, 5, $Asignacion, 0, 'L',0,0,'','',true);
					//estado
					$Estado = $lines['Estado'];
	...
	*/
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxref-$this->posxnum-1,2, $outputlangs->transnoentities("Nro"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxtic-$this->posxref-1,2, $outputlangs->transnoentities("Ref"),'','C');
		}

		$pdf->line($this->posxref-1, $tab_top, $this->posxref-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxtic-1, $tab_top+1);
			$pdf->MultiCell($this->posxdate-$this->posxtic-1,2, $outputlangs->transnoentities("Ticket"),'','C');
		}

		$pdf->line($this->posxtic-1, $tab_top, $this->posxtic-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxdate-1, $tab_top+1);
			$pdf->MultiCell($this->posxinm-$this->posxdate-1,2, $outputlangs->transnoentities("date"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxloc-$this->posxinm-1,2, $outputlangs->transnoentities("Inmueble"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxpro-$this->posxloc-1,2, $outputlangs->transnoentities("Location"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxusu-$this->posxpro-1,2, $outputlangs->transnoentities("Problema"),'','C');
		}
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxusu-1, $tab_top+1);
			$pdf->MultiCell($this->posxasig-$this->posxusu-1,2, $outputlangs->transnoentities("User"),'','C');
		}


		$pdf->line($this->posxusu-1, $tab_top, $this->posxusu-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxasig-1, $tab_top+1);
			$pdf->MultiCell($this->posxresp-$this->posxasig-1,2, $outputlangs->transnoentities("Asignado"),'','C');
		}

		$pdf->line($this->posxasig-1, $tab_top, $this->posxasig-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxresp-1, $tab_top+1);
			$pdf->MultiCell($this->posxstatus-$this->posxresp-1,2, $outputlangs->transnoentities("Responsable"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->page_hauteur-$this->marge_haute-$this->posxstatus-1,2, $outputlangs->transnoentities("statut"),'','C');
		}
