 ...
 */
class pdf_licencia extends ModelePDFAssistance
{
	var $db;
	var $name;
	var $um;
  ...
  // Objet societe qui emet
	...
	*/
	function __construct($db)
	{
		...
		//Array para el reporte de licencia
		$arrayReporte = unserialize($_SESSION['arrayReporte']);
		//echo('PeriodYear : '.$_SESSION['period_year']);exit;
	...
	*/
	...
	// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		...
		//echo('valor de ref en report : '.$valref);exit;
		...
		if ($conf->assistance->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$posxmargen=10;
				// 1. autorizacion
				$aut = 'Autorizado para salida del personal : ';
				$pdf->SetXY($posxmargen, 34);
				$pdf->MultiCell(140, 1, $aut, 0,'L',0);

				// 2. Fecha
				/*$fec ='Fecha: '.dol_print_date($arrayReporte['fecha'],'day');
				$pdf->SetXY(150,34);
				$pdf->MultiCell(60, 1, $fec, 0, 'L',0,0,'','',false);
				$nexY = $pdf->GetY()+7;*/
				...
				$pdf->MultiCell(220, 1, $nom, 0, 'L',0);

				$nexY = $pdf->GetY()+2;
				// 4. Area func
				$nom = 'Area Funcional : '.$arrayReporte['area'];
				$pdf->SetXY($posxmargen, $nexY);
				$pdf->MultiCell(220, 1, $nom, 0, 'L',0);
				...
				$nexYfin = $nexY;
				//Fecha Inicio
				$feini = 'DESDE : '.dol_print_date($arrayReporte['fechaini'],'daytext');
				$pdf->SetXY($posxmargen, $nexY);
				$pdf->MultiCell($this->page_largeur/2, 1, $feini, 0, 'L',0);

				//$nexY = $pdf->GetY()+7;
				/*$diaini = 'Dia : ';
				$pdf->SetXY($posxmargen, $nexY);
				$pdf->MultiCell($this->page_largeur/2-50, 1, $diaini, 0, 'L',0);*/

				$horaini = 'Hora : '.dol_print_date($arrayReporte['fechaini'],'hour');
				$pdf->SetXY($this->page_largeur/2-30, $nexY);
				$pdf->MultiCell($this->page_largeur/2-30, 1, $horaini, 0, 'L',0);

				//Fecha Final
				$fefin = 'HASTA : '.dol_print_date($arrayReporte['fechafin'],'daytext');
				$pdf->SetXY(($this->page_largeur/2)+2, $nexYfin);
				$pdf->MultiCell($this->page_largeur/2, 1, $fefin, 0, 'L',0);

				/*$nexYfin = $pdf->GetY()+7;
				$diafin = 'Dia : ';
				$pdf->SetXY(($this->page_largeur/2)+2, $nexYfin);
				$pdf->MultiCell($this->page_largeur/2-50, 1, $diafin, 0, 'L',0);*/

				$horafin = 'Hora : '.dol_print_date($arrayReporte['fechafin'],'hour');
				$pdf->SetXY($this->page_largeur-45, $nexYfin);
				$pdf->MultiCell($this->page_largeur/2-30, 1, $horafin, 0, 'L',0);

				$nexY = $pdf->GetY()+16;

				//Motivo
				$mot = 'MOTIVO : '.$arrayReporte['motivo'];
				$pdf->SetXY($posxmargen, $nexY);
				$pdf->MultiCell($this->page_largeur-22, 1, $mot, 0, 'L',0);

				$nexY = $pdf->GetY()+30;

				//Firma Solicitante
				$fs = 'FIRMA SOLICITANTE';
				$pdf->SetXY($posxmargen, $nexY);
				$pdf->MultiCell(100, 1, $fs, 0, 'C',0);
				//Firma Director
				$fd = 'FIRMA DIRECTOR';
				$pdf->SetXY($this->page_largeur/2, $nexY);
				$pdf->MultiCell($this->posxfi-$this->posxin-1, 1, $fd, 0, 'C',0);
	...
	// Output Rect
		...
		if($type_page==0)
		{
			$this->printRect($pdf,$this->marge_gauche-4, 32, $this->page_largeur-$this->marge_gauche-$this->marge_droite+7, 118, $hidetop, $hidebottom);
		...
		if (empty($hidetop))
		{
			//linea que baja separa la cabecera con el subtitulo
			$pdf->line($this->marge_gauche-4, $tab_top+14, $this->page_largeur-$this->marge_haute+3, $tab_top+14);		// line prend une position y en 2eme param et 4eme param
			$pdf->line($this->page_largeur/2, $tab_top+14, $this->page_largeur/2, $tab_top+36);		// line prend une position y en 2eme param et 4eme param
			$pdf->line($this->marge_gauche-4, $tab_top+36, $this->page_largeur-$this->marge_haute+3, $tab_top+36);		// line prend une position y en 2eme param et 4eme param
			$pdf->line($this->marge_gauche-4, $tab_top+58, $this->page_largeur-$this->marge_haute+3, $tab_top+58);		// line prend une position y en 2eme param et 4eme param
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$title=$outputlangs->transnoentities("PAPELETA DE SALIDA");
		$pdf->MultiCell(102, 3, $title, '', 'C');
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities('Date')." : " . dol_print_date($arrayReporte['fecha'],'day'), '', 'R');
		//typelicence
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities('Type')." : " . $object->typelicence, '', 'R');
		//status
		$posy+=4;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities('Status')." : " . $object->getLibStatut(1), '', 'R');
		$posy+=1;
	...
	function converDateLatin($dateLatin){

		$arrayMeses = array('Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre');
		//var_dump($arrayMeses);exit;
		$dmy = explode("/",$dateLatin);

		$dateLatin = $dmy[0].'/'.$arrayMeses[$dmy[1]-1].'/'.$dmy[2];
		return $dateLatin;
	}
}
?>
