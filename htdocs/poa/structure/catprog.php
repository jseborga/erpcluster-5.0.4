...
$parameters=array();
$reshook=$hookmanager->executeHooks('doActions',$parameters,$objstructure,$action);    // Note that $action and $object may have been modified by some hooks
...
if (empty($reshook))
{
	if ($cancel) 
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/structure/catprog.php?id='.$id.($ido?'&ido='.$ido:''),1);
			header("Location: ".$urltogo);
			exit;
		}		
		if ($idr > 0 || ! empty($ref)) $ret = $objstructure->fetch($idr,$ref);
		$action='';
	}
	...
	if ($action == 'add')
	{
		...
		$error = 0;
		$objstructure->ref       	= GETPOST('ref');
		$objstructure->label     	= GETPOST('label');
		$objstructure->pseudonym 	= GETPOST('pseudonym');
		$objstructure->fk_father 	= GETPOST('fk_father')+0;
		$objstructure->fk_area   	= GETPOST('fk_area');
		$objstructure->fk_area_ej	= GETPOST('fk_area_ej')+0;
		$objstructure->fk_poa_objetive = GETPOST('fk_poa_objetive')+0;
		$objstructure->gestion= $_SESSION['period_year'];
		$objstructure->type 		= GETPOST('type');
		$objstructure->pos     	= 1;
		$objstructure->version 	= 1;
		$objstructure->fk_user_create=$user->id;
		$objstructure->fk_user_mod=$user->id;
		$objstructure->datec = dol_now();
		$objstructure->datem = dol_now();
		$objstructure->tms = dol_now();	

		//si tiene father
		if ($objtmp->fetch($objstructure->fk_father) && $objstructure->fk_father > 0)
		{
			$objstructure->pos = $objtmp->pos + 1;
			$objstructure->sigla = $objtmp->sigla.str_pad($objstructure->ref, $conf->global->POA_CODE_SIZE_STRUCTURE, "0", STR_PAD_LEFT);
			$objstructure->type = $objtmp->type;
		}
		else
		{
			$objstructure->pos = 1;
			$objstructure->sigla = str_pad($objstructure->ref, $conf->global->POA_CODE_SIZE_STRUCTURE, "0", STR_PAD_LEFT);
		}

		$objstructure->entity  = $conf->entity;
		$objstructure->statut  = 0;

		if (empty($objstructure->ref))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Ref")), null, 'errors');
		}
		if (empty($objstructure->label))
		{
			$error++;
			setEventMessages($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("Label")), null, 'errors');
		}

		if ($objstructure->fk_area <= 0)
		{
			$error++;
		...
		if (! $error)
		{
			$result=$objstructure->create($user);
			if ($result > 0)
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/poa/structure/catprog.php?id='.$id.($ido?'&ido='.$ido:''),1);
			...
			{
				// Creation KO
				if (! empty($objstructure->errors)) setEventMessages(null, $objstructure->errors, 'errors');
				else  setEventMessages($objstructure->error, null, 'errors');
				$action='create';
			}
		}
		else
	...
	if ($action == 'update')
	{
		...
		if (! $error)
		{
			$result=$objstructure->update($user);
			if ($result > 0)
			{
				$action='view';
			}
	...
	{
		$result=$objstructure->delete($user);
		if ($result > 0)
		{
			// Delete OK
...
if ($object->id > 0)
{
	...
	dol_fiche_head($head, 'catprog', $langs->trans("Planstrategic"), 0, 'poa_plan');

	print '<table class="border centpercent">'."\n";
	// print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td>'.$object->label.'</td></tr>';
	// 
	$sigla = str_pad($object->ref, $conf->global->POA_CODE_SIZE_PLAN_STRATEGIC, "0", STR_PAD_LEFT);
	print '<tr><td>'.$langs->trans("Fieldrefpoa").'</td><td>'.$sigla.'</td>';
	...
	if ($ido>0)
	{
		...
		print '<tr><td>'.$langs->trans("Fieldrefpoa").'</td><td>'.$sigla.$objobjetive->sigla.'</td>';
		print '<td>'.$langs->trans("Objetive").'</td><td>'.$objobjetive->label.'</td></tr>';
	...
	print '</div>'."\n";
	$nObjetive--; 
	include DOL_DOCUMENT_ROOT.'/poa/structure/tpl/structure.tpl.php';
