...
/**
 *   	\file       codprueba/licenceslog_list.php
 *		\ingroup    codprueba
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-08-22 11:49
 */
...
dol_include_once('/codprueba/class/licenceslog.class.php');

// Load traductions files requiredby by page
$langs->load("codprueba");
$langs->load("other");
...
$search_fk_licence=GETPOST('search_fk_licence','int');
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'codpruebalist';

// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array
$hookmanager->initHooks(array('codpruebalist'));
$extrafields = new ExtraFields($db);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('codprueba');
...
't.fk_licence'=>array('label'=>$langs->trans("Fieldfk_licence"), 'checked'=>1),
...
$search_status='';
    ...
    $permtoread = $user->rights->licenceslog->read;
    $permtodelete = $user->rights->licenceslog->delete;
    $uploaddir = $conf->licenceslog->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."licences_log as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."licences_log_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_licence) $sql.= natural_search("fk_licence",$search_fk_licence);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/licenceslog/card.php?id='.$id);
    exit;
}
...
if ($user->rights->codprueba->supprimer) $arrayofmassactions['delete']=$langs->trans("Delete");
...
// 
if (! empty($arrayfields['t.fk_licence']['checked'])) print_liste_field_titre($arrayfields['t.fk_licence']['label'],$_SERVER['PHP_SELF'],'t.fk_licence','',$params,'',$sortfield,$sortorder);
...
print '<tr class="liste_titre">';
// 
if (! empty($arrayfields['t.fk_licence']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_licence" value="'.$search_fk_licence.'" size="10"></td>';
...
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
    ...
    print $formfile->showdocuments('massfilesarea_codprueba','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
