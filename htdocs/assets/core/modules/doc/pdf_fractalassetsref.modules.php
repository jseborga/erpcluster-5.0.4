 ...
 */
class pdf_fractalassetsref extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
  ...
  // Minimum version of PHP required by module
   ...
   //*
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "assetsass";
		$this->description = $langs->trans('PDFCodeasset');
...
function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
{
			...
			if($this->type_page==1)
				...
				foreach ((array) $aReportdetasset AS $i => $lines)
				{
					...
					if($type=='D')
					{
						...
						$pdf->MultiCell($this->posxcodext-$this->posxcod-1, 5, $Codigo, 0, 'L',0);

						// code extendido

						$Codigo = $lines['Codigoext'];
						$pdf->SetXY($this->posxcodext, $curY);
						$pdf->MultiCell($this->posxlabel-$this->posxcodext-1, 5, $Codigo, 0, 'L',0);

						//etiqueta
						$Etiqueta = $lines['Etiqueta'];
						$pdf->SetXY($this->posxlabel, $curY);
						$pdf->MultiCell($this->posxfadq-$this->posxlabel-1, 5, dol_trunc($Etiqueta,41), 0, 'L',0);
						...
						$pdf->MultiCell($this->posxdep-$this->posxcos-1, 5, price(price2num($costo,'MT')), 0, 'R',0);
						/*
						//inmueble
						$Inmueble = $lines['Inmueble'];
						...
						*/
						...
						$pdf->SetXY($this->posxdep, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxresp-$this->posxdep-1, 5, dol_trunc($Departament,15), 0, 'L',0);

						//responsable
						$Responsable = $lines['Responsable'];
						$pdf->SetXY($this->posxresp, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxvaact-$this->posxresp-1, 5, dol_trunc($Responsable,20), 0, 'L',0);
						/*
						//condicion
						$Condicion = $lines['Condicion'];
						$pdf->SetXY($this->posxcond, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxvaact-$this->posxcond-1, 5, $Condicion, 0, 'L',0);
						*/

						//valor actualizado
						$ValorAct = $lines['Valoract'];
						$pdf->SetXY($this->posxvaact, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxdeacum-$this->posxvaact-1, 5,price(price2num($ValorAct,'MT')), 0, 'R',0);
						// depreciacion acumulada
						$Depreacum = $lines['Depreacum'];
						$pdf->SetXY($this->posxdeacum, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->posxsald-$this->posxdeacum-1, 5,price(price2num($Depreacum,'MT')), 0, 'R',0);
						//saldo
						$Balance = $lines['Balance'];
						$pdf->SetXY($this->posxsald, $curY);
						//$pdf->MultiCell($this->posxuni-$this->posxdesc-1, 3, $desc, 0, 'L',0);
						$pdf->MultiCell($this->page_largeur-$this->marge_haute-$this->posxsald-1, 5, price(price2num($Balance,'MT')), 0, 'R',0);
						/*
						// estado
						$Estado = $lines['Estado'];
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxcodext-$this->posxcod-1,2, $outputlangs->transnoentities("Code"),'','C');
		}
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxcodext-1, $tab_top+1);
			$pdf->MultiCell($this->posxlabel-$this->posxcodext-1,2, $outputlangs->transnoentities("Fieldref_ext"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxdep-$this->posxcos-1,2, $outputlangs->transnoentities("Fieldcoste"),'','C');
		}
		...
		*/
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxdep-1, $tab_top+1);
			$pdf->MultiCell($this->posxresp-$this->posxdep-1,2, $outputlangs->transnoentities("Departament"),'','C');
		}
		$pdf->line($this->posxdep-1, $tab_top, $this->posxdep-1, $tab_top + $tab_height);


		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxresp-1, $tab_top+1);
			$pdf->MultiCell($this->posxvaact-$this->posxresp-1,2, $outputlangs->transnoentities("Responsible"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxvaact-$this->posxcond-1,2, $outputlangs->transnoentities("Been"),'','C');
		}
		...
		*/
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxvaact-1, $tab_top+1);
			$pdf->MultiCell($this->posxdeacum-$this->posxvaact-1,2, $outputlangs->transnoentities("ValorAct"),'','C');
		}

		$pdf->line($this->posxvaact-1, $tab_top, $this->posxvaact-1, $tab_top + $tab_height);

		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxdeacum-1, $tab_top+1);
			$pdf->MultiCell($this->posxsald-$this->posxdeacum-1,2, $outputlangs->transnoentities("DepreciationAcum"),'','C');
		}
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxsald-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_haute-$this->posxsald-1,2, $outputlangs->transnoentities("Balance"),'','C');
		}

		$pdf->line($this->posxsald-1, $tab_top, $this->posxsald-1, $tab_top + $tab_height);
		/*
		if (empty($hidetop))
		{
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Reportassetscod");
