...
class pdf_sales extends ModelePDFContab
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->contab->dir_output)
		{
			...
			else
			{
				...
				$dir = $conf->contab->dir_output."/contab/".$period_year.'/sales';
				$file = $dir . "/sales.pdf";
			...
			if (file_exists($dir))
			{
				...
				foreach ( (array)$aReporte[1] as $j => $line)
				{
					...
					if($line['producto'] == -1){
						...
						$pdf->MultiCell($this->posxfin-$this->posxini-1, 1,$aGrupos[$idGr] , 0, 'L',1);
						$aPosY[] = $pdf->GetY();
						$idGr++;
					}else{

					$pdf->SetXY($this->posxini, $curY);
					$pdf->MultiCell($this->posxprod-$this->posxini-1, 1, $line['producto'], 0, 'L',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxprod, $curY);
					$pdf->MultiCell($this->posxunid-$this->posxprod-1, 1, $line['unidad'], 0, 'C',0);
					$aPosY[] = $pdf->GetY();
					$pdf->SetXY($this->posxunid, $curY);
					$pdf->MultiCell($this->posxvent-$this->posxunid-1, 1, price(price2num($line['total_venta'],'MT')), 0, 'R',0);
					$aPosY[] =$pdf->GetY();
					$pdf->SetXY($this->posxvent, $curY);
					$pdf->MultiCell($this->posxcost-$this->posxvent-1, 1, price(price2num($line['total_costo'],'MT')), 0, 'R',0);

					$pdf->SetXY($this->posxcost, $curY);
					$pdf->MultiCell($this->posxfin-$this->posxcost-1, 1, price(price2num($line['rendimiento'],'MT')), 0, 'R',0);
					}
					$nexY = max($aPosY);
					unset($aPosY);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxprod-$this->posxini-1,2, $outputlangs->transnoentities("Product"),'','C');
			$pdf->line($this->posxprod-1, $tab_top, $this->posxprod-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxprod-1, $tab_top+2);
			$pdf->MultiCell($this->posxunid-$this->posxprod-1,2, $outputlangs->transnoentities("Unit"),'','C');
			$pdf->line($this->posxunid-1, $tab_top, $this->posxunid-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxunid-1, $tab_top+2);
			$pdf->MultiCell($this->posxvent-$this->posxunid-1,2, $outputlangs->transnoentities("Totalsale"),'','C');
			$pdf->line($this->posxvent-1, $tab_top, $this->posxvent-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxvent-1, $tab_top+2);
			$pdf->MultiCell($this->posxcost-$this->posxvent-1,2, $outputlangs->transnoentities("Totalcost"),'','C');
			$pdf->line($this->posxcost-1, $tab_top, $this->posxcost-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxcost-1, $tab_top+2);
			$pdf->MultiCell($this->posxfin-$this->posxcost-1,2, $outputlangs->transnoentities("Yield"),'','C');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, html_entity_decode($outputlangs->transnoentities('Dateini'))." : " . dol_print_date($aReporte[2],'day'), '', 'R');
