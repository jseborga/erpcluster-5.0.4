...
$help_url='';
$title = $langs->trans('Budgettaskproduct');
...
$sql.= " WHERE 1 = 1";
$sql.= " AND t.fk_product_budget = ".$idr;
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				if ($key2 == 'ref')
				{
					...
					$obj->$key2 = $objBudgettaskproduct->getNomUrl();
				}
				if ($key2 == 'active')
				{
					$img = 'switch_off';
				...
				if ($key2 == 'status')
				{
					$objBudgettaskproduct->status = $obj->$key2;
					$obj->$key2 = $objBudgettaskproduct->getLibStatut(3);
				}
...
if ($user->rights->budget->tasksupp->write)
	print '<a class="butAction" href="' . dol_buildpath('/budget/productbudget/supplies.php?id='.$id.'&idr='.$idr.'&action=create',1).'">' . $langs->trans('Create') . '</a>';
print '</div>';
