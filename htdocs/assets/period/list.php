...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield="t.period_year,t.period_month"; // Set here default search field
...
if (!$user->rights->assets->per->read) accessforbidden();
...
if (empty($reshook))
{
	...
	if ($action=='actaf' && $user->rights->assets->per->actaf)
	{
	...
	if ($action=='actper' && $user->rights->assets->per->act)
	{
	...
	if ($action == 'add')
	{
		...
		if (! $error)
		{
			...
			{
				// Creation OK
				$urltogo=$backtopage?$backtopage:dol_buildpath('/assets/period/list.php',1);
				header("Location: ".$urltogo);
				exit;
			}
			{
...
//	$sql.= " AND t.period_year = ".($_SESSION['period_year']?$_SESSION['period_year']:date('Y'));

if ($search_entity) $sql.= natural_search("entity",$search_entity);
if ($search_period_month) $sql.= natural_search("period_month",$search_period_month);
...
if ($search_status_af) $sql.= natural_search("status_af",$search_status_af);
if ($search_status_al) $sql.= natural_search("status_al",$search_status_al);
...
if ($resql)
{
	...
	if ($search_period_month != '') $params.= '&amp;search_period_month='.urlencode($search_period_month);
	...
	if ($search_status_af != '') $params.= '&amp;search_status_af='.urlencode($search_status_af);
	if ($search_status_al != '') $params.= '&amp;search_status_al='.urlencode($search_status_al);
	...
	//
    	...
    	if (! empty($arrayfields['t.period_month']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_period_month" value="'.$search_period_month.'" size="10"></td>';
    	...
    	if (! empty($arrayfields['t.status_af']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_status_af" value="'.$search_status_af.'" size="10"></td>';
    	if (! empty($arrayfields['t.status_al']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_status_al" value="'.$search_status_al.'" size="10"></td>';
    ...
    if ($action == 'create')
    {
    	...
    	if (! empty($arrayfields['t.period_year']['checked']))
    	{
    		$yearmin = date('Y')-10;
    		$yearmax = date('Y')+5;
    		//$yearmin = ($_SESSION['period_year']?$_SESSION['period_year']:date('Y'));
    		//$yearmax = ($_SESSION['period_year']?$_SESSION['period_year']:date('Y'));

    		print '<td>'.'<input type="number" name="period_year" min="'.$yearmin.'" max="'.$yearmax.'" value="'.(GETPOST('period_year')?GETPOST('period_year'):($_SESSION['period_year']?$_SESSION['period_year']:date('Y'))).'" required>'.'</td>';
    		if (! $i) $totalarray['nbfield']++;
    	}
    ...
    while ($i < min($num, $limit))
    {
    	...
    	if ($obj)
    	{
    		...
    		if (! empty($arrayfields['t.statut']['checked']))
    		{
    			...
    			if ($obj->statut == 1)
    				$switch = 'switch_on';
    			if ($user->rights->almacen->per->act)
    			{
    		...
    		if (! empty($arrayfields['t.status_af']['checked']))
    		{
    			...
    			if ($obj->status_af == 1)
    				$switchaf = 'switch_on';
    			if ($user->rights->assets->per->actaf)
    			{
	...
	if (empty($reshook))
	{
		if ($user->rights->almacen->per->write)
		{
