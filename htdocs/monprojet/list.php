...
$projectsListId = $projectstatic->getMonProjectsAuthorizedForUser($user,($mine?$mine:($user->rights->projet->all->lire?2:($user->societe_id?2:0))),1,$socid);
...
// if ($mine ||
//     ! $user->rights->projet->all->lire ||
//     $user->array_options['options_view_projet'])
...
if (!$user->array_options['options_view_projet'])
{
 ...
 // else
 //   if ($socid) $sql.= "  OR (p.fk_soc = ".$socid.")";
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans("CPI"),'',"","",$param,"");
	print_liste_field_titre($langs->trans("SPI"),'',"","",$param,"");
	print_liste_field_titre($langs->trans("CSI"),'',"","",$param,"");
	...
	while ($i < $num)
	{
		...
		$userAccess = $projectstatic->restrictedProjectAreaadd($user);
		...
		if ($userAccess >= 0)
		{
			...
			$tasksarray=$taskadd->getTasksArray(0, 0, $objp->projectid, $socid, 0);
			$tasksrole = 0;
			$aTotalVal = array();
			if (count($tasksarray) > 0)
			{
				$j=0; $level=0;
				$aTotalVal=monprojectLinesares($j, 0, $tasksarray, $level, true, 0, $tasksrole, $objp->projectid, 1,$aTotalVal);
			}

		//indice de desempeno
			$lCalc = $aTotalVal['lCalc'];
			$CPI = 0;
			if ($aTotalVal['AC']>0)
				$CPI = price2num($aTotalVal['EV']/$aTotalVal['AC'],'MT');
		  //BCWP/ACWP
			$SPI = 0;
			if ($aTotalVal['PV']>0)
				$SPI = price2num($aTotalVal['EV']/$aTotalVal['PV'],'MT');
		   //BCWP/BCWS
		//calcular indicador relacion costo y cronograma
			$CSI = price2num($CPI * $SPI,'MT');
