...
dol_include_once('/orgman/class/tablesdef.class.php');
dol_include_once('/orgman/lib/orgman.lib.php');
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		...
		$object->with_limit=GETPOST('with_limit','int');
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/tables/list.php',1));
			exit;
		}
		else
		{
	...
	// Action to delete
	if ($action == 'confirm_validate' || $action == 'confirm_novalidate')
	{
...
llxHeader('',$langs->trans('Tabledef'),'');
...
// ORIGIN
$aNametables = $db->DDLListTables('demo');
$label = (GETPOST('origin')?GETPOST('label'):$objectdet->label);
$optionslabel = '';
...
$optionslabel = '<option value="">'.$langs->trans('Select').'</option>';
foreach ($aNametables AS $j => $line)
{
	$selected = '';
	if ($label == $line) $selected = ' selected';
	$optionslabel.= '<option value="'.$line.'" '.$selected.'>'.$line.'</option>';
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("TablesdefNew"));
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldref").'</td><td><input class="flat minwidth300" type="text" name="ref" value="'.GETPOST('ref').'" placeholder="'.$langs->trans('Ref').'" maxlength="30"></td></tr>';
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldlabel").'</td><td><input class="flat minwidth300" type="text" name="label" value="'.GETPOST('label').'" placeholder="'.$langs->trans('Label').'" maxlength="200"></td></tr>';

	/*

	print '<tr>';
	print '<td width="20%" class="fieldrequired">'.$langs->trans("Fieldlabel").'</td><td>';
	print '<select  name="label" value="'.GETPOST('label').'" >'.$optionslabel.'</select>';;
	print '</td>';
	print '</tr>';
	...
	*/
	...
	print '<tr>';
	print '<td class="fieldrequired">'.$langs->trans("Withlimit").'</td><td>';
	print $form->selectyesno('with_limit',(GETPOST('with_limit')?GETPOST('with_limit'):$object->with_limit),1);
	print '</td>';
	print '</tr>';

	print '<tr>';
	print '<td class="fieldrequired">'.$langs->trans("Fieldactive").'</td><td>';
...
{
	print load_fiche_titre($langs->trans("Tablesdefedit"));
	...
	print '<tr><td class="fieldrequired">'.$langs->trans("Fieldref").'</td><td><input class="flat minwidth300" type="text" name="ref" value="'.$object->ref.'" placeholder="'.$langs->trans('Ref').'" maxlength="30"></td></tr>';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	print load_fiche_titre($langs->trans("Tablesdef"));

	$head = tables_prepare_head($object);
	$titre=$langs->trans('Tablesdef');

	dol_fiche_head($head, 'card', $titre, 0, 'bill');

	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteTablesdef'), $langs->trans('ConfirmDeleteTablesdef'), 'confirm_delete', '', 1, 1);
		print $formconfirm;
	}
	if ($action == 'validate' || $action == 'novalidate') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, ($action=='validate'?$langs->trans('ValidateTablesdef'):$langs->trans('NovalidateTablesdef')), ($action=='validate'?$langs->trans('ConfirmValidatetablesdef'):$langs->trans('ConfirmNovalidatetablesdef')), ($action=='validate'?'confirm_validate':'confirm_novalidate'), '', 1, 2);
		print $formconfirm;
	}
	...
	if (empty($reshook))
	{
		...
		if ($object->status == 0 && $user->rights->orgman->tab->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>'."\n";
		}
		if ($object->status == 1 && $user->rights->orgman->tab->val)
		{
		...
		if ($object->status == 0 && $user->rights->orgman->tab->val)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=validate">'.$langs->trans("Validate").'</a></div>'."\n";
		}

		if ($object->status==0 && $user->rights->orgman->tab->del)
		{
