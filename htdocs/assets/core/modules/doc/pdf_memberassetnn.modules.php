...
class pdf_memberassetnn extends ModelePDFAssets
{
	var $db;
	var $name;
	var $um;
   ...
   // Minimum version of PHP required by module
				...
				foreach ( (array)$aReportdetasset as $j => $line)
				{
					...
					$pdf->SetFont('','', $default_font_size-1);
						...
						if(empty($line['Codigo'])){
							...
							$pdf->MultiCell($this->posxfead-$this->posxeti-1, 1, $line['Etiqueta'], 0, 'L',0);
						}else{
							//nro
							$pdf->SetXY($this->posxnro, $curY);
							$pdf->MultiCell($this->posxcod-$this->posxnro-1, 1, $nro, 0, 'C',0);
							//cod
							$pdf->SetXY($this->posxcod, $curY);
							$pdf->MultiCell($this->posxeti-$this->posxcod-1, 1, $line['Codigo'], 0, 'L',0);
							//eti
							$pdf->SetXY($this->posxeti, $curY);
							$pdf->MultiCell($this->posxfead-$this->posxeti-1, 1, dol_trunc($line['Etiqueta'],70), 0, 'L',0);
							$nexYaux = $pdf->GetY();
							//fead
							$pdf->SetXY($this->posxfead, $curY);
							$pdf->MultiCell($this->posxcos-$this->posxfead-1, 1, dol_print_date($line['FechaAdquisicion'],"day"), 0, 'L',0);
							//inm
							$pdf->SetXY($this->posxcos, $curY);
							$pdf->MultiCell($this->posxres-$this->posxcos-1, 1, price($line['costo']), 0, 'R',0);
							//location
							//$pdf->SetXY($this->posxinmu, $curY);
							//$pdf->MultiCell($this->posxloc-$this->posxinmu-1, 1,$line['Inmueble'], 0, 'L',0);

							//$pdf->SetXY($this->posxloc, $curY);
							//$pdf->MultiCell($this->posxres-$this->posxloc-1, 1, $line['location'], 0, 'C',0);
							...
							$pdf->MultiCell($this->posxcon-$this->posxres-1, 1, dol_trunc($line['Responsable'],25), 0, 'L',0);

							$pdf->SetXY($this->posxcon, $curY);
							$pdf->MultiCell($this->posxfin-$this->posxcon-1, 1, $line['Condicion'], 0, 'C',0);

							$pdf->SetXY($this->posxest, $curY);
							$pdf->MultiCell($this->page_largeur-$this->marge_haute-$this->posxest-1, 1, $line['Estado'], 0, 'C',0);
							//$nexYend = $pdf->GetY();
							$nro++;
						}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		*/
		...
		if (empty($hidetop)){
			...
			$pdf->MultiCell($this->posxeti-$this->posxcod-1,2, $outputlangs->transnoentities("Code"),'','C');
			$pdf->line($this->posxcod-1, $tab_top, $this->posxcod-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxeti-1, $tab_top+3);
			$pdf->MultiCell($this->posxfead-$this->posxeti-1,2, $outputlangs->transnoentities("Label"),'','C');
			$pdf->line($this->posxeti-1, $tab_top, $this->posxeti-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxfead-1, $tab_top+2);
			$pdf->MultiCell($this->posxcos-$this->posxfead-1,2, $outputlangs->transnoentities("Fielddate_adq"),'','C');
			$pdf->line($this->posxfead-1, $tab_top, $this->posxfead-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxcos-1, $tab_top+2);
			$pdf->MultiCell($this->posxres-$this->posxcos-1,2, $outputlangs->transnoentities("Coste"),'','C');
			$pdf->line($this->posxcos-1, $tab_top, $this->posxcos-1, $tab_top + $tab_height);

			//$pdf->SetXY($this->posxinmu-1, $tab_top+2);
			//$pdf->MultiCell($this->posxloca-$this->posxinmu-1,2, $outputlangs->transnoentities("Property"),'','C');
			//$pdf->line($this->posxinmu-1, $tab_top, $this->posxinmu-1, $tab_top + $tab_height);

			//$pdf->SetXY($this->posxloca-1, $tab_top+2);
			//$pdf->MultiCell($this->posxres-$this->posxloca-1,2, $outputlangs->transnoentities("Location"),'','C');
			//$pdf->line($this->posxloca-1, $tab_top, $this->posxloca-1, $tab_top + $tab_height);

			//$pdf->SetXY($this->posxloca-1, $tab_top+2);
			//$pdf->MultiCell($this->posxfeas-$this->posxloca-1,2, $outputlangs->transnoentities("Fieldassignment_date"),'','C');
			//$pdf->line($this->posxfeas-1, $tab_top, $this->posxfeas-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxres-1, $tab_top+2);
			$pdf->MultiCell($this->posxcon-$this->posxres-1,2, $outputlangs->transnoentities("Responsible"),'','C');
			$pdf->line($this->posxres-1, $tab_top, $this->posxres-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxcon-1, $tab_top+2);
			$pdf->MultiCell($this->posxest-$this->posxcon-1,2, $outputlangs->transnoentities("Condition"),'','C');
			$pdf->line($this->posxcon-1, $tab_top, $this->posxcon-1, $tab_top + $tab_height);

			$pdf->SetXY($this->posxest-1, $tab_top+2);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager,$typeTitulo)
	{
		...
		$pdf->SetFont('','B',$default_font_size+1);
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Dateini")." : ". dol_print_date($aExtras['date_ini'],'daytext'), '', 'R');

			//Fecha de elaborarion
			$posy+=7;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Datefin")." : " . dol_print_date($aExtras['date_fin'],'daytext'), '', 'R');
