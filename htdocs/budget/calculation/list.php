...
/**
 *   	\file       budget/parametercalculation_list.php
 *		\ingroup    budget
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2018-05-15 10:27
 */
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'budgetparametercalculationlist';
...
$help_url='';
$title = $langs->trans('Parametercalculation');

// Put here content of your page

// Example : Adding jquery code
print '<script type="text/javascript" language="javascript">
jQuery(document).ready(function() { function init_myfunc() { jQuery("#myid").removeAttr(\'disabled\'); jQuery("#myid").attr(\'disabled\',\'disabled\'); } init_myfunc(); jQuery("#mybutton").click(function() { init_myfunc(); 	}); }); </script>';
...
$sql.= " FROM ".MAIN_DB_PREFIX."parameter_calculation as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."parameter_calculation_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/parametercalculation/card.php?id='.$id);
	exit;
}
	...
	if ($obj)
	{
		...
		foreach ($arrayfields as $key => $value) {
			if (!empty($arrayfields[$key]['checked'])) {
				...
				$aKey = explode('.',$key);
				$key2 = $aKey[1];
				if ($key2 == 'ref' || $key2 == 'code')
				{
					$object->id = $obj->rowid;
					if ($key2=='ref') $object->ref = $obj->ref;
					if ($key2=='code') $object->ref = $obj->code;
