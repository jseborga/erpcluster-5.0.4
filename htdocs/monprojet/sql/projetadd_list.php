...
/**
 *   	\file       monprojet/projetadd_list.php
 *		\ingroup    monprojet
 *		\brief      This file is an example of a php page
 *					Initialy built by build_class_from_table on 2017-02-03 16:19
 */
...
dol_include_once('/monprojet/class/projetadd.class.php');
...
$search_programmed=GETPOST('search_programmed','alpha');
$search_fk_contracting=GETPOST('search_fk_contracting','int');
$search_fk_supervising=GETPOST('search_fk_supervising','int');
$search_use_resource=GETPOST('search_use_resource','alpha');
...
$hookmanager->initHooks(array('projetaddlist'));
...
't.programmed'=>array('label'=>$langs->trans("Fieldprogrammed"), 'checked'=>1),
't.fk_contracting'=>array('label'=>$langs->trans("Fieldfk_contracting"), 'checked'=>1),
't.fk_supervising'=>array('label'=>$langs->trans("Fieldfk_supervising"), 'checked'=>1),
't.use_resource'=>array('label'=>$langs->trans("Fielduse_resource"), 'checked'=>1),
...
$search_fk_entrepot='';
$search_programmed='';
$search_fk_contracting='';
$search_fk_supervising='';
$search_use_resource='';
$search_origin='';
$search_originid='';
...
$sql.= " t.rowid,";
		...
		$sql .= " t.fk_entrepot,";
		$sql .= " t.programmed,";
		$sql .= " t.fk_contracting,";
		$sql .= " t.fk_supervising,";
		$sql .= " t.use_resource,";
		$sql .= " t.origin,";
		$sql .= " t.originid,";
...
$sql.= " FROM ".MAIN_DB_PREFIX."projet_add as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."projet_add_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_programmed) $sql.= natural_search("programmed",$search_programmed);
if ($search_fk_contracting) $sql.= natural_search("fk_contracting",$search_fk_contracting);
if ($search_fk_supervising) $sql.= natural_search("fk_supervising",$search_fk_supervising);
if ($search_use_resource) $sql.= natural_search("use_resource",$search_use_resource);
...
if ($search_programmed != '') $params.= '&amp;search_programmed='.urlencode($search_programmed);
if ($search_fk_contracting != '') $params.= '&amp;search_fk_contracting='.urlencode($search_fk_contracting);
if ($search_fk_supervising != '') $params.= '&amp;search_fk_supervising='.urlencode($search_fk_supervising);
if ($search_use_resource != '') $params.= '&amp;search_use_resource='.urlencode($search_use_resource);
if ($search_origin != '') $params.= '&amp;search_origin='.urlencode($search_origin);
if ($search_originid != '') $params.= '&amp;search_originid='.urlencode($search_originid);
...
if (! empty($arrayfields['t.programmed']['checked'])) print_liste_field_titre($arrayfields['t.programmed']['label'],$_SERVER['PHP_SELF'],'t.programmed','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_contracting']['checked'])) print_liste_field_titre($arrayfields['t.fk_contracting']['label'],$_SERVER['PHP_SELF'],'t.fk_contracting','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_supervising']['checked'])) print_liste_field_titre($arrayfields['t.fk_supervising']['label'],$_SERVER['PHP_SELF'],'t.fk_supervising','',$params,'',$sortfield,$sortorder);
if (! empty($arrayfields['t.use_resource']['checked'])) print_liste_field_titre($arrayfields['t.use_resource']['label'],$_SERVER['PHP_SELF'],'t.use_resource','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.programmed']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_programmed" value="'.$search_programmed.'" size="10"></td>';
if (! empty($arrayfields['t.fk_contracting']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_contracting" value="'.$search_fk_contracting.'" size="10"></td>';
if (! empty($arrayfields['t.fk_supervising']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_fk_supervising" value="'.$search_fk_supervising.'" size="10"></td>';
if (! empty($arrayfields['t.use_resource']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_use_resource" value="'.$search_use_resource.'" size="10"></td>';
