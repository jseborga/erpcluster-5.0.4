...
/**
 *      \file       htdocs/poa/appoint/liste.php
 *      \ingroup    Appoints
 *      \brief      Page liste des appoints
 */
...
if (!$user->rights->poa->appoint->leer)
  accessforbidden();
...
if (! $sortfield) $sortfield="t.code_appoint";
...
if ($result)
{
    ...
    llxHeader("",$langs->trans("Liste appoints"),$help_url);
    
    print_barre_liste($langs->trans("Liste appoints"), $page, "liste.php", "", $sortfield, $sortorder,'',$num);
    
    print '<table class="noborder" width="100%">';
    
    print "<tr class=\"liste_titre\">";
    print_liste_field_titre($langs->trans("Ref"),"liste.php", "p.ref","","","");
    print_liste_field_titre($langs->trans("Type"),"liste.php", "t.code_appoint","","","");
    print_liste_field_titre($langs->trans("Contrat"),"", "","","","");
    print_liste_field_titre($langs->trans("Dateappoint"),"liste.php", "t.date_appoint","","","");   
    print_liste_field_titre($langs->trans("Name"),"liste.php", "t.issuer","","","");
    ...
    if ($num) {
      	...
      	while ($i < min($num,$limit))
		{
	  		...
	  		print "<tr $bc[$var]>";
		  	print '<td><a href="fiche.php?id='.$obj->id.'" title="'.$obj->ref.'">'.img_object($langs->trans("Appoing"),'user').' '.$obj->ref.'</a></td>';
	  		print '<td>'.select_code_appoint($obj->code_appoint,'code_appoint','',0,1,'code').'</td>';
	  		if ($res > 0 && $objcon->id == $obj->fk_contrat)
	  			print '<td>'.$objcon->array_options['options_ref_contrato'].'</td>';
	  		else
	  			print '<td>&nbsp;</td>';
	  		print '<td>'.dol_print_date($obj->date_appoint,'day').'</td>';
