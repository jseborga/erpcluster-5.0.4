...
/**
 *   	\file       budget/budgettaskproduction_list.php
 *		\ingroup    budget
...
$contextpage=GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'budgetlist';
...
't.fk_budget_task'=>array('label'=>$langs->trans("Fieldfk_budget_task"), 'align'=>'align="left"', 'checked'=>1),
't.fk_variable'=>array('label'=>$langs->trans("Fieldfk_variable"), 'align'=>'align="left"', 'checked'=>1),
't.fk_budget_task_product'=>array('label'=>$langs->trans("Fieldfk_budget_task_product"), 'align'=>'align="left"', 'checked'=>1),
...
$object=new Budgettaskproduction($db);
...
$search_status='';
	...
	$permtoread = $user->rights->budgettaskproduction->read;
	$permtodelete = $user->rights->budgettaskproduction->delete;
	$uploaddir = $conf->budgettaskproduction->dir_output;
...
$sql.= " FROM ".MAIN_DB_PREFIX."budget_task_production as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."budget_task_production_extrafields as ef on (t.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_budget_task) $sql.= natural_search("fk_budget_task",$search_fk_budget_task);
if ($search_fk_variable) $sql.= natural_search("fk_variable",$search_fk_variable);
if ($search_fk_budget_task_product) $sql.= natural_search("fk_budget_task_product",$search_fk_budget_task_product);
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/budgettaskproduction/card.php?id='.$id);
	exit;
}
...
if (! empty($arrayfields['t.fk_variable']['checked'])) print_liste_field_titre($arrayfields['t.fk_variable']['label'],$_SERVER['PHP_SELF'],'t.fk_variable','',$params,$arrayfields['t.fk_variable']['align'],$sortfield,$sortorder);
if (! empty($arrayfields['t.fk_budget_task_product']['checked'])) print_liste_field_titre($arrayfields['t.fk_budget_task_product']['label'],$_SERVER['PHP_SELF'],'t.fk_budget_task_product','',$params,$arrayfields['t.fk_budget_task_product']['align'],$sortfield,$sortorder);
...
if (! empty($arrayfields['t.fk_variable']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_variable']['align'].'><input type="text" class="flat" name="search_fk_variable" value="'.$search_fk_variable.'" size="10"></td>';
if (! empty($arrayfields['t.fk_budget_task_product']['checked'])) print '<td class="liste_titre" '.$arrayfields['t.fk_budget_task_product']['align'].'><input type="text" class="flat" name="search_fk_budget_task_product" value="'.$search_fk_budget_task_product.'" size="10"></td>';
...
if ($massaction == 'builddoc' || $action == 'remove_file' || $show_files)
{
	...
	print $formfile->showdocuments('massfilesarea_budget','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'');
}
else
{
