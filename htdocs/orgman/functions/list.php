...
dol_include_once('/orgman/class/cfunctions.class.php');
...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield="t.sigla"; // Set here default search field
...
$arrayfields=array(
	...
	't.sigla'=>array('label'=>$langs->trans("Fieldcode"), 'checked'=>1),
...
$object=new Cfunctions($db);
if (($id > 0 || ! empty($ref)) && $action != 'add')
{
	$result=$object->fetch($id,$ref);
	if ($result < 0) dol_print_error($db);
}
$objecttmp = new Cfunctions($db);
$objUser = new User($db);
...
if (empty($reshook))
{
	if ($cancel)
	{
		if ($action != 'addlink')
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/partida/list.php',1);
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/orgman/functions/list.php',1);
			header("Location: ".$urltogo);
			exit;
		}
		...
		$object->sigla=GETPOST('ref','alpha');
		if (!empty($code_father)) $object->sigla=$code_father.'.'.GETPOST('ref','alpha');
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/orgman/functions/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Clasificador de Gastos Por Finalidad y Función');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_functions as t";
...
if ($search_code) $sql.= natural_search("sigla",$search_code);
...
if ($action == 'delete') {
	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteClassificationofExpendituresbyPurposeandFunction'), $langs->trans('DeleteClassificationofExpendituresbyPurposeandFunction'), 'confirm_delete', '', 0, 2);
	print $formconfirm;
}
...
if ($action != 'create' && $action != 'edit')
{
	...
	if (! empty($arrayfields['t.sigla']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_code" value="'.$search_code.'" size="10"></td>';
...
$objecttmp->fetchAll('ASC', 'sigla', 0, 0, array('entity'=>$conf->entity), 'AND',$filterstatic);
$options = '<option value="">'.$langs->trans('Selectfunctions').'</option>';
$indent = '';
$idFather = 0;
$aPartida = array();
...
while ($i < min($num, $limit))
{
	...
	if ($obj)
	{
		...
		if ($action == 'edit' && $id == $obj->rowid)
		{
			...
			if (! empty($arrayfields['t.sigla']['checked']))
			{
				print '<td nowrap>'.$obj->code_father.'.'.'<input type="text" name="ref" value="'.$obj->ref.'" size="3">'.'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked']))
			{
			...
			if (! empty($arrayfields['t.code_father']['checked']))
			{
				...
				foreach ($linesfather AS $j => $line)
				{
					...
					$selected = '';
					if ($action == 'edit' && $obj->code_father == $line->sigla) $selected = ' selected ';
					$options.= '<option value="'.$line->sigla.'" '.$selected.' title="'.$line->label.'">'.$indent.$line->sigla.' - '.dol_trunc($line->label,40).'</option>';
				}
		...
		else
		{
			if (! empty($arrayfields['t.sigla']['checked']))
			{
				...
				print '<td>'.$object->getNomUrl(1,'', 0, 24, '','edit').'</td>';
				if (! $i) $totalarray['nbfield']++;
			}
			if (! empty($arrayfields['t.label']['checked']))
			{
		...
		// Action column
			...
			if ($action != 'create' && $action != 'createsub')
			{
				if ($user->rights->orgman->fun->write)
					print '<a href="'.$_SERVER['PHP_SELF'].'?code_father='.$obj->sigla.'&action=create">'.img_picto($langs->trans('New(ao)').' '.$objCname->label,'rightarrow').'</a>';

				if ($user->rights->orgman->fun->del)
				{
					$filter = " AND t.code_father = '".$obj->sigla."'";
					$filter.= " AND t.entity = ".$conf->entity;
					$restmp = $objecttmp->fetchAll('','',0,0,array(1=>1),'AND',$filter);
					if (empty($restmp))
