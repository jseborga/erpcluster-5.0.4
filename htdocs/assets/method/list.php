...
/**
 *   	\file       assets/cassetsbeen_list.php
 *		\ingroup    assets
...
$hookmanager->initHooks(array('cassetsbeenlist'));
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/assets/method/list.php',1);
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/assets/method/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('Depreciationmethod');
...
$sql.= " FROM ".MAIN_DB_PREFIX."c_assets_method as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_assets_been_extrafields as ef on (u.rowid = ef.fk_object)";
...
if ($resql)
{
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('Deletebeen'), $langs->trans('ConfirmDeletebeen'), 'confirm_delete', '', 0, 1);
		print $formconfirm;
	}
